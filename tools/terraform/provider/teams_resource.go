...
// This file implements the "mdmlabdm_teams" Terraform resource.
...
type teamsResource struct {
	client *mdmlabdm_client.MDMlabDMClient
}
...
func (r *teamsResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	client, ok := req.ProviderData.(*mdmlabdm_client.MDMlabDMClient)

	if !ok {
		resp.Diagnostics.AddError(
			"Unexpected Data Source Configure Type",
			fmt.Sprintf("Expected *MDMlabDMClient, got: %T. Please report this issue to the provider developers.", req.ProviderData),
		)
...
// to the MDMlabDM API and convert it to the Terraform state.
func teamModelToTF(ctx context.Context, tm *mdmlabdm_client.TeamGetResponse, tf *TeamModel) error {
...
func (r *teamsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(diag.NewErrorDiagnostic(
			"failed to convert mdmlab api return to TF structs",
			fmt.Sprintf("failed to convert mdmlab api return to TF structs: %s", err)))
...
// Read queries the MDMlabDM API and sets the TF state to what it finds.
...
// differ. It will then update the team in MDMlabDM to match the plan,
...
func (r *teamsResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	var upTeam *mdmlabdm_client.TeamGetResponse
	var err error
...
// as input, query the MDMlabDM API to get the ID, and then set the ID.
