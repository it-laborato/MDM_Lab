...
func createApp(reader io.Reader, stdout io.Writer, stderr io.Writer, exitErrHandler cli.ExitErrHandlerFunc) *cli.App {
	app := cli.NewApp()
	app.Name = "desktop"
	app.Usage = "Tool to generate the MDMlab Desktop application"
	...
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			Name:    "version",
			Usage:   "Version of the MDMlab Desktop application",
...
func macos() *cli.Command {
	return &cli.Command{
		Name:        "macos",
		Usage:       "Creates the MDMlab Desktop Application for macOS",
		Description: "Builds and signs the MDMlab Desktop .app bundle for macOS",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:    "authority",
...
func createMacOSApp(version, authority string, notarize bool) error {
	const (
		appDir           = "MDMlab Desktop.app"
		bundleIdentifier = "com.mdmlabdm.desktop"
...
`
	...
	if runtime.GOOS != "darwin" {
		return errors.New(`the "MDMlab Desktop" macOS app can only be created from macOS`)
	}
	...
	zlog.Info().Str("command", buildExec.String()).Msg("Build mdmlab-desktop executable amd64")
	...
	zlog.Info().Str("command", buildExec.String()).Msg("Build mdmlab-desktop executable arm64")
	if err := buildExec.Run(); err != nil {
	...
	if authority != "" {
		...
		zlog.Info().Str("command", codeSign.String()).Msg("Sign MDMlab Desktop.app")
	...
	if notarize {
		...
		if err := packaging.Notarize(notarizationZip, "com.mdmlabdm.desktop"); err != nil {
			return err
		}
