...
// as Orbit would do following reception of a MDMlab server notification of
// pending script(s) to execute.
//
// It allows to run such scripts without having to build and deploy orbit on
// the target host and without having to enroll that host in mdmlab and have the
// mdmlab server send script execution requests to it.
//
