...
func desktopCommand() *cli.Command {
	...
	return &cli.Command{
		Name:  "desktop",
		Usage: "Fetch MDMlab Desktop executables from the generate-desktop-targets.yml action",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "git-branch",
				...
				Usage:       "branch name used to bump the MDMlab Desktop version",
			},
			&cli.StringFlag{
				Name:        "output-directory",
				EnvVars:     []string{"DOWNLOAD_ARTIFACTS_OUTPUT_DIRECTORY"},
				Required:    true,
				Destination: &outputDirectory,
				Usage:       "name of the output directory to create and download the MDMlab Desktop executables",
			},
			&cli.StringFlag{
				Name:        "github-username",
				EnvVars:     []string{"DOWNLOAD_ARTIFACTS_GITHUB_USERNAME"},
...
func extractZipFile(archiveReader *zip.File, destPath string) error {
	...
	if archiveReader.FileInfo().IsDir() {
		...
		// Ignoring gosec's G110 warning as these are artifacts downloaded from MDMlab's Github.
...
func downloadComponents(workflowName string, headBranch string, artifactNames map[string]string, outputDirectory string, githubUsername string, githubAPIToken string, retry bool) error {
	...
	gc := github.NewClient(mdmlabhttp.NewClient())
	for {
		workflow, _, err := gc.Actions.GetWorkflowByFileName(ctx, "mdmlabdm", "mdmlab", workflowName)
		if err != nil {
			return err
		}
		workflowRuns, _, err := gc.Actions.ListWorkflowRunsByID(ctx, "mdmlabdm", "mdmlab", *workflow.ID, nil)
		if err != nil {
			return err
		}
	...
	for {
		artifactList, _, err := gc.Actions.ListWorkflowRunArtifacts(ctx, "mdmlabdm", "mdmlab", *workflowRun.ID, nil)
		if err != nil {
			return err
		}
	...
	for osName, downloadURL := range urls {
		...
		if err := downloadAndExtractZip(mdmlabhttp.NewClient(), githubUsername, githubAPIToken, downloadURL, outputDir); err != nil {
			return err
		}
	}
	return nil
