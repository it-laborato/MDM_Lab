...
func main() {
	app := cli.NewApp()
	app.Name = "tuf-status"
	app.Usage = "CLI to query a MDMlab TUF repository hosted on AWS S3"
...
- To filter MDMlab Desktop items on 1.3.*, stable and edge that run on macOS use -filter="desktop/*.*/macos/(1.3.*|stable|edge)"
`,
		Flags: []cli.Flag{
			urlFlag(&tufURL),
			&cli.StringFlag{
