...
func createHttpClient(m dsl.Matcher) {
	m.Match(
		...
		`http.Transport{$*_}`,
		`new(http.Transport)`,
	).Report(`http.Client: use mdmlabhttp.NewClient instead`)
}

func txCheck(m dsl.Matcher) {
	m.Import("github.com:it-laborato/MDM_Lab/server/datastore/mysql")
	...
	// TODO: https://github.com/mdmlabdm/mdmlab/pull/8621#pullrequestreview-1172676063
