...
func main() {
	...
	softwareName := flag.String("software_name", "", "Name of the software as ingested by MDMlab")
	softwareVersion := flag.String("software_version", "", "Version of the software as ingested by MDMlab")
	softwareSource := flag.String("software_source", "", "Source for this software (e.g. 'apps' for macOS applications)")
	softwareBundleIdentifier := flag.String("software_bundle_identifier", "", "Bundle identifier of the software as ingested by MDMlab (for macOS apps only)")

	// Fields to fetch software (and the found vulnerabilities) from a MDMlab instance.
	// This mode of operation then compares the CVEs found by the MDMlab instance with the CVEs found by this new run of vulnerability processing.
	...
	var softwareCPEs []mdmlab.SoftwareCPE
	...
	ds.InsertSoftwareVulnerabilityFunc = func(ctx context.Context, vuln mdmlab.SoftwareVulnerability, source mdmlab.VulnerabilitySource) (bool, error) {
		return true, nil
	}
	...
	ds.DeleteOutOfDateOSVulnerabilitiesFunc = func(ctx context.Context, source mdmlab.VulnerabilitySource, duration time.Duration) error {
		return nil
	}
...
func getSoftwareName(software []mdmlab.Software, softwareID uint) string {
	for _, s := range software {
		if s.ID == softwareID {
...
func (s *softwareIterator) Value() (*mdmlab.Software, error) {
	ss := &s.software[s.i]
	s.i += 1
	return ss, nil
}
...
func getSoftwareFromURL(url, apiToken string, debug bool) []mdmlab.Software {
	...
	var filteredSoftware []mdmlab.Software
	for _, s := range software {
