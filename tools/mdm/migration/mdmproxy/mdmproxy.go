...
type mdmProxy struct {
	...
	mdmlabProxy        *httputil.ReverseProxy
...
func (m *mdmProxy) handleProxy(w http.ResponseWriter, r *http.Request) {
	...
	// Migrated UDIDs go to the MDMlab server, otherwise requests go to the existing server.
	if udid != "" && m.isUDIDMigrated(udid) {
		log.Printf("%s %s (%s) -> MDMlab", r.Method, r.URL.String(), udid)
		if m.debug {
			log.Printf("MDMlab request: %s", string(body))
		}
		m.mdmlabProxy.ServeHTTP(w, r)
	} else {
...
func makeExistingProxy(existingURL, existingDNSName string) *httputil.ReverseProxy {
	...
	if err != nil {
		panic("failed to parse mdmlab-url: " + err.Error())
	}
...
func makeMDMlabProxy(mdmlabURL string, debug bool) *httputil.ReverseProxy {
	targetURL, err := url.Parse(mdmlabURL)
	if err != nil {
		panic("failed to parse mdmlab-url: " + err.Error())
	}
	proxy := httputil.NewSingleHostReverseProxy(targetURL)
	if debug {
		proxy.ModifyResponse = func(r *http.Response) error {
			...
			log.Println("MDMlab response: ", string(b))
...
func main() {
	...
	mdmlabURL := flag.String("mdmlab-url", "", "MDMlab MDM server URL (full path) (required)")
	migratePercentage := flag.Int("migrate-percentage", 0, "Percentage of clients to migrate from existing MDM to MDMlab")
	...
	proxy := mdmProxy{
		...
		mdmlabProxy:        makeMDMlabProxy(*mdmlabURL, *debug),
		debug:             *debug,
	...
	if *mdmlabURL == "" {
		log.Fatal("--mdmlab-url must be set")
	}
	...
	log.Printf("--mdmlab-url set: %s", *mdmlabURL)
