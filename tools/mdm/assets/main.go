...
var (
	...
	validNames = map[mdmlab.MDMAssetName]struct{}{
		mdmlab.MDMAssetABMCert:            {},
		mdmlab.MDMAssetABMTokenDeprecated: {},
		...
		mdmlab.MDMAssetVPPTokenDeprecated: {},
	}
)
...
func setupDS(privateKey, userName, password, address, name string) *mysql.Datastore {
	...
	ds, err := mysql.New(
		...
		mysql.WithMDMlabConfig(&config.MDMlabConfig{
...
func main() {
	...
	case "import":
		...
		if _, ok := validNames[mdmlab.MDMAssetName(flagImportName)]; !ok {
			log.Fatalf("invalid asset name %s", flagImportName)
		}

		err := ds.ReplaceMDMConfigAssets(ctx,
			[]mdmlab.MDMConfigAsset{{Name: mdmlab.MDMAssetName(flagImportName), Value: []byte(flagImportValue)}}, nil)
		if err != nil {
	...
	case "export":
		...
		if flagExportName != "" {
			if _, ok := validNames[mdmlab.MDMAssetName(flagExportName)]; !ok {
				log.Fatalf("invalid asset name %s", flagExportName)
			}

			names = []mdmlab.MDMAssetName{mdmlab.MDMAssetName(flagExportName)}
		}
		...
		fmt.Printf(`You can set the following on your MDMlab configuration:
