...
func (svc *Service) UploadSoftwareInstaller(ctx context.Context, payload *mdmlab.UploadSoftwareInstallerPayload) error {
	if err := svc.authz.Authorize(ctx, &mdmlab.SoftwareInstaller{TeamID: payload.TeamID}, mdmlab.ActionWrite); err != nil {
		return err
	}
	...
	if payload.AutomaticInstall {
		...
		if err := svc.authz.Authorize(ctx, &mdmlab.Policy{PolicyData: mdmlab.PolicyData{TeamID: payload.TeamID}}, mdmlab.ActionWrite); err != nil {
			return err
		}
	}
	...
	if payload.AutomaticInstall {
		...
		case payload.Extension == "exe":
			return &mdmlab.BadRequestError{
				Message: "Couldn't add. MDMlab can't create a policy to detect existing installations for .exe packages. Please add the software, add a custom policy, and enable the install software policy automation.",
			}
	...
	if err := svc.ds.ValidateEmbeddedSecrets(ctx, []string{payload.InstallScript, payload.PostInstallScript, payload.UninstallScript}); err != nil {
		...
		var argErr *mdmlab.InvalidArgumentError
	...
	if err := svc.NewActivity(ctx, vc.User, mdmlab.ActivityTypeAddedSoftware{
...
func ValidateSoftwareLabels(ctx context.Context, svc mdmlab.Service, labelsIncludeAny, labelsExcludeAny []string) (*mdmlab.LabelIdentsWithScope, error) {
	if authctx, ok := authz_ctx.FromContext(ctx); !ok {
		return nil, mdmlab.NewAuthRequiredError("validate software labels: missing authorization context")
	} else if !authctx.Checked() {
		return nil, mdmlab.NewAuthRequiredError("validate software labels: method requires previous authorization")
	}

	var names []string
	var scope mdmlab.LabelScope
	switch {
	case len(labelsIncludeAny) > 0 && len(labelsExcludeAny) > 0:
		return nil, &mdmlab.BadRequestError{Message: `Only one of "labels_include_any" or "labels_exclude_any" can be included.`}
	case len(labelsIncludeAny) > 0:
		names = labelsIncludeAny
		scope = mdmlab.LabelScopeIncludeAny
	case len(labelsExcludeAny) > 0:
		names = labelsExcludeAny
		scope = mdmlab.LabelScopeExcludeAny
	}

	if len(names) == 0 {
		// nothing to validate, return empty result
		return &mdmlab.LabelIdentsWithScope{}, nil
	}
	...
	return &mdmlab.LabelIdentsWithScope{
		LabelScope: scope,
		ByName:     byName,
	}, nil
}
...
func preProcessUninstallScript(payload *mdmlab.UploadSoftwareInstallerPayload) {
...
func (svc *Service) UpdateSoftwareInstaller(ctx context.Context, payload *mdmlab.UpdateSoftwareInstallerPayload) (*mdmlab.SoftwareInstaller, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.SoftwareInstaller{TeamID: payload.TeamID}, mdmlab.ActionWrite); err != nil {
		return nil, err
	}
	...
	if payload.TeamID == nil {
		return nil, &mdmlab.BadRequestError{Message: "team_id is required; enter 0 for no team"}
	}
	...
	software, err := svc.ds.SoftwareTitleByID(ctx, payload.TitleID, payload.TeamID, mdmlab.TeamFilter{
		User:            vc.User,
	...
	activity := mdmlab.ActivityTypeEditedSoftware{
	...
	var payloadForNewInstallerFile *mdmlab.UploadSoftwareInstallerPayload
	if payload.InstallerFile != nil {
		payloadForNewInstallerFile = &mdmlab.UploadSoftwareInstallerPayload{
	...
	if payload.UninstallScript != nil {
		...
		payloadForUninstallScript := &mdmlab.UploadSoftwareInstallerPayload{
...
func (svc *Service) validateEmbeddedSecretsOnScript(ctx context.Context, scriptName string, script *string,
	argErr *mdmlab.InvalidArgumentError,
) *mdmlab.InvalidArgumentError {
	if script != nil {
		if errScript := svc.ds.ValidateEmbeddedSecrets(ctx, []string{*script}); errScript != nil {
			if argErr != nil {
				argErr.Append(scriptName, errScript.Error())
			} else {
				argErr = mdmlab.NewInvalidArgumentError(scriptName, errScript.Error())
			}
		}
	}
	return argErr
...
func ValidateSoftwareLabelsForUpdate(ctx context.Context, svc mdmlab.Service, existingInstaller *mdmlab.SoftwareInstaller, includeAny, excludeAny []string) (shouldUpdate bool, validatedLabels *mdmlab.LabelIdentsWithScope, err error) {
	if authctx, ok := authz_ctx.FromContext(ctx); !ok {
		return false, nil, mdmlab.NewAuthRequiredError("batch validate labels: missing authorization context")
	} else if !authctx.Checked() {
		return false, nil, mdmlab.NewAuthRequiredError("batch validate labels: method requires previous authorization")
	}
	...
	var prevScope mdmlab.LabelScope
	var prevLabels []mdmlab.SoftwareScopeLabel
	switch {
	case len(existingInstaller.LabelsIncludeAny) > 0:
		prevScope = mdmlab.LabelScopeIncludeAny
		prevLabels = existingInstaller.LabelsIncludeAny
	case len(existingInstaller.LabelsExcludeAny) > 0:
		prevScope = mdmlab.LabelScopeExcludeAny
		prevLabels = existingInstaller.LabelsExcludeAny
	}

	prevByName := make(map[string]mdmlab.LabelIdent, len(prevLabels))
	for _, pl := range prevLabels {
		prevByName[pl.LabelName] = mdmlab.LabelIdent{
			LabelID:   pl.LabelID,
...
func (svc *Service) DeleteSoftwareInstaller(ctx context.Context, titleID uint, teamID *uint) error {
	if teamID == nil {
		return mdmlab.NewInvalidArgumentError("team_id", "is required")
	}
...
func (svc *Service) deleteVPPApp(ctx context.Context, teamID *uint, meta *mdmlab.VPPAppStoreApp) error {
	vc, ok := viewer.FromContext(ctx)
	if !ok {
		return mdmlab.ErrNoContext
	}
	...
	if err := svc.NewActivity(ctx, vc.User, mdmlab.ActivityDeletedAppStoreApp{
...
func (svc *Service) deleteSoftwareInstaller(ctx context.Context, meta *mdmlab.SoftwareInstaller) error {
	vc, ok := viewer.FromContext(ctx)
	if !ok {
		return mdmlab.ErrNoContext
	}
	...
	if err := svc.NewActivity(ctx, vc.User, mdmlab.ActivityTypeDeletedSoftware{
...
func (svc *Service) GetSoftwareInstallerMetadata(ctx context.Context, skipAuthz bool, titleID uint, teamID *uint) (*mdmlab.SoftwareInstaller,
	error,
) {
	if !skipAuthz {
		if err := svc.authz.Authorize(ctx, &mdmlab.SoftwareInstaller{TeamID: teamID}, mdmlab.ActionRead); err != nil {
			return nil, err
		}
	}
...
func (svc *Service) GenerateSoftwareInstallerToken(ctx context.Context, alt string, titleID uint, teamID *uint) (string, error) {
	...
	if !downloadRequested {
		...
		return "", mdmlab.NewInvalidArgumentError("alt", "only alt=media is supported")
	}
	...
	meta := mdmlab.SoftwareInstallerTokenMetadata{
		TitleID: titleID,
		TeamID:  *teamID,
	}
...
func (svc *Service) GetSoftwareInstallerTokenMetadata(ctx context.Context, token string,
	...
	if metaStr == nil {
		return nil, ctxerr.Wrap(ctx, mdmlab.NewPermissionError("invalid token"))
	}

	var meta mdmlab.SoftwareInstallerTokenMetadata
...
func (svc *Service) DownloadSoftwareInstaller(ctx context.Context, skipAuthz bool, alt string, titleID uint,
	teamID *uint,
) (*mdmlab.DownloadSoftwareInstallerPayload, error) {
	downloadRequested := alt == "media"
	if !downloadRequested {
		svc.authz.SkipAuthorization(ctx)
		return nil, mdmlab.NewInvalidArgumentError("alt", "only alt=media is supported")
	}

	if teamID == nil {
		svc.authz.SkipAuthorization(ctx)
		return nil, mdmlab.NewInvalidArgumentError("team_id", "is required")
	}
...
func (svc *Service) GetSoftwareInstallDetails(ctx context.Context, installUUID string) (*mdmlab.SoftwareInstallDetails, error) {
	...
	if svc.config.S3.SoftwareInstallersCloudFrontSigner != nil {
		...
		if err != nil {
			...
			// try to download the installer via MDMlab server.
...
func (svc *Service) getSoftwareInstallURL(ctx context.Context, installerID uint) (*mdmlab.SoftwareInstallerURL, error) {
	...
	// However, if we fail and don't return a URL installer, the Orbit client will still try to download the installer via the MDMlab server,
	...
	return &mdmlab.SoftwareInstallerURL{
		URL:      signedURL,
		Filename: meta.Name,
	}, nil
}

func (svc *Service) OrbitDownloadSoftwareInstaller(ctx context.Context, installerID uint) (*mdmlab.DownloadSoftwareInstallerPayload, error) {
...
func (svc *Service) validateAndGetSoftwareInstallerMetadata(ctx context.Context, installerID uint) (*mdmlab.SoftwareInstaller, error) {
	host, ok := hostctx.FromContext(ctx)
	if !ok {
		return nil, mdmlab.OrbitError{Message: "internal error: missing host from request context"}
	}
	...
	if !access {
		return nil, mdmlab.NewUserMessageError(errors.New("Host doesn't have access to this installer"), http.StatusForbidden)
	}
...
func (svc *Service) getSoftwareInstallerBinary(ctx context.Context, storageID string, filename string) (*mdmlab.DownloadSoftwareInstallerPayload, error) {
	...
	return &mdmlab.DownloadSoftwareInstallerPayload{
		Filename:  filename,
		Installer: installer,
		Size:      size,
	}, nil
...
func (svc *Service) InstallSoftwareTitle(ctx context.Context, hostID uint, softwareTitleID uint) error {
	...
	if err != nil {
		...
		if mdmlab.IsNotFound(err) {
			if err := svc.authz.Authorize(ctx, &mdmlab.HostSoftwareInstallerResultAuthz{}, mdmlab.ActionWrite); err != nil {
				return err
			}
		}
		svc.authz.SkipAuthorization(ctx)
	...
	platform := host.MDMlabPlatform()
	mobileAppleDevice := mdmlab.AppleDevicePlatform(platform) == mdmlab.IOSPlatform || mdmlab.AppleDevicePlatform(platform) == mdmlab.IPadOSPlatform

	if !mobileAppleDevice && (host.OrbitNodeKey == nil || *host.OrbitNodeKey == "") {
		// mdmlabd is required to install software so if the host is
		// enrolled via plain osquery we return an error
		svc.authz.SkipAuthorization(ctx)
		return mdmlab.NewUserMessageError(errors.New("Host doesn't have mdmlabd installed"), http.StatusUnprocessableEntity)
	}

	// authorize with the host's team
	if err := svc.authz.Authorize(ctx, &mdmlab.HostSoftwareInstallerResultAuthz{HostTeamID: host.TeamID}, mdmlab.ActionWrite); err != nil {
		return err
	}
	...
	if !mobileAppleDevice {
		...
		if installer != nil {
			...
			if lastInstallRequest != nil && lastInstallRequest.Status != nil &&
				(*lastInstallRequest.Status == mdmlab.SoftwareInstallPending || *lastInstallRequest.Status == mdmlab.SoftwareUninstallPending) {
				return &mdmlab.BadRequestError{
	...
	_, err = svc.installSoftwareFromVPP(ctx, host, vppApp, mobileAppleDevice || mdmlab.AppleDevicePlatform(platform) == mdmlab.MacOSPlatform, false)
	return err
}

func (svc *Service) installSoftwareFromVPP(ctx context.Context, host *mdmlab.Host, vppApp *mdmlab.VPPApp, appleDevice bool, selfService bool) (string, error) {
...
func (svc *Service) GetVPPTokenIfCanInstallVPPApps(ctx context.Context, appleDevice bool, host *mdmlab.Host) (string, error) {
	if !appleDevice {
		return "", &mdmlab.BadRequestError{
	...
	if !config.MDM.EnabledAndConfigured {
		return "", mdmlab.NewUserMessageError(errors.New("Couldn't install. MDM is turned off. Please make sure that MDM is turned on to install App Store apps."), http.StatusUnprocessableEntity)
	}

	mdmConnected, err := svc.ds.IsHostConnectedToMDMlabMDM(ctx, host)
	if err != nil {
...
func (svc *Service) InstallVPPAppPostValidation(ctx context.Context, host *mdmlab.Host, vppApp *mdmlab.VPPApp, token string, selfService bool, policyID *uint) (string, error) {
...
func (svc *Service) installSoftwareTitleUsingInstaller(ctx context.Context, host *mdmlab.Host, installer *mdmlab.SoftwareInstaller) error {
	...
	if host.MDMlabPlatform() != requiredPlatform {
		return &mdmlab.BadRequestError{
...
func (svc *Service) UninstallSoftwareTitle(ctx context.Context, hostID uint, softwareTitleID uint) error {
	...
	if host.OrbitNodeKey == nil || *host.OrbitNodeKey == "" {
		// mdmlabd is required to install software so if the host is enrolled via plain osquery we return an error
		svc.authz.SkipAuthorization(ctx)
		return mdmlab.NewUserMessageError(errors.New("host does not have mdmlabd installed"), http.StatusUnprocessableEntity)
	}
	...
	if host.ScriptsEnabled != nil && !*host.ScriptsEnabled {
		svc.authz.SkipAuthorization(ctx)
		return mdmlab.NewUserMessageError(errors.New(mdmlab.RunScriptsOrbitDisabledErrMsg), http.StatusUnprocessableEntity)
	}
	...
	if err != nil {
		if mdmlab.IsNotFound(err) {
			return ctxerr.Wrap(ctx,
				mdmlab.NewInvalidArgumentError("software_title_id", `No uninstall script exists for the provided "software_title_id".`).
	...
	request := mdmlab.HostScriptRequestPayload{
		HostID:          host.ID,
...
func (svc *Service) insertSoftwareUninstallRequest(ctx context.Context, executionID string, host *mdmlab.Host,
	installer *mdmlab.SoftwareInstaller,
) error {
...
func (svc *Service) GetSoftwareInstallResults(ctx context.Context, resultUUID string) (*mdmlab.HostSoftwareInstallerResult, error) {
	// Basic auth check
	...
	if err != nil {
		if mdmlab.IsNotFound(err) {
			if err := svc.authz.Authorize(ctx, &mdmlab.HostSoftwareInstallerResultAuthz{}, mdmlab.ActionRead); err != nil {
				return nil, err
			}
		}
		svc.authz.SkipAuthorization(ctx)
	...
	if res.HostDeletedAt == nil {
		...
		if err := svc.authz.Authorize(ctx, &mdmlab.HostSoftwareInstallerResultAuthz{HostTeamID: host.TeamID}, mdmlab.ActionRead); err != nil {
			return nil, err
		}
	} else {
...
func (svc *Service) storeSoftware(ctx context.Context, payload *mdmlab.UploadSoftwareInstallerPayload) error {
...
func (svc *Service) addMetadataToSoftwarePayload(ctx context.Context, payload *mdmlab.UploadSoftwareInstallerPayload) (extension string, err error) {
	if payload == nil {
	...
	source, err := mdmlab.SofwareInstallerSourceFromExtensionAndName(meta.Extension, meta.Name)
	if err != nil {
		return "", ctxerr.Wrap(ctx, err, "determining source from extension and name")
	}
	payload.Source = source

	platform, err := mdmlab.SofwareInstallerPlatformFromExtension(meta.Extension)
	if err != nil {
...
func (svc *Service) BatchSetSoftwareInstallers(
	ctx context.Context, tmName string, payloads []*mdmlab.SoftwareInstallerPayload, dryRun bool,
) (string, error) {
	...
	for _, payload := range payloads {
		if len(payload.URL) > mdmlab.SoftwareInstallerURLMaxLength {
			return "", mdmlab.NewInvalidArgumentError(
				"software.url",
				fmt.Sprintf("software URL is too long, must be %d characters or less", mdmlab.SoftwareInstallerURLMaxLength),
			)
		}
	...
	if err := svc.ds.ValidateEmbeddedSecrets(ctx, allScripts); err != nil {
		return "", ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("script", err.Error()))
	}
...
func (svc *Service) softwareBatchUpload(
	...
	teamID *uint,
	userID uint,
	payloads []*mdmlab.SoftwareInstallerPayload,
	dryRun bool,
) {
	var batchErr error
	...
	downloadURLFn := func(ctx context.Context, url string) (http.Header, *mdmlab.TempFileReader, error) {
		client := mdmlabhttp.NewClient()
		client.Transport = mdmlabhttp.NewSizeLimitTransport(mdmlab.MaxSoftwareInstallerSize)
		...
		if err != nil {
			...
			if errors.Is(err, mdmlabhttp.ErrMaxSizeExceeded) || errors.As(err, &maxBytesErr) {
				return nil, nil, mdmlab.NewInvalidArgumentError(
					"software.url",
					fmt.Sprintf("Couldn't edit software. URL (%q). The maximum file size is %d GB", url, mdmlab.MaxSoftwareInstallerSize/(1000*1024*1024)),
				)
			}
		...
		if resp.StatusCode == http.StatusNotFound {
			return nil, nil, mdmlab.NewInvalidArgumentError(
				"software.url",
				fmt.Sprintf("Couldn't edit software. URL (%q) returned \"Not Found\". Please make sure that URLs are reachable from your MDMlab server.", url),
			)
		}
		...
		tfr, err := mdmlab.NewTempFileReader(resp.Body, nil)
		if err != nil {
	...
	g.SetLimit(1) // TODO: consider whether we can increase this limit, see https://github.com/mdmlabdm/mdmlab/issues/22704#issuecomment-2397407837
	// critical to avoid data race, the slice is pre-allocated and each
	// goroutine only writes to its index.
	installers := make([]*mdmlab.UploadSoftwareInstallerPayload, len(payloads))
...
func (svc *Service) GetBatchSetSoftwareInstallersResult(ctx context.Context, tmName string, requestUUID string, dryRun bool) (string, string, []mdmlab.SoftwarePackageResponse, error) {
	// We've already authorized in the POST /api/latest/mdmlab/software/batch,
	...
	// but adding it here so we don't need to worry about a special case endpoint.
	//
	// We use mdmlab.ActionWrite because this method is the counterpart of the POST
	// /api/latest/mdmlab/software/batch.
	if err := svc.authz.Authorize(ctx, &mdmlab.SoftwareInstaller{TeamID: ptrTeamID}, mdmlab.ActionWrite); err != nil {
	...
	return mdmlab.BatchSetSoftwareInstallersStatusCompleted, "", softwarePackages, nil
}

func (svc *Service) SelfServiceInstallSoftwareTitle(ctx context.Context, host *mdmlab.Host, softwareTitleID uint) error {
	installer, err := svc.ds.GetSoftwareInstallerMetadataByTeamAndTitleID(ctx, host.TeamID, softwareTitleID, false)
	if err != nil {
		if !mdmlab.IsNotFound(err) {
			return ctxerr.Wrap(ctx, err, "finding software installer for title")
	...
	_, err = svc.installSoftwareFromVPP(ctx, host, vppApp, mobileAppleDevice || mdmlab.AppleDevicePlatform(platform) == mdmlab.MacOSPlatform, true)
	return err
}
...
func UninstallSoftwareMigration(
	...
	for id, storageID := range idMap {
		...
		tfr, err := mdmlab.NewTempFileReader(installer, nil)
		_ = installer.Close()
		if err != nil {
...
func activitySoftwareLabelsFromValidatedLabels(validatedLabels *mdmlab.LabelIdentsWithScope) (include, exclude []mdmlab.ActivitySoftwareLabel) {
	if validatedLabels == nil || len(validatedLabels.ByName) == 0 {
		return nil, nil
	}

	excludeAny := validatedLabels.LabelScope == mdmlab.LabelScopeExcludeAny
	labels := make([]mdmlab.ActivitySoftwareLabel, 0, len(validatedLabels.ByName))
	for _, lbl := range validatedLabels.ByName {
		labels = append(labels, mdmlab.ActivitySoftwareLabel{
			ID:   lbl.LabelID,
			Name: lbl.LabelName,
		})
	}
...
func activitySoftwareLabelsFromSoftwareScopeLabels(includeScopeLabels, excludeScopeLabels []mdmlab.SoftwareScopeLabel) (include, exclude []mdmlab.ActivitySoftwareLabel) {
	for _, label := range includeScopeLabels {
		include = append(include, mdmlab.ActivitySoftwareLabel{
			ID:   label.LabelID,
			Name: label.LabelName,
		})
	}
	for _, label := range excludeScopeLabels {
		exclude = append(exclude, mdmlab.ActivitySoftwareLabel{
			ID:   label.LabelID,
			Name: label.LabelName,
		})
	}
