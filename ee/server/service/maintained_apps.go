...
func (svc *Service) AddMDMlabMaintainedApp(
	ctx context.Context,
	teamID *uint,
	appID uint,
	...
	client := mdmlabhttp.NewClient(mdmlabhttp.WithTimeout(timeout))
...
func (svc *Service) ListMDMlabMaintainedApps(ctx context.Context, teamID *uint, opts mdmlab.ListOptions) ([]mdmlab.MaintainedApp, *mdmlab.PaginationMetadata, error) {
	...
	if teamID == nil {
		authErr = svc.authz.Authorize(ctx, &mdmlab.MaintainedApp{}, mdmlab.ActionRead)
	} else { // viewing the maintained app list when showing team-specific info requires access to that team
		authErr = svc.authz.Authorize(ctx, &mdmlab.SoftwareInstaller{TeamID: teamID}, mdmlab.ActionRead)
	}
	...
	avail, meta, err := svc.ds.ListAvailableMDMlabMaintainedApps(ctx, teamID, opts)
	if err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "listing available mdmlab managed apps")
	}
...
func (svc *Service) GetMDMlabMaintainedApp(ctx context.Context, appID uint) (*mdmlab.MaintainedApp, error) {
	...
	if err := svc.authz.Authorize(ctx, &mdmlab.MaintainedApp{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}

	app, err := svc.ds.GetMaintainedAppByID(ctx, appID)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "get mdmlab maintained app")
	}
