...
func (svc *Service) GetAppleBM(ctx context.Context) (*mdmlab.AppleBM, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.AppleBM{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
	...
	if len(tokens) > 1 {
		return nil, errors.New("This API endpoint has been deprecated. Please use the new GET /abm_tokens API endpoint documented here: https://mdmlabdm.com/learn-more-about/apple-business-manager-tokens-api")
	}

	abmToken := tokens[0]

	var appleBM mdmlab.AppleBM
...
func (svc *Service) MDMAppleEnableFileVaultAndEscrow(ctx context.Context, teamID *uint) error {
	cert, err := assets.X509Cert(ctx, svc.ds, mdmlab.MDMAssetCACert)
	if err != nil {
	...
	params := fileVaultProfileOptions{
		PayloadIdentifier:    mobileconfig.MDMlabFileVaultPayloadIdentifier,
		PayloadName:          mdm.MDMlabFileVaultProfileName,
	...
	cp, err := mdmlab.NewMDMAppleConfigProfile(contents.Bytes(), teamID)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "enabling FileVault")
	}
...
func (svc *Service) MDMAppleDisableFileVaultAndEscrow(ctx context.Context, teamID *uint) error {
	err := svc.ds.DeleteMDMAppleConfigProfileByTeamAndIdentifier(ctx, teamID, mobileconfig.MDMlabFileVaultPayloadIdentifier)
...
func (svc *Service) UpdateMDMAppleSetup(ctx context.Context, payload mdmlab.MDMAppleSetupPayload) error {
	if err := svc.authz.Authorize(ctx, payload, mdmlab.ActionWrite); err != nil {
		return err
	}
...
func (svc *Service) updateAppConfigMDMAppleSetup(ctx context.Context, payload mdmlab.MDMAppleSetupPayload) error {
...
func (svc *Service) updateMacOSSetupEnableEndUserAuth(ctx context.Context, enable bool, teamID *uint, teamName *string) error {
	...
	if enable {
		act = mdmlab.ActivityTypeEnabledMacosSetupEndUserAuth{TeamID: teamID, TeamName: teamName}
	} else {
		act = mdmlab.ActivityTypeDisabledMacosSetupEndUserAuth{TeamID: teamID, TeamName: teamName}
	}
...
func (svc *Service) validateMDMAppleSetupPayload(ctx context.Context, payload mdmlab.MDMAppleSetupPayload) error {
	// appconfig is only used internally, it's fine to read it unobfuscated
	// (svc.AppConfigObfuscated must not be used because the write-only users
	// such as gitops will fail to access it).
	ac, err := svc.ds.AppConfig(ctx)
	...
	if !ac.MDM.EnabledAndConfigured {
		return &mdmlab.MDMNotConfiguredError{}
	}
	...
	if payload.EnableEndUserAuthentication != nil && *payload.EnableEndUserAuthentication {
		if ac.MDM.EndUserAuthentication.IsEmpty() {
			...
			return mdmlab.NewInvalidArgumentError("enable_end_user_authentication",
...
func (svc *Service) MDMAppleUploadBootstrapPackage(ctx context.Context, name string, pkg io.Reader, teamID uint) error {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleBootstrapPackage{TeamID: teamID}, mdmlab.ActionWrite); err != nil {
		return err
	}
	...
	bp := &mdmlab.MDMAppleBootstrapPackage{
		TeamID: teamID,
		Name:   name,
	...
	if err := svc.NewActivity(
		...
		mdmlab.ActivityTypeAddedBootstrapPackage{BootstrapPackageName: name, TeamID: ptrTeamId, TeamName: ptrTeamName},
	); err != nil {
...
func (svc *Service) GetMDMAppleBootstrapPackageMetadata(ctx context.Context, teamID uint, forUpdate bool) (*mdmlab.MDMAppleBootstrapPackage, error) {
	act := mdmlab.ActionRead
	if forUpdate {
		act = mdmlab.ActionWrite
	}
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleBootstrapPackage{TeamID: teamID}, act); err != nil {
		return nil, err
	}
...
func (svc *Service) DeleteMDMAppleBootstrapPackage(ctx context.Context, teamID *uint) error {
	...
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleBootstrapPackage{TeamID: tmID}, mdmlab.ActionWrite); err != nil {
		return err
	}
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx),
		mdmlab.ActivityTypeDeletedBootstrapPackage{BootstrapPackageName: meta.Name, TeamID: ptrTeamID, TeamName: ptrTeamName},
	); err != nil {
...
func (svc *Service) GetMDMAppleBootstrapPackageSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleBootstrapPackageSummary, error) {
	...
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleBootstrapPackage{TeamID: tmID}, mdmlab.ActionRead); err != nil {
		return &mdmlab.MDMAppleBootstrapPackageSummary{}, err
	}
	...
	if err != nil {
		return &mdmlab.MDMAppleBootstrapPackageSummary{}, ctxerr.Wrap(ctx, err, "getting bootstrap package summary")
	}
...
func (svc *Service) MDMCreateEULA(ctx context.Context, name string, f io.ReadSeeker) error {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMEULA{}, mdmlab.ActionWrite); err != nil {
		return err
	}
...
func (svc *Service) MDMGetEULAMetadata(ctx context.Context) (*mdmlab.MDMEULA, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMEULA{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) SetOrUpdateMDMAppleSetupAssistant(ctx context.Context, asst *mdmlab.MDMAppleSetupAssistant) (*mdmlab.MDMAppleSetupAssistant, error) {
	if err := svc.authz.Authorize(ctx, asst, mdmlab.ActionWrite); err != nil {
		return nil, err
	}
	...
	if _, ok := m["configuration_web_url"]; ok && endUserAuthEnabled {
		return nil, ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("profile", `Couldn't edit macos_setup_assistant. First, disable end user authentication before adding an automatic enrollment (DEP) profile with a configuration_web_url.`))
	}

	if _, ok := m["url"]; ok {
		return nil, ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("profile", `Couldn't edit macos_setup_assistant. The automatic enrollment profile can't include url.`))
	}
	if _, ok := m["await_device_configured"]; ok {
		return nil, ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("profile", `Couldn't edit macos_setup_assistant. The profile can't include "await_device_configured" option.`))
	}
	...
	if prevAsst == nil || newAsst.Name != prevAsst.Name || newAsst.UploadedAt.After(prevAsst.UploadedAt) {
		...
		if err := svc.NewActivity(
			ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeChangedMacosSetupAssistant{
...
func (svc *Service) GetMDMAppleSetupAssistant(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleSetupAssistant, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: teamID}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) DeleteMDMAppleSetupAssistant(ctx context.Context, teamID *uint) error {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: teamID}, mdmlab.ActionWrite); err != nil {
		return err
	}
	...
	if prevAsst != nil {
		...
		if err := svc.NewActivity(
			ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeDeletedMacosSetupAssistant{
				TeamID:   teamID,
				TeamName: teamName,
				Name:     prevAsst.Name,
			}); err != nil {
...
func (svc *Service) InitiateMDMAppleSSO(ctx context.Context) (string, error) {
	...
	if settings.IsEmpty() {
		err := &mdmlab.BadRequestError{Message: "organization not configured to use sso"}
	...
	authSettings := sso.Settings{
		Metadata:                    metadata,
		AssertionConsumerServiceURL: appConfig.MDMUrl() + svc.config.Server.URLPrefix + "/api/v1/mdmlab/mdm/sso/callback",
		SessionStore:                svc.ssoSessionStore,
		OriginalURL:                 "/api/v1/mdmlab/mdm/sso/callback",
	}
...
func (svc *Service) InitiateMDMAppleSSOCallback(ctx context.Context, auth mdmlab.Auth) string {
	...
	return fmt.Sprintf("%s?%s", apple_mdm.MDMlabUISSOCallbackPath, q.Encode())
}

func (svc *Service) mdmSSOHandleCallbackAuth(ctx context.Context, auth mdmlab.Auth) (string, string, string, error) {
	appConfig, err := svc.ds.AppConfig(ctx)
	if err != nil {
	...
	err = sso.ValidateAudiences(
		*metadata,
		...
		settings.EntityID,
		appConfig.MDMUrl(),
		appConfig.MDMUrl()+svc.config.Server.URLPrefix+"/api/v1/mdmlab/mdm/sso/callback",
	)
	if err != nil {
	...
	// must be an email.
	//
	// For more details, check https://github.com/mdmlabdm/mdmlab/issues/10744#issuecomment-1540605146
	...
	err = svc.ds.InsertMDMIdPAccount(ctx, &mdmlab.MDMIdPAccount{
		Username: username,
...
func (svc *Service) getAutomaticEnrollmentProfile(ctx context.Context) (*mdmlab.MDMAppleEnrollmentProfile, error) {
	prof, err := svc.ds.GetMDMAppleEnrollmentProfileByType(ctx, mdmlab.MDMAppleEnrollmentTypeAutomatic)
	if err != nil && !mdmlab.IsNotFound(err) {
...
func (svc *Service) MDMApplePreassignProfile(ctx context.Context, payload mdmlab.MDMApplePreassignProfilePayload) error {
	...
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMConfigProfileAuthz{}, mdmlab.ActionWrite); err != nil {
...
func (svc *Service) MDMAppleMatchPreassignment(ctx context.Context, externalHostIdentifier string) error {
	...
	// load the host and ensure it is enrolled in MDMlab MDM
	...
	if !connected {
		err = errors.New("host is not enrolled in MDMlab MDM")
...
func (svc *Service) getOrCreatePreassignTeam(ctx context.Context, groups []string) (*mdmlab.Team, error) {
	...
	if err != nil {
		...
		// activities, etc.
		payload := mdmlab.TeamPayload{Name: &teamName}
		...
		if _, err := svc.ApplyTeamSpecs(ctx, []*mdmlab.TeamSpec{spec}, mdmlab.ApplyTeamSpecOptions{}); err != nil {
			return nil, err
		}
...
// profile to a host (in mdmlab.MDMApplePreassignProfilePayload). That field is
...
func (svc *Service) GetMDMDiskEncryptionSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMDiskEncryptionSummary, error) {
	...
	var macOS mdmlab.MDMAppleFileVaultSummary
	...
	var windows mdmlab.MDMWindowsBitLockerSummary
...
func (svc *Service) mdmAppleEditedAppleOSUpdates(ctx context.Context, teamID *uint, appleDevice mdmlab.AppleDevice, updates mdmlab.AppleOSUpdateSettings) error {
	const (
	...
	case mdmlab.MacOS:
		...
		osUpdatesProfileName = mdm.MDMlabMacOSUpdatesProfileName
		labelName = mdmlab.BuiltinLabelMacOS14Plus // OS update DDMs are supported on macOS 14+ devices.
	case mdmlab.IOS:
		softwareUpdateIdentifier = "ios" + softwareUpdateIdentSuffix
		osUpdatesProfileName = mdm.MDMlabIOSUpdatesProfileName
		labelName = mdmlab.BuiltinLabelIOS
	case mdmlab.IPadOS:
		softwareUpdateIdentifier = "ipados" + softwareUpdateIdentSuffix
		osUpdatesProfileName = mdm.MDMlabIPadOSUpdatesProfileName
		labelName = mdmlab.BuiltinLabelIPadOS
	default:
	...
	d := mdmlab.NewMDMAppleDeclaration(rawDecl, teamID, osUpdatesProfileName, softwareUpdateType, softwareUpdateIdentifier)
	...
	d.LabelsIncludeAll = []mdmlab.ConfigurationProfileLabel{
...
func (svc *Service) mdmWindowsEnableOSUpdates(ctx context.Context, teamID *uint, updates mdmlab.WindowsUpdates) error {
	var contents bytes.Buffer
	...
	err := svc.ds.SetOrUpdateMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{
		TeamID: teamID,
		Name:   mdm.MDMlabWindowsOSUpdatesProfileName,
...
func (svc *Service) mdmWindowsDisableOSUpdates(ctx context.Context, teamID *uint) error {
	err := svc.ds.DeleteMDMWindowsConfigProfileByTeamAndName(ctx, teamID, mdm.MDMlabWindowsOSUpdatesProfileName)
...
func (svc *Service) GetMDMManualEnrollmentProfile(ctx context.Context) ([]byte, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleManualEnrollmentProfile{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) ListABMTokens(ctx context.Context) ([]*mdmlab.ABMToken, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.AppleBM{}, mdmlab.ActionList); err != nil {
		return nil, err
	}
...
func (svc *Service) UpdateABMTokenTeams(ctx context.Context, tokenID uint, macOSTeamID, iOSTeamID, iPadOSTeamID *uint) (*mdmlab.ABMToken, error) {
	...
	token.MacOSTeam = mdmlab.ABMTokenTeam{Name: mdmlab.TeamNameNoTeam}
	token.MacOSDefaultTeamID = nil
	token.IOSTeam = mdmlab.ABMTokenTeam{Name: mdmlab.TeamNameNoTeam}
	token.IOSDefaultTeamID = nil
	token.IPadOSTeam = mdmlab.ABMTokenTeam{Name: mdmlab.TeamNameNoTeam}
...
func (svc *Service) decryptUploadedABMToken(ctx context.Context, token io.Reader) (encryptedToken []byte, decryptedToken *client.OAuth1Tokens, err error) {
	...
	pair, err := svc.ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{
		mdmlab.MDMAssetABMCert,
		mdmlab.MDMAssetABMKey,
	}, nil)
	if err != nil {
		if mdmlab.IsNotFound(err) {
			return nil, nil, ctxerr.Wrap(ctx, &mdmlab.BadRequestError{
	...
	derCert, _ := pem.Decode(pair[mdmlab.MDMAssetABMCert].Value)
	if derCert == nil {
	...
	decryptedToken, err = assets.DecryptRawABMToken(encryptedToken, cert, pair[mdmlab.MDMAssetABMKey].Value)
	if err != nil {
		return nil, nil, ctxerr.Wrap(ctx, &mdmlab.BadRequestError{
