...
func setup(t *testing.T) (*mock.Store, *Service) {
	...
	ds.GetAllMDMConfigAssetsByNameFunc = func(ctx context.Context, assetNames []mdmlab.MDMAssetName,
		...
		return map[mdmlab.MDMAssetName]mdmlab.MDMConfigAsset{
			mdmlab.MDMAssetCACert:   {Value: []byte(testCert)},
			mdmlab.MDMAssetCAKey:    {Value: []byte(testKey)},
			mdmlab.MDMAssetAPNSKey:  {Value: []byte(testKey)},
			mdmlab.MDMAssetAPNSCert: {Value: []byte(testCert)},
		}, nil
	}
...
func TestMDMAppleEnableFileVaultAndEscrow(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		ds.NewMDMAppleConfigProfileFunc = func(ctx context.Context, p mdmlab.MDMAppleConfigProfile) (*mdmlab.MDMAppleConfigProfile, error) {
			...
			require.Equal(t, p.Identifier, mobileconfig.MDMlabFileVaultPayloadIdentifier)
			require.Equal(t, p.Name, mdm.MDMlabFileVaultProfileName)
