...
func (svc *Service) processCalendarEvent(ctx context.Context, eventDetails *mdmlab.CalendarEventDetails,
	googleCalendarIntegrationConfig *mdmlab.GoogleCalendarIntegration, userCalendar mdmlab.UserCalendar) error {

	var generatedTag string
	// This flag indicates that calendar event should no longer exist, and we can stop watching it.
	stopChannel := false
	var genBodyFn mdmlab.CalendarGenBodyFn = func(conflict bool) (body string, ok bool, err error) {
	...
	event, updated, err := userCalendar.GetAndUpdateEvent(&eventDetails.CalendarEvent, genBodyFn, mdmlab.CalendarGetAndUpdateEventOpts{})
	if err != nil {
	...
	if updated && event != nil {
		...
		_, err = svc.ds.CreateOrUpdateCalendarEvent(ctx, event.UUID, event.Email, event.StartTime, event.EndTime, event.Data,
			event.TimeZone, eventDetails.HostID, mdmlab.CalendarWebhookStatusNone)
		if err != nil {
