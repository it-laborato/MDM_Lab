...
import (
	...
	scepclient "github.com/it-laborato/MDM_Lab/server/mdm/scep/client"
...
func (svc *scepProxyService) PKIOperation(ctx context.Context, data []byte, identifier string) ([]byte, error) {
	...
	if !strings.HasPrefix(profileUUID, mdmlab.MDMAppleProfileUUIDPrefix) {
	...
	if profile.Status == nil || *profile.Status != mdmlab.MDMDeliveryPending {
		// This could happen if MDMlab DB was updated before the profile was updated on the host.
...
func NewSCEPProxyService(ds mdmlab.Datastore, logger log.Logger) scepserver.ServiceWithIdentifier {
...
func ValidateNDESSCEPAdminURL(ctx context.Context, proxy mdmlab.NDESSCEPProxyIntegration) error {
	_, err := GetNDESSCEPChallenge(ctx, proxy)
	return err
}

func GetNDESSCEPChallenge(ctx context.Context, proxy mdmlab.NDESSCEPProxyIntegration) (string, error) {
	adminURL, username, password := proxy.AdminURL, proxy.Username, proxy.Password
	// Get the challenge from NDES
	client := mdmlabhttp.NewClient(mdmlabhttp.WithTimeout(*NDESTimeout))
	client.Transport = ntlmssp.Negotiator{
		RoundTripper: mdmlabhttp.NewTransport(),
	}
...
func ValidateNDESSCEPURL(ctx context.Context, proxy mdmlab.NDESSCEPProxyIntegration, logger log.Logger) error {
