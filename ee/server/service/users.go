...
func (svc *Service) GetSSOUser(ctx context.Context, auth mdmlab.Auth) (*mdmlab.User, error) {
	...
	if err := mdmlab.ValidateEmail(auth.UserID()); err != nil {
	...
	case err == nil:
		...
		// If JIT provisioning is disabled, then MDMlab does not attempt to change
		...
		ssoRolesInfo, err := mdmlab.RolesFromSSOAttributes(auth.AssertionAttributes())
		if err != nil {
		...
		// come from mdmlab.RolesFromSSOAttributes which already checks for duplicates.
		...
		if err := mdmlab.LogRoleChangeActivities(ctx, svc, user, oldGlobalRole, oldTeamsRoles, user); err != nil {
	...
	user, err = svc.Service.NewUser(ctx, mdmlab.UserPayload{
		Name:       &displayName,
	...
	if err := svc.NewActivity(
		ctx,
		user,
		mdmlab.ActivityTypeUserAddedBySSO{},
	); err != nil {
...
//
// rolesChanged assumes that there cannot be multiple role entries for the same team.
func rolesChanged(oldGlobal *string, oldTeams []mdmlab.UserTeam, newGlobal *string, newTeams []mdmlab.UserTeam) bool {
	...
	oldTeamsMap := make(map[uint]mdmlab.UserTeam, len(oldTeams))
...
// to a `mdmlab.User` struct fields `GlobalRole` and `Teams` respectively.
func (svc *Service) userRolesFromSSOAttributes(ctx context.Context, ssoRolesInfo mdmlab.SSORolesInfo) (globalRole *string, teamsRoles []mdmlab.UserTeam, err error) {
	for _, teamRole := range ssoRolesInfo.Teams {
		...
		teamsRoles = append(teamsRoles, mdmlab.UserTeam{
			Team: *team,
			Role: teamRole.Role,
		})
	}
