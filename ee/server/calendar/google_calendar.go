...
type GoogleCalendarConfig struct {
	...
	IntegrationConfig *mdmlab.GoogleCalendarIntegration
...
func NewGoogleCalendar(config *GoogleCalendarConfig) *GoogleCalendar {
	...
	case config.IntegrationConfig.ApiKey[mdmlab.GoogleCalendarEmail] == loadEmail:
		config.API = &GoogleCalendarLoadAPI{Logger: config.Logger}
	case config.IntegrationConfig.ApiKey[mdmlab.GoogleCalendarEmail] == MockEmail:
	...
	resp, err := lowLevelAPI.withRetry(
		func() (any, error) {
			...
			return lowLevelAPI.service.Events.Watch(calendarID, &calendar.Channel{
				...
				Type: "web_hook",
				Address: fmt.Sprintf("%s/api/v1/mdmlab/calendar/webhook/%s",
...
func (c *GoogleCalendar) Configure(userEmail string) error {
	...
	err := c.config.API.Configure(
		c.config.Context, c.config.IntegrationConfig.ApiKey[mdmlab.GoogleCalendarEmail],
		c.config.IntegrationConfig.ApiKey[mdmlab.GoogleCalendarPrivateKey], adjustedUserEmail,
		c.config.ServerURL,
	)
	if err != nil {
...
func (c *GoogleCalendar) UpdateEventBody(event *mdmlab.CalendarEvent,
	genBodyFn mdmlab.CalendarGenBodyFn) (string, error) {
	...
	conflict := strings.Contains(gEvent.Description, mdmlab.CalendarEventConflictText)
	var ok bool
...
func (c *GoogleCalendar) GetAndUpdateEvent(event *mdmlab.CalendarEvent, genBodyFn mdmlab.CalendarGenBodyFn,
	opts mdmlab.CalendarGetAndUpdateEventOpts) (
	*mdmlab.CalendarEvent, bool, error,
) {
	// We assume that the MDMlab event has not already ended. We will simply return it if it has not been modified.
	details, err := c.unmarshalDetails(event)
	if err != nil {
	...
	updateTimezone := func() error {
		...
		// nil if cal event created before MDMlab tracked timezone
	...
	case googleapi.IsNotModified(err):
		if tzUpdated {
			...
			// has been, so update the MDMlab event's timezone
	...
	if !deleted && gEvent.Status != "cancelled" {
		...
		if !deleted {
			...
			mdmlabEvent, err := c.googleEventToMDMlabEvent(*startTime, *endTime, gEvent, event.UUID, details.ChannelID, details.ResourceID,
				details.BodyTag)
			if err != nil {
				return nil, false, err
			}
			return mdmlabEvent, true, nil
		}
	}
	...
	var createOpts mdmlab.CalendarCreateEventOpts
	...
	mdmlabEvent, err := c.CreateEvent(newStartDate, genBodyFn, createOpts)
	if err != nil {
		return nil, false, err
	}
	return mdmlabEvent, true, nil
...
func (c *GoogleCalendar) unmarshalDetails(event *mdmlab.CalendarEvent) (*eventDetails, error) {
...
func (c *GoogleCalendar) CreateEvent(dayOfEvent time.Time,
	genBodyFn mdmlab.CalendarGenBodyFn,
	opts mdmlab.CalendarCreateEventOpts) (*mdmlab.CalendarEvent, error) {
...
func (c *GoogleCalendar) createEvent(
	dayOfEvent time.Time, genBodyFn mdmlab.CalendarGenBodyFn, timeNow func() time.Time,
	opts mdmlab.CalendarCreateEventOpts,
	...
	if dayEnd.Before(now) {
		...
		return nil, ctxerr.Wrap(c.config.Context, mdmlab.DayEndedError{Msg: "cannot schedule an event for a day that has already ended"})
	}
	...
	if !dayStart.After(now) {
		...
		if !dayStart.Before(dayEnd) {
			return nil, ctxerr.Wrap(c.config.Context, mdmlab.DayEndedError{Msg: "no time available for event"})
		}
	}
	eventStart := dayStart
	...
	// Convert Google event to MDMlab event. Body tag will be updated by the calling function.
	mdmlabEvent, err := c.googleEventToMDMlabEvent(eventStart, eventEnd, event, eventUUID, channelID, resourceID, "body_tag")
	if err != nil {
		return nil, err
	}
...
func (c *GoogleCalendar) googleEventToMDMlabEvent(startTime time.Time, endTime time.Time, event *calendar.Event, eventUUID string,
	channelID string, resourceID string, bodyTag string) (
	*mdmlab.CalendarEvent, error,
) {
	tzName := c.location.String()
	mdmlabEvent := &mdmlab.CalendarEvent{}
	mdmlabEvent.StartTime = startTime
	mdmlabEvent.EndTime = endTime
	mdmlabEvent.Email = c.currentUserEmail
	mdmlabEvent.TimeZone = &tzName
	mdmlabEvent.UUID = eventUUID
	...
	mdmlabEvent.Data = detailsJson
	return mdmlabEvent, nil
}

func (c *GoogleCalendar) DeleteEvent(event *mdmlab.CalendarEvent) error {
	details, err := c.unmarshalDetails(event)
	if err != nil {
		return err
	}
...
func (c *GoogleCalendar) StopEventChannel(event *mdmlab.CalendarEvent) error {
	details, err := c.unmarshalDetails(event)
	if err != nil {
		return err
	}
...
func (c *GoogleCalendar) Get(event *mdmlab.CalendarEvent, key string) (interface{}, error) {
	if key == "channelID" {
		details, err := c.unmarshalDetails(event)
		if err != nil {
			return nil, err
