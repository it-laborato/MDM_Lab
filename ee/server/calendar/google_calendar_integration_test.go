...
func (s *googleCalendarIntegrationTestSuite) TestCreateGetDeleteEvent() {
	...
	event, err := gCal.CreateEvent(eventDate, genBodyFn, mdmlab.CalendarCreateEventOpts{})
	require.NoError(t, err)
	...
	eventRsp, updated, err := gCal.GetAndUpdateEvent(event, genBodyFn, mdmlab.CalendarGetAndUpdateEventOpts{})
	require.NoError(t, err)
