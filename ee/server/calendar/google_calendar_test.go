...
func TestGoogleCalendar_Configure(t *testing.T) {
	...
	// Happy path test
	var cal mdmlab.UserCalendar = NewGoogleCalendar(makeConfig(mockAPI))
	...
	config := &GoogleCalendarConfig{
		...
		IntegrationConfig: &mdmlab.GoogleCalendarIntegration{
			ApiKey: map[string]string{
				mdmlab.GoogleCalendarEmail:      baseServiceEmail,
				mdmlab.GoogleCalendarPrivateKey: basePrivateKey,
			},
		},
		Logger:    logger,
		API:       mockAPI,
...
func TestGoogleCalendar_DeleteEvent(t *testing.T) {
	...
	err := cal.Configure(baseUserEmail)
	assert.NoError(t, err)
	err = cal.DeleteEvent(&mdmlab.CalendarEvent{Data: []byte(`{"ID":"event-id"}`)})
	assert.NoError(t, err)
...
func TestGoogleCalendar_unmarshalDetails(t *testing.T) {
	...
	assert.NoError(t, err)
	details, err := gCal.unmarshalDetails(&mdmlab.CalendarEvent{Data: []byte(`{"id":"event-id","etag":"event-eTag"}`)})
	assert.NoError(t, err)
	assert.Equal(t, "event-id", details.ID)
	assert.Equal(t, "event-eTag", details.ETag)

	// Missing ETag is OK
	details, err = gCal.unmarshalDetails(&mdmlab.CalendarEvent{Data: []byte(`{"id":"event-id"}`)})
	assert.NoError(t, err)
	assert.Equal(t, "event-id", details.ID)
	assert.Equal(t, "", details.ETag)

	// Bad JSON
	_, err = gCal.unmarshalDetails(&mdmlab.CalendarEvent{Data: []byte(`{"bozo`)})
	assert.Error(t, err)

	// Missing id
	_, err = gCal.unmarshalDetails(&mdmlab.CalendarEvent{Data: []byte(`{"myId":"event-id","etag":"event-eTag"}`)})
	assert.Error(t, err)
}
...
func TestGoogleCalendar_GetAndUpdateEvent(t *testing.T) {
	...
	var genBodyFn mdmlab.CalendarGenBodyFn = func(bool) (string, bool, error) {
	...
	// ETag matches
	retrievedEvent, updated, err := cal.GetAndUpdateEvent(event, genBodyFn, mdmlab.CalendarGetAndUpdateEventOpts{})
	assert.NoError(t, err)
	...
	eventBadDetails := &mdmlab.CalendarEvent{
		StartTime: time.Now(),
		EndTime:   time.Now().Add(time.Hour),
		Data:      []byte(`{"bozo`),
	}
	_, _, err = cal.GetAndUpdateEvent(eventBadDetails, genBodyFn, mdmlab.CalendarGetAndUpdateEventOpts{})
	assert.Error(t, err)
	...
	_, _, err = cal.GetAndUpdateEvent(event, genBodyFn, mdmlab.CalendarGetAndUpdateEventOpts{})
	assert.ErrorIs(t, err, assert.AnError)
	...
	retrievedEvent, updated, err = cal.GetAndUpdateEvent(event, genBodyFn, mdmlab.CalendarGetAndUpdateEventOpts{UpdateTimezone: true})
	assert.NoError(t, err)
	assert.True(t, updated)
	assert.NotEqual(t, event, retrievedEvent)
	require.NotNil(t, retrievedEvent)
...
func TestGoogleCalendar_CreateEvent(t *testing.T) {
	...
	event, err := cal.CreateEvent(date, genBodyFn, mdmlab.CalendarCreateEventOpts{})
	require.NoError(t, err)
	...
	date = time.Now().Add(-48 * time.Hour)
	_, err = cal.CreateEvent(date, genBodyFn, mdmlab.CalendarCreateEventOpts{})
	assert.ErrorAs(t, err, &mdmlab.DayEndedError{})
	...
	_, err = gCal.createEvent(date, genBodyFn, timeNow, mdmlab.CalendarCreateEventOpts{})
	assert.ErrorAs(t, err, &mdmlab.DayEndedError{})
	...
	event, err = gCal.createEvent(date, genBodyFn, timeNow, mdmlab.CalendarCreateEventOpts{})
	require.NoError(t, err)
	assert.Equal(t, expectedStartTime.UTC(), event.StartTime.UTC())
	assert.Equal(t, expectedStartTime.Add(eventLength).UTC(), event.EndTime.UTC())
	...
	event, err = gCal.CreateEvent(date, genBodyFn, mdmlab.CalendarCreateEventOpts{})
	require.NoError(t, err)
	assert.Equal(t, expectedStartTime.UTC(), event.StartTime.UTC())
	assert.Equal(t, expectedStartTime.Add(eventLength).UTC(), event.EndTime.UTC())
	...
	expectedStartTime = time.Date(date.Year(), date.Month(), date.Day(), endHour-1, 30, 0, 0, location)
	event, err = gCal.CreateEvent(date, genBodyConflictFn, mdmlab.CalendarCreateEventOpts{})
	require.NoError(t, err)
	assert.Equal(t, expectedStartTime.UTC(), event.StartTime.UTC())
	assert.Equal(t, expectedStartTime.Add(eventLength).UTC(), event.EndTime.UTC())
	...
	_, err = gCal.CreateEvent(date, genBodyFn, mdmlab.CalendarCreateEventOpts{})
	assert.ErrorIs(t, err, assert.AnError)
