...
var (
	...
	macosVulnerableSoftware            []mdmlab.Software
	vsCodeExtensionsVulnerableSoftware []mdmlab.Software
...
func loadMacOSVulnerableSoftware() {
	...
	for _, line := range lines {
		...
		macosVulnerableSoftware = append(macosVulnerableSoftware, mdmlab.Software{
...
func loadExtraVulnerableSoftware() {
	...
	for _, line := range lines {
		...
		vsCodeExtensionsVulnerableSoftware = append(vsCodeExtensionsVulnerableSoftware, mdmlab.Software{
...
type agent struct {
	...
	os                            string
	// deviceAuthToken holds MDMlab Desktop device authentication token.
	//
	...
	// (client side) against MDMlab MDM.
	...
	// Software installed on the host via MDMlab. Key is the software name + version + bundle identifier.
	...
	// /api/v1/osquery/log requests to the MDMlab server fail.
	//
...
func (a *agent) runOrbitLoop() {
	...
	if !a.disableMDMlabDesktop {
		tokenRotationEnabled = orbitClient.GetServerCapabilities().Has(mdmlab.CapabilityOrbitEndpoints) &&
			orbitClient.GetServerCapabilities().Has(mdmlab.CapabilityTokenRotation)
	...
	// checkToken is used to simulate MDMlab Desktop polling until a token is
	...
	// MDMlab Desktop performs a burst of check token requests when it's initialized
	if !a.disableMDMlabDesktop {
		checkToken()
	}
	...
	// mdmlab desktop polls for policy compliance every 5 minutes
	mdmlabDesktopPolicyTicker := time.Tick(5 * time.Minute)
	...
	for {
		...
		case <-orbitTokenRemoteCheckTicker:
			if !a.disableMDMlabDesktop && tokenRotationEnabled {
				if err := deviceClient.CheckToken(*a.deviceAuthToken); err != nil {
					a.stats.IncrementOrbitErrors()
					log.Println("deviceClient.CheckToken: ", err)
					continue
		...
		case <-orbitTokenRotationTicker:
			if !a.disableMDMlabDesktop && tokenRotationEnabled {
				...
				// mdmlab desktop performs a burst of check token requests after a token is rotated
				checkToken()
			}
		...
		case <-mdmlabDesktopPolicyTicker:
			if !a.disableMDMlabDesktop {
...
func (a *agent) doDeclarativeManagement(cmd *mdm.Command) {
	...
	for _, d := range items.Declarations.Activations {
		...
		var act mdmlab.MDMAppleDDMActivation
	...
	for _, d := range items.Declarations.Configurations {
		...
		report.StatusItems.Management.Declarations.Configurations = []mdmlab.MDMAppleDDMStatusDeclaration{
			{Active: true, Valid: mdmlab.MDMAppleDeclarationValid, Identifier: d.Identifier, ServerToken: d.ServerToken},
		}
...
func (a *agent) runWindowsMDMLoop() {
	...
	for range mdmCheckInTicker {
		...
		for _, c := range cmds {
			...
			a.winMDMClient.AppendResponse(mdmlab.SyncMLCmd{
...
func (a *agent) execScripts(execIDs []string, orbitClient *service.OrbitClient) {
	...
	for _, execID := range execIDs {
		if a.disableScriptExec {
			...
			if err := orbitClient.SaveHostScriptResult(&mdmlab.HostScriptResultPayload{
				ExecutionID: execID,
...
func (a *agent) orbitEnroll() error {
	...
	response := a.waitingDo(func() *http.Request {
		request, err := http.NewRequest("POST", a.serverAddress+"/api/mdmlab/orbit/enroll", bytes.NewReader(jsonBytes))
		if err != nil {
			panic(err)
		}
...
func (a *agent) softwareMacOS() []map[string]string {
	...
	for i := 0; i < len(commonSoftware); i++ {
		...
		commonSoftware[i] = map[string]string{
			...
			"version":           "0.0.1",
			"bundle_identifier": fmt.Sprintf("com.mdmlabdm.osquery-perf.common_%d", i),
			"source":            "apps",
	...
	for i := 0; i < len(uniqueSoftware); i++ {
		...
		uniqueSoftware[i] = map[string]string{
			...
			"version":           "1.1.1",
			"bundle_identifier": fmt.Sprintf("com.mdmlabdm.osquery-perf.unique_%s_%d", a.CachedString("hostname"), i),
			"source":            "apps",
			"last_opened_at":    lastOpenedAt,
...
func (a *mdmAgent) softwareIOSandIPadOS(source string) []mdmlab.Software {
	commonSoftware := make([]map[string]string, a.softwareCount.common)
	for i := 0; i < len(commonSoftware); i++ {
		commonSoftware[i] = map[string]string{
	...
	mdmlabSoftware := make([]mdmlab.Software, len(software))
	for i, s := range software {
		mdmlabSoftware[i] = mdmlab.Software{
			Name:             s["name"],
			Version:          s["version"],
	...
	return []map[string]string{
		{
			...
			"provider_id":           mdmlab.WellKnownMDMMDMlab,
...
func (a *agent) runLiveQuery(query string) (results []map[string]string, status *mdmlab.OsqueryStatus, message *string, stats *mdmlab.Stats) {
	if a.liveQueryFailProb > 0.0 && rand.Float64() <= a.liveQueryFailProb {
		ss := mdmlab.OsqueryStatus(1)
...
func (a *agent) runLiveYaraQuery(query string) (results []map[string]string, status *mdmlab.OsqueryStatus, message *string, stats *mdmlab.Stats) {
...
func (a *agent) runLiveMockQuery(query string) (results []map[string]string, status *mdmlab.OsqueryStatus, message *string, stats *mdmlab.Stats) {
	ss := mdmlab.OsqueryStatus(0)
	return []map[string]string{
			{
...
func (a *agent) processQuery(name, query string, cachedResults *cachedResults) (
	...
	status *mdmlab.OsqueryStatus, message *string, stats *mdmlab.Stats,
) {
	const (
		hostPolicyQueryPrefix = "mdmlab_policy_query_"
		hostDetailQueryPrefix = "mdmlab_detail_query_"
		liveQueryPrefix       = "mdmlab_distributed_query_"
	)
	statusOK := mdmlab.StatusOK
	statusNotOK := mdmlab.OsqueryStatus(1)
	...
	case name == hostDetailQueryPrefix+"munki_info":
		ss := mdmlab.OsqueryStatus(rand.Intn(2))
		if ss == mdmlab.StatusOK {
			results = a.munkiInfo()
		}
		return true, results, &ss, nil, nil
...
func (a *agent) DistributedWrite(queries map[string]string) error {
	r := service.SubmitDistributedQueryResultsRequest{
		Results:  make(mdmlab.OsqueryDistributedQueryResults),
		Statuses: make(map[string]mdmlab.OsqueryStatus),
		Messages: make(map[string]string),
		Stats:    make(map[string]*mdmlab.Stats),
	}
	r.NodeKey = a.nodeKey
	...
	for _, name := range queryNames {
		...
		if !handled {
			...
			r.Statuses[name] = mdmlab.StatusOK
		} else {
			if results != nil {
...
func main() {
	...
	var (
		...
		commonSoftwareCount                          = flag.Int("common_software_count", 10, "Number of common installed applications reported to mdmlab")
		commonVSCodeExtensionsSoftwareCount          = flag.Int("common_vscode_extensions_software_count", 5, "Number of common vscode_extensions installed applications reported to mdmlab")
		...
		vulnerableSoftwareCount     = flag.Int("vulnerable_software_count", 10, "Number of vulnerable installed applications reported to mdmlab")
		withLastOpenedSoftwareCount = flag.Int("with_last_opened_software_count", 10, "Number of applications that may report a last opened timestamp to mdmlab")
		...
		commonUserCount             = flag.Int("common_user_count", 10, "Number of common host users reported to mdmlab")
		uniqueUserCount             = flag.Int("unique_user_count", 10, "Number of unique host users reported to mdmlab")
		...
		mdmProb          = flag.Float64("mdm_prob", 0.0, "Probability of a host enrolling via MDMlab MDM (applies for macOS and Windows hosts, implies orbit enrollment on Windows) [0, 1]")
		...
		disableMDMlabDesktop = flag.Bool("disable_mdmlab_desktop", false, "Disable MDMlab Desktop")
