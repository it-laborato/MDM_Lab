...
func (s *integrationGitopsTestSuite) TestMDMlabGitops() {
	t := s.T()
	const mdmlabGitopsRepo = "https://github.com/mdmlabdm/mdmlab-gitops"

	mdmlabctlConfig := s.createMDMlabctlConfig()
...
func (s *integrationGitopsTestSuite) createMDMlabctlConfig() *os.File {
	t := s.T()
...
`, s.server.URL, token,
	...
	require.NoError(t, err)
	return mdmlabctlConfig
}

func (s *integrationGitopsTestSuite) TestMDMlabGitopsWithMDMlabSecrets() {
	t := s.T()
	const (
		secretName1 = "NAME"
		secretName2 = "length"
	...
	// Check script(s)
	scriptID, err := s.ds.GetScriptIDByName(ctx, "mdmlab-secret.sh", nil)
	require.NoError(t, err)
	expected, err := os.ReadFile("testdata/gitops/lib/mdmlab-secret.sh")
	require.NoError(t, err)
	...
	allProfiles, _, err := s.ds.ListMDMConfigProfiles(ctx, nil, mdmlab.ListOptions{})
	require.NoError(t, err)
