...
type goqueryClient struct {
	...
	// query against MDMlab. Keep a mapping so that we know how to target
	// the host.
	hostnameByUUID map[string]string
}

func newGoqueryClient(mdmlabClient *service.Client) *goqueryClient {
...
func goqueryCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			goquery.Run(newGoqueryClient(mdmlab), gqconfig.Config{})
			return nil
		},
	}
}
