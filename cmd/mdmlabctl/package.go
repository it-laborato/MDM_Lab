...
import (
	...
	"github.com/it-laborato/MDM_Lab/pkg/filepath_windows"
...
func packageCommand() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				Name:        "update-roots",
				Usage:       "Root key JSON metadata for update server (from mdmlabctl updates roots)",
			...
			&cli.StringFlag{
				...
				Usage:       "Path to a TLS client certificate to use when connecting to the update server. This functionality is licensed under the MDMlab EE License. Usage requires a current MDMlab EE subscription.",
				Destination: &opt.UpdateTLSClientCertificate,
			},
			&cli.StringFlag{
				Name:        "update-tls-client-key",
				Usage:       "Path to a TLS client private key to use when connecting to the update server. This functionality is licensed under the MDMlab EE License. Usage requires a current MDMlab EE subscription.",
			...
			&cli.BoolFlag{
				Name:        "mdmlab-desktop",
				Usage:       "Include the MDMlab Desktop Application in the package",
			...
			eemdmlabctl.LocalWixDirFlag(&opt.LocalWixDir),
			&cli.StringFlag{
			...
			&cli.StringFlag{
				Name:        "end-user-email",
				Usage:       "End user's email that populates human to host mapping in MDMlab (only available on Windows and Linux)",
		...
		Action: func(c *cli.Context) error {
			...
			if opt.EndUserEmail != "" {
				if !mdmlab.IsLooseEmail(opt.EndUserEmail) {
			...
			fmt.Println("Generating your mdmlabd agent...")
			...
			fmt.Printf(`
Success! You generated mdmlabd at %s

To add a new %s device to MDMlab, install mdmlabd by %s.

Learn more: https://mdmlabdm.com/guides/enroll-hosts

To add other devices to MDMlab, distribute mdmlabd using Chef, Ansible, Jamf, or Puppet. Learn how: https://mdmlabdm.com/learn-more-about/enrolling-hosts
