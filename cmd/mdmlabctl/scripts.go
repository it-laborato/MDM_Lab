...
func runScriptCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: `mdmlabctl run-script [options]`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     "script-path",
			...
			&cli.UintFlag{
				Name:     "team",
				Usage:    `Available in MDMlab Premium. ID of the team that the saved script belongs to. 0 targets hosts assigned to “No team” (default: 0).`,
				Required: false,
			},
			&cli.BoolFlag{
				Name:     "async",
		...
		Action: func(c *cli.Context) error {
			...
			if appCfg.ServerSettings.ScriptsDisabled {
				return errors.New(mdmlab.RunScriptScriptsDisabledGloballyErrMsg)
			}
			...
			if err != nil {
				...
				if errors.As(err, &nfe) {
					return errors.New(mdmlab.HostNotFoundErrMsg)
				}
				var sce mdmlab.ErrWithStatusCode
				if errors.As(err, &sce) {
					if sce.StatusCode() == http.StatusForbidden {
						return errors.New(mdmlab.RunScriptForbiddenErrMsg)
					}
				}
				return err
			}

			if h.Status != mdmlab.StatusOnline {
				return errors.New(mdmlab.RunScriptHostOfflineErrMsg)
			}
			...
			if path != "" || args > 0 {
				...
				// for the shorter
				if err := mdmlab.ValidateHostScriptContents(string(b), false); err != nil {
					if err.Error() == mdmlab.RunScripUnsavedMaxLenErrMsg {
						return errors.New("Script is too large. Script referenced by '--script-path' is limited to 10,000 characters. To run larger script save it to MDMlab and use '--script-name'.")
					}
					return err
				}
			}
...
func renderScriptResult(c *cli.Context, res *mdmlab.HostScriptResult) error {
...
`))
	...
	if len(res.Output) >= mdmlab.UnsavedScriptMaxRuneLen && utf8.RuneCountInString(res.Output) >= mdmlab.UnsavedScriptMaxRuneLen {
		data.Output = "MDMlab records the last 10,000 characters to prevent downtime.\n\n" + res.Output
	} else {
...
func validateScriptPath(path string) error {
	...
	return errors.New(mdmlab.RunScriptInvalidTypeErrMsg)
}
