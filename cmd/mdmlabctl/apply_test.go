...
var userRoleSpecList = []*mdmlab.User{
	{
...
func TestApplyUserRoles(t *testing.T) {
	...
	ds.SaveUsersFunc = func(ctx context.Context, users []*mdmlab.User) error {
		for _, u := range users {
			switch u.Email {
...
`)
	...
	assert.Equal(t, mdmlab.RoleMaintainer, userRoleSpecList[1].Teams[0].Role)
}
...
func TestApplyTeamSpecs(t *testing.T) {
	...
	teamsByName := map[string]*mdmlab.Team{
		"team1": {
			ID:          42,
			Name:        "team1",
	...
	enrolledSecretsCalled := make(map[uint][]*mdmlab.EnrollSecret)
...
`)
	...
	assert.Equal(t, []*mdmlab.EnrollSecret{{Secret: "AAA"}}, enrolledSecretsCalled[uint(42)])
...
`, mobileCfgPath))
	...
	newMDMSettings = mdmlab.TeamMDM{
		...
		MacOSSettings: mdmlab.MacOSSettings{
			CustomSettings: []mdmlab.MDMProfileSpec{{Path: mobileCfgPath}},
		},
		MacOSSetup: mdmlab.MacOSSetup{
			EnableReleaseDeviceManually: optjson.SetBool(false),
		},
...
`)
	...
	newMDMSettings = mdmlab.TeamMDM{
		...
		MacOSSettings: mdmlab.MacOSSettings{ // macos settings not provided, so not cleared
			CustomSettings: []mdmlab.MDMProfileSpec{{Path: mobileCfgPath}},
		},
		MacOSSetup: mdmlab.MacOSSetup{
			EnableReleaseDeviceManually: optjson.SetBool(false),
	...
	assert.JSONEq(t, string(newAgentOpts), string(*teamsByName["team1"].Config.AgentOptions))
	assert.Equal(t, newMDMSettings, teamsByName["team1"].Config.MDM)
	assert.Equal(t, []*mdmlab.EnrollSecret{{Secret: "BBB"}}, enrolledSecretsCalled[uint(42)])
	assert.True(t, ds.ApplyEnrollSecretsFuncInvoked)
...
`)
	...
	newMDMSettings = mdmlab.TeamMDM{
		...
		MacOSSettings: mdmlab.MacOSSettings{
			CustomSettings: []mdmlab.MDMProfileSpec{},
		},
		MacOSSetup: mdmlab.MacOSSetup{
			EnableReleaseDeviceManually: optjson.SetBool(false),
		},
...
`,
	...
	assert.Equal(
		t, mdmlab.HostStatusWebhookSettings{
			DaysCount:      14,
	...
	assert.Equal(t, mdmlab.FailingPoliciesWebhookSettings{}, teamsByName["team1"].Config.WebhookSettings.FailingPoliciesWebhook)
	// enroll secret not cleared since not provided
	assert.Equal(t, []*mdmlab.EnrollSecret{{Secret: "BBB"}}, enrolledSecretsCalled[uint(42)])
...
`,
	...
	assert.Equal(
		t, mdmlab.TeamGoogleCalendarIntegration{
			Enable:     true,
...
func TestApplyAppConfig(t *testing.T) {
	...
	ds.SetOrUpdateMDMWindowsConfigProfileFunc = func(ctx context.Context, cp mdmlab.MDMWindowsConfigProfile) error {
		return nil
	}
	...
	savedAppConfig := &mdmlab.AppConfig{
		OrgInfo:        mdmlab.OrgInfo{OrgName: "MDMlab"},
	...
	ds.TeamsSummaryFunc = func(ctx context.Context) ([]*mdmlab.TeamSummary, error) {
		return []*mdmlab.TeamSummary{{Name: "team1", ID: 1}}, nil
	}
...
`)
	...
	assert.Equal(t, "[+] applied mdmlab config\n", runAppForTest(t, []string{"apply", "-f", name}))
...
func TestApplyAppConfigDryRunIssue(t *testing.T) {
	// reproduces the bug fixed by https://github.com/mdmlabdm/mdmlab/pull/8194
	_, ds := runServerWithMockedDS(t)
	...
	currentAppConfig := &mdmlab.AppConfig{
		OrgInfo: mdmlab.OrgInfo{OrgName: "MDMlab"}, ServerSettings: mdmlab.ServerSettings{ServerURL: "https://example.org"},
	}
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
	...
	// first, set the default app config's agent options as set after mdmlabctl setup
	name := writeTmpYml(t, `---
apiVersion: v1
kind: config
spec:
...
`)
	...
	assert.Equal(t, "[+] would've applied mdmlab config\n", runAppForTest(t, []string{"apply", "--dry-run", "-f", name}))
...
func TestApplyAppConfigUnknownFields(t *testing.T) {
	...
	var savedAppConfig *mdmlab.AppConfig
	ds.SaveAppConfigFunc = func(ctx context.Context, config *mdmlab.AppConfig) error {
		savedAppConfig = config
		return nil
	}
...
`)
	...
	runAppCheckErr(t, []string{"apply", "-f", name},
		"applying mdmlab config: PATCH /api/latest/mdmlab/config received status 400 Bad Request: unsupported key provided: \"enabled_software_inventory\"",
	)
...
func TestApplyAppConfigDeprecatedFields(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		return &mdmlab.AppConfig{OrgInfo: mdmlab.OrgInfo{OrgName: "MDMlab"}, ServerSettings: mdmlab.ServerSettings{ServerURL: "https://example.org"}}, nil
	}
...
func TestApplyAsGitOps(t *testing.T) {
	...
	var teamEnrollSecrets []*mdmlab.EnrollSecret
	ds.ApplyEnrollSecretsFunc = func(ctx context.Context, teamID *uint, secrets []*mdmlab.EnrollSecret) error {
	...
	ds.GetMDMAppleBootstrapPackageMetaFunc = func(ctx context.Context, teamID uint) (*mdmlab.MDMAppleBootstrapPackage, error) {
		return nil, &notFoundError{}
	}
	ds.InsertMDMAppleBootstrapPackageFunc = func(ctx context.Context, bp *mdmlab.MDMAppleBootstrapPackage, pkgStore mdmlab.MDMBootstrapPackageStore) error {
		return nil
	}
	ds.SetOrUpdateMDMWindowsConfigProfileFunc = func(ctx context.Context, cp mdmlab.MDMWindowsConfigProfile) error {
		return nil
...
`, mobileConfigPath, emptySetupAsst))
	...
	assert.Equal(t, mdmlab.MDM{
		...
		MacOSSettings: mdmlab.MacOSSettings{
			CustomSettings: []mdmlab.MDMProfileSpec{{Path: mobileConfigPath}},
		},
...
`, mobileConfigPath))
	...
	assert.Equal(t, []*mdmlab.EnrollSecret{{Secret: "BBB"}}, teamEnrollSecrets)
	assert.True(t, ds.ApplyEnrollSecretsFuncInvoked)
...
`, bootstrapURL))
	...
	var appliedSecrets []*mdmlab.EnrollSecret
	ds.ApplyEnrollSecretsFunc = func(ctx context.Context, teamID *uint, secrets []*mdmlab.EnrollSecret) error {
	...
	// Apply labels.
	var appliedLabels []*mdmlab.LabelSpec
	...
	// Apply packs.
	var appliedPacks []*mdmlab.PackSpec
	ds.ApplyPackSpecsFunc = func(ctx context.Context, specs []*mdmlab.PackSpec) error {
		appliedPacks = specs
		return nil
	}
	...
	ds.ApplyQueriesFunc = func(ctx context.Context, authorID uint, queries []*mdmlab.Query, queriesToDiscardResults map[uint]struct{}) error {
...
func TestApplyLabels(t *testing.T) {
	...
	ubuntuLabel := &mdmlab.Label{
		ID:                  8,
		Name:                mdmlab.BuiltinLabelNameUbuntuLinux,
		Query:               "select 1 from os_version where platform = 'ubuntu';",
		Description:         "All Ubuntu hosts",
	...
	ds.LabelsByNameFunc = func(ctx context.Context, names []string) (map[string]*mdmlab.Label, error) {
		assert.ElementsMatch(t, []string{mdmlab.BuiltinLabelNameUbuntuLinux}, names)
		return map[string]*mdmlab.Label{
			mdmlab.BuiltinLabelNameUbuntuLinux: ubuntuLabel,
		}, nil
	}
...
`)
	...
	expectedErrMsg := "applying packs: POST /api/latest/mdmlab/spec/packs received status 400 Bad request: pack payload verification: pack scheduled query interval must be an integer greater than 1 and less than 604800"
...
func TestApplyMacosSetup(t *testing.T) {
	...
	setupServer := func(t *testing.T, premium bool) *mock.Store {
		...
		license := &mdmlab.LicenseInfo{Tier: tier, Expiration: time.Now().Add(24 * time.Hour)}
		depStorage := SetupMockDEPStorageAndMockDEPServer(t)
		_, ds := runServerWithMockedDS(t, &service.TestServerOpts{License: license, DEPStorage: depStorage})

		tm1 := &mdmlab.Team{ID: 1, Name: "tm1"}
		teamsByName := map[string]*mdmlab.Team{
			"tm1": tm1,
		}
		teamsByID := map[uint]*mdmlab.Team{
			tm1.ID: tm1,
		}
		ds.NewActivityFunc = func(
			ctx context.Context, user *mdmlab.User, activity mdmlab.ActivityDetails, details []byte, createdAt time.Time,
		...
		ds.ListTeamsFunc = func(ctx context.Context, filter mdmlab.TeamFilter, opt mdmlab.ListOptions) ([]*mdmlab.Team, error) {
			tms := make([]*mdmlab.Team, 0, len(teamsByName))
		...
		mockStore.Lock()
		mockStore.appConfig = &mdmlab.AppConfig{
			OrgInfo:        mdmlab.OrgInfo{OrgName: "MDMlab"},
			ServerSettings: mdmlab.ServerSettings{ServerURL: "https://example.org"},
			MDM:            mdmlab.MDM{EnabledAndConfigured: true},
		...
		ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
			...
			return clone.(*mdmlab.AppConfig), err
		}
		...
		ds.SaveAppConfigFunc = func(ctx context.Context, info *mdmlab.AppConfig) error {
			...
			mockStore.appConfig = clone.(*mdmlab.AppConfig)
			return nil
		}
		...
		asstsByTeam := make(map[uint]*mdmlab.MDMAppleSetupAssistant)
		asstID := 0
		ds.SetOrUpdateMDMAppleSetupAssistantFunc = func(ctx context.Context, asst *mdmlab.MDMAppleSetupAssistant) (*mdmlab.MDMAppleSetupAssistant, error) {
			asstID++
...
`
	...
	t.Run("free license", func(t *testing.T) {
		...
		runAppCheckErr(t, []string{"apply", "-f", name}, `applying mdmlab config: missing or invalid license`)
		...
		runAppCheckErr(t, []string{"apply", "-f", name}, `applying mdmlab config: PATCH /api/latest/mdmlab/config received status 422 Validation Failed: missing or invalid license`)
		assert.False(t, ds.SetOrUpdateMDMAppleSetupAssistantFuncInvoked)
		assert.False(t, ds.GetMDMAppleBootstrapPackageMetaFuncInvoked)
		assert.False(t, ds.InsertMDMAppleBootstrapPackageFuncInvoked)
		assert.False(t, ds.DeleteMDMAppleBootstrapPackageFuncInvoked)
	...
	t.Run("new bootstrap package", func(t *testing.T) {
		cases := []struct {
			...
			{"signed.pkg", nil},
			{"unsigned.pkg", errors.New("applying mdmlab config: Couldn’t edit bootstrap_package. The bootstrap_package must be signed. Learn how to sign the package in the MDMlab documentation: https://mdmlabdm.com/docs/using-mdmlab/mdm-macos-setup-experience#step-2-sign-the-package")},
			{"invalid.tar.gz", errors.New("applying mdmlab config: Couldn’t edit bootstrap_package. The file must be a package (.pkg).")},
			{"wrong-toc.pkg", errors.New("applying mdmlab config: checking package signature: decompressing TOC: unexpected EOF")},
		}
	...
	t.Run("replace bootstrap package", func(t *testing.T) {
		...
		ds.GetMDMAppleBootstrapPackageMetaFunc = func(ctx context.Context, teamID uint) (*mdmlab.MDMAppleBootstrapPackage, error) {
			mockStore.Lock()
			defer mockStore.Unlock()
			return &mdmlab.MDMAppleBootstrapPackage{
				TeamID:    0,
				Name:      pkgName,
...
func TestApplySpecs(t *testing.T) {
	...
	setupDS := func(ds *mock.Store) {
		...
		ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
			return &mdmlab.AppConfig{AgentOptions: &agentOpts}, nil
		}
...
spec:
  server_settings:
    server_url: ""
`,
			wantErr: `422 Validation Failed: MDMlab server URL must be present`,
		},
		{
...
`,
			flags:      []string{"--force"},
			wantOutput: `[+] applied mdmlab config`,
		},
		{
...
`,
			flags: []string{"--dry-run"},
			wantOutput: `[!] ignoring labels, dry run mode only supported for 'config' and 'team' specs
[+] would've applied mdmlab config
  ...
  team:
    ...
    mdm:
      macos_settings:
        enable_disk_encryption: true
`,
			wantErr: `Couldn't update macos_settings because MDM features aren't turned on in MDMlab.`,
		},
		{
...
spec:
  mdm:
    windows_enabled_and_configured: true
`,
			wantErr: `422 Validation Failed: Couldn't turn on Windows MDM. Please configure MDMlab with a certificate and key pair first.`,
		},
		{
...
func TestApplyFileExtensionValidation(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			// Set up arguments
			args := []string{"mdmlabctl", "apply", "-f", tmpFilePath}
