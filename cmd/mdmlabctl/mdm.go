	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			for _, ident := range hostIdents {
				...
				mdmHostPlatform := mdmlab.MDMPlatform(host.Platform)
				...
				if host.MDM.ConnectedToMDMlab == nil || !*host.MDM.ConnectedToMDMlab {
					return errors.New(`Can't run the MDM command because one or more hosts have MDM turned off. Run the following command to see a list of hosts with MDM on: mdmlabctl get hosts --mdm.`)
				}
			...
			if len(hostUUIDs) == 0 || notFoundCount > 0 {
				...
				return errors.New(mdmlab.TargetedHostsDontExistErrMsg)
			}
			...
			if err != nil {
				if errors.Is(err, service.ErrMissingLicense) && mdmPlatform == "windows" {
					return errors.New(mdmlab.WindowsMDMRequiresPremiumCmdMessage)
				}
...
Hosts will run the command the next time they check into MDMlab.

Copy and run this command to see results:

mdmlabctl get mdm-command-results --id=%v
`, result.CommandUUID)
...
func mdmUnlockCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if mdmlab.MDMPlatform(host.Platform) == "darwin" {
				fmt.Fprintf(c.App.Writer, `
...
mdmlabctl get host %s`, hostIdent)
...
func hostMdmActionSetup(c *cli.Context, hostIdent string, actionType string) (client *service.Client, host *service.HostDetailResponse, err error) {
	...
	if err != nil {
		var nfe service.NotFoundErr
		if errors.As(err, &nfe) {
			return nil, nil, errors.New(mdmlab.HostNotFoundErrMsg)
		}
