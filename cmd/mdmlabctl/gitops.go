...
func gitopsCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      "gitops",
		Usage:     "Synchronize MDMlab configuration with provided file. This command is intended to be used in a GitOps workflow.",
		UsageText: `mdmlabctl gitops [options]`,
		Flags: []cli.Flag{
			&cli.StringSliceFlag{
				Name:        "f",
				Required:    true,
		...
		Action: func(c *cli.Context) error {
			...
			// Check license
			mdmlabClient, err := clientFromCLI(c)
			if err != nil {
				return err
			}
			appConfig, err := mdmlabClient.GetAppConfig()
			if err != nil {
				return err
			}
			...
			var teamDryRunAssumptions *mdmlab.TeamSpecsDryRunAssumptions
			...
			allMDMlabSecrets := make(map[string]string)
			for _, flFilename := range flFilenames.Value() {
				baseDir := filepath.Dir(flFilename)
				...
				if isGlobalConfig {
					...
					logf("[!] skipping team config %s since teams are only supported for premium MDMlab users\n", flFilename)
					continue
				}
				...
				if isGlobalConfig && totalFilenames > 1 && !(totalFilenames == 2 && noTeamPresent) && isPremium {
					abmTeams, hasMissingABMTeam, usesLegacyABMConfig, err = checkABMTeamAssignments(config, mdmlabClient)
					if err != nil {
						return err
					}

					vppTeams, hasMissingVPPTeam, err = checkVPPTeamAssignments(config, mdmlabClient)
					if err != nil {
						return err
					}
				...
				if flDryRun {
					incomingSecrets := mdmlabClient.GetGitOpsSecrets(config)
				...
				err = mdmlabClient.SaveEnvSecrets(allMDMlabSecrets, config.MDMlabSecrets, flDryRun)
				if err != nil {
					return err
				}
				assumptions, err := mdmlabClient.DoGitOps(c.Context, config, flFilename, logf, flDryRun, teamDryRunAssumptions, appConfig, teamsSoftwareInstallers, teamsVPPApps, teamsScripts)
				if err != nil {
					return err
				}
			...
			if len(abmTeams) > 0 && hasMissingABMTeam {
				if err = applyABMTokenAssignmentIfNeeded(c, teamNames, abmTeams, originalABMConfig, usesLegacyABMConfig, flDryRun, mdmlabClient); err != nil {
					return err
				}
			}
			if len(vppTeams) > 0 && hasMissingVPPTeam {
				if err = applyVPPTokenAssignmentIfNeeded(c, teamNames, vppTeams, originalVPPConfig, flDryRun, mdmlabClient); err != nil {
					return err
				}
			}
			if flDeleteOtherTeams && appConfig.License.IsPremium() { // skip team deletion for non-premium users
				teams, err := mdmlabClient.ListTeams("")
				if err != nil {
					return err
				}
				for _, team := range teams {
					if !slices.Contains(teamNames, team.Name) {
						...
						if flDryRun {
							...
							if err := mdmlabClient.DeleteTeam(team.ID); err != nil {
								return err
							}
						}
					}
...
func checkABMTeamAssignments(config *spec.GitOps, mdmlabClient *service.Client) (
	...
	if mdm, ok := config.OrgSettings["mdm"]; ok {
		if mdmMap, ok := mdm.(map[string]any); ok {
			...
			if hasLegacyConfig && hasNewConfig {
				return nil, false, false, errors.New(mdmlab.AppleABMDefaultTeamDeprecatedMessage)
			}

			abmToks, err := mdmlabClient.CountABMTokens()
			if err != nil {
...
func applyABMTokenAssignmentIfNeeded(
	...
	flDryRun bool,
	mdmlabClient *service.Client,
) error {
	if usesLegacyConfig && len(abmTeamNames) > 1 {
		return errors.New(mdmlab.AppleABMDefaultTeamDeprecatedMessage)
	}
	...
	if err := mdmlabClient.ApplyAppConfig(appConfigUpdate, mdmlab.ApplySpecOptions{}); err != nil {
		return fmt.Errorf("applying mdmlab config: %w", err)
	}
	return nil
}

func checkVPPTeamAssignments(config *spec.GitOps, mdmlabClient *service.Client) (
...
func applyVPPTokenAssignmentIfNeeded(
	...
	for _, vppTeam := range vppTeamNames {
		if !mdmlab.IsReservedTeamName(vppTeam) && !slices.Contains(teamNames, vppTeam) {
	...
	if err := mdmlabClient.ApplyAppConfig(appConfigUpdate, mdmlab.ApplySpecOptions{}); err != nil {
		return fmt.Errorf("applying mdmlab config for volume_purchasing_program teams: %w", err)
	}
	return nil
}
