...
const (
	standardQueryLibraryUrl   = "https://raw.githubusercontent.com/mdmlabdm/mdmlab/main/docs/01-Using-MDMlab/standard-query-library/standard-query-library.yml"
	...
	licenseError = "License key is invalid. Please check your license key and try again, or run `mdmlabctl preview` without the --license-key option to try MDMlab Free."
...
func newDockerCompose() (dockerCompose, error) {
	...
	return dockerCompose{}, errors.New("`docker compose` is required for the mdmlabctl preview experience.\n\nPlease install `docker compose` (https://docs.docker.com/compose/install/).")
}
...
func previewCommand() *cli.Command {
	return &cli.Command{
		...
		Usage:   "Start a sandbox deployment of the MDMlab server",
		Description: `Start a sandbox deployment of the MDMlab server using Docker and docker compose. Docker tools must be available in the environment.
...
Use the stop and reset subcommands to manage the server and dependencies once started.`,
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				...
				Usage: "License key to enable MDMlab Premium (optional)",
			},
			&cli.StringFlag{
				Name:  tagFlagName,
				Usage: "Run a specific version of MDMlab",
				Value: "latest",
			},
			&cli.StringFlag{
			...
			&cli.StringFlag{
				...
				Usage:  "Use custom local directory for the pulling configuration from mdmlabdm/mdmlab (used for development/testing)",
				Value:  "",
				Hidden: true,
			},
			&cli.BoolFlag{
		...
		Action: func(c *cli.Context) error {
			...
			if previewConfigDir := c.String(previewConfigPathFlagName); previewConfigDir != "" {
				...
				mdmlabBranch := c.String(previewConfigFlagName)
				fmt.Printf("Downloading dependencies from mdmlabdm/mdmlab:%s into %s...\n", mdmlabBranch, previewDir)
				githubHTTPClient := mdmlabhttp.NewGithubClient()
				githubClient := github.NewClient(githubHTTPClient)
				if err := downloadFromMDMlabRepo(
					context.Background(),
					githubHTTPClient,
					githubClient,
			...
			// Make sure directories are writable, otherwise the MDMlab
			...
			if err := os.Setenv("FLEET_VERSION", c.String(tagFlagName)); err != nil {
				return fmt.Errorf("failed to set MDMlab version: %w", err)
			}
			...
			cmd := compose.Command("up", "-d", "--remove-orphans", "mysql01", "redis01", "mdmlab01")
			...
			// Start mdmlab02 (UI server) after mdmlab01 (agent/mdmlabctl server)
			// has finished starting up so that there is no conflict with
			// running database migrations.
			cmd = compose.Command("up", "-d", "--remove-orphans", "mdmlab02")
			cmd.Env = append(os.Environ(), "FLEET_LICENSE_KEY="+licenseKey)
			out, err = cmd.CombinedOutput()
			if err != nil {
				fmt.Println(string(out))
			...
			mdmlabClient, err := service.NewClient(address, true, "", "")
			if err != nil {
				return fmt.Errorf("Error creating MDMlab API client handler: %w", err)
			}

			token, err := mdmlabClient.Setup(email, "Admin", password, "MDMlab for osquery")
			if err != nil {
				switch ctxerr.Cause(err).(type) {
				case service.SetupAlreadyErr:
					// Ignore this error
				default:
					return fmt.Errorf("Error setting up MDMlab: %w", err)
				}
			}
			...
			if err != nil {
				...
				fmt.Println("Configured mdmlabctl in the 'preview' context to avoid overwriting existing config.")
				context = "preview"
			...
			if err := writeConfig(configPath, config); err != nil {
				return fmt.Errorf("Error writing mdmlabctl configuration: %w", err)
			}

			// Create client and get enroll secret
			client, err := unauthenticatedClientFromCLI(c)
			if err != nil {
				return fmt.Errorf("Error making mdmlabctl client: %w", err)
			}

			token, err = client.Login(email, password)
			if err != nil {
				return fmt.Errorf("mdmlabctl login failed: %w", err)
			}
			...
			teamsSoftwareInstallers := make(map[string][]mdmlab.SoftwarePackageResponse)
			teamsScripts := make(map[string][]mdmlab.ScriptResponse)
			teamsVPPApps := make(map[string][]mdmlab.VPPAppResponse)
			_, _, _, _, err = client.ApplyGroup(c.Context, false, specs, ".", logf, nil, mdmlab.ApplyClientSpecOptions{}, teamsSoftwareInstallers, teamsVPPApps, teamsScripts)
			if err != nil {
				return err
			}
			...
			// Docker image used for preview (mdmlabdm/mdmlabctl:latest) is released
			...
			}, mdmlab.ApplySpecOptions{}); err != nil {
			...
			fmt.Println("MDMlab will now log you into the UI automatically.")
			...
			fmt.Println("Preview environment complete. Enjoy using MDMlab!")
...
func previewDirectory() string {
	...
	return filepath.Join(homeDir, ".mdmlab", "preview")
}

// downloadFromMDMlabRepo downloads a specific directory from the mdmlabdm/mdmlab repository.
// It creates files and folders as world writable as this is mainly used for testing.
func downloadFromMDMlabRepo(
	ctx context.Context,
	client *http.Client,
	...
	for _, content := range contents {
		if content.GetType() == "file" {
			...
			return downloadFromMDMlabRepo(ctx,
				client,
				githubClient,
...
func waitStartup() error {
	...
	client := mdmlabhttp.NewClient(mdmlabhttp.WithTLSClientConfig(&tls.Config{InsecureSkipVerify: true}))
...
func checkDocker() error {
	...
	if _, err := exec.LookPath("docker"); err != nil {
		return errors.New("Docker is required for the mdmlabctl preview experience.\n\nPlease install Docker (https://docs.docker.com/get-docker/).")
	}

	// Check running
	if err := exec.Command("docker", "info").Run(); err != nil {
		return errors.New("Please start Docker daemon before running mdmlabctl preview.")
	}
...
func previewStopCommand() *cli.Command {
	return &cli.Command{
		Name:  "stop",
		Usage: "Stop the MDMlab preview server and dependencies",
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				fmt.Println(string(out))
				return fmt.Errorf("Failed to run %s stop for MDMlab server and dependencies", compose)
			}
			...
			fmt.Println("MDMlab preview server and dependencies stopped. Start again with mdmlabctl preview.")
...
func previewResetCommand() *cli.Command {
	return &cli.Command{
		Name:  "reset",
		Usage: "Reset the MDMlab preview server and dependencies",
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				fmt.Println(string(out))
				return fmt.Errorf("Failed to run %s rm -sf for MDMlab server and dependencies.", compose)
			}
			...
			fmt.Println("MDMlab preview server and dependencies reset. Start again with mdmlabctl preview.")
...
// cleanUpSocketFiles cleans up mdmlab-osqueryd's socket file
