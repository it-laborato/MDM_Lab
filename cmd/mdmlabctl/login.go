...
func loginCommand() *cli.Command {
	...
	return &cli.Command{
		Name:  "login",
		Usage: "Login to MDMlab",
		UsageText: `
mdmlabctl login [options]

Interactively prompts for email and password if not specified in the flags or environment variables.

Trying to login with SSO or MFA? First, login to the MDMlab UI and retrieve your API token from the "My account" page. Then set your API token with the mdmlabctl config set --token <your-api-token-here> command. You're now logged in to mdmlabctl.
`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "email",
		...
		Action: func(c *cli.Context) error {
			mdmlab, err := unauthenticatedClientFromCLI(c)
			if err != nil {
				return err
			}
			...
			// CLI history.
			if flEmail == "" {
				fmt.Println("Log in using the standard MDMlab credentials.")
				fmt.Print("Email: ")
			...
			token, err := mdmlab.Login(flEmail, flPassword)
			if err != nil {
			...
			fmt.Printf("[+] MDMlab login successful and context configured!\n")
