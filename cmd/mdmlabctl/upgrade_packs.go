...
func upgradePacksCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mdmlabctl upgrade-packs [options]`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			packsByID := make(map[uint]*mdmlab.Pack, len(packsDB))
...
To import these queries to MDMlab, you can merge the data in the output file with your existing query configuration and run `+"`mdmlabctl apply`"+`.

Note that existing 2017 "Packs" have been left intact. To avoid running duplicate queries on your hosts, visit %s/packs/manage and disable all 2017 "Packs" after upgrading. MDMlab will continue to support these until the next major version release, when 2017 "Packs" will be automatically converted to queries.
...
func writeQuerySpecsToFile(filename string, specs []*mdmlab.QuerySpec) error {
	...
	for _, spec := range specs {
		qYaml := mdmlab.QueryObject{
			ObjectMetadata: mdmlab.ObjectMetadata{
				ApiVersion: mdmlab.ApiVersion,
...
func mapQueriesToPacks(packs []*mdmlab.PackSpec, queries []mdmlab.Query) map[*mdmlab.PackSpec][]*mdmlab.Query {
	queriesByName := make(map[string]*mdmlab.Query, len(queries))
	for _, q := range queries {
		q := q // avoid taking address of iteration var
		queriesByName[q.Name] = &q
	}

	queriesByPack := make(map[*mdmlab.PackSpec][]*mdmlab.Query, len(packs))
...
// converted.
func upgradePackToQueriesSpecs(packSpec *mdmlab.PackSpec, packDB *mdmlab.Pack, packQueries []*mdmlab.Query, ts time.Time) ([]*mdmlab.QuerySpec, int) {
	...
	schedByName := make(map[string]*mdmlab.PackSpecQuery, len(packSpec.Queries))
	...
	for _, pq := range packQueries {
		...
		for _, tm := range packSpec.Targets.Teams {
			...
			newSpecs = append(newSpecs, &mdmlab.QuerySpec{
				Name:               newQueryName,
				Description:        desc,
				Query:              pq.Query,
				TeamName:           tm,
