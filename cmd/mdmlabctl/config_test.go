...
func TestCustomHeadersConfig(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
		require.Equal(t, "custom", r.Header.Get("X-MDMlab-Test"))
		require.Equal(t, "another", r.Header.Get("X-MDMlab-MoreTest"))
	...
	runAppForTest(t, []string{
		...
		"--config", configFile,
		"--token", "abcd",
		"--custom-header", "X-MDMlab-Test:custom",
		"--custom-header", "X-MDMlab-MoreTest:another",
		"--address", srv.URL,
	})
