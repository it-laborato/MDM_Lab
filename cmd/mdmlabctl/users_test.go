...
func TestUserDelete(t *testing.T) {
	...
	ds.NewActivityFunc = func(
		...
		assert.Equal(t, mdmlab.ActivityTypeDeletedUser{}.ActivityName(), activity.ActivityName())
		return nil
	}
...
var _ mdmlab.NotFoundError = (*notFoundError)(nil)
...
// TestUserCreateForcePasswordReset tests that the `mdmlabctl user create` command
...
func TestCreateBulkUsers(t *testing.T) {
	...
	ds.TeamsSummaryFunc = func(ctx context.Context) ([]*mdmlab.TeamSummary, error) {
		team1 := &mdmlab.TeamSummary{
			ID: 1,
		}
		team2 := &mdmlab.TeamSummary{
			ID: 2,
		}
...
func TestDeleteBulkUsers(t *testing.T) {
	...
	for _, user := range csvLines[1:] {
		...
		users = append(users, mdmlab.User{
			Name:  name,
			Email: email,
			ID:    id,
		})
