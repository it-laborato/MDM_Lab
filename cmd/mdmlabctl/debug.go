...
func debugCommand() *cli.Command {
	return &cli.Command{
		Name:  "debug",
		Usage: "Tools for debugging MDMlab",
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
...
func outfileName(name string) string {
	return fmt.Sprintf("mdmlab-%s-%s", name, nowFn().Format("20060102150405Z"))
}
...
func debugProfileCommand() *cli.Command {
	return &cli.Command{
		Name:      "profile",
		Usage:     "Record a CPU profile from the MDMlab server.",
		...
		Action: func(c *cli.Context) error {
			...
			profile, err := mdmlab.DebugPprof("profile")
			if err != nil {
				return err
			}
...
func debugCmdlineCommand() *cli.Command {
	return &cli.Command{
		Name:  "cmdline",
		Usage: "Get the command line used to invoke the MDMlab server.",
		Flags: []cli.Flag{
			outfileFlag(),
			configFlag(),
			contextFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			cmdline, err := mdmlab.DebugPprof("cmdline")
			if err != nil {
				return err
			}
...
func debugHeapCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Report the allocated memory in the MDMlab server.",
		...
		Action: func(c *cli.Context) error {
			...
			profile, err := mdmlab.DebugPprof(name)
			if err != nil {
				return err
			}
...
func debugGoroutineCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Get stack traces of all goroutines (threads) in the MDMlab server.",
...
func debugTraceCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Record an execution trace on the MDMlab server.",
		...
		Action: func(c *cli.Context) error {
			...
			for _, profile := range profiles {
				...
				case "errors":
					...
					ext = jsonExtension
					err = mdmlab.DebugErrors(&buf, false)
					if err == nil {
						res = buf.Bytes()
					}
				...
				case "db-locks":
					ext = jsonExtension
					res, err = mdmlab.DebugDBLocks()
				case "db-innodb-status":
					ext = jsonExtension
					res, err = mdmlab.DebugInnoDBStatus()
				case "db-process-list":
					ext = jsonExtension
					res, err = mdmlab.DebugProcessList()

				default:
					ext = profileExtension
					res, err = mdmlab.DebugPprof(profile)
				}
...
func debugConnectionCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      "connection",
		ArgsUsage: "[<address>]",
		Usage:     "Investigate the cause of a connection failure to the MDMlab server.",
		Description: `Run a number of checks to debug a connection failure to the MDMlab
server.
...
the default context used if none is explicitly specified.`,
		...
		Action: func(c *cli.Context) error {
			...
			} else if cert := getMDMlabCertificate(c); cert != "" {
				return errors.New("the --mdmlab-certificate flag can only be set when an <address> is provided")
			}
			...
			if cc.Address == "" {
				return errors.New(`set the MDMlab API address with: mdmlabctl config set --address https://localhost:8080
or provide an <address> argument to debug: mdmlabctl debug connection localhost:8080`)
			}
			...
			//
			// Scenarios:
			// 	- If a --mdmlab-certificate is provided, use it as root CA.
			// 	- If a --mdmlab-certificate is not provided, but a cc.RootCA is set in the configuration, use it as root CA.
			// 	- If a --mdmlab-certificate is not provided and there isn't a cc.RootCA set in the configuration, use the embedded certs as root CA.
			//
			usingEmbeddedCA := false
			if usingHTTPS {
				certPath := getMDMlabCertificate(c)
				if certPath != "" {
					// if a certificate is provided, use it as root CA
					cc.RootCA = certPath
					cc.TLSSkipVerify = false
				} else if cc.RootCA == "" { // --mdmlab-certificate is not set
					// If a certificate is not provided and a cc.RootCA is not set in the configuration,
					// then use the embedded root CA which is used by osquery to connect to MDMlab.
			...
			if usingHTTPS {
				rootCA := cc.RootCA
				if usingEmbeddedCA {
					rootCA += " (embedded certs used by default to generate mdmlabd packages)"
				}
			...
			if usingHTTPS {
				if err := checkMDMlabCert(c.Context, timeoutPerCheck, cc.RootCA, baseURL.Host); err != nil {
...
func debugMigrations() *cli.Command {
	return &cli.Command{
		...
		Description: `Run a check for database migrations on the mdmlab server.

It returns the list of migrations that are missing.
Such migrations can be applied via "mdmlab prepare db" before running "mdmlab serve".
`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			case mdmlab.NoMigrationsCompleted:
				...
				fmt.Println("Your MDMlab database is not initialized. MDMlab cannot start up.\n" +
					"MDMlab server must be run with \"prepare db\" to perform the migrations.")
...
func debugErrorsCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Save the recorded mdmlab server errors to a file.",
		UsageText: "Recording of errors and their retention period is controlled via the --logging_error_retention_period mdmlab command flag.",
		Flags: []cli.Flag{
			outfileFlag(),
			configFlag(),
			contextFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			if err := mdmlab.DebugErrors(out, flush); err != nil {
				return err
			}
...
func checkMDMlabCert(ctx context.Context, timeout time.Duration, certPath, addr string) error {
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()
