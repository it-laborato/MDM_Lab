...
func clientFromCLI(c *cli.Context) (*service.Client, error) {
	mdmlabClient, err := unauthenticatedClientFromCLI(c)
	if err != nil {
		return nil, err
	}
	...
	if !c.IsSet("config") && flag.Lookup("test.v") != nil {
		mdmlabClient.SetToken("AAAA")
		return mdmlabClient, nil
	}
	...
	token, ok := t.(string)
	if !ok {
		fmt.Fprintln(os.Stderr, "Token invalid. Please log in with: mdmlabctl login")
		return nil, fmt.Errorf("token config value expected type %T, got %T: %+v", "", t, t)
	}
	if token == "" {
		fmt.Fprintln(os.Stderr, "Token missing. Please log in with: mdmlabctl login")
		return nil, errors.New("token config value missing")
	}
	mdmlabClient.SetToken(token)

	// Check if version matches mdmlab server. Also ensures that the token is valid.
	clientInfo := version.Version()

	serverInfo, err := mdmlabClient.Version()
	if err != nil {
		if errors.Is(err, service.ErrUnauthenticated) {
			fmt.Fprintln(os.Stderr, "Token invalid or session expired. Please log in with: mdmlabctl login")
		}
		return nil, err
	}
	...
	switch appCfg, err := mdmlabClient.GetAppConfig(); {
	case err == nil:
		if appCfg.MDM.AppleBMTermsExpired {
			mdmlab.WriteAppleBMTermsExpiredBanner(os.Stderr)
			// This is just a warning, continue ...
		}
...
func unauthenticatedClientFromConfig(cc Context, debug bool, outputWriter io.Writer, errWriter io.Writer) (*service.Client, error) {
	...
	if cc.Address == "" {
		return nil, errors.New("set the MDMlab API address with: mdmlabctl config set --address https://localhost:8080")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error creating MDMlab API client handler: %w", err)
	}
...
func rawHTTPClientFromConfig(cc Context) (*http.Client, *url.URL, error) {
	...
	cli := mdmlabhttp.NewClient(mdmlabhttp.WithTLSClientConfig(&tls.Config{
...
// apiCommand mdmlabctl api [options] uri
// -F, --field <key=value>
...
func apiCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mdmlabctl api [options] [url]`,
		Flags: []cli.Flag{
			&cli.StringSliceFlag{
				Name:    "F",
		...
		Action: func(c *cli.Context) error {
			...
			if !strings.HasPrefix(uriString, "/api/v1/mdmlab") {
				uriString = fmt.Sprintf("/api/v1/mdmlab%s", uriString)
			}
			...
			resp, err := mdmlabClient.AuthenticatedDoCustomHeaders(method, uriString, params.Encode(), nil, headers)
			if err != nil {
				return err
			}
