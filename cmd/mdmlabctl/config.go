...
func configFlag() cli.Flag {
	...
	defaultConfigPath := filepath.Join(homeDir, ".mdmlab", "config")
	return &cli.StringFlag{
		Name:    "config",
		Value:   defaultConfigPath,
		EnvVars: []string{"CONFIG"},
		Usage:   "Path to the mdmlabctl config file",
	}
}
...
func contextFlag() cli.Flag {
	return &cli.StringFlag{
		...
		Usage:   "Name of mdmlabctl config context to use",
	}
}
...
func configSetCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mdmlabctl config set [options]`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			&cli.StringFlag{
				...
				Usage:       "Address of the MDMlab server",
			},
			&cli.StringFlag{
				Name:        "email",
				...
				Value:       "",
				Destination: &flEmail,
				Usage:       "Email to use when connecting to the MDMlab server",
			},
			&cli.StringFlag{
				Name:        "token",
			...
			&cli.StringFlag{
				...
				Usage:       "Specify RootCA chain used to communicate with MDMlab",
			},
			&cli.StringFlag{
				Name:        "url-prefix",
				...
				Usage:       "Specify URL Prefix to use with MDMlab server (copy from server configuration)",
			},
			&cli.StringSliceFlag{
				Name:        "custom-header",
...
func configGetCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: `mdmlabctl config get [options]`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
		},
