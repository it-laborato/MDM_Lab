...
func printLabel(c *cli.Context, label *mdmlab.LabelSpec) error {
	spec := specGeneric{
		Kind:    mdmlab.LabelKind,
		Version: mdmlab.ApiVersion,
		Spec:    label,
	}
...
func printQuerySpec(c *cli.Context, query *mdmlab.QuerySpec) error {
	spec := specGeneric{
		Kind:    mdmlab.QueryKind,
		Version: mdmlab.ApiVersion,
		Spec:    query,
	}
...
func printPack(c *cli.Context, pack *mdmlab.PackSpec) error {
	spec := specGeneric{
		Kind:    mdmlab.PackKind,
		Version: mdmlab.ApiVersion,
		Spec:    pack,
	}
...
func printSecret(c *cli.Context, secret *mdmlab.EnrollSecretSpec) error {
	spec := specGeneric{
		Kind:    mdmlab.EnrollSecretKind,
		Version: mdmlab.ApiVersion,
		Spec:    secret,
	}
...
func printHost(c *cli.Context, host *mdmlab.HostResponse) error {
	spec := specGeneric{
		Kind:    mdmlab.HostKind,
		Version: mdmlab.ApiVersion,
		Spec:    host,
	}
...
type enrichedAppConfigPresenter mdmlab.EnrichedAppConfig
...
func usersToUserRoles(users []mdmlab.User) UserRoles {
...
func printUserRoles(c *cli.Context, users []mdmlab.User) error {
	spec := specGeneric{
		Kind:    mdmlab.UserRolesKind,
		Version: mdmlab.ApiVersion,
...
func printTeams(c *cli.Context, teams []mdmlab.Team) error {
	for _, team := range teams {
		var teamItem interface{} = team
		if c.Bool(yamlFlagName) {
			teamSpec, err := mdmlab.TeamSpecFromTeam(&team)
			if err != nil {
				return err
			}
			teamItem = teamSpec
...
func queryToTableRow(query mdmlab.Query, teamName string) []string {
	platform := "all"
	if query.Platform != "" {
	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if name == "" {
				...
				if me == nil {
					return errors.New("/api/latest/mdmlab/me returned an empty user")
				}
				...
				if c.Bool(yamlFlagName) || c.Bool(jsonFlagName) {
					for _, query := range queries {
						if err := printQuerySpec(c, &mdmlab.QuerySpec{
							Name:        query.Name,
...
func userIsObserver(user mdmlab.User) (bool, error) {
	if user.GlobalRole != nil {
		return *user.GlobalRole == mdmlab.RoleObserver || *user.GlobalRole == mdmlab.RoleObserverPlus, nil
	} // Team user
	if len(user.Teams) == 0 {
		return false, errUserNoRoles
	}
	for _, team := range user.Teams {
		if team.Role != mdmlab.RoleObserver && team.Role != mdmlab.RoleObserverPlus {
			return false, nil
		}
	}
	return true, nil
...
func getPacksCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			addQueries := func(pack *mdmlab.PackSpec) {
...
func getAppConfigCommand() *cli.Command {
	return &cli.Command{
		Name:  "config",
		Usage: "Retrieve the MDMlab app configuration",
		Flags: []cli.Flag{
			jsonFlag(),
			yamlFlag(),
			configFlag(),
...
func getHostsCommand() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				Name:  "mdm",
				Usage: "Filters hosts by hosts that have MDM turned on in MDMlab and are connected to MDMlab's MDM server.",
			},
			&cli.BoolFlag{
				Name:  "mdm-pending",
				Usage: "Filters hosts by hosts ordered via Apple Business Manager (ABM). These will automatically enroll to MDMlab and turn on MDM when they're unboxed.",
			},
		},
		Action: func(c *cli.Context) error {
			client, err := clientFromCLI(c)
			...
			if identifier == "" {
				...
				if c.Bool("mdm") || c.Bool("mdm-pending") {
					...
					if c.Bool("mdm") {
						query.Set("connected_to_mdmlab", "true")
					}
					if c.Bool("mdm-pending") {
						// hosts pending enrollment in MDMlab's MDM server
						query.Set("mdm_name", mdmlab.WellKnownMDMMDMlab)
						query.Set("mdm_enrollment_status", string(mdmlab.MDMEnrollStatusPending))
					}
				}
...
func getCarvesCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			carves, err := client.ListCarves(mdmlab.CarveListOptions{Expired: expired})
			if err != nil {
				return err
			}
...
func getTeamsYAMLFlag() cli.Flag {
	return &cli.BoolFlag{
		Name:  yamlFlagName,
		Usage: "Output team configuration in yaml format. Intended for use with \"mdmlabctl apply -f\"",
	}
}
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				...
				if errors.As(err, &nfe) {
					log(c, "Error: No Apple Push Notification service (APNs) certificate found. Use `mdmlabctl generate mdm-apple` and then `mdmlab serve` with `mdm` configuration to turn on MDM features.\n")
					return nil
				}
			...
			if mdm.RenewDate.Before(time.Now()) {
				...
				color.New(color.FgRed).Fprintln(c.App.Writer, "\nERROR: Your Apple Push Notification service (APNs) certificate is expired. MDM features are turned off. To renew your APNs certificate, follow these instructions: https://mdmlabdm.com/learn-more-about/renew-apns")
			} else if mdm.RenewDate.Before(warnDate) {
				// certificate will soon expire, print a warning
				color.New(color.FgYellow).Fprintln(c.App.Writer, "\nWARNING: Your Apple Push Notification service (APNs) certificate is less than 30 days from expiration. If it expires, MDM features will be turned off. To renew your APNs certificate, follow these instructions: https://mdmlabdm.com/learn-more-about/renew-apns")
			}
	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				var nfe service.NotFoundErr
				if errors.As(err, &nfe) {
					log(c, "Error: No Apple Business Manager server token found. Use `mdmlabctl generate mdm-apple-bm` and then `mdmlab serve` with `mdm` configuration to automatically enroll macOS hosts to MDMlab.\n")
					return nil
				}
			...
			if bm.RenewDate.Before(time.Now()) {
				// certificate is expired, print an error
				color.New(color.FgRed).Fprintln(c.App.Writer, "\nERROR: Your Apple Business Manager (ABM) server token is expired. Laptops newly purchased via ABM will not automatically enroll in MDMlab. To renew your ABM server token, follow these instructions: https://mdmlabdm.com/docs/using-mdmlab/faq#how-can-i-renew-my-apple-business-manager-server-token")
			} else if bm.RenewDate.Before(warnDate) {
				// certificate will soon expire, print a warning
				color.New(color.FgYellow).Fprintln(c.App.Writer, "\nWARNING: Your Apple Business Manager (ABM) server token is less than 30 days from expiration. If it expires, laptops newly purchased via ABM will not automatically enroll in MDMlab. To renew your ABM server token, follow these instructions: https://mdmlabdm.com/docs/using-mdmlab/faq#how-can-i-renew-my-apple-business-manager-server-token")
			}
	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				var nfe service.NotFoundErr
				if errors.As(err, &nfe) {
					return errors.New("The command doesn't exist. Please provide a valid command ID. To see a list of commands that were run, run `mdmlabctl get mdm-commands`.")
				}
...
func getMDMCommandsCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			opts := mdmlab.MDMCommandListOptions{
			...
			if err != nil {
				if strings.Contains(err.Error(), mdmlab.HostIdentiferNotFound) {
					return errors.New(mdmlab.HostIdentiferNotFound)
				}
				return err
			}
			if len(results) == 0 && opts.Filters.HostIdentifier == "" && opts.Filters.RequestType == "" {
				log(c, "You haven't run any MDM commands. Run MDM commands with the `mdmlabctl mdm run-command` command.\n")
				return nil
			}
