...
// Launch is the entrypoint that sets up and runs the MDMlab commands.
func main() {
...
func createRootCmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		...
		Long: `MDMlab server (https://mdmlabdm.com)
...
func applyDevFlags(cfg *config.MDMlabConfig) {
	cfg.Mysql.Username = "mdmlab"
	cfg.Mysql.Database = "mdmlab"
	cfg.Mysql.Password = "insecure"

	if cfg.Prometheus.BasicAuth.Username == "" {
		cfg.Prometheus.BasicAuth.Username = "mdmlab"
	}
...
func initLogger(cfg config.MDMlabConfig) kitlog.Logger {
