...
import (
	...
	eewebhooks "github.com/it-laborato/MDM_Lab/ee/server/webhooks"
	...
	"github.com/it-laborato/MDM_Lab/server/policies"
	...
	"github.com/it-laborato/MDM_Lab/server/vulnerabilities/customcve"
	"github.com/it-laborato/MDM_Lab/server/vulnerabilities/goval_dictionary"
	...
	"github.com/it-laborato/MDM_Lab/server/webhooks"
...
func newVulnerabilitiesSchedule(
	...
	const name = string(mdmlab.CronVulnerabilities)
...
func scanVulnerabilities(
	...
	vulns := make([]mdmlab.SoftwareVulnerability, 0, len(nvdVulns)+len(ovalVulns)+len(macOfficeVulns))
...
func checkWinVulnerabilities(
	...
	config *config.VulnerabilitiesConfig,
	collectVulns bool,
) []mdmlab.OSVulnerability {
	var results []mdmlab.OSVulnerability
...
func checkOvalVulnerabilities(
	...
	var results []mdmlab.SoftwareVulnerability
...
func newAutomationsSchedule(
	...
	failingPoliciesSet mdmlab.FailingPolicySet,
) (*schedule.Schedule, error) {
	const (
		name            = string(mdmlab.CronAutomations)
...
func triggerFailingPoliciesAutomation(
	...
	err = policies.TriggerFailingPoliciesAutomation(ctx, ds, logger, failingPoliciesSet, func(policy *mdmlab.Policy, cfg policies.FailingPolicyAutomationConfig) error {
...
func newWorkerIntegrationsSchedule(
	...
	const (
		name = string(mdmlab.CronWorkerIntegrations)
	...
	// depStorage could be nil if mdm is not configured for mdmlab, in which case
	...
	// configuration if we're not on a mdmlab-owned server. Technically, the ServerURL
	...
	s := schedule.New(
		...
		schedule.WithJob("integrations_worker", func(ctx context.Context) error {
			...
			jira.MDMlabURL = appConfig.ServerSettings.ServerURL
			zendesk.MDMlabURL = appConfig.ServerSettings.ServerURL
...
func newCleanupsAndAggregationSchedule(
	...
	const (
		name            = string(mdmlab.CronCleanupsThenAggregation)
...
func newFrequentCleanupsSchedule(
	...
	const (
		name            = string(mdmlab.CronFrequentCleanups)
...
func newUsageStatisticsSchedule(ctx context.Context, instanceID string, ds mdmlab.Datastore, config config.MDMlabConfig, license *mdmlab.LicenseInfo, logger kitlog.Logger) (*schedule.Schedule, error) {
	const (
		name            = string(mdmlab.CronUsageStatistics)
		defaultInterval = 1 * time.Hour
	)
	s := schedule.New(
		ctx, name, instanceID, defaultInterval, ds, ds,
		...
		schedule.WithJob(
			"try_send_statistics",
			func(ctx context.Context) error {
				// NOTE(mna): this is not a route from the mdmlab server (not in server/service/handler.go) so it
				// will not automatically support the /latest/ versioning. Leaving it as /v1/ for that reason.
				return trySendStatistics(ctx, ds, mdmlab.StatisticsFrequency, "https://mdmlabdm.com/api/v1/webhooks/receive-usage-analytics", config)
			},
		),
	)
...
func trySendStatistics(ctx context.Context, ds mdmlab.Datastore, frequency time.Duration, url string, config config.MDMlabConfig) error {
...
func newAppleMDMDEPProfileAssigner(
	...
	const name = string(mdmlab.CronAppleMDMDEPProfileAssigner)
...
func appleMDMDEPSyncerJob(
	...
	var mdmlabSyncer *apple_mdm.DEPService
	return func(ctx context.Context) error {
		...
		if mdmlabSyncer == nil {
			mdmlabSyncer = apple_mdm.NewDEPService(ds, depStorage, logger)
		}

		return mdmlabSyncer.RunAssigner(ctx)
	}
}
...
func newAppleMDMProfileManagerSchedule(
	...
	const (
		name = string(mdmlab.CronMDMAppleProfileManager)
...
func newWindowsMDMProfileManagerSchedule(
	...
	const (
		name = string(mdmlab.CronMDMWindowsProfileManager)
		// Note: per a request from #g-product we are running this cron
		// every 30 seconds, we should re-evaluate how we handle the
		// cron interval as we scale to more hosts.
		defaultInterval = 30 * time.Second
...
func cleanupCronStatsOnShutdown(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger, instanceID string) {
	if err := ds.UpdateAllCronStatsForInstance(ctx, instanceID, mdmlab.CronStatsStatusPending, mdmlab.CronStatsStatusCanceled); err != nil {
...
func newActivitiesStreamingSchedule(
	...
	ds mdmlab.Datastore,
	logger kitlog.Logger,
	auditLogger mdmlab.JSONLogger,
) (*schedule.Schedule, error) {
	const (
		name     = string(mdmlab.CronActivitiesStreaming)
...
func cronActivitiesStreaming(
	...
	for {
		...
		activitiesToStream, _, err := ds.ListActivities(ctx, mdmlab.ListActivitiesOptions{
...
// detail interval of all (osquery-capable) hosts in MDMlab).
...
// Once all customers are using on MDMlab 4.57 or later, this job can be removed.
func cronUninstallSoftwareMigration(
	...
	const (
		name            = string(mdmlab.CronUninstallSoftwareMigration)
		defaultInterval = 24 * time.Hour
	)
...
func newMaintainedAppSchedule(
	...
	const (
		name            = string(mdmlab.CronMaintainedApps)
		defaultInterval = 24 * time.Hour
	...
	s := schedule.New(
		ctx, name, instanceID, defaultInterval, ds, ds,
		schedule.WithLogger(logger),
		// ensures it runs a few seconds after MDMlab is started
