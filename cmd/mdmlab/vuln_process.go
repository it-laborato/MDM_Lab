...
func createVulnProcessingCmd(configManager config.Manager) *cobra.Command {
	vulnProcessingCmd := &cobra.Command{
		Use:   "vuln_processing",
		Short: "Run the vulnerability processing features of MDMlab",
		Long: `The vuln_processing command is intended for advanced configurations that want to externally manage 
vulnerability processing. By default the MDMlab server command internally manages vulnerability processing via scheduled
...
by an exit code of zero.`,
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			logger = kitlog.With(logger, mdmlab.CronVulnerabilities)
			...
			lock, err := ds.Lock(ctx, string(mdmlab.CronVulnerabilities), "vuln_processing_command", lockDuration)
			if err != nil {
			...
			defer func() {
				uerr := ds.Unlock(ctx, string(mdmlab.CronVulnerabilities), "vuln_processing_command")
				if uerr != nil {
...
func configureVulnPath(vulnConfig config.VulnerabilitiesConfig, appConfig *mdmlab.AppConfig, logger kitlog.Logger) (vulnPath string) {
	switch {
	case vulnConfig.DatabasesPath != "" && appConfig != nil && appConfig.VulnerabilitySettings.DatabasesPath != "":
		...
		level.Info(logger).Log(
			"msg", "mdmlab config takes precedence over app config when both are configured",
...
func getVulnFuncs(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger, config *config.VulnerabilitiesConfig) []NamedVulnFunc {
