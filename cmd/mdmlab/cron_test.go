...
func TestMigrateABMTokenDuringDEPCronJob(t *testing.T) {
	...
	const tokenOrgName = "mdmlab"

	// insert an ABM token as if it had been migrated by the DB migration script
	tok := mysql.SetTestABMAssets(t, ds, "")
	// tok, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{EncryptedToken: abmToken, RenewAt: time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC)})
	// require.NoError(t, err)
	...
	defProf, err := ds.GetMDMAppleEnrollmentProfileByType(ctx, mdmlab.MDMAppleEnrollmentTypeAutomatic)
	require.NoError(t, err)
	...
	teams, err := ds.ListTeams(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.ListOptions{})
	require.NoError(t, err)
