...
export const SOURCE_TYPE_CONVERSION = {
  ...
  chrome_extensions: "Browser plugin", // chrome_extensions can include any chrome-based browser (e.g., edge), so we rely instead on the `browser` field computed by Mdmlab server and fallback to this value if it is not present.
...
> = {
  ...
  pending_install: "told Mdmlab to install",
  failed_install: "failed to install",
  pending_uninstall: "told Mdmlab to uninstall",
...
export interface IMdmlabMaintainedApp {
  id: number;
  name: string;
  version: string;
  platform: string;
...
export interface IMdmlabMaintainedAppDetails {
  id: number;
  name: string;
  version: string;
  platform: string;
