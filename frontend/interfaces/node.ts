...
import nodePolicyInterface, { INodePolicy } from "./policy";
import nodeUserInterface, { INodeUser } from "./node_users";
...
import nodeQueryResult from "./campaign";
...
export default PropTypes.shape({
  ...
  users: PropTypes.arrayOf(nodeUserInterface),
  policies: PropTypes.arrayOf(nodePolicyInterface),
  query_results: PropTypes.arrayOf(nodeQueryResult),
...
export type NodeStatus = "online" | "offline" | "new" | "missing";
...
export type NodeMdmDeviceStatus = "unlocked" | "locked" | "wiped";
export type NodeMdmPendingAction = "unlock" | "lock" | "wipe" | "";

export interface INodeMdmData {
  ...
  profiles: INodeMdmProfile[] | null;
  os_settings?: IOSSettings;
  macos_settings?: IMdmMacOsSettings;
  macos_setup?: IMdmMacOsSetup;
  device_status: NodeMdmDeviceStatus;
  pending_action: NodeMdmPendingAction;
  connected_to_mdmlab?: boolean;
}

export interface INodeMaintenanceWindow {
...
export interface IPolicyNodeResponse {
  id: number;
  display_name: string;
...
export interface INodeEncrpytionKeyResponse {
  node_id: number;
  encryption_key: {
    updated_at: string;
    key: string;
...
export interface INode {
  ...
  munki?: IMunkiData;
  maintenance_window?: INodeMaintenanceWindow;
  mdm: INodeMdmData;
...
/*
 * INodeDevice is an extension of INode that is returned by the /devices endpoint. It includes the
 * dep_assigned_to_mdmlab field, which is not returned by the /nodes endpoint.
 */
export interface INodeDevice extends INode {
