...
const ENDPOINT = "/latest/mdmlab"; // modify the ENDPOINT string to correspond to your API spec
...
const REQUEST_RESPONSE_MAPPINGS: IResponses = {
  GET: {
    ...
    "/nodes?page=0&per_page=20&order_key=display_name&order_direction=asc":
      RESPONSES.ALL_HOSTS,
    // this basic path only matches with '1337' as the value for the team id query param
    "/nodes?page=0&per_page=20&order_key=display_name&order_direction=asc&team_id=1337":
      RESPONSES.HOSTS_TEAM_1337,
    // this wildcard path matches with any other value for the team id query param
    "/nodes?page=0&per_page=20&order_key=display_name&order_direction=asc&team_id={team_id}":
      RESPONSES.HOSTS_TEAM_ID,
    // this basic path only matches with '1337' as the value for the node id route param
    "/nodes/1337": RESPONSES.HOST_1337,
    // this wildcard path matches with any other value for the node id route param
    "/nodes/*id": RESPONSES.HOST_ID,
    // this wildcard path matches with any value for the node id route param
    "/nodes/:id/device_mapping": RESPONSES.DEVICE_MAPPING,
    // this wildcard path matches with any value for the node id route param
    "nodes/{*}/macadmins": RESPONSES.MACADMINS,
    // this is a basic path with no wildcards
    "nodes/count": {
      count: 1,
    },
