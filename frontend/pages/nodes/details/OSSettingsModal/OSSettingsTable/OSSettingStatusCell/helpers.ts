...
const APPLE_PROFILE_VERIFIED_DISPLAY_CONFIG: ProfileDisplayOption = {
  ...
  tooltip: (innerProps) =>
    innerProps.isDiskEncryptionProfile
      ? "The node turned disk encryption on and sent the key to Mdmlab. " +
        "Mdmlab verified."
      : "The node applied the setting. Mdmlab verified.",
} as const;
...
const MAC_PROFILE_VERIFYING_DISPLAY_CONFIG: ProfileDisplayOption = {
  ...
  tooltip: (innerProps) =>
    innerProps.isDiskEncryptionProfile
      ? "The node acknowledged the MDM command to turn on disk encryption. " +
        "Mdmlab is verifying with osquery and retrieving the disk encryption key. " +
        "This may take up to one hour."
      : "The node acknowledged the MDM command to apply the setting. Mdmlab is " +
...
export const PROFILE_DISPLAY_CONFIG: ProfileDisplayConfig = {
  install: {
    ...
    pending: {
      ...
      tooltip: (innerProps) =>
        innerProps.isDiskEncryptionProfile
          ? "The nodes will receive the MDM command to turn on disk encryption " +
            "when the nodes come online."
          : "The node will receive the MDM command to apply the setting when the " +
            "node comes online.",
    },
    action_required: {
  ...
  remove: {
    pending: {
      ...
      tooltip: (innerProps) =>
        innerProps.isDiskEncryptionProfile
          ? "The node will receive the MDM command to remove the disk encryption profile when the " +
            "node comes online."
          : "The node will receive the MDM command to remove the setting when the node " +
            "comes online.",
    },
...
export const WINDOWS_DISK_ENCRYPTION_DISPLAY_CONFIG: WindowsDiskEncryptionDisplayConfig = {
  verified: {
    ...
    iconName: "success",
    tooltip: () =>
      "The node turned disk encryption on and sent the key to Mdmlab. Mdmlab verified.",
  },
  verifying: {
    statusText: "Verifying",
    iconName: "success-outline",
    tooltip: () =>
      "The node acknowledged the MDM command to turn on disk encryption. Mdmlab is verifying with " +
  ...
  pending: {
    ...
    iconName: "pending-outline",
    tooltip: () =>
      "The node will receive the MDM command to turn on disk encryption when the node comes online.",
  },
  failed: {
    statusText: "Failed",
    iconName: "error",
