...
import {
  ...
  SoftwareSource,
  formatSoftwareType,
  hasNodeSoftwareAppLastInstall,
  hasNodeSoftwarePackageLastInstall,
...
interface ISoftwareDetailsModalProps {
  ...
  onExit: () => void;
  // install details will not be shown if Mdmlab doesn't have them, regardless of this setting
...
const InstallDetailsContent = ({
  ...
  if (hasNodeSoftwareAppLastInstall(software)) {
    return (
      <AppInstallDetails
  ...
  } else if (hasNodeSoftwarePackageLastInstall(software)) {
    return (
...
const SoftwareDetailsModal = ({
  ...
  const hasLastInstall =
    hasNodeSoftwarePackageLastInstall(software) ||
    hasNodeSoftwareAppLastInstall(software);
  return (
    <Modal title={software.name} className={baseClass} onExit={onExit}>
      <>
        {hasLastInstall && !hideInstallDetails ? (
          <TabsContent nodeDisplayName={nodeDisplayName} software={software} />
        ) : (
          <SoftwareDetailsContent software={software} />
        )}
