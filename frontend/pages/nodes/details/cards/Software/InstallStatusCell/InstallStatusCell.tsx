...
import { INodeSoftware, SoftwareInstallStatus } from "interfaces/software";
...
> = {
  installed: {
    ...
    tooltip: ({ isAppStoreApp }) =>
      isAppStoreApp
        ? "The node acknowledged the MDM command to install App Store app."
  ...
  pending_install: {
    ...
    tooltip: () =>
      "Mdmlab is installing or will install when the node comes online.",
  },
  pending_uninstall: {
    iconName: "pending-outline",
    ...
    tooltip: () => (
      <>
        Mdmlab is uninstalling or will uninstall
        <br />
        software when the node comes online.
      </>
    ),
  },
  failed_install: {
    ...
    tooltip: () => (
      <>
        The node failed to install software.
        <br />
  ...
  failed_uninstall: {
    ...
    tooltip: () => (
      <>
        The node failed to uninstall software.
        <br />
  ...
  avaiableForInstall: {
    ...
    tooltip: ({ softwareName, isAppStoreApp }) =>
      isAppStoreApp ? (
        <>
          App Store app can be installed on the node. Select{" "}
          <b>Actions {">"} Install</b> to install.
        </>
      ) : (
        <>
          {softwareName ? <b>{softwareName}</b> : "Software"} can be installed
          on the node. Select <b>Actions {">"} Install</b> to install.
        </>
      ),
  },
  selfService: {
    ...
    tooltip: ({ softwareName }) => (
      <>
        ...
        the node. End users can install from{" "}
        <b>Mdmlab Desktop {">"} Self-service</b>.
      </>
    ),
  },
};

type IInstallStatusCellProps = INodeSoftware;
