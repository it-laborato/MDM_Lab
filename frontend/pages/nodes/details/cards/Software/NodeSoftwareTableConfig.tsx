...
import { getDropdownOptionTooltipContent } from "../../NodeDetailsPage/NodeActionsDropdown/helpers";
...
type ISoftwareTableConfig = Column<INodeSoftware>;
type ITableHeaderProps = IHeaderProps<INodeSoftware>;
type ITableNumberCellProps = INumberCellProps<INodeSoftware>;
type ITableStringCellProps = IStringCellProps<INodeSoftware>;
type IInstalledStatusCellProps = CellProps<
  INodeSoftware,
  INodeSoftware["status"]
>;
type IInstalledVersionsCellProps = CellProps<
  INodeSoftware,
  INodeSoftware["installed_versions"]
>;
...
export const generateActions = ({
  ...
  if (hideActions) {
    ...
    // if node's scripts are disabled, and this isn't a VPP app, disable
    // install/uninstall with tooltip
    if (!nodeScriptsEnabled && !app_store_app) {
    ...
    // user has software write permission for node
  ...
  if (app_store_app) {
    ...
    if (!nodeMDMEnrolled) {
      ...
      actions[indexInstallAction].tooltipContent =
        "To install, turn on MDM for this node.";
    }
  }
  return actions;
};
...
interface ISoftwareTableHeadersProps {
  ...
  teamId: number;
  onSelectAction: (software: INodeSoftware, action: string) => void;
  nodeMDMEnrolled?: boolean;
}
