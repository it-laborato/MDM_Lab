...
const ReportUpdatedCell = ({
  ...
  const renderCellValue = () => {
    ...
    if (interval) {
      if (discard_data && automations_enabled) {
        // this is also the only case where the row is NOT clickable with a link to the node's HQR
              ...
              return (
                <>
                  ...
                  >
                    {
                      <>
                        Results from this query are not reported in Mdmlab.
                        <br />
      ...
      // Query is scheduled to run on node, but hasn't yet
      if (!last_fetched) {
            ...
            formatter={(val) => (
              <>
                ...
                <ReactTooltip
                  ...
                  place="top"
                >
                  Mdmlab is collecting query results.
                  <br />
                  Check back later.
                </ReactTooltip>
              </>
    ...
    return (
      <>
        <TextCell
          ...
          formatter={HumanTimeDiffWithMdmlabLaunchCutoff}
          className={`${baseClass}__value`}
        />
      </>
    );
  ...
  return (
    <span className={baseClass}>
      {renderCellValue()}
      {should_link_to_hqr && nodeId && queryId && (
        // parent row has same onClick functionality but link here is required for keyboard accessibility
        <Link
          className={`${baseClass}__link`}
          title="link to node query report"
          to={PATHS.HOST_QUERY_REPORT(nodeId, queryId)}
        >
