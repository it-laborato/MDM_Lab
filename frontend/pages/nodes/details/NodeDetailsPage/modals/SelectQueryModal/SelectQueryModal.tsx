...
const SelectQueryModal = ({
  ...
  >(
    [
      {
        scope: "queries",
        teamId: nodeTeamId || API_ALL_TEAMS_ID,
        mergeInherited: nodeTeamId !== API_ALL_TEAMS_ID,
      },
    ],
  ...
  const onQueryNodeSaved = (selectedQuery: ISchedulableQuery) => {
    setSelectedQueryTargetsByType(DEFAULT_TARGETS_BY_TYPE);
    router.push(
  ...
  const isNodesTeamObserverPlus = currentUser
    ? permissions.isObserverPlus(currentUser, nodeTeamId)
    : false;

  const [queriesFilter, setQueriesFilter] = useState("");

  if (isOnlyObserver && !isObserverPlus && !isNodesTeamObserverPlus) {
  ...
  const renderDescription = (): JSX.Element => {
    return (
      <div className={`${baseClass}__description`}>
        Choose a query to run on this node
        {(!isOnlyObserver || isObserverPlus || isNodesTeamObserverPlus) && (
          <>
            {" "}
            or{" "}
            <Button variant="text-link" onClick={onQueryNodeCustom}>
        ...
        queriesFiltered?.map((query) => {
          return (
            <Button
              ...
              onClick={() => onQueryNodeSaved(query)}
            >
              <>
