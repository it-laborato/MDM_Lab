...
import { upcomingActivityComponentMap } from "pages/nodes/details/cards/Activity/ActivityConfig";
...
interface ICancelActivityModalProps {
  nodeId: number;
  activity: INodeUpcomingActivity;
  onCancel: () => void;
}
...
const CancelActivityModal = ({
  ...
  const onCancelActivity = async () => {
    try {
      await activitiesAPI.cancelNodeActivity(nodeId, activity.uuid);
