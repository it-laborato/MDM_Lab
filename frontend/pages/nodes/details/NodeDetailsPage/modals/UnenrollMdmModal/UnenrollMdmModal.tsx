...
const UnenrollMdmModal = ({ nodeId, onClose }: IUnenrollMdmModalProps) => {
  ...
  const submitUnenrollMdm = async () => {
    ...
    try {
      await mdmAPI.unenrollNodeFromMdm(nodeId, 5000);
      renderFlash(
        "success",
        "Turning off MDM or will turn off when the node comes online."
      );
      onClose();
    ...
    return (
      <>
        <p className={`${baseClass}__description`}>
          Settings configured by Mdmlab will be removed.
          <br />
          <br />
