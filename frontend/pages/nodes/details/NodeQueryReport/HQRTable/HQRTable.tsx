...
const DEFAULT_CSV_TITLE = "Node-Specific Query Report";
...
const HQRTable = ({
    ...
    const onExportQueryResults = (evt: React.MouseEvent<HTMLButtonElement>) => {
      ...
      FileSaver.saveAs(
        generateCSVQueryResults(
          ...
          generateCSVFilename(
            queryName && nodeName
              ? `'${queryName}' query report results for node '${nodeName}'`
              : DEFAULT_CSV_TITLE
          ),
          columnConfigs,
          true
  ...
  }, [onShowQuery, filteredResults, queryName, nodeName, columnConfigs]);
    ...
    if (!lastFetched) {
      ...
      return (
        <EmptyTable
          ...
          info={`Mdmlab is collecting query results from ${nodeName}. Check back later.`}
        />
      );
    }
    if (reportClipped) {
      return (
        <EmptyTable
          ...
          info="This query has paused reporting in Mdmlab, and no results were saved for this node."
        />
      );
    }
    return (
      ...
      <EmptyTable
        ...
        info={`This query has run on ${nodeName}, but returned no data for this node.`}
      />
    );
  }, [lastFetched, nodeName, reportClipped]);
    ...
    return (
      <>
        ...
        <span className="last-fetched">
          Last fetched{" "}
          <HumanTimeDiffWithMdmlabLaunchCutoff timeString={lastFetched ?? ""} />
        </span>
      </>
    );
