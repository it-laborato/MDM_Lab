...
import { OS_SETTINGS_FILTER_OPTIONS } from "../../NodesPageConfig";
...
const baseClass = "nodes-filter-block";

type PlatformLabelNameFromAPI = keyof typeof PLATFORM_TYPE_ICONS;

interface INodesFilterBlockProps {
  /**
   * An object of params the the NodeFilterBlock uses to render the correct
...
/**
 * Renders the filtering section of the Manage Nodes Page. This will handle rendering
 * the correct filter pills and any filter dropdowns associated with those pills.
 */
const NodesFilterBlock = ({
  params: {
    policyId,
    macSettingsStatus,
    softwareId,
  ...
  const renderOSFilterBlock = () => {
    ...
    const TooltipDescription = (
      <span>
        Nodes with {formatOperatingSystemDisplayName(name_only || name)},
        <br />
  ...
  const renderVulnerabilityFilterBlock = () => {
    ...
    return (
      <FilterPill
        label={vulnerability}
        tooltipDescription={<span>Nodes affected by the specified CVE.</span>}
  ...
  const renderSoftwareFilterBlock = (additionalClearParams?: string[]) => {
    ...
    // const TooltipDescription = (
    //   <span>
    //     Nodes with {name || "Unknown software"},
    //     <br />
  ...
  const renderMDMEnrollmentFilterBlock = () => {
    ...
    const MDM_STATUS_PILL_TOOLTIP: Record<string, React.ReactNode> = {
      ...
      pending: (
        <span>
          Nodes ordered using Apple <br />
          Business Manager (ABM). <br />
          They will automatically enroll <br />
          to Mdmlab and turn on MDM <br />
    ...
    if (munkiIssueDetails) {
      return (
        <FilterPill
          ...
          tooltipDescription={
            <span>
              Nodes that reported this Munki issue <br />
              the last time Munki ran on each node.
            </span>
          }
  ...
  const renderLowDiskSpaceFilterBlock = () => {
    const TooltipDescription = (
      <span>
        Nodes that have {lowDiskSpaceNodes} GB or less <br />
  ...
  if (
    ...
    const renderFilterPill = () => {
      switch (true) {
        ...
        case showSelectedLabel && !!lowDiskSpaceNodes:
          return (
            <>
...
export default NodesFilterBlock;
