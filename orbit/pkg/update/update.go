...
const (
	//
	// For users using MDMlab's TUF:
	// 	- orbit 1.38.0+ we migrate TUF from https://tuf.mdmlabctl.com to https://updates.mdmlabdm.com.
	...
	//
	// For both MDMlab's TUF and custom TUF, mdmlabd packages built with mdmlabctl 4.63.0+ will contain both files
	// `updates-metadata.json` and `tuf-metadata.json` (same content) to support downgrades to orbit 1.37.0 or lower.

	//
	// The following variables are used by `mdmlabctl package` and by orbit in the migration to the new TUF repository.
	//

	DefaultURL          = `https://updates.mdmlabdm.com`
	...
	OldMDMlabTUFURL      = "https://tuf.mdmlabctl.com"
...
func NewUpdater(opt Options) (*Updater, error) {
	...
	//
	// NewUpdater is used when packaging Orbit (`mdmlabctl package`) and by Orbit
	...
	if meta["root.json"] == nil {
		// NOTE: This path is currently only used when (1) packaging Orbit (`mdmlabctl package`), or
		// (2) in the edge-case when orbit's metadata JSON local file is removed for some reason, or
		// (3) first run on TUF migration from https://tuf.mdmlabctl.com to https://updates.mdmlabdm.com.
		//
		// When edge-case (2) happens, orbit will attempt to use MDMlab DM's root JSON
...
func (u *Updater) checkExec(target, tmpPath string, customCheckExec func(execPath string) error) error {
	...
	if !containsArch && strings.HasSuffix(os.Args[0], "mdmlabctl") {
...
// HasAccessToNewTUFServer will verify if the agent has access to MDMlab's new TUF
...
func createTUFRemoteStore(opt Options, serverURL string) (client.RemoteStore, error) {
	...
	httpClient := mdmlabhttp.NewClient(mdmlabhttp.WithTLSClientConfig(tlsConfig))
