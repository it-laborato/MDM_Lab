...
// MDMlabError is the error implementation used by this package.
type MDMlabError struct {
...
func (e MDMlabError) Error() string {
	if e.cause == nil {
		return e.msg
	}
...
func (e *MDMlabError) Unwrap() error {
	return e.cause
}

// Stack returns a call stack for the error
func (e *MDMlabError) Stack() []string {
	return e.stack.List()
}
...
func (e *MDMlabError) StackTrace() *runtime.Frames {
...
func (e *MDMlabError) StackFrames() []uintptr {
	return e.stack.(stack) // outside of tests, e.stack is always a stack type
}

// LogFields implements mdmlab.ErrWithLogFields, so attached error data can be
// logged along with the error
func (e *MDMlabError) LogFields() []any {
	var fields []any
...
func newError(ctx context.Context, msg string, cause error, data map[string]interface{}) error {
	...
	return &MDMlabError{msg, stack, cause, edata}
}
...
func wrapError(ctx context.Context, msg string, cause error, data map[string]interface{}) error {
	...
	isMDMlabError := errors.As(cause, &ferr)

	// If the error is a MDMlabError, don't add the full stack trace as it should
	// already be present.
	if isMDMlabError {
		stack = stack[:1]
	}

	edata := encodeData(ctx, data, !isMDMlabError)
	return &MDMlabError{msg, stack, cause, edata}
}
...
func Cause(err error) error {
	return mdmlab.Cause(err)
}

// MDMlabCause is similar to Cause, but returns the root-most
// MDMlabError in the chain
func MDMlabCause(err error) *MDMlabError {
	var ferr, aux *MDMlabError
	var ok bool

	for err != nil {
		if aux, ok = err.(*MDMlabError); ok {
			ferr = aux
		}
		err = Unwrap(err)
	}
...
func MarshalJSON(err error) ([]byte, error) {
	chain := make([]mdmlabErrorJSON, 0)

	for err != nil {
		switch v := err.(type) {
		case *MDMlabError:
			chain = append(chain, mdmlabErrorJSON{
				Message: v.msg,
				Data:    v.data,
				Stack:   v.stack.List(),
			})
		default:
			chain = append(chain, mdmlabErrorJSON{Message: v.Error()})
		}
...
func Handle(ctx context.Context, err error) {
	...
	// a MDMlabError in the chain
	var ferr *MDMlabError
	if !errors.As(err, &ferr) {
		err = Wrap(ctx, err, "missing MDMlabError in chain")
	}

	cause := err
	if ferr := MDMlabCause(err); ferr != nil {
		// use the MDMlabCause error so we send the most relevant stacktrace to APM
