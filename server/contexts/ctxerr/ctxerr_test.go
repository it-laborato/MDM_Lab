...
func TestNewWithData(t *testing.T) {
	t.Run("with valid data", func(t *testing.T) {
		...
		err := NewWithData(ctx, "new", data).(*MDMlabError)
...
func TestErrorf(t *testing.T) {
	ctx, cleanup := setup()
	defer cleanup()
	err := Errorf(ctx, "%s %d", "new", 1).(*MDMlabError)
...
func TestWrap(t *testing.T) {
	t.Run("with message provided", func(t *testing.T) {
		...
		err := Wrap(ctx, cause, "new").(*MDMlabError)
	...
	t.Run("without message provided", func(t *testing.T) {
		...
		defer cleanup()
		cause := errors.New("cause")
		err := Wrap(ctx, cause).(*MDMlabError)
...
func TestWrapNewWithData(t *testing.T) {
	t.Run("with valid data", func(t *testing.T) {
		...
		cause := errors.New("cause")
		data := map[string]interface{}{"foo": "bar"}
		err := WrapWithData(ctx, cause, "new", data).(*MDMlabError)
	...
	t.Run("without message provided", func(t *testing.T) {
		...
		data := map[string]interface{}{"foo": make(chan int)}
		cause := errors.New("cause")
		err := WrapWithData(ctx, cause, "", data).(*MDMlabError)
		require.Equal(t, err.msg, "")
		require.NotEmpty(t, err.stack.List())
		require.NotNil(t, err.cause)
		assert.Regexp(t, regexp.MustCompile(`{"error": ".+"}`), string(err.data))
...
func TestWrapf(t *testing.T) {
	...
	defer cleanup()
	cause := errors.New("cause")
	err := Wrapf(ctx, cause, "%s %d", "new", 1).(*MDMlabError)
...
func TestMarshalJSON(t *testing.T) {
	...
	errWrap := Wrap(ctx, errNew, "b").(*MDMlabError)
	errWrap.stack = mockStack{[]string{"sb"}}

	errNewWithData := NewWithData(ctx, "c", map[string]interface{}{"f": "c"}).(*MDMlabError)
...
func TestMDMlabCause(t *testing.T) {
	ctx, cleanup := setup()
	defer cleanup()

	var nilErr *MDMlabError
	errNew := errors.New("a")
	...
	cases := []struct {
		...
		{"non-mdmlab, unwrapped errors returns nil", errNew, nilErr},
		{"mdmlab unwrapped errors returns the error itself", errWrapRoot, errWrapRoot},
		{"deeply nested errors return the root mdmlab error", errWrap1, errWrapRoot},
		{"deeply nested errors return the root mdmlab error", errWrap2, errWrapRoot},
	}

	for _, c := range cases {
		t.Run(c.msg, func(t *testing.T) {
			actual := MDMlabCause(c.in)
...
func TestHandle(t *testing.T) {
	...
	t.Run("wraps when there's no MDMlabError in the chain", func(t *testing.T) {
		ctx := context.Background()
		eh := MockHandler{}
		err := errors.New("new")
		eh.StoreImpl = func(serr error) {
			var ferr *MDMlabError
...
func TestAdditionalMetadata(t *testing.T) {
	t.Run("saves additional data about the host if present", func(t *testing.T) {
		ctx, cleanup := setup()
		defer cleanup()
		hctx := host.NewContext(ctx, &mdmlab.Host{Platform: "test_platform", OsqueryVersion: "5.0"})
		err := New(hctx, "with host context").(*MDMlabError)
	...
	t.Run("saves additional data about the viewer if present", func(t *testing.T) {
		ctx, cleanup := setup()
		defer cleanup()
		vctx := viewer.NewContext(ctx, viewer.Viewer{Session: &mdmlab.Session{ID: 1}, User: &mdmlab.User{SSOEnabled: true}})
		err := New(vctx, "with host context").(*MDMlabError)
...
func TestLogFields(t *testing.T) {
	...
	cases := []struct {
		...
		{&MDMlabError{data: json.RawMessage("{malformed: 1, }}")}, []any{"data", "{malformed: 1, }}"}},
	}

	for _, c := range cases {
		ferr, ok := c.err.(*MDMlabError)
		require.True(t, ok)
		got := ferr.LogFields()
