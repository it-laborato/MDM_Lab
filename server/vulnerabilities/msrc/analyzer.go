...
func Analyze(
	...
	toInsert := make(map[string]mdmlab.OSVulnerability)
	toDelete := make(map[string]mdmlab.OSVulnerability)

	// Run vulnerability detection for all hosts in this batch (hIDs)
	// and store the results in 'found'.
	var found []mdmlab.OSVulnerability
	for cve, v := range bulletin.Vulnerabities {
		...
		if isVuln {
			found = append(found, mdmlab.OSVulnerability{
				OSID:              os.ID,
				CVE:               cve,
	...
	var existing []mdmlab.OSVulnerability
	...
	err = utils.BatchProcess(toDelete, func(v []mdmlab.OSVulnerability) error {
	...
	var inserted []mdmlab.OSVulnerability
	if collectVulns {
		inserted = make([]mdmlab.OSVulnerability, 0, len(toInsert))
	}

	err = utils.BatchProcess(toInsert, func(v []mdmlab.OSVulnerability) error {
		n, err := ds.InsertOSVulnerabilities(ctx, v, mdmlab.MSRCSource)
		if err != nil {
			return err
		}
...
func loadBulletin(os mdmlab.OperatingSystem, dir string) (*msrc.SecurityBulletin, error) {
