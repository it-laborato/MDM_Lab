...
func RecentVulns[T mdmlab.Vulnerability](
	vulns []T,
	meta []mdmlab.CVEMeta,
) ([]T, map[string]mdmlab.CVEMeta) {
	if len(vulns) == 0 {
		return nil, nil
	}

	recent := make(map[string]mdmlab.CVEMeta)
	for _, r := range meta {
		recent[r.CVE] = r
	}
...
func BatchProcess[T mdmlab.Vulnerability](
	values map[string]T,
...
func VulnsDelta[T mdmlab.Vulnerability](
	found []T,
	existing []T,
