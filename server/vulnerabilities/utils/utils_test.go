...
func TestRecentVulns(t *testing.T) {
	...
	t.Run("no NVD nor OVAL vulns", func(t *testing.T) {
		vulns, meta := RecentVulns[mdmlab.SoftwareVulnerability](nil, meta)
		require.Empty(t, vulns)
		require.Empty(t, meta)
	})

	t.Run("filters vulnerabilities based on max age", func(t *testing.T) {
		ovalVulns := []mdmlab.SoftwareVulnerability{
			{CVE: "cve-recent-1"},
			{CVE: "cve-recent-2"},
			{CVE: "cve-recent-2"},
			{CVE: "cve-outdated-1"},
		...
		nvdVulns := []mdmlab.SoftwareVulnerability{
			{CVE: "cve-recent-1"},
			{CVE: "cve-recent-3"},
			{CVE: "cve-outdated-2"},
			{CVE: "cve-outdated-3"},
		...
		var input []mdmlab.SoftwareVulnerability
		...
		expectedMeta := map[string]mdmlab.CVEMeta{
...
func TestVulnsDelta(t *testing.T) {
	t.Run("no existing vulnerabilities", func(t *testing.T) {
		var found []mdmlab.SoftwareVulnerability
		var existing []mdmlab.SoftwareVulnerability
	...
	t.Run("existing match found", func(t *testing.T) {
		found := []mdmlab.SoftwareVulnerability{
	...
	t.Run("existing differ from found", func(t *testing.T) {
		...
		expectedToInsert := []mdmlab.SoftwareVulnerability{
			{SoftwareID: 3, CVE: "cve_5"},
			{SoftwareID: 3, CVE: "cve_6"},
		}

		expectedToDelete := []mdmlab.SoftwareVulnerability{
			{SoftwareID: 2, CVE: "cve_3"},
			{SoftwareID: 2, CVE: "cve_4"},
		}
