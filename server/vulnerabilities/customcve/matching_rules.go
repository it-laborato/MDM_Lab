...
func (r CVEMatchingRule) match(ctx context.Context, ds mdmlab.Datastore) ([]mdmlab.SoftwareVulnerability, error) {
	var vulns []mdmlab.SoftwareVulnerability
	filter := mdmlab.VulnSoftwareFilter{
...
func CheckCustomVulnerabilities(ctx context.Context, ds mdmlab.Datastore, logger log.Logger, periodicity time.Duration) ([]mdmlab.SoftwareVulnerability, error) {
	...
	for _, v := range vulns {
		ok, err := ds.InsertSoftwareVulnerability(ctx, v, mdmlab.CustomSource)
		if err != nil {
	...
	if err := ds.DeleteOutOfDateVulnerabilities(ctx, mdmlab.CustomSource, 2*periodicity); err != nil {
