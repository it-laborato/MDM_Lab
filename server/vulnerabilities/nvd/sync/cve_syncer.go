...
func (s *CVE) initSync(ctx context.Context) error {
	// Remove any legacy feeds from previous versions of MDMlab.
...
// removeLegacyFeeds removes all the legacy feed files downloaded by previous versions of MDMlab.
...
func convertAPI20CVEToLegacy(cve nvdapi.CVE, logger log.Logger) *schema.NVDCVEFeedJSON10DefCVEItem {
	...
	if len(descriptions) == 0 {
		// Populate a blank description to prevent MDMlab cron job from crashing: https://github.com/mdmlabdm/mdmlab/issues/21239
		descriptions = append(descriptions, &schema.CVEJSON40LangString{
			Lang:  "en",
			Value: "",
		})
