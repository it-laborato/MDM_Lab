...
// from the latest release of github.com/mdmlabdm/nvd. Skips downloading if CPE translations is newer than the release.
func DownloadCPETranslationsFromGithub(vulnPath string, cpeTranslationsURL string) error {
	...
	client := mdmlabhttp.NewGithubClient()
...
func (c CPETranslations) Translate(reCache *regexpCache, s *mdmlab.Software) (CPETranslation, bool, error) {
	for _, item := range c {
...
func (c CPETranslationSoftware) Matches(reCache *regexpCache, s *mdmlab.Software) (bool, error) {
