...
import (
	...
	"github.com:it-laborato/MDM_Lab/server/vulnerabilities/nvd/tools/providers/nvd"
...
func GetGitHubCVEAssetPath() (string, error) {
	...
	ghClient := github.NewClient(mdmlabhttp.NewGithubClient())
...
// vulnerabilities database folder to software CPEs in the mdmlab database.
...
func TranslateCPEToCVE(
	...
	// NVD feed file.
	softwareVulns := make(map[string]mdmlab.SoftwareVulnerability)
	osVulns := make(map[string]mdmlab.OSVulnerability)
	...
	var newVulns []mdmlab.SoftwareVulnerability
	for _, vuln := range softwareVulns {
		ok, err := ds.InsertSoftwareVulnerability(ctx, vuln, mdmlab.NVDSource)
		if err != nil {
	...
	for _, vuln := range osVulns {
		_, err := ds.InsertOSVulnerability(ctx, vuln, mdmlab.NVDSource)
		if err != nil {
			level.Error(logger).Log("cpe processing", "error", "err", err)
			continue
		}
	...
	if err = ds.DeleteOutOfDateVulnerabilities(ctx, mdmlab.NVDSource, 2*periodicity); err != nil {
		level.Error(logger).Log("msg", "error deleting out of date vulnerabilities", "err", err)
	}
	if err = ds.DeleteOutOfDateOSVulnerabilities(ctx, mdmlab.NVDSource, 2*periodicity); err != nil {
...
func GetMacOSCPEs(ctx context.Context, ds mdmlab.Datastore) ([]osCPEWithNVDMeta, error) {
...
func checkCVEs(
...
) ([]mdmlab.SoftwareVulnerability, []mdmlab.OSVulnerability, error) {
	...
	// This is done to speed up the matching process (PR https://github.com/mdmlabdm/mdmlab/pull/17298).
	...
	var foundSoftwareVulns []mdmlab.SoftwareVulnerability
	var foundOSVulns []mdmlab.OSVulnerability
