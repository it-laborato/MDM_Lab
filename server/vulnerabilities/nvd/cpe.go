...
// from the latest release of github.com/mdmlabdm/nvd. Skips downloading if CPE database is newer than the release.
func DownloadCPEDBFromGithub(vulnPath string, cpeDBURL string) error {
	...
	githubClient := mdmlabhttp.NewGithubClient()
...
func cpeGeneralSearchQuery(software *mdmlab.Software) (string, []interface{}, error) {
...
func CPEFromSoftware(logger log.Logger, db *sqlx.DB, software *mdmlab.Software, translations CPETranslations, reCache *regexpCache) (string, error) {
...
func consumeCPEBuffer(
	...
	batch []mdmlab.SoftwareCPE,
) error {
	var toDelete []mdmlab.SoftwareCPE
	var toUpsert []mdmlab.SoftwareCPE
...
func TranslateSoftwareToCPE(
	...
	nonOvalIterator, err := ds.AllSoftwareIterator(
		ctx,
		mdmlab.SoftwareIterQueryOptions{
...
func translateSoftwareToCPEWithIterator(
	...
	vulnPath string,
	logger kitlog.Logger,
	iterator mdmlab.SoftwareIterator,
) error {
	...
	for iterator.Next() {
		...
		if software.Version == "" {
			...
			// generated by a previous version of MDMlab.
		} else {
		...
		buffer = append(buffer, mdmlab.SoftwareCPE{SoftwareID: software.ID, CPE: cpe})
