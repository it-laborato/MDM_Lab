...
func (d *threadSafeDSMock) ListSoftwareCPEs(ctx context.Context) ([]mdmlab.SoftwareCPE, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.Store.ListSoftwareCPEs(ctx)
}

func (d *threadSafeDSMock) InsertSoftwareVulnerability(ctx context.Context, vuln mdmlab.SoftwareVulnerability, src mdmlab.VulnerabilitySource) (bool, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func TestTranslateCPEToCVE(t *testing.T) {
	...
	cveTests := map[string]struct {
		...
		// Skipping test while troubleshooting https://github.com/mdmlabdm/mdmlab/issues/24286
		//
	...
	t.Run("find_vulns_on_cpes", func(t *testing.T) {
		...
		ds.ListSoftwareCPEsFunc = func(ctx context.Context) ([]mdmlab.SoftwareCPE, error) {
			...
			for cpe := range cveTests {
				softwareCPEs = append(softwareCPEs, mdmlab.SoftwareCPE{CPE: cpe, SoftwareID: i})
		...
		var osIDs []uint
		ds.ListOperatingSystemsForPlatformFunc = func(ctx context.Context, p string) ([]mdmlab.OperatingSystem, error) {
			var oss []mdmlab.OperatingSystem
			for _, os := range cveOSTests {
				oss = append(oss, mdmlab.OperatingSystem{
					ID:       os.osID,
		...
		ds.InsertOSVulnerabilityFunc = func(ctx context.Context, vuln mdmlab.OSVulnerability, src mdmlab.VulnerabilitySource) (bool, error) {
			osCVELock.Lock()
	...
	t.Run("recent_vulns", func(t *testing.T) {
		...
		softwareCPEs := []mdmlab.SoftwareCPE{
...
func TestGetMacOSCPEs(t *testing.T) {
	...
	ds.ListOperatingSystemsForPlatformFunc = func(ctx context.Context, p string) ([]mdmlab.OperatingSystem, error) {
		return []mdmlab.OperatingSystem{os}, nil
	}
