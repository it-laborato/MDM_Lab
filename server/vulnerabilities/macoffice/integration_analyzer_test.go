...
func TestIntegrationsAnalyzer(t *testing.T) {
	...
	t.Run("vulnerable versions", func(t *testing.T) {
		...
		var powerpoint mdmlab.HostSoftwareEntry
		var word mdmlab.HostSoftwareEntry
		...
		ok, err := ds.InsertSoftwareVulnerability(ctx, mdmlab.SoftwareVulnerability{
			SoftwareID: word.ID, CVE: "3000-3000",
		}, mdmlab.MacOfficeReleaseNotesSource)
		require.True(t, ok)
		require.NoError(t, err)
		...
		stored, err := ds.ListSoftwareVulnerabilitiesByHostIDsSource(ctx, []uint{host.ID}, mdmlab.MacOfficeReleaseNotesSource)
		require.NoError(t, err)
