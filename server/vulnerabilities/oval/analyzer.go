...
func Analyze(
	...
	toInsertSet := make(map[string]mdmlab.SoftwareVulnerability)
	toDeleteSet := make(map[string]mdmlab.SoftwareVulnerability)
	...
	for {
		...
		foundInBatch := make(map[uint][]mdmlab.SoftwareVulnerability)
		for _, hostID := range hostIDs {
			hostID := hostID
			software, err := ds.ListSoftwareForVulnDetection(ctx, mdmlab.VulnSoftwareFilter{HostID: &hostID})
			if err != nil {
				return nil, err
			}
	...
	err = utils.BatchProcess(toDeleteSet, func(v []mdmlab.SoftwareVulnerability) error {
	...
	var inserted []mdmlab.SoftwareVulnerability
	if collectVulns {
