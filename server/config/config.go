...
// ServerConfig defines configs related to the MDMlab server
...
// LicenseConfig defines configs related to licensing MDMlab.
type LicenseConfig struct {
	Key              string `yaml:"key"`
	EnforceHostLimit bool   `yaml:"enforce_host_limit"`
}

// VulnerabilitiesConfig defines configs related to vulnerability processing within MDMlab.
...
// UpgradesConfig defines configs related to mdmlab server upgrades.
...
// PrometheusConfig holds the configuration for MDMlab's prometheus metrics.
...
// PackagingConfig holds configuration to build and retrieve MDMlab packages
...
// MDMlabConfig stores the application configuration. Each subcategory is
...
type MDMConfig struct {
	...
	// AppleEnable enables Apple MDM functionality on MDMlab.
...
// filled into the MDMlabConfig struct
func (man Manager) addConfigs() {
	addMysqlConfig := func(prefix, defaultAddr, usageSuffix string) {
		...
		man.addConfigString(prefix+".username", "mdmlab",
		...
		man.addConfigString(prefix+".database", "mdmlab",
	...
	man.addConfigString("server.key", "./tools/osquery/mdmlab.key",
		"MDMlab TLS key path")
	...
	man.addConfigBool("server.sandbox_enabled", false,
		"When enabled, MDMlab limits some features for the Sandbox")
	man.addConfigBool("server.websockets_allow_unsafe_origin", false, "Disable checking the origin header on websocket connections, this is sometimes necessary when proxies rewrite origin headers between the client and the MDMlab webserver")
	...
	// License
	man.addConfigString("license.key", "", "MDMlab license key (to enable MDMlab Premium features)")
	...
	man.addConfigString("vulnerabilities.databases_path", "/tmp/vulndbs",
		"Path where MDMlab will download the data feeds to check CVEs")
	...
	man.addConfigString("vulnerabilities.cpe_database_url", "",
		"URL from which to get the latest CPE database. If empty, it will be downloaded from the latest release available at https://github.com/mdmlabdm/nvd/releases.")
	man.addConfigString("vulnerabilities.cpe_translations_url", "",
		"URL from which to get the latest CPE translations. If empty, it will be downloaded from the latest release available at https://github.com/mdmlabdm/nvd/releases.")
...
// MDMlabConfig struct
func (man Manager) LoadConfig() MDMlabConfig {
	man.loadConfigFile()
...
// Manager manages the addition and retrieval of config values for MDMlab
// configs. It's only public API method is LoadConfig, which will return the
// populated MDMlabConfig struct.
type Manager struct {
	viper    *viper.Viper
...
// decrypting the encrypted value that is usually provided via the mdmlab
// server's flags.
func SetTestMDMConfig(t testing.TB, cfg *MDMlabConfig, cert, key []byte, wstepCertAndKeyDir string) {
