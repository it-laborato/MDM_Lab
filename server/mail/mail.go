// Package mail provides implementations of the MDMlab MailService
package mail
...
func NewService(config config.MDMlabConfig) (mdmlab.MailService, error) {
...
type sender interface {
	sendMail(e mdmlab.Email, msg []byte) error
}

func Test(mailer mdmlab.MailService, e mdmlab.Email) error {
...
type fromFunc func(e mdmlab.Email) (string, error)

func getMessageBody(e mdmlab.Email, f fromFunc) ([]byte, error) {
...
func getFrom(e mdmlab.Email) (string, error) {
	return "From: " + e.SMTPSettings.SMTPSenderAddress + "\r\n", nil
}

func (m mailService) SendEmail(e mdmlab.Email) error {
...
func (m mailService) CanSendEmail(smtpSettings mdmlab.SMTPSettings) bool {
...
func smtpAuth(e mdmlab.Email) (smtp.Auth, error) {
	if e.SMTPSettings.SMTPAuthenticationType != mdmlab.AuthTypeNameUserNamePassword {
		return nil, nil
	}
	...
	var auth smtp.Auth
	switch authMethod {
	case mdmlab.AuthMethodNameCramMD5:
		auth = smtp.CRAMMD5Auth(username, password)
	case mdmlab.AuthMethodNamePlain:
		auth = smtp.PlainAuth("", username, password, server)
	case mdmlab.AuthMethodNameLogin:
...
func (m mailService) sendMail(e mdmlab.Email, msg []byte) error {
	...
	if e.SMTPSettings.SMTPAuthenticationMethod == mdmlab.AuthMethodNameCramMD5 {
...
func dialTimeout(addr string, tlsConfig *tls.Config) (client *smtp.Client, err error) {
	...
	// port scan using the email functionality in MDMlab)
