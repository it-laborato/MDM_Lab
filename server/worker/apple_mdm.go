...
const (
	...
	// backward compatibility (processing existing jobs after a mdmlab upgrade)
...
func (a *AppleMDM) runPostManualEnrollment(ctx context.Context, args appleMDMArgs) error {
	if isMacOS(args.Platform) {
		if _, err := a.installMDMlabd(ctx, args.HostUUID); err != nil {
...
func (a *AppleMDM) runPostDEPEnrollment(ctx context.Context, args appleMDMArgs) error {
	...
	if isMacOS(args.Platform) {
		mdmlabdCmdUUID, err := a.installMDMlabd(ctx, args.HostUUID)
		if err != nil {
			return ctxerr.Wrap(ctx, err, "installing post-enrollment packages")
		}
		awaitCmdUUIDs = append(awaitCmdUUIDs, mdmlabdCmdUUID)
...
// after a MDMlab migration to a new version.
func (a *AppleMDM) runPostDEPReleaseDevice(ctx context.Context, args appleMDMArgs) error {
	...
	for _, cmdUUID := range args.EnrollmentCommands {
		...
		for _, r := range res {
			...
			if r.Status == mdmlab.MDMAppleStatusAcknowledged || r.Status == mdmlab.MDMAppleStatusError ||
				r.Status == mdmlab.MDMAppleStatusCommandFormatError {
				completed = true
				break
			}
		}
	...
	for _, prof := range profs {
		...
		if prof.Status == nil || *prof.Status == mdmlab.MDMDeliveryPending {
...
func (a *AppleMDM) installMDMlabd(ctx context.Context, hostUUID string) (string, error) {
	manifestURL := mdmlabdbase.GetPKGManifestURL()
	cmdUUID := uuid.New().String()
	if err := a.Commander.InstallEnterpriseApplication(ctx, []string{hostUUID}, cmdUUID, manifestURL); err != nil {
		return "", err
	}
	a.Log.Log("info", "sent command to install mdmlabd", "host_uuid", hostUUID)
	return cmdUUID, nil
}
...
func (a *AppleMDM) getSignedURL(ctx context.Context, meta *mdmlab.MDMAppleBootstrapPackage) string {
	var url string
	if a.BootstrapPackageStore != nil {
		pkgID := hex.EncodeToString(meta.Sha256)
		signedURL, err := a.BootstrapPackageStore.Sign(ctx, pkgID)
		switch {
		case errors.Is(err, mdmlab.ErrNotConfigured):
