  ...
  "transition": {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			err = jira.Run(license.NewContext(context.Background(), &mdmlab.LicenseInfo{Tier: c.licenseTier}), json.RawMessage(c.payload))
			require.NoError(t, err)
		})
	}
}
...
func TestJiraQueueVulnJobs(t *testing.T) {
	...
	t.Run("same vulnerability on multiple software only queue one job", func(t *testing.T) {
		...
		meta := make(map[string]mdmlab.CVEMeta, len(vulns))
		for _, v := range vulns {
			meta[v.CVE] = mdmlab.CVEMeta{CVE: v.CVE}
		}
	...
	t.Run("success", func(t *testing.T) {
		...
		err := QueueJiraVulnJobs(ctx, ds, logger, []mdmlab.SoftwareVulnerability{{CVE: theCVE}}, meta)
		require.NoError(t, err)
		require.True(t, ds.NewJobFuncInvoked)
	})
...
func TestJiraQueueFailingPolicyJob(t *testing.T) {
	...
	t.Run("success global", func(t *testing.T) {
		...
		err := QueueJiraFailingPolicyJob(ctx, ds, logger,
			&mdmlab.Policy{PolicyData: mdmlab.PolicyData{ID: 1, Name: "p1"}}, []mdmlab.PolicySetHost{{ID: 1, Hostname: "h1"}})
		require.NoError(t, err)
		require.True(t, ds.NewJobFuncInvoked)
	...
	t.Run("success team", func(t *testing.T) {
		...
		err := QueueJiraFailingPolicyJob(ctx, ds, logger,
			&mdmlab.Policy{PolicyData: mdmlab.PolicyData{ID: 1, Name: "p1", TeamID: ptr.Uint(2)}}, []mdmlab.PolicySetHost{{ID: 1, Hostname: "h1"}})
		require.NoError(t, err)
		require.True(t, ds.NewJobFuncInvoked)
		ds.NewJobFuncInvoked = false
	})
	...
	t.Run("no host", func(t *testing.T) {
		...
		err := QueueJiraFailingPolicyJob(ctx, ds, logger,
			&mdmlab.Policy{PolicyData: mdmlab.PolicyData{ID: 1, Name: "p1"}}, []mdmlab.PolicySetHost{})
		require.NoError(t, err)
...
func TestJiraRunClientUpdate(t *testing.T) {
	...
	ds.TeamFunc = func(ctx context.Context, tid uint) (*mdmlab.Team, error) {
		...
		if jira0.ProjectKey == "1" {
			...
			jira0.ProjectKey = "2"
			teamCfg.Config.Integrations.Jira = []*mdmlab.TeamJiraIntegration{&jira0}
