...
type failingPolicyArgs struct {
	...
	Hosts          []mdmlab.PolicySetHost `json:"hosts"`
...
func NewWorker(ds mdmlab.Datastore, log kitlog.Logger) *Worker {
	return &Worker{
		ds:       ds,
		log:      log,
...
func QueueJob(ctx context.Context, ds mdmlab.Datastore, name string, args interface{}) (*mdmlab.Job, error) {
...
func QueueJobWithDelay(ctx context.Context, ds mdmlab.Datastore, name string, args interface{}, delay time.Duration) (*mdmlab.Job, error) {
		...
		for _, job := range jobs {
			...
			if err := w.processJob(ctx, job); err != nil {
				...
				if job.Retries < maxRetries {
					...
					job.State = mdmlab.JobStateFailure
				}
			} else {
				job.State = mdmlab.JobStateSuccess
				job.Error = ""
			}
...
func (w *Worker) processJob(ctx context.Context, job *mdmlab.Job) error {
...
type failingPoliciesTplArgs struct {
	...
	PolicyCritical bool
	TeamID         *uint
	Hosts          []mdmlab.PolicySetHost
}

func newFailingPoliciesTplArgs(mdmlabURL string, args *failingPolicyArgs) *failingPoliciesTplArgs {
