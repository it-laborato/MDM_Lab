...
* [{{ .DisplayName }}]({{ $.MDMlabURL }}/hosts/{{ .ID }})
...
1. Go to the [Software]({{ .MDMlabURL }}/software/manage) page in MDMlab.
...
This ticket was created automatically by your MDMlab Zendesk integration.
`)),
	...
	FailingPolicyDescription: template.Must(template.New("").Parse(
		`{{ if .PolicyCritical }}This policy is marked as **Critical** in MDMlab.
...
View hosts that failed {{ .PolicyName }} on the [**Hosts**]({{ .MDMlabURL }}/hosts/manage/?order_key=hostname&order_direction=asc&{{ if .TeamID }}team_id={{ .TeamID }}&{{ end }}policy_id={{ .PolicyID }}&policy_response=failing) page in MDMlab.

----

This issue was created automatically by your MDMlab Zendesk integration.
`)),
}
...
func (z *Zendesk) runVuln(ctx context.Context, cli ZendeskClient, args zendeskArgs) error {
	...
	var hosts []mdmlab.HostVulnerabilitySummary
	var err error
...
func (z *Zendesk) runFailingPolicy(ctx context.Context, cli ZendeskClient, args zendeskArgs) error {
	tplArgs := newFailingPoliciesTplArgs(z.MDMlabURL, args.FailingPolicy)
...
func QueueZendeskVulnJobs(
	...
	recentVulns []mdmlab.SoftwareVulnerability,
	cveMeta map[string]mdmlab.CVEMeta,
) error {
...
func QueueZendeskFailingPolicyJob(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger,
	policy *mdmlab.Policy, hosts []mdmlab.PolicySetHost,
) error {
	attrs := []interface{}{
		"enabled", "true",
