...
type macosSetupAssistantArgs struct {
	...
	// Note that only DEP-enrolled hosts in MDMlab MDM should be provided.
...
func (m *MacosSetupAssistant) runUpdateAllProfiles(ctx context.Context, args macosSetupAssistantArgs) error {
	...
	processTeam := func(team *mdmlab.TeamSummary) error {
		var teamID *uint
		if team != nil {
			teamID = &team.ID
		}
...
func (m *MacosSetupAssistant) getTeamNoTeam(ctx context.Context, tmID *uint) (*mdmlab.Team, error) {
...
func ProcessDEPCooldowns(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger) error {
