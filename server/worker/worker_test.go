...
func TestWorker(t *testing.T) {
	...
	ds.GetQueuedJobsFunc = func(ctx context.Context, maxNumJobs int, now time.Time) ([]*mdmlab.Job, error) {
	...
	ds.UpdateJobFunc = func(ctx context.Context, id uint, job *mdmlab.Job) (*mdmlab.Job, error) {
		assert.Equal(t, mdmlab.JobStateSuccess, job.State)
		return job, nil
	}
...
func TestWorkerRetries(t *testing.T) {
	...
	ds.GetQueuedJobsFunc = func(ctx context.Context, maxNumJobs int, now time.Time) ([]*mdmlab.Job, error) {
		if theJob.State == mdmlab.JobStateQueued {
			return []*mdmlab.Job{theJob}, nil
		}
		return nil, nil
	}
	...
	ds.UpdateJobFunc = func(ctx context.Context, id uint, job *mdmlab.Job) (*mdmlab.Job, error) {
		...
		if job.State == mdmlab.JobStateFailure {
			jobFailed = true
...
func TestWorkerMiddleJobFails(t *testing.T) {
	...
	ds.GetQueuedJobsFunc = func(ctx context.Context, maxNumJobs int, now time.Time) ([]*mdmlab.Job, error) {
		var queued []*mdmlab.Job
		for _, j := range jobs {
			if j.State == mdmlab.JobStateQueued {
	...
	require.Equal(t, mdmlab.JobStateSuccess, jobs[0].State)
	require.Equal(t, mdmlab.JobStateQueued, jobs[1].State)
	require.Equal(t, 1, jobs[1].Retries)
	require.Equal(t, mdmlab.JobStateSuccess, jobs[2].State)
