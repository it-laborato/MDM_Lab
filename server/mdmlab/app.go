...
type SMTPSettings struct {
	...
	// SMTPServer is the host name of the SMTP server MDMlab will use to send mail
...
// VulnerabilitySettings is part of the AppConfig which defines how mdmlab will behave
// while scanning for vulnerabilities in the host software
type VulnerabilitySettings struct {
	// DatabasesPath is the directory where mdmlab will store the different databases
...
type MDM struct {
	...
	// requests from mdmlabd requests on customer networks.  AppleServerURL DNS should resolve to the
	// same IP as the MDMlab Server URL.
	// If not set, the server will use MDMlab server URL (recommended).
	...
	// AppleBMEnabledAndConfigured is set to true if MDMlab has been
	...
	// EnabledAndConfigured is set to true if MDMlab has been
	...
	// WindowsEnabledAndConfigured indicates if MDMlab MDM is enabled for Windows.
...
type AppConfig struct {
	...
	// MDMlabDesktop holds settings for MDMlab Desktop that can be changed via the API.
	MDMlabDesktop MDMlabDesktopSettings `json:"mdmlab_desktop"`

	// VulnerabilitySettings defines how mdmlab will behave while scanning for vulnerabilities in the host software
...
func (c *AppConfig) Copy() *AppConfig {
	...
	// MDMlabDesktopSettings: nothing needs cloning
	...
	// MDMlabDesktop: nothing needs cloning
...
// EnrichedAppConfig contains the AppConfig along with additional mdmlab
// instance configuration settings as returned by the
// "GET /api/latest/mdmlab/config" API endpoint (and mdmlabctl get config).
...
// OrgInfo contains general info about the organization using MDMlab.
type OrgInfo struct {
	...
	// https://mdmlabdm.com/company/contact is used.
	ContactURL string `json:"contact_url"`
}

const DefaultOrgInfoContactURL = "https://mdmlabdm.com/company/contact"

// ServerSettings contains general settings about the MDMlab application.
...
func (f *Features) ApplyDefaultsForNewInstalls() {
	...
	// next in already running mdmlabs
...
// MDMlabDesktopSettings contains settings used to configure MDMlab Desktop.
type MDMlabDesktopSettings struct {
	// TransparencyURL is the URL used for the “About MDMlab” link in the MDMlab Desktop menu.
	TransparencyURL string `json:"transparency_url"`
}

// DefaultTransparencyURL is the default URL used for the “About MDMlab” link in the MDMlab Desktop menu.
const DefaultTransparencyURL = "https://mdmlabdm.com/transparency"
...
// EnrollSecretSpec is the mdmlabctl spec type for enroll secrets.
...
const (
	...
	// TierPremium is MDMlab Premium aka the paid license.
	TierPremium = "premium"
	// TierFree is MDMlab Free aka the free license.
	TierFree = "free"
	// TierTrial is MDMlab Premium but in trial mode
...
// LicenseInfo contains information about the MDMlab license.
...
const (
	HeaderLicenseKey          = "X-MDMlab-License"
...
// mdmlab instance (as configured for the cli, either via flags, env vars or the
