...
// installer files. MDMlab supports storing to the local filesystem and to an
// S3 bucket.
...
// required for a client to pull in and install software from the mdmlab server
type SoftwareInstallDetails struct {
	...
	// InstallerID is the unique identifier for the software package metadata in MDMlab.
...
type SoftwareInstaller struct {
	...
	// MDMlabLibraryAppID is the related MDMlab-maintained app for this installer (if not nil).
	MDMlabLibraryAppID *uint `json:"-" db:"mdmlab_library_app_id"`
...
type SoftwarePackageOrApp struct {
	...
	// AutomaticInstallPolicies is present for MDMlab maintained apps and custom packages
...
type SoftwarePackageSpec struct {
	...
	// software package. Only present after parsing a GitOps file on the mdmlabctl
...
// HostSoftwareInstallResultPayload is the payload provided by mdmlabd to record
...
func NewTempFileReader(from io.Reader, tempDirFn func() string) (*TempFileReader, error) {
	...
	tempFile, err := os.CreateTemp(tempDirFn(), "mdmlab-temp-file-*")
	if err != nil {
		return nil, err
	}
