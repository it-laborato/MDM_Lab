...
// an enrollment profile in MDMlab.
...
// MDMAppleEnrollmentProfile represents an Apple MDM enrollment profile in MDMlab.
// Such enrollment profiles are used to enroll Apple devices to MDMlab.
type MDMAppleEnrollmentProfile struct {
	// ID is the unique identifier of the enrollment in MDMlab.
...
// MDMAppleManualEnrollmentProfile is used for authorization checks to get the standard MDMlab manual
...
type MDMAppleInstaller struct {
	// ID is the unique identifier of the installer in MDMlab.
	ID uint `json:"id" db:"id"`
...
type MDMAppleDevice struct {
	...
	// It's set to false when a device unenrolls from MDMlab.
...
// MDMAppleConfigProfile represents an Apple MDM configuration profile in MDMlab.
...
type MDMAppleConfigProfile struct {
	...
	// MDMlab. For Apple profiles, it is the letter "a" followed by a uuid.
	ProfileUUID string `db:"profile_uuid" json:"profile_uuid"`
	// Deprecated: ProfileID is the old unique id of the configuration profile in
	// MDMlab. It is still maintained and generated for new profiles, but only
	...
	// MDMlab requires that Identifier must be unique in combination with the Name and TeamID.
	Identifier string `db:"identifier" json:"identifier"`
	// Name corresponds to the payload display name of the associated mobileconfig payload.
	// MDMlab requires that Name must be unique in combination with the Identifier and TeamID.
...
func (cp MDMAppleConfigProfile) ValidateUserProvided() error {
	...
	if _, ok := mdmlabNames[cp.Name]; ok {
...
// therefore is not, as far as MDMlab knows, currently on the host).
// The profile in Pending status could be on the host, but MDMlab has not received an Acknowledged status yet.
...
// MDMAppleMDMlabdConfig contains the fields used to configure
// `mdmlabd` in macOS devices via a configuration profile.
type MDMAppleMDMlabdConfig struct {
	MDMlabURL      string
	EnrollSecret  string
	EnableScripts bool
}
...
type HostDEPAssignment struct {
	// HostID is the id of the host in MDMlab.
	HostID uint `db:"host_id"`
	// AddedAt is the timestamp when MDMlab was notified that device was added to the MDMlab MDM
	// server in Apple Busines Manager (ABM).
	AddedAt time.Time `db:"added_at"`
	// DeletedAt is the timestamp  when MDMlab was notified that device was deleted from the MDMlab
...
func (h *HostDEPAssignment) IsDEPAssignedToMDMlab() bool {
	if h == nil {
		return false
	}
...
// used as there are plans to evolve the `mdmlabctl get mdm-commands` command
// output in the future to list one row per command instead of one per
// command-host combination, and this mdmlabctl command is the only use of this
...
//
// https://github.com/mdmlabdm/mdmlab/issues/11008#issuecomment-1503466119
...
type MDMAppleDeclaration struct {
	...
	// MDMlab. Since we use the same endpoints for declarations and profiles:
...
type MDMAppleHostDeclaration struct {
	...
	// Status represent the current state of the declaration, as known by the MDMlab server.
	Status *MDMDeliveryStatus `db:"status" json:"status"`
	...
	// either by the MDM protocol or the MDMlab server.
	Detail string `db:"detail" json:"detail"`
...
// package files. MDMlab supports storing to the database and to an S3 bucket.
