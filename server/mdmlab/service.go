...
type Service interface {
	...
	// EnrollOrbit enrolls an orbit instance to MDMlab by using the host information + enroll secret
	...
	// agent options. It also returns any notifications that mdmlab wants to surface
	// to mdmlabd (formerly orbit).
	GetOrbitConfig(ctx context.Context) (OrbitConfig, error)

	// LogMDMlabdError logs an error report from a `mdmlabd` component
	LogMDMlabdError(ctx context.Context, errData MDMlabdError) error
	...
	// GetMDMlabDesktopSummary returns a summary of the host used by MDMlab Desktop to operate.
	GetMDMlabDesktopSummary(ctx context.Context) (DesktopSummary, error)
	...
	// /////////////////////////////////////////////////////////////////////////////
	// UserService contains methods for managing a MDMlab User.
	...
	// credentials are valid, then responds with an URL to the MDMlab UI to
	...
	// the host, which is either set by the mdmlabd installer at startup (via a
	...
	// /////////////////////////////////////////////////////////////////////////////
	// AppConfigService provides methods for configuring  the MDMlab application
	...
	// SetupRequired returns whether the app config setup needs to be performed (only when first initializing a MDMlab
	// server).
	...
	// LoggingConfig parses config.MDMlabConfig instance and returns a Logging.
	LoggingConfig(ctx context.Context) (*Logging, error)

	// EmailConfig parses config.MDMlabConfig and returns an EmailConfig
	...
	// InviteNewUser creates an invite for a new user to join MDMlab.
	...
	// /////////////////////////////////////////////////////////////////////////////
	// TargetService **NOTE: SearchTargets will be removed in MDMlab 5.0**
	...
	// UploadMDMAppleInstaller uploads an Apple installer to MDMlab.
	...
	// DeleteMDMAppleInstaller deletes an Apple installer from MDMlab.
	...
	// ListABMTokens lists all the ABM tokens in MDMlab.
	ListABMTokens(ctx context.Context) ([]*ABMToken, error)

	// CountABMTokens counts the ABM tokens in MDMlab.
	...
	// profile used for MDMlab MDM enrollment from the specified device.
	...
	// given team that enables FileVault with a config that allows MDMlab to
	...
	// AddMDMlabMaintainedApp adds a MDMlab-maintained app to the given team.
	AddMDMlabMaintainedApp(ctx context.Context, teamID *uint, appID uint, installScript, preInstallQuery, postInstallScript, uninstallScript string, selfService bool, labelsIncludeAny, labelsExcludeAny []string) (uint, error)
	// ListMDMlabMaintainedApps lists MDMlab-maintained apps available to a specific team
	ListMDMlabMaintainedApps(ctx context.Context, teamID *uint, opts ListOptions) ([]MaintainedApp, *PaginationMetadata, error)
	// GetMDMlabMaintainedApp returns a MDMlab-maintained app by ID
	GetMDMlabMaintainedApp(ctx context.Context, appID uint) (*MaintainedApp, error)
