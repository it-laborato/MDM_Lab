...
// mdmlab-osquery installers. This was originally implemented to support the
// MDMlab Sandbox and is not expected to be used outside of this:
// https://mdmlabdm.com/docs/configuration/mdmlab-server-configuration#packaging
...
// Datastore combines all the interfaces in the MDMlab DAL
type Datastore interface {
	...
	// GetCompletedCampaigns returns the IDs of the campaigns that are in the mdmlab.QueryComplete state and that are in the
	...
	// IsHostConnectedToMDMlabMDM verifies if the host has an active MDMlab MDM enrollment with this server
	IsHostConnectedToMDMlabMDM(ctx context.Context, host *Host) (bool, error)

	// AreHostsConnectedToMDMlabMDM checks each host MDM enrollment with
	...
	AreHostsConnectedToMDMlabMDM(ctx context.Context, hosts []*Host) (map[string]bool, error)
	...
	// MDMTurnOff updates MDMlab host information related to MDM when a
	...
	ShouldSendStatistics(ctx context.Context, frequency time.Duration, config config.MDMlabConfig) (StatisticsPayload, bool, error)
	...
	SetOrUpdateMDMData(ctx context.Context, hostID uint, isServer, enrolled bool, serverURL string, installedFromDep bool, name string, mdmlabEnrollRef string) error
	...
	// host based on the MDM IdP account information associated with the provided mdmlab enrollment reference.
	SetOrUpdateHostEmailsFromMdmIdpAccounts(ctx context.Context, hostID uint, mdmlabEnrollmentRef string) error
	...
	// IsCVEKnownToMDMlab checks if the provided CVE is known to MDMlab.
	IsCVEKnownToMDMlab(ctx context.Context, cve string) (bool, error)
	...
	// this is mainly aimed to internal usage within the MDMlab server.
	...
	// Such enrollment profiles allow devices to enroll to MDMlab MDM.
	...
	// NewMDMAppleInstaller creates and stores an Apple installer to MDMlab.
	...
	// IngestMDMAppleDevicesFromDEPSync creates new MDMlab host records for MDM-enrolled devices that are
	// not already enrolled in MDMlab. It returns the number of hosts created, and an error.
	...
	// MDM-enrolled devices via OTA that are not already enrolled in MDMlab.
	IngestMDMAppleDeviceFromOTAEnrollment(ctx context.Context, teamID *uint, deviceInfo MDMAppleMachineInfo) error

	// MDMAppleUpsertHost creates or matches a MDMlab host record for an
	// MDM-enrolled device.
	MDMAppleUpsertHost(ctx context.Context, mdmHost *Host) error

	// RestoreMDMApplePendingDEPHost restores a host that was previously deleted from MDMlab.
	...
	// that are enrolled or pending enrollment in MDMlab's MDM via DEP for the
	...
	// that are enrolled or pending enrollment in MDMlab's MDM via DEP in the
	...
	// for the host with the given serial number. The host must be DEP assigned to MDMlab.
	...
	LockHostViaScript(ctx context.Context, request *HostScriptRequestPayload, hostMDMlabPlatform string) error

	// UnlockHostViaScript sends a script to unlock a host and updates the
	// states in host_mdm_actions
	UnlockHostViaScript(ctx context.Context, request *HostScriptRequestPayload, hostMDMlabPlatform string) error
	...
	// pending.
	UnlockHostManually(ctx context.Context, hostID uint, hostMDMlabPlatform string, ts time.Time) error
	...
	// states in host_mdm_actions.
	WipeHostViaScript(ctx context.Context, request *HostScriptRequestPayload, hostMDMlabPlatform string) error
	...
	// ListAvailableMDMlabMaintainedApps returns a list of
	// MDMlab-maintained apps available to a specific team (or the full list of apps if no team is specified)
	ListAvailableMDMlabMaintainedApps(ctx context.Context, teamID *uint, opt ListOptions) ([]MaintainedApp, *PaginationMetadata, error)

	// GetMaintainedAppByID gets a MDMlab-maintained app by its ID.
	...
	SetOrUpdateHostGPSModule(ctx context.Context, hostID uint, module *HostGPSModule) error
}

// MDMAppleStore wraps nanomdm's storage and adds methods to deal with
// MDMlab-specific use cases.
...
// status of a host's MDM profiles. The MDMlab Datastore satisfies this interface.
