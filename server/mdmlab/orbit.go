...
// OrbitConfigNotifications are notifications that the mdmlab server sends to
// mdmlabd (orbit) so that it can run commands or more generally react to this
// information.
type OrbitConfigNotifications struct {
	...
	// disabled and the device was enrolled in MDMlab's MDM (see
	...
	// RunSetupExperience indicates whether or not Orbit should run the MDMlab setup experience
...
type OrbitConfig struct {
	...
	// UpdateChannels contains the TUF channels to use on mdmlabd components.
	//
...
// OrbitUpdateChannels hold the update channels that can be configured in mdmlabd agents.
type OrbitUpdateChannels struct {
	...
	// Desktop holds the MDMlab Desktop channel.
