...
const (
	...
	// third-party MDM solution to MDMlab.
...
// accepted. The MDMlab Datastore satisfies this interface.
...
func (bp *MDMAppleBootstrapPackage) URL(host string) (string, error) {
	...
	pkgURL.Path = "/api/latest/mdmlab/mdm/bootstrap"
...
// see https://github.com/mdmlabdm/mdmlab/blob/e9fd28717d474668ca626efbacdd0615d42b2e0a/server/service/osquery.go#L950
...
// HostMDMProfileRetryCount represents the number of times MDMlab has attempted to install
...
type MDMProfilesSummary struct {
	// Verified includes each host where MDMlab has verified the installation of all of the
...
//
//   - verified: the MDM command was successfully applied, and MDMlab has
//     independently verified the status. This is a terminal state.
//
//   - verifying: the MDM command was successfully applied, but MDMlab has not
...
const (
	...
	// NDES SCEP server. It is used by MDMlab's SCEP proxy.
...
func FilterMacOSOnlyProfilesFromIOSIPadOS(profiles []*MDMAppleProfilePayload) []*MDMAppleProfilePayload {
	...
	for _, profilePayload := range profiles {
		if (profilePayload.HostPlatform == "ios" || profilePayload.HostPlatform == "ipados") &&
			(profilePayload.ProfileName == mdm_types.MDMlabdConfigProfileName ||
				profilePayload.ProfileName == mdm_types.MDMlabFileVaultProfileName) {
			continue
		}
