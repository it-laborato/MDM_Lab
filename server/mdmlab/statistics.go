...
type StatisticsPayload struct {
	...
	MDMlabVersion                  string `json:"mdmlabVersion"`
	...
	// HostsEnrolledByOrbitVersion is a count of hosts enrolled to MDMlab grouped by orbit version
	HostsEnrolledByOrbitVersion []HostsCountByOrbitVersion `json:"hostsEnrolledByOrbitVersion"`
	// HostsEnrolledByOsqueryVersion is a count of hosts enrolled to MDMlab grouped by osquery version
	...
	// NumHostsNotResponding is a count of hosts that connect to MDMlab successfully but fail to submit results for distributed queries.
	...
	// The number of hosts with MDMlab desktop installed.
	NumHostsMDMlabDesktopEnabled int `json:"numHostsMDMlabDesktopEnabled"`
}
