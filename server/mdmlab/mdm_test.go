...
func TestMDMAppleBootstrapPackage(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "http://example.com/api/latest/mdmlab/mdm/bootstrap?token=abc-def", url)
...
func TestMDMProfileSpecUnmarshalJSON(t *testing.T) {
	...
	t.Run("changing labels", func(t *testing.T) {
		...
		// struct, see
		// https://github.com/mdmlabdm/mdmlab/blob/d1144df1318b50482cbd9eb996b863443975f138/server/service/appconfig.go#L334-L335
		//
		...
		// https://github.com/mdmlabdm/mdmlab/blob/1042702def54f095335d8b42ed5fdcc90468fa0d/server/mdmlab/mdm.go#L551-L552

		storedConfig := mdmlab.AppConfig{
		...
		incomingConfig := mdmlab.AppConfig{
			OrgInfo: mdmlab.OrgInfo{
				OrgName: "Test",
			},
			MDM: mdmlab.MDM{
...
func TestMDMProfileSpecsMatch(t *testing.T) {
	tests := []struct {
		name     string
		a        []mdmlab.MDMProfileSpec
		b        []mdmlab.MDMProfileSpec
		expected bool
	}{
		{
			name:     "Empty Slices",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result := mdmlab.MDMProfileSpecsMatch(tc.a, tc.b)
...
func TestFilterMacOSOnlyProfilesFromIOSIPadOS(t *testing.T) {
	for _, tc := range []struct {
		profiles         []*mdmlab.MDMAppleProfilePayload
		expectedProfiles []*mdmlab.MDMAppleProfilePayload
	}{
		{
			profiles:         []*mdmlab.MDMAppleProfilePayload{},
			expectedProfiles: []*mdmlab.MDMAppleProfilePayload{},
		},
		{
			profiles: []*mdmlab.MDMAppleProfilePayload{
				{
					ProfileName:  mdmlabmdm.MDMlabFileVaultProfileName,
					HostPlatform: "darwin",
				},
				{
					ProfileName:  mdmlabmdm.MDMlabFileVaultProfileName,
				...
				{
					ProfileName:  mdmlabmdm.MDMlabdConfigProfileName,
					HostPlatform: "ipados",
				},
				{
					ProfileName:  mdmlabmdm.MDMlabdConfigProfileName,
		...
		actualProfiles := mdmlab.FilterMacOSOnlyProfilesFromIOSIPadOS(tc.profiles)
