...
var (
	...
	MDMNotConfiguredMessage              = "MDM features aren't turned on in MDMlab. For more information about setting up MDM, please visit https://mdmlabdm.com/docs/using-mdmlab"
	WindowsMDMNotConfiguredMessage       = "Windows MDM isn't turned on. Visit https://mdmlabdm.com/docs/using-mdmlab to learn how to turn on MDM."
	AppleMDMNotConfiguredMessage         = "macOS MDM isn't turned on. Visit https://mdmlabdm.com/docs/using-mdmlab to learn how to turn on MDM."
	AppleABMDefaultTeamDeprecatedMessage = "mdm.apple_bm_default_team has been deprecated. Please use the new mdm.apple_business_manager key documented here: https://mdmlabdm.com/learn-more-about/apple-business-manager-gitops"
...
func (e licenseError) Error() string {
	return "Requires MDMlab Premium license"
}
...
// related to mdmlab logic specifically. Other errors, such as mysql errors, shouldn't
...
// NewError returns a mdmlab error with the code and message specified
...
// NewErrorf returns a mdmlab error with the code, and message formatted
...
// MDMlabdError is an error that can be reported by any of the mdmlabd
// components.
type MDMlabdError struct {
	...
	// Vital errors are always reported to MDMlab server.
	Vital bool `json:"vital"`
}

// Error implements the error interface
func (fe MDMlabdError) Error() string {
...
// MDMlab Desktop)
func (fe MDMlabdError) MarshalZerologObject(e *zerolog.Event) {
...
func (fe MDMlabdError) ToMap() map[string]any {
...
// Message that may surfaced by the server or the mdmlabctl client.
const (
	// Hosts, general
	HostNotFoundErrMsg           = "Host doesn't exist. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mdmlabdm.com/learn-more-about/host-identifiers"
	NoHostsTargetedErrMsg        = "No hosts targeted. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mdmlabdm.com/learn-more-about/host-identifiers"
	TargetedHostsDontExistErrMsg = "One or more targeted hosts don't exist. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mdmlabdm.com/learn-more-about/host-identifiers"
	...
	RunScriptForbiddenErrMsg               = "You don't have the right permissions in MDMlab to run the script."
	RunScriptAlreadyRunningErrMsg          = "A script is already running on this host. Please wait about 5 minutes to let it finish."
	RunScriptHostTimeoutErrMsg             = "MDMlab didn't hear back from the host in under 5 minutes (timeout for live scripts). MDMlab doesn't know if the script ran because it didn't receive the result. Please try again."
	RunScriptScriptsDisabledGloballyErrMsg = "Running scripts is disabled in organization settings."
	RunScriptDisabledErrMsg                = "Scripts are disabled for this host. To run scripts, deploy the mdmlabd agent with scripts enabled."
	RunScriptsOrbitDisabledErrMsg          = "Couldn't run script. To run a script, deploy the mdmlabd agent with --enable-scripts."
	...
	RunScriptGatewayTimeoutErrMsg          = "Gateway timeout. MDMlab didn't hear back from the host and doesn't know if the script ran. Please make sure your load balancer timeout isn't shorter than the MDMlab server timeout."
