...
func NewNudgeConfig(macOSUpdates AppleOSUpdateSettings) (*NudgeConfig, error) {
	...
	// when providing JSON config, so for compatibility with existing mdmlabd versions we're using a somewhat sane
	// UTC value here (noon Pacific Standard Time, no revision for DST). See https://github.com/mdmlabdm/mdmlab/pull/23320
	...
	return &NudgeConfig{
		OSVersionRequirements: []nudgeOSVersionRequirements{{
			...
			AboutUpdateURLs: []nudgeAboutUpdateURLs{{
				Language:       "en",
				AboutUpdateURL: "https://mdmlabdm.com/learn-more-about/os-updates",
			}},
		}},
