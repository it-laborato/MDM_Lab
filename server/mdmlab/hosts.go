...
const (
	...
	HostIdentiferNotFound = "Host doesn't exist. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mdmlabdm.com/learn-more-about/host-identifiers"
)
...
type HostListOptions struct {
	...
	// Deprecated: SoftwareIDFilter is deprecated as of MDMlab 4.42. It is
	...
	// managed by MDMlab. If specified, the SoftwareTitleIDFilter must also be specified.
	...
	// with less than N gigs of disk space available). Note that this is a MDMlab
	// Premium feature, MDMlab Free ignores the setting (it forces it to nil to
	// disable it).
	...
	// (this is a Premium feature that gets forced to false on MDMlab Free)
	...
	// ConnectedToMDMlabFilter filters hosts that have an active MDM
	// connection with this MDMlab instance.
	ConnectedToMDMlabFilter *bool
}
...
type Host struct {
	...
	DesktopVersion *string       `json:"mdmlab_desktop_version" db:"mdmlab_desktop_version" csv:"mdmlab_desktop_version"`
	...
	// to be unenrolled from its old MDM solution, in the "migrate to MDMlab MDM"
	...
	// DEPAssignedToMDMlab is set to true if the host is assigned to MDMlab in Apple Business Manager.
	// It is a *bool becase we want it to be returned from only a subset of endpoints related to
	// Orbit and MDMlab Desktop. Otherwise, it will be set to NULL so it is omitted from JSON
	// responses.
	//
	// The boolean is based on information ingested from the Apple DEP API that is stored in the
	// host_dep_assignments table.
	DEPAssignedToMDMlab *bool `json:"dep_assigned_to_mdmlab,omitempty" db:"dep_assigned_to_mdmlab" csv:"-"`
...
type HostHealth struct {
	...
	FailingCriticalPoliciesCount *int                           `json:"failing_critical_policies_count,omitempty"` // MDMlab Premium Only
...
type HostHealthFailingPolicy struct {
	ID         uint    `json:"id"`
	Name       string  `json:"name"`
	Critical   *bool   `json:"critical,omitempty"` // MDMlab Premium Only
...
type MDMHostData struct {
	...
	// DEPProfileError is a boolean representing whether MDMlab received a "FAILED" response when
	...
	// EncryptionKeyAvailable indicates if MDMlab was able to retrieve and
	...
	// ConnectedToMDMlab indicates if the host has an active MDM connection
	// with this MDMlab instance. This boolean is not filled by all
	// host-returning methods.
	ConnectedToMDMlab *bool `json:"connected_to_mdmlab" csv:"-" db:"connected_to_mdmlab"`
}
...
// IsDEPAssignedToMDMlab returns true if the host was assigned to the MDMlab
// server in ABM.
func (h *Host) IsDEPAssignedToMDMlab() bool {
	return h.DEPAssignedToMDMlab != nil && *h.DEPAssignedToMDMlab
}
...
// unenrolled from MDMlab's Windows MDM (if it MDM was disabled).
func (h *Host) IsEligibleForWindowsMDMUnenrollment(isConnectedToMDMlabMDM bool) bool {
	return h.MDMlabPlatform() == "windows" &&
		h.IsOsqueryEnrolled() &&
...
func (h *Host) Status(now time.Time) HostStatus {
	...
	// NOTE: As of MDMlab 4.15 StatusMIA is deprecated and will be removed in MDMlab 5.0
...
// MDMlabPlatform returns the host's generic platform as supported by MDMlab.
func (h *Host) MDMlabPlatform() string {
...
func PlatformFromHost(hostPlatform string) string {
	...
	case hostPlatform == "darwin", hostPlatform == "windows",
		...
		// TODO remove this once that customer migrates to MDMlabd for Chrome
		hostPlatform == "CrOS",
		// MDMlab now supports Chrome via mdmlabd
		hostPlatform == "chrome",
		hostPlatform == "ios",
		hostPlatform == "ipados":
		return hostPlatform
...
const (
	...
	DeviceMappingCustomInstaller      = "custom_installer" // set by mdmlabd via device-authenticated API
...
// HasJSONProfileAssigned returns true if MDMlab has assigned an ADE/DEP JSON
// profile to the host, and it'll be enrolled into MDMlab the next time the host
...
type OSVersion struct {
	...
	// "Ubuntu", or "macOS". NOTE: In MDMlab 5.0, this field will no longer include the alphanumeric version.
	Name string `json:"name"`
	// NameOnly is the name of the operating system, e.g., "Microsoft Windows 11 Enterprise",
	// "Ubuntu", or "macOS". NOTE: In MDMlab 5.0, this field be removed.
...
type HostSoftwareInstalledPath struct {
	...
	// TeamIdentifier (not to be confused with MDMlab's team IDs) is the Apple's "Team ID" (aka "Developer ID"
...
// for DEP migration from a third-party provider into MDMlab.
func IsEligibleForDEPMigration(host *Host, mdmInfo *HostMDM, isConnectedToMDMlabMDM bool) bool {
	return mdmInfo != nil &&
		host.IsOsqueryEnrolled() &&
		host.IsDEPAssignedToMDMlab() &&
		...
		// the host needs migration if it's not MDMlab
		//
		// this is because we can't always rely on nano setting
		// `nano_enrollment.active = 1` since sometimes MDMlab won't get
		// the checkout message from the host.
		(!isConnectedToMDMlabMDM || mdmInfo.Name != WellKnownMDMMDMlab)
}
...
// and is able to migrate to MDMlab.
func IsEligibleForManualMigration(host *Host, mdmInfo *HostMDM, isConnectedToMDMlabMDM bool) (bool, error) {
	...
	return goodVersion &&
		...
		!mdmInfo.HasJSONProfileAssigned() &&
		mdmInfo.Enrolled &&
		(!isConnectedToMDMlabMDM || mdmInfo.Name != WellKnownMDMMDMlab), nil
}
