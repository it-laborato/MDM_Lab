...
func (c CmdID) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if c.IncludeMDMlabComment {
		if err := e.EncodeToken(xml.Comment(" CmdID generated by MDMlab ")); err != nil {
			return err
		}
	}
...
// WindowsMDMRequiresPremiumCmdMessage is the error message displayed by mdmlabctl mdm
...
// command names.
const WindowsMDMRequiresPremiumCmdMessage = "Missing or invalid license. Wipe command is available in MDMlab Premium only."

// IsPremium returns true if the command is available for MDMlab premium only.
...
func BuildMDMWindowsProfilePayloadFromMDMResponse(
	// IMPORTANT: The cmdWithSecret.RawCommand may contain a MDMlab secret variable value, so it should never be exposed or saved.
