...
// for running a script synchronously (so that mdmlabctl can display it) and to
...
func HostScriptTimeoutMessage(seconds *int) string {
	...
	return fmt.Sprintf("Timeout. MDMlab stopped the script after %d seconds to protect host performance.", timeout)
}
...
type HostLockWipeStatus struct {
	// HostMDMlabPlatform is the mdmlab-normalized platform of the host, i.e. the
	// result of host.MDMlabPlatform().
	HostMDMlabPlatform string
...
func (s *HostLockWipeStatus) IsPendingLock() bool {
	if s.HostMDMlabPlatform == "darwin" || s.HostMDMlabPlatform == "ios" || s.HostMDMlabPlatform == "ipados" {
