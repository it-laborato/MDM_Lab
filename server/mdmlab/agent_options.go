...
type AgentOptions struct {
	...
	// UpdateChannels holds the configured channels for mdmlabd components.
...
// at the time of the MDMlab release.
func ValidateJSONAgentOptions(ctx context.Context, ds Datastore, rawJSON json.RawMessage, isPremium bool) error {
	...
	if len(opts.CommandLineStartUpFlags) > 0 {
		...
		// We prevent setting the following flags because they can break mdmlabd.
...
type osqueryAgentOptions struct {
	...
	// See https://github.com/mdmlabdm/mdmlab/issues/7871#issuecomment-1265531018
