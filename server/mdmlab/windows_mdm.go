...
// zero because these states are not in MDMlab's current implementation of BitLocker management.
...
// MDMWindowsConfigProfile represents a Windows MDM profile in MDMlab.
type MDMWindowsConfigProfile struct {
	// ProfileUUID is the unique identifier of the configuration profile in
	// MDMlab. For Windows profiles, it is the letter "w" followed by a uuid.
...
func (m *MDMWindowsConfigProfile) ValidateUserProvided() error {
	...
	// keep track of certain elements to perform MDMlab-validations.
	//
	...
	for {
		...
		case xml.CharData:
			if inLocURI {
				if err := validateMDMlabProvidedLocURI(string(t)); err != nil {
					return err
				}
			}
		}
...
var mdmlabProvidedLocURIValidationMap = map[string][2]string{
	syncml.MDMlabBitLockerTargetLocURI: {"BitLocker", "mdm.enable_disk_encryption"},
	syncml.MDMlabOSUpdateTargetLocURI:  {"Windows updates", "mdm.windows_updates"},
}

func validateMDMlabProvidedLocURI(locURI string) error {
	sanitizedLocURI := strings.TrimSpace(locURI)
	for mdmlabLocURI, errHints := range mdmlabProvidedLocURIValidationMap {
		if strings.Contains(sanitizedLocURI, mdmlabLocURI) {
