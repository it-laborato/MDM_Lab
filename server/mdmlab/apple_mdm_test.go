...
func TestMDMAppleConfigProfileScreenPayloadIdentifiers(t *testing.T) {
	cases := []struct {
		...
		{
			testName:           "AllMDMlabProfilesScreened",
			payloadIdentifiers: []string{"com.mdmlabdm.mdmlab.mdm.filevault", "com.mdmlabdm.mdmlabd.config"},
			shouldFail:         []string{"com.mdmlabdm.mdmlab.mdm.filevault", "com.mdmlabdm.mdmlabd.config"},
		},
		{
			testName:           "FileVault",
			payloadIdentifiers: []string{"com.mdmlabdm.mdmlab.mdm.filevault"},
			shouldFail:         []string{"com.mdmlabdm.mdmlab.mdm.filevault"},
		},
		{
			testName:           "MDMlabd config",
			payloadIdentifiers: []string{"com.mdmlabdm.mdmlabd.config"},
			shouldFail:         []string{"com.mdmlabdm.mdmlabd.config"},
		},
		{
			testName:           "OtherPayloadTypesOK",
		...
		{
			testName:           "Mixed",
			payloadIdentifiers: []string{"com.mdmlabdm.mdmlab.mdm.filevault", "com.my.custom.profile", "com.test.example"},
			shouldFail:         []string{"com.mdmlabdm.mdmlab.mdm.filevault"},
		},
		{
			testName:           "NoPayloadContent",
...
func TestHostDEPAssignment(t *testing.T) {
	cases := []struct {
		...
		{
			testName: "was assigned MDMlab but now deleted",
			input: HostDEPAssignment{
				HostID:    1,
				AddedAt:   time.Now(),
	...
	for _, c := range cases {
		t.Run(c.testName, func(t *testing.T) {
			require.Equal(t, c.expect, c.input.IsDEPAssignedToMDMlab())
		})
	}
}
...
func TestMDMProfileIsWithinGracePeriod(t *testing.T) {
	...
	params := mobileconfig.MDMlabdProfileOptions{
		...
		PayloadName:  mdm.MDMlabdConfigProfileName,
	}
