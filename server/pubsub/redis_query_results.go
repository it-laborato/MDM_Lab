...
var _ mdmlab.QueryResultStore = &redisQueryResults{}
...
func NewRedisQueryResults(pool mdmlab.RedisPool, duplicateResults bool, logger log.Logger) *redisQueryResults {
...
func (r *redisQueryResults) Pool() mdmlab.RedisPool {
	return r.pool
}

func (r *redisQueryResults) WriteResult(result mdmlab.DistributedQueryResult) error {
...
func (r *redisQueryResults) ReadChannel(ctx context.Context, query mdmlab.DistributedQueryCampaign) (<-chan interface{}, error) {
		...
		for {
			...
			case msg, ok := <-msgChannel:
				if !ok {
					...
					// when MDMlab's connection to Redis has been interrupted unexpectedly.
					//
				...
				switch msg := msg.(type) {
				case redigo.Message:
					var res mdmlab.DistributedQueryResult
