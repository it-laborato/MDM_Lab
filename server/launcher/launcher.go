...
// clients to connect to MDMlab. These clients use a gRPC transport to hit the equivalent API
// endpoints.
//
// MDMlab intends to maintain support for Launcher as long as the Launcher API remains as closely
// mapped to the osquery API as it is currently, and MDMlab users continue to use Launcher. In the
// future as more MDMlab users migrate to Orbit, this may be deprecated.
package launcher
...
func (svc *launcherWrapper) PublishResults(ctx context.Context, nodeKey string, results []distributed.Result) (string, string, bool, error) {
	...
	osqueryResults := make(mdmlab.OsqueryDistributedQueryResults, len(results))
	statuses := make(map[string]mdmlab.OsqueryStatus, len(results))
	stats := make(map[string]*mdmlab.Stats, len(results))

	for _, result := range results {
		statuses[result.QueryName] = mdmlab.OsqueryStatus(result.Status)
		osqueryResults[result.QueryName] = result.Rows
		if result.QueryStats != nil {
			stats[result.QueryName] = &mdmlab.Stats{
...
// In the mdmlab.OsqueryService authentication is done via endpoint middleware, but all launcher endpoints require
