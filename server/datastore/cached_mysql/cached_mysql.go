...
//     mdmlab.ScheduledQueryList for an example, or packsList in this package
//     for an alternative approach).
//  4. Add the cached item to mdmlab/tools/cloner-check/main.go (in the
...
func (c *cloneCache) Get(ctx context.Context, k string) (mdmlab.Cloner, bool) {
...
func (c *cloneCache) Set(ctx context.Context, k string, x mdmlab.Cloner, d time.Duration) {
	clone, err := x.Clone()
	if err != nil {
...
func New(ds mdmlab.Datastore, opts ...Option) mdmlab.Datastore {
	c := &cachedMysql{
		Datastore:            ds,
...
func (ds *cachedMysql) NewAppConfig(ctx context.Context, info *mdmlab.AppConfig) (*mdmlab.AppConfig, error) {
...
func (ds *cachedMysql) AppConfig(ctx context.Context) (*mdmlab.AppConfig, error) {
	if x, found := ds.c.Get(ctx, appConfigKey); found {
		ac, ok := x.(*mdmlab.AppConfig)
		if ok {
			return ac, nil
		}
	}
...
func (ds *cachedMysql) SaveAppConfig(ctx context.Context, info *mdmlab.AppConfig) error {
...
func (ds *cachedMysql) ListPacksForHost(ctx context.Context, hid uint) ([]*mdmlab.Pack, error) {
...
func (ds *cachedMysql) ListScheduledQueriesInPack(ctx context.Context, packID uint) (mdmlab.ScheduledQueryList, error) {
	key := fmt.Sprintf(scheduledQueriesKey, packID)
	if x, found := ds.c.Get(ctx, key); found {
		scheduledQueries, ok := x.(mdmlab.ScheduledQueryList)
		if ok {
...
func (ds *cachedMysql) TeamFeatures(ctx context.Context, teamID uint) (*mdmlab.Features, error) {
	key := fmt.Sprintf(teamFeaturesKey, teamID)
	if x, found := ds.c.Get(ctx, key); found {
		if features, ok := x.(*mdmlab.Features); ok {
			return features, nil
		}
	}
...
func (ds *cachedMysql) TeamMDMConfig(ctx context.Context, teamID uint) (*mdmlab.TeamMDM, error) {
	key := fmt.Sprintf(teamMDMConfigKey, teamID)
	if x, found := ds.c.Get(ctx, key); found {
		if cfg, ok := x.(*mdmlab.TeamMDM); ok {
			return cfg, nil
		}
	}
...
func (ds *cachedMysql) SaveTeam(ctx context.Context, team *mdmlab.Team) (*mdmlab.Team, error) {
...
func (ds *cachedMysql) QueryByName(ctx context.Context, teamID *uint, name string) (*mdmlab.Query, error) {
...
func (ds *cachedMysql) GetAllMDMConfigAssetsByName(ctx context.Context, assetNames []mdmlab.MDMAssetName,
	queryerContext sqlx.QueryerContext) (map[mdmlab.MDMAssetName]mdmlab.MDMConfigAsset, error) {
	...
	cachedAssets := make(map[mdmlab.MDMAssetName]mdmlab.MDMConfigAsset)
	var missingAssets []mdmlab.MDMAssetName

	for _, name := range assetNames {
		key := fmt.Sprintf(mdmConfigAssetKey, name, latestHashes[name])

		if x, found := ds.c.Get(ctx, key); found {
			asset, ok := x.(mdmlab.MDMConfigAsset)
			if ok {
