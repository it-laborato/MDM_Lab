...
func (ds *Datastore) ListScheduledQueriesInPackWithStats(ctx context.Context, id uint, opts mdmlab.ListOptions) ([]*mdmlab.ScheduledQuery, error) {
	query := `
		SELECT
			sq.id,
			sq.pack_id,
		...
		WHERE sq.pack_id = ?
	`
	params := []interface{}{false, mdmlab.AggregatedStatsTypeScheduledQuery, id}
	query, params = appendListOptionsWithCursorToSQL(query, params, &opts)
	results := []*mdmlab.ScheduledQuery{}
...
func (ds *Datastore) ListScheduledQueriesInPack(ctx context.Context, id uint) (mdmlab.ScheduledQueryList, error) {
	query := `
		SELECT
			sq.id,
			sq.pack_id,
...
func (ds *Datastore) NewScheduledQuery(ctx context.Context, sq *mdmlab.ScheduledQuery, opts ...mdmlab.OptionalArg) (*mdmlab.ScheduledQuery, error) {
	return insertScheduledQueryDB(ctx, ds.writer(ctx), sq)
}

func insertScheduledQueryDB(ctx context.Context, q sqlx.ExtContext, sq *mdmlab.ScheduledQuery) (*mdmlab.ScheduledQuery, error) {
...
func (ds *Datastore) SaveScheduledQuery(ctx context.Context, sq *mdmlab.ScheduledQuery) (*mdmlab.ScheduledQuery, error) {
	return saveScheduledQueryDB(ctx, ds.writer(ctx), sq)
}

func saveScheduledQueryDB(ctx context.Context, exec sqlx.ExecerContext, sq *mdmlab.ScheduledQuery) (*mdmlab.ScheduledQuery, error) {
	query := `
...
func (ds *Datastore) ScheduledQuery(ctx context.Context, id uint) (*mdmlab.ScheduledQuery, error) {
	query := `
		SELECT
			sq.id,
			sq.created_at,
...
`
	...
	if batchSize <= 0 {
		batchSize = mdmlab.DefaultScheduledQueryIDsByNameBatchSize
	}
...
func (ds *Datastore) AsyncBatchSaveHostsScheduledQueryStats(ctx context.Context, stats map[uint][]mdmlab.ScheduledQueryStats, batchSize int) (int, error) {
