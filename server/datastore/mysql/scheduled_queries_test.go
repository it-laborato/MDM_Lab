...
func testScheduledQueriesListInPackWithStats(t *testing.T, ds *Datastore) {
	...
	queries := []*mdmlab.Query{
		...
		{Name: "bar", Description: "do some bars", Query: "select baz from bar", Logging: mdmlab.LoggingDifferential},
	}
	...
	specs := []*mdmlab.PackSpec{
		{
			Name:    "baz",
			Targets: mdmlab.PackSpecTargets{Labels: []string{}},
	...
	gotQueries, err := ds.ListScheduledQueriesInPackWithStats(context.Background(), 1, mdmlab.ListOptions{})
	require.NoError(t, err)
...
func testScheduledQueriesNew(t *testing.T, ds *Datastore) {
	u1 := test.NewUser(t, ds, "Admin", "admin@mdmlab.co", true)
	q1 := test.NewQuery(t, ds, nil, "foo", "select * from time;", u1.ID, true)
	p1 := test.NewPack(t, ds, "baz")

	query, err := ds.NewScheduledQuery(context.Background(), &mdmlab.ScheduledQuery{
		PackID:  p1.ID,
		QueryID: q1.ID,
		Name:    "foo-scheduled",
	})
...
func testScheduledQueriesIDsByName(t *testing.T, ds *Datastore) {
	...
	queries := []*mdmlab.Query{
		{Name: "foo", Description: "get the foos", Query: "select * from foo", Logging: mdmlab.LoggingSnapshot},
		{Name: "bar", Description: "do some bars", Query: "select * from bar", Logging: mdmlab.LoggingSnapshot},
		{Name: "foo2", Description: "get the foos", Query: "select * from foo2", Logging: mdmlab.LoggingSnapshot},
		{Name: "bar2", Description: "do some bars", Query: "select * from bar2", Logging: mdmlab.LoggingSnapshot},
	}
...
func testScheduledQueriesAsyncBatchSaveStats(t *testing.T, ds *Datastore) {
	...
	assertStats := func(m map[uint][]mdmlab.ScheduledQueryStats) {
	...
	m := map[uint][]mdmlab.ScheduledQueryStats{
		h1.ID: {
			{
