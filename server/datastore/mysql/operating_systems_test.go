...
func TestListOperatingSystems(t *testing.T) {
	...
	osByID := make(map[uint]mdmlab.OperatingSystem)
	for _, os := range list {
		osByID[os.ID] = os
	}
...
func TestCleanupHostOperatingSystems(t *testing.T) {
	...
	testHosts := make([]*mdmlab.Host, 10)
	osByHostID := make(map[uint]mdmlab.OperatingSystem)
	...
	assertDeletedOS := func(expectDeletedIDs []uint) {
		...
		byID := make(map[uint]mdmlab.OperatingSystem)
		for _, os := range list {
			byID[os.ID] = os
		}
...
func seedOperatingSystems(t *testing.T, ds *Datastore) map[uint]mdmlab.OperatingSystem {
	osSeeds := []mdmlab.OperatingSystem{
		{
	...
	storedById := make(map[uint]mdmlab.OperatingSystem)
...
func isSameOS(t *testing.T, os1 mdmlab.OperatingSystem, os2 mdmlab.OperatingSystem) bool {
