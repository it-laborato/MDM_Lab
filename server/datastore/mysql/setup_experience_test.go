...
func testEnqueueSetupExperienceItems(t *testing.T, ds *Datastore) {
	...
	err = ds.SetSetupExperienceScript(ctx, &mdmlab.Script{Name: "script1", ScriptContents: "SCRIPT 1", TeamID: &team1.ID})
	require.NoError(t, err)
	err = ds.SetSetupExperienceScript(ctx, &mdmlab.Script{Name: "script2", ScriptContents: "SCRIPT 2", TeamID: &team2.ID})
	require.NoError(t, err)
...
func testGetSetupExperienceTitles(t *testing.T, ds *Datastore) {
	...
	tfr4, err := mdmlab.NewTempFileReader(strings.NewReader("hello2"), t.TempDir)
	require.NoError(t, err)
	...
	titles, count, meta, err := ds.ListSetupExperienceSoftwareTitles(ctx, team1.ID, mdmlab.ListOptions{})
	require.NoError(t, err)
	...
	titles, count, meta, err = ds.ListSetupExperienceSoftwareTitles(ctx, team2.ID, mdmlab.ListOptions{})
	require.NoError(t, err)
	assert.Len(t, titles, 1)
	...
	app2 := &mdmlab.VPPApp{Name: "vpp_app_2", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "2", Platform: mdmlab.IOSPlatform}}, BundleIdentifier: "b2"}
...
func testSetSetupExperienceTitles(t *testing.T, ds *Datastore) {
	...
	softwareTitles, _, _, err := ds.ListSoftwareTitles(ctx, mdmlab.SoftwareTitleListOptions{TeamID: &team1.ID}, mdmlab.TeamFilter{TeamID: &team1.ID})
	require.NoError(t, err)
	...
	softwareTitles, _, _, err = ds.ListSoftwareTitles(ctx, mdmlab.SoftwareTitleListOptions{TeamID: &team2.ID}, mdmlab.TeamFilter{TeamID: &team2.ID})
	require.NoError(t, err)
...
func testSetupExperienceStatusResults(t *testing.T, ds *Datastore) {
	...
	user, err := ds.NewUser(ctx, &mdmlab.User{Name: "Foo", Email: "foo@example.com", GlobalRole: ptr.String("admin"), Password: []byte("12characterslong!")})
	require.NoError(t, err)
	installerID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{Filename: "test.app", Version: "1.0.0", UserID: user.ID, ValidatedLabels: &mdmlab.LabelIdentsWithScope{}})
	require.NoError(t, err)
	...
	assert.NoError(t, err)
	vppApp, err := ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{BundleIdentifier: "com.test.test", Name: "test.app", LatestVersion: "1.0.0"}, nil)
	require.NoError(t, err)
	...
	insertSetupExperienceStatusResult := func(sesr *mdmlab.SetupExperienceStatusResult) {
	...
	expRes := []*mdmlab.SetupExperienceStatusResult{
		{
			HostUUID:            hostUUID,
			Name:                "software",
...
func testSetupExperienceScriptCRUD(t *testing.T, ds *Datastore) {
	...
	wantScriptNoTeam := &mdmlab.Script{
		Name:           "script",
		ScriptContents: "echo bar",
	}
	...
	err = ds.SetSetupExperienceScript(ctx, &mdmlab.Script{Name: "script", ScriptContents: "echo baz"})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)

	// try to create another script with no team id and a different name
	err = ds.SetSetupExperienceScript(ctx, &mdmlab.Script{Name: "script2", ScriptContents: "echo baz"})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)

	// try to add a script for a team that doesn't exist
	var fkErr mdmlab.ForeignKeyError
	err = ds.SetSetupExperienceScript(ctx, &mdmlab.Script{TeamID: ptr.Uint(42), Name: "script", ScriptContents: "echo baz"})
	require.Error(t, err)
