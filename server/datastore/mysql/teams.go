...
func (ds *Datastore) NewTeam(ctx context.Context, team *mdmlab.Team) (*mdmlab.Team, error) {
...
func (ds *Datastore) Team(ctx context.Context, tid uint) (*mdmlab.Team, error) {
	return teamDB(ctx, ds.reader(ctx), tid, true)
}

func (ds *Datastore) TeamWithoutExtras(ctx context.Context, tid uint) (*mdmlab.Team, error) {
	return teamDB(ctx, ds.reader(ctx), tid, false)
}

func teamDB(ctx context.Context, q sqlx.QueryerContext, tid uint, withExtras bool) (*mdmlab.Team, error) {
	stmt := `
	...
	if withExtras {
		if err := loadSecretsForTeamsDB(ctx, q, []*mdmlab.Team{team}); err != nil {
...
func saveTeamSecretsDB(ctx context.Context, q sqlx.ExtContext, team *mdmlab.Team) error {
	if team.Secrets == nil {
		return nil
	}
...
func (ds *Datastore) DeleteTeam(ctx context.Context, tid uint) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		_, err = tx.ExecContext(ctx, `DELETE FROM pack_targets WHERE type=? AND target_id=?`, mdmlab.TargetTeam, tid)
		if err != nil {
...
func (ds *Datastore) TeamByName(ctx context.Context, name string) (*mdmlab.Team, error) {
	// We must normalize the name for full Unicode support (Unicode equivalence).
...
func (ds *Datastore) loadExtrasForTeam(ctx context.Context, team *mdmlab.Team) (*mdmlab.Team, error) {
	if err := loadSecretsForTeamsDB(ctx, ds.reader(ctx), []*mdmlab.Team{team}); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "getting secrets for teams")
	}
...
func (ds *Datastore) TeamByFilename(ctx context.Context, filename string) (*mdmlab.Team, error) {
	stmt := `
		SELECT ` + teamColumns + ` FROM teams
			WHERE filename = ?
	`
...
func loadUsersForTeamDB(ctx context.Context, q sqlx.QueryerContext, team *mdmlab.Team) error {
	sql := `
		SELECT u.name, u.id, u.email, ut.role
		FROM user_teams ut JOIN users u ON (ut.user_id = u.id)
		WHERE ut.team_id = ?
	`
	rows := []mdmlab.TeamUser{}
...
func loadHostCountForTeamDB(ctx context.Context, q sqlx.QueryerContext, team *mdmlab.Team) error {
	stmt := `
...
func loadFeaturesForTeamDB(ctx context.Context, q sqlx.QueryerContext, team *mdmlab.Team) error {
...
func saveUsersForTeamDB(ctx context.Context, exec sqlx.ExecerContext, team *mdmlab.Team) error {
...
func (ds *Datastore) SaveTeam(ctx context.Context, team *mdmlab.Team) (*mdmlab.Team, error) {
	// We must normalize the name for full Unicode support (Unicode equivalence).
	team.Name = norm.NFC.String(team.Name)
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		query := `
...
// mdmlab.ListOptions
func (ds *Datastore) ListTeams(ctx context.Context, filter mdmlab.TeamFilter, opt mdmlab.ListOptions) ([]*mdmlab.Team, error) {
	query := fmt.Sprintf(`
			SELECT `+teamColumns+`,
...
func loadSecretsForTeamsDB(ctx context.Context, q sqlx.QueryerContext, teams []*mdmlab.Team) error {
...
func (ds *Datastore) TeamsSummary(ctx context.Context) ([]*mdmlab.TeamSummary, error) {
	teamsSummary := []*mdmlab.TeamSummary{}
...
func (ds *Datastore) SearchTeams(ctx context.Context, filter mdmlab.TeamFilter, matchQuery string, omit ...uint) ([]*mdmlab.Team, error) {
	sql := fmt.Sprintf(`
			SELECT %s,
				(SELECT count(*) FROM user_teams WHERE team_id = t.id) AS user_count,
				(SELECT count(*) FROM hosts WHERE team_id = t.id) AS host_count
...
func (ds *Datastore) TeamEnrollSecrets(ctx context.Context, teamID uint) ([]*mdmlab.EnrollSecret, error) {
	sql := `
...
func (ds *Datastore) TeamFeatures(ctx context.Context, tid uint) (*mdmlab.Features, error) {
	return teamFeaturesDB(ctx, ds.reader(ctx), tid)
}

func teamFeaturesDB(ctx context.Context, q sqlx.QueryerContext, tid uint) (*mdmlab.Features, error) {
...
func (ds *Datastore) TeamMDMConfig(ctx context.Context, tid uint) (*mdmlab.TeamMDM, error) {
...
// that uses it.
func (ds *Datastore) DeleteIntegrationsFromTeams(ctx context.Context, deletedIntgs mdmlab.Integrations) error {
	const (
	...
	for rows.Next() {
		...
		if matches, _ := tm.Config.Integrations.MatchWithIntegrations(deletedIntgs); len(matches.Jira)+len(matches.Zendesk) > 0 {
			delJira, _ := mdmlab.IndexJiraIntegrations(matches.Jira)
			delZendesk, _ := mdmlab.IndexZendeskIntegrations(matches.Zendesk)

			var keepJira []*mdmlab.TeamJiraIntegration
			...
			var keepZendesk []*mdmlab.TeamZendeskIntegration
