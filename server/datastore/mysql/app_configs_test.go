...
func testAppConfigOrgInfo(t *testing.T, ds *Datastore) {
	...
	info2.SMTPSettings.SMTPAuthenticationType = mdmlab.AuthTypeNameUserNamePassword
	info2.SMTPSettings.SMTPUserName = "username"
	info2.SMTPSettings.SMTPPassword = "password"
	info2.SMTPSettings.SMTPEnableTLS = false
	info2.SMTPSettings.SMTPAuthenticationMethod = mdmlab.AuthMethodNameCramMD5
...
func testAppConfigEnrollSecrets(t *testing.T, ds *Datastore) {
	...
	// Remove team secrets
	err = ds.ApplyEnrollSecrets(ctx, &team1.ID, []*mdmlab.EnrollSecret{})
	assert.NoError(t, err)
	secret, err = ds.VerifyEnrollSecret(ctx, "one_secret")
	assert.Error(t, err)
	assert.Nil(t, secret)
...
func testAppConfigEnrollSecretRoundtrip(t *testing.T, ds *Datastore) {
	...
	expectedSecrets := []*mdmlab.EnrollSecret{
		{Secret: "one_secret"},
		{Secret: "two_secret"},
	}
...
func testAggregateEnrollSecretPerTeam(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(context.Background(), &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	err = ds.ApplyEnrollSecrets(context.Background(), &team1.ID, []*mdmlab.EnrollSecret{
	...
	_, err = ds.NewTeam(context.Background(), &mdmlab.Team{Name: "team2"})
	require.NoError(t, err)

	// a team with a single enroll secret
	team3, err := ds.NewTeam(context.Background(), &mdmlab.Team{Name: "team3"})
	require.NoError(t, err)
	err = ds.ApplyEnrollSecrets(context.Background(), &team3.ID, []*mdmlab.EnrollSecret{
	...
	require.ElementsMatch(t, []*mdmlab.EnrollSecret{
...
func testIsEnrollSecretAvailable(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.ApplyEnrollSecrets(ctx, nil, []*mdmlab.EnrollSecret{{Secret: "globalSecret"}}))
	require.NoError(t, ds.ApplyEnrollSecrets(ctx, &team1.ID, []*mdmlab.EnrollSecret{{Secret: "teamSecret"}}))
...
func testNDESSCEPProxyPassword(t *testing.T, ds *Datastore) {
	...
	ac.Integrations.NDESSCEPProxy = optjson.Any[mdmlab.NDESSCEPProxyIntegration]{
		Valid: true,
		Set:   true,
		Value: mdmlab.NDESSCEPProxyIntegration{
			AdminURL: adminURL,
			Username: username,
			Password: password,
			URL:      url,
	...
	checkProxyConfig := func() {
		...
		assert.Equal(t, mdmlab.MaskedPassword, result.Integrations.NDESSCEPProxy.Value.Password)
	}

	checkProxyConfig()

	checkPassword := func() {
		assets, err := ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetNDESPassword}, nil)
		require.NoError(t, err)
		require.Len(t, assets, 1)
		assert.Equal(t, password, string(assets[mdmlab.MDMAssetNDESPassword].Value))
	}
	checkPassword()

	// Set password to masked password -- should not update
	ac.Integrations.NDESSCEPProxy.Value.Password = mdmlab.MaskedPassword
	err = ds.SaveAppConfig(ctx, ac)
	require.NoError(t, err)
	checkProxyConfig()
	checkPassword()
