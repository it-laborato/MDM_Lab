...
func testActivityUsernameChange(t *testing.T, ds *Datastore) {
	...
	activities, _, err := ds.ListActivities(context.Background(), mdmlab.ListActivitiesOptions{})
	require.NoError(t, err)
...
func testActivityNew(t *testing.T, ds *Datastore) {
	...
	ctx := context.WithValue(context.Background(), mdmlab.ActivityWebhookContextKey, "bozo")
	...
	opt := mdmlab.ListActivitiesOptions{
...
func testListActivitiesStreamed(t *testing.T, ds *Datastore) {
	...
	nonStreamed, _, err := ds.ListActivities(context.Background(), mdmlab.ListActivitiesOptions{
	...
	streamed, _, err := ds.ListActivities(context.Background(), mdmlab.ListActivitiesOptions{
...
func testActivityEmptyUser(t *testing.T, ds *Datastore) {
	...
	require.NoError(
		t, ds.NewActivity(
			ctx, nil, mdmlab.ActivityInstalledAppStoreApp{
				HostID:          1,
	...
	require.NoError(t, err)
	assert.Len(t, activities, 2)
	assert.Equal(t, "MDMlab", *activities[1].ActorFullName)
}
...
func testActivityPaginationMetadata(t *testing.T, ds *Datastore) {
	...
	cases := []struct {
		...
		count int
		meta  *mdmlab.PaginationMetadata
	}{
		{
			"default options",
			mdmlab.ListOptions{PerPage: 0},
			3,
			&mdmlab.PaginationMetadata{},
		},
		{
			"per page 2",
			mdmlab.ListOptions{PerPage: 2},
			2,
			&mdmlab.PaginationMetadata{HasNextResults: true},
		},
		{
			"per page 2 - page 1",
			mdmlab.ListOptions{PerPage: 2, Page: 1},
			1,
			&mdmlab.PaginationMetadata{HasPreviousResults: true},
		},
		{
			"per page 3",
			mdmlab.ListOptions{PerPage: 3},
			3,
			&mdmlab.PaginationMetadata{},
		},
		{
			`after "0" - orderKey "a.id"`,
			mdmlab.ListOptions{After: "0", OrderKey: "a.id"},
			3,
			nil,
		},
		{
			"per page 4",
			mdmlab.ListOptions{PerPage: 4},
			3,
			&mdmlab.PaginationMetadata{},
		},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			activities, metadata, err := ds.ListActivities(context.Background(), mdmlab.ListActivitiesOptions{ListOptions: c.opts})
			require.NoError(t, err)
...
func testListHostUpcomingActivities(t *testing.T, ds *Datastore) {
	...
	scr1, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "A",
		ScriptContents: "A",
	})
	require.NoError(t, err)
	scr2, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "B",
		ScriptContents: "B",
	})
	require.NoError(t, err)
	...
	installer1, err := mdmlab.NewTempFileReader(strings.NewReader("echo"), t.TempDir)
	require.NoError(t, err)
	sw1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
	...
	require.NoError(t, err)
	installer2, err := mdmlab.NewTempFileReader(strings.NewReader("echo"), t.TempDir)
	require.NoError(t, err)
	sw2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
	...
	require.NoError(t, err)
	installer3, err := mdmlab.NewTempFileReader(strings.NewReader("echo"), t.TempDir)
	require.NoError(t, err)
	sw3, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
	...
	vppApp := &mdmlab.VPPApp{
		Name: "vpp_no_team_app_1", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "3", Platform: mdmlab.MacOSPlatform}},
		BundleIdentifier: "b3",
	}
	...
	hsr, err := ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h1.ID, ScriptContents: "sync", UserID: &u.ID, SyncRequest: true})
	require.NoError(t, err)
	...
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h1.ID, ScriptID: &scr1.ID, ScriptContents: scr1.ScriptContents, UserID: &u.ID})
	require.NoError(t, err)
	h1A := hsr.ExecutionID
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h1.ID, ScriptID: &scr2.ID, ScriptContents: scr2.ScriptContents, UserID: &u.ID})
	require.NoError(t, err)
	h1B := hsr.ExecutionID
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h1.ID, ScriptContents: "C", UserID: &u.ID})
	require.NoError(t, err)
	h1C := hsr.ExecutionID
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h1.ID, ScriptContents: "D"})
	require.NoError(t, err)
	h1D := hsr.ExecutionID
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h1.ID, ScriptContents: "E"})
	require.NoError(t, err)
	h1E := hsr.ExecutionID
	...
	// No user for this one and not Self-service, means it was installed by MDMlab
	policy, err := ds.NewTeamPolicy(ctx, 0, &u.ID, mdmlab.PolicyPayload{
		Name:  "Test Policy",
		Query: "SELECT 1",
	})
	require.NoError(t, err)
	h1MDMlab, err := ds.InsertSoftwareInstallRequest(noUserCtx, h1.ID, sw1Meta.InstallerID, false, &policy.ID)
	require.NoError(t, err)

	// create a single pending request for h2, as well as a non-pending one
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h2.ID, ScriptID: &scr1.ID, ScriptContents: scr1.ScriptContents, UserID: &u.ID})
	require.NoError(t, err)
	h2A := hsr.ExecutionID
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h2.ID, ScriptContents: "F", UserID: &u.ID})
	require.NoError(t, err)
	_, _, err = ds.SetHostScriptExecutionResult(ctx,
		&mdmlab.HostScriptResultPayload{HostID: h2.ID, ExecutionID: hsr.ExecutionID, Output: "ok", ExitCode: 0})
	require.NoError(t, err)
	h2F := hsr.ExecutionID
	...
	setupExpScript := &mdmlab.Script{Name: "setup_experience_script", ScriptContents: "setup_experience"}
	err = ds.SetSetupExperienceScript(ctx, setupExpScript)
	require.NoError(t, err)
	ses, err := ds.GetSetupExperienceScript(ctx, h2.TeamID)
	require.NoError(t, err)
	hsr, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h2.ID, ScriptContents: "setup_experience", SetupExperienceScriptID: &ses.ID})
	require.NoError(t, err)
	...
	_, err = ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: h4.ID, ScriptID: &scr1.ID, ScriptContents: scr1.ScriptContents, UserID: &u.ID})
	require.NoError(t, err)
	// h4A := hsr.ExecutionID
	...
	endTime = SetOrderedCreatedAtTimestamps(t, ds, endTime, "host_software_installs", "execution_id", h1FooInstalled, h1MDMlab)
	endTime = SetOrderedCreatedAtTimestamps(t, ds, endTime, "host_software_installs", "execution_id", h1MDMlab)
	...
	cases := []struct {
		...
		{
			opts:      mdmlab.ListOptions{PerPage: 2},
			hostID:    h1.ID,
			wantExecs: []string{h1A, h1B},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: false, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{Page: 1, PerPage: 2},
			hostID:    h1.ID,
			wantExecs: []string{h1Bar, h1C},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: true, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{Page: 2, PerPage: 2},
			hostID:    h1.ID,
			wantExecs: []string{h1D, h1E},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: true, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{Page: 3, PerPage: 2},
			hostID:    h1.ID,
			wantExecs: []string{h1MDMlab, vppCommand1},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{PerPage: 4},
			hostID:    h1.ID,
			wantExecs: []string{h1A, h1B, h1Bar, h1C},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: false, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{Page: 1, PerPage: 4},
			hostID:    h1.ID,
			wantExecs: []string{h1D, h1E, h1MDMlab, vppCommand1},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{Page: 2, PerPage: 4},
			hostID:    h1.ID,
			wantExecs: []string{},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 8},
		},
		{
			opts:      mdmlab.ListOptions{PerPage: 5},
			hostID:    h2.ID,
			wantExecs: []string{h2SelfService, h2Bar, h2A, vppCommand2, h2SetupExp},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: false, TotalResults: 5},
		},
		{
			opts:      mdmlab.ListOptions{},
			hostID:    h3.ID,
			wantExecs: []string{},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: false, TotalResults: 0},
		},
		{
			opts:      mdmlab.ListOptions{},
			hostID:    h4.ID,
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("%v: %#v", c.hostID, c.opts), func(t *testing.T) {
			...
			c.opts.OrderDirection = mdmlab.OrderAscending
			...
			for i, a := range acts {
				...
				case mdmlab.ActivityInstalledAppStoreApp{}.ActivityName():
				...
				if _, ok := execIDsFromPolicyAutomation[wantExec]; ok {
					...
					require.Equal(t, "MDMlab", *a.ActorFullName, "result %d", i)
...
func testListHostPastActivities(t *testing.T, ds *Datastore) {
	getDetails := func(a *mdmlab.Activity) map[string]any {
	...
	cases := []struct {
		...
		opts    mdmlab.ListActivitiesOptions
		expMeta *mdmlab.PaginationMetadata
	}{
		{
			name:    "fetch page one",
			expActs: []dummyActivity{activities[0]},
			expMeta: &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: false},
			opts: mdmlab.ListActivitiesOptions{
				ListOptions: mdmlab.ListOptions{
					Page:    0,
					PerPage: 1,
		...
		},
		{
			...
			expMeta: &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
			opts: mdmlab.ListActivitiesOptions{
				ListOptions: mdmlab.ListOptions{
					Page:    1,
					PerPage: 1,
		...
		},
		{
			...
			expActs: activities,
			expMeta: &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: false},
			opts: mdmlab.ListActivitiesOptions{
				ListOptions: mdmlab.ListOptions{
					Page:    0,
					PerPage: 2,
...
func testCleanupActivitiesAndAssociatedData(t *testing.T, ds *Datastore) {
	...
	nonSavedQuery1, err := ds.NewQuery(ctx, &mdmlab.Query{
		Name:    "nonSavedQuery1",
		Saved:   false,
		Query:   "SELECT 1;",
	...
	require.NoError(t, err)
	savedQuery1, err := ds.NewQuery(ctx, &mdmlab.Query{
		Name:    "savedQuery1",
		Saved:   true,
		Query:   "SELECT 2;",
		Logging: mdmlab.LoggingSnapshot,
	...
	require.NoError(t, err)
	distributedQueryCampaign1, err := ds.NewDistributedQueryCampaign(ctx, &mdmlab.DistributedQueryCampaign{
		QueryID: nonSavedQuery1.ID,
		Status:  mdmlab.QueryComplete,
		UserID:  user1.ID,
	})
	require.NoError(t, err)
	_, err = ds.NewDistributedQueryCampaignTarget(ctx, &mdmlab.DistributedQueryCampaignTarget{
	...
	activities, _, err := ds.ListActivities(ctx, mdmlab.ListActivitiesOptions{})
	require.NoError(t, err)
