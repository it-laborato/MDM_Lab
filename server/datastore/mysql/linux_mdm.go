...
func (ds *Datastore) GetLinuxDiskEncryptionSummary(ctx context.Context, teamID *uint) (mdmlab.MDMLinuxDiskEncryptionSummary, error) {
	var args []interface{}
	...
	var counts []countRow
	summary := mdmlab.MDMLinuxDiskEncryptionSummary{}
...
func sqlCaseLinuxOSSettingsStatus() string {
	...
	CASE WHEN 
		...
		AND hdek.base64_encrypted != ''
		AND hdek.client_error = '' THEN
		'` + string(mdmlab.OSSettingsVerified) + `'
	WHEN hdek.client_error IS NOT NULL
	AND hdek.client_error != '' THEN
		'` + string(mdmlab.OSSettingsFailed) + `'
	WHEN hdek.base64_encrypted IS NULL
		OR (hdek.base64_encrypted = ''
		AND hdek.client_error = '') THEN
		'` + string(mdmlab.OSSettingsPending) + `'
	END`
}
...
func sqlCaseLinuxDiskEncryptionStatus() string {
	...
	CASE WHEN 
		...
		AND hdek.base64_encrypted != ''
		AND hdek.client_error = '' THEN
		'` + string(mdmlab.DiskEncryptionVerified) + `'
	WHEN hdek.client_error IS NOT NULL
	AND hdek.client_error != '' THEN
		'` + string(mdmlab.DiskEncryptionFailed) + `'
	WHEN hdek.base64_encrypted IS NULL
		OR (hdek.base64_encrypted = ''
		AND hdek.client_error = '') THEN
		'` + string(mdmlab.DiskEncryptionActionRequired) + `'
	END`
}
