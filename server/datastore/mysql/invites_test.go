...
func testInvitesCreate(t *testing.T, ds *Datastore) {
	...
	invite := &mdmlab.Invite{
		...
		Teams: []mdmlab.UserTeam{
			...
			{Role: mdmlab.RoleMaintainer, Team: mdmlab.Team{ID: 3}},
		},
	}
	...
	_, err = ds.NewInvite(context.Background(), &mdmlab.Invite{
...
func setupTestInvites(t *testing.T, ds mdmlab.Datastore) {
	admin := &mdmlab.Invite{
		Email:      "admin@foo.com",
		Name:       "Xadmin",
		Token:      "admin",
...
func testInvitesUpdate(t *testing.T, ds *Datastore) {
	...
	sentinelInvite := &mdmlab.Invite{
		Email: "user@sentinel.com",
		Name:  "sentinel",
		Token: "some_sentinel",
		Teams: []mdmlab.UserTeam{
			{Role: mdmlab.RoleAdmin, Team: mdmlab.Team{ID: 1}},
		},
	}
	...
	invite.GlobalRole = null.StringFrom(mdmlab.RoleAdmin)
	invite.Teams = nil
	...
	require.NoError(t, err)
	assert.Equal(t, invite.ID, verify.ID)
	assert.Equal(t, null.StringFrom(mdmlab.RoleAdmin), verify.GlobalRole)
	...
	assert.Equal(t, mdmlab.RoleAdmin, verify.Teams[0].Role)
}
