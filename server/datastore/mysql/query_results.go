...
func (ds *Datastore) OverwriteQueryResultRows(ctx context.Context, rows []*mdmlab.ScheduledQueryResultRow, maxQueryReportRows int) (err error) {
	if len(rows) == 0 {
		return nil
	}
...
func (ds *Datastore) QueryResultRows(ctx context.Context, queryID uint, filter mdmlab.TeamFilter) ([]*mdmlab.ScheduledQueryResultRow, error) {
	...
	results := []*mdmlab.ScheduledQueryResultRow{}
...
func (ds *Datastore) QueryResultRowsForHost(ctx context.Context, queryID, hostID uint) ([]*mdmlab.ScheduledQueryResultRow, error) {
	selectStmt := `
