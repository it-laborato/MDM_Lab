...
func testStatisticsShouldSend(t *testing.T, ds *Datastore) {
	...
	premiumLicense := &mdmlab.LicenseInfo{Tier: mdmlab.TierPremium, Organization: "MDMlab"}
	freeLicense := &mdmlab.LicenseInfo{Tier: mdmlab.TierFree}
	...
	stats, shouldSend, err := ds.ShouldSendStatistics(license.NewContext(ctx, premiumLicense), time.Millisecond, mdmlabConfig)
	require.NoError(t, err)
	assert.True(t, shouldSend)
	assert.Equal(t, "premium", stats.LicenseTier)
	assert.Equal(t, "MDMlab", stats.Organization)
	...
	assert.Equal(t, []mdmlab.HostsCountByOsqueryVersion{}, stats.HostsEnrolledByOsqueryVersion) // should be empty slice instead of nil
	assert.Equal(t, []mdmlab.HostsCountByOrbitVersion{}, stats.HostsEnrolledByOrbitVersion)     // should be empty slice instead of nil
	...
	assert.Equal(t, 0, stats.NumHostsMDMlabDesktopEnabled)
	...
	require.NoError(t, err)
	_, err = ds.NewUser(ctx, &mdmlab.User{
		Password:                 []byte("foobar"),
		AdminForcedPasswordReset: false,
	...
	_, err = ds.NewTeam(ctx, &mdmlab.Team{
		Name:        "footeam",
	...
	_, err = ds.NewGlobalPolicy(ctx, ptr.Uint(1), mdmlab.PolicyPayload{
		Name:        "testpolicy",
		Query:       "select 1;",
	...
	_, err = ds.NewLabel(ctx, &mdmlab.Label{
		Name:        "testlabel",
		Query:       "select 1;",
		Platform:    "darwin",
	...
	cfg, err := ds.NewAppConfig(ctx, &mdmlab.AppConfig{
	...
	assert.NotEmpty(t, stats.MDMlabVersion)
	assert.Equal(t, "premium", stats.LicenseTier)
	assert.Equal(t, "MDMlab", stats.Organization)
	...
	assert.Equal(t, `[{"count":10,"loc":["a","b","c"]}]`, string(stats.StoredErrors))
	assert.Equal(t, []mdmlab.HostsCountByOsqueryVersion{{OsqueryVersion: "4.9.0", NumHosts: 1}}, stats.HostsEnrolledByOsqueryVersion)
	assert.Equal(t, []mdmlab.HostsCountByOrbitVersion{{OrbitVersion: "1.1.0", NumHosts: 1}}, stats.HostsEnrolledByOrbitVersion)
	assert.Equal(t, false, stats.AIFeaturesDisabled)
	assert.Equal(t, false, stats.MaintenanceWindowsEnabled)
	assert.Equal(t, false, stats.MaintenanceWindowsConfigured)
	assert.Equal(t, 1, stats.NumHostsMDMlabDesktopEnabled)

	err = ds.RecordStatisticsSent(ctx)
	require.NoError(t, err)

	// If we try right away, it shouldn't ask to send
	stats, shouldSend, err = ds.ShouldSendStatistics(license.NewContext(ctx, premiumLicense), mdmlab.StatisticsFrequency, mdmlabConfig)
	require.NoError(t, err)
	...
	assert.Equal(t, 5, stats.NumWeeklyPolicyViolationDaysActual)
	require.ElementsMatch(t, []mdmlab.HostsCountByOSVersion{
	...
	time.Sleep(1100 * time.Millisecond) // ensure the DB timestamp is not in the same second
	stats, shouldSend, err = ds.ShouldSendStatistics(license.NewContext(ctx, freeLicense), time.Millisecond, mdmlabConfig)
	require.NoError(t, err)
	assert.True(t, shouldSend)
	assert.Equal(t, firstIdentifier, stats.AnonymousIdentifier)
	...
	mdmlabConfig.Vulnerabilities = config.VulnerabilitiesConfig{DatabasesPath: "some/path/vulns"}
	stats, shouldSend, err = ds.ShouldSendStatistics(license.NewContext(ctx, freeLicense), time.Millisecond, mdmlabConfig)
	require.NoError(t, err)
	assert.True(t, shouldSend)
