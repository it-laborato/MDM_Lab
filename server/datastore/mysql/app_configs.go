...
func (ds *Datastore) NewAppConfig(ctx context.Context, info *mdmlab.AppConfig) (*mdmlab.AppConfig, error) {
...
func (ds *Datastore) AppConfig(ctx context.Context) (*mdmlab.AppConfig, error) {
	return appConfigDB(ctx, ds.reader(ctx))
}

func appConfigDB(ctx context.Context, q sqlx.QueryerContext) (*mdmlab.AppConfig, error) {
...
func (ds *Datastore) SaveAppConfig(ctx context.Context, info *mdmlab.AppConfig) error {
	return ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		if info.Integrations.NDESSCEPProxy.Valid {
			if info.Integrations.NDESSCEPProxy.Set &&
				...
				info.Integrations.NDESSCEPProxy.Value.Password != mdmlab.MaskedPassword {
				err := ds.insertOrReplaceConfigAsset(ctx, tx, mdmlab.MDMConfigAsset{
					Name:  mdmlab.MDMAssetNDESPassword,
...
func (ds *Datastore) insertOrReplaceConfigAsset(ctx context.Context, tx sqlx.ExtContext, asset mdmlab.MDMConfigAsset) error {
	assets, err := ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{asset.Name}, tx)
	if err != nil {
		if mdmlab.IsNotFound(err) {
			return ds.InsertMDMConfigAssets(ctx, []mdmlab.MDMConfigAsset{asset}, tx)
		}
	...
	if !bytes.Equal(currentAsset.Value, asset.Value) {
		return ds.ReplaceMDMConfigAssets(ctx, []mdmlab.MDMConfigAsset{asset}, tx)
	}
	// asset already exists and is the same, so not need to update
	return nil
}

func (ds *Datastore) VerifyEnrollSecret(ctx context.Context, secret string) (*mdmlab.EnrollSecret, error) {
	var s mdmlab.EnrollSecret
...
func (ds *Datastore) ApplyEnrollSecrets(ctx context.Context, teamID *uint, secrets []*mdmlab.EnrollSecret) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		return applyEnrollSecretsDB(ctx, tx, teamID, secrets)
	})
}

func applyEnrollSecretsDB(ctx context.Context, q sqlx.ExtContext, teamID *uint, secrets []*mdmlab.EnrollSecret) error {
	...
	var existingSecrets []*mdmlab.EnrollSecret
	...
	if len(newSecrets) > 0 {
		...
		if _, err := q.ExecContext(ctx, sql, args...); err != nil {
			if IsDuplicate(err) {
				...
				err = alreadyExists("secret", mdmlab.MaskedPassword)
			}
...
func (ds *Datastore) GetEnrollSecrets(ctx context.Context, teamID *uint) ([]*mdmlab.EnrollSecret, error) {
	return getEnrollSecretsDB(ctx, ds.reader(ctx), teamID)
}

func getEnrollSecretsDB(ctx context.Context, q sqlx.QueryerContext, teamID *uint) ([]*mdmlab.EnrollSecret, error) {
	var args []interface{}
...
func (ds *Datastore) AggregateEnrollSecretPerTeam(ctx context.Context) ([]*mdmlab.EnrollSecret, error) {
	query := `
          SELECT
             COALESCE((
             SELECT
...
func (ds *Datastore) ApplyYaraRules(ctx context.Context, rules []mdmlab.YaraRule) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		return applyYaraRulesDB(ctx, tx, rules)
	})
}

func applyYaraRulesDB(ctx context.Context, q sqlx.ExtContext, rules []mdmlab.YaraRule) error {
...
func (ds *Datastore) GetYaraRules(ctx context.Context) ([]mdmlab.YaraRule, error) {
	sql := "SELECT name, contents FROM yara_rules"
	rules := []mdmlab.YaraRule{}
...
func (ds *Datastore) YaraRuleByName(ctx context.Context, name string) (*mdmlab.YaraRule, error) {
	query := "SELECT name, contents FROM yara_rules WHERE name = ?"
	rule := mdmlab.YaraRule{}
