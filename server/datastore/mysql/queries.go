...
func (ds *Datastore) ApplyQueries(ctx context.Context, authorID uint, queries []*mdmlab.Query, queriesToDiscardResults map[uint]struct{}) error {
...
func (ds *Datastore) applyQueriesInTx(ctx context.Context, authorID uint, queries []*mdmlab.Query) (err error) {
...
func (ds *Datastore) QueryByName(
	...
	if err := ds.loadPacksForQueries(ctx, []*mdmlab.Query{&query}); err != nil {
...
func (ds *Datastore) NewQuery(
	...
	query.Packs = []mdmlab.Pack{}
	return query, nil
}

func (ds *Datastore) SaveQuery(ctx context.Context, q *mdmlab.Query, shouldDiscardResults bool, shouldDeleteStats bool) (err error) {
	if err := q.Verify(); err != nil {
		return ctxerr.Wrap(ctx, err)
	}
...
func (ds *Datastore) deleteQueryStats(ctx context.Context, queryIDs []uint) {
	...
	stmt = fmt.Sprintf("DELETE FROM aggregated_stats WHERE type = '%s' AND id IN (?)", mdmlab.AggregatedStatsTypeScheduledQuery)
	stmt, args, err = sqlx.In(stmt, queryIDs)
	if err != nil {
...
func (ds *Datastore) Query(ctx context.Context, id uint) (*mdmlab.Query, error) {
	return query(ctx, ds.reader(ctx), id)
}

func query(ctx context.Context, db sqlx.QueryerContext, id uint) (*mdmlab.Query, error) {
	sqlQuery := `
		SELECT 
			q.id,
			q.team_id,
	...
	`
	query := &mdmlab.Query{}
	if err := sqlx.GetContext(ctx, db, query, sqlQuery, false, mdmlab.AggregatedStatsTypeScheduledQuery, id); err != nil {
		if err == sql.ErrNoRows {
	...
	if err := loadPacksForQueries(ctx, db, []*mdmlab.Query{query}); err != nil {
...
// determined by passed in mdmlab.ListOptions, count of total queries returned without limits, and
// pagination metadata
func (ds *Datastore) ListQueries(ctx context.Context, opt mdmlab.ListQueryOptions) ([]*mdmlab.Query, int, *mdmlab.PaginationMetadata, error) {
	getQueriesStmt := `
		SELECT
			q.id,
			q.team_id,
	...
	args := []interface{}{false, mdmlab.AggregatedStatsTypeScheduledQuery}
	...
	var meta *mdmlab.PaginationMetadata
	if opt.ListOptions.IncludeMetadata {
		meta = &mdmlab.PaginationMetadata{HasPreviousResults: opt.ListOptions.Page > 0}
...
func (ds *Datastore) loadPacksForQueries(ctx context.Context, queries []*mdmlab.Query) error {
	return loadPacksForQueries(ctx, ds.reader(ctx), queries)
}

func loadPacksForQueries(ctx context.Context, db sqlx.QueryerContext, queries []*mdmlab.Query) error {
	if len(queries) == 0 {
		return nil
	}
	...
	name_queries := map[string]*mdmlab.Query{}
	// Used for the IN clause
	names := []string{}
	for _, q := range queries {
		q.Packs = make([]mdmlab.Pack, 0)
...
func (ds *Datastore) ListScheduledQueriesForAgents(ctx context.Context, teamID *uint, queryReportsDisabled bool) ([]*mdmlab.Query, error) {
	sqlStmt := `
		SELECT
			q.name,
			q.query,
	...
	args = append(args, queryReportsDisabled, mdmlab.LoggingSnapshot)

	results := []*mdmlab.Query{}
...
func (ds *Datastore) GetLiveQueryStats(ctx context.Context, queryID uint, hostIDs []uint) ([]*mdmlab.LiveQueryStats, error) {
...
func (ds *Datastore) UpdateLiveQueryStats(ctx context.Context, queryID uint, stats []*mdmlab.LiveQueryStats) error {
	if len(stats) == 0 {
		return nil
	}
