...
func TestUp_20241203125346(t *testing.T) {
	...
	execNoErr(t, db, `INSERT INTO cron_stats (name, instance, stats_type, status) VALUES (?, 'foo', ?, ?)`, mdmlab.CronMaintainedApps, mdmlab.CronStatsTypeScheduled, mdmlab.CronStatsStatusCompleted)
	execNoErr(t, db, `INSERT INTO cron_stats (name, instance, stats_type, status) VALUES (?, 'foo', ?, ?)`, mdmlab.CronMaintainedApps, mdmlab.CronStatsTypeTriggered, mdmlab.CronStatsStatusCompleted)
	...
	execNoErr(
		t,
		db,
		`INSERT INTO mdmlab_library_apps (name, token, version, platform, installer_url, sha256, bundle_identifier, install_script_content_id, uninstall_script_content_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		"Zoom",
		"zoom",
		"6.2.11.43613",
		"darwin",
	...
	var fmas []mdmlab.MaintainedApp
	err = db.Select(&fmas, `SELECT name, token FROM mdmlab_library_apps`)
	require.NoError(t, err)
	...
	var stats []mdmlab.CronStats
	err = db.Select(&stats, `SELECT name, instance, stats_type, status FROM cron_stats`)
	require.NoError(t, err)
	require.Len(t, stats, 1)
	require.Equal(t, string(mdmlab.CronMaintainedApps), stats[0].Name)
	require.Equal(t, mdmlab.CronStatsTypeTriggered, stats[0].StatsType)
	require.Equal(t, mdmlab.CronStatsStatusCompleted, stats[0].Status)
}
