...
CREATE TABLE vpp_tokens (
	...
	-- encrypted token, encrypted with the MDMlabConfig.Server.PrivateKey value
	token                  blob NOT NULL,
	...
	-- the Go code.
	-- [1]: https://mdmlabdm.slack.com/archives/C03C41L5YEL/p1724073772972669
...
`
	...
	var token []byte
	if err := txx.Get(&token, getVPP, mdmlab.MDMAssetVPPTokenDeprecated); err != nil {
...
`
	...
	if _, err = tx.Exec(delVPP, deletionUUID, mdmlab.MDMAssetVPPTokenDeprecated); err != nil {
