...
CREATE TABLE abm_tokens (
	...
	-- with the MDMlabConfig.Server.PrivateKey
	token                  blob NOT NULL,
...
`
	...
	var token []byte
	if err := txx.Get(&token, getABM, mdmlab.MDMAssetABMTokenDeprecated); err != nil {
...
`
	...
	// ~immediately at MDMlab startup) to ensure the token's information is
	// filled.
	// https://github.com/mdmlabdm/mdmlab/pull/21287#discussion_r1715891448
...
`
	...
	if _, err = tx.Exec(delABM, deletionUUID, mdmlab.MDMAssetABMTokenDeprecated); err != nil {
