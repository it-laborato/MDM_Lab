...
func TestUp_20240301173035(t *testing.T) {
	...
	// mdmlab platform is left empty for pre-existing rows
	err = sqlx.Select(db, &hostActions, `SELECT host_id, lock_ref, mdmlab_platform FROM host_mdm_actions`)
	require.NoError(t, err)
	...
	require.Empty(t, hostActions[0].MDMlabPlatform)
}
