...
func Up_20250109150150(tx *sql.Tx) error {
	...
	// "original" script contents for FMAs that are created when the mdmlab_library_apps table is populated.
	selectStmt := `
SELECT 
	...
	fla.token AS token
FROM 
	mdmlab_library_apps fla 
	JOIN script_contents sc 
	ON fla.install_script_content_id = sc.id
WHERE fla.token IN (?)
`

	// This is the list of MDMlab-maintained apps we want to update ("token" is an ID found on the brew
	// metadata)
	...
	if err != nil {
		return fmt.Errorf("building SQL in statement for selecting mdmlab maintained apps: %w", err)
	}

	txx := sqlx.Tx{Tx: tx, Mapper: reflectx.NewMapperFunc("db", sqlx.NameMapper)}
	if err := txx.Select(&scriptsToModify, stmt, args...); err != nil {
		// if this migration is running on a brand-new MDMlab deployment, then there won't be
		// anything in the mdmlab_library_apps table, so we can just exit.
