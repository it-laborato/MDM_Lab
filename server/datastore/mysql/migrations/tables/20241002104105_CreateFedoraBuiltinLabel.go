...
`
	...
	_, err := tx.Exec(
		stmt,
		mdmlab.BuiltinLabelFedoraLinux,
		"All Fedora hosts",
	...
	if err != nil {
		if driverErr, ok := err.(*mysql.MySQLError); ok {
			if driverErr.Number == mysqlerr.ER_DUP_ENTRY {
				return fmt.Errorf("a label with the name %q already exists, please rename it before applying this migration: %w", mdmlab.BuiltinLabelFedoraLinux, err)
			}
		}
		return err
	}
