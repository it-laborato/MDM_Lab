...
func TestUp_20241003145349(t *testing.T) {
	...
	var count int
	err := db.Get(&count, "SELECT COUNT(*) FROM mdmlab_library_apps")
	require.NoError(t, err)
	require.Zero(t, count)

	// column was added and value is NULL
	var mdmlabLibraryAppID *uint
	err = db.Get(&mdmlabLibraryAppID, "SELECT mdmlab_library_app_id FROM software_installers WHERE id = ?", swiID)
	require.NoError(t, err)
	require.Nil(t, mdmlabLibraryAppID)
}
