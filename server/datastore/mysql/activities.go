...
func (ds *Datastore) NewActivity(
	...
	processed, _ := ctx.Value(mdmlab.ActivityWebhookContextKey).(bool)
	if !processed {
		return ctxerr.New(
			ctx, "activity webhook not processed. Please use svc.NewActivity instead of ds.NewActivity. This is a MDMlab server bug.",
		)
	}
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// get in the 10K+.
		if ah, ok := activity.(mdmlab.ActivityHosts); ok {
...
func (ds *Datastore) ListActivities(ctx context.Context, opt mdmlab.ListActivitiesOptions) ([]*mdmlab.Activity, *mdmlab.PaginationMetadata, error) {
	// Fetch activities

	activities := []*mdmlab.Activity{}
	activitiesQ := `
		SELECT
			a.id,
			a.user_id,
...
// tasks.
func (ds *Datastore) ListHostUpcomingActivities(ctx context.Context, hostID uint, opt mdmlab.ListOptions) ([]*mdmlab.Activity, *mdmlab.PaginationMetadata, error) {
	...
	countStmt, args, err := sqlx.Named(countStmt, map[string]any{
		...
		"software_status_install_pending":   mdmlab.SoftwareInstallPending,
		"software_status_uninstall_pending": mdmlab.SoftwareUninstallPending,
	})
	if err != nil {
	...
	if count == 0 {
		return []*mdmlab.Activity{}, &mdmlab.PaginationMetadata{}, nil
	}
	...
	listStmts := []string{
		...
		`SELECT
			...
			IF(hsr.policy_id IS NOT NULL, 'MDMlab', u.name) as name,
			u.id as user_id,
...
`,
		...
		`SELECT
			...
			-- so we mark those as "MDMlab"
			IF(hsi.user_id IS NULL AND NOT hsi.self_service, 'MDMlab', u.name) AS name,
...
SELECT
	...
	-- so we mark those as "MDMlab"
	IF(hvsi.user_id IS NULL AND NOT hvsi.self_service, 'MDMlab', u.name) AS name,
	u.id AS user_id,
	u.gravatar_url as gravatar_url,
	u.email as user_email,
...
`,
	...
	listStmt, args, err = sqlx.Named(listStmt, map[string]any{
		"host_id":                           hostID,
		"ran_script_type":                   mdmlab.ActivityTypeRanScript{}.ActivityName(),
		"installed_software_type":           mdmlab.ActivityTypeInstalledSoftware{}.ActivityName(),
		"uninstalled_software_type":         mdmlab.ActivityTypeUninstalledSoftware{}.ActivityName(),
		"installed_app_store_app_type":      mdmlab.ActivityInstalledAppStoreApp{}.ActivityName(),
		"max_wait_time":                     seconds,
		"software_status_install_pending":   mdmlab.SoftwareInstallPending,
		"software_status_uninstall_pending": mdmlab.SoftwareUninstallPending,
	})
	...
	var activities []*mdmlab.Activity
	...
	metaData := &mdmlab.PaginationMetadata{HasPreviousResults: opt.Page > 0, TotalResults: count}
...
func (ds *Datastore) ListHostPastActivities(ctx context.Context, hostID uint, opt mdmlab.ListOptions) ([]*mdmlab.Activity, *mdmlab.PaginationMetadata, error) {
	const listStmt = `
	SELECT
