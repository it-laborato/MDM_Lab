...
func TestMDMShared(t *testing.T) {
	...
	cases := []struct {
		...
		{"TestIsHostConnectedToMDMlabMDM", testIsHostConnectedToMDMlabMDM},
		{"TestAreHostsConnectedToMDMlabMDM", testAreHostsConnectedToMDMlabMDM},
...
func testMDMCommands(t *testing.T, ds *Datastore) {
	...
	cmds, err := ds.ListMDMCommands(ctx, mdmlab.TeamFilter{}, &mdmlab.MDMCommandListOptions{})
	require.NoError(t, err)
	require.Empty(t, cmds)

	// enroll a windows device
	windowsH, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	require.NoError(t, err)
	windowsEnrollment := &mdmlab.MDMWindowsEnrolledDevice{
	...
	macH, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:       "macos-test",
	...
	cmds, err = ds.ListMDMCommands(
		...
		mdmlab.TeamFilter{User: test.UserAdmin},
		&mdmlab.MDMCommandListOptions{
			ListOptions: mdmlab.ListOptions{OrderKey: "hostname"},
		})
	require.NoError(t, err)
	...
	winCmd2 := &mdmlab.MDMWindowsCommand{
		CommandUUID:  uuid.NewString(),
		RawCommand:   []byte("<Exec></Exec>"),
		TargetLocURI: "./test/uri2",
	}
	winCmd3 := &mdmlab.MDMWindowsCommand{
		CommandUUID:  uuid.NewString(),
		RawCommand:   []byte("<Exec></Exec>"),
	...
	// add macos host
	macH2, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	cmds, err = ds.ListMDMCommands(
		...
		&mdmlab.MDMCommandListOptions{
			...
			ListOptions: mdmlab.ListOptions{OrderKey: "hostname", OrderDirection: mdmlab.OrderAscending},
		},
	)
	require.NoError(t, err)
	require.Len(t, cmds, 2)
...
func testBatchSetMDMProfiles(t *testing.T, ds *Datastore) {
	applyAndExpect := func(
		newAppleSet []*mdmlab.MDMAppleConfigProfile,
		newWindowsSet []*mdmlab.MDMWindowsConfigProfile,
		newAppleDeclSet []*mdmlab.MDMAppleDeclaration,
		tmID *uint,
		wantApple []*mdmlab.MDMAppleConfigProfile,
		wantWindows []*mdmlab.MDMWindowsConfigProfile,
		wantAppleDecl []*mdmlab.MDMAppleDeclaration,
		wantUpdates mdmlab.MDMProfilesUpdates,
	) {
		ctx := context.Background()
	...
	withTeamIDApple := func(p *mdmlab.MDMAppleConfigProfile, tmID uint) *mdmlab.MDMAppleConfigProfile {
		p.TeamID = &tmID
		return p
	}

	withTeamIDDecl := func(d *mdmlab.MDMAppleDeclaration, tmID uint) *mdmlab.MDMAppleDeclaration {
		d.TeamID = &tmID
		return d
	}

	withTeamIDWindows := func(p *mdmlab.MDMWindowsConfigProfile, tmID uint) *mdmlab.MDMWindowsConfigProfile {
		p.TeamID = &tmID
		return p
	}

	// empty set for no team (both Apple and Windows)
	applyAndExpect(nil, nil, nil, nil, nil, nil, nil, mdmlab.MDMProfilesUpdates{})

	// single Apple and Windows profile set for a specific team
	applyAndExpect(
		[]*mdmlab.MDMAppleConfigProfile{configProfileForTest(t, "N1", "I1", "a")},
		[]*mdmlab.MDMWindowsConfigProfile{windowsConfigProfileForTest(t, "W1", "l1")},
		[]*mdmlab.MDMAppleDeclaration{declForTest("D1", "D1", "foo")},
		ptr.Uint(1),
		[]*mdmlab.MDMAppleConfigProfile{
		...
		[]*mdmlab.MDMAppleDeclaration{withTeamIDDecl(declForTest("D1", "D1", "foo"), 1)},
		mdmlab.MDMProfilesUpdates{AppleConfigProfile: true, WindowsConfigProfile: true, AppleDeclaration: true},
	)
	...
	applyAndExpect(
		...
		mdmlab.MDMProfilesUpdates{AppleConfigProfile: false, WindowsConfigProfile: false, AppleDeclaration: false},
	)
...
func testListMDMConfigProfiles(t *testing.T, ds *Datastore) {
	...
	opts := mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true}
	...
	require.Equal(t, *meta, mdmlab.PaginationMetadata{})

	// add mdmlab-managed Apple profiles for the team and globally
	...
	// add mdmlab-managed Windows profiles for the team and globally
	for name := range mdm_types.MDMlabReservedProfileNames() {
		_, err = ds.NewMDMWindowsConfigProfile(
			ctx,
			mdmlab.MDMWindowsConfigProfile{Name: name, TeamID: &team.ID, SyncML: winProf},
		)
		require.NoError(t, err)
		_, err = ds.NewMDMWindowsConfigProfile(
			ctx,
			mdmlab.MDMWindowsConfigProfile{Name: name, TeamID: nil, SyncML: winProf},
		)
		require.NoError(t, err)
	}
	...
	profB, err := ds.NewMDMWindowsConfigProfile(
		ctx,
		mdmlab.MDMWindowsConfigProfile{Name: "B", TeamID: &team.ID, SyncML: winProf},
	)
	require.NoError(t, err)
	...
	var labels []*mdmlab.Label
	for i := 0; i < 8; i++ {
		lbl, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "l" + strconv.Itoa(i), Query: "select 1"})
		require.NoError(t, err)
	...
	for i := 0; i < 3; i++ {
		...
		if i == 0 {
			acp.LabelsIncludeAll = []mdmlab.ConfigurationProfileLabel{
		...
		wcp := mdmlab.MDMWindowsConfigProfile{
			Name:   string(rune('C' + inc + 2)),
			TeamID: nil,
			SyncML: winProf,
		}
		if i == 0 {
			wcp.LabelsIncludeAll = []mdmlab.ConfigurationProfileLabel{
	...
	profLabels := map[string][]mdmlab.ConfigurationProfileLabel{
		"C": {
	...
	cases := []struct {
	...
	}{
		{
			"all global",
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true},
			[]string{"A", "C", "E", "G", "I", "K", "M"},
			mdmlab.PaginationMetadata{},
		},
		{
			"all team",
			&team.ID,
...

		{
			...
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2},
			[]string{"A", "C"},
			mdmlab.PaginationMetadata{HasNextResults: true},
		},
		{
			"page 1 per page 2, global",
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 1},
			[]string{"E", "G"},
			mdmlab.PaginationMetadata{HasPreviousResults: true, HasNextResults: true},
		},
		{
			"page 2 per page 2, global",
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 2},
			[]string{"I", "K"},
			mdmlab.PaginationMetadata{HasPreviousResults: true, HasNextResults: true},
		},
		{
			...
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 3},
			[]string{"M"},
			mdmlab.PaginationMetadata{HasPreviousResults: true},
		},
		{
			"page 4 per page 2, global",
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 4},
			[]string{},
			mdmlab.PaginationMetadata{HasPreviousResults: true},
		},
...

		{
			...
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3},
			[]string{"A", "C", "E"},
			mdmlab.PaginationMetadata{HasNextResults: true},
		},
		{
			...
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3, Page: 1},
			[]string{"G", "I", "K"},
			mdmlab.PaginationMetadata{HasPreviousResults: true, HasNextResults: true},
		},
		{
			...
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3, Page: 2},
			[]string{"M"},
			mdmlab.PaginationMetadata{HasPreviousResults: true},
		},
		{
			...
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3, Page: 3},
			[]string{},
			mdmlab.PaginationMetadata{HasPreviousResults: true},
		},
...

		{
			"no metadata, global",
			nil,
			mdmlab.ListOptions{OrderKey: "name", IncludeMetadata: false, PerPage: 2, Page: 1},
			[]string{"E", "G"},
			mdmlab.PaginationMetadata{},
		},
		{
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			var gotMeta mdmlab.PaginationMetadata
			if meta != nil {
				gotMeta = *meta
			}
...
func assertHostProfiles(t *testing.T, ds *Datastore, want map[*mdmlab.Host][]anyProfile) {
	ctx := context.Background()
...
func testBulkSetPendingMDMHostProfiles(t *testing.T, ds *Datastore) {
	...
	hostIDsFromHosts := func(hosts ...*mdmlab.Host) []uint {
	...
	getProfs := func(teamID *uint) []*mdmlab.MDMConfigProfilePayload {
		// TODO(roberto): the docs says that you can pass a comma separated
		// list of columns to OrderKey, but that doesn't seem to work
		profs, _, err := ds.ListMDMConfigProfiles(ctx, teamID, mdmlab.ListOptions{})
		require.NoError(t, err)
		sort.Slice(profs, func(i, j int) bool {
			l, r := profs[i], profs[j]
	...
	var darwinHosts []*mdmlab.Host // not pre-allocating, causes gosec false positive
	for i := 0; i < 3; i++ {
	...
	i = 4
	linuxHost, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      fmt.Sprintf("test-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("test-uuid-%d", i),
	...
	i = 5
	var windowsHosts []*mdmlab.Host // not preallocating, causes gosec false positive
	for j := 0; j < 3; j++ {
		h, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	assert.False(t, updates.AppleDeclaration)
	assert.False(t, updates.WindowsConfigProfile)
	assertHostProfiles(t, ds, map[*mdmlab.Host][]anyProfile{
		darwinHosts[0]:  {},
		darwinHosts[1]:  {},
		darwinHosts[2]:  {},
		unenrolledHost:  {},
	...
	macGlobalProfiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "G1a", "G1a", "a"),
		configProfileForTest(t, "G2a", "G2a", "b"),
		configProfileForTest(t, "G3a", "G3a", "c"),
	}
	macGlobalDeclarations := []*mdmlab.MDMAppleDeclaration{
		declForTest("G1d", "G1d", "foo"),
		declForTest("G2d", "G2d", "bar"),
	}
	winGlobalProfiles := []*mdmlab.MDMWindowsConfigProfile{
	...
	tm1DarwinProfiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "T1.1a", "T1.1a", "d"),
		configProfileForTest(t, "T1.2a", "T1.2a", "e"),
	}
	tm1WindowsProfiles := []*mdmlab.MDMWindowsConfigProfile{
	...
	err = ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{
		{
			...
			Status: &mdmlab.MDMDeliveryVerifying, OperationType: mdmlab.MDMOperationTypeRemove, Checksum: []byte("csum"),
		},
		{
		...
		},
		{
			...
			Status: &mdmlab.MDMDeliveryFailed, OperationType: mdmlab.MDMOperationTypeRemove, Checksum: []byte("csum"),
		},
	})
	require.NoError(t, err)

	// add a profile to team1, and remove profile T1.1 on Apple, T1.2 on Windows
	newTm1DarwinProfiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "T1.2a", "T1.2a", "e"),
		configProfileForTest(t, "T1.3a", "T1.3a", "f"),
	}
	newTm1WindowsProfiles := []*mdmlab.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "T1.1w", "T1.1"),
	...
	newDarwinGlobalProfiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "G2a", "G2a", "b"),
		configProfileForTest(t, "G3a", "G3a", "c"),
		configProfileForTest(t, "G4a", "G4a", "d"),
	}
	newWindowsGlobalProfiles := []*mdmlab.MDMWindowsConfigProfile{
	...
	tm2DarwinProfiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "T2.1a", "T2.1a", "a"),
	}

	tm2WindowsProfiles := []*mdmlab.MDMWindowsConfigProfile{
	...
	setProfileLabels := func(t *testing.T, p *mdmlab.MDMConfigProfilePayload, labels ...*mdmlab.Label) {
...
func testGetHostMDMProfilesExpectedForVerification(t *testing.T, ds *Datastore) {
	...
	setup1 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 1"})
		require.NoError(t, err)
		...
		profs, _, err := ds.ListMDMConfigProfiles(ctx, &team.ID, mdmlab.ListOptions{})
		require.NoError(t, err)
	...
	setup2 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 2"})
		require.NoError(t, err)
		...
		label, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "test_label_1"})
		require.NoError(t, err)
	...
	setup3 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 3"})
		require.NoError(t, err)
		...
		testLabel2, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "test_label_2"})
		require.NoError(t, err)

		testLabel3, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "test_label_3"})
		require.NoError(t, err)
	...
	setup4 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 4"})
		require.NoError(t, err)
		...
		testLabel4, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "test_label_4"})
		require.NoError(t, err)
	...
	setup5 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 5"})
		require.NoError(t, err)
		...
		// create profiles for team 1
		profiles := []*mdmlab.MDMWindowsConfigProfile{
	...
	setup6 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 6"})
		require.NoError(t, err)
		...
		label, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "test_label_6"})
		require.NoError(t, err)
	...
	setup7 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 7"})
		require.NoError(t, err)
		...
		testLabel2, err := ds.NewLabel(ctx, &mdmlab.Label{Name: uuid.NewString()})
		require.NoError(t, err)

		testLabel3, err := ds.NewLabel(ctx, &mdmlab.Label{Name: uuid.NewString()})
		require.NoError(t, err)
	...
	setup8 := func() (uint, uint) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 8"})
		require.NoError(t, err)
		...
		label, err := ds.NewLabel(ctx, &mdmlab.Label{Name: uuid.NewString()})
		require.NoError(t, err)
	...
	tests := []struct {
		...
		wantMac     map[string]*mdmlab.ExpectedMDMProfile
		wantWindows map[string]*mdmlab.ExpectedMDMProfile
		os          string
	}{
		{
...
func testBatchSetProfileLabelAssociations(t *testing.T, ds *Datastore) {
	...
	wantOtherWin := []mdmlab.ConfigurationProfileLabel{
	...
	wantOtherMac := []mdmlab.ConfigurationProfileLabel{
	...
	for platform, uuid := range platforms {
		expectLabels := func(t *testing.T, profUUID, platform string, want []mdmlab.ConfigurationProfileLabel) {
			p := platform
			if p == "darwin" {
				p = "apple"
			}
			...
			var got []mdmlab.ConfigurationProfileLabel
			ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
		...
		t.Run("empty input "+platform, func(t *testing.T) {
			want := []mdmlab.ConfigurationProfileLabel{}
		...
		t.Run("valid input "+platform, func(t *testing.T) {
			profileLabels := []mdmlab.ConfigurationProfileLabel{
		...
		t.Run("invalid profile UUID "+platform, func(t *testing.T) {
			invalidProfileLabels := []mdmlab.ConfigurationProfileLabel{
	...
	t.Run("unsupported platform", func(t *testing.T) {
		err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
			_, err := batchSetProfileLabelAssociationsDB(
				ctx,
				tx,
				[]mdmlab.ConfigurationProfileLabel{{}},
				"unsupported",
			)
			return err
		})
...
func testBatchSetMDMProfilesTransactionError(t *testing.T, ds *Datastore) {
	...
	lbl, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "label", Query: "select 1"})
	require.NoError(t, err)
	...
	for _, c := range cases {
		t.Run(c.windowsErr+" "+c.appleErr, func(t *testing.T) {
			...
			appleProfs := []*mdmlab.MDMAppleConfigProfile{
				configProfileForTest(t, "N1", "I1", "a"),
				configProfileForTest(t, "N2", "I2", "b"),
			}
			winProfs := []*mdmlab.MDMWindowsConfigProfile{
...
func testMDMEULA(t *testing.T, ds *Datastore) {
	...
	var ae mdmlab.AlreadyExistsError
	err = ds.MDMInsertEULA(ctx, eula)
...
func testSCEPRenewalHelpers(t *testing.T, ds *Datastore) {
	...
	addCert := func(notAfter time.Time, h *mdmlab.Host) {
		serial, err := scepDepot.Serial()
		require.NoError(t, err)
		cert := &x509.Certificate{
			SerialNumber: serial,
			Subject: pkix.Name{
				CommonName: "MDMlab Identity",
			},
			NotAfter: notAfter,
	...
	var i int
	setHost := func(notAfter time.Time) *mdmlab.Host {
		i++
		h, err := ds.NewHost(ctx, &mdmlab.Host{
			Hostname:      fmt.Sprintf("test-host%d-name", i),
			OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
	...
	// see https://github.com/mdmlabdm/mdmlab/issues/19149
	...
	checkSCEPRenew := func(assoc mdmlab.SCEPIdentityAssociation, want *string) {
		var got *string
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
			return sqlx.GetContext(
				ctx,
	...
	err = ds.SetCommandForPendingSCEPRenewal(ctx, []mdmlab.SCEPIdentityAssociation{}, "foo")
	...
	err = ds.SetCommandForPendingSCEPRenewal(ctx, []mdmlab.SCEPIdentityAssociation{assocs[0]}, "foo")
	require.NoError(t, err)
	...
	err = ds.SetCommandForPendingSCEPRenewal(
		ctx,
		[]mdmlab.SCEPIdentityAssociation{{HostUUID: "foo", SHA256: "bar"}},
		"bar",
	)
...
func testMDMProfilesSummaryAndHostFilters(t *testing.T, ds *Datastore) {
	...
	checkSummaryWindows := func(t *testing.T, teamID *uint, expected mdmlab.MDMProfilesSummary) {
	...
	checkSummaryMac := func(t *testing.T, teamID *uint, expected mdmlab.MDMProfilesSummary) {
	...
	checkListHostsFilterOSSettings := func(t *testing.T, teamID *uint, status mdmlab.OSSettingsStatus, expectedIDs []uint) {
		gotHosts, err := ds.ListHosts(
			ctx,
			mdmlab.TeamFilter{User: test.UserAdmin},
			mdmlab.HostListOptions{TeamFilter: teamID, OSSettingsFilter: status},
		)
		require.NoError(t, err)
	...
	checkExpected := func(t *testing.T, teamID *uint, ep hostIDsByProfileStatus) {
		expectSummaryWindows := map[mdmlab.MDMDeliveryStatus]uint{}
		expectSummaryMac := map[mdmlab.MDMDeliveryStatus]uint{}
		...
		checkSummaryMac(t, teamID, mdmlab.MDMProfilesSummary{
			Pending:   expectSummaryMac[mdmlab.MDMDeliveryPending],
			Failed:    expectSummaryMac[mdmlab.MDMDeliveryFailed],
			Verifying: expectSummaryMac[mdmlab.MDMDeliveryVerifying],
			Verified:  expectSummaryMac[mdmlab.MDMDeliveryVerified],
		})

		checkSummaryWindows(t, teamID, mdmlab.MDMProfilesSummary{
			Pending:   expectSummaryWindows[mdmlab.MDMDeliveryPending],
			Failed:    expectSummaryWindows[mdmlab.MDMDeliveryFailed],
			Verifying: expectSummaryWindows[mdmlab.MDMDeliveryVerifying],
			Verified:  expectSummaryWindows[mdmlab.MDMDeliveryVerified],
		})

		checkListHostsFilterOSSettings(
			t,
			teamID,
			mdmlab.OSSettingsVerified,
			ep[mdmlab.MDMDeliveryVerified],
		)
		checkListHostsFilterOSSettings(
			t,
			teamID,
			mdmlab.OSSettingsVerifying,
			ep[mdmlab.MDMDeliveryVerifying],
		)
		checkListHostsFilterOSSettings(
			t,
			teamID,
			mdmlab.OSSettingsFailed,
			ep[mdmlab.MDMDeliveryFailed],
		)
		checkListHostsFilterOSSettings(
			t,
			teamID,
			mdmlab.OSSettingsPending,
			ep[mdmlab.MDMDeliveryPending],
		)
	}
	...
	checkMacHostProfiles := func(t *testing.T, hostUUID string, statusByProfUUID map[string]string) {
		...
		for _, prof := range profs {
			...
			require.Equal(t, mdmlab.MDMDeliveryStatus(ep), *prof.Status)
		}
	}
	...
	macHostsByID := make(map[uint]*mdmlab.Host, 5)
	winHostsByID := make(map[uint]*mdmlab.Host, 5)
	for i := 0; i < 10; i++ {
		p := "windows"
		if i >= 5 {
			p = "darwin"
	...
	upsertHostProfileStatus(t, hosts[0].UUID, "w1", &mdmlab.MDMDeliveryPending)

	checkExpected(t, nil, hostIDsByProfileStatus{
		mdmlab.MDMDeliveryPending: []uint{hosts[0].ID},
	})

	// add some mac profiles with different statuses
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mdmlab.MDMDeliveryFailed)
	upsertHostProfileStatus(t, hosts[9].UUID, "a2", &mdmlab.MDMDeliveryPending)
	upsertHostProfileStatus(t, hosts[9].UUID, "a3", &mdmlab.MDMDeliveryVerifying)
	upsertHostProfileStatus(t, hosts[9].UUID, "a4", &mdmlab.MDMDeliveryVerified)

	// add some mac declarations with different statuses
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mdmlab.MDMDeliveryFailed)
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mdmlab.MDMDeliveryPending)
	upsertHostProfileStatus(t, hosts[9].UUID, "d3", &mdmlab.MDMDeliveryVerifying)
	upsertHostProfileStatus(t, hosts[9].UUID, "d4", &mdmlab.MDMDeliveryVerified)

	checkExpected(t, nil, hostIDsByProfileStatus{
		mdmlab.MDMDeliveryPending: []uint{hosts[0].ID},
		mdmlab.MDMDeliveryFailed:  []uint{hosts[9].ID},
	})
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mdmlab.MDMDeliveryPending)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mdmlab.MDMDeliveryPending)
	expectedHostProfiles["d1"] = "pending"
	checkExpected(t, nil, hostIDsByProfileStatus{
		mdmlab.MDMDeliveryPending: []uint{hosts[0].ID, hosts[9].ID},
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)

	// set pending mac declaration to failed, host status is now failed
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mdmlab.MDMDeliveryFailed)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mdmlab.MDMDeliveryVerifying)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mdmlab.MDMDeliveryVerifying)
	expectedHostProfiles["a1"] = "verifying"
	upsertHostProfileStatus(t, hosts[9].UUID, "a2", &mdmlab.MDMDeliveryVerifying)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mdmlab.MDMDeliveryVerifying)
	expectedHostProfiles["d1"] = "verifying"
	checkExpected(t, nil, hostIDsByProfileStatus{
		mdmlab.MDMDeliveryPending:   []uint{hosts[0].ID},
		mdmlab.MDMDeliveryVerifying: []uint{hosts[9].ID},
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)
	...
	// verifying
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mdmlab.MDMDeliveryVerified)
	expectedHostProfiles["a1"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "a2", &mdmlab.MDMDeliveryVerified)
	expectedHostProfiles["a2"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "a3", &mdmlab.MDMDeliveryVerified)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mdmlab.MDMDeliveryVerified)
	expectedHostProfiles["d1"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mdmlab.MDMDeliveryVerified)
	expectedHostProfiles["d2"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "d3", &mdmlab.MDMDeliveryVerified)
	expectedHostProfiles["d3"] = "verified"
	checkExpected(t, nil, hostIDsByProfileStatus{
		mdmlab.MDMDeliveryPending:  []uint{hosts[0].ID},
		mdmlab.MDMDeliveryVerified: []uint{hosts[9].ID},
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)
	...
	checkExpected(t, nil, hostIDsByProfileStatus{
		mdmlab.MDMDeliveryPending:  []uint{hosts[0].ID},
		mdmlab.MDMDeliveryVerified: []uint{hosts[9].ID}, // all profiles were verified
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)
...
func testAreHostsConnectedToMDMlabMDM(t *testing.T, ds *Datastore) {
	ctx := context.Background()

	notConnectedMac, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      "macos-test",
	...
	connectedMac, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      "macos-test",
		OsqueryHostID: ptr.String("osquery-macos"),
		NodeKey:       ptr.String("node-key-macos"),
		UUID:          uuid.NewString(),
	...
	disconnectedWithoutCheckoutMac, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	notConnectedWin, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      "windows-test",
	...
	connectedWin, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      "windows-test",
		OsqueryHostID: ptr.String("osquery-windows"),
		NodeKey:       ptr.String("node-key-windows"),
		UUID:          uuid.NewString(),
	...
	disconnectedWithoutCheckoutWin, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	require.NoError(t, err)
	windowsEnrollmentDisconnectedWithoutCheckout := &mdmlab.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
	...
	connectedMap, err := ds.AreHostsConnectedToMDMlabMDM(ctx, []*mdmlab.Host{
		notConnectedMac,
		connectedMac,
		connectedWin,
		notConnectedWin,
...
func testIsHostConnectedToMDMlabMDM(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	macH, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      "macos-test",
		OsqueryHostID: ptr.String("osquery-macos"),
	...
	connected, err := ds.IsHostConnectedToMDMlabMDM(ctx, macH)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	connected, err = ds.IsHostConnectedToMDMlabMDM(ctx, windowsH)
	require.NoError(t, err)
	require.False(t, connected)
...
func testBulkSetPendingMDMHostProfilesExcludeAny(t *testing.T, ds *Datastore) {
	...
	var labels []*mdmlab.Label
	for i := 0; i < 6; i++ {
		lbl, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "exclude-label-" + strconv.Itoa(i), Query: "select 1"})
		require.NoError(t, err)
		labels = append(labels, lbl)
	}
	...
	windowsProfs := []*mdmlab.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "W1", "W1", labels[2]),
	}
	appleDecls := []*mdmlab.MDMAppleDeclaration{
	...
	var i int
	winHost, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	i++
	appleHost, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	i++
	winHost2, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      fmt.Sprintf("win-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("win-uuid-%d", i),
	...
	i++
	appleHost2, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      fmt.Sprintf("apple-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("apple-uuid-%d", i),
