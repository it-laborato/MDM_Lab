...
import (
	...
	mdmlabmdm "github.com/it-laborato/MDM_Lab/server/mdm"
...
func testNewMDMAppleConfigProfileDuplicateName(t *testing.T, ds *Datastore) {
	...
	profC, err := ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "c", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	profBTm, err := ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "b", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "a", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	// create a duplicate name with a Windows profile for team
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "a", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
}
...
func testNewMDMAppleConfigProfileLabels(t *testing.T, ds *Datastore) {
	...
	require.True(t, mdmlab.IsForeignKey(err))
	...
	require.NoError(t, err)
	cp.LabelsIncludeAll = []mdmlab.ConfigurationProfileLabel{
...
func testNewMDMAppleConfigProfileDuplicateIdentifier(t *testing.T, ds *Datastore) {
	...
	duplicateCP := mdmlab.MDMAppleConfigProfile{
	...
	lbl, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "lbl", Query: "select 1"})
	require.NoError(t, err)

	labelCP := mdmlab.MDMAppleConfigProfile{
		Name:         "label-based",
...
func generateCP(name string, identifier string, teamID uint) *mdmlab.MDMAppleConfigProfile {
	mc := mobileconfig.Mobileconfig([]byte(name + identifier))
	return &mdmlab.MDMAppleConfigProfile{
		Name:         name,
...
func testListMDMAppleConfigProfiles(t *testing.T, ds *Datastore) {
	...
	expectedTeam0 := []*mdmlab.MDMAppleConfigProfile{}
	expectedTeam1 := []*mdmlab.MDMAppleConfigProfile{}
	...
	// add mdmlab-managed profiles for the team and globally
	for idf := range mobileconfig.MDMlabPayloadIdentifiers() {
...
func storeDummyConfigProfileForTest(t *testing.T, ds *Datastore) *mdmlab.MDMAppleConfigProfile {
	dummyMC := mobileconfig.Mobileconfig([]byte("DummyTestMobileconfigBytes"))
...
func checkConfigProfile(t *testing.T, expected, actual mdmlab.MDMAppleConfigProfile) {
...
func checkConfigProfileWithChecksum(t *testing.T, expected, actual mdmlab.MDMAppleConfigProfile) {
...
func testHostDetailsMDMProfiles(t *testing.T, ds *Datastore) {
	...
	p0, err := ds.NewMDMAppleConfigProfile(ctx, mdmlab.MDMAppleConfigProfile{Name: "Name0", Identifier: "Identifier0", Mobileconfig: []byte("profile0-bytes")})
	require.NoError(t, err)

	p1, err := ds.NewMDMAppleConfigProfile(ctx, mdmlab.MDMAppleConfigProfile{Name: "Name1", Identifier: "Identifier1", Mobileconfig: []byte("profile1-bytes")})
	require.NoError(t, err)

	p2, err := ds.NewMDMAppleConfigProfile(ctx, mdmlab.MDMAppleConfigProfile{Name: "Name2", Identifier: "Identifier2", Mobileconfig: []byte("profile2-bytes")})
	require.NoError(t, err)
	...
	h0, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	expectedProfiles0 := map[string]mdmlab.HostMDMAppleProfile{
		p0.ProfileUUID: {HostUUID: h0.UUID, Name: p0.Name, ProfileUUID: p0.ProfileUUID, CommandUUID: "cmd0-uuid", Status: &mdmlab.MDMDeliveryPending, OperationType: mdmlab.MDMOperationTypeInstall, Detail: ""},
		p1.ProfileUUID: {HostUUID: h0.UUID, Name: p1.Name, ProfileUUID: p1.ProfileUUID, CommandUUID: "cmd1-uuid", Status: &mdmlab.MDMDeliveryVerifying, OperationType: mdmlab.MDMOperationTypeInstall, Detail: ""},
		p2.ProfileUUID: {HostUUID: h0.UUID, Name: p2.Name, ProfileUUID: p2.ProfileUUID, CommandUUID: "cmd2-uuid", Status: &mdmlab.MDMDeliveryFailed, OperationType: mdmlab.MDMOperationTypeRemove, Detail: "Error removing profile"},
	}

	expectedProfiles1 := map[string]mdmlab.HostMDMAppleProfile{
		p0.ProfileUUID: {HostUUID: h1.UUID, Name: p0.Name, ProfileUUID: p0.ProfileUUID, CommandUUID: "cmd0-uuid", Status: &mdmlab.MDMDeliveryFailed, OperationType: mdmlab.MDMOperationTypeInstall, Detail: "Error installing profile"},
		p1.ProfileUUID: {HostUUID: h1.UUID, Name: p1.Name, ProfileUUID: p1.ProfileUUID, CommandUUID: "cmd1-uuid", Status: &mdmlab.MDMDeliveryVerifying, OperationType: mdmlab.MDMOperationTypeInstall, Detail: ""},
		p2.ProfileUUID: {HostUUID: h1.UUID, Name: p2.Name, ProfileUUID: p2.ProfileUUID, CommandUUID: "cmd2-uuid", Status: &mdmlab.MDMDeliveryFailed, OperationType: mdmlab.MDMOperationTypeRemove, Detail: "Error removing profile"},
	}
	...
	err = ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mdmlab.HostMDMAppleProfile{
	...
	h1InstallPending.Status = &mdmlab.MDMDeliveryPending
	...
	assert.Equal(t, &mdmlab.MDMDeliveryVerifying, gotProfs[0].Status)
...
func TestIngestMDMAppleDevicesFromDEPSync(t *testing.T) {
	...
	hosts := listHostsCheckCount(t, ds, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{}, 10)
	wantSerials := []string{}
	for _, h := range hosts {
	...
	hosts = listHostsCheckCount(t, ds, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{}, len(wantSerials))
	gotSerials := []string{}
	for _, h := range hosts {
...
func TestDEPSyncTeamAssignment(t *testing.T) {
	...
	hosts := listHostsCheckCount(t, ds, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{}, 2)
	for _, h := range hosts {
		require.Nil(t, h.TeamID)
	}

	// create a team
	team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "test team"})
	require.NoError(t, err)
	...
	nonExistentTeam := &mdmlab.Team{ID: 8888}
	depDevices = []godep.Device{
	...
	hosts = listHostsCheckCount(t, ds, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{}, 4)
	for _, h := range hosts {
...
func TestMDMEnrollment(t *testing.T) {
	...
	cases := []struct {
		...
		{"TestHostAlreadyExistsInMDMlab", testIngestMDMAppleHostAlreadyExistsInMDMlab},
		...
		{"TestNonDarwinHostAlreadyExistsInMDMlab", testIngestMDMNonDarwinHostAlreadyExistsInMDMlab},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			defer TruncateTables(t, ds)
...
func testIngestMDMAppleHostAlreadyExistsInMDMlab(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateMDMData(ctx, host.ID, false, false, "https://mdmlabdm.com", true, mdmlab.WellKnownMDMMDMlab, "")
	require.NoError(t, err)
	hosts := listHostsCheckCount(t, ds, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{}, 1)
	require.Equal(t, testSerial, hosts[0].HardwareSerial)
	require.Equal(t, testUUID, hosts[0].UUID)

	err = ds.MDMAppleUpsertHost(ctx, &mdmlab.Host{
		UUID:           testUUID,
		HardwareSerial: testSerial,
	})
	require.NoError(t, err)
...
func testIngestMDMNonDarwinHostAlreadyExistsInMDMlab(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	testSerial := "test-serial"
	testUUID := "test-uuid"
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateMDMData(ctx, host.ID, false, false, "https://mdmlabdm.com", true, "MDMlab MDM", "")
	require.NoError(t, err)
	hosts := listHostsCheckCount(t, ds, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{}, 1)
	require.Equal(t, testSerial, hosts[0].HardwareSerial)
	require.Equal(t, testUUID, hosts[0].UUID)
...
func testUpdateHostTablesOnMDMUnenroll(t *testing.T, ds *Datastore) {
	...
	profiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "z"),
	}

	err = ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{
		{
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateHostDiskEncryptionKey(ctx, &mdmlab.Host{ID: hostID}, "asdf", "", nil)
	require.NoError(t, err)
...
func expectAppleProfiles(
	...
	ds *Datastore,
	tmID *uint,
	want []*mdmlab.MDMAppleConfigProfile,
) map[string]string {
	if tmID == nil {
		tmID = ptr.Uint(0)
	}
	// don't use ds.ListMDMAppleConfigProfiles as it leaves out
	// mdmlab-managed profiles.
	var got []*mdmlab.MDMAppleConfigProfile
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		ctx := context.Background()
	...
	wantMap := make(map[string]*mdmlab.MDMAppleConfigProfile, len(want))
	for _, cp := range want {
...
func expectAppleDeclarations(
	...
	ds *Datastore,
	tmID *uint,
	want []*mdmlab.MDMAppleDeclaration,
) map[string]string {
	if tmID == nil {
		tmID = ptr.Uint(0)
	}

	var got []*mdmlab.MDMAppleDeclaration
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		ctx := context.Background()
	...
	wantMap := make(map[string]*mdmlab.MDMAppleDeclaration, len(want))
	for _, cp := range want {
		wantMap[cp.Identifier] = cp
	}
	...
	for _, gotD := range got {
		...
		require.True(t, strings.HasPrefix(gotD.DeclarationUUID, mdmlab.MDMAppleDeclarationUUIDPrefix))
...
func testBatchSetMDMAppleProfiles(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	applyAndExpect := func(newSet []*mdmlab.MDMAppleConfigProfile, tmID *uint, want []*mdmlab.MDMAppleConfigProfile) map[string]string {
		err := ds.BatchSetMDMAppleProfiles(ctx, tmID, newSet)
		require.NoError(t, err)
		return expectAppleProfiles(t, ds, tmID, want)
	}
	getProfileByTeamAndIdentifier := func(tmID *uint, identifier string) *mdmlab.MDMAppleConfigProfile {
		var prof mdmlab.MDMAppleConfigProfile
		var teamID uint
		if tmID != nil {
			teamID = *tmID
		}
	...
	withTeamID := func(p *mdmlab.MDMAppleConfigProfile, tmID uint) *mdmlab.MDMAppleConfigProfile {
		p.TeamID = &tmID
		return p
	}
	withUploadedAt := func(p *mdmlab.MDMAppleConfigProfile, ua time.Time) *mdmlab.MDMAppleConfigProfile {
		p.UploadedAt = ua
		return p
	}
	...
	mTm1 := applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "a"),
	}, ptr.Uint(1), []*mdmlab.MDMAppleConfigProfile{
	...
	mNoTm := applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	}, nil, []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	})
	...
	mTm1b := applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
	...
	mNoTmb := applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
	...
	// for tm1
	mTm1c := applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
	...
	applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
	...
	// simulate profiles being added by mdmlab
	mdmlabProfiles := []*mdmlab.MDMAppleConfigProfile{}
	expectMDMlabProfiles := []*mdmlab.MDMAppleConfigProfile{}
	for fp := range mobileconfig.MDMlabPayloadIdentifiers() {
		mdmlabProfiles = append(mdmlabProfiles, configProfileForTest(t, fp, fp, fp))
		expectMDMlabProfiles = append(expectMDMlabProfiles, withTeamID(configProfileForTest(t, fp, fp, fp), 1))
	}

	applyAndExpect(mdmlabProfiles, nil, mdmlabProfiles)
	applyAndExpect(mdmlabProfiles, ptr.Uint(1), expectMDMlabProfiles)

	// add no-team profiles
	applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	}, nil, append([]*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	}, mdmlabProfiles...))
	...
	applyAndExpect([]*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "a"),
		configProfileForTest(t, "N2", "I2", "b"),
	}, ptr.Uint(1), append([]*mdmlab.MDMAppleConfigProfile{
		withTeamID(configProfileForTest(t, "N1", "I1", "a"), 1),
		withTeamID(configProfileForTest(t, "N2", "I2", "b"), 1),
	}, expectMDMlabProfiles...))

	// cleaning profiles still leaves the profile managed by MDMlab
	applyAndExpect(nil, nil, mdmlabProfiles)
	applyAndExpect(nil, ptr.Uint(1), expectMDMlabProfiles)
}
...
func configProfileForTest(t *testing.T, name, identifier, uuid string, labels ...*mdmlab.Label) *mdmlab.MDMAppleConfigProfile {
	prof := configProfileBytesForTest(name, identifier, uuid)
	cp, err := mdmlab.NewMDMAppleConfigProfile(prof, nil)
	require.NoError(t, err)
	...
	for _, lbl := range labels {
		...
		case strings.HasPrefix(lbl.Name, "exclude-"):
			cp.LabelsExcludeAny = append(cp.LabelsExcludeAny, mdmlab.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		case strings.HasPrefix(lbl.Name, "include-any-"):
			cp.LabelsIncludeAny = append(cp.LabelsIncludeAny, mdmlab.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		default:
			cp.LabelsIncludeAll = append(cp.LabelsIncludeAll, mdmlab.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		}
	}
...
func declForTest(name, identifier, payloadContent string, labels ...*mdmlab.Label) *mdmlab.MDMAppleDeclaration {
	tmpl := `{
		"Type": "com.apple.configuration.decl%s",
		"Identifier": "com.mdmlab.config%s",
		"Payload": {
	...
	decl := &mdmlab.MDMAppleDeclaration{
		RawJSON:    declBytes,
		Identifier: fmt.Sprintf("com.mdmlab.config%s", identifier),
		Name:       name,
	}

	for _, l := range labels {
		if strings.HasPrefix(l.Name, "exclude-") {
			decl.LabelsExcludeAny = append(decl.LabelsExcludeAny, mdmlab.ConfigurationProfileLabel{LabelName: l.Name, LabelID: l.ID})
		} else {
			decl.LabelsIncludeAll = append(decl.LabelsIncludeAll, mdmlab.ConfigurationProfileLabel{LabelName: l.Name, LabelID: l.ID})
		}
	}
...
func teamConfigProfileForTest(t *testing.T, name, identifier, uuid string, teamID uint) *mdmlab.MDMAppleConfigProfile {
	prof := configProfileBytesForTest(name, identifier, uuid)
	cp, err := mdmlab.NewMDMAppleConfigProfile(configProfileBytesForTest(name, identifier, uuid), &teamID)
	require.NoError(t, err)
	sum := md5.Sum(prof) // nolint:gosec // used only to hash for efficient comparisons
	cp.Checksum = sum[:]
	return cp
...
func testMDMAppleProfileManagement(t *testing.T, ds *Datastore) {
	...
	matchProfiles := func(want, got []*mdmlab.MDMAppleProfilePayload) {
	...
	globalProfiles := []*mdmlab.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "z"),
		configProfileForTest(t, "N2", "I2", "b"),
	...
	// a macOS host that's not MDM enrolled into MDMlab shouldn't
	...
	profilesToInstall, err = ds.ListMDMAppleProfilesToInstall(ctx)
	require.NoError(t, err)
	matchProfiles([]*mdmlab.MDMAppleProfilePayload{
	...
	teamProfiles := []*mdmlab.MDMAppleConfigProfile{
	...
	err = ds.BulkUpsertMDMAppleHostProfiles(
		ctx, []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{
			{
	...
	verified := []*mdmlab.HostMacOSProfile{
...
// testIngestMDMAppleHostAlreadyExistsInMDMlab)
func checkMDMHostRelatedTables(t *testing.T, ds *Datastore, hostID uint, expectedSerial string, expectedModel string) {
	...
	require.NoError(t, err)
	require.Equal(t, mdmlab.WellKnownMDMMDMlab, mdmSolution.Name)
...
func nanoEnrollAndSetHostMDMData(t *testing.T, ds *Datastore, host *mdmlab.Host, withUser bool) {
	ctx := context.Background()
	...
	err = ds.SetOrUpdateMDMData(ctx, host.ID, false, true, expectedMDMServerURL, true, mdmlab.WellKnownMDMMDMlab, "")
	require.NoError(t, err)
}

func nanoEnroll(t *testing.T, ds *Datastore, host *mdmlab.Host, withUser bool) {
...
func upsertHostCPs(
	hosts []*mdmlab.Host,
	profiles []*mdmlab.MDMAppleConfigProfile,
	opType mdmlab.MDMOperationType,
	status *mdmlab.MDMDeliveryStatus,
	ctx context.Context,
	ds *Datastore,
	t *testing.T,
) {
	upserts := []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{}
	for _, h := range hosts {
		for _, cp := range profiles {
			...
			payload := mdmlab.MDMAppleBulkUpsertHostProfilePayload{
...
func testAggregateMacOSSettingsStatusWithFileVault(t *testing.T, ds *Datastore) {
	...
	checkListHosts := func(status mdmlab.OSSettingsStatus, teamID *uint, expected []*mdmlab.Host) bool {
		expectedIDs := []uint{}
		...
		gotHosts, err := ds.ListHosts(
			ctx,
			mdmlab.TeamFilter{User: &mdmlab.User{GlobalRole: ptr.String("admin")}},
			mdmlab.HostListOptions{MacOSSettingsFilter: status, TeamFilter: teamID},
		)
		gotIDs := []uint{}
	...
	var noTeamCPs []*mdmlab.MDMAppleConfigProfile
	for i := 0; i < 10; i++ {
	...
	fvNoTeam, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP("filevault", "com.mdmlabdm.mdmlab.mdm.filevault", 0))
	require.NoError(t, err)

	// upsert all host profiles with nil status, counts all as pending
	upsertHostCPs(hosts, append(noTeamCPs, fvNoTeam), mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	...
	upsertHostCPs(hosts, noTeamCPs, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	upsertHostCPs(hosts, noTeamCPs, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts), res.Pending) // still pending because filevault not installed
	...
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{fvNoTeam}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryPending, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{fvNoTeam}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[0], profilesByIdentifier([]*mdmlab.HostMacOSProfile{{Identifier: fvNoTeam.Identifier, DisplayName: fvNoTeam.Name, InstallDate: time.Now()}})))
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts)-1, res.Pending)
	...
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, nil, hosts[2:]))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, nil, hosts[1:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, nil, hosts[0:1]))

	// create a team
	team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "test"})
	require.NoError(t, err)
	...
	upsertHostCPs(hosts[9:10], append(noTeamCPs, fvNoTeam), mdmlab.MDMOperationTypeRemove, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	...
	var teamCPs []*mdmlab.MDMAppleConfigProfile
	for i := 0; i < 2; i++ {
	...
	fvTeam, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP(mdmlabmdm.MDMlabFileVaultProfileName, mobileconfig.MDMlabFileVaultPayloadIdentifier, team.ID))
	require.NoError(t, err)

	upsertHostCPs(hosts[9:10], append(teamCPs, fvTeam), mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &team.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	// check that list hosts by status matches summary
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, &team.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &team.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &team.ID, hosts[9:10]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &team.ID, []*mdmlab.Host{}))

	upsertHostCPs(hosts[9:10], append(teamCPs, fvTeam), mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &team.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	// check that list hosts by status matches summary
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, &team.ID, hosts[9:10]))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &team.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &team.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &team.ID, []*mdmlab.Host{}))
	...
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &team.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &team.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &team.ID, hosts[9:10]))
}

func testMDMAppleHostsProfilesStatus(t *testing.T, ds *Datastore) {
	ctx := context.Background()

	checkFilterHostsByMacOSSettings := func(status mdmlab.OSSettingsStatus, teamID *uint, expected []*mdmlab.Host) bool {
		expectedIDs := []uint{}
		for _, h := range expected {
			expectedIDs = append(expectedIDs, h.ID)
		}
		...
		gotHosts, err := ds.ListHosts(ctx, mdmlab.TeamFilter{User: &mdmlab.User{GlobalRole: ptr.String("admin")}}, mdmlab.HostListOptions{MacOSSettingsFilter: status, TeamFilter: teamID})
		gotIDs := []uint{}
		for _, h := range gotHosts {
			gotIDs = append(gotIDs, h.ID)
		}
	...
	checkFilterHostsByOSSettings := func(status mdmlab.OSSettingsStatus, teamID *uint, expected []*mdmlab.Host) bool {
		expectedIDs := []uint{}
		for _, h := range expected {
			expectedIDs = append(expectedIDs, h.ID)
		}

		gotHosts, err := ds.ListHosts(ctx, mdmlab.TeamFilter{User: &mdmlab.User{GlobalRole: ptr.String("admin")}}, mdmlab.HostListOptions{OSSettingsFilter: status, TeamFilter: teamID})
		gotIDs := []uint{}
		for _, h := range gotHosts {
			gotIDs = append(gotIDs, h.ID)
		}
	...
	upsertHostCPs(hosts, noTeamCPs, mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	res, err := ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	require.Equal(t, uint(0), res.Verifying)
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, nil, hosts))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, ptr.Uint(0), hosts))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, ptr.Uint(0), []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, ptr.Uint(0), []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, ptr.Uint(0), []*mdmlab.Host{}))

	// all hosts pending install of all profiles
	upsertHostCPs(hosts, noTeamCPs, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryPending, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts), res.Pending) // each host only counts once
	...
	upsertHostCPs(hosts[0:2], noTeamCPs[0:1], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryFailed, ctx, ds, t)
	// hosts[0] and hosts[1] have one profile pending as nil
	upsertHostCPs(hosts[0:2], noTeamCPs[3:4], mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	// hosts[0] also failed another profile
	upsertHostCPs(hosts[0:1], noTeamCPs[1:2], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryFailed, ctx, ds, t)
	// hosts[4] has all profiles reported as nil (pending)
	upsertHostCPs(hosts[4:5], noTeamCPs, mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	// hosts[5] has one profile reported as nil (pending)
	upsertHostCPs(hosts[5:6], noTeamCPs[0:1], mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	...
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, nil, hosts[2:]))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, nil, hosts[0:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, ptr.Uint(0), hosts[2:]))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, ptr.Uint(0), hosts[0:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, ptr.Uint(0), []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, ptr.Uint(0), []*mdmlab.Host{}))

	// hosts[0:3] installed a third profile
	upsertHostCPs(hosts[0:3], noTeamCPs[2:3], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil) // get summary for profiles with no team
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	require.Equal(t, uint(0), res.Verifying)          // no change, host must apply all profiles count as latest
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, nil, pendingHosts))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, nil, hosts[0:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, ptr.Uint(0), pendingHosts))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, ptr.Uint(0), hosts[0:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, ptr.Uint(0), []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, ptr.Uint(0), []*mdmlab.Host{}))

	// hosts[9] installed all profiles but one is with status nil (pending)
	upsertHostCPs(hosts[9:10], noTeamCPs[:9], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	upsertHostCPs(hosts[9:10], noTeamCPs[9:10], mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	pendingHosts = hosts[2:6:6]
	pendingHosts = append(pendingHosts, hosts[7:]...)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil) // get summary for profiles with no team
	require.NoError(t, err)
	...
	upsertHostCPs(hosts[9:10], noTeamCPs, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	pendingHosts = hosts[2:6:6]
	...
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, nil, pendingHosts))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, nil, hosts[0:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, nil, hosts[9:10]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, nil, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, ptr.Uint(0), pendingHosts))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, ptr.Uint(0), hosts[0:2]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, ptr.Uint(0), hosts[9:10]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, ptr.Uint(0), []*mdmlab.Host{}))

	// create a team
	tm, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "rocket"})
	require.NoError(t, err)
	...
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &tm.ID, []*mdmlab.Host{}))

	// transfer hosts[9] to new team
	err = ds.AddHostsToTeam(ctx, &tm.ID, []uint{hosts[9].ID})
	require.NoError(t, err)
	// remove all no team profiles from hosts[9]
	upsertHostCPs(hosts[9:10], noTeamCPs, mdmlab.MDMOperationTypeRemove, &mdmlab.MDMDeliveryPending, ctx, ds, t)
	...
	require.Equal(t, uint(0), res.Verifying)
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, &tm.ID, hosts[9:10]))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &tm.ID, []*mdmlab.Host{}))
	...
	upsertHostCPs(hosts[9:10], teamCPs, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	...
	upsertHostCPs(hosts[9:10], noTeamCPs, mdmlab.MDMOperationTypeRemove, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &tm.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	require.True(t, checkListHosts(mdmlab.OSSettingsPending, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &tm.ID, hosts[9:10]))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &tm.ID, []*mdmlab.Host{}))

	// verify one profile on hosts[9]
	upsertHostCPs(hosts[9:10], teamCPs[0:1], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &tm.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	upsertHostCPs(hosts[9:10], teamCPs[1:], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &tm.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	require.True(t, checkListHosts(mdmlab.OSSettingsFailed, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerifying, &tm.ID, []*mdmlab.Host{}))
	require.True(t, checkListHosts(mdmlab.OSSettingsVerified, &tm.ID, hosts[9:10]))
...
func testDoNotIgnoreMDMClientError(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{{
	...
	require.Equal(t, mdmlab.MDMOperationTypeRemove, cps[0].OperationType)
	require.NotNil(t, cps[0].Status)
	require.Equal(t, mdmlab.MDMDeliveryPending, *cps[0].Status)

	// simulate remove failed with client error message
	require.NoError(t, ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mdmlab.HostMDMAppleProfile{
		CommandUUID:   "c1",
		HostUUID:      "h1",
	...
	require.Equal(t, mdmlab.MDMOperationTypeRemove, cps[0].OperationType)
	require.NotNil(t, cps[0].Status)
	require.Equal(t, mdmlab.MDMDeliveryFailed, *cps[0].Status)
...
func createDiskEncryptionRecord(ctx context.Context, ds *Datastore, t *testing.T, host *mdmlab.Host, key string, decryptable bool,
	threshold time.Time) {
...
func TestMDMAppleFileVaultSummary(t *testing.T) {
	...
	// no teams tests =====
	noTeamFVProfile, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP(mdmlabmdm.MDMlabFileVaultProfileName, mobileconfig.MDMlabFileVaultPayloadIdentifier, 0))
	require.NoError(t, err)

	// verifying status
	verifyingHost := hosts[0]
	upsertHostCPs(
		[]*mdmlab.Host{verifyingHost},
	...
	upsertHostCPs(
		[]*mdmlab.Host{requiredActionHost},
		[]*mdmlab.MDMAppleConfigProfile{noTeamFVProfile},
		mdmlab.MDMOperationTypeInstall,
	...
	// failed status
	failedHost := hosts[3]
	upsertHostCPs([]*mdmlab.Host{failedHost}, []*mdmlab.MDMAppleConfigProfile{noTeamFVProfile}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryFailed, ctx, ds, t)
	...
	upsertHostCPs([]*mdmlab.Host{removingEnforcementHost}, []*mdmlab.MDMAppleConfigProfile{noTeamFVProfile}, mdmlab.MDMOperationTypeRemove, &mdmlab.MDMDeliveryPending, ctx, ds, t)
	fvProfileSummary, err = ds.GetMDMAppleFileVaultSummary(ctx, nil)
	...
	tm, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team-1"})
	require.NoError(t, err)
	team1FVProfile, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP(mdmlabmdm.MDMlabFileVaultProfileName, mobileconfig.MDMlabFileVaultPayloadIdentifier, tm.ID))
	require.NoError(t, err)
	err = ds.AddHostsToTeam(ctx, &tm.ID, []uint{verifyingTeam1Host.ID})
	require.NoError(t, err)

	upsertHostCPs([]*mdmlab.Host{verifyingTeam1Host}, []*mdmlab.MDMAppleConfigProfile{team1FVProfile}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	...
	// verified status
	upsertHostCPs(
		[]*mdmlab.Host{verifyingTeam1Host},
		[]*mdmlab.MDMAppleConfigProfile{team1FVProfile},
		mdmlab.MDMOperationTypeInstall,
...
func testGetMDMAppleCommandResults(t *testing.T, ds *Datastore) {
	...
	enrolledHosts := make([]*mdmlab.Host, 3)
	for i := 0; i < 3; i++ {
		h, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	i := 3
	unenrolledHost, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      fmt.Sprintf("test-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("test-uuid-%d", i),
	...
	require.Equal(t, res[0], &mdmlab.MDMCommandResult{
	...
	require.ElementsMatch(t, res, []*mdmlab.MDMCommandResult{
		{
			HostUUID:    enrolledHosts[0].UUID,
			CommandUUID: uuid2,
			Status:      "Acknowledged",
...
func testBulkUpsertMDMAppleConfigProfile(t *testing.T, ds *Datastore) {
	...
	globalCP := &mdmlab.MDMAppleConfigProfile{
		Name:         "DummyTestName",
		Identifier:   "DummyTestIdentifier",
		Mobileconfig: mc,
		TeamID:       nil,
	}
	teamCP := &mdmlab.MDMAppleConfigProfile{
		Name:         "DummyTestName",
		Identifier:   "DummyTestIdentifier",
		Mobileconfig: mc,
		TeamID:       ptr.Uint(1),
	}
	allProfiles := []*mdmlab.MDMAppleConfigProfile{globalCP, teamCP}
...
func testMDMAppleBootstrapPackageCRUD(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	var nfe mdmlab.NotFoundError
	var aerr mdmlab.AlreadyExistsError

	err := ds.InsertMDMAppleBootstrapPackage(ctx, &mdmlab.MDMAppleBootstrapPackage{}, nil)
	require.Error(t, err)

	bp1 := &mdmlab.MDMAppleBootstrapPackage{
		TeamID: uint(0),
		Name:   t.Name(),
	...
	bp2 := &mdmlab.MDMAppleBootstrapPackage{
		TeamID: uint(2),
		Name:   t.Name(),
		Sha256: sha256.New().Sum(nil),
		Bytes:  []byte("content"),
...
func testListMDMAppleCommands(t *testing.T, ds *Datastore) {
	...
	// create a team
	tm1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	...
	// no commands yet
	res, err := ds.ListMDMAppleCommands(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, &mdmlab.MDMCommandListOptions{})
	require.NoError(t, err)
	require.Empty(t, res)
	...
	require.ElementsMatch(t, res, []*mdmlab.MDMAppleCommand{
		{
	...
	res, err = ds.ListMDMAppleCommands(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, &mdmlab.MDMCommandListOptions{
		ListOptions: mdmlab.ListOptions{Page: 0, PerPage: 3, OrderKey: "device_id", OrderDirection: mdmlab.OrderDescending},
	})
	require.NoError(t, err)
	require.Len(t, res, 3)
	...
	res, err = ds.ListMDMAppleCommands(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, &mdmlab.MDMCommandListOptions{
		ListOptions: mdmlab.ListOptions{Page: 1, PerPage: 3, OrderKey: "device_id", OrderDirection: mdmlab.OrderDescending},
	})
	require.NoError(t, err)
	require.Len(t, res, 1)

	// filter by a user from team tm1, can only see that team's host
	u1, err := ds.NewUser(ctx, &mdmlab.User{
		...
		Teams: []mdmlab.UserTeam{
			{Team: *tm1, Role: mdmlab.RoleObserver},
		},
	})
	require.NoError(t, err)
	...
	res, err = ds.ListMDMAppleCommands(ctx, mdmlab.TeamFilter{User: u1}, &mdmlab.MDMCommandListOptions{
		ListOptions: mdmlab.ListOptions{PerPage: 3},
	})
	require.NoError(t, err)
	require.Len(t, res, 0)

	// now with IncludeObserver set to true
	res, err = ds.ListMDMAppleCommands(ctx, mdmlab.TeamFilter{User: u1, IncludeObserver: true}, &mdmlab.MDMCommandListOptions{
		ListOptions: mdmlab.ListOptions{PerPage: 3, OrderKey: "updated_at", OrderDirection: mdmlab.OrderDescending},
	})
	require.NoError(t, err)
	require.Len(t, res, 1)
	require.NotZero(t, res[0].UpdatedAt)
...
func testMDMAppleSetupAssistant(t *testing.T, ds *Datastore) {
	...
	// create for no team
	noTeamAsst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{Name: "test", Profile: json.RawMessage("{}")})
	require.NoError(t, err)
	...
	_, err = ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: ptr.Uint(123), Name: "test", Profile: json.RawMessage("{}")})
	require.Error(t, err)
	require.ErrorContains(t, err, "foreign key constraint fails")

	// create a team
	tm, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "tm"})
	require.NoError(t, err)

	// create for existing team
	tmAsst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test", Profile: json.RawMessage("{}")})
	require.NoError(t, err)
	...
	tok1, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "o1", EncryptedToken: []byte(uuid.NewString())})
	require.NoError(t, err)
	...
	// with both tokens
	tok2, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "o2", EncryptedToken: []byte(uuid.NewString())})
	require.NoError(t, err)
	...
	tmAsst2, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test2", Profile: json.RawMessage(`{"x":2}`)})
	require.NoError(t, err)
	...
	noTeamAsst2, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{Name: "test3", Profile: json.RawMessage(`{"x": 3}`)})
	require.NoError(t, err)
	...
	tmAsst3, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test2", Profile: json.RawMessage(`{"x":2}`)})
	require.NoError(t, err)
	...
	tmAsst4, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test2", Profile: json.RawMessage(`{"x":3}`)})
	require.NoError(t, err)
...
func testMDMAppleEnrollmentProfile(t *testing.T, ds *Datastore) {
	...
	_, err := ds.GetMDMAppleEnrollmentProfileByType(ctx, mdmlab.MDMAppleEnrollmentTypeAutomatic)
	require.Error(t, err)
	require.ErrorIs(t, err, sql.ErrNoRows)
	...
	profAuto, err := ds.NewMDMAppleEnrollmentProfile(ctx, mdmlab.MDMAppleEnrollmentProfilePayload{
		Type:       "automatic",
		DEPProfile: &rawMsg,
		Token:      "abcd",
	})
	...
	profMan, err := ds.NewMDMAppleEnrollmentProfile(ctx, mdmlab.MDMAppleEnrollmentProfilePayload{
		Type:       "manual",
		DEPProfile: &rawMsg,
		Token:      "efgh",
	})
	...
	getProf, err := ds.GetMDMAppleEnrollmentProfileByType(ctx, mdmlab.MDMAppleEnrollmentTypeAutomatic)
	require.NoError(t, err)
	getProf.UpdateCreateTimestamps = mdmlab.UpdateCreateTimestamps{}
...
func testListMDMAppleSerials(t *testing.T, ds *Datastore) {
	...
	// create a mix of DEP-enrolled hosts, non-MDMlab-MDM, pending DEP-enrollment
	hosts := make([]*mdmlab.Host, 7)
	for i := 0; i < len(hosts); i++ {
		...
		switch {
		case i <= 3:
			// assigned in ABM to MDMlab
			err = ds.UpsertMDMAppleHostDEPAssignments(ctx, []mdmlab.Host{*h}, abmToken.ID)
			require.NoError(t, err)
		case i == 4:
			// not ABM assigned
		case i == 5:
	...
	tm1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	tm2, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team2"})
	require.NoError(t, err)
...
func testSetVerifiedMacOSProfiles(t *testing.T, ds *Datastore) {
	...
	expectedHostMDMStatus := make(map[uint]map[string]mdmlab.MDMDeliveryStatus)
	...
	require.NoError(t, err)
	require.Len(t, cps, 4)
	storedByIdentifier := make(map[string]*mdmlab.MDMAppleConfigProfile)
	for _, cp := range cps {
	...
	for i := 0; i < 3; i++ {
		...
		hosts = append(hosts, h)
		expectedHostMDMStatus[h.ID] = map[string]mdmlab.MDMDeliveryStatus{
			cp1.Identifier: mdmlab.MDMDeliveryPending,
			cp2.Identifier: mdmlab.MDMDeliveryVerifying,
			cp3.Identifier: mdmlab.MDMDeliveryVerified,
			cp4.Identifier: mdmlab.MDMDeliveryPending,
		}
	}
	...
	adHocSetVerifying := func(hostUUID, profileIndentifier string) {
		ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
			_, err := tx.ExecContext(ctx,
				...
				mdmlab.MDMDeliveryVerifying, hostUUID, profileIndentifier)
			return err
		})
	}

	// initialize the host MDM profile statuses
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{storedByIdentifier[cp1.Identifier]}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryPending, ctx, ds, t)
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{storedByIdentifier[cp2.Identifier]}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{storedByIdentifier[cp3.Identifier]}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerified, ctx, ds, t)
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{storedByIdentifier[cp4.Identifier]}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryPending, ctx, ds, t)
	checkHostMDMProfileStatuses()

	// statuses don't change during the grace period if profiles are missing (i.e. not installed)
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[0], map[string]*mdmlab.HostMacOSProfile{}))
	checkHostMDMProfileStatuses()

	// if install date is before the updated at timestamp of the profile, statuses don't change
	// during the grace period
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[1], profilesByIdentifier([]*mdmlab.HostMacOSProfile{
		{
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[2], profilesByIdentifier([]*mdmlab.HostMacOSProfile{
		{
			Identifier:  cp2.Identifier,
			DisplayName: cp2.Name,
	...
	expectedHostMDMStatus[hosts[2].ID][cp2.Identifier] = mdmlab.MDMDeliveryVerified
	expectedHostMDMStatus[hosts[2].ID][cp4.Identifier] = mdmlab.MDMDeliveryVerified
	checkHostMDMProfileStatuses()
	...
	expectedHostMDMStatus[hosts[2].ID][cp1.Identifier] = mdmlab.MDMDeliveryVerified // cp1 can go from pending to verified
	expectedHostMDMStatus[hosts[2].ID][cp3.Identifier] = mdmlab.MDMDeliveryPending  // first retry for cp3
	expectedHostMDMStatus[hosts[2].ID][cp4.Identifier] = mdmlab.MDMDeliveryPending  // first retry for cp4
	checkHostMDMProfileStatuses()
	...
	expectedHostMDMStatus[hosts[2].ID][cp3.Identifier] = mdmlab.MDMDeliveryFailed // still missing after retry so expect cp3 to fail
	expectedHostMDMStatus[hosts[2].ID][cp4.Identifier] = mdmlab.MDMDeliveryFailed // still missing after retry so expect cp4 to fail
	checkHostMDMProfileStatuses()
	...
	expectedHostMDMStatus[hosts[2].ID][cp2.Identifier] = mdmlab.MDMDeliveryPending // first retry for cp2
	checkHostMDMProfileStatuses()
	// simulate retry command acknowledged by setting status to "verifying"
	...
	expectedHostMDMStatus[hosts[2].ID][cp2.Identifier] = mdmlab.MDMDeliveryFailed // still outdated after retry so expect cp2 to fail
	checkHostMDMProfileStatuses()
}
...
func TestCopyDefaultMDMAppleBootstrapPackage(t *testing.T) {
	...
	checkStoredBP := func(teamID uint, wantErr error, wantNewToken bool, wantBP *mdmlab.MDMAppleBootstrapPackage) {
		var gotBP mdmlab.MDMAppleBootstrapPackage
	...
	defaultBP := &mdmlab.MDMAppleBootstrapPackage{
		TeamID: noTeamID,
		Name:   "name",
	...
	defaultBP2 := &mdmlab.MDMAppleBootstrapPackage{
		TeamID: noTeamID,
		Name:   "new name",
	...
	tc.Config.WebhookSettings.FailingPoliciesWebhook = mdmlab.FailingPoliciesWebhookSettings{
		Enable:         true,
...
func TestHostDEPAssignments(t *testing.T) {
	...
	t.Run("DEP enrollment", func(t *testing.T) {
		...
		require.Equal(t, mdmlab.WellKnownMDMMDMlab, getHostResp.MDM.Name)
		require.Nil(t, getHostResp.DEPAssignedToMDMlab) // always nil for get host
		...
		testHost, err := ds.EnrollOrbit(ctx, true, mdmlab.OrbitHostInfo{HardwareSerial: depSerial, Platform: "darwin", HardwareUUID: depUUID, Hostname: "dep-host"}, depOrbitNodeKey, nil)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.True(t, *h.DEPAssignedToMDMlab)
		h, err = ds.LoadHostByDeviceAuthToken(ctx, depDeviceTok, 1*time.Hour)
		require.NoError(t, err)
		require.True(t, *h.DEPAssignedToMDMlab)

		// simulate osquery report of MDM detail query
		err = ds.SetOrUpdateMDMData(ctx, testHost.ID, false, true, expectedMDMServerURL, true, mdmlab.WellKnownMDMMDMlab, "")
		require.NoError(t, err)
	...
	t.Run("manual enrollment", func(t *testing.T) {
		...
		err = ds.MDMAppleUpsertHost(ctx, &mdmlab.Host{HardwareSerial: manualSerial, UUID: manualUUID})
		require.NoError(t, err)
		...
		// simulate initial osquery enrollment via Orbit
		manualHost, err := ds.EnrollOrbit(ctx, true, mdmlab.OrbitHostInfo{HardwareSerial: manualSerial, Platform: "darwin", HardwareUUID: manualUUID, Hostname: "maunual-host"}, manualOrbitNodeKey, nil)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.False(t, *h.DEPAssignedToMDMlab)
...
%s
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("%q %q %q", c.prefix, c.content, c.suffix), func(t *testing.T) {
			...
			prof, err := ds.NewMDMAppleConfigProfile(ctx, mdmlab.MDMAppleConfigProfile{
			...
			goProf := mdmlab.MDMApplePreassignProfilePayload{Profile: mc}
...
func testMDMAppleResetEnrollment(t *testing.T, ds *Datastore) {
	...
	cp, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP("name0", "identifier0", 0))
	require.NoError(t, err)
	upsertHostCPs([]*mdmlab.Host{host}, []*mdmlab.MDMAppleConfigProfile{cp}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerified, ctx, ds, t)
...
func testLockUnlockWipeMacOS(t *testing.T, ds *Datastore) {
	...
	err = ds.UnlockHostManually(ctx, host.ID, host.MDMlabPlatform(), time.Now().UTC())
	require.NoError(t, err)
...
func testMDMAppleDDMDeclarationsToken(t *testing.T, ds *Datastore) {
	...
	decl, err := ds.NewMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		Identifier: "decl-1",
		Name:       "decl-1",
	})
	require.NoError(t, err)
	...
	decl2, err := ds.NewMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		Identifier: "decl-2",
		Name:       "decl-2",
	})
	require.NoError(t, err)
...
func testMDMAppleSetPendingDeclarationsAs(t *testing.T, ds *Datastore) {
	...
	for i := 0; i < 10; i++ {
		_, err := ds.NewMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
	...
	checkStatus := func(declarations []mdmlab.HostMDMAppleProfile, wantStatus mdmlab.MDMDeliveryStatus, wantDetail string) {
	...
	checkStatus(profs, mdmlab.MDMDeliveryPending, "")

	err = ds.MDMAppleSetPendingDeclarationsAs(ctx, h.UUID, &mdmlab.MDMDeliveryFailed, "mock error")
	require.NoError(t, err)
	profs, err = ds.GetHostMDMAppleProfiles(ctx, h.UUID)
	require.NoError(t, err)
	checkStatus(profs, mdmlab.MDMDeliveryFailed, "mock error")
}

func testSetOrUpdateMDMAppleDDMDeclaration(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	l1, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "l1", Query: "select 1"})
	require.NoError(t, err)
	l2, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "l2", Query: "select 2"})
	require.NoError(t, err)
	tm1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "tm1"})
	require.NoError(t, err)

	d1, err := ds.NewMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		Identifier: "i1",
		Name:       "d1",
	...
	d1tm1, err := ds.SetOrUpdateMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		Identifier: "i1",
		Name:       "d1",
		TeamID:     &tm1.ID,
		RawJSON:    json.RawMessage(`{"Identifier": "i1"}`),
	...
	d1, err = ds.SetOrUpdateMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		Identifier:       "i1b",
		Name:             "d1",
		RawJSON:          json.RawMessage(`{"Identifier": "i1b"}`),
		LabelsIncludeAll: []mdmlab.ConfigurationProfileLabel{{LabelName: l1.Name, LabelID: l1.ID}},
	})
	require.NoError(t, err)
	...
	d1, err = ds.SetOrUpdateMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		...
		Name:             "d1",
		RawJSON:          json.RawMessage(`{"Identifier": "i1b"}`),
		LabelsIncludeAll: []mdmlab.ConfigurationProfileLabel{{LabelName: l2.Name, LabelID: l2.ID}},
	})
	require.NoError(t, err)
	require.Equal(t, d1.DeclarationUUID, d1Ori.DeclarationUUID)
	...
	d1tm1B, err := ds.SetOrUpdateMDMAppleDeclaration(ctx, &mdmlab.MDMAppleDeclaration{
		Identifier:       "i1b",
		Name:             "d1",
		TeamID:           &tm1.ID,
		RawJSON:          json.RawMessage(`{"Identifier": "i1b"}`),
...
func TestMDMAppleProfileVerification(t *testing.T) {
	...
	type testCase struct {
		name           string
		initialStatus  mdmlab.MDMDeliveryStatus
		expectedStatus mdmlab.MDMDeliveryStatus
		expectedDetail string
	}

	setupTestProfile := func(t *testing.T, suffix string) *mdmlab.MDMAppleConfigProfile {
	...
	setProfileUploadedAt := func(t *testing.T, cp *mdmlab.MDMAppleConfigProfile, ua time.Time) {
		ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
	...
	checkHostStatus := func(t *testing.T, h *mdmlab.Host, expectedStatus mdmlab.MDMDeliveryStatus, expectedDetail string) error {
		gotProfs, err := ds.GetHostMDMAppleProfiles(ctx, h.UUID)
		if err != nil {
			return err
		}
	...
	initializeProfile := func(t *testing.T, h *mdmlab.Host, cp *mdmlab.MDMAppleConfigProfile, status mdmlab.MDMDeliveryStatus, prevRetries uint) {
		upsertHostCPs([]*mdmlab.Host{h}, []*mdmlab.MDMAppleConfigProfile{cp}, mdmlab.MDMOperationTypeInstall, &status, ctx, ds, t)
	...
	t.Run("MissingProfileWithRetry", func(t *testing.T) {
		...
		cases := []testCase{
			{
				...
				initialStatus:  mdmlab.MDMDeliveryPending,
				expectedStatus: mdmlab.MDMDeliveryPending, // no change
			},
			{
				name:           "VerifyingThenMissing",
				initialStatus:  mdmlab.MDMDeliveryVerifying,
				expectedStatus: mdmlab.MDMDeliveryFailed, // change to failed
			},
			{
				name:           "VerifiedThenMissing",
				initialStatus:  mdmlab.MDMDeliveryVerified,
				expectedStatus: mdmlab.MDMDeliveryFailed, // change to failed
			},
			{
				name:           "FailedThenMissing",
				initialStatus:  mdmlab.MDMDeliveryFailed,
				expectedStatus: mdmlab.MDMDeliveryFailed, // no change
			},
		}
		...
		for i, tc := range cases {
			...
			var reportedProfiles []*mdmlab.HostMacOSProfile // no profiles reported for this test
			...
			require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
			if tc.expectedStatus == mdmlab.MDMDeliveryFailed {
				// grace period expired, first failure gets retried so status should be pending and empty detail
				require.NoError(t, checkHostStatus(t, h, mdmlab.MDMDeliveryPending, ""), tc.name)
			}

			if tc.initialStatus != mdmlab.MDMDeliveryPending {
				// after retry, assume successful install profile command so status should be verifying
				upsertHostCPs([]*mdmlab.Host{h}, []*mdmlab.MDMAppleConfigProfile{cp}, mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, ctx, ds, t)
				// report osquery results
				require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
				// now we see the expected status
				require.NoError(t, checkHostStatus(t, h, tc.expectedStatus, string(mdmlab.HostMDMProfileDetailFailedWasVerifying)), tc.name) // grace period expired, max retries so check expected status
			}
		}
	})
	...
	t.Run("OutdatedProfile", func(t *testing.T) {
		...
		cases := []testCase{
			...
			},
			{
				...
				initialStatus:  mdmlab.MDMDeliveryVerifying,
				expectedStatus: mdmlab.MDMDeliveryFailed, // change to failed
				expectedDetail: string(mdmlab.HostMDMProfileDetailFailedWasVerifying),
			},
			{
				name:           "VerifiedThenFoundOutdated",
				initialStatus:  mdmlab.MDMDeliveryVerified,
				expectedStatus: mdmlab.MDMDeliveryFailed, // change to failed
				expectedDetail: string(mdmlab.HostMDMProfileDetailFailedWasVerified),
			},
			{
		...
		for i, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				h := test.NewHost(t, ds, tc.name, tc.name, tc.name, tc.name, twoMinutesAgo)
				cp := setupTestProfile(t, fmt.Sprintf("%s-%d", tc.name, i))
				reportedProfiles := []*mdmlab.HostMacOSProfile{
					{
						DisplayName: cp.Name,
		...
		cases := []testCase{
			{
				...
				initialStatus:  mdmlab.MDMDeliveryPending,
				expectedStatus: mdmlab.MDMDeliveryVerified, // pending can go to verified if found
				expectedDetail: "",
			},
			{
				name:           "VerifyingThenFoundExpected",
				initialStatus:  mdmlab.MDMDeliveryVerifying,
				expectedStatus: mdmlab.MDMDeliveryVerified, // change to verified
				expectedDetail: "",
			},
			{
				name:           "VerifiedThenFoundExpected",
				initialStatus:  mdmlab.MDMDeliveryVerified,
				expectedStatus: mdmlab.MDMDeliveryVerified, // no change
				expectedDetail: "",
			},
			{
				name:           "FailedThenFoundExpected",
				initialStatus:  mdmlab.MDMDeliveryFailed,
				expectedStatus: mdmlab.MDMDeliveryVerified, // failed can become verified if found later
				expectedDetail: "",
			},
		}
		...
		cases := []testCase{
			{
				...
				initialStatus:  mdmlab.MDMDeliveryPending,
				expectedStatus: mdmlab.MDMDeliveryVerified, // profile can go from pending to verified
				expectedDetail: "",
			},
			{
	...
	t.Run("EarliestInstallDate", func(t *testing.T) {
		...
		require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
		require.NoError(t, checkHostStatus(t, h, mdmlab.MDMDeliveryVerifying, "")) // no change
		...
		require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
		require.NoError(t, checkHostStatus(t, h, mdmlab.MDMDeliveryFailed, string(mdmlab.HostMDMProfileDetailFailedWasVerifying))) // set to failed
		...
		require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
		require.NoError(t, checkHostStatus(t, h, mdmlab.MDMDeliveryVerified, "")) // set to verified based on earliest install date
	})
}

func profilesByIdentifier(profiles []*mdmlab.HostMacOSProfile) map[string]*mdmlab.HostMacOSProfile {
	byIdentifier := map[string]*mdmlab.HostMacOSProfile{}
...
func TestRestorePendingDEPHost(t *testing.T) {
	...
	t.Run("DEP enrollment", func(t *testing.T) {
		...
		checkStoredHost := func(t *testing.T, hostID uint, expectedHost *mdmlab.Host) {
		...
		setupTestHost := func(t *testing.T) (pendingHost, mdmEnrolledHost *mdmlab.Host) {
			depSerial := "dep-serial"
			depUUID := "dep-uuid"
			depOrbitNodeKey := "dep-orbit-node-key"
			...
			require.Equal(t, mdmlab.WellKnownMDMMDMlab, pendingHost.MDM.Name)
			...
			// simulate osquery report of MDM detail query
			err = ds.SetOrUpdateMDMData(ctx, depHostID, false, true, expectedMDMServerURL, true, mdmlab.WellKnownMDMMDMlab, "")
			require.NoError(t, err)
			...
			require.Equal(t, mdmlab.WellKnownMDMMDMlab, mdmEnrolledHost.MDM.Name)
...
func testMDMConfigAsset(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	assets := []mdmlab.MDMConfigAsset{
		{
			Name:  mdmlab.MDMAssetCACert,
			Value: []byte("a"),
		},
		{
			Name:  mdmlab.MDMAssetCAKey,
			Value: []byte("b"),
		},
	}
	wantAssets := map[mdmlab.MDMAssetName]mdmlab.MDMConfigAsset{}
	...
	a, err := ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey}, nil)
	require.NoError(t, err)
	require.Equal(t, wantAssets, a)

	h, err := ds.GetAllMDMConfigAssetsHashes(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey})
	require.NoError(t, err)
	require.Len(t, h, 2)
	require.NotEmpty(t, h[mdmlab.MDMAssetCACert])
	require.NotEmpty(t, h[mdmlab.MDMAssetCAKey])

	// try to fetch an asset that doesn't exist
	var nfe mdmlab.NotFoundError
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetABMCert}, ds.writer(ctx))
	require.ErrorAs(t, err, &nfe)
	require.Nil(t, a)

	h, err = ds.GetAllMDMConfigAssetsHashes(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetABMCert})
	require.ErrorAs(t, err, &nfe)
	require.Nil(t, h)

	// try to fetch a mix of assets that exist and doesn't exist
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetABMCert}, nil)
	require.ErrorIs(t, err, ErrPartialResult)
	require.Len(t, a, 1)

	h, err = ds.GetAllMDMConfigAssetsHashes(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetABMCert})
	require.ErrorIs(t, err, ErrPartialResult)
	require.Len(t, h, 1)
	require.NotEmpty(t, h[mdmlab.MDMAssetCACert])
	...
	newAssets := []mdmlab.MDMConfigAsset{
		{
			Name:  mdmlab.MDMAssetCACert,
			Value: []byte("c"),
		},
	...
	wantNewAssets := map[mdmlab.MDMAssetName]mdmlab.MDMConfigAsset{}
	...
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey}, ds.reader(ctx))
	require.NoError(t, err)
	...
	err = ds.DeleteMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey})
	require.NoError(t, err)
	...
	expected := make(map[string]mdmlab.MDMConfigAsset)
	...
	for _, got := range ar {
		...
		require.NoError(t, err)
		require.Equal(t, expected[string(d)].Name, mdmlab.MDMAssetName(got.Name))
	...
	// Hard delete
	err = ds.HardDeleteMDMConfigAsset(ctx, mdmlab.MDMAssetCACert)
	require.NoError(t, err)
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey}, nil)
	require.ErrorAs(t, err, &nfe)
	require.Nil(t, a)
	...
	var result bool
	err = sqlx.GetContext(ctx, ds.reader(ctx), &result, "SELECT 1 FROM mdm_config_assets WHERE name = ?", mdmlab.MDMAssetCACert)
	assert.ErrorIs(t, err, sql.ErrNoRows)

	// other (non-hard deleted asset still present)
	err = sqlx.GetContext(ctx, ds.reader(ctx), &result, "SELECT 1 FROM mdm_config_assets WHERE name = ?", mdmlab.MDMAssetCAKey)
...
func testListIOSAndIPadOSToRefetch(t *testing.T, ds *Datastore) {
	...
	hostCount := 0
	newHost := func(platform string) *mdmlab.Host {
		h, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	require.NoError(t, ds.AddHostMDMCommands(ctx, []mdmlab.HostMDMCommand{{
		HostID:      iOS0.ID,
...
func testIngestMDMAppleDevicesFromDEPSyncIOSIPadOS(t *testing.T, ds *Datastore) {
	...
	hosts, err := ds.ListHosts(ctx, mdmlab.TeamFilter{
		User: &mdmlab.User{
			GlobalRole: ptr.String(mdmlab.RoleAdmin),
		},
	}, mdmlab.HostListOptions{})
	require.NoError(t, err)
...
func testMDMAppleProfilesOnIOSIPadOS(t *testing.T, ds *Datastore) {
	...
	// Add the MDMlabd configuration and  profile that are only for macOS.
	params := mobileconfig.MDMlabdProfileOptions{
		EnrollSecret: t.Name(),
		ServerURL:    "https://example.com",
		PayloadType:  mobileconfig.MDMlabdConfigPayloadIdentifier,
		PayloadName:  mdmlabmdm.MDMlabdConfigProfileName,
	}
	var contents bytes.Buffer
	err := mobileconfig.MDMlabdProfileTemplate.Execute(&contents, params)
	require.NoError(t, err)
	mdmlabdConfigProfile, err := mdmlab.NewMDMAppleConfigProfile(contents.Bytes(), nil)
	require.NoError(t, err)
	_, err = ds.NewMDMAppleConfigProfile(ctx, *mdmlabdConfigProfile)
	require.NoError(t, err)

	// For the FileVault profile we re-use the MDMlabdProfileTemplate
	// (because fileVaultProfileTemplate is not exported)
	var contents2 bytes.Buffer
	params.PayloadName = mdmlabmdm.MDMlabFileVaultProfileName
	params.PayloadType = mobileconfig.MDMlabFileVaultPayloadIdentifier
	err = mobileconfig.MDMlabdProfileTemplate.Execute(&contents2, params)
	require.NoError(t, err)
	fileVaultProfile, err := mdmlab.NewMDMAppleConfigProfile(contents2.Bytes(), nil)
	require.NoError(t, err)
...
func testHostDetailsMDMProfilesIOSIPadOS(t *testing.T, ds *Datastore) {
	...
	p0, err := ds.NewMDMAppleConfigProfile(ctx, mdmlab.MDMAppleConfigProfile{
		Name:         "Name0",
	...
	iOS, err := ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	require.NoError(t, err)
	iPadOS, err := ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	expectedProfilesIOS := map[string]mdmlab.HostMDMAppleProfile{
		p0.ProfileUUID: {
			HostUUID:      iOS.UUID,
			Name:          p0.Name,
	...
	expectedProfilesIPadOS := map[string]mdmlab.HostMDMAppleProfile{
		p0.ProfileUUID: {
	...
	for _, tc := range []struct {
		host             *mdmlab.Host
		expectedProfiles map[string]mdmlab.HostMDMAppleProfile
	}{
		{
			host:             iOS,
		...
		require.Equal(t, mdmlab.MDMDeliveryVerified, *gotProfs[0].Status)
	}
}
...
func testMDMAppleBootstrapPackageWithS3(t *testing.T, ds *Datastore) {
	...
	bpMatchesWithoutContent := func(want, got *mdmlab.MDMAppleBootstrapPackage) {
	...
	err := ds.InsertMDMAppleBootstrapPackage(ctx, &mdmlab.MDMAppleBootstrapPackage{}, pkgStore)
	require.Error(t, err)
	...
	err = ds.CopyDefaultMDMAppleBootstrapPackage(ctx, &mdmlab.AppConfig{}, 1)
	require.NoError(t, err)
	...
	bp3 := &mdmlab.MDMAppleBootstrapPackage{
		TeamID: uint(3),
		Name:   "bp3",
...
func testMDMAppleGetAndUpdateABMToken(t *testing.T, ds *Datastore) {
	...
	tm2, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team2"})
	require.NoError(t, err)
	tm3, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team3"})
	require.NoError(t, err)
	...
	t1, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "unused", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, t1.ID)
	t2, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	...
	require.Equal(t, mdmlab.TeamNameNoTeam, tok.MacOSTeamName)
	require.Equal(t, mdmlab.TeamNameNoTeam, tok.IOSTeamName)
	require.Equal(t, mdmlab.TeamNameNoTeam, tok.IPadOSTeamName)
	...
	require.Equal(t, mdmlab.TeamNameNoTeam, tokReload.IPadOSTeamName)
	require.Equal(t, mdmlab.TeamNameNoTeam, tokReload.IPadOSTeam.Name)
	...
	require.Equal(t, "org-name", tokReload.OrganizationName)
	require.Equal(t, "name@example.com", tokReload.AppleID)
	require.Equal(t, mdmlab.TeamNameNoTeam, tokReload.MacOSTeamName)
	require.Equal(t, mdmlab.TeamNameNoTeam, tokReload.MacOSTeam.Name)
	...
	require.Equal(t, mdmlab.TeamNameNoTeam, expTok.MacOSTeamName)
	require.Equal(t, mdmlab.TeamNameNoTeam, expTok.MacOSTeam.Name)
...
func testMDMAppleABMTokensTermsExpired(t *testing.T, ds *Datastore) {
	...
	t1, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "abm1", EncryptedToken: []byte(encTok1)})
	require.NoError(t, err)
	require.NotEmpty(t, t1.ID)
	encTok2 := uuid.NewString()
	t2, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "abm2", EncryptedToken: []byte(encTok2)})
	require.NoError(t, err)
	require.NotEmpty(t, t2.ID)
	// this one simulates a mirated token - empty name
	encTok3 := uuid.NewString()
	t3, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "", EncryptedToken: []byte(encTok3)})
	require.NoError(t, err)
...
func testMDMGetABMTokenOrgNamesAssociatedWithTeam(t *testing.T, ds *Datastore) {
	...
	tok1, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "org1", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, tok1.ID)

	tok2, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "org2", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, tok1.ID)

	tok3, err := ds.InsertABMToken(ctx, &mdmlab.ABMToken{OrganizationName: "org3", EncryptedToken: []byte(encTok), MacOSDefaultTeamID: &tm2.ID})
	require.NoError(t, err)
	require.NotEmpty(t, tok1.ID)
	...
	h3, err := ds.NewHost(ctx, &mdmlab.Host{
		Hostname:      "test-host3-name",
	...
	h4, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	require.NoError(t, ds.UpsertMDMAppleHostDEPAssignments(ctx, []mdmlab.Host{*h1, *h4}, tok1.ID))
	require.NoError(t, ds.UpsertMDMAppleHostDEPAssignments(ctx, []mdmlab.Host{*h2}, tok3.ID))
	require.NoError(t, ds.UpsertMDMAppleHostDEPAssignments(ctx, []mdmlab.Host{*h3}, tok2.ID))
...
func testHostMDMCommands(t *testing.T, ds *Datastore) {
	...
	hostCommands := []mdmlab.HostMDMCommand{
		{
			HostID:      h.ID,
	...
	allCommands = append(allCommands, mdmlab.HostMDMCommand{
		HostID:      badHostID,
		CommandType: "command-1",
	})
...
func testIngestMDMAppleDeviceFromOTAEnrollment(t *testing.T, ds *Datastore) {
	...
	otaDevices := []mdmlab.MDMAppleMachineInfo{
...
func TestGetMDMAppleOSUpdatesSettingsByHostSerial(t *testing.T) {
	...
	getConfigSettings := func(teamID uint, key string) *mdmlab.AppleOSUpdateSettings {
		var settings mdmlab.AppleOSUpdateSettings
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
	...
	checkExpectedVersion := func(t *testing.T, gotSettings *mdmlab.AppleOSUpdateSettings, expectedVersion string) {
		if expectedVersion == "" {
	...
	encTok := uuid.NewString()
	abmToken, err := ds.InsertABMToken(context.Background(), &mdmlab.ABMToken{OrganizationName: "unused", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, abmToken.ID)
...
func testMDMManagedCertificates(t *testing.T, ds *Datastore) {
	...
	err = ds.BulkUpsertMDMManagedCertificates(ctx, []*mdmlab.MDMBulkUpsertManagedCertificatePayload{
		{
			HostUUID:             host.UUID,
			ProfileUUID:          initialCP.ProfileUUID,
...
func testAppleMDMSetBatchAsyncLastSeenAt(t *testing.T, ds *Datastore) {
	...
	// create some hosts, all enrolled
	enrolledHosts := make([]*mdmlab.Host, 2)
	...
	getHostLastSeenAt := func(h *mdmlab.Host) time.Time {
...
func testMDMAppleProfileLabels(t *testing.T, ds *Datastore) {
	...
	hostLabel, err := ds.NewHost(ctx, &mdmlab.Host{
	...
	// include-any labels
	l1, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "include-any-1", Query: "select 1"})
	require.NoError(t, err)

	l2, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "include-any-2", Query: "select 1"})
	require.NoError(t, err)

	l3, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "include-any-3", Query: "select 1"})
	require.NoError(t, err)

	// include-all labels
	l4, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "include-all-4", Query: "select 1"})
	require.NoError(t, err)

	l5, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "include-all-5", Query: "select 1"})
	require.NoError(t, err)

	// exclude-any labels
	l6, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "exclude-any-6", Query: "select 1"})
	require.NoError(t, err)

	l7, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "exclude-any-7", Query: "select 1"})
	require.NoError(t, err)
...
func testAggregateMacOSSettingsAllPlatforms(t *testing.T, ds *Datastore) {
	...
	upsertHostCPs(hosts, []*mdmlab.MDMAppleConfigProfile{cp}, mdmlab.MDMOperationTypeInstall, nil, ctx, ds, t)
	res, err := ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts), res.Pending)
