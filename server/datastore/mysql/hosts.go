...
func (ds *Datastore) NewHost(ctx context.Context, host *mdmlab.Host) (*mdmlab.Host, error) {
...
func (ds *Datastore) SerialUpdateHost(ctx context.Context, host *mdmlab.Host) error {
...
func (ds *Datastore) SaveHostPackStats(ctx context.Context, teamID *uint, hostID uint, stats []mdmlab.PackStats) error {
	return saveHostPackStatsDB(ctx, ds.writer(ctx), teamID, hostID, stats)
}

func saveHostPackStatsDB(ctx context.Context, db *sqlx.DB, teamID *uint, hostID uint, stats []mdmlab.PackStats) error {
...
func loadHostPackStatsDB(ctx context.Context, db sqlx.QueryerContext, hid uint, hostPlatform string) ([]mdmlab.PackStats, error) {
	...
	ds := dialect.From(goqu.I("scheduled_queries").As("sq")).Select(
		...
		goqu.Or(
			...
			goqu.L("FIND_IN_SET(?, sq.platform)", mdmlab.PlatformFromHost(hostPlatform)).Neq(0),
		),
	)
...
func loadHostScheduledQueryStatsDB(ctx context.Context, db sqlx.QueryerContext, hid uint, hostPlatform string, teamID *uint) ([]mdmlab.QueryStats, error) {
	var teamID_ uint
	if teamID != nil {
		teamID_ = *teamID
	}
	...
	args := []interface{}{
		...
		hid,
		hid,
		mdmlab.PlatformFromHost(hostPlatform),
...
func loadHostUsersDB(ctx context.Context, db sqlx.QueryerContext, hostID uint) ([]mdmlab.HostUser, error) {
	sql := `SELECT username, groupname, uid, user_type, shell FROM host_users WHERE host_id = ? and removed_at IS NULL`
	var users []mdmlab.HostUser
...
func (ds *Datastore) ListHostUsers(ctx context.Context, hostID uint) ([]mdmlab.HostUser, error) {
...
// deleted from when a host is deleted in MDMlab:
...
func deleteHosts(ctx context.Context, tx sqlx.ExtContext, hostIDs []uint) error {
	...
	stmt, args, err = sqlx.In(`DELETE FROM pack_targets WHERE type = ? AND target_id IN (?)`, mdmlab.TargetHost, hostIDs)
	if err != nil {
...
func (ds *Datastore) Host(ctx context.Context, id uint) (*mdmlab.Host, error) {
	sqlStatement := `
SELECT
  h.id,
  h.osquery_host_id,
  ...
  hoi.desktop_version AS mdmlab_desktop_version,
...
`
	...
	if host.DiskEncryptionEnabled != nil && !(*host.DiskEncryptionEnabled) && mdmlab.IsLinux(host.Platform) {
		// omit disk encryption information for linux if it is not enabled, as we
		// cannot know for sure that it is not encrypted (See
		// https://github.com/mdmlabdm/mdmlab/issues/3906).
	...
	var (
		globalQueriesStats   []mdmlab.QueryStats
		hostTeamQueriesStats []mdmlab.QueryStats
	)
	...
	if len(globalQueriesStats) > 0 {
		host.PackStats = append(host.PackStats, mdmlab.PackStats{
			PackName:   "Global",
			Type:       "global",
...
func queryStatsToScheduledQueryStats(queriesStats []mdmlab.QueryStats, packName string) []mdmlab.ScheduledQueryStats {
	scheduledQueriesStats := make([]mdmlab.ScheduledQueryStats, 0, len(queriesStats))
	for _, queryStats := range queriesStats {
		scheduledQueriesStats = append(scheduledQueriesStats, mdmlab.ScheduledQueryStats{
...
const hostMDMSelect = `,
	JSON_OBJECT(
		...
		'dep_profile_error',
		CASE
			WHEN hdep.assign_profile_response = '` + string(mdmlab.DEPAssignProfileResponseFailed) + `' THEN CAST(TRUE AS JSON)
	...
	// query, please update the AreHostsConnectedToMDMlabMDM
...
func amountEnrolledHostsByOSDB(ctx context.Context, db sqlx.QueryerContext) (byOS map[string][]mdmlab.HostsCountByOSVersion, totalCount int, err error) {
  ...
  `
	...
	byOS = make(map[string][]mdmlab.HostsCountByOSVersion)
	for _, h := range hostsByOS {
		totalCount += h.NumHosts
		byVersion := byOS[h.Platform]
		byVersion = append(byVersion, mdmlab.HostsCountByOSVersion{
			Version:     h.OSVersion,
...
func (ds *Datastore) ListHosts(ctx context.Context, filter mdmlab.TeamFilter, opt mdmlab.HostListOptions) ([]*mdmlab.Host, error) {
	sql := `SELECT
    h.id,
    h.osquery_host_id,
    h.created_at,
...
func (ds *Datastore) applyHostFilters(
	ctx context.Context, opt mdmlab.HostListOptions, sqlStmt string, filter mdmlab.TeamFilter, selectParams []interface{},
	...
	connectedToMDMlabJoin := ""
	if opt.ConnectedToMDMlabFilter != nil && *opt.ConnectedToMDMlabFilter ||
	...
	sqlStmt, whereParams = filterHostsByConnectedToMDMlab(sqlStmt, opt, whereParams)
	var err error
...
func filterHostsByTeam(sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByMDM(sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByConnectedToMDMlab(sql string, opt mdmlab.HostListOptions, params []any) (string, []any) {
	if opt.ConnectedToMDMlabFilter != nil && *opt.ConnectedToMDMlabFilter {
...
func filterHostsByOS(sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByPolicy(sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByStatus(now time.Time, sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
	switch opt.StatusFilter {
	case mdmlab.StatusNew:
		sql += "AND DATE_ADD(h.created_at, INTERVAL 1 DAY) >= ?"
		params = append(params, now)
	case mdmlab.StatusOnline:
		sql += fmt.Sprintf("AND DATE_ADD(COALESCE(hst.seen_time, h.created_at), INTERVAL LEAST(h.distributed_interval, h.config_tls_refresh) + %d SECOND) > ?", mdmlab.OnlineIntervalBuffer)
		params = append(params, now)
	case mdmlab.StatusOffline:
		sql += fmt.Sprintf("AND DATE_ADD(COALESCE(hst.seen_time, h.created_at), INTERVAL LEAST(h.distributed_interval, h.config_tls_refresh) + %d SECOND) <= ?", mdmlab.OnlineIntervalBuffer)
		params = append(params, now)
	case mdmlab.StatusMIA, mdmlab.StatusMissing:
...
func filterHostsByMacOSSettingsStatus(sql string, opt mdmlab.HostListOptions, params []any) (string, []any, error) {
...
func filterHostsByMacOSDiskEncryptionStatus(sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
...
func (ds *Datastore) filterHostsByOSSettingsStatus(sql string, opt mdmlab.HostListOptions, params []interface{}, isDiskEncryptionEnabled bool) (string, []interface{}, error) {
	if !opt.OSSettingsFilter.IsValid() {
		return sql, params, nil
	}
	...
	// and adds extra where clauses to filter out Windows hosts that are not enrolled to MDMlab MDM
	...
	if isDiskEncryptionEnabled {
		bitlockerStatus = fmt.Sprintf(`
            ...
            ELSE
                ''
            END`,
			ds.whereBitLockerStatus(mdmlab.DiskEncryptionVerified),
			ds.whereBitLockerStatus(mdmlab.DiskEncryptionVerifying),
			ds.whereBitLockerStatus(mdmlab.DiskEncryptionEnforcing),
			ds.whereBitLockerStatus(mdmlab.DiskEncryptionFailed),
		)
	}
...
func (ds *Datastore) filterHostsByOSSettingsDiskEncryptionStatus(sql string, opt mdmlab.HostListOptions, params []interface{}, enableDiskEncryption bool) (string, []interface{}) {
	...
	case mdmlab.DiskEncryptionVerified:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mdmlab.DiskEncryptionVerified)
		}
		subqueryMacOS, subqueryParams = subqueryFileVaultVerified()

	case mdmlab.DiskEncryptionVerifying:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mdmlab.DiskEncryptionVerifying)
		}
	...
	case mdmlab.DiskEncryptionEnforcing:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mdmlab.DiskEncryptionEnforcing)
		}
		subqueryMacOS, subqueryParams = subqueryFileVaultEnforcing()

	case mdmlab.DiskEncryptionFailed:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mdmlab.DiskEncryptionFailed)
		}
...
func filterHostsByMDMBootstrapPackageStatus(sql string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
	...
	case mdmlab.MDMBootstrapPackageFailed:
		subquery += ` AND ncr.status = 'Error'`
	case mdmlab.MDMBootstrapPackagePending:
		subquery += ` AND (ncr.status IS NULL OR (ncr.status != 'Acknowledged' AND ncr.status != 'Error'))`
	case mdmlab.MDMBootstrapPackageInstalled:
...
func filterHostsByVulnerability(sqlstmt string, opt mdmlab.HostListOptions, params []interface{}) (string, []interface{}) {
...
func (ds *Datastore) CountHosts(ctx context.Context, filter mdmlab.TeamFilter, opt mdmlab.HostListOptions) (int, error) {
	sql := `SELECT count(*) `
...
func (ds *Datastore) GenerateHostStatusStatistics(ctx context.Context, filter mdmlab.TeamFilter, now time.Time, platform *string, lowDiskSpace *int) (*mdmlab.HostSummary, error) {
	...
	if platform != nil {
		...
		args = append(args, mdmlab.ExpandPlatform(*platform))
	}
	...
	sqlStatement := fmt.Sprintf(`
			...
			WHERE %s
			LIMIT 1;
		`, mdmlab.OnlineIntervalBuffer, mdmlab.OnlineIntervalBuffer, lowDiskSelect, hostDisksJoin, whereClause)

	stmt, args, err := sqlx.In(sqlStatement, args...)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "generating host statistics statement")
	}
	summary := mdmlab.HostSummary{TeamID: filter.TeamID}
	...
	var platforms []*mdmlab.HostSummaryPlatform
	stmt, args, err = sqlx.In(sqlStatement, args...)
	if err != nil {
...
//
// NOTE: orbit and osquery running as part of mdmlabd on a device are identified
// with the same entry in the hosts table.
type enrolledHostInfo struct {
	// ID is the identifier of the host.
	ID uint
	// LastEnrolledAt is the time the host last enrolled to MDMlab.
...
// serial number lookup if MDMlab MDM is enabled on the server (as we must be
...
func (ds *Datastore) EnrollOrbit(ctx context.Context, isMDMEnabled bool, hostInfo mdmlab.OrbitHostInfo, orbitNodeKey string, teamID *uint) (*mdmlab.Host, error) {
	if orbitNodeKey == "" {
		...
		case err == nil:
			if enrolledHostInfo.NodeKeySet {
				...
				level.Warn(ds.logger).Log(
					"msg", "orbit host with duplicate identifier has enrolled in MDMlab and will overwrite existing host data",
...
// EnrollHost enrolls the osquery agent to MDMlab.
func (ds *Datastore) EnrollHost(ctx context.Context, isMDMEnabled bool, osqueryHostID, hardwareUUID, hardwareSerial, nodeKey string, teamID *uint, cooldown time.Duration) (*mdmlab.Host, error) {
	if osqueryHostID == "" {
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		default:
			...
			if enrolledHostInfo.NodeKeySet {
				...
				level.Warn(ds.logger).Log(
					"msg", "osquery host with duplicate identifier has enrolled in MDMlab and will overwrite existing host data",
...
// For more detail, see: https://github.com/mdmlabdm/mdmlab/issues/15476
func (ds *Datastore) getContextTryStmt(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
	...
	if stmt := ds.loadOrPrepareStmt(ctx, query); stmt != nil {
		...
		//
		// - see https://github.com/mdmlabdm/mdmlab/issues/20781 for an
...
func (ds *Datastore) LoadHostByNodeKey(ctx context.Context, nodeKey string) (*mdmlab.Host, error) {
	query := `
    SELECT
      h.id,
      h.osquery_host_id,
...
// If the node key is invalid it returns a NotFoundError.
func (ds *Datastore) LoadHostByOrbitNodeKey(ctx context.Context, nodeKey string) (*mdmlab.Host, error) {
	query := `
    SELECT
      h.id,
      h.osquery_host_id,
      ...
      h.public_ip,
      h.orbit_node_key,
      IF(hdep.host_id AND ISNULL(hdep.deleted_at), true, false) AS dep_assigned_to_mdmlab,
      hd.encrypted as disk_encryption_enabled,
...
func (ds *Datastore) LoadHostByDeviceAuthToken(ctx context.Context, authToken string, tokenTTL time.Duration) (*mdmlab.Host, error) {
	const query = `
    SELECT
      h.id,
      h.osquery_host_id,
...
`
	...
	if err != nil {
		if IsDuplicate(err) {
			return mdmlab.ConflictError{Message: "auth token conflicts with another host"}
		}
...
func (ds *Datastore) SearchHosts(ctx context.Context, filter mdmlab.TeamFilter, matchQuery string, omit ...uint) ([]*mdmlab.Host, error) {
	query := `SELECT
    h.id,
    h.osquery_host_id,
    h.created_at,
...
func (ds *Datastore) HostIDsByIdentifier(ctx context.Context, filter mdmlab.TeamFilter, hostIdentifiers []string) ([]uint, error) {
...
func (ds *Datastore) ListHostsLiteByUUIDs(ctx context.Context, filter mdmlab.TeamFilter, uuids []string) ([]*mdmlab.Host, error) {
	if len(uuids) == 0 {
		return nil, nil
	}
...
func (ds *Datastore) ListHostsLiteByIDs(ctx context.Context, ids []uint) ([]*mdmlab.Host, error) {
	if len(ids) == 0 {
		return nil, nil
	}
...
func (ds *Datastore) HostByIdentifier(ctx context.Context, identifier string) (*mdmlab.Host, error) {
	stmt := `
    SELECT
      h.id,
      h.osquery_host_id,
...
func (ds *Datastore) SaveHostUsers(ctx context.Context, hostID uint, users []mdmlab.HostUser) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		return saveHostUsersDB(ctx, tx, hostID, users)
	})
}

func saveHostUsersDB(ctx context.Context, tx sqlx.ExtContext, hostID uint, users []mdmlab.HostUser) error {
	currentHostUsers, err := loadHostUsersDB(ctx, tx, hostID)
	if err != nil {
		return err
	}

	keyForUser := func(u *mdmlab.HostUser) string { return fmt.Sprintf("%d\x00%s", u.Uid, u.Username) }
...
func (ds *Datastore) FailingPoliciesCount(ctx context.Context, host *mdmlab.Host) (uint, error) {
	if host.MDMlabPlatform() == "" {
...
func (ds *Datastore) ListPoliciesForHost(ctx context.Context, host *mdmlab.Host) ([]*mdmlab.HostPolicy, error) {
	if host.MDMlabPlatform() == "" {
		// We log to help troubleshooting in case this happens.
		level.Error(ds.logger).Log("err", "unrecognized platform", "hostID", host.ID, "platform", host.Platform) //nolint:errcheck
	}
	...
	var policies []*mdmlab.HostPolicy
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, host.ID, host.ID, host.MDMlabPlatform()); err != nil {
...
func (ds *Datastore) ListHostDeviceMapping(ctx context.Context, id uint) ([]*mdmlab.HostDeviceMapping, error) {
...
func deviceMappingTranslateSourceColumn(hostEmailsTableAlias string) string {
	...
	return fmt.Sprintf(` CASE WHEN %ssource LIKE '%s%%' THEN '%s' ELSE %[1]ssource END `,
		hostEmailsTableAlias, mdmlab.DeviceMappingCustomPrefix, mdmlab.DeviceMappingCustomReplacement)
}

func (ds *Datastore) listHostDeviceMappingDB(ctx context.Context, q sqlx.QueryerContext, hostID uint) ([]*mdmlab.HostDeviceMapping, error) {
	stmt := fmt.Sprintf(`
    SELECT
      id,
      host_id,
...
func (ds *Datastore) ReplaceHostDeviceMapping(ctx context.Context, hid uint, mappings []*mdmlab.HostDeviceMapping, source string) error {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		toIns := make(map[string]*mdmlab.HostDeviceMapping)
		for _, m := range mappings {
			toIns[m.Email+"\n"+m.Source] = m
		}

		var prevMappings []*mdmlab.HostDeviceMapping
...
func (ds *Datastore) SetOrUpdateCustomHostDeviceMapping(ctx context.Context, hostID uint, email, source string) ([]*mdmlab.HostDeviceMapping, error) {
	const (
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		if source == mdmlab.DeviceMappingCustomOverride {
			// must delete any existing custom installer
			if _, err := tx.ExecContext(ctx, delStmt, hostID, mdmlab.DeviceMappingCustomInstaller); err != nil {
		...
		if rowsAffected == 0 {
			...
			if source == mdmlab.DeviceMappingCustomInstaller {
				// custom installer can only insert if there's no custom override
				stmt = insInstallerStmt
				params = append(params, hostID, mdmlab.DeviceMappingCustomOverride)
			}
...
func (ds *Datastore) ReplaceHostBatteries(ctx context.Context, hid uint, mappings []*mdmlab.HostBattery) error {
	for _, m := range mappings {
		if hid != m.HostID {
	...
	keyFn := func(b *mdmlab.HostBattery) string {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// need to be deleted and inserted.
		toIns := make(map[string]*mdmlab.HostBattery)
		...
		var prevMappings []*mdmlab.HostBattery
...
func (ds *Datastore) SetOrUpdateMunkiInfo(ctx context.Context, hostID uint, version string, errors, warnings []string) error {
	...
	msgToID, err := ds.getOrInsertMunkiIssues(ctx, errors, warnings, mdmlab.DefaultMunkiIssuesBatchSize)
	if err != nil {
		return err
	}
...
func (ds *Datastore) SetOrUpdateMDMData(
	...
	return ds.updateOrInsert(
		ctx,
		`UPDATE host_mdm SET enrolled = ?, server_url = ?, installed_from_dep = ?, mdm_id = ?, is_server = ?, mdmlab_enroll_ref = ? WHERE host_id = ?`,
		`INSERT INTO host_mdm (enrolled, server_url, installed_from_dep, mdm_id, is_server, mdmlab_enroll_ref, host_id) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		enrolled, serverURL, installedFromDep, mdmID, isServer, mdmlabEnrollmentRef, hostID,
	)
}
...
func (ds *Datastore) SetOrUpdateHostEmailsFromMdmIdpAccounts(
	...
	var email *string
	if mdmlabEnrollmentRef != "" {
		idp, err := ds.GetMDMIdPAccountByUUID(ctx, mdmlabEnrollmentRef)
		if err != nil {
			return err
		}
		email = &idp.Email
	...
	return ds.updateOrInsert(
		...
		email, hostID, mdmlab.DeviceMappingMDMIdpAccounts,
	)
}
...
func (ds *Datastore) GetHostOrbitInfo(ctx context.Context, hostID uint) (*mdmlab.HostOrbitInfo, error) {
	var orbit mdmlab.HostOrbitInfo
	err := sqlx.GetContext(
...
func (ds *Datastore) GetHostMDM(ctx context.Context, hostID uint) (*mdmlab.HostMDM, error) {
	...
	err := sqlx.GetContext(ctx, ds.reader(ctx), &hmdm, `
		...
		WHERE hm.host_id = ?`, mdmlab.UnknownMDMName, hostID)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (ds *Datastore) GetHostMDMCheckinInfo(ctx context.Context, hostUUID string) (*mdmlab.HostMDMCheckinInfo, error) {
	var hmdm mdmlab.HostMDMCheckinInfo
	...
	err := sqlx.GetContext(ctx, ds.writer(ctx), &hmdm, `
		SELECT
			...
			hda.host_id IS NOT NULL AND hda.deleted_at IS NULL as dep_assigned_to_mdmlab,
...
func (ds *Datastore) GetMDMSolution(ctx context.Context, mdmID uint) (*mdmlab.MDMSolution, error) {
	var solution mdmlab.MDMSolution
...
func (ds *Datastore) GetHostMunkiIssues(ctx context.Context, hostID uint) ([]*mdmlab.HostMunkiIssue, error) {
	var issues []*mdmlab.HostMunkiIssue
...
func (ds *Datastore) GetMunkiIssue(ctx context.Context, munkiIssueID uint) (*mdmlab.MunkiIssue, error) {
	var issue mdmlab.MunkiIssue
...
func (ds *Datastore) AggregatedMunkiVersion(ctx context.Context, teamID *uint) ([]mdmlab.AggregatedMunkiVersion, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var versions []mdmlab.AggregatedMunkiVersion
...
func (ds *Datastore) AggregatedMunkiIssues(ctx context.Context, teamID *uint) ([]mdmlab.AggregatedMunkiIssue, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var result []mdmlab.AggregatedMunkiIssue
...
func (ds *Datastore) AggregatedMDMStatus(ctx context.Context, teamID *uint, platform string) (mdmlab.AggregatedMDMStatus, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var status mdmlab.AggregatedMDMStatus
	...
	if err != nil {
		if err == sql.ErrNoRows {
			// not having stats is not an error
			return mdmlab.AggregatedMDMStatus{}, time.Time{}, nil
		}
		return mdmlab.AggregatedMDMStatus{}, time.Time{}, ctxerr.Wrap(ctx, err, "selecting mdm status")
	}
	if err := json.Unmarshal(statusJson.JsonValue, &status); err != nil {
		return mdmlab.AggregatedMDMStatus{}, time.Time{}, ctxerr.Wrap(ctx, err, "unmarshaling mdm status")
	}
	return status, statusJson.UpdatedAt, nil
}

func platformKey(key mdmlab.AggregatedStatsType, platform string) mdmlab.AggregatedStatsType {
	if platform == "" {
		return key
	}
	return key + "_" + mdmlab.AggregatedStatsType(platform)
}

func (ds *Datastore) AggregatedMDMSolutions(ctx context.Context, teamID *uint, platform string) ([]mdmlab.AggregatedMDMSolutions, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var result []mdmlab.AggregatedMDMSolutions
	var resultJSON struct {
		JsonValue []byte    `db:"json_value"`
		UpdatedAt time.Time `db:"updated_at"`
	}
...
func (ds *Datastore) generateAggregatedMunkiIssues(ctx context.Context, teamID *uint) error {
	...
	var issues []mdmlab.AggregatedMunkiIssue
	query := `
  SELECT
...
func (ds *Datastore) generateAggregatedMDMStatus(ctx context.Context, teamID *uint, platform string) error {
	var (
		id          = uint(0)
		globalStats = true
		status      mdmlab.AggregatedMDMStatus
	)
	// NOTE: ds.UpdateHostTablesOnMDMUnenroll sets installed_from_dep = 0 so DEP hosts are not counted as pending after unenrollment
	query := `SELECT
...
func (ds *Datastore) generateAggregatedMDMSolutions(ctx context.Context, teamID *uint, platform string) error {
	var (
		id          = uint(0)
		globalStats = true
		results     []mdmlab.AggregatedMDMSolutions
		whereAnd    = "WHERE"
	)
	query := `SELECT
				mdms.id,
...
func (ds *Datastore) HostLite(ctx context.Context, id uint) (*mdmlab.Host, error) {
...
func (ds *Datastore) UpdateHostOsqueryIntervals(ctx context.Context, id uint, intervals mdmlab.HostOsqueryIntervals) error {
	sqlStatement := `
		UPDATE hosts SET
			distributed_interval = ?,
			config_tls_refresh = ?,
...
func (ds *Datastore) UpdateHost(ctx context.Context, host *mdmlab.Host) error {
...
func (ds *Datastore) OSVersion(ctx context.Context, osVersionID uint, teamFilter *mdmlab.TeamFilter) (
	*mdmlab.OSVersion, *time.Time, error,
) {
...
func (ds *Datastore) OSVersions(
	...
	var filtered []mdmlab.OSVersion
	for _, os := range counts {
		if (platform == nil || *platform == os.Platform || (*platform == "linux" && mdmlab.IsLinux(os.Platform))) && (name == nil || version == nil || (*name == os.NameOnly && *version == os.Version)) {
			filtered = append(filtered, os)
		}
	}
	counts = filtered
	...
	// aggregate counts by name and version
	byNameVers := make(map[string]mdmlab.OSVersion)
	for _, os := range counts {
		key := fmt.Sprintf("%s %s", os.NameOnly, os.Version)
		val, ok := byNameVers[key]
		if !ok {
			// omit os id
			byNameVers[key] = mdmlab.OSVersion{Name: os.Name, NameOnly: os.NameOnly, Version: os.Version, Platform: os.Platform, HostsCount: os.HostsCount, OSVersionID: os.OSVersionID}
		} else {
			newVal := val
...
func (ds *Datastore) executeOSVersionQuery(ctx context.Context, teamFilter *mdmlab.TeamFilter) (
...
func (ds *Datastore) unmarshalOSVersions(jsonValue *json.RawMessage) ([]mdmlab.OSVersion, error) {
	var versions []mdmlab.OSVersion
	if jsonValue != nil {
...
func (ds *Datastore) UpdateOSVersions(ctx context.Context) error {
	...
	statsByTeamID := make(map[uint][]mdmlab.OSVersion)
	// stats are also aggregated globally per os version
	globalStats := make(map[uint]mdmlab.OSVersion)
	...
	for _, id := range teamIDs {
		if _, ok := statsByTeamID[id]; !ok {
			statsByTeamID[id] = []mdmlab.OSVersion{}
		}
	}
	// same for "no team"
	if _, ok := statsByTeamID[0]; !ok {
		statsByTeamID[0] = []mdmlab.OSVersion{}
	}
	...
	globalArray := make([]mdmlab.OSVersion, 0, len(globalStats))
...
// count. See https://github.com/mdmlabdm/mdmlab/pull/6782#discussion_r926103758.
// TODO: Update once the underlying osquery issue has been resolved.
func (ds *Datastore) ListHostBatteries(ctx context.Context, hid uint) ([]*mdmlab.HostBattery, error) {
	const stmt = `
    SELECT
      host_id,
      serial_number,
...
`
	...
	var batteries []*mdmlab.HostBattery
...
func (ds *Datastore) ListUpcomingHostMaintenanceWindows(ctx context.Context, hid uint) ([]*mdmlab.HostMaintenanceWindow, error) {
	stmt := `
		SELECT
			ce.start_time,
			ce.timezone
	...
	var mws []*mdmlab.HostMaintenanceWindow
...
//
// Notes:
//   - We use `2 * interval`, because of the artificial jitter added to the intervals in MDMlab.
//   - Default values for:
...
func countHostsNotRespondingDB(ctx context.Context, db sqlx.QueryerContext, logger log.Logger, config config.MDMlabConfig) (int, error,
) {
	interval := config.Osquery.DetailUpdateInterval.Seconds()

	// The primary `WHERE` clause is intended to capture where MDMlab hasn't received a distributed write
...
func amountHostsByOrbitVersionDB(ctx context.Context, db sqlx.QueryerContext) ([]mdmlab.HostsCountByOrbitVersion, error) {
	counts := make([]mdmlab.HostsCountByOrbitVersion, 0)
...
func amountHostsByOsqueryVersionDB(ctx context.Context, db sqlx.QueryerContext) ([]mdmlab.HostsCountByOsqueryVersion, error) {
	counts := make([]mdmlab.HostsCountByOsqueryVersion, 0)
...
func numHostsMDMlabDesktopEnabledDB(ctx context.Context, db sqlx.QueryerContext) (int, error) {
	var count int
	const stmt = `
...
func (ds *Datastore) GetMatchingHostSerials(ctx context.Context, serials []string) (map[string]*mdmlab.Host, error) {
	result := map[string]*mdmlab.Host{}
	if len(serials) == 0 {
		return result, nil
	}
...
func (ds *Datastore) GetHostHealth(ctx context.Context, id uint) (*mdmlab.HostHealth, error) {
	sqlStmt := `
	...
	host := &mdmlab.Host{ID: id, Platform: hh.Platform}
	...
	for _, s := range host.Software {
		if len(s.Vulnerabilities) > 0 {
			hh.VulnerableSoftware = append(hh.VulnerableSoftware, mdmlab.HostHealthVulnerableSoftware{
				ID:      s.ID,
				Name:    s.Name,
				Version: s.Version,
			})
	...
	for _, p := range policies {
		if p.Response == "fail" {
			...
			hh.FailingPolicies = append(hh.FailingPolicies, &mdmlab.HostHealthFailingPolicy{
				ID:         p.ID,
				Name:       p.Name,
...
func (ds *Datastore) HostLiteByIdentifier(ctx context.Context, identifier string) (*mdmlab.HostLite, error) {
	return ds.loadHostLite(ctx, nil, &identifier)
}

func (ds *Datastore) HostLiteByID(ctx context.Context, id uint) (*mdmlab.HostLite, error) {
	return ds.loadHostLite(ctx, &id, nil)
}

func (ds *Datastore) loadHostLite(ctx context.Context, id *uint, identifier *string) (*mdmlab.HostLite, error) {
...
// Specified in story https://github.com/mdmlabdm/mdmlab/issues/18115
