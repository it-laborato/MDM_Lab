...
import (
	...
	"github.com/it-laborato/MDM_Lab/server/datastore/mysql/migrations/data"
...
// Datastore is an implementation of mdmlab.Datastore interface backed by
// MySQL
type Datastore struct {
	replica mdmlab.DBReader // so it cannot be used to perform writes
	primary *sqlx.DB
...
func (ds *Datastore) reader(ctx context.Context) mdmlab.DBReader {
...
func newDB(conf *config.MysqlConfig, opts *dbOptions) (*sqlx.DB, error) {
	...
	for attempt := 0; attempt < opts.maxAttempts; attempt++ {
		...
		opts.logger.Log("mysql", fmt.Sprintf(
			"could not connect to db: %v, sleeping %v %v", dbError, interval, conf))
		time.Sleep(interval)
	}
...
func (ds *Datastore) MigrationStatus(ctx context.Context) (*mdmlab.MigrationStatus, error) {
...
// It assumes some deployments may have performed migrations out of order.
func compareMigrations(knownTable goose.Migrations, knownData goose.Migrations, appliedTable, appliedData []int64) *mdmlab.MigrationStatus {
	if len(appliedTable) == 0 && len(appliedData) == 0 {
		return &mdmlab.MigrationStatus{
			StatusCode: mdmlab.NoMigrationsCompleted,
		}
	}
	...
	if equalData && equalTable {
		return &mdmlab.MigrationStatus{
			StatusCode: mdmlab.AllMigrationsCompleted,
		}
	}
	...
	if len(missingTable) > 0 || len(missingData) > 0 {
		return &mdmlab.MigrationStatus{
			StatusCode:   mdmlab.SomeMigrationsCompleted,
	...
	return &mdmlab.MigrationStatus{
		StatusCode:   mdmlab.UnknownMigrations,
...
var (
	knownUnknownTableMigrations = map[int64]struct{}{
		// This migration was introduced incorrectly in mdmlab-v4.4.0 and its
		// timestamp was changed in mdmlab-v4.4.1.
		20210924114500: {},
	}
	knownUnknownDataMigrations = map[int64]struct{}{
		...
		20171212182459: {},
		// Deleted in
		// https://github.com/mdmlabdm/mdmlab/commit/fd61dcab67f341c9e47fb6cb968171650c19a681
		20161223115449: {},
		20170309091824: {},
		20171027173700: {},
		20171212182458: {},
...
func appendListOptionsToSelect(ds *goqu.SelectDataset, opts mdmlab.ListOptions) *goqu.SelectDataset {
	ds = appendOrderByToSelect(ds, opts)
	ds = appendLimitOffsetToSelect(ds, opts)
	return ds
}

func appendOrderByToSelect(ds *goqu.SelectDataset, opts mdmlab.ListOptions) *goqu.SelectDataset {
	if opts.OrderKey != "" {
		...
		for _, key := range ordersKeys {
			...
			if opts.OrderDirection == mdmlab.OrderDescending {
...
func appendLimitOffsetToSelect(ds *goqu.SelectDataset, opts mdmlab.ListOptions) *goqu.SelectDataset {
...
func appendListOptionsToSQL(sql string, opts *mdmlab.ListOptions) (string, []interface{}) {
...
func appendListOptionsWithCursorToSQL(sql string, params []interface{}, opts *mdmlab.ListOptions) (string, []interface{}) {
	...
	if orderKey != "" {
		...
		if opts.TestSecondaryOrderKey != "" {
			direction := "ASC"
			if opts.TestSecondaryOrderDirection == mdmlab.OrderDescending {
				direction = "DESC"
			}
...
func (ds *Datastore) whereFilterHostsByTeams(filter mdmlab.TeamFilter, hostKey string) string {
	if filter.User == nil {
	...
	if filter.User.GlobalRole != nil {
		...
		case mdmlab.RoleAdmin, mdmlab.RoleMaintainer, mdmlab.RoleObserverPlus:
	...
	for _, team := range filter.User.Teams {
		if team.Role == mdmlab.RoleAdmin ||
			team.Role == mdmlab.RoleMaintainer ||
			team.Role == mdmlab.RoleObserverPlus ||
			(team.Role == mdmlab.RoleObserver && filter.IncludeObserver) {
...
// SQL.
func (ds *Datastore) whereFilterGlobalOrTeamIDByTeams(filter mdmlab.TeamFilter, filterTableAlias string) string {
...
func (ds *Datastore) whereFilterGlobalOrTeamIDByTeamsWithSqlFilter(
	filter mdmlab.TeamFilter, globalSqlFilter string, teamIDSqlFilter string,
) string {
	if filter.User == nil {
		// This is likely unintentional, however we would like to return no
		// results rather than panicking or returning some other error. At least
...
func (ds *Datastore) whereFilterTeams(filter mdmlab.TeamFilter, teamKey string) string {
	if filter.User == nil {
		// This is likely unintentional, however we would like to return no
		// results rather than panicking or returning some other error. At least
		// log.
	...
	if filter.User.GlobalRole != nil {
		switch *filter.User.GlobalRole {
		case mdmlab.RoleAdmin, mdmlab.RoleMaintainer, mdmlab.RoleGitOps, mdmlab.RoleObserverPlus:
			return "TRUE"
		case mdmlab.RoleObserver:
			if filter.IncludeObserver {
				return "TRUE"
...
func (ds *Datastore) InnoDBStatus(ctx context.Context) (string, error) {
	...
	if err != nil {
		...
		// This can be set by DB admin like: GRANT PROCESS,SELECT ON *.* TO 'mdmlab'@'%';
...
func (ds *Datastore) ProcessList(ctx context.Context) ([]mdmlab.MySQLProcess, error) {
	var processList []mdmlab.MySQLProcess
	// using the writer even when doing a read to get the data from the main db node
