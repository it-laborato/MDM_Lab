...
func (ds *Datastore) NewHostScriptExecutionRequest(ctx context.Context, request *mdmlab.HostScriptRequestPayload) (*mdmlab.HostScriptResult, error) {
	var res *mdmlab.HostScriptResult
...
func (ds *Datastore) NewInternalScriptExecutionRequest(ctx context.Context, request *mdmlab.HostScriptRequestPayload) (*mdmlab.HostScriptResult, error) {
	var res *mdmlab.HostScriptResult
	var err error
	return res, ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		if request.ScriptContentID == 0 {
...
func newHostScriptExecutionRequest(ctx context.Context, tx sqlx.ExtContext, request *mdmlab.HostScriptRequestPayload, isInternal bool) (*mdmlab.HostScriptResult, error) {
	const (
	...
	var script mdmlab.HostScriptResult
...
func (ds *Datastore) SetHostScriptExecutionResult(ctx context.Context, result *mdmlab.HostScriptResultPayload) (*mdmlab.HostScriptResult,
	string, error,
) {
...
`
	...
	var hsr *mdmlab.HostScriptResult
	var action string
...
func (ds *Datastore) ListPendingHostScriptExecutions(ctx context.Context, hostID uint, onlyShowInternal bool) ([]*mdmlab.HostScriptResult, error) {
	internalWhere := ""
	if onlyShowInternal {
  ...
  ORDER BY
	...
	var results []*mdmlab.HostScriptResult
...
func (ds *Datastore) GetHostScriptExecutionResult(ctx context.Context, execID string) (*mdmlab.HostScriptResult, error) {
	return ds.getHostScriptExecutionResultDB(ctx, ds.reader(ctx), execID)
}

func (ds *Datastore) getHostScriptExecutionResultDB(ctx context.Context, q sqlx.QueryerContext, execID string) (*mdmlab.HostScriptResult, error) {
	const getStmt = `
  SELECT
    hsr.id,
    hsr.host_id,
...
`
	...
	var result mdmlab.HostScriptResult
...
func (ds *Datastore) NewScript(ctx context.Context, script *mdmlab.Script) (*mdmlab.Script, error) {
	var res sql.Result
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func insertScript(ctx context.Context, tx sqlx.ExtContext, script *mdmlab.Script, scriptContentsID uint) (sql.Result, error) {
	const insertStmt = `
INSERT INTO
  scripts (
...
func (ds *Datastore) Script(ctx context.Context, id uint) (*mdmlab.Script, error) {
	return ds.getScriptDB(ctx, ds.reader(ctx), id)
}

func (ds *Datastore) getScriptDB(ctx context.Context, q sqlx.QueryerContext, id uint) (*mdmlab.Script, error) {
	const getStmt = `
SELECT
  id,
  team_id,
...
var errDeleteScriptWithAssociatedPolicy = &mdmlab.ConflictError{Message: "Couldn't delete. Policy automation uses this script. Please remove this script from associated policy automations and try again."}
...
func (ds *Datastore) ListScripts(ctx context.Context, teamID *uint, opt mdmlab.ListOptions) ([]*mdmlab.Script, *mdmlab.PaginationMetadata, error) {
	var scripts []*mdmlab.Script
...
func (ds *Datastore) GetHostScriptDetails(ctx context.Context, hostID uint, teamID *uint, opt mdmlab.ListOptions, hostPlatform string) ([]*mdmlab.HostScriptDetail, *mdmlab.PaginationMetadata, error) {
	var globalOrTeamID uint
	if teamID != nil {
		globalOrTeamID = *teamID
	}
	...
	case hostPlatform == "windows":
		...
		extension = `%.ps1`
	case mdmlab.IsUnixLike(hostPlatform):
...
WHERE
	...
	results := make([]*mdmlab.HostScriptDetail, 0, len(rows))
	for _, r := range rows {
		results = append(results, mdmlab.NewHostScriptDetail(hostID, r.ScriptID, r.Name, r.ExecutionID, r.ExecutedAt, r.ExitCode, r.HSRID))
	}

	return results, metaData, nil
}

func (ds *Datastore) BatchSetScripts(ctx context.Context, tmID *uint, scripts []*mdmlab.Script) ([]mdmlab.ScriptResponse, error) {
...
`
	...
	// of processing
	incomingScripts := make(map[string]*mdmlab.Script, len(scripts))
	for i, p := range scripts {
		incomingNames[i] = p.Name
		incomingScripts[p.Name] = p
	}

	var insertedScripts []mdmlab.ScriptResponse
	if err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var existingScripts []*mdmlab.Script
...
func (ds *Datastore) GetHostLockWipeStatus(ctx context.Context, host *mdmlab.Host) (*mdmlab.HostLockWipeStatus, error) {
	const stmt = `
		SELECT
			lock_ref,
			wipe_ref,
...
`
	...
	var mdmActions struct {
		...
		MDMlabPlatform string  `db:"mdmlab_platform"`
	}
	mdmlabPlatform := host.MDMlabPlatform()
	status := &mdmlab.HostLockWipeStatus{
		HostMDMlabPlatform: mdmlabPlatform,
	}
	...
	// if we have a mdmlab platform stored in host_mdm_actions, use it instead of
	// the host.MDMlabPlatform() because the platform can be overwritten with an
	// unknown OS name when a Wipe gets executed.
	if mdmActions.MDMlabPlatform != "" {
		mdmlabPlatform = mdmActions.MDMlabPlatform
		status.HostMDMlabPlatform = mdmlabPlatform
	}

	switch mdmlabPlatform {
...
func (ds *Datastore) getHostMDMWindowsCommand(ctx context.Context, cmdUUID, hostUUID string) (*mdmlab.MDMCommand, *mdmlab.MDMCommandResult, error) {
	...
	// that specific host.
	var cmdRes *mdmlab.MDMCommandResult
...
func (ds *Datastore) getHostMDMAppleCommand(ctx context.Context, cmdUUID, hostUUID string) (*mdmlab.MDMCommand, *mdmlab.MDMCommandResult, error) {
	cmd, err := ds.getMDMCommand(ctx, ds.reader(ctx), cmdUUID)
	if err != nil {
	...
	for _, r := range cmdResults {
		...
		if r.Status == mdmlab.MDMAppleStatusAcknowledged || r.Status == mdmlab.MDMAppleStatusError || r.Status == mdmlab.MDMAppleStatusCommandFormatError {
			cmdRes = r
			break
		}
	}
...
func (ds *Datastore) LockHostViaScript(ctx context.Context, request *mdmlab.HostScriptRequestPayload, hostMDMlabPlatform string) error {
	var res *mdmlab.HostScriptResult
...
// host_mdm_actions in a single transaction.
func (ds *Datastore) UnlockHostViaScript(ctx context.Context, request *mdmlab.HostScriptRequestPayload, hostMDMlabPlatform string) error {
	var res *mdmlab.HostScriptResult
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) WipeHostViaScript(ctx context.Context, request *mdmlab.HostScriptRequestPayload, hostMDMlabPlatform string) error {
	var res *mdmlab.HostScriptResult
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) UnlockHostManually(ctx context.Context, hostID uint, hostMDMlabPlatform string, ts time.Time) error {
	const stmt = `
	INSERT INTO host_mdm_actions
	(
		host_id,
	...
	_, err := ds.writer(ctx).ExecContext(ctx, stmt, hostID, unlockRef, hostMDMlabPlatform)
...
WHERE
  ...
  AND NOT EXISTS (
    SELECT 1 FROM mdmlab_library_apps fla
