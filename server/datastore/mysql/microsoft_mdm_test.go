...
func testMDMWindowsDiskEncryption(t *testing.T, ds *Datastore) {
	...
	checkBitLockerSummary := func(t *testing.T, teamID *uint, expected mdmlab.MDMWindowsBitLockerSummary) {
	...
	checkMDMProfilesSummary := func(t *testing.T, teamID *uint, expected mdmlab.MDMProfilesSummary) {
	...
	checkListHostsFilterOSSettings := func(t *testing.T, teamID *uint, status mdmlab.OSSettingsStatus, expectedIDs []uint) {
		gotHosts, err := ds.ListHosts(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{TeamFilter: teamID, OSSettingsFilter: status})
		require.NoError(t, err)
	...
	checkListHostsFilterDiskEncryption := func(t *testing.T, teamID *uint, status mdmlab.DiskEncryptionStatus, expectedIDs []uint) {
		gotHosts, err := ds.ListHosts(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{TeamFilter: teamID, OSSettingsDiskEncryptionFilter: status})
		require.NoError(t, err)
		...
		count, err := ds.CountHosts(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{TeamFilter: teamID, OSSettingsDiskEncryptionFilter: status})
		require.NoError(t, err)
		require.Equal(t, len(expectedIDs), count, fmt.Sprintf("status: %s", status))
	}

	checkHostBitLockerStatus := func(t *testing.T, expected mdmlab.DiskEncryptionStatus, hostIDs []uint) {
	...
	type hostIDsByDEStatus map[mdmlab.DiskEncryptionStatus][]uint
	type hostIDsByProfileStatus map[mdmlab.MDMDeliveryStatus][]uint
	...
	expectedProfilesFromDE := func(expectedDE hostIDsByDEStatus) hostIDsByProfileStatus {
		...
		expectedProfiles[mdmlab.MDMDeliveryPending] = []uint{}
		for status, hostIDs := range expectedDE {
			switch status {
			case mdmlab.DiskEncryptionVerified:
				expectedProfiles[mdmlab.MDMDeliveryVerified] = hostIDs
			case mdmlab.DiskEncryptionVerifying:
				expectedProfiles[mdmlab.MDMDeliveryVerifying] = hostIDs
			case mdmlab.DiskEncryptionFailed:
				expectedProfiles[mdmlab.MDMDeliveryFailed] = hostIDs
			case mdmlab.DiskEncryptionEnforcing, mdmlab.DiskEncryptionRemovingEnforcement, mdmlab.DiskEncryptionActionRequired:
				expectedProfiles[mdmlab.MDMDeliveryPending] = append(expectedProfiles[mdmlab.MDMDeliveryPending], hostIDs...)
			}
		}
	...
	checkExpected := func(t *testing.T, teamID *uint, expectedDE hostIDsByDEStatus, expectedProfiles ...hostIDsByProfileStatus) {
		...
		for _, status := range []mdmlab.DiskEncryptionStatus{
		...
		checkBitLockerSummary(t, teamID, mdmlab.MDMWindowsBitLockerSummary{
			Verified:            uint(len(expectedDE[mdmlab.DiskEncryptionVerified])),
			Verifying:           uint(len(expectedDE[mdmlab.DiskEncryptionVerifying])),
			Failed:              uint(len(expectedDE[mdmlab.DiskEncryptionFailed])),
			Enforcing:           uint(len(expectedDE[mdmlab.DiskEncryptionEnforcing])),
			RemovingEnforcement: uint(len(expectedDE[mdmlab.DiskEncryptionRemovingEnforcement])),
			ActionRequired:      uint(len(expectedDE[mdmlab.DiskEncryptionActionRequired])),
		})

		checkMDMProfilesSummary(t, teamID, mdmlab.MDMProfilesSummary{
			Pending:   uint(len(ep[mdmlab.MDMDeliveryPending])),
			Failed:    uint(len(ep[mdmlab.MDMDeliveryFailed])),
			Verifying: uint(len(ep[mdmlab.MDMDeliveryVerifying])),
			Verified:  uint(len(ep[mdmlab.MDMDeliveryVerified])),
		})

		checkListHostsFilterOSSettings(t, teamID, mdmlab.OSSettingsVerified, ep[mdmlab.MDMDeliveryVerified])
		checkListHostsFilterOSSettings(t, teamID, mdmlab.OSSettingsVerifying, ep[mdmlab.MDMDeliveryVerifying])
		checkListHostsFilterOSSettings(t, teamID, mdmlab.OSSettingsFailed, ep[mdmlab.MDMDeliveryFailed])
		checkListHostsFilterOSSettings(t, teamID, mdmlab.OSSettingsPending, ep[mdmlab.MDMDeliveryPending])
	}
	...
	upsertHostProfileStatus := func(t *testing.T, hostUUID string, profUUID string, status mdmlab.MDMDeliveryStatus) {
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
	...
	for i := 0; i < 10; i++ {
		...
		require.NoError(t, ds.SetOrUpdateMDMData(ctx, h.ID, false, true, "https://example.com", false, mdmlab.WellKnownMDMMDMlab, ""))
	...
	t.Run("Disk encryption enabled", func(t *testing.T) {
		...
		t.Run("Bitlocker enforcing-verifying-verified", func(t *testing.T) {
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
			})
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mdmlab.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
			})
			...
			cases := []struct {
				...
				expectedWithinGracePeriod  mdmlab.DiskEncryptionStatus
				expectedOutsideGracePeriod mdmlab.DiskEncryptionStatus
			}{
				{
					name:                       "encrypted reported after key",
					hostDisksEncrypted:         true,
					reportedAfterKey:           true,
					expectedWithinGracePeriod:  mdmlab.DiskEncryptionVerified,
					expectedOutsideGracePeriod: mdmlab.DiskEncryptionVerified,
				},
				{
					name:                       "encrypted reported before key",
					hostDisksEncrypted:         true,
					reportedAfterKey:           false,
					expectedWithinGracePeriod:  mdmlab.DiskEncryptionVerifying,
					expectedOutsideGracePeriod: mdmlab.DiskEncryptionVerifying,
				},
				{
					name:                       "not encrypted reported before key",
					hostDisksEncrypted:         false,
					reportedAfterKey:           false,
					expectedWithinGracePeriod:  mdmlab.DiskEncryptionEnforcing,
					expectedOutsideGracePeriod: mdmlab.DiskEncryptionEnforcing,
				},
				{
			...
			for _, c := range cases {
				t.Run(c.name, func(t *testing.T) {
					...
					t.Run("within grace period", func(t *testing.T) {
						...
						if c.expectedWithinGracePeriod == mdmlab.DiskEncryptionEnforcing {
							expected[mdmlab.DiskEncryptionEnforcing] = append([]uint{testHostID}, otherWindowsHostIDs...)
						} else {
							expected[c.expectedWithinGracePeriod] = []uint{testHostID}
							expected[mdmlab.DiskEncryptionEnforcing] = otherWindowsHostIDs
						}
					...
					t.Run("outside grace period", func(t *testing.T) {
						expected := make(hostIDsByDEStatus)
						if c.expectedOutsideGracePeriod == mdmlab.DiskEncryptionEnforcing {
							expected[mdmlab.DiskEncryptionEnforcing] = append([]uint{testHostID}, otherWindowsHostIDs...)
						} else {
		...
		t.Run("BitLocker failed status", func(t *testing.T) {
			...
			expected := hostIDsByDEStatus{
				mdmlab.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mdmlab.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[2].ID, hosts[3].ID, hosts[4].ID},
			}

			checkExpected(t, nil, expected)

			// bitlocker failed status determines MDM aggregate status (profiles status is ignored)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mdmlab.MDMDeliveryFailed)
			checkExpected(t, nil, expected)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mdmlab.MDMDeliveryPending)
			checkExpected(t, nil, expected)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mdmlab.MDMDeliveryVerifying)
			checkExpected(t, nil, expected)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mdmlab.MDMDeliveryVerified)
			checkExpected(t, nil, expected)

			// profiles failed status determines MDM aggregate status (bitlocker status is ignored)
			upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", mdmlab.MDMDeliveryFailed)
			expectedProfiles := expectedProfilesFromDE(expected)
			expectedProfiles[mdmlab.MDMDeliveryFailed] = append(expectedProfiles[mdmlab.MDMDeliveryFailed], hosts[0].ID)
			expectedProfiles[mdmlab.MDMDeliveryVerified] = []uint{}
		...
		t.Run("BitLocker team filtering", func(t *testing.T) {
			// Test team filtering
			team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team"})
			require.NoError(t, err)
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mdmlab.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mdmlab.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[3].ID, hosts[4].ID}, // hosts[2] is no longer included in the no team summary
			})
			...
			checkExpected(t, &team.ID, hostIDsByDEStatus{
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[2].ID}, // disk encryption is enabled for team so hosts[2] is counted
			})
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mdmlab.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mdmlab.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[3].ID, hosts[4].ID},
			})
		})
		...
		t.Run("BitLocker Windows server excluded", func(t *testing.T) {
			require.NoError(t, ds.SetOrUpdateMDMData(ctx,
				...
				true, "https://example.com", false, mdmlab.WellKnownMDMMDMlab, ""))

			// Check Windows servers not counted
			checkExpected(t, nil, hostIDsByDEStatus{
				mdmlab.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mdmlab.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mdmlab.DiskEncryptionEnforcing: []uint{hosts[4].ID}, // hosts[3] is not counted
			})
		})
		...
		t.Run("OS settings filters include Windows and macOS hosts", func(t *testing.T) {
			...
			checkBitLockerSummary(t, nil, mdmlab.MDMWindowsBitLockerSummary{
				Verified:            1,
				Verifying:           0,
				Failed:              1,
				Enforcing:           1,
			...
			checkListHostsFilterDiskEncryption(t, nil, mdmlab.DiskEncryptionFailed, []uint{hosts[1].ID, hosts[5].ID})
			checkListHostsFilterOSSettings(t, nil, mdmlab.OSSettingsFailed, []uint{hosts[1].ID, hosts[5].ID})
		})
	})
}
...
func testMDMWindowsProfilesSummary(t *testing.T, ds *Datastore) {
	...
	checkListHostsFilterOSSettings := func(t *testing.T, teamID *uint, status mdmlab.OSSettingsStatus, expectedIDs []uint) {
		...
		count, err := ds.CountHosts(ctx, mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.HostListOptions{TeamFilter: teamID, OSSettingsFilter: status})
		require.NoError(t, err)
	...
	upsertHostProfileStatus := func(t *testing.T, hostUUID string, profUUID string, status *mdmlab.MDMDeliveryStatus) {
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
			...
			_, err = q.ExecContext(ctx, stmt, mdmlab.MDMOperationTypeInstall, hostUUID, profUUID)
			return err
		})
	}
	...
	t.Run("profiles summary accounts for bitlocker status", func(t *testing.T) {
		t.Run("bitlocker disabled", func(t *testing.T) {
			...
			upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mdmlab.MDMDeliveryPending)
			expected[mdmlab.MDMDeliveryPending] = []uint{hosts[0].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", &mdmlab.MDMDeliveryFailed)
			expected[mdmlab.MDMDeliveryFailed] = []uint{hosts[1].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[2].UUID, "some-windows-profile", &mdmlab.MDMDeliveryVerifying)
			expected[mdmlab.MDMDeliveryVerifying] = []uint{hosts[2].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[3].UUID, "some-windows-profile", &mdmlab.MDMDeliveryVerified)
			expected[mdmlab.MDMDeliveryVerified] = []uint{hosts[3].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[4].UUID, "some-windows-profile", nil)
			// nil status is treated as pending
			expected[mdmlab.MDMDeliveryPending] = append(expected[mdmlab.MDMDeliveryPending], hosts[4].ID)
			checkExpected(t, nil, expected)
		...
		t.Run("bitlocker enabled", func(t *testing.T) {
			...
			t.Run("bitlocker pending", func(t *testing.T) {
				expected := hostIDsByProfileStatus{
					mdmlab.MDMDeliveryPending: []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				}
				...
				expected[mdmlab.MDMDeliveryFailed] = []uint{hosts[1].ID}
				expected[mdmlab.MDMDeliveryPending] = []uint{hosts[0].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)
			...
			t.Run("bitlocker verifying", func(t *testing.T) {
				...
				// no profiles
				expected[mdmlab.MDMDeliveryVerifying] = []uint{hosts[0].ID}
				expected[mdmlab.MDMDeliveryPending] = []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mdmlab.MDMDeliveryFailed)
				// status for hosts[0] now failed because any failed status takes precedence
				expected = hostIDsByProfileStatus{
					mdmlab.MDMDeliveryFailed:    []uint{hosts[0].ID},
					mdmlab.MDMDeliveryVerifying: []uint{},
					mdmlab.MDMDeliveryPending:   []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mdmlab.MDMDeliveryPending)
				// status for hosts[0] now pending because profiles status pendiing takes precedence over bitlocker status verifying
				expected[mdmlab.MDMDeliveryFailed] = []uint{}
				expected[mdmlab.MDMDeliveryPending] = []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mdmlab.MDMDeliveryVerifying)
				// status for hosts[0] now verifying because both profiles status and bitlocker status are verifying
				expected[mdmlab.MDMDeliveryPending] = []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				expected[mdmlab.MDMDeliveryVerifying] = []uint{hosts[0].ID}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mdmlab.MDMDeliveryVerified)
				...
				expected[mdmlab.MDMDeliveryVerifying] = []uint{}
				expected[mdmlab.MDMDeliveryPending] = []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)
			...
			t.Run("bitlocker verified", func(t *testing.T) {
				...
				checkExpected(t, nil, hostIDsByProfileStatus{
					mdmlab.MDMDeliveryVerified: []uint{hosts[0].ID},
					mdmlab.MDMDeliveryPending:  []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				})
				...
				expected = hostIDsByProfileStatus{
					mdmlab.MDMDeliveryFailed:   []uint{hosts[0].ID},
					mdmlab.MDMDeliveryVerified: []uint{},
					mdmlab.MDMDeliveryPending:  []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				}
				checkExpected(t, nil, expected)
				...
				expected[mdmlab.MDMDeliveryPending] = []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				expected[mdmlab.MDMDeliveryVerified] = []uint{hosts[0].ID}
				expected[mdmlab.MDMDeliveryVerifying] = []uint{}
				checkExpected(t, nil, expected)
	...
	t.Run("profiles summary accounts for host profiles with mixed statuses", func(t *testing.T) {
		for i := 0; i < 5; i++ {
			...
			upsertHostProfileStatus(t, hosts[1].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mdmlab.MDMDeliveryPending)
			// upsert five profiles for hosts[2] with verifying statuses
			upsertHostProfileStatus(t, hosts[2].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mdmlab.MDMDeliveryVerifying)
			// upsert five profiles for hosts[3] with verified statuses
			upsertHostProfileStatus(t, hosts[3].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mdmlab.MDMDeliveryVerified)
			// upsert five profiles for hosts[4] with failed statuses
			upsertHostProfileStatus(t, hosts[4].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mdmlab.MDMDeliveryFailed)
		}

		expected := hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID},
			mdmlab.MDMDeliveryVerifying: []uint{hosts[2].ID},
			mdmlab.MDMDeliveryVerified:  []uint{hosts[3].ID},
			mdmlab.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// add some other windows hosts that won't be be assigned any profiles
		otherHosts := make([]*mdmlab.Host, 0, 5)
		for i := 0; i < 5; i++ {
			u := uuid.New().String()
			h, err := ds.NewHost(ctx, &mdmlab.Host{
				DetailUpdatedAt: time.Now(),
		...
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mdmlab.MDMDeliveryFailed)
		}
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{},
			mdmlab.MDMDeliveryVerifying: []uint{},
			mdmlab.MDMDeliveryVerified:  []uint{},
			mdmlab.MDMDeliveryFailed:    []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// upsert some-profile-0 to pending status for hosts[0:4]
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mdmlab.MDMDeliveryPending)
		}
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID},
			mdmlab.MDMDeliveryVerifying: []uint{},
			mdmlab.MDMDeliveryVerified:  []uint{},
			mdmlab.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		...
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mdmlab.MDMDeliveryVerifying)
		}
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID},
			mdmlab.MDMDeliveryVerifying: []uint{hosts[2].ID, hosts[3].ID},
			mdmlab.MDMDeliveryVerified:  []uint{},
			mdmlab.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)
		...
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mdmlab.MDMDeliveryVerified)
		}
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID},
			mdmlab.MDMDeliveryVerifying: []uint{hosts[2].ID},
		...
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, otherHosts[0].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mdmlab.MDMDeliveryVerifying: []uint{},
			mdmlab.MDMDeliveryVerified:  []uint{},
			mdmlab.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		...
		// create a new team
		t1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: uuid.NewString()})
		require.NoError(t, err)
		require.NotNil(t, t1)
		...
		expectedTeam1 := hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending:   []uint{hosts[1].ID},
			mdmlab.MDMDeliveryVerifying: []uint{hosts[2].ID},
		}
		checkExpected(t, &t1.ID, expectedTeam1)
		// hosts[1:2] are not counted for no team
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending: []uint{hosts[0].ID, hosts[3].ID, otherHosts[0].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mdmlab.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// report otherHosts[0] as a server
		require.NoError(t, ds.SetOrUpdateMDMData(ctx, otherHosts[0].ID, true, true, "https://example.com", false, mdmlab.WellKnownMDMMDMlab, ""))
		// otherHosts[0] is no longer counted
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending: []uint{hosts[0].ID, hosts[3].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mdmlab.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// report hosts[0] as a server
		require.NoError(t, ds.SetOrUpdateMDMData(ctx, hosts[0].ID, true, true, "https://example.com", false, mdmlab.WellKnownMDMMDMlab, ""))
		// hosts[0] is no longer counted
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending: []uint{hosts[3].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mdmlab.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)
		...
		expected = hostIDsByProfileStatus{
			mdmlab.MDMDeliveryPending: []uint{otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mdmlab.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)
...
func testMDMWindowsInsertCommandForHosts(t *testing.T, ds *Datastore) {
	...
	d1 := &mdmlab.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
	...
	d2 := &mdmlab.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
	...
	d3 := &mdmlab.MDMWindowsEnrolledDevice{
...
func windowsEnroll(t *testing.T, ds mdmlab.Datastore, h *mdmlab.Host) string {
	ctx := context.Background()
	d1 := &mdmlab.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
...
func testMDMWindowsProfileManagement(t *testing.T, ds *Datastore) {
	...
	// a windows host that's not MDM enrolled into MDMlab shouldn't
	...
	profilesMatch := func(t *testing.T, want []string, profs []*mdmlab.MDMWindowsProfilePayload) {
		got := []string{}
	...
	err = ds.BulkUpsertMDMWindowsHostProfiles(
		ctx, []*mdmlab.MDMWindowsBulkUpsertHostProfilePayload{
			{
...
func testBulkOperationsMDMWindowsHostProfiles(t *testing.T, ds *Datastore) {
	...
	getAllHostProfiles := func() []*mdmlab.MDMWindowsProfilePayload {
		var hostProfiles []*mdmlab.MDMWindowsProfilePayload
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
	...
	err := ds.BulkUpsertMDMWindowsHostProfiles(ctx, []*mdmlab.MDMWindowsBulkUpsertHostProfilePayload{})
	require.NoError(t, err)
	...
	for i, p := range hostsProfs {
		...
		require.Equal(t, mdmlab.MDMOperationTypeInstall, p.OperationType)
		require.Equal(t, &mdmlab.MDMDeliveryVerifying, p.Status)
	}
	...
	for i, p := range hostsProfs {
		require.Equal(t, profiles[i], p.ProfileUUID)
		require.Equal(t, mdmlab.MDMOperationTypeInstall, p.OperationType)
		require.Equal(t, &mdmlab.MDMDeliveryVerified, p.Status)
	}

	// empty payload
	err = ds.BulkDeleteMDMWindowsHostsConfigProfiles(ctx, []*mdmlab.MDMWindowsProfilePayload{})
	require.NoError(t, err)
	hostsProfs = getAllHostProfiles()
	require.Len(t, hostsProfs, 5)
...
func testMDMWindowsConfigProfiles(t *testing.T, ds *Datastore) {
	...
	profA, err := ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "a", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	require.NotEmpty(t, profA.ProfileUUID)
	profB, err := ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "b", TeamID: ptr.Uint(0), SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	profATm, err := ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "a", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "b", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	// create a duplicate of Apple for no-team
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "c", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	...
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "b", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	...
	_, err = ds.NewMDMWindowsConfigProfile(
		ctx,
		mdmlab.MDMWindowsConfigProfile{
			Name:             "fake-labels",
			TeamID:           nil,
			SyncML:           []byte("<Replace></Replace>"),
			LabelsIncludeAll: []mdmlab.ConfigurationProfileLabel{{LabelName: "foo", LabelID: 1}},
		})
	...
	profWithLabel, err := ds.NewMDMWindowsConfigProfile(
		...
		mdmlab.MDMWindowsConfigProfile{
			...
			TeamID:           nil,
			SyncML:           []byte("<Replace></Replace>"),
			LabelsIncludeAll: []mdmlab.ConfigurationProfileLabel{{LabelName: label.Name, LabelID: label.ID}},
		})
	require.NoError(t, err)
...
func testSetOrReplaceMDMWindowsConfigProfile(t *testing.T, ds *Datastore) {
	...
	getProfileByTeamAndName := func(tmID *uint, name string) *mdmlab.MDMWindowsConfigProfile {
	...
	profs1 := expectWindowsProfiles(t, ds, nil, []*mdmlab.MDMWindowsConfigProfile{&cp1})
	...
	profs2 := expectWindowsProfiles(t, ds, nil, []*mdmlab.MDMWindowsConfigProfile{&cp2})
	...
	expectWindowsProfiles(t, ds, nil, []*mdmlab.MDMWindowsConfigProfile{&cp2})
	...
	profs3 := expectWindowsProfiles(t, ds, ptr.Uint(2), []*mdmlab.MDMWindowsConfigProfile{&cp4})
	...
	expectWindowsProfiles(t, ds, nil, []*mdmlab.MDMWindowsConfigProfile{&cp2, &cp5})
	...
	expectWindowsProfiles(t, ds, nil, []*mdmlab.MDMWindowsConfigProfile{&cp2, &cp6})
}
...
func testMDMWindowsProfileLabels(t *testing.T, ds *Datastore) {
	...
	// "include-any" labels
	l1, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	l2, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	l3, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	// include-all labels
	l4, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	l5, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	// exclude-all labels
	l6, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	l7, err := ds.NewLabel(ctx, &mdmlab.Label{
	...
	require.NoError(t, err)
	require.ElementsMatch(t, []*mdmlab.MDMWindowsProfilePayload{
...
func expectWindowsProfiles(
	...
	ds *Datastore,
	tmID *uint,
	want []*mdmlab.MDMWindowsConfigProfile,
) map[string]string {
	if tmID == nil {
		tmID = ptr.Uint(0)
	}

	var got []*mdmlab.MDMWindowsConfigProfile
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		ctx := context.Background()
	...
	wantMap := make(map[string]*mdmlab.MDMWindowsConfigProfile, len(want))
	for _, cp := range want {
		wantMap[cp.Name] = cp
	}
...
func testBatchSetMDMWindowsProfiles(t *testing.T, ds *Datastore) {
	...
	applyAndExpect := func(newSet []*mdmlab.MDMWindowsConfigProfile, tmID *uint, want []*mdmlab.MDMWindowsConfigProfile,
		wantUpdated bool,
	) map[string]string {
	...
	withTeamID := func(p *mdmlab.MDMWindowsConfigProfile, tmID uint) *mdmlab.MDMWindowsConfigProfile {
		p.TeamID = &tmID
		return p
	}
	withUploadedAt := func(p *mdmlab.MDMWindowsConfigProfile, ua time.Time) *mdmlab.MDMWindowsConfigProfile {
		p.UploadedAt = ua
		return p
	}
	...
	mTm1 := applyAndExpect([]*mdmlab.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "N1", "l1"),
	}, ptr.Uint(1), []*mdmlab.MDMWindowsConfigProfile{
	...
	applyAndExpect([]*mdmlab.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "N1", "l1"),
	}, nil, []*mdmlab.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "N1", "l1"),
	}, true)
	...
	mTm1b := applyAndExpect([]*mdmlab.MDMWindowsConfigProfile{
	...
	// for tm1
	mTm1c := applyAndExpect([]*mdmlab.MDMWindowsConfigProfile{
...
func windowsConfigProfileForTest(t *testing.T, name, locURI string, labels ...*mdmlab.Label) *mdmlab.MDMWindowsConfigProfile {
	prof := &mdmlab.MDMWindowsConfigProfile{
		Name: name,
	...
	for _, lbl := range labels {
		...
		case strings.HasPrefix(lbl.Name, "exclude-"):
			prof.LabelsExcludeAny = append(prof.LabelsExcludeAny, mdmlab.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		case strings.HasPrefix(lbl.Name, "include-any-"):
			prof.LabelsIncludeAny = append(prof.LabelsIncludeAny, mdmlab.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		default:
			prof.LabelsIncludeAll = append(prof.LabelsIncludeAll, mdmlab.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		}
	}
...
func createResponseAsEnrichedSyncML(t *testing.T, enrolledDevice *mdmlab.MDMWindowsEnrolledDevice, atomicCommandUUID string,
	replaceCommandUUID string) mdmlab.EnrichedSyncML {
...
`, enrolledDevice.MDMDeviceID, atomicCommandUUID, replaceCommandUUID)
	syncML := &mdmlab.SyncML{}
	err := xml.Unmarshal([]byte(rawResponse), syncML)
	require.NoError(t, err)
	syncML.Raw = []byte(rawResponse)
	enrichedSyncML := mdmlab.NewEnrichedSyncML(syncML)
	return enrichedSyncML
}

func createEnrolledDevice(t *testing.T, ds *Datastore) *mdmlab.MDMWindowsEnrolledDevice {
	enrolledDevice := &mdmlab.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
