...
import (
	...
	nanodep_mysql "github.com:it-laborato/MDM_Lab/server/mdm/nanodep/storage/mysql"
	"github.com:it-laborato/MDM_Lab/server/mdm/nanomdm/mdm"
	nanomdm_mysql "github.com:it-laborato/MDM_Lab/server/mdm/nanomdm/storage/mysql"
...
//
// Always returns "0" as stale token because mdmlab.Datastore always returns a valid push certificate.
...
func (s *NanoMDMStorage) StorePushCert(ctx context.Context, pemCert, pemKey []byte) error {
	return errors.New("please use mdmlab.Datastore to manage MDM assets")
}
...
//   - The method performs mdmlab-specific actions after the command is enqueued.
func (s *NanoMDMStorage) EnqueueDeviceLockCommand(
	...
	return common_mysql.WithRetryTxx(ctx, s.db, func(tx sqlx.ExtContext) error {
		...
		if _, err := tx.ExecContext(ctx, stmt, host.ID, cmd.CommandUUID, pin, host.MDMlabPlatform()); err != nil {
...
func (s *NanoMDMStorage) EnqueueDeviceWipeCommand(ctx context.Context, host *mdmlab.Host, cmd *mdm.Command) error {
	return common_mysql.WithRetryTxx(ctx, s.db, func(tx sqlx.ExtContext) error {
		if err := enqueueCommandDB(ctx, tx, []string{host.UUID}, cmd); err != nil {
			return err
		}
...
func (s *NanoMDMStorage) GetAllMDMConfigAssetsByName(ctx context.Context, assetNames []mdmlab.MDMAssetName,
...
func (s *NanoMDMStorage) GetABMTokenByOrgName(ctx context.Context, orgName string) (*mdmlab.ABMToken, error) {
...
// DEP auth tokens from the tables managed by MDMlab.
...
// https://github.com/mdmlabdm/mdmlab/issues/21177 for more details.
