...
func getPercentileQuery(aggregate mdmlab.AggregatedStatsType, time string, percentile string) string {
	switch aggregate { //nolint:gocritic // ignore singleCaseSwitch
	case mdmlab.AggregatedStatsTypeScheduledQuery:
...
func setP50AndP95Map(
	ctx context.Context, tx sqlx.QueryerContext, aggregate mdmlab.AggregatedStatsType, time string, id uint, statsMap map[string]interface{},
) error {
	var p50, p95 float64
...
func (ds *Datastore) UpdateQueryAggregatedStats(ctx context.Context) error {
	err := walkIdsInTable(
		ctx, ds.reader(ctx), "queries", func(queryID uint) error {
			return ds.CalculateAggregatedPerfStatsPercentiles(ctx, mdmlab.AggregatedStatsTypeScheduledQuery, queryID)
		})
	if err != nil {
...
func (ds *Datastore) CalculateAggregatedPerfStatsPercentiles(ctx context.Context, aggregate mdmlab.AggregatedStatsType, queryID uint) error {
...
func getTotalExecutionsQuery(aggregate mdmlab.AggregatedStatsType) string {
	switch aggregate { //nolint:gocritic // ignore singleCaseSwitch
	case mdmlab.AggregatedStatsTypeScheduledQuery:
