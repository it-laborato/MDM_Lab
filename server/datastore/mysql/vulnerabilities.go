...
func (ds *Datastore) Vulnerability(ctx context.Context, cve string, teamID *uint, includeCVEScores bool) (*mdmlab.VulnerabilityWithMetadata, error) {
	var vuln mdmlab.VulnerabilityWithMetadata
...
func (ds *Datastore) OSVersionsByCVE(ctx context.Context, cve string, teamID *uint) (vos []*mdmlab.VulnerableOS, updatedAt time.Time, err error) {
	var teamFilter *mdmlab.TeamFilter
	if teamID != nil {
		teamFilter = &mdmlab.TeamFilter{TeamID: teamID}
	}
	...
	for _, osv := range osvs.OSVersions {
		for _, id := range verResolvedDedup {
			if osv.OSVersionID == id.OSVersionID {
				vos = append(vos, &mdmlab.VulnerableOS{
					OSVersion:         osv,
...
func (ds *Datastore) SoftwareByCVE(ctx context.Context, cve string, teamID *uint) (vs []*mdmlab.VulnerableSoftware, updatedAt time.Time, err error) {
	var args []interface{}
	selectStmt := `
		SELECT
			s.id,
...
func (ds *Datastore) ListVulnerabilities(ctx context.Context, opt mdmlab.VulnListOptions) ([]mdmlab.VulnerabilityWithMetadata, *mdmlab.PaginationMetadata, error) {
	...
	// Execute the query
	var vulns []mdmlab.VulnerabilityWithMetadata
...
func (ds *Datastore) CountVulnerabilities(ctx context.Context, opt mdmlab.VulnListOptions) (uint, error) {
	selectStmt := `
		SELECT
			COUNT(*)
		FROM (
...
func (ds *Datastore) IsCVEKnownToMDMlab(ctx context.Context, cve string) (bool, error) {
	var count uint
