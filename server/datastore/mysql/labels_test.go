...
func testLabelsAddAllHosts(deferred bool, t *testing.T, db *Datastore) {
	...
	// this host
	err = db.ApplyLabelSpecs(context.Background(), []*mdmlab.LabelSpec{
		{
			Name:     "label5",
...
func testLabelsListHostsInLabel(t *testing.T, db *Datastore) {
	h1, err := db.NewHost(context.Background(), &mdmlab.Host{
	...
	h2, err := db.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	h3, err := db.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	err = db.SetOrUpdateMDMData(ctx, h1.ID, false, true, simpleMDM, true, mdmlab.WellKnownMDMSimpleMDM, "") // enrollment: automatic
	require.NoError(t, err)
	err = db.SetOrUpdateMDMData(ctx, h2.ID, false, true, kandji, true, mdmlab.WellKnownMDMKandji, "") // enrollment: automatic
	require.NoError(t, err)
	err = db.SetOrUpdateMDMData(ctx, h3.ID, false, false, simpleMDM, false, mdmlab.WellKnownMDMSimpleMDM, "") // enrollment: unenrolled
	require.NoError(t, err)
	...
	err = db.ApplyLabelSpecs(context.Background(), []*mdmlab.LabelSpec{l1})
	require.Nil(t, err)

	filter := mdmlab.TeamFilter{User: test.UserAdmin}

	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{}, 0)

	for _, h := range []*mdmlab.Host{h1, h2, h3} {
		err = db.RecordLabelQueryExecutions(context.Background(), h, map[uint]*bool{l1.ID: ptr.Bool(true)}, time.Now(), false)
		require.NoError(t, err)
	}

	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{}, 3)

	hosts := listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{LowDiskSpaceFilter: ptr.Int(35), ListOptions: mdmlab.ListOptions{OrderKey: "id", After: "1"}}, 2)
	require.Equal(t, h2.ID, hosts[0].ID)
	require.Equal(t, h3.ID, hosts[1].ID)

	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{LowDiskSpaceFilter: ptr.Int(35)}, 3)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{LowDiskSpaceFilter: ptr.Int(25)}, 2)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{LowDiskSpaceFilter: ptr.Int(15)}, 1)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{LowDiskSpaceFilter: ptr.Int(5)}, 0)

	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{MDMIDFilter: ptr.Uint(99)}, 0)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{MDMIDFilter: ptr.Uint(simpleMDMID)}, 2)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{MDMIDFilter: ptr.Uint(kandjiID)}, 1)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{MDMNameFilter: ptr.String(mdmlab.WellKnownMDMSimpleMDM)}, 2)
	listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{MDMNameFilter: ptr.String(mdmlab.WellKnownMDMSimpleMDM), MDMEnrollmentStatusFilter: mdmlab.MDMEnrollStatusEnrolled}, 1)
}

func listHostsInLabelCheckCount(
	t *testing.T, db *Datastore, filter mdmlab.TeamFilter, labelID uint, opt mdmlab.HostListOptions, expectedCount int,
) []*mdmlab.Host {
...
func testLabelsListHostsInLabelAndStatus(t *testing.T, db *Datastore) {
	...
	{
		hosts := listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{StatusFilter: mdmlab.StatusOnline}, 1)
		assert.Equal(t, "foo.local", hosts[0].Hostname)
	}

	{
		hosts := listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{StatusFilter: mdmlab.StatusMIA}, 2)
...
func testLabelsListHostsInLabelAndTeamFilter(deferred bool, t *testing.T, db *Datastore) {
	...
	team1, err := db.NewTeam(context.Background(), &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)

	team2, err := db.NewTeam(context.Background(), &mdmlab.Team{Name: "team2"})
	require.NoError(t, err)
	...
	for _, h := range []*mdmlab.Host{h1, h2} {
		err = db.RecordLabelQueryExecutions(context.Background(), h, map[uint]*bool{l1.ID: ptr.Bool(true)}, time.Now(), deferred)
		assert.Nil(t, err)
	}

	{
		hosts := listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{StatusFilter: mdmlab.StatusOnline}, 1)
		assert.Equal(t, "foo.local", hosts[0].Hostname)
	}

	{
		hosts := listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{StatusFilter: mdmlab.StatusMIA}, 1)
		assert.Equal(t, "bar.local", hosts[0].Hostname)
	}

	{
		hosts := listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: &team1.ID}, 1)
		assert.Equal(t, "foo.local", hosts[0].Hostname)
	}

	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: &team2.ID}, 0)        // no hosts assigned to team 2
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: teamIDFilterZero}, 1) // h2 not assigned to any team
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: teamIDFilterNil}, 2)  // h1 and h2

	// test team filter in combination with macos settings filter
	nanoEnrollAndSetHostMDMData(t, db, h1, false)
	require.NoError(t, err)
	require.NoError(t, db.BulkUpsertMDMAppleHostProfiles(context.Background(), []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{
		{
	...
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: &team1.ID, MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 1) // h1
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: &team2.ID, MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 0) // wrong team
	// macos settings filter does not support "all teams" so teamIDFilterNil acts the same as teamIDFilterZero
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: teamIDFilterZero, MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 0) // no team
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: teamIDFilterNil, MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 0)  // no team
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 0)                               // no team
	...
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: teamIDFilterZero, MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 1) // h2
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{TeamFilter: teamIDFilterNil, MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 1)  // h2
	listHostsInLabelCheckCount(t, db, userFilter, l1.ID, mdmlab.HostListOptions{MacOSSettingsFilter: mdmlab.OSSettingsVerifying}, 1)                               // h2
}
...
func testLabelsBuiltIn(t *testing.T, db *Datastore) {
	...
	assert.Equal(t, mdmlab.LabelTypeBuiltIn, hits[0].LabelType)
	assert.Equal(t, mdmlab.LabelTypeBuiltIn, hits[1].LabelType)
}

func testLabelsListUniqueHostsInLabels(t *testing.T, db *Datastore) {
	hosts := make([]*mdmlab.Host, 4)
	for i := range hosts {
		h, err := db.NewHost(context.Background(), &mdmlab.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
			SeenTime:        time.Now(),
	...
	require.NoError(t, db.ApplyLabelSpecs(context.Background(), []*mdmlab.LabelSpec{&l1, &l2}))
	...
	labels, err := db.ListLabels(context.Background(), filter, mdmlab.ListOptions{})
	require.Nil(t, err)
...
func testLabelsChangeDetails(t *testing.T, db *Datastore) {
	...
	err := db.ApplyLabelSpecs(context.Background(), []*mdmlab.LabelSpec{&label})
	require.Nil(t, err)
...
func setupLabelSpecsTest(t *testing.T, ds mdmlab.Datastore) []*mdmlab.LabelSpec {
	for i := 0; i < 10; i++ {
	...
	expectedSpecs := []*mdmlab.LabelSpec{
		{
			Name:        "foo",
...
func testLabelsQueriesForCentOSHost(t *testing.T, db *Datastore) {
	...
	label, err := db.NewLabel(context.Background(), &mdmlab.Label{
...
func testDeleteLabel(t *testing.T, db *Datastore) {
	ctx := context.Background()
	l, err := db.NewLabel(ctx, &mdmlab.Label{
		Name:  t.Name(),
		Query: "query1",
	})
	require.NoError(t, err)

	p, err := db.NewPack(ctx, &mdmlab.Pack{
		Name:     t.Name(),
		LabelIDs: []uint{l.ID},
	})
	require.NoError(t, err)
	...
	installer, err := mdmlab.NewTempFileReader(strings.NewReader("echo"), t.TempDir)
	require.NoError(t, err)
	installerID, _, err := db.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
	...
	l2, err := db.NewLabel(ctx, &mdmlab.Label{
		Name:  t.Name() + "2",
		Query: "query2",
	})
	require.NoError(t, err)
...
func testLabelsSummary(t *testing.T, db *Datastore) {
	...
	labels, err := db.ListLabels(context.Background(), mdmlab.TeamFilter{}, mdmlab.ListOptions{})
	require.NoError(t, err)
	...
	labelsByID := make(map[uint]*mdmlab.Label)
	for _, l := range labels {
		labelsByID[l.ID] = l
	}
	...
	_, err = db.NewLabel(context.Background(), &mdmlab.Label{
		Name:  "bing",
		Query: "query bing",
	})
	require.NoError(t, err)
...
func testListHostsInLabelIssues(t *testing.T, ds *Datastore) {
	...
	hosts = listHostsInLabelCheckCount(t, ds, filter, l1.ID, mdmlab.HostListOptions{}, 10)
	...
	checkLabelHostIssues(t, ds, l1.ID, filter, h2.ID, mdmlab.HostListOptions{}, 2, 0)
	...
	checkLabelHostIssues(t, ds, l1.ID, filter, hosts[6].ID, mdmlab.HostListOptions{}, 0, 4)
	...
	checkLabelHostIssues(t, ds, l1.ID, filter, h2.ID, mdmlab.HostListOptions{}, 1, 1)
	...
	checkLabelHostIssues(t, ds, l1.ID, filter, h2.ID, mdmlab.HostListOptions{}, 0, 1)

	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), h1, map[uint]*bool{p.ID: ptr.Bool(false)}, time.Now(), false))
	checkLabelHostIssues(t, ds, l1.ID, filter, h1.ID, mdmlab.HostListOptions{}, 1, 1)

	checkLabelHostIssues(t, ds, l1.ID, filter, h1.ID, mdmlab.HostListOptions{DisableIssues: true}, 0, 0)
	checkLabelHostIssues(t, ds, l1.ID, filter, hosts[6].ID, mdmlab.HostListOptions{DisableIssues: true}, 0, 0)
}

func checkLabelHostIssues(
	t *testing.T, ds *Datastore, lid uint, filter mdmlab.TeamFilter, hid uint, opts mdmlab.HostListOptions,
...
func testHostMemberOfAllLabels(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	foobarLabel, err := ds.NewLabel(ctx, &mdmlab.Label{
		Name:                "Foobar",
		Query:               "SELECT 1;",
		LabelType:           mdmlab.LabelTypeRegular,
		LabelMembershipType: mdmlab.LabelMembershipTypeDynamic,
	...
	require.NoError(t, err)
	zoobarLabel, err := ds.NewLabel(ctx, &mdmlab.Label{
		Name:                "Zoobar",
		Query:               "SELECT 2;",
		LabelType:           mdmlab.LabelTypeRegular,
		LabelMembershipType: mdmlab.LabelMembershipTypeDynamic,
...
func testLabelsListHostsInLabelOSSettings(t *testing.T, db *Datastore) {
	...
	// add two hosts to MDM to enforce disk encryption, mdmlab doesn't enforce settings on centos so h3 is not included
	for _, h := range []*mdmlab.Host{h1, h2} {
		windowsEnroll(t, db, h)
		require.NoError(t, db.SetOrUpdateMDMData(context.Background(), h.ID, false, true, "https://example.com", false, mdmlab.WellKnownMDMMDMlab, ""))
	}
	...
	checkHosts := func(t *testing.T, gotHosts []*mdmlab.Host, expectedIDs []uint) {
	...
	// baseline no filter
	hosts := listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{}, 3)
	checkHosts(t, hosts, []uint{h1.ID, h2.ID, h3.ID})

	t.Run("os_settings_disk_encryption", func(t *testing.T) {
		hosts = listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{OSSettingsDiskEncryptionFilter: mdmlab.DiskEncryptionVerified}, 1)
		checkHosts(t, hosts, []uint{h1.ID})
		hosts = listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{OSSettingsDiskEncryptionFilter: mdmlab.DiskEncryptionEnforcing}, 1)
		checkHosts(t, hosts, []uint{h2.ID})
	})

	t.Run("os_settings", func(t *testing.T) {
		hosts = listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{OSSettingsFilter: mdmlab.OSSettingsVerified}, 1)
		checkHosts(t, hosts, []uint{h1.ID})
		hosts = listHostsInLabelCheckCount(t, db, filter, l1.ID, mdmlab.HostListOptions{OSSettingsFilter: mdmlab.OSSettingsPending}, 1)
		checkHosts(t, hosts, []uint{h2.ID})
	})
}
...
func testAddDeleteLabelsToFromHost(t *testing.T, ds *Datastore) {
	...
	label1, err := ds.NewLabel(ctx, &mdmlab.Label{
		Name:                "label1",
		Query:               "SELECT 1;",
		LabelType:           mdmlab.LabelTypeRegular,
		LabelMembershipType: mdmlab.LabelMembershipTypeManual,
	...
	require.NoError(t, err)
	label2, err := ds.NewLabel(ctx, &mdmlab.Label{
		Name:                "label2",
		Query:               "SELECT 2;",
		LabelType:           mdmlab.LabelTypeRegular,
		LabelMembershipType: mdmlab.LabelMembershipTypeManual,
...
func labelIDFromName(t *testing.T, ds mdmlab.Datastore, name string) uint {
	allLbls, err := ds.ListLabels(context.Background(), mdmlab.TeamFilter{User: test.UserAdmin}, mdmlab.ListOptions{})
	require.Nil(t, err)
