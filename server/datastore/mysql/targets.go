...
func (ds *Datastore) CountHostsInTargets(ctx context.Context, filter mdmlab.TeamFilter, targets mdmlab.HostTargets, now time.Time) (mdmlab.TargetMetrics, error) {
	...
	// As of MDMlab 4.15, mia hosts are also included in the total for offline hosts
	sql := fmt.Sprintf(`
		SELECT
			COUNT(*) total,
		...
		WHERE %s AND %s`,
		mdmlab.OnlineIntervalBuffer, mdmlab.OnlineIntervalBuffer,
	...
	if err != nil {
		return mdmlab.TargetMetrics{}, ctxerr.Wrap(ctx, err, "sqlx.In CountHostsInTargets")
	}

	res := mdmlab.TargetMetrics{}
	err = sqlx.GetContext(ctx, ds.reader(ctx), &res, query, args...)
	if err != nil {
		return mdmlab.TargetMetrics{}, ctxerr.Wrap(ctx, err, "sqlx.Get CountHostsInTargets")
	}
...
func targetSQLCondAndArgs(targets mdmlab.HostTargets) (sql string, args []interface{}) {
...
func (ds *Datastore) HostIDsInTargets(ctx context.Context, filter mdmlab.TeamFilter, targets mdmlab.HostTargets) ([]uint, error) {
	if len(targets.HostIDs) == 0 && len(targets.LabelIDs) == 0 && len(targets.TeamIDs) == 0 {
		// No need to query if no targets selected
		return []uint{}, nil
	}
