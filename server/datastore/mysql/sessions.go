...
func (ds *Datastore) SessionByMFAToken(ctx context.Context, token string, sessionKeySize int) (*mdmlab.Session, *mdmlab.User, error) {
	var userID uint
	err := sqlx.GetContext(
		ctx,
		ds.reader(ctx),
...
func (ds *Datastore) SessionByKey(ctx context.Context, key string) (*mdmlab.Session, error) {
	sqlStatement := `
...
func (ds *Datastore) SessionByID(ctx context.Context, id uint) (*mdmlab.Session, error) {
	return ds.sessionByID(ctx, ds.reader(ctx), id)
}

func (ds *Datastore) sessionByID(ctx context.Context, q sqlx.QueryerContext, id uint) (*mdmlab.Session, error) {
	sqlStatement := `
		SELECT s.*, u.api_only FROM sessions s
		LEFT JOIN users u
		ON s.user_id = u.id
...
func (ds *Datastore) ListSessionsForUser(ctx context.Context, id uint) ([]*mdmlab.Session, error) {
	sqlStatement := `
		SELECT s.*, u.api_only FROM sessions s
		INNER JOIN users u
		ON s.user_id = u.id
...
func (ds *Datastore) NewSession(ctx context.Context, userID uint, sessionKeySize int) (*mdmlab.Session, error) {
	return ds.makeSessionInTransaction(ctx, ds.writer(ctx), userID, sessionKeySize)
}

func (ds *Datastore) makeSessionInTransaction(ctx context.Context, tx sqlx.ExtContext, userID uint, sessionKeySize int) (*mdmlab.Session, error) {
...
func (ds *Datastore) DestroySession(ctx context.Context, session *mdmlab.Session) error {
...
func (ds *Datastore) MarkSessionAccessed(ctx context.Context, session *mdmlab.Session) error {
	sqlStatement := `
		UPDATE sessions SET
		accessed_at = ?
		WHERE id = ?
