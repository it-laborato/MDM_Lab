...
func (ds *Datastore) UpsertSecretVariables(ctx context.Context, secretVariables []mdmlab.SecretVariable) error {
	...
	var variablesToInsert []mdmlab.SecretVariable
	var variablesToUpdate []mdmlab.SecretVariable
	for _, secretVariable := range secretVariables {
...
func (ds *Datastore) GetSecretVariables(ctx context.Context, names []string) ([]mdmlab.SecretVariable, error) {
	if len(names) == 0 {
		return nil, nil
	}
	...
	var secretVariables []mdmlab.SecretVariable
...
func (ds *Datastore) expandEmbeddedSecrets(ctx context.Context, document string) (string, []mdmlab.SecretVariable, error) {
	embeddedSecrets := mdmlab.ContainsPrefixVars(document, mdmlab.ServerSecretPrefix)
	...
	if len(missingSecrets) > 0 {
		return "", nil, mdmlab.MissingSecretsError{MissingSecrets: missingSecrets}
	}

	expanded := mdmlab.MaybeExpand(document, func(s string) (string, bool) {
		if !strings.HasPrefix(s, mdmlab.ServerSecretPrefix) {
			return "", false
		}
		val, ok := secretMap[strings.TrimPrefix(s, mdmlab.ServerSecretPrefix)]
		return val, ok
	})
...
func (ds *Datastore) ValidateEmbeddedSecrets(ctx context.Context, documents []string) error {
	...
	for _, document := range documents {
		vars := mdmlab.ContainsPrefixVars(document, mdmlab.ServerSecretPrefix)
		for _, v := range vars {
	...
	if len(missingSecrets) > 0 {
		return &mdmlab.MissingSecretsError{MissingSecrets: missingSecrets}
	}
