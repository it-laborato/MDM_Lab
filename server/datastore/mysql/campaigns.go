...
func (ds *Datastore) NewDistributedQueryCampaign(ctx context.Context, camp *mdmlab.DistributedQueryCampaign) (*mdmlab.DistributedQueryCampaign, error) {
...
func (ds *Datastore) DistributedQueryCampaign(ctx context.Context, id uint) (*mdmlab.DistributedQueryCampaign, error) {
	sql := `
...
func (ds *Datastore) SaveDistributedQueryCampaign(ctx context.Context, camp *mdmlab.DistributedQueryCampaign) error {
	sqlStatement := `
...
func (ds *Datastore) DistributedQueryCampaignsForQuery(ctx context.Context, queryID uint) ([]*mdmlab.DistributedQueryCampaign, error) {
	var campaigns []*mdmlab.DistributedQueryCampaign
...
func (ds *Datastore) DistributedQueryCampaignTargetIDs(ctx context.Context, id uint) (*mdmlab.HostTargets, error) {
	sqlStatement := `
		SELECT * FROM distributed_query_campaign_targets WHERE distributed_query_campaign_id = ?
	`
	targets := []mdmlab.DistributedQueryCampaignTarget{}
	...
	return &mdmlab.HostTargets{HostIDs: hostIDs, LabelIDs: labelIDs, TeamIDs: teamIDs}, nil
}

func (ds *Datastore) NewDistributedQueryCampaignTarget(ctx context.Context, target *mdmlab.DistributedQueryCampaignTarget) (*mdmlab.DistributedQueryCampaignTarget, error) {
	sqlStatement := `
...
func (ds *Datastore) CleanupDistributedQueryCampaigns(ctx context.Context, now time.Time) (expired uint, err error) {
	...
	`
	result, err := ds.writer(ctx).ExecContext(ctx, sqlStatement, mdmlab.QueryComplete,
		mdmlab.QueryWaiting, now.Add(-1*time.Minute),
		mdmlab.QueryRunning, now.Add(-24*time.Hour))
	if err != nil {
