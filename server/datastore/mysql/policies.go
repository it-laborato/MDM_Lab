...
func (ds *Datastore) NewGlobalPolicy(ctx context.Context, authorID *uint, args mdmlab.PolicyPayload) (*mdmlab.Policy, error) {
...
func (ds *Datastore) Policy(ctx context.Context, id uint) (*mdmlab.Policy, error) {
	return policyDB(ctx, ds.reader(ctx), id, nil)
}

func policyDB(ctx context.Context, q sqlx.QueryerContext, id uint, teamID *uint) (*mdmlab.Policy, error) {
	teamWhere := "TRUE"
	args := []interface{}{id}
	if teamID != nil {
		teamWhere = "team_id = ?"
...
func (ds *Datastore) PolicyLite(ctx context.Context, id uint) (*mdmlab.PolicyLite, error) {
	var policy mdmlab.PolicyLite
	err := sqlx.GetContext(
...
func (ds *Datastore) SavePolicy(ctx context.Context, p *mdmlab.Policy, shouldRemoveAllPolicyMemberships bool, removePolicyStats bool) error {
...
var (
	errMismatchedInstallerTeam = &mdmlab.BadRequestError{Message: "software installer is associated with a different team"}
	errMismatchedScriptTeam    = &mdmlab.BadRequestError{Message: "script is associated with a different team"}
)
...
func assertTeamMatches(ctx context.Context, db sqlx.QueryerContext, teamID uint, softwareInstallerID *uint, scriptID *uint, vppAppsTeamsID *uint) error {
	if softwareInstallerID != nil {
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return &mdmlab.BadRequestError{Message: "A software installer with the supplied ID does not exist"}
			}
			return err
	...
	if vppAppsTeamsID != nil {
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return &mdmlab.BadRequestError{Message: "A VPP app with the supplied ID does not exist"}
			}
			return err
	...
	if scriptID != nil {
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return &mdmlab.BadRequestError{Message: "A script with the supplied ID does not exist"}
			}
			return err
...
func (ds *Datastore) RecordPolicyQueryExecutions(ctx context.Context, host *mdmlab.Host, results map[uint]*bool, updated time.Time, deferredSaveHost bool) error {
	vals := []interface{}{}
	bindvars := []string{}
	if len(results) > 0 {
		// Sort the results to have generated SQL queries ordered to minimize
		// deadlocks. See https://github.com/mdmlabdm/mdmlab/issues/1146.
...
func (ds *Datastore) ListGlobalPolicies(ctx context.Context, opts mdmlab.ListOptions) ([]*mdmlab.Policy, error) {
...
func listPoliciesDB(ctx context.Context, q sqlx.QueryerContext, teamID *uint, opts mdmlab.ListOptions) ([]*mdmlab.Policy, error) {
	var args []interface{}
...
func getInheritedPoliciesForTeam(ctx context.Context, q sqlx.QueryerContext, teamID uint, opts mdmlab.ListOptions) ([]*mdmlab.Policy, error) {
	var args []interface{}
...
func (ds *Datastore) PoliciesByID(ctx context.Context, ids []uint) (map[uint]*mdmlab.Policy, error) {
	...
	policiesByID := make(map[uint]*mdmlab.Policy, len(ids))
...
func (ds *Datastore) PolicyQueriesForHost(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
	...
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &rows, stmt, host.TeamID, host.MDMlabPlatform()); err != nil {
...
func (ds *Datastore) NewTeamPolicy(ctx context.Context, teamID uint, authorID *uint, args mdmlab.PolicyPayload) (policy *mdmlab.Policy, err error) {
	return newTeamPolicy(ctx, ds.writer(ctx), teamID, authorID, args)
}

func newTeamPolicy(ctx context.Context, db sqlx.ExtContext, teamID uint, authorID *uint, args mdmlab.PolicyPayload) (*mdmlab.Policy, error) {
	if args.QueryID != nil {
...
func (ds *Datastore) ListTeamPolicies(ctx context.Context, teamID uint, opts mdmlab.ListOptions, iopts mdmlab.ListOptions) (teamPolicies, inheritedPolicies []*mdmlab.Policy, err error) {
...
func (ds *Datastore) ListMergedTeamPolicies(ctx context.Context, teamID uint, opts mdmlab.ListOptions) ([]*mdmlab.Policy, error) {
	var args []interface{}
...
func (ds *Datastore) TeamPolicy(ctx context.Context, teamID uint, policyID uint) (*mdmlab.Policy, error) {
...
func (ds *Datastore) ApplyPolicySpecs(ctx context.Context, authorID uint, specs []*mdmlab.PolicySpec) error {
	...
	teamIDToPolicies := make(map[*uint][]*mdmlab.PolicySpec, 1)
...
func (ds *Datastore) AsyncBatchInsertPolicyMembership(ctx context.Context, batch []mdmlab.PolicyMembershipResult) error {
...
func cleanupPolicyMembershipOnPolicyUpdate(
	...
	for _, platform := range splitPlatforms {
		expandedPlatforms = append(expandedPlatforms, mdmlab.ExpandPlatform(strings.TrimSpace(platform))...)
	}
...
func (ds *Datastore) OutdatedAutomationBatch(ctx context.Context) ([]mdmlab.PolicyFailure, error) {
	var failures []mdmlab.PolicyFailure
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func (ds *Datastore) UpdateHostPolicyCounts(ctx context.Context) error {
	...
	// https://github.com/mdmlabdm/mdmlab/blob/main/server/datastore/mysql/migrations/tables/20231215122713_InsertPolicyStatsData.go#L12
...
func (ds *Datastore) GetCalendarPolicies(ctx context.Context, teamID uint) ([]mdmlab.PolicyCalendarData, error) {
	query := `SELECT id, name FROM policies WHERE team_id = ? AND calendar_events_enabled;`
	var policies []mdmlab.PolicyCalendarData
...
func (ds *Datastore) GetPoliciesWithAssociatedInstaller(ctx context.Context, teamID uint, policyIDs []uint) ([]mdmlab.PolicySoftwareInstallerData, error) {
	if len(policyIDs) == 0 {
		return nil, nil
	}
	...
	var policies []mdmlab.PolicySoftwareInstallerData
...
func (ds *Datastore) GetPoliciesWithAssociatedVPP(ctx context.Context, teamID uint, policyIDs []uint) ([]mdmlab.PolicyVPPData, error) {
	if len(policyIDs) == 0 {
		return nil, nil
	}
	...
	var policies []mdmlab.PolicyVPPData
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, args...); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "get policies with associated installer")
	}
	return policies, nil
...
func (ds *Datastore) GetPoliciesWithAssociatedScript(ctx context.Context, teamID uint, policyIDs []uint) ([]mdmlab.PolicyScriptData, error) {
	if len(policyIDs) == 0 {
		return nil, nil
	}
	...
	var policies []mdmlab.PolicyScriptData
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, args...); err != nil {
...
func (ds *Datastore) GetTeamHostsPolicyMemberships(
	...
	policyIDs []uint,
	hostID *uint,
) ([]mdmlab.HostPolicyMembershipData, error) {
	query := `
	SELECT
...
`
	...
	var hosts []mdmlab.HostPolicyMembershipData
...
func (ds *Datastore) getPoliciesBySoftwareTitleIDs(
	...
	teamID *uint,
) ([]mdmlab.AutomaticInstallPolicy, error) {
...
`
	...
	var policies []mdmlab.AutomaticInstallPolicy
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, args...); err != nil {
