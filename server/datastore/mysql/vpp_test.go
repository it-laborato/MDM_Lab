...
func testVPPAppMetadata(t *testing.T, ds *Datastore) {
	...
	require.Equal(t, &mdmlab.VPPAppStoreApp{Name: "vpp1", VPPAppID: vpp1}, meta)

	// try to add the same app again, update self_service field
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mdmlab.MacOSPlatform}, SelfService: true},
	}, nil)
	require.NoError(t, err)
	...
	require.Equal(t, &mdmlab.VPPAppStoreApp{Name: "vpp1", VPPAppID: vpp1, SelfService: true}, meta)
	...
	require.NoError(t, err)
	meta.VPPAppsTeamsID = 0
	require.Equal(t, &mdmlab.VPPAppStoreApp{Name: "vpp2", VPPAppID: vpp2}, meta)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.app.vpp2",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mdmlab.MacOSPlatform}, SelfService: true},
	}, &team1.ID)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	meta.VPPAppsTeamsID = 0
	require.Equal(t, &mdmlab.VPPAppStoreApp{Name: "vpp2", VPPAppID: vpp2, SelfService: true}, meta)
	...
	va3, err := ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp3", BundleIdentifier: "com.app.vpp3",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_3", Platform: mdmlab.MacOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	meta.VPPAppsTeamsID = 0
	require.Equal(t, &mdmlab.VPPAppStoreApp{Name: "vpp3", VPPAppID: vpp3}, meta)
...
func testVPPAppStatus(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 0, Failed: 0, Installed: 0}, summary)
	...
	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 1, Failed: 0, Installed: 0}, summary)

	// record a failed result
	createVPPAppInstallResult(t, ds, h1, cmd1, mdmlab.MDMAppleStatusError)

	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 0, Failed: 1, Installed: 0}, summary)
	...
	createVPPAppInstallResult(t, ds, h2, cmd3, mdmlab.MDMAppleStatusAcknowledged)
	...
	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 1, Failed: 0, Installed: 1}, summary)

	// mark the pending request as successful too
	createVPPAppInstallResult(t, ds, h1, cmd2, mdmlab.MDMAppleStatusAcknowledged)

	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 0, Failed: 0, Installed: 2}, summary)
	...
	createVPPAppInstallResult(t, ds, h3, cmd4, mdmlab.MDMAppleStatusAcknowledged)

	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, &team1.ID, vpp2)
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 0, Failed: 0, Installed: 1}, summary)
	...
	createVPPAppInstallResult(t, ds, h3, cmd5, mdmlab.MDMAppleStatusAcknowledged)
	cmd6 := createVPPAppInstallRequest(t, ds, h1, vpp3.AdamID, user.ID)
	createVPPAppInstallResult(t, ds, h1, cmd6, mdmlab.MDMAppleStatusCommandFormatError)
	createVPPAppInstallRequest(t, ds, h2, vpp3.AdamID, user.ID)

	// for no team, it sees the failed and pending counts
	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp3)
	require.NoError(t, err)
	require.Equal(t, &mdmlab.VPPAppStatusSummary{Pending: 1, Failed: 1, Installed: 0}, summary)
...
func createVPPAppInstallRequest(t *testing.T, ds *Datastore, host *mdmlab.Host, adamID string, userID uint) string {
	ctx := context.Background()
...
func createVPPAppInstallResult(t *testing.T, ds *Datastore, host *mdmlab.Host, cmdUUID string, status string) {
	ctx := context.Background()
...
func testVPPApps(t *testing.T, ds *Datastore) {
	...
	// Create a team
	team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "foobar"})
	require.NoError(t, err)
	...
	app2 := &mdmlab.VPPApp{Name: "vpp_app_2", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "2", Platform: mdmlab.MacOSPlatform}}, BundleIdentifier: "b2"}
	...
	appNoTeam1 := &mdmlab.VPPApp{
		Name: "vpp_no_team_app_1", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "3", Platform: mdmlab.MacOSPlatform}},
		BundleIdentifier: "b3",
	}
	appNoTeam2 := &mdmlab.VPPApp{
		Name: "vpp_no_team_app_2", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "4", Platform: mdmlab.MacOSPlatform}},
		BundleIdentifier: "b4",
	}
	...
	u, err := ds.NewUser(ctx, &mdmlab.User{
		Password:   []byte("p4ssw0rd.123"),
		Name:       "user1",
		Email:      "user1@example.com",
		GlobalRole: ptr.String(mdmlab.RoleAdmin),
	...
	require.NoError(t, err)
	assert.Equal(t, map[mdmlab.VPPAppID]mdmlab.VPPAppTeam{
	...
	var appTitles []mdmlab.SoftwareTitle
...
func testSetTeamVPPApps(t *testing.T, ds *Datastore) {
	...
	// Create a team
	team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "vpp gang"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	app3 := &mdmlab.VPPApp{Name: "vpp_app_3", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "3", Platform: mdmlab.MacOSPlatform}}, BundleIdentifier: "b3"}
	_, err = ds.InsertVPPAppWithTeam(ctx, app3, nil)
	require.NoError(t, err)
	app4 := &mdmlab.VPPApp{Name: "vpp_app_4", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "4", Platform: mdmlab.MacOSPlatform}}, BundleIdentifier: "b4"}
	...
	err = ds.SetTeamVPPApps(ctx, &team.ID, []mdmlab.VPPAppTeam{
	...
	require.NoError(t, err)
	policy1, err := ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{
		Name:           "Policy 1",
		Query:          "SELECT 1;",
		Platform:       "darwin",
	...
	require.NoError(t, err)
	policy2, err := ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{
		Name:           "Policy 2",
		Query:          "SELECT 1;",
		Platform:       "darwin",
...
func testGetVPPAppByTeamAndTitleID(t *testing.T, ds *Datastore) {
	...
	fooApp, err := ds.InsertVPPAppWithTeam(ctx,
		&mdmlab.VPPApp{VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "foo", Platform: mdmlab.MacOSPlatform}}, BundleIdentifier: "b1", Name: "Foo"},
		&team.ID)
	require.NoError(t, err)
	...
	barApp, err := ds.InsertVPPAppWithTeam(ctx,
		&mdmlab.VPPApp{VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "bar", Platform: mdmlab.MacOSPlatform}}, BundleIdentifier: "b2", Name: "Bar"}, nil)
	require.NoError(t, err)
...
func testVPPTokensCRUD(t *testing.T, ds *Datastore) {
	...
	team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "Kritters"})
	assert.NoError(t, err)

	team2, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "Zingers"})
	assert.NoError(t, err)
	...
	assert.Error(t, err)
	assert.True(t, mdmlab.IsNotFound(err))
	...
	assert.Equal(t, mdmlab.TeamNameNoTeam, upTok.Teams[0].Name)
	...
	assert.Equal(t, mdmlab.TeamNameNoTeam, tok.Teams[0].Name)
	...
	assert.Equal(t, mdmlab.TeamNameNoTeam, toks[0].Teams[0].Name)
	...
	assert.Equal(t, mdmlab.TeamNameNoTeam, teamTok.Teams[0].Name)
	...
	t1app, err := ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mdmlab.MacOSPlatform}},
	}, &team.ID)
	require.NoError(t, err)
	...
	t1Policy, err := ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{
		Name:           "p1",
		Query:          "SELECT 1;",
	...
	t2app, err := ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mdmlab.MacOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
	...
	t2Policy, err := ds.NewTeamPolicy(ctx, team2.ID, nil, mdmlab.PolicyPayload{
		Name:           "p1",
		Query:          "SELECT 1;",
	...
	assert.Contains(t, tokTeams.Teams, mdmlab.TeamTuple{ID: team2.ID, Name: team2.Name})
	assert.Contains(t, tokTeams.Teams, mdmlab.TeamTuple{ID: 0, Name: mdmlab.TeamNameNoTeam})
	...
	assert.Contains(t, tokTeam2.Teams, mdmlab.TeamTuple{ID: team2.ID, Name: team2.Name})
	assert.Contains(t, tokTeam2.Teams, mdmlab.TeamTuple{ID: 0, Name: mdmlab.TeamNameNoTeam})
	...
	t1Policy2, err := ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{
		Name:           "t1p2",
		Query:          "SELECT 1;",
		VPPAppsTeamsID: &t1meta.VPPAppsTeamsID,
	})
...
func testVPPTokenAppTeamAssociations(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "Kritters"})
	assert.NoError(t, err)
...
func testGetOrInsertSoftwareTitleForVPPApp(t *testing.T, ds *Datastore) {
	...
	for _, platform := range mdmlab.VPPAppsPlatforms {
...
func testDeleteVPPAssignedToPolicy(t *testing.T, ds *Datastore) {
	...
	p1, err := ds.NewTeamPolicy(ctx, mdmlab.PolicyNoTeamID, nil, mdmlab.PolicyPayload{
		Name:           "p1",
		Query:          "SELECT 1;",
	...
	_, err = ds.DeleteTeamPolicies(ctx, mdmlab.PolicyNoTeamID, []uint{p1.ID})
	require.NoError(t, err)
...
func testVPPTokenTeamAssignment(t *testing.T, ds *Datastore) {
	...
	team2, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team2" + t.Name()})
	require.NoError(t, err)
	...
	err = ds.SetTeamVPPApps(ctx, &team1.ID, []mdmlab.VPPAppTeam{
		{VPPAppID: app1.VPPAppID, InstallDuringSetup: ptr.Bool(true)},
		{VPPAppID: app2.VPPAppID, SelfService: true},
	})
	require.NoError(t, err)
