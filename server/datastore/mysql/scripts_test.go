...
func testHostScriptResult(t *testing.T, ds *Datastore) {
	...
	createdScript, err := ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{
		HostID:         1,
		ScriptContents: "echo",
		UserID:         &u.ID,
		SyncRequest:    true,
	...
	hsr, action, err := ds.SetHostScriptExecutionResult(ctx, &mdmlab.HostScriptResultPayload{
		HostID:      1,
	...
	hsr, _, err = ds.SetHostScriptExecutionResult(ctx, &mdmlab.HostScriptResultPayload{
		HostID:      1,
		ExecutionID: createdScript.ExecutionID,
		Output:      "foobarbaz",
		Runtime:     22,
	...
	_, _, err = ds.SetHostScriptExecutionResult(ctx, &mdmlab.HostScriptResultPayload{
		HostID:      1,
		ExecutionID: createdScript.ExecutionID,
		Output:      largeOutput,
		Runtime:     10,
	...
	createdUnsignedScript, err := ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{
		HostID:         1,
		ScriptContents: "echo",
		UserID:         &u.ID,
		SyncRequest:    true,
	...
	unsignedScriptResult, _, err := ds.SetHostScriptExecutionResult(ctx, &mdmlab.HostScriptResultPayload{
		HostID:      1,
...
func testScripts(t *testing.T, ds *Datastore) {
	...
	scriptGlobal, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "a",
		ScriptContents: "echo",
	})
	require.NoError(t, err)
	...
	_, err = ds.NewScript(ctx, &mdmlab.Script{
		Name:           "a",
		TeamID:         ptr.Uint(123),
		ScriptContents: "echo",
	})
	require.Error(t, err)
	var fkErr mdmlab.ForeignKeyError
	...
	require.NoError(t, err)
	scriptTeam, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "a",
		TeamID:         &tm.ID,
	...
	require.Error(t, err)
	var existsErr mdmlab.AlreadyExistsError
	...
	require.NoError(t, err)
	scriptTeam2, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "a",
		TeamID:         &tm.ID,
		ScriptContents: "echo",
	})
...
func testListScripts(t *testing.T, ds *Datastore) {
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		_, err = ds.NewScript(ctx, &mdmlab.Script{Name: string('a' + byte(i)), TeamID: &tm1.ID, ScriptContents: "echo"})
		require.NoError(t, err)
	}

	// create a single script for team 2
	_, err = ds.NewScript(ctx, &mdmlab.Script{Name: "a", TeamID: &tm2.ID, ScriptContents: "echo"})
	require.NoError(t, err)
	...
	cases := []struct {
		...
		{
			opts:      mdmlab.ListOptions{PerPage: 3},
			teamID:    &tm1.ID,
			wantNames: []string{"a", "b", "c"},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: false},
		},
		{
			opts:      mdmlab.ListOptions{Page: 1, PerPage: 3},
			teamID:    &tm1.ID,
			wantNames: []string{"d", "e"},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
		},
		{
			opts:      mdmlab.ListOptions{Page: 2, PerPage: 3},
			teamID:    &tm1.ID,
			wantNames: nil,
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
		},
...
func testGetHostScriptDetails(t *testing.T, ds *Datastore) {
	...
	scripts, _, err := ds.ListScripts(ctx, nil, mdmlab.ListOptions{})
	require.NoError(t, err)
...
VALUES
	...
	insertResults(t, 42, &mdmlab.Script{Name: "script-6", ScriptContents: "echo script-6"}, now.Add(-1*time.Minute), "execution-6-1", ptr.Int64(0))

	t.Run("results match expected formatting and filtering", func(t *testing.T) {
		res, _, err := ds.GetHostScriptDetails(ctx, 42, nil, mdmlab.ListOptions{}, "")
		require.NoError(t, err)
		require.Len(t, res, 6)
		for _, r := range res {
			switch r.ScriptID {
	...
	t.Run("empty slice returned if no scripts", func(t *testing.T) {
		res, _, err := ds.GetHostScriptDetails(ctx, 42, ptr.Uint(1), mdmlab.ListOptions{}, "") // team 1 has no scripts
		require.NoError(t, err)
	...
	t.Run("windows ps1 scripts are supported", func(t *testing.T) {
		res, _, err := ds.GetHostScriptDetails(ctx, 42, nil, mdmlab.ListOptions{}, "windows")
		require.NoError(t, err)
		require.NotNil(t, res)
		require.Len(t, res, 1)
...
VALUES
	...
	applyAndExpect := func(newSet []*mdmlab.Script, tmID *uint, want []*mdmlab.Script) map[string]uint {
		...
		got, _, err := ds.ListScripts(ctx, tmID, mdmlab.ListOptions{})
		require.NoError(t, err)
	...
	// create a team
	tm1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "_tm1"})
	require.NoError(t, err)

	// apply single script set for tm1
	sTm1 := applyAndExpect([]*mdmlab.Script{
		{Name: "N1", ScriptContents: "C1"},
	}, ptr.Uint(tm1.ID), []*mdmlab.Script{
		{Name: "N1", TeamID: ptr.Uint(tm1.ID)},
	})
	n1WithTeamID := sTm1["N1"]

	teamPolicy, err := ds.NewTeamPolicy(ctx, tm1.ID, nil, mdmlab.PolicyPayload{
	...
	sNoTm := applyAndExpect([]*mdmlab.Script{
		{Name: "N1", ScriptContents: "C1"},
	}, nil, []*mdmlab.Script{
		{Name: "N1", TeamID: nil},
	})
	...
	noTeamPolicy, err := ds.NewTeamPolicy(ctx, mdmlab.PolicyNoTeamID, nil, mdmlab.PolicyPayload{
		Name:     "No Team Policy",
		Query:    "SELECT 1",
		Platform: "darwin",
	...
	sTm1b := applyAndExpect([]*mdmlab.Script{
		{Name: "N1", ScriptContents: "C1"},
	...
	sNoTmb := applyAndExpect([]*mdmlab.Script{
	...
	// script for tm1
	sTm1c := applyAndExpect([]*mdmlab.Script{
...
func testLockHostViaScript(t *testing.T, ds *Datastore) {
	...
	err = ds.LockHostViaScript(ctx, &mdmlab.HostScriptRequestPayload{
	...
	status, err := ds.GetHostLockWipeStatus(ctx, &mdmlab.Host{ID: windowsHostID, Platform: "windows", UUID: "uuid"})
	require.NoError(t, err)
	require.Equal(t, "windows", status.HostMDMlabPlatform)
	...
	_, action, err := ds.SetHostScriptExecutionResult(ctx, &mdmlab.HostScriptResultPayload{
		HostID:      s.HostID,
...
func testUnlockHostViaScript(t *testing.T, ds *Datastore) {
	...
	err = ds.UnlockHostViaScript(ctx, &mdmlab.HostScriptRequestPayload{
		HostID:         hostID,
		ScriptContents: script,
		UserID:         &user.ID,
		SyncRequest:    false,
	...
	// verify that we have created entries in host_mdm_actions and host_script_results
	status, err := ds.GetHostLockWipeStatus(ctx, &mdmlab.Host{ID: hostID, Platform: "windows", UUID: "uuid"})
	require.NoError(t, err)
	require.Equal(t, "windows", status.HostMDMlabPlatform)
...
func testLockUnlockWipeViaScripts(t *testing.T, ds *Datastore) {
	...
	for i, platform := range []string{"windows", "linux"} {
		...
		t.Run(platform, func(t *testing.T) {
			status, err := ds.GetHostLockWipeStatus(ctx, &mdmlab.Host{ID: hostID, Platform: platform, UUID: "uuid"})
			require.NoError(t, err)
			...
			case "linux":
				// record a request to wipe the host
				err = ds.WipeHostViaScript(ctx, &mdmlab.HostScriptRequestPayload{
					HostID:         hostID,
					ScriptContents: "wipe",
					UserID:         &user.ID,
					SyncRequest:    false,
...
func testLockUnlockManually(t *testing.T, ds *Datastore) {
	...
	status, err := ds.GetHostLockWipeStatus(ctx, &mdmlab.Host{ID: 1, Platform: "darwin", UUID: "uuid"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	status, err = ds.GetHostLockWipeStatus(ctx, &mdmlab.Host{ID: 2, Platform: "darwin", UUID: "uuid"})
	require.NoError(t, err)
	require.False(t, status.UnlockRequestedAt.IsZero())
	require.WithinDuration(t, today, status.UnlockRequestedAt, 1*time.Second)
}

func checkLockWipeState(t *testing.T, status *mdmlab.HostLockWipeStatus, unlocked, locked, wiped, pendingUnlock, pendingLock, pendingWipe bool) {
...
func testCleanupUnusedScriptContents(t *testing.T, ds *Datastore) {
	...
	res, err := ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{ScriptContents: "echo something_else", SyncRequest: true})
	require.NoError(t, err)
...
func testDeleteScriptsAssignedToPolicy(t *testing.T, ds *Datastore) {
	...
	p1, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mdmlab.PolicyPayload{
		Name:     "p1",
		Query:    "SELECT 1;",
		ScriptID: &script.ID,
	})
...
func testDeletePendingHostScriptExecutionsForPolicy(t *testing.T, ds *Datastore) {
	...
	team1, _ := ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})

	script1, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "script1.sh",
		TeamID:         &team1.ID,
		ScriptContents: "hello world",
	})
	require.NoError(t, err)
	script2, err := ds.NewScript(ctx, &mdmlab.Script{
		Name:           "script2.sh",
		TeamID:         &team1.ID,
		ScriptContents: "hello world",
	})
	...
	p2, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mdmlab.PolicyPayload{
		Name:     "p2",
		Query:    "SELECT 2;",
		ScriptID: &script2.ID,
	})
	...
	scriptExecution, err := ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{
		HostID:         1,
		ScriptContents: "echo",
		UserID:         &user.ID,
		PolicyID:       &p1.ID,
