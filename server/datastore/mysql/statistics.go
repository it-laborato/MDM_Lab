...
type statistics struct {
	mdmlab.UpdateCreateTimestamps
	Identifier string `db:"anonymous_identifier"`
}

func (ds *Datastore) ShouldSendStatistics(ctx context.Context, frequency time.Duration, config config.MDMlabConfig) (mdmlab.StatisticsPayload, bool, error) {
	lic, _ := license.FromContext(ctx)

	computeStats := func(stats *mdmlab.StatisticsPayload, since time.Time) error {
		...
		numHostsMDMlabDesktopEnabled, err := numHostsMDMlabDesktopEnabledDB(ctx, ds.reader(ctx))
		if err != nil {
			return ctxerr.Wrap(ctx, err, "number of hosts with MDMlab desktop installed")
		}
		...
		teams, err := ds.ListTeams(ctx, mdmlab.TeamFilter{User: &mdmlab.User{
		...
		stats.NumHostsMDMlabDesktopEnabled = numHostsMDMlabDesktopEnabled
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			if err != nil {
				return mdmlab.StatisticsPayload{}, false, ctxerr.Wrap(ctx, err, "generate random text")
			}
			_, err = ds.writer(ctx).ExecContext(ctx, `INSERT INTO statistics(anonymous_identifier) VALUES (?)`, anonIdentifier)
			if err != nil {
				return mdmlab.StatisticsPayload{}, false, ctxerr.Wrap(ctx, err, "insert statistics")
			}

			// compute active weekly users since now - frequency
			stats := mdmlab.StatisticsPayload{
				AnonymousIdentifier: anonIdentifier,
				MDMlabVersion:        version.Version().Version,
			...
			if err := computeStats(&stats, time.Now().Add(-frequency)); err != nil {
				return mdmlab.StatisticsPayload{}, false, ctxerr.Wrap(ctx, err, "compute statistics")
			}

			return stats, true, nil
		}
		return mdmlab.StatisticsPayload{}, false, ctxerr.Wrap(ctx, err, "get statistics")
	}
