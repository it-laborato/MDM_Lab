...
func testCampaignsDistributedQuery(t *testing.T, ds *Datastore) {
	...
	campaign := test.NewCampaign(t, ds, query.ID, mdmlab.QueryRunning, mockClock.Now())
	...
	checkTargets(t, ds, campaign.ID, mdmlab.HostTargets{})

	test.AddHostToCampaign(t, ds, campaign.ID, h1.ID)
	checkTargets(t, ds, campaign.ID, mdmlab.HostTargets{HostIDs: []uint{h1.ID}})

	test.AddLabelToCampaign(t, ds, campaign.ID, l1.ID)
	checkTargets(t, ds, campaign.ID, mdmlab.HostTargets{HostIDs: []uint{h1.ID}, LabelIDs: []uint{l1.ID}})

	test.AddLabelToCampaign(t, ds, campaign.ID, l2.ID)
	checkTargets(t, ds, campaign.ID, mdmlab.HostTargets{HostIDs: []uint{h1.ID}, LabelIDs: []uint{l1.ID, l2.ID}})

	test.AddHostToCampaign(t, ds, campaign.ID, h2.ID)
	test.AddHostToCampaign(t, ds, campaign.ID, h3.ID)

	checkTargets(t, ds, campaign.ID, mdmlab.HostTargets{HostIDs: []uint{h1.ID, h2.ID, h3.ID}, LabelIDs: []uint{l1.ID, l2.ID}})
}
...
func testCampaignsCleanupDistributedQuery(t *testing.T, ds *Datastore) {
	...
	c1 := test.NewCampaign(t, ds, query.ID, mdmlab.QueryWaiting, mockClock.Now())
	c2 := test.NewCampaign(t, ds, query.ID, mdmlab.QueryRunning, mockClock.Now())
	...
	assert.Equal(t, uint(1), expired)
	{
		...
		require.Nil(t, err)
		assert.Equal(t, c1.QueryID, retrieved.QueryID)
		assert.Equal(t, mdmlab.QueryComplete, retrieved.Status)
	}
	{
		retrieved, err := ds.DistributedQueryCampaign(ctx, c2.ID)
		require.Nil(t, err)
	...
	c3 := test.NewCampaign(t, ds, query.ID, mdmlab.QueryWaiting, mockClock.Now().AddDate(0, 0, -8))
	{
		retrieved, err := ds.DistributedQueryCampaign(ctx, c3.ID)
		require.Nil(t, err)
		assert.Equal(t, c3.QueryID, retrieved.QueryID)
		assert.Equal(t, mdmlab.QueryWaiting, retrieved.Status)
	}
...
func testCampaignsSaveDistributedQuery(t *testing.T, ds *Datastore) {
	user := test.NewUser(t, ds, t.Name(), t.Name()+"zwass@mdmlab.co", true)
	...
	require.Equal(t, mdmlab.QueryWaiting, gotC.Status)

	c1.Status = mdmlab.QueryComplete
	require.NoError(t, ds.SaveDistributedQueryCampaign(context.Background(), c1))

	gotC, err = ds.DistributedQueryCampaign(context.Background(), c1.ID)
	require.NoError(t, err)
	require.Equal(t, mdmlab.QueryComplete, gotC.Status)
}

func checkTargets(t *testing.T, ds mdmlab.Datastore, campaignID uint, expectedTargets mdmlab.HostTargets) {
