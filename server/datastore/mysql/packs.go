...
func (ds *Datastore) ApplyPackSpecs(ctx context.Context, specs []*mdmlab.PackSpec) (err error) {
...
func applyPackSpecDB(ctx context.Context, tx sqlx.ExtContext, spec *mdmlab.PackSpec) error {
	if spec.Name == "" {
	...
	for _, l := range spec.Targets.Labels {
		if _, err := tx.ExecContext(ctx, query, packID, mdmlab.TargetLabel, l); err != nil {
	...
	for _, t := range spec.Targets.Teams {
		if _, err := tx.ExecContext(ctx, query, packID, mdmlab.TargetTeam, t); err != nil {
...
func (ds *Datastore) GetPackSpecs(ctx context.Context) ([]*mdmlab.PackSpec, error) {
	var specs []*mdmlab.PackSpec
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		// Get basic specs
...
`
			if err := sqlx.SelectContext(ctx, tx, &spec.Targets.Labels, query, spec.ID, mdmlab.TargetLabel); err != nil {
...
`
			if err := sqlx.SelectContext(ctx, tx, &spec.Targets.Teams, query, spec.ID, mdmlab.TargetTeam); err != nil {
...
func (ds *Datastore) GetPackSpec(ctx context.Context, name string) (*mdmlab.PackSpec, error) {
	spec := &mdmlab.PackSpec{}
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		// Get basic spec
...
func (ds *Datastore) PackByName(ctx context.Context, name string, opts ...mdmlab.OptionalArg) (*mdmlab.Pack, bool, error) {
	sqlStatement := `
		SELECT *
			FROM packs
			WHERE name = ?
...
func (ds *Datastore) NewPack(ctx context.Context, pack *mdmlab.Pack, opts ...mdmlab.OptionalArg) (*mdmlab.Pack, error) {
...
func replacePackTargetsDB(ctx context.Context, tx sqlx.ExecerContext, pack *mdmlab.Pack) error {
	...
	if len(pack.LabelIDs) > 0 {
		...
		for _, id := range pack.LabelIDs {
			args = append(args, pack.ID, mdmlab.TargetLabel, id)
		}
	...
	if len(pack.HostIDs) > 0 {
		...
		for _, id := range pack.HostIDs {
			args = append(args, pack.ID, mdmlab.TargetHost, id)
		}
		values := strings.TrimSuffix(
	...
	if len(pack.TeamIDs) > 0 {
		...
		for _, id := range pack.TeamIDs {
			args = append(args, pack.ID, mdmlab.TargetTeam, id)
		}
		values := strings.TrimSuffix(
...
func loadPackTargetsDB(ctx context.Context, q sqlx.QueryerContext, pack *mdmlab.Pack) error {
	var targets []mdmlab.Target
	sql := `
...
func (ds *Datastore) SavePack(ctx context.Context, pack *mdmlab.Pack) error {
...
// DeletePack deletes a mdmlab.Pack so that it won't show up in results.
func (ds *Datastore) DeletePack(ctx context.Context, name string) error {
	return ds.deleteEntityByName(ctx, packsTable, name)
}

// Pack fetch mdmlab.Pack with matching ID
func (ds *Datastore) Pack(ctx context.Context, pid uint) (*mdmlab.Pack, error) {
	return packDB(ctx, ds.reader(ctx), pid)
}

func packDB(ctx context.Context, q sqlx.QueryerContext, pid uint) (*mdmlab.Pack, error) {
...
func teamScheduleName(team *mdmlab.Team) string {
	return fmt.Sprintf("Team: %s", team.Name)
}

// ListPacks returns all mdmlab.Pack records limited and sorted by mdmlab.ListOptions
func (ds *Datastore) ListPacks(ctx context.Context, opt mdmlab.PackListOptions) ([]*mdmlab.Pack, error) {
	query := `SELECT * FROM packs WHERE pack_type IS NULL OR pack_type = ''`
	if opt.IncludeSystemPacks {
		query = `SELECT * FROM packs`
	}
	var packs []*mdmlab.Pack
...
func (ds *Datastore) ListPacksForHost(ctx context.Context, hid uint) ([]*mdmlab.Pack, error) {
...
func listPacksForHost(ctx context.Context, db sqlx.QueryerContext, hid uint) ([]*mdmlab.Pack, error) {
	query := `
	...
	if err := sqlx.SelectContext(ctx, db, &packs, query,
		mdmlab.TargetLabel, hid, mdmlab.TargetHost, hid, mdmlab.TargetTeam, hid,
