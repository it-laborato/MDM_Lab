...
import (
	...
	"github.com:it-laborato/MDM_Lab/pkg/automatic_policy"
...
`
	var results []string
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &results, stmt, hostID, mdmlab.SoftwareInstallPending); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "list pending software installs")
	}
	return results, nil
}

func (ds *Datastore) GetSoftwareInstallDetails(ctx context.Context, executionId string) (*mdmlab.SoftwareInstallDetails, error) {
	const stmt = `
  SELECT
  ...
  WHERE
	...
	result := &mdmlab.SoftwareInstallDetails{}
...
func (ds *Datastore) MatchOrCreateSoftwareInstaller(ctx context.Context, payload *mdmlab.UploadSoftwareInstallerPayload) (installerID, titleID uint, err error) {
...
func (ds *Datastore) createAutomaticPolicy(ctx context.Context, tx sqlx.ExtContext, payload *mdmlab.UploadSoftwareInstallerPayload, softwareInstallerID uint) error {
	...
	if _, err := newTeamPolicy(ctx, tx, teamID, userID, mdmlab.PolicyPayload{
...
func (ds *Datastore) getOrGenerateSoftwareInstallerTitleID(ctx context.Context, payload *mdmlab.UploadSoftwareInstallerPayload) (uint, error) {
...
func (ds *Datastore) addSoftwareTitleToMatchingSoftware(ctx context.Context, titleID uint, payload *mdmlab.UploadSoftwareInstallerPayload) error {
...
func setOrUpdateSoftwareInstallerLabelsDB(ctx context.Context, tx sqlx.ExtContext, installerID uint, labels mdmlab.LabelIdentsWithScope) error {
	...
	if len(labelIds) > 0 {
		...
		case mdmlab.LabelScopeIncludeAny:
			exclude = false
		case mdmlab.LabelScopeExcludeAny:
			exclude = true
		default:
...
func (ds *Datastore) SaveInstallerUpdates(ctx context.Context, payload *mdmlab.UpdateSoftwareInstallerPayload) error {
...
func (ds *Datastore) GetSoftwareInstallerMetadataByID(ctx context.Context, id uint) (*mdmlab.SoftwareInstaller, error) {
	query := `
SELECT
	si.id,
	si.team_id,
...
WHERE
	...
	var dest mdmlab.SoftwareInstaller
...
func (ds *Datastore) GetSoftwareInstallerMetadataByTeamAndTitleID(ctx context.Context, teamID *uint, titleID uint, withScriptContents bool) (*mdmlab.SoftwareInstaller, error) {
...
WHERE
  si.title_id = ? AND si.global_or_team_id = ?`,
	...
	var exclAny, inclAny []mdmlab.SoftwareScopeLabel
...
func (ds *Datastore) getSoftwareInstallerLabels(ctx context.Context, installerID uint) ([]mdmlab.SoftwareScopeLabel, error) {
	query := `
SELECT
	label_id,
	exclude,
...
WHERE
	...
	var labels []mdmlab.SoftwareScopeLabel
...
var (
	errDeleteInstallerWithAssociatedPolicy = &mdmlab.ConflictError{Message: "Couldn't delete. Policy automation uses this software. Please disable policy automation for this software and try again."}
	errDeleteInstallerInstalledDuringSetup = &mdmlab.ConflictError{Message: "Couldn't delete. This software is installed when new Macs boot. Please remove software in Controls > Setup experience and try again."}
)
...
func (ds *Datastore) deletePendingSoftwareInstallsForPolicy(ctx context.Context, teamID *uint, policyID uint) error {
	...
	`
	_, err := ds.writer(ctx).ExecContext(ctx, deleteStmt, policyID, mdmlab.SoftwareInstallPending, globalOrTeamID)
	if err != nil {
...
func (ds *Datastore) GetSoftwareInstallResults(ctx context.Context, resultsUUID string) (*mdmlab.HostSoftwareInstallerResult, error) {
	query := `
SELECT
	hsi.execution_id AS execution_id,
...
WHERE
	...
	var dest mdmlab.HostSoftwareInstallerResult
...
func (ds *Datastore) GetSummaryHostSoftwareInstalls(ctx context.Context, installerID uint) (*mdmlab.SoftwareInstallerStatusSummary, error) {
	var dest mdmlab.SoftwareInstallerStatusSummary
...
WHERE
	...
	query, args, err := sqlx.Named(stmt, map[string]interface{}{
		...
		"software_status_pending_install":   mdmlab.SoftwareInstallPending,
		"software_status_failed_install":    mdmlab.SoftwareInstallFailed,
		"software_status_pending_uninstall": mdmlab.SoftwareUninstallPending,
		"software_status_failed_uninstall":  mdmlab.SoftwareUninstallFailed,
...
func (ds *Datastore) vppAppJoin(appID mdmlab.VPPAppID, status mdmlab.SoftwareInstallerStatus) (string, []interface{}, error) {
	// Since VPP does not have uninstaller yet, we map the generic pending/failed statuses to the install statuses
	switch status {
	case mdmlab.SoftwarePending:
		status = mdmlab.SoftwareInstallPending
	case mdmlab.SoftwareFailed:
...
func (ds *Datastore) softwareInstallerJoin(installerID uint, status mdmlab.SoftwareInstallerStatus) (string, []interface{}, error) {
	statusFilter := "hsi.status = :status"
	var status2 mdmlab.SoftwareInstallerStatus
	switch status {
	case mdmlab.SoftwarePending:
		status = mdmlab.SoftwareInstallPending
		status2 = mdmlab.SoftwareUninstallPending
	case mdmlab.SoftwareFailed:
		status = mdmlab.SoftwareInstallFailed
		status2 = mdmlab.SoftwareUninstallFailed
	default:
		// no change
	}
	if status2 != "" {
...
func (ds *Datastore) GetHostLastInstallData(ctx context.Context, hostID, installerID uint) (*mdmlab.HostLastInstallData, error) {
	stmt := `
...
`
	...
	var hostLastInstall mdmlab.HostLastInstallData
...
func (ds *Datastore) CleanupUnusedSoftwareInstallers(ctx context.Context, softwareInstallStore mdmlab.SoftwareInstallerStore, removeCreatedBefore time.Time) error {
...
func (ds *Datastore) BatchSetSoftwareInstallers(ctx context.Context, tmID *uint, installers []*mdmlab.UploadSoftwareInstallerPayload) error {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		for _, installer := range installers {
			...
			if len(installer.ValidatedLabels.ByName) == 0 {
				...
				excludeLabels := installer.ValidatedLabels.LabelScope == mdmlab.LabelScopeExcludeAny
...
func (ds *Datastore) HasSelfServiceSoftwareInstallers(ctx context.Context, hostPlatform string, hostTeamID *uint) (bool, error) {
	if mdmlab.IsLinux(hostPlatform) {
		hostPlatform = "linux"
	}
	stmt := `SELECT 1
		WHERE EXISTS (
...
func (ds *Datastore) GetSoftwareInstallers(ctx context.Context, teamID uint) ([]mdmlab.SoftwarePackageResponse, error) {
...
`
	var softwarePackages []mdmlab.SoftwarePackageResponse
