...
func connectMySQL(t testing.TB, testName string, opts *DatastoreTestOptions) *Datastore {
	...
	// For use with WithMDMlabConfig. Note that since we're setting up the DB in a different way
	...
	ds, err := New(cfg, clock.NewMockClock(), Logger(dslogger), LimitAttempts(1), replicaOpt, SQLMode("ANSI"), WithMDMlabConfig(&tc))
	require.Nil(t, err)
...
func GetAggregatedStats(ctx context.Context, ds *Datastore, aggregate mdmlab.AggregatedStatsType, id uint) (mdmlab.AggregatedStats, error) {
	result := mdmlab.AggregatedStats{}
	stmt := `
	SELECT
...
func CreateABMKeyCertIfNotExists(t testing.TB, ds *Datastore) {
	...
	_, err = ds.GetAllMDMConfigAssetsByName(context.Background(), []mdmlab.MDMAssetName{
		mdmlab.MDMAssetABMKey,
	}, nil)
	if err != nil {
		var nfe mdmlab.NotFoundError
		require.ErrorAs(t, err, &nfe)
		assets = append(assets, mdmlab.MDMConfigAsset{Name: mdmlab.MDMAssetABMKey, Value: keyPEM})
	}
	...
	if err != nil {
		var nfe mdmlab.NotFoundError
		require.ErrorAs(t, err, &nfe)
		assets = append(assets, mdmlab.MDMConfigAsset{Name: mdmlab.MDMAssetABMCert, Value: certPEM})
	}
...
func CreateAndSetABMToken(t testing.TB, ds *Datastore, orgName string) *mdmlab.ABMToken {
	...
	certPEM := assets[mdmlab.MDMAssetABMCert].Value
	...
	tok, err := ds.InsertABMToken(context.Background(), &mdmlab.ABMToken{EncryptedToken: []byte(tokenBytes), OrganizationName: orgName})
	require.NoError(t, err)
	return tok
}

func SetTestABMAssets(t testing.TB, ds *Datastore, orgName string) *mdmlab.ABMToken {
	...
	assets := []mdmlab.MDMConfigAsset{
		{Name: mdmlab.MDMAssetABMCert, Value: certPEM},
		{Name: mdmlab.MDMAssetABMKey, Value: keyPEM},
		{Name: mdmlab.MDMAssetAPNSCert, Value: apnsCert},
		{Name: mdmlab.MDMAssetAPNSKey, Value: apnsKey},
		{Name: mdmlab.MDMAssetCACert, Value: certPEM},
		{Name: mdmlab.MDMAssetCAKey, Value: keyPEM},
	}

	err = ds.InsertMDMConfigAssets(context.Background(), assets, nil)
	require.NoError(t, err)

	tok, err := ds.InsertABMToken(context.Background(), &mdmlab.ABMToken{EncryptedToken: tokenBytes, OrganizationName: orgName})
	require.NoError(t, err)
