...
func (ds *Datastore) MDMWindowsGetEnrolledDeviceWithDeviceID(ctx context.Context, mdmDeviceID string) (*mdmlab.MDMWindowsEnrolledDevice, error) {
	stmt := `SELECT
		id,
		mdm_device_id,
		mdm_hardware_id,
	...
	var winMDMDevice mdmlab.MDMWindowsEnrolledDevice
...
// database.
func (ds *Datastore) MDMWindowsInsertEnrolledDevice(ctx context.Context, device *mdmlab.MDMWindowsEnrolledDevice) error {
	stmt := `
...
func (ds *Datastore) MDMWindowsInsertCommandForHosts(ctx context.Context, hostUUIDsOrDeviceIDs []string, cmd *mdmlab.MDMWindowsCommand) error {
...
func (ds *Datastore) mdmWindowsInsertCommandForHostsDB(ctx context.Context, tx sqlx.ExecerContext, hostUUIDsOrDeviceIDs []string, cmd *mdmlab.MDMWindowsCommand) error {
...
// given device ID.
func (ds *Datastore) MDMWindowsGetPendingCommands(ctx context.Context, deviceID string) ([]*mdmlab.MDMWindowsCommand, error) {
	var commands []*mdmlab.MDMWindowsCommand
...
func (ds *Datastore) MDMWindowsSaveResponse(ctx context.Context, deviceID string, enrichedSyncML mdmlab.EnrichedSyncML) error {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		var matchingCmds []mdmlab.MDMWindowsCommand
		...
		var (
			args                     []any
			sb                       strings.Builder
			potentialProfilePayloads []*mdmlab.MDMWindowsProfilePayload
		...
		for _, cmd := range matchingCmds {
			...
			if status, ok := enrichedSyncML.CmdRefUUIDToStatus[cmd.CommandUUID]; ok && status.Data != nil {
				...
				if status.Cmd != nil && *status.Cmd == mdmlab.CmdAtomic {
					...
					pp, err := mdmlab.BuildMDMWindowsProfilePayloadFromMDMResponse(cmdWithSecret, enrichedSyncML.CmdRefUUIDToStatus,
...
func updateMDMWindowsHostProfileStatusFromResponseDB(
	ctx context.Context,
	tx sqlx.ExtContext,
	payloads []*mdmlab.MDMWindowsProfilePayload,
) error {
	if len(payloads) == 0 {
		return nil
	}
	...
	// command later on.
	uuidsToPayloads := make(map[string]*mdmlab.MDMWindowsProfilePayload, len(payloads))
	...
	var matchingHostProfiles []mdmlab.MDMWindowsProfilePayload
	...
	for _, hp := range matchingHostProfiles {
		...
		if payload.Status != nil && *payload.Status == mdmlab.MDMDeliveryFailed {
...
func (ds *Datastore) GetMDMWindowsCommandResults(ctx context.Context, commandUUID string) ([]*mdmlab.MDMCommandResult, error) {
	query := `
SELECT
    mwe.host_uuid,
    wmcr.command_uuid,
...
// - host_mdm: hmdm
// - host_disks: hd
func (ds *Datastore) whereBitLockerStatus(status mdmlab.DiskEncryptionStatus) string {
	const (
...
func (ds *Datastore) GetMDMWindowsBitLockerSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMWindowsBitLockerSummary, error) {
...
WHERE
	...
	var res mdmlab.MDMWindowsBitLockerSummary
	stmt := fmt.Sprintf(
		sqlFmt,
...
func (ds *Datastore) GetMDMWindowsBitLockerStatus(ctx context.Context, host *mdmlab.Host) (*mdmlab.HostMDMDiskEncryption, error) {
	if host == nil {
...
WHERE
	...
	var dest struct {
		Status mdmlab.DiskEncryptionStatus `db:"status"`
	...
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &dest, stmt, host.ID); err != nil {
		...
		dest.Status = mdmlab.DiskEncryptionEnforcing
	}
	...
	if dest.Status == "" {
		...
		dest.Status = mdmlab.DiskEncryptionFailed
	}
...
func (ds *Datastore) GetMDMWindowsConfigProfile(ctx context.Context, profileUUID string) (*mdmlab.MDMWindowsConfigProfile, error) {
	stmt := `
SELECT
	profile_uuid,
	team_id,
...
WHERE
	...
	var res mdmlab.MDMWindowsConfigProfile
...
func subqueryHostsMDMWindowsOSSettingsStatusFailed() (string, []interface{}, error) {
	...
	args := []interface{}{
		...
		mdm.ListMDMlabReservedWindowsProfileNames(),
	}
...
func subqueryHostsMDMWindowsOSSettingsStatusVerifying() (string, []interface{}, error) {
	...
	args := []interface{}{
		...
		mdmlab.MDMOperationTypeInstall,
		mdm.ListMDMlabReservedWindowsProfileNames(),
		[]interface{}{mdmlab.MDMDeliveryVerifying, mdmlab.MDMDeliveryVerified},
	}
	return sqlx.In(sql, args...)
}
...
func (ds *Datastore) GetMDMWindowsProfilesSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMProfilesSummary, error) {
...
func (ds *Datastore) ListMDMWindowsProfilesToInstall(ctx context.Context) ([]*mdmlab.MDMWindowsProfilePayload, error) {
	var result []*mdmlab.MDMWindowsProfilePayload
...
) ([]*mdmlab.MDMWindowsProfilePayload, error) {
...
`, windowsMDMProfilesDesiredStateQuery)
	...
	var err error
	args := []any{mdmlab.MDMOperationTypeInstall}
	...
	if len(hostUUIDs) > 0 {
		if len(onlyProfileUUIDs) > 0 {
			...
			query, args, err = sqlx.In(query, hostUUIDs, hostUUIDs, hostUUIDs, hostUUIDs, mdmlab.MDMOperationTypeInstall)
		}
		if err != nil {
	...
	var profiles []*mdmlab.MDMWindowsProfilePayload
	err = sqlx.SelectContext(ctx, tx, &profiles, query, args...)
	return profiles, err
}

func (ds *Datastore) ListMDMWindowsProfilesToRemove(ctx context.Context) ([]*mdmlab.MDMWindowsProfilePayload, error) {
	var result []*mdmlab.MDMWindowsProfilePayload
	err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) BulkUpsertMDMWindowsHostProfiles(ctx context.Context, payload []*mdmlab.MDMWindowsBulkUpsertHostProfilePayload) error {
	if len(payload) == 0 {
		return nil
	}
...
func (ds *Datastore) BulkDeleteMDMWindowsHostsConfigProfiles(ctx context.Context, profs []*mdmlab.MDMWindowsProfilePayload) error {
...
func (ds *Datastore) bulkDeleteMDMWindowsHostsConfigProfilesDB(
	ctx context.Context,
	tx sqlx.ExtContext,
	profs []*mdmlab.MDMWindowsProfilePayload,
) error {
	if len(profs) == 0 {
		return nil
	}
...
func (ds *Datastore) NewMDMWindowsConfigProfile(ctx context.Context, cp mdmlab.MDMWindowsConfigProfile) (*mdmlab.MDMWindowsConfigProfile, error) {
...
func (ds *Datastore) SetOrUpdateMDMWindowsConfigProfile(ctx context.Context, cp mdmlab.MDMWindowsConfigProfile) error {
	profileUUID := "w" + uuid.New().String()
	stmt := `
INSERT INTO
	mdm_windows_configuration_profiles (profile_uuid, team_id, name, syncml, uploaded_at)
...
`
	...
	// or processing
	incomingProfs := make(map[string]*mdmlab.MDMWindowsConfigProfile, len(profiles))
	for i, p := range profiles {
		incomingNames[i] = p.Name
		incomingProfs[p.Name] = p
	}

	var existingProfiles []*mdmlab.MDMWindowsConfigProfile
	...
	for n := range mdm.MDMlabReservedProfileNames() {
	...
	if len(incomingNames) > 0 {
		var newlyInsertedProfs []*mdmlab.MDMWindowsConfigProfile
...
func (ds *Datastore) bulkSetPendingMDMWindowsHostProfilesDB(
	...
	var (
		pargs            []any
		profilesToInsert = make(map[string]*mdmlab.MDMWindowsProfilePayload)
		psb              strings.Builder
		batchCount       int
	)
	...
	executeUpsertBatch := func(valuePart string, args []any) error {
		...
		var existingProfiles []mdmlab.MDMWindowsProfilePayload
	...
	for _, p := range profilesToInstall {
		profilesToInsert[fmt.Sprintf("%s\n%s", p.ProfileUUID, p.HostUUID)] = &mdmlab.MDMWindowsProfilePayload{
...
func (ds *Datastore) GetHostMDMWindowsProfiles(ctx context.Context, hostUUID string) ([]mdmlab.HostMDMWindowsProfile, error) {
	stmt := fmt.Sprintf(`
SELECT
	profile_uuid,
...
host_uuid = ? AND profile_name NOT IN(?) AND NOT (operation_type = '%s' AND COALESCE(status, '%s') IN('%s', '%s'))`,
	...
	stmt, args, err := sqlx.In(stmt, hostUUID, mdm.ListMDMlabReservedWindowsProfileNames())
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "building in statement")
	}

	var profiles []mdmlab.HostMDMWindowsProfile
...
func (ds *Datastore) WipeHostViaWindowsMDM(ctx context.Context, host *mdmlab.Host, cmd *mdmlab.MDMWindowsCommand) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if _, err := tx.ExecContext(ctx, stmt, host.ID, cmd.CommandUUID, host.MDMlabPlatform()); err != nil {
