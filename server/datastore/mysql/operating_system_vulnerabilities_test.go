...
func testListOSVulnerabilitiesByOS(t *testing.T, ds *Datastore) {
	...
	for _, v := range vulns {
		_, err := ds.InsertOSVulnerability(ctx, v, mdmlab.MSRCSource)
		require.NoError(t, err)
	}

	t.Run("returns matching", func(t *testing.T) {
		expected := []mdmlab.OSVulnerability{
			{CVE: "cve-1", OSID: 1, ResolvedInVersion: ptr.String("1.2.3"), Source: mdmlab.MSRCSource},
			{CVE: "cve-3", OSID: 1, ResolvedInVersion: ptr.String("10.14.2"), Source: mdmlab.MSRCSource},
			{CVE: "cve-2", OSID: 1, ResolvedInVersion: ptr.String("8.123.1"), Source: mdmlab.MSRCSource},
		}
...
func testListVulnsByOsNameAndVersion(t *testing.T, ds *Datastore) {
	...
	_, err = ds.InsertOSVulnerabilities(ctx, vulns, mdmlab.MSRCSource)
	require.NoError(t, err)
...
func testInsertOSVulnerabilities(t *testing.T, ds *Datastore) {
	...
	c, err := ds.InsertOSVulnerabilities(ctx, vulns, mdmlab.MSRCSource)
	require.NoError(t, err)
	require.Equal(t, int64(3), c)

	expected := []mdmlab.OSVulnerability{
		{CVE: "cve-1", OSID: 1, Source: mdmlab.MSRCSource},
		{CVE: "cve-3", OSID: 1, Source: mdmlab.MSRCSource},
		{CVE: "cve-2", OSID: 1, Source: mdmlab.MSRCSource},
	}
...
func testInsertOSVulnerability(t *testing.T, ds *Datastore) {
	...
	expected.Source = mdmlab.MSRCSource
...
func testDeleteOSVulnerabilities(t *testing.T, ds *Datastore) {
	...
	toDelete := []mdmlab.OSVulnerability{
		{CVE: "cve-2", OSID: 1},
	}
	...
	actual, err := ds.ListOSVulnerabilitiesByOS(ctx, 1)
	require.NoError(t, err)
	require.ElementsMatch(t, []mdmlab.OSVulnerability{
		{CVE: "cve-1", OSID: 1, Source: mdmlab.MSRCSource},
		{CVE: "cve-3", OSID: 1, Source: mdmlab.MSRCSource},
	}, actual)
}
...
func testDeleteOutOfDateOSVulnerabilities(t *testing.T, ds *Datastore) {
	...
	oldVuln := mdmlab.OSVulnerability{
		CVE: "cve-1", OSID: 1,
	}

	newVuln := mdmlab.OSVulnerability{
		CVE: "cve-2", OSID: 1,
	}

	_, err := ds.InsertOSVulnerability(ctx, oldVuln, mdmlab.NVDSource)
	require.NoError(t, err)

	_, err = ds.writer(ctx).ExecContext(ctx, "UPDATE operating_system_vulnerabilities SET updated_at = ?", yesterday)
	require.NoError(t, err)

	_, err = ds.InsertOSVulnerability(ctx, newVuln, mdmlab.NVDSource)
	require.NoError(t, err)

	// Delete out of date vulns
	err = ds.DeleteOutOfDateOSVulnerabilities(ctx, mdmlab.NVDSource, 2*time.Hour)
	require.NoError(t, err)

	actual, err := ds.ListOSVulnerabilitiesByOS(ctx, 1)
	require.NoError(t, err)
	require.Len(t, actual, 1)
	require.ElementsMatch(t, []mdmlab.OSVulnerability{newVuln}, actual)
}
