...
func addRequestTypeFilter(stmt string, filter *mdmlab.MDMCommandFilters, params []interface{}) (string, []interface{}) {
...
func (ds *Datastore) getMDMCommand(ctx context.Context, q sqlx.QueryerContext, cmdUUID string) (*mdmlab.MDMCommand, error) {
	stmt, _ := getCombinedMDMCommandsQuery(ds, "")
	stmt += "command_uuid = ?"

	var cmd mdmlab.MDMCommand
...
func (ds *Datastore) BatchSetMDMProfiles(ctx context.Context, tmID *uint, macProfiles []*mdmlab.MDMAppleConfigProfile,
	winProfiles []*mdmlab.MDMWindowsConfigProfile, macDeclarations []*mdmlab.MDMAppleDeclaration) (updates mdmlab.MDMProfilesUpdates,
	err error,
) {
...
func (ds *Datastore) ListMDMConfigProfiles(ctx context.Context, teamID *uint, opt mdmlab.ListOptions) ([]*mdmlab.MDMConfigProfilePayload, *mdmlab.PaginationMetadata, error) {
	// this lists custom profiles, it explicitly filters out the mdmlab-reserved
	// ones (reserved identifiers for Apple profiles, reserved names for Windows).

	var profs []*mdmlab.MDMConfigProfilePayload
...
`
	...
	mdmlabIdentsMap := mobileconfig.MDMlabPayloadIdentifiers()
	mdmlabIdentifiers := make([]string, 0, len(mdmlabIdentsMap))
	for k := range mdmlabIdentsMap {
		mdmlabIdentifiers = append(mdmlabIdentifiers, k)
	}
	mdmlabNamesMap := mdm.MDMlabReservedProfileNames()
	mdmlabNames := make([]string, 0, len(mdmlabNamesMap))
	for k := range mdmlabNamesMap {
		mdmlabNames = append(mdmlabNames, k)
	}

	args := []any{globalOrTeamID, mdmlabIdentifiers, globalOrTeamID, mdmlabNames, globalOrTeamID, mdmlabNames}
	...
	if opt.IncludeMetadata {
		metaData = &mdmlab.PaginationMetadata{HasPreviousResults: opt.Page > 0}
	...
	profMap := make(map[string]*mdmlab.MDMConfigProfilePayload, len(profs))
	for _, prof := range profs {
...
func (ds *Datastore) listProfileLabelsForProfiles(ctx context.Context, winProfUUIDs, macProfUUIDs, macDeclUUIDs []string) ([]mdmlab.ConfigurationProfileLabel, error) {
	// load the labels associated with those profiles
	const labelsStmt = `
SELECT
...
`
	...
	var labels []mdmlab.ConfigurationProfileLabel
...
func (ds *Datastore) bulkSetPendingMDMHostProfilesDB(
	...
	if len(profileUUIDs) > 0 {
		...
		for _, puid := range profileUUIDs {
			if strings.HasPrefix(puid, mdmlab.MDMAppleProfileUUIDPrefix) { //nolint:gocritic // ignore ifElseChain
				macProfUUIDs = append(macProfUUIDs, puid)
			} else if strings.HasPrefix(puid, mdmlab.MDMAppleDeclarationUUIDPrefix) {
				hasAppleDecls = true
			} else {
...
func (ds *Datastore) UpdateHostMDMProfilesVerification(ctx context.Context, host *mdmlab.Host, toVerify, toFail, toRetry []string) error {
...
func setMDMProfilesRetryDB(ctx context.Context, tx sqlx.ExtContext, host *mdmlab.Host, identifiersOrNames []string) error {
...
func setMDMProfilesFailedDB(ctx context.Context, tx sqlx.ExtContext, host *mdmlab.Host, identifiersOrNames []string) error {
	if len(identifiersOrNames) == 0 {
		return nil
	}
...
WHERE
	...
	args := []interface{}{
		...
		mdmlab.HostMDMProfileDetailFailedWasVerifying,
		mdmlab.HostMDMProfileDetailFailedWasVerified,
...
func setMDMProfilesVerifiedDB(ctx context.Context, tx sqlx.ExtContext, host *mdmlab.Host, identifiersOrNames []string) error {
	if len(identifiersOrNames) == 0 {
		return nil
	}
...
func (ds *Datastore) GetHostMDMProfilesExpectedForVerification(ctx context.Context, host *mdmlab.Host) (map[string]*mdmlab.ExpectedMDMProfile, error) {
	var teamID uint
	if host.TeamID != nil {
		teamID = *host.TeamID
	}
...
func (ds *Datastore) getHostMDMWindowsProfilesExpectedForVerification(ctx context.Context, teamID, hostID uint) (map[string]*mdmlab.ExpectedMDMProfile, error) {
	stmt := `
...
HAVING
	...
	count_host_labels = 0
`
	var profiles []*mdmlab.ExpectedMDMProfile
	...
	byName := make(map[string]*mdmlab.ExpectedMDMProfile, len(profiles))
...
func (ds *Datastore) getHostMDMAppleProfilesExpectedForVerification(ctx context.Context, teamID, hostID uint) (map[string]*mdmlab.ExpectedMDMProfile, error) {
	stmt := `
-- profiles without labels
SELECT
...
`
	...
	var rows []*mdmlab.ExpectedMDMProfile
	...
	byIdentifier := make(map[string]*mdmlab.ExpectedMDMProfile, len(rows))
	for _, r := range rows {
...
func (ds *Datastore) GetHostMDMProfilesRetryCounts(ctx context.Context, host *mdmlab.Host) ([]mdmlab.HostMDMProfileRetryCount, error) {
	const darwinStmt = `
SELECT
	profile_identifier,
	retries
...
WHERE
	...
	var dest []mdmlab.HostMDMProfileRetryCount
...
func (ds *Datastore) GetHostMDMProfileRetryCountByCommandUUID(ctx context.Context, host *mdmlab.Host, cmdUUID string) (mdmlab.HostMDMProfileRetryCount, error) {
	const darwinStmt = `
SELECT
...
WHERE
	...
	case "windows":
		stmt = windowsStmt
	default:
		return mdmlab.HostMDMProfileRetryCount{}, fmt.Errorf("unsupported platform %s", host.Platform)
	}
...
func batchSetProfileLabelAssociationsDB(
	...
	labelsToInsert := make(map[string]*mdmlab.ConfigurationProfileLabel, len(profileLabels))
...
func (ds *Datastore) MDMGetEULAMetadata(ctx context.Context) (*mdmlab.MDMEULA, error) {
...
func (ds *Datastore) MDMGetEULABytes(ctx context.Context, token string) (*mdmlab.MDMEULA, error) {
...
func (ds *Datastore) MDMInsertEULA(ctx context.Context, eula *mdmlab.MDMEULA) error {
...
func (ds *Datastore) GetHostCertAssociationsToExpire(ctx context.Context, expiryDays, limit int) ([]mdmlab.SCEPIdentityAssociation, error) {
...
SELECT
    ...
    COALESCE(MAX(hm.mdmlab_enroll_ref), '') AS enroll_reference,
...
LIMIT ?`, expiryDays, limit)
	...
	var uuids []mdmlab.SCEPIdentityAssociation
...
func (ds *Datastore) SetCommandForPendingSCEPRenewal(ctx context.Context, assocs []mdmlab.SCEPIdentityAssociation, cmdUUID string) error {
	if len(assocs) == 0 {
		return nil
	}
...
func (ds *Datastore) GetHostMDMProfileInstallStatus(ctx context.Context, hostUUID string, profUUID string) (mdmlab.MDMDeliveryStatus, error) {
...
`, table, column)
	...
	if err := sqlx.GetContext(ctx, ds.writer(ctx), &status, selectStmt, mdmlab.MDMDeliveryPending, mdmlab.MDMOperationTypeInstall, hostUUID, profUUID); err != nil {
		if err == sql.ErrNoRows {
...
func getTableAndColumnNameForHostMDMProfileUUID(profUUID string) (table, column string, err error) {
	switch {
	case strings.HasPrefix(profUUID, mdmlab.MDMAppleDeclarationUUIDPrefix):
		return "host_mdm_apple_declarations", "declaration_uuid", nil
	case strings.HasPrefix(profUUID, mdmlab.MDMAppleProfileUUIDPrefix):
		return "host_mdm_apple_profiles", "profile_uuid", nil
	case strings.HasPrefix(profUUID, mdmlab.MDMWindowsProfileUUIDPrefix):
...
func (ds *Datastore) AreHostsConnectedToMDMlabMDM(ctx context.Context, hosts []*mdmlab.Host) (map[string]bool, error) {
	var (
		appleUUIDs []any
		winUUIDs   []any
	)
	...
	setConnectedUUIDs := func(stmt string, uuids []any, mp map[string]bool) error {
		...
		if len(uuids) > 0 {
			...
			if err != nil {
				return ctxerr.Wrap(ctx, err, "retrieving hosts connected to mdmlab")
			}
		}
	...
	// `connected_to_mdmlab` condition, and any relevant filters.
	const appleStmt = `
	  SELECT ne.id
...
func (ds *Datastore) IsHostConnectedToMDMlabMDM(ctx context.Context, host *mdmlab.Host) (bool, error) {
	mp, err := ds.AreHostsConnectedToMDMlabMDM(ctx, []*mdmlab.Host{host})
	if err != nil {
		return false, ctxerr.Wrap(ctx, err, "finding if host is connected to MDMlab MDM")
	}
