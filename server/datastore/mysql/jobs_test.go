...
func testQueueAndProcessJobs(t *testing.T, ds *Datastore) {
	...
	j1 := &mdmlab.Job{Name: "j1", State: mdmlab.JobStateQueued}
	j2 := &mdmlab.Job{Name: "j2", State: mdmlab.JobStateQueued, NotBefore: time.Now().Add(time.Hour)}
	...
	j1.State = mdmlab.JobStateSuccess
