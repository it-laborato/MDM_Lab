...
func upsertCarveDB(ctx context.Context, writer sqlx.ExecerContext, metadata *mdmlab.CarveMetadata) (int64, error) {
...
func (ds *Datastore) NewCarve(ctx context.Context, metadata *mdmlab.CarveMetadata) (*mdmlab.CarveMetadata, error) {
...
func (ds *Datastore) UpdateCarve(ctx context.Context, metadata *mdmlab.CarveMetadata) error {
	return updateCarveDB(ctx, ds.writer(ctx), metadata)
}

func updateCarveDB(ctx context.Context, exec sqlx.ExecerContext, metadata *mdmlab.CarveMetadata) error {
	stmt := `
...
func (ds *Datastore) Carve(ctx context.Context, carveId int64) (*mdmlab.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata
		WHERE id = ?`,
	...
	var metadata mdmlab.CarveMetadata
...
func (ds *Datastore) CarveBySessionId(ctx context.Context, sessionId string) (*mdmlab.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata
		WHERE session_id = ?`,
...
func (ds *Datastore) CarveByName(ctx context.Context, name string) (*mdmlab.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata
		WHERE name = ?`,
...
func (ds *Datastore) ListCarves(ctx context.Context, opt mdmlab.CarveListOptions) ([]*mdmlab.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata`,
		carveSelectFields,
...
func (ds *Datastore) NewBlock(ctx context.Context, metadata *mdmlab.CarveMetadata, blockId int64, data []byte) error {
...
func (ds *Datastore) GetBlock(ctx context.Context, metadata *mdmlab.CarveMetadata, blockId int64) ([]byte, error) {
	stmt := `
		SELECT data
		FROM carve_blocks
