...
func (ds *Datastore) UpsertMaintainedApp(ctx context.Context, app *mdmlab.MaintainedApp) (*mdmlab.MaintainedApp, error) {
	const upsertStmt = `
INSERT INTO
	mdmlab_library_apps (
...
func (ds *Datastore) GetMaintainedAppByID(ctx context.Context, appID uint) (*mdmlab.MaintainedApp, error) {
	const stmt = `
SELECT
	fla.id,
	fla.name,
...
func (ds *Datastore) ListAvailableMDMlabMaintainedApps(ctx context.Context, teamID *uint, opt mdmlab.ListOptions) ([]mdmlab.MaintainedApp, *mdmlab.PaginationMetadata, error) {
	stmt := `SELECT fla.id, fla.name, fla.version, fla.platform, fla.updated_at FROM mdmlab_library_apps fla `
	var args []any
	...
	if err := sqlx.GetContext(ctx, dbReader, &counts, getAppsCountStmt, args...); err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "get mdmlab maintained apps count")
	}

	stmtPaged, args := appendListOptionsWithCursorToSQL(stmt, args, &opt)

	var avail []mdmlab.MaintainedApp
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &avail, stmtPaged, args...); err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "selecting available mdmlab managed apps")
	}

	meta := &mdmlab.PaginationMetadata{HasPreviousResults: opt.Page > 0, TotalResults: uint(counts)} //nolint:gosec // dismiss G115
