...
func testGetQueryResultRows(t *testing.T, ds *Datastore) {
	...
	query1Rows := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host.ID,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), query1Rows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert Result Row for different Scheduled Query
	query2 := test.NewQuery(t, ds, nil, "New Query 2", "SELECT 1", user.ID, true)
	query2Rows := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query2.ID,
			HostID:      host.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), query2Rows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	results, err := ds.QueryResultRows(context.Background(), query.ID, mdmlab.TeamFilter{User: test.UserAdmin})
	require.NoError(t, err)
	...
	results, err = ds.QueryResultRows(context.Background(), query2.ID, mdmlab.TeamFilter{User: test.UserAdmin})
	require.NoError(t, err)
...
func testGetQueryResultRowsForHost(t *testing.T, ds *Datastore) {
	...
	host1ResultRows := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), host1ResultRows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert 1 Result Row for Query1 Host2
	host2ResultRows := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host2.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host2ResultRows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testQueryResultRowsTeamFilter(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	observerTeam, err := ds.NewTeam(context.Background(), &mdmlab.Team{
		Name: "observerTeam",
	})
	require.NoError(t, err)

	teamUser, err := ds.NewUser(context.Background(), &mdmlab.User{
		Password:   []byte("foo"),
		Salt:       "bar",
		Name:       "teamUser",
	...
	globalRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), globalRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	teamRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      teamHost.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), teamRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	observerTeamRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), observerTeamRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testCountResultsForQuery(t *testing.T, ds *Datastore) {
	...
	host1ResultRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query1.ID,
			HostID:      host.ID,
			LastFetched: mockTime,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), host1ResultRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert Nil Result Row for Query1, nil data rows are not counted
	host2ResultRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query1.ID,
			HostID:      host2.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host2ResultRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert 5 Result Rows for Query2
	resultRow2 := &mdmlab.ScheduledQueryResultRow{
		QueryID:     query2.ID,
		HostID:      host.ID,
		LastFetched: mockTime,
		Data: ptr.RawMessage([]byte(`{
	...
	var resultRows []*mdmlab.ScheduledQueryResultRow
	for i := 0; i < 5; i++ {
		resultRows = append(resultRows, resultRow2)
	}

	err = ds.OverwriteQueryResultRows(context.Background(), resultRows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testCountResultsForQueryAndHost(t *testing.T, ds *Datastore) {
	...
	host1Query2 := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query2.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host1Query2, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
	...
	host3ResultRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query2.ID,
			HostID:      host3.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host3ResultRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testOverwriteQueryResultRows(t *testing.T, ds *Datastore) {
	...
	initialRow := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host.ID,
			LastFetched: mockTime,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), initialRow, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Overwrite Result Rows with new data
	newMockTime := mockTime.Add(2 * time.Minute)
	overwriteRows := []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host.ID,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), overwriteRows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testQueryResultRowsDoNotExceedMaxRows(t *testing.T, ds *Datastore) {
	...
	// Generate max rows -1
	maxRows := mdmlab.DefaultMaxQueryReportRows - 1
	maxMinusOneRows := make([]*mdmlab.ScheduledQueryResultRow, maxRows)
	for i := 0; i < maxRows; i++ {
		maxMinusOneRows[i] = &mdmlab.ScheduledQueryResultRow{
			QueryID:     query.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
			Data:        ptr.RawMessage([]byte(`{"model": "USB Mouse", "vendor": "Logitech"}`)),
	...
	err := ds.OverwriteQueryResultRows(context.Background(), maxMinusOneRows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Add an empty data rows which do not count towards the max
	err = ds.OverwriteQueryResultRows(context.Background(), []*mdmlab.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host2.ID,
			LastFetched: mockTime,
	...
	require.NoError(t, err)
	require.Equal(t, mdmlab.DefaultMaxQueryReportRows, count)
	...
	rows := mdmlab.DefaultMaxQueryReportRows + 50
	largeBatchRows := make([]*mdmlab.ScheduledQueryResultRow, rows)
	for i := 0; i < rows; i++ {
		largeBatchRows[i] = &mdmlab.ScheduledQueryResultRow{
			QueryID:     query2.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
			Data:        ptr.RawMessage([]byte(`{"model": "USB Mouse", "vendor": "Logitech"}`)),
	...
	err = ds.OverwriteQueryResultRows(context.Background(), largeBatchRows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Len(t, allResults, mdmlab.DefaultMaxQueryReportRows)
...
func testQueryResultRows(t *testing.T, ds *Datastore) {
	...
	filter := mdmlab.TeamFilter{User: user, IncludeObserver: true}
...
func testCleanupQueryResultRows(t *testing.T, ds *Datastore) {
	...
	rows := []*mdmlab.ScheduledQueryResultRow{
		{
	...
	err = ds.OverwriteQueryResultRows(context.Background(), rows, mdmlab.DefaultMaxQueryReportRows)
	require.NoError(t, err)
	...
	results, err := ds.QueryResultRows(context.Background(), queryNoDiscard.ID, mdmlab.TeamFilter{User: user})
	require.NoError(t, err)
	require.Len(t, results, 2)

	// Verify that the rows with discard data set to true are removed
	results, err = ds.QueryResultRows(context.Background(), queryDiscardTrue.ID, mdmlab.TeamFilter{User: user})
	require.NoError(t, err)
	require.Len(t, results, 0)
}
