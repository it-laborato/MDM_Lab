...
func (ds *Datastore) ApplyLabelSpecs(ctx context.Context, specs []*mdmlab.LabelSpec) (err error) {
	...
	`
		...
		for _, s := range specs {
			...
			if s.LabelType == mdmlab.LabelTypeBuiltIn ||
				s.LabelMembershipType != mdmlab.LabelMembershipTypeManual {
...
func (ds *Datastore) GetLabelSpecs(ctx context.Context) ([]*mdmlab.LabelSpec, error) {
	var specs []*mdmlab.LabelSpec
	// Get basic specs
	...
	for _, spec := range specs {
		if spec.LabelType != mdmlab.LabelTypeBuiltIn &&
			spec.LabelMembershipType == mdmlab.LabelMembershipTypeManual {
...
func (ds *Datastore) GetLabelSpec(ctx context.Context, name string) (*mdmlab.LabelSpec, error) {
	var specs []*mdmlab.LabelSpec
	query := `
...
func (ds *Datastore) getLabelHostnames(ctx context.Context, label *mdmlab.LabelSpec) error {
	sql := `
		SELECT hostname
		FROM hosts
		WHERE id IN
...
// NewLabel creates a new mdmlab.Label
func (ds *Datastore) NewLabel(ctx context.Context, label *mdmlab.Label, opts ...mdmlab.OptionalArg) (*mdmlab.Label, error) {
	query := `
	INSERT INTO labels (
		name,
		description,
...
func (ds *Datastore) SaveLabel(ctx context.Context, label *mdmlab.Label, teamFilter mdmlab.TeamFilter) (*mdmlab.Label, []uint, error) {
...
// DeleteLabel deletes a mdmlab.Label
func (ds *Datastore) DeleteLabel(ctx context.Context, name string) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		_, err = tx.ExecContext(ctx, `DELETE FROM pack_targets WHERE type=? AND target_id=?`, mdmlab.TargetLabel, labelID)
		if err != nil {
...
// Label returns a mdmlab.Label identified by lid if one exists.
func (ds *Datastore) Label(ctx context.Context, lid uint, teamFilter mdmlab.TeamFilter) (*mdmlab.Label, []uint, error) {
	return ds.labelDB(ctx, lid, teamFilter, ds.reader(ctx))
}

func (ds *Datastore) labelDB(ctx context.Context, lid uint, teamFilter mdmlab.TeamFilter, q sqlx.QueryerContext) (*mdmlab.Label, []uint, error) {
	stmt := fmt.Sprintf(`
		SELECT
		       l.*,
	...
	var hostIDs []uint
	if label.LabelMembershipType == mdmlab.LabelMembershipTypeManual && label.HostCount > 0 {
...
// ListLabels returns all labels limited or sorted by mdmlab.ListOptions.
// MatchQuery not supported
func (ds *Datastore) ListLabels(ctx context.Context, filter mdmlab.TeamFilter, opt mdmlab.ListOptions) ([]*mdmlab.Label, error) {
	if opt.After != "" {
		return nil, &mdmlab.BadRequestError{Message: "parameter 'after' is not supported"}
	}
	if opt.MatchQuery != "" {
		return nil, &mdmlab.BadRequestError{Message: "parameter 'query' is not supported"}
	}
...
func platformForHost(host *mdmlab.Host) string {
...
func (ds *Datastore) LabelQueriesForHost(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
	var rows *sql.Rows
	var err error
	platform := platformForHost(host)
	query := `SELECT id, query FROM labels WHERE platform = ? OR platform = '' AND label_membership_type = ?`
	rows, err = ds.reader(ctx).QueryContext(ctx, query, platform, mdmlab.LabelMembershipTypeDynamic)
...
func (ds *Datastore) RecordLabelQueryExecutions(ctx context.Context, host *mdmlab.Host, results map[uint]*bool, updated time.Time, deferredSaveHost bool) error {
...
// ListLabelsForHost returns a list of mdmlab.Label for a given host id.
func (ds *Datastore) ListLabelsForHost(ctx context.Context, hid uint) ([]*mdmlab.Label, error) {
	sqlStatement := `
...
// ListHostsInLabel returns a list of mdmlab.Host that are associated
// with mdmlab.Label referenced by Label ID
func (ds *Datastore) ListHostsInLabel(ctx context.Context, filter mdmlab.TeamFilter, lid uint, opt mdmlab.HostListOptions) ([]*mdmlab.Host, error) {
	queryFmt := `
    SELECT
      h.id,
      h.osquery_host_id,
...
func (ds *Datastore) applyHostLabelFilters(ctx context.Context, filter mdmlab.TeamFilter, lid uint, query string, opt mdmlab.HostListOptions) (string, []interface{}, error) {
...
func (ds *Datastore) CountHostsInLabel(ctx context.Context, filter mdmlab.TeamFilter, lid uint, opt mdmlab.HostListOptions) (int, error) {
...
func (ds *Datastore) ListUniqueHostsInLabels(ctx context.Context, filter mdmlab.TeamFilter, labels []uint) ([]*mdmlab.Host, error) {
	if len(labels) == 0 {
...
func (ds *Datastore) searchLabelsWithOmits(ctx context.Context, filter mdmlab.TeamFilter, query string, omit ...uint) ([]*mdmlab.Label, error) {
...
func (ds *Datastore) addAllHostsLabelToList(ctx context.Context, filter mdmlab.TeamFilter, labels []*mdmlab.Label, omit ...uint) ([]*mdmlab.Label, error) {
	sql := fmt.Sprintf(`
			SELECT *,
				(SELECT COUNT(1)
					FROM label_membership lm JOIN hosts h ON (lm.host_id = h.id)
	...
	var allHosts mdmlab.Label
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &allHosts, sql, mdmlab.LabelTypeBuiltIn); err != nil {
...
func (ds *Datastore) searchLabelsDefault(ctx context.Context, filter mdmlab.TeamFilter, omit ...uint) ([]*mdmlab.Label, error) {
	sql := fmt.Sprintf(`
			SELECT *,
				(SELECT COUNT(1)
					FROM label_membership lm JOIN hosts h ON (lm.host_id = h.id)
...
// SearchLabels performs wildcard searches on mdmlab.Label name
func (ds *Datastore) SearchLabels(ctx context.Context, filter mdmlab.TeamFilter, query string, omit ...uint) ([]*mdmlab.Label, error) {
	transformedQuery := transformQuery(query)
...
func (ds *Datastore) LabelsByName(ctx context.Context, names []string) (map[string]*mdmlab.Label, error) {
	if len(names) == 0 {
		return map[string]*mdmlab.Label{}, nil
	}
	...
	result := make(map[string]*mdmlab.Label, len(labels))
	for _, label := range labels {
...
func (ds *Datastore) LabelsSummary(ctx context.Context) ([]*mdmlab.LabelSummary, error) {
	labelsSummary := []*mdmlab.LabelSummary{}
