...
func testUpsertSecretVariables(t *testing.T, ds *Datastore) {
	...
	createExpectedSecrets := func() []mdmlab.SecretVariable {
		secrets := make([]mdmlab.SecretVariable, 0, len(secretMap))
		for name, value := range secretMap {
			secrets = append(secrets, mdmlab.SecretVariable{Name: name, Value: value})
		}
		return secrets
	}
...
func testValidateEmbeddedSecrets(t *testing.T, ds *Datastore) {
	noSecrets := `
This document contains to mdmlab secrets.
...
func testExpandEmbeddedSecrets(t *testing.T, ds *Datastore) {
	noSecrets := `
This document contains no mdmlab secrets.
$FLEET_VAR_XX $HOSTNAME ${SOMETHING_ELSE}
`
