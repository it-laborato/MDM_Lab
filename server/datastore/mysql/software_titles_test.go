...
func testSoftwareSyncHostsSoftwareTitles(t *testing.T, ds *Datastore) {
	...
	cmpNameVersionCount := func(want, got []mdmlab.SoftwareTitleListResult) {
		cmp := make([]mdmlab.SoftwareTitleListResult, len(got))
		for i, sw := range got {
			cmp[i] = mdmlab.SoftwareTitleListResult{Name: sw.Name, HostsCount: sw.HostsCount}
		}
	...
	globalOpts := mdmlab.SoftwareTitleListOptions{ListOptions: mdmlab.ListOptions{OrderKey: "hosts_count", OrderDirection: mdmlab.OrderDescending}}
	globalCounts := listSoftwareTitlesCheckCount(t, ds, 2, 2, globalOpts)

	want := []mdmlab.SoftwareTitleListResult{
	...
	allSw := listSoftwareTitlesCheckCount(t, ds, 1, 1, mdmlab.SoftwareTitleListOptions{})
	want = []mdmlab.SoftwareTitleListResult{
		{Name: "foo", HostsCount: 2},
	}
	...
	team1Opts := mdmlab.SoftwareTitleListOptions{
		TeamID:      ptr.Uint(team1.ID),
		ListOptions: mdmlab.ListOptions{OrderKey: "hosts_count", OrderDirection: mdmlab.OrderDescending},
	}
	...
	team2Opts := mdmlab.SoftwareTitleListOptions{
...
func testOrderSoftwareTitles(t *testing.T, ds *Datastore) {
	...
	installer1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		Title:           "installer1",
		Source:          "apps",
		InstallScript:   "echo",
	...
	installer2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		Title:           "installer2",
		Source:          "apps",
		InstallScript:   "echo",
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		...
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mdmlab.IPadOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	titles, _, _, err := ds.ListSoftwareTitles(ctx, mdmlab.SoftwareTitleListOptions{
		ListOptions: mdmlab.ListOptions{
			OrderKey:       "hosts_count",
			OrderDirection: mdmlab.OrderDescending,
		},
		TeamID: ptr.Uint(0),
	}, mdmlab.TeamFilter{User: &mdmlab.User{GlobalRole: ptr.String(mdmlab.RoleAdmin)}})
	require.NoError(t, err)
...
func listSoftwareTitlesCheckCount(t *testing.T, ds *Datastore, expectedListCount int, expectedFullCount int, opts mdmlab.SoftwareTitleListOptions) []mdmlab.SoftwareTitleListResult {
	titles, count, _, err := ds.ListSoftwareTitles(context.Background(), opts, mdmlab.TeamFilter{User: &mdmlab.User{GlobalRole: ptr.String(mdmlab.RoleAdmin)}})
	require.NoError(t, err)
...
func testTeamFilterSoftwareTitles(t *testing.T, ds *Datastore) {
	...
	userGlobalAdmin, err := ds.NewUser(ctx, &mdmlab.User{Name: "user1", Password: []byte("test"), Email: "test1@email.com", GlobalRole: ptr.String(mdmlab.RoleAdmin)})
	require.NoError(t, err)
	userTeam1Admin, err := ds.NewUser(ctx, &mdmlab.User{Name: "user2", Password: []byte("test"), Email: "test2@email.com", Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: team1.ID}, Role: mdmlab.RoleAdmin}}})
	require.NoError(t, err)
	userTeam2Admin, err := ds.NewUser(ctx, &mdmlab.User{Name: "user3", Password: []byte("test"), Email: "test3@email.com", Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: team2.ID}, Role: mdmlab.RoleAdmin}}})
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		...
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mdmlab.IOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		...
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_3", Platform: mdmlab.MacOSPlatform}, SelfService: true},
	}, ptr.Uint(0))
	require.NoError(t, err)
	...
	globalTeamFilter := mdmlab.TeamFilter{User: userGlobalAdmin, IncludeObserver: true}
	titles, count, _, err := ds.ListSoftwareTitles(
		context.Background(), mdmlab.SoftwareTitleListOptions{
			ListOptions: mdmlab.ListOptions{},
			TeamID:      nil,
		}, globalTeamFilter,
	)
	...
	assert.Equal(
		t,
		barTitle,
		mdmlab.SoftwareTitleListResult{
			ID:              title.ID,
			Name:            title.Name,
			Source:          title.Source,
	...
	team1TeamFilter := mdmlab.TeamFilter{User: userTeam1Admin, IncludeObserver: true}
	titles, count, _, err = ds.ListSoftwareTitles(
		context.Background(), mdmlab.SoftwareTitleListOptions{ListOptions: mdmlab.ListOptions{}, TeamID: &team1.ID}, team1TeamFilter,
	)
	...
	assert.Equal(t, titles[0], mdmlab.SoftwareTitleListResult{ID: title.ID, Name: title.Name, Source: title.Source, Browser: title.Browser, HostsCount: title.HostsCount, VersionsCount: title.VersionsCount, Versions: title.Versions, CountsUpdatedAt: title.CountsUpdatedAt})

	// Testing the team 2 user
	titles, count, _, err = ds.ListSoftwareTitles(context.Background(), mdmlab.SoftwareTitleListOptions{ListOptions: mdmlab.ListOptions{}, TeamID: &team2.ID}, mdmlab.TeamFilter{
		User:            userTeam2Admin,
	...
	titles, _, _, err = ds.ListSoftwareTitles(
		context.Background(), mdmlab.SoftwareTitleListOptions{ListOptions: mdmlab.ListOptions{}, SelfServiceOnly: true, TeamID: &team1.ID}, team1TeamFilter,
	)
	// installer1 is associated with team 1
	require.NoError(t, err)
	require.Len(t, titles, 1)
	...
	titles, _, _, err = ds.ListSoftwareTitles(context.Background(), mdmlab.SoftwareTitleListOptions{ListOptions: mdmlab.ListOptions{}, SelfServiceOnly: true, TeamID: &team2.ID}, mdmlab.TeamFilter{
		User:            userTeam2Admin,
		IncludeObserver: true,
	})
	require.NoError(t, err)
	...
	titles, _, _, err = ds.ListSoftwareTitles(context.Background(), mdmlab.SoftwareTitleListOptions{ListOptions: mdmlab.ListOptions{}, SelfServiceOnly: true, TeamID: ptr.Uint(0)}, mdmlab.TeamFilter{
...
func sortTitlesByName(titles []mdmlab.SoftwareTitleListResult) {
...
func testListSoftwareTitlesInstallersOnly(t *testing.T, ds *Datastore) {
	...
	titles, counts, _, err := ds.ListSoftwareTitles(ctx, mdmlab.SoftwareTitleListOptions{
		ListOptions: mdmlab.ListOptions{
			OrderKey:       "name",
			OrderDirection: mdmlab.OrderAscending,
		},
...
func testListSoftwareTitlesAvailableForInstallFilter(t *testing.T, ds *Datastore) {
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		...
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mdmlab.IPadOSPlatform}},
	}, nil)
	require.NoError(t, err)
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.example.vpp2",
	...
	require.NoError(t, err)
	vpp2, err := ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.example.vpp2",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mdmlab.IOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	vppPolicy, err := ds.NewTeamPolicy(ctx, mdmlab.PolicyNoTeamID, nil, mdmlab.PolicyPayload{
		Name:           "VPP Policy",
		Query:          "SELECT 1;",
...
func testListSoftwareTitlesAllTeams(t *testing.T, ds *Datastore) {
	...
	macOSInstallerNoTeam, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		Title:            "foobar",
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		...
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_canva", Platform: mdmlab.IOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "Canva", BundleIdentifier: "com.example.canva",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_canva", Platform: mdmlab.MacOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name: "Canva", BundleIdentifier: "com.example.canva",
		VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_app_canva", Platform: mdmlab.IPadOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
...
func testUploadedSoftwareExists(t *testing.T, ds *Datastore) {
	...
	tm, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "Team Foo"})
	require.NoError(t, err)
	user1 := test.NewUser(t, ds, "Alice", "alice@example.com", true)
...
func testListSoftwareTitlesVulnerabilityFilters(t *testing.T, ds *Datastore) {
	...
	globalUser := &mdmlab.User{GlobalRole: ptr.String(mdmlab.RoleAdmin)}

	tc := []struct {
		name           string
		opts           mdmlab.SoftwareTitleListOptions
	...
	assertTitles := func(t *testing.T, titles []mdmlab.SoftwareTitleListResult, expectedTitles []string) {
		t.Helper()
	...
	for _, tt := range tc {
		t.Run(tt.name, func(t *testing.T) {
			titles, _, _, err := ds.ListSoftwareTitles(ctx, tt.opts, mdmlab.TeamFilter{User: globalUser})
			if tt.err != nil {
				require.Error(t, err)
