...
func (ds *Datastore) SoftwareTitleByID(ctx context.Context, id uint, teamID *uint, tmFilter mdmlab.TeamFilter) (*mdmlab.SoftwareTitle, error) {
	var (
...
GROUP BY
	...
	var title mdmlab.SoftwareTitle
...
func (ds *Datastore) ListSoftwareTitles(
	...
	if opt.ListOptions.After != "" {
		return nil, 0, nil, mdmlab.NewInvalidArgumentError("after", "not supported for software titles")
	}

	if len(strings.Split(opt.ListOptions.OrderKey, ",")) > 1 {
		return nil, 0, nil, mdmlab.NewInvalidArgumentError("order_key", "multicolumn order key not supported for software titles")
	}
	...
	if (opt.MinimumCVSS > 0 || opt.MaximumCVSS > 0 || opt.KnownExploit) && !opt.VulnerableOnly {
		return nil, 0, nil, mdmlab.NewInvalidArgumentError("query", "min_cvss_score, max_cvss_score, and exploit can only be provided with vulnerable=true")
	}
	...
	if len(softwareList) == 0 {
		return nil, counts, &mdmlab.PaginationMetadata{}, nil
	}
	...
	for i, title := range softwareList {
		...
		if title.PackageName != nil {
			...
			title.SoftwarePackage = &mdmlab.SoftwarePackageOrApp{
		...
		if title.VPPAppAdamID != nil {
			...
			title.AppStoreApp = &mdmlab.SoftwarePackageOrApp{
	...
	var versions []mdmlab.SoftwareVersion
	...
	titles := make([]mdmlab.SoftwareTitleListResult, 0, len(softwareList))
...
func spliceSecondaryOrderBySoftwareTitlesSQL(stmt string, opts mdmlab.ListOptions) string {
	if opts.OrderKey == "" {
		return stmt
	}
...
func selectSoftwareTitlesSQL(opt mdmlab.SoftwareTitleListOptions) (string, []any) {
	stmt := `
SELECT
	st.id,
	st.name,
...
func (ds *Datastore) selectSoftwareVersionsSQL(titleIDs []uint, teamID *uint, tmFilter mdmlab.TeamFilter, withCounts bool) (
	string, []any, error,
) {
