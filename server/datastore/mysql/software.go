...
func softwareSliceToMap(softwareItems []mdmlab.Software) map[string]mdmlab.Software {
	result := make(map[string]mdmlab.Software, len(softwareItems))
...
func (ds *Datastore) UpdateHostSoftware(ctx context.Context, hostID uint, software []mdmlab.Software) (*mdmlab.UpdateHostSoftwareDBResult, error) {
...
func (ds *Datastore) UpdateHostSoftwareInstalledPaths(
	...
	mutationResults *mdmlab.UpdateHostSoftwareDBResult,
) error {
...
func (ds *Datastore) getHostSoftwareInstalledPaths(
	...
	[]mdmlab.HostSoftwareInstalledPath,
	error,
) {
	stmt := `
	...
	var result []mdmlab.HostSoftwareInstalledPath
...
func hostSoftwareInstalledPathsDelta(
	hostID uint,
	reported map[string]struct{},
	stored []mdmlab.HostSoftwareInstalledPath,
	hostSoftware []mdmlab.Software,
) (
	...
	sIDLookup := map[uint]mdmlab.Software{}
	for _, s := range hostSoftware {
		sIDLookup[s.ID] = s
	}

	sUnqStrLook := map[string]mdmlab.Software{}
	for _, s := range hostSoftware {
		sUnqStrLook[s.ToUniqueStr()] = s
	}

	iSPathLookup := make(map[string]mdmlab.HostSoftwareInstalledPath)
	for _, r := range stored {
		...
		key := fmt.Sprintf(
			"%s%s%s%s%s",
			r.InstalledPath, mdmlab.SoftwareFieldSeparator, r.TeamIdentifier, mdmlab.SoftwareFieldSeparator, s.ToUniqueStr(),
		)
		iSPathLookup[key] = r
	...
	for key := range reported {
		parts := strings.SplitN(key, mdmlab.SoftwareFieldSeparator, 3)
		...
		toInsert = append(toInsert, mdmlab.HostSoftwareInstalledPath{
			HostID:         hostID,
			SoftwareID:     s.ID,
...
func nothingChanged(current, incoming []mdmlab.Software, minLastOpenedAtDiff time.Duration) (
	map[string]mdmlab.Software, map[string]mdmlab.Software, bool,
) {
	// Process incoming software to ensure there are no duplicates, since the same software can be installed at multiple paths.
	incomingMap := make(map[string]mdmlab.Software, len(current)) // setting len(current) as the length since that should be the common case
...
func (ds *Datastore) ListSoftwareByHostIDShort(ctx context.Context, hostID uint) ([]mdmlab.Software, error) {
...
) (*mdmlab.UpdateHostSoftwareDBResult, error) {
	r := &mdmlab.UpdateHostSoftwareDBResult{}
...
func checkForDeletedInstalledSoftware(ctx context.Context, tx sqlx.ExtContext, deleted []mdmlab.Software, inserted []mdmlab.Software,
	hostID uint,
) error {
	...
	if len(deletedTitles) > 0 {
		installedTitles, err := getInstalledByMDMlabSoftwareTitles(ctx, tx, hostID)
		if err != nil {
			return err
		}
...
func (ds *Datastore) getExistingSoftware(
	ctx context.Context, current map[string]mdmlab.Software, incoming map[string]mdmlab.Software,
) (
	currentSoftware []softwareIDChecksum, incomingChecksumToSoftware map[string]mdmlab.Software,
	incomingChecksumToTitle map[string]mdmlab.SoftwareTitle, err error,
) {
	// Compute checksums for all incoming software, which we will use for faster retrieval, since checksum is a unique index
	incomingChecksumToSoftware = make(map[string]mdmlab.Software, len(current))
	...
	incomingChecksumToTitle = make(map[string]mdmlab.SoftwareTitle, len(newSoftware))
	if len(newSoftware) > 0 {
		...
		var existingSoftwareTitlesForNewSoftware []mdmlab.SoftwareTitle
...
func UniqueSoftwareTitleStr(values ...string) string {
	return strings.Join(values, mdmlab.SoftwareFieldSeparator)
}
...
func deleteUninstalledHostSoftwareDB(
	...
	hostID uint,
	currentMap map[string]mdmlab.Software,
	incomingMap map[string]mdmlab.Software,
) ([]mdmlab.Software, error) {
	var deletesHostSoftwareIDs []uint
	var deletedSoftware []mdmlab.Software
...
func computeRawChecksum(sw mdmlab.Software) ([]byte, error) {
...
func (ds *Datastore) insertNewInstalledHostSoftwareDB(
	...
	softwareChecksums map[string]mdmlab.Software,
	existingTitlesForNewSoftware map[string]mdmlab.SoftwareTitle,
) ([]mdmlab.Software, error) {
	var insertsHostSoftware []interface{}
	var insertedSoftware []mdmlab.Software
	...
	if len(softwareChecksums) > 0 {
		...
		for i := 0; i < len(keys); i += softwareInsertBatchSize {
			...
			newTitlesNeeded := make(map[string]mdmlab.SoftwareTitle)
			for j := start; j < end; j++ {
				checksum := keys[j]
...
func selectSoftwareSQL(opts mdmlab.SoftwareListOptions) (string, []interface{}, error) {
	ds := dialect.
...
func countSoftwareDB(
	...
	opts.ListOptions = mdmlab.ListOptions{
...
func (ds *Datastore) LoadHostSoftware(ctx context.Context, host *mdmlab.Host, includeCVEScores bool) error {
	opts := mdmlab.SoftwareListOptions{
		HostID:           &host.ID,
	...
	pathSignatureInformation := make(map[uint][]mdmlab.PathSignatureInformation)
	for _, ip := range installedPaths {
		installedPathsList[ip.SoftwareID] = append(installedPathsList[ip.SoftwareID], ip.InstalledPath)
		pathSignatureInformation[ip.SoftwareID] = append(pathSignatureInformation[ip.SoftwareID], mdmlab.PathSignatureInformation{
			InstalledPath:  ip.InstalledPath,
			TeamIdentifier: ip.TeamIdentifier,
		})
	}

	host.Software = make([]mdmlab.HostSoftwareEntry, 0, len(software))
	for _, s := range software {
		host.Software = append(host.Software, mdmlab.HostSoftwareEntry{
			Software:                 s,
...
func (si *softwareIterator) Value() (*mdmlab.Software, error) {
	dest := mdmlab.Software{}
...
// iteration using the returned mdmlab.SoftwareIterator is done.
func (ds *Datastore) AllSoftwareIterator(
	ctx context.Context,
	query mdmlab.SoftwareIterQueryOptions,
) (mdmlab.SoftwareIterator, error) {
	if !query.IsValid() {
...
func (ds *Datastore) UpsertSoftwareCPEs(ctx context.Context, cpes []mdmlab.SoftwareCPE) (int64, error) {
	var args []interface{}
...
func (ds *Datastore) DeleteSoftwareCPEs(ctx context.Context, cpes []mdmlab.SoftwareCPE) (int64, error) {
	if len(cpes) == 0 {
		return 0, nil
	}
...
func (ds *Datastore) ListSoftwareCPEs(ctx context.Context) ([]mdmlab.SoftwareCPE, error) {
	var result []mdmlab.SoftwareCPE
...
func (ds *Datastore) ListSoftware(ctx context.Context, opt mdmlab.SoftwareListOptions) ([]mdmlab.Software, *mdmlab.PaginationMetadata, error) {
	...
	if opt.ListOptions.IncludeMetadata {
		...
		metaData = &mdmlab.PaginationMetadata{HasPreviousResults: opt.ListOptions.Page > 0}
...
func (ds *Datastore) CountSoftware(ctx context.Context, opt mdmlab.SoftwareListOptions) (int, error) {
...
func (ds *Datastore) DeleteSoftwareVulnerabilities(ctx context.Context, vulnerabilities []mdmlab.SoftwareVulnerability) error {
...
func (ds *Datastore) DeleteOutOfDateVulnerabilities(ctx context.Context, source mdmlab.VulnerabilitySource, duration time.Duration) error {
...
func (ds *Datastore) SoftwareByID(ctx context.Context, id uint, teamID *uint, includeCVEScores bool, tmFilter *mdmlab.TeamFilter) (*mdmlab.Software, error) {
...
func (ds *Datastore) HostVulnSummariesBySoftwareIDs(ctx context.Context, softwareIDs []uint) ([]mdmlab.HostVulnerabilitySummary, error) {
	stmt := `
		SELECT DISTINCT
			h.id,
			h.hostname,
	...
	var result []mdmlab.HostVulnerabilitySummary
	...
	for _, r := range qR {
		...
		mapped := mdmlab.HostVulnerabilitySummary{
			ID:          r.HostID,
			Hostname:    r.HostName,
...
// ** DEPRECATED **
func (ds *Datastore) HostsByCVE(ctx context.Context, cve string) ([]mdmlab.HostVulnerabilitySummary, error) {
	stmt := `
		SELECT DISTINCT
				(h.id),
				h.hostname,
...
func (ds *Datastore) InsertCVEMeta(ctx context.Context, cveMeta []mdmlab.CVEMeta) error {
	query := `
...
func (ds *Datastore) ListSoftwareVulnerabilitiesByHostIDsSource(
	...
	hostIDs []uint,
	source mdmlab.VulnerabilitySource,
) (map[uint][]mdmlab.SoftwareVulnerability, error) {
	result := make(map[uint][]mdmlab.SoftwareVulnerability)
...
func (ds *Datastore) ListSoftwareForVulnDetection(
	ctx context.Context,
	filters mdmlab.VulnSoftwareFilter,
) ([]mdmlab.Software, error) {
	var result []mdmlab.Software
	var sqlstmt string
	var args []interface{}
...
func (ds *Datastore) ListCVEs(ctx context.Context, maxAge time.Duration) ([]mdmlab.CVEMeta, error) {
	var result []mdmlab.CVEMeta
...
func (ds *Datastore) ListHostSoftware(ctx context.Context, host *mdmlab.Host, opts mdmlab.HostSoftwareTitleListOptions) ([]*mdmlab.HostSoftwareWithInstaller, *mdmlab.PaginationMetadata, error) {
...
INNER JOIN software_cve scve ON scve.software_id = s.id
	...
	if opts.OnlyAvailableForInstall {
		// Get software that has a package/VPP installer but was not installed with MDMlab
...
`
	...
	namedArgs := map[string]any{
		"host_id":                   host.ID,
		"host_platform":             host.MDMlabPlatform(),
	...
	if opts.OnlyAvailableForInstall || (opts.IncludeAvailableForInstall && !opts.VulnerableOnly) {
		namedArgs["vpp_apps_platforms"] = mdmlab.VPPAppsPlatforms
		if mdmlab.IsLinux(host.Platform) {
			namedArgs["host_compatible_platforms"] = mdmlab.HostLinuxOSs
		} else {
			namedArgs["host_compatible_platforms"] = []string{host.MDMlabPlatform()}
		}
	...
	type hostSoftware struct {
		mdmlab.HostSoftwareWithInstaller
	...
	for _, hs := range hostSoftwareList {
		...
		if hs.PackageName != nil {
			...
			hs.SoftwarePackage = &mdmlab.SoftwarePackageOrApp{
				Name:        *hs.PackageName,
				Version:     version,
			...
			if hs.LastInstallInstallUUID != nil && *hs.LastInstallInstallUUID != "" {
				hs.SoftwarePackage.LastInstall = &mdmlab.HostSoftwareInstall{
			...
			if hs.LastUninstallScriptExecutionID != nil && *hs.LastUninstallScriptExecutionID != "" {
				hs.SoftwarePackage.LastUninstall = &mdmlab.HostSoftwareUninstall{
		...
		if hs.VPPAppAdamID != nil {
			...
			hs.AppStoreApp = &mdmlab.SoftwarePackageOrApp{
			...
			// promote the last install info to the proper destination fields
			if hs.LastInstallInstallUUID != nil && *hs.LastInstallInstallUUID != "" {
				hs.AppStoreApp.LastInstall = &mdmlab.HostSoftwareInstall{
	...
	if len(titleIDs) > 0 {
		...
		WHERE
			st.id IN (?)
`
		var installedVersions []*mdmlab.HostSoftwareInstalledVersion
		...
		bySoftwareID := make(map[uint]*mdmlab.HostSoftwareInstalledVersion, len(hostSoftwareList))
	...
	`
			...
			for _, path := range installedPaths {
				...
				if ver.Source == "apps" {
					ver.SignatureInformation = append(ver.SignatureInformation, mdmlab.PathSignatureInformation{
	...
	software := make([]*mdmlab.HostSoftwareWithInstaller, 0, len(hostSoftwareList))
	for _, hs := range hostSoftwareList {
		hs := hs
...
func (ds *Datastore) SetHostSoftwareInstallResult(ctx context.Context, result *mdmlab.HostSoftwareInstallResultPayload) error {
	const stmt = `
		UPDATE
...
func getInstalledByMDMlabSoftwareTitles(ctx context.Context, qc sqlx.QueryerContext, hostID uint) ([]mdmlab.SoftwareTitle, error) {
...
`
	...
	var titles []mdmlab.SoftwareTitle
