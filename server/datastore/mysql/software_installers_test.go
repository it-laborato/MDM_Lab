...
func testListPendingSoftwareInstalls(t *testing.T, ds *Datastore) {
	...
	err := ds.UpsertSecretVariables(ctx, []mdmlab.SecretVariable{
		{
			Name:  "RUBBER",
			Value: "DUCKY",
		},
	...
	tfr2, err := mdmlab.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerID2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:     "world",
	...
	tfr3, err := mdmlab.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerID3, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
...
func testSoftwareInstallRequests(t *testing.T, ds *Datastore) {
	...
	for tc, teamID := range cases {
		t.Run(tc, func(t *testing.T) {
			...
			installerID, titleID, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
				Title:           "foo",
				Source:          "bar",
				InstallScript:   "echo",
				TeamID:          teamID,
			...
			tag := "-pending_install"
			hostPendingInstall, err := ds.NewHost(ctx, &mdmlab.Host{
			...
			tag = "-failed_install"
			hostFailedInstall, err := ds.NewHost(ctx, &mdmlab.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			tag = "-installed"
			hostInstalled, err := ds.NewHost(ctx, &mdmlab.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			hostPendingUninstall, err := ds.NewHost(ctx, &mdmlab.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			hostFailedUninstall, err := ds.NewHost(ctx, &mdmlab.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			tag = "-uninstalled"
			hostUninstalled, err := ds.NewHost(ctx, &mdmlab.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			userTeamFilter := mdmlab.TeamFilter{
				User: &mdmlab.User{GlobalRole: ptr.String("admin")},
			}

			// list hosts with software install pending requests
			expectStatus := mdmlab.SoftwareInstallPending
			hosts, err := ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
			...
			expectStatus = mdmlab.SoftwarePending
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mdmlab.SoftwareInstallFailed
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mdmlab.SoftwareFailed
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mdmlab.SoftwareInstalled
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mdmlab.SoftwareUninstallPending
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mdmlab.SoftwareUninstallFailed
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			// list all hosts with the software title that shows up in host_software (after mdmlabd software query is run)
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mdmlab.HostListOptions{
				ListOptions:           mdmlab.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				TeamFilter:            teamID,
...
func testGetSoftwareInstallResult(t *testing.T, ds *Datastore) {
	...
	for _, tc := range []struct {
		name                    string
		expectedStatus          mdmlab.SoftwareInstallerStatus
	...
	}{
		{
			...
			expectedStatus:          mdmlab.SoftwareInstallPending,
		...
		},
		{
			...
			expectedStatus:          mdmlab.SoftwareInstallFailed,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedStatus == mdmlab.SoftwareInstallPending { // expect pending to be deleted
...
func testCleanupUnusedSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	tfr0, err := mdmlab.NewTempFileReader(ins0File, t.TempDir)
	require.NoError(t, err)
	assertExisting([]string{ins0})

	swi, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
...
func testBatchSetSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	// create a team
	team, err := ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name()})
	require.NoError(t, err)
	...
	assertSoftware := func(wantTitles []mdmlab.SoftwareTitle) {
		tmFilter := mdmlab.TeamFilter{User: &mdmlab.User{GlobalRole: ptr.String(mdmlab.RoleAdmin)}}
		titles, _, _, err := ds.ListSoftwareTitles(
			ctx,
			mdmlab.SoftwareTitleListOptions{TeamID: &team.ID},
			tmFilter,
		)
		require.NoError(t, err)
	...
	assertSoftware(nil)
	err = ds.BatchSetSoftwareInstallers(ctx, &team.ID, []*mdmlab.UploadSoftwareInstallerPayload{})
	require.NoError(t, err)
	softwareInstallers, err = ds.GetSoftwareInstallers(ctx, team.ID)
	require.NoError(t, err)
	require.Empty(t, softwareInstallers)
	...
	assertSoftware([]mdmlab.SoftwareTitle{
	...
	tfr1, err := mdmlab.NewTempFileReader(ins1File, t.TempDir)
	require.NoError(t, err)
	err = ds.BatchSetSoftwareInstallers(ctx, &team.ID, []*mdmlab.UploadSoftwareInstallerPayload{
		{
			InstallScript:      "install",
...
func testHasSelfServiceSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_1", Platform: platform}}, Name: "vpp1", BundleIdentifier: "com.app.vpp1"}, nil)
	require.NoError(t, err)
	hasSelfService, err = ds.HasSelfServiceSoftwareInstallers(ctx, platform, nil)
	require.NoError(t, err)
	assert.False(t, hasSelfService)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_2", Platform: platform}, SelfService: true}, Name: "vpp2", BundleIdentifier: "com.app.vpp2"}, nil)
	require.NoError(t, err)
	hasSelfService, err = ds.HasSelfServiceSoftwareInstallers(ctx, platform, nil)
	require.NoError(t, err)
	assert.True(t, hasSelfService)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "adam_vpp_3", Platform: mdmlab.MacOSPlatform}, SelfService: true}, Name: "vpp3", BundleIdentifier: "com.app.vpp3"}, &team.ID)
	require.NoError(t, err)
	// Check darwin
...
func testDeleteSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	softwareInstallerID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:   "install",
		InstallerFile:   tfr0,
		StorageID:       ins0,
...
func testDeletePendingSoftwareInstallsForPolicy(t *testing.T, ds *Datastore) {
	...
	policy1, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mdmlab.PolicyPayload{
		Name:                "p1",
		Query:               "SELECT 1;",
	...
	policy2, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mdmlab.PolicyPayload{
		Name:                "p2",
		Query:               "SELECT 2;",
	...
	err = sqlx.GetContext(ctx, ds.reader(ctx), &count, hostSoftwareInstallsCount, mdmlab.SoftwareInstallPending, executionID)
	require.NoError(t, err)
...
func testGetHostLastInstallData(t *testing.T, ds *Datastore) {
	...
	softwareInstallerID1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:   "install",
		InstallerFile:   tfr0,
		StorageID:       ins0,
		Filename:        "installer.pkg",
	...
	require.NoError(t, err)
	softwareInstallerID2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
	...
	require.Equal(t, installUUID1, host1LastInstall.ExecutionID)
	require.NotNil(t, host1LastInstall.Status)
	require.Equal(t, mdmlab.SoftwareInstalled, *host1LastInstall.Status)
	...
	require.Equal(t, installUUID3, host1LastInstall.ExecutionID)
	require.NotNil(t, host1LastInstall.Status)
	require.Equal(t, mdmlab.SoftwareInstallFailed, *host1LastInstall.Status)
...
func testBatchSetSoftwareInstallersScopedViaLabels(t *testing.T, ds *Datastore) {
	...
	tm1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "1"})
	require.NoError(t, err)
	tm2, err := ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "2"})
	require.NoError(t, err)
	user := test.NewUser(t, ds, "Alice", "alice@example.com", true)

	// create some installer payloads to be used by test cases
	installers := make([]*mdmlab.UploadSoftwareInstallerPayload, 3)
	for i := range installers {
		file := bytes.NewReader([]byte("installer" + fmt.Sprint(i)))
		tfr, err := mdmlab.NewTempFileReader(file, t.TempDir)
		require.NoError(t, err)
		installers[i] = &mdmlab.UploadSoftwareInstallerPayload{
			InstallScript:   "install",
			InstallerFile:   tfr,
	...
	labels := make([]*mdmlab.Label, 4)
	for i := range labels {
		lbl, err := ds.NewLabel(ctx, &mdmlab.Label{Name: "label" + fmt.Sprint(i)})
		require.NoError(t, err)
		labels[i] = lbl
	}
	...
	cases := []struct {
		...
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[0]}},
			},
		},
		{
			...
			payload: []testPayload{
				...
				{Installer: installers[1], Labels: []*mdmlab.Label{labels[1]}, Exclude: true},
			},
		},
		{
			...
			payload: []testPayload{
				{Installer: installers[0], ShouldCancelPending: ptr.Bool(false)},
				{Installer: installers[1], Labels: []*mdmlab.Label{labels[1]}, Exclude: false, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[0], labels[1]}, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[1]}, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[0], labels[2]}, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm2,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[0]}, Exclude: false},
				{Installer: installers[1], Labels: []*mdmlab.Label{labels[0], labels[1], labels[2]}, Exclude: true},
				{Installer: installers[2], Labels: []*mdmlab.Label{labels[1], labels[2]}, Exclude: false},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[0], labels[2]}, ShouldCancelPending: ptr.Bool(false)},
		...
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mdmlab.Label{labels[1], labels[3]}, ShouldCancelPending: ptr.Bool(true)},
		...
		},
		{
			...
			payload: []testPayload{
				...
				{Installer: installers[2], Labels: []*mdmlab.Label{labels[1], labels[2]}, Exclude: false, ShouldCancelPending: ptr.Bool(false)},
			},
		},
		{
	...
	for _, c := range cases {
		...
		payload := make([]*mdmlab.UploadSoftwareInstallerPayload, len(c.payload))
		for i, p := range c.payload {
			installer := *p.Installer
			installer.ValidatedLabels = &mdmlab.LabelIdentsWithScope{LabelScope: mdmlab.LabelScopeIncludeAny}
			if p.Exclude {
				installer.ValidatedLabels.LabelScope = mdmlab.LabelScopeExcludeAny
			}
			byName := make(map[string]mdmlab.LabelIdent, len(p.Labels))
			for _, lbl := range p.Labels {
				byName[lbl.Name] = mdmlab.LabelIdent{LabelName: lbl.Name, LabelID: lbl.ID}
			}
		...
		// expected ones.
		installersByFilename := make(map[string]*mdmlab.SoftwareInstaller, len(installers))
...
func testMatchOrCreateSoftwareInstallerWithAutomaticPolicies(t *testing.T, ds *Datastore) {
	...
	require.Equal(t, mdmlab.PolicyNoTeamID, *noTeamPolicies[0].TeamID)
...
Software won't be installed on Linux hosts with RPM-based distributions because this policy's query is written to always pass on these hosts.`, team2Policies[0].Description)
	...
	// Test rpm.
	installerID4, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		InstallerFile:    tfr1,
		Extension:        "rpm",
		StorageID:        "storage4",
		Filename:         "barzoo1",
...
Software won't be installed on Linux hosts with Debian-based distributions because this policy's query is written to always pass on these hosts.`, team2Policies[1].Description)
	...
	team3, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team 3"})
	require.NoError(t, err)

	_, err = ds.NewTeamPolicy(ctx, team3.ID, &user1.ID, mdmlab.PolicyPayload{
	...
	team3Policies, _, err := ds.ListTeamPolicies(ctx, team3.ID, mdmlab.ListOptions{}, mdmlab.ListOptions{})
	require.NoError(t, err)
