...
func (ds *Datastore) NewMDMAppleConfigProfile(ctx context.Context, cp mdmlab.MDMAppleConfigProfile) (*mdmlab.MDMAppleConfigProfile, error) {
...
(SELECT ?, ?, ?, ?, ?, UNHEX(MD5(?)), CURRENT_TIMESTAMP(), ? FROM DUAL WHERE
	...
	err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		// record the ID as we want to return a mdmlab.Profile instance with it
		// filled in.
		profileID, _ = res.LastInsertId()

		labels := make([]mdmlab.ConfigurationProfileLabel, 0, len(cp.LabelsIncludeAll)+len(cp.LabelsIncludeAny)+len(cp.LabelsExcludeAny))
...
func formatErrorDuplicateConfigProfile(err error, cp *mdmlab.MDMAppleConfigProfile) error {
	switch {
...
func formatErrorDuplicateDeclaration(err error, decl *mdmlab.MDMAppleDeclaration) error {
	switch {
...
func (ds *Datastore) ListMDMAppleConfigProfiles(ctx context.Context, teamID *uint) ([]*mdmlab.MDMAppleConfigProfile, error) {
	stmt := `
SELECT
	profile_uuid,
	profile_id,
...
ORDER BY name`
	...
	mdmlabIdentifiers := []string{}
	for idf := range mobileconfig.MDMlabPayloadIdentifiers() {
		mdmlabIdentifiers = append(mdmlabIdentifiers, idf)
	}
	stmt, args, err := sqlx.In(stmt, teamID, mdmlabIdentifiers)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "sqlx.In ListMDMAppleConfigProfiles")
	}

	var res []*mdmlab.MDMAppleConfigProfile
...
func (ds *Datastore) GetMDMAppleConfigProfileByDeprecatedID(ctx context.Context, profileID uint) (*mdmlab.MDMAppleConfigProfile, error) {
	return ds.getMDMAppleConfigProfileByIDOrUUID(ctx, profileID, "")
}

func (ds *Datastore) GetMDMAppleConfigProfile(ctx context.Context, profileUUID string) (*mdmlab.MDMAppleConfigProfile, error) {
	return ds.getMDMAppleConfigProfileByIDOrUUID(ctx, 0, profileUUID)
}

func (ds *Datastore) getMDMAppleConfigProfileByIDOrUUID(ctx context.Context, id uint, uuid string) (*mdmlab.MDMAppleConfigProfile, error) {
	stmt := `
SELECT
	profile_uuid,
	profile_id,
...
func (ds *Datastore) GetMDMAppleDeclaration(ctx context.Context, declUUID string) (*mdmlab.MDMAppleDeclaration, error) {
	stmt := `
SELECT
	declaration_uuid,
	team_id,
...
WHERE
	...
	var res mdmlab.MDMAppleDeclaration
...
func (ds *Datastore) DeleteMDMAppleConfigProfile(ctx context.Context, profileUUID string) error {
	...
	if strings.HasPrefix(profileUUID, mdmlab.MDMAppleDeclarationUUIDPrefix) {
		return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func deleteUnsentAppleHostMDMProfile(ctx context.Context, tx sqlx.ExtContext, uuid string) error {
	...
	if _, err := tx.ExecContext(ctx, stmt, uuid, mdmlab.MDMOperationTypeInstall); err != nil {
...
func (ds *Datastore) GetHostMDMAppleProfiles(ctx context.Context, hostUUID string) ([]mdmlab.HostMDMAppleProfile, error) {
	stmt := fmt.Sprintf(`
SELECT
profile_uuid,
...
WHERE
  host_uuid = ? AND declaration_name NOT IN (?) AND NOT (operation_type = '%s' AND COALESCE(status, '%s') IN('%s', '%s'))`,
	...
	stmt, args, err := sqlx.In(stmt, hostUUID, hostUUID, mdmlabmdm.ListMDMlabReservedMacOSDeclarationNames())
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "building in statement")
	}

	var profiles []mdmlab.HostMDMAppleProfile
...
func (ds *Datastore) GetHostMDMCertificateProfile(ctx context.Context, hostUUID string,
	profileUUID string,
) (*mdmlab.HostMDMCertificateProfile, error) {
	stmt := `
	SELECT
		hmap.host_uuid,
		hmap.profile_uuid,
	...
	var profile mdmlab.HostMDMCertificateProfile
...
func (ds *Datastore) NewMDMAppleEnrollmentProfile(
	ctx context.Context,
	payload mdmlab.MDMAppleEnrollmentProfilePayload,
) (*mdmlab.MDMAppleEnrollmentProfile, error) {
...
`,
	...
	return &mdmlab.MDMAppleEnrollmentProfile{
...
func (ds *Datastore) ListMDMAppleEnrollmentProfiles(ctx context.Context) ([]*mdmlab.MDMAppleEnrollmentProfile, error) {
	var enrollmentProfiles []*mdmlab.MDMAppleEnrollmentProfile
...
func (ds *Datastore) GetMDMAppleEnrollmentProfileByToken(ctx context.Context, token string) (*mdmlab.MDMAppleEnrollmentProfile, error) {
	var enrollment mdmlab.MDMAppleEnrollmentProfile
...
func (ds *Datastore) GetMDMAppleEnrollmentProfileByType(ctx context.Context, typ mdmlab.MDMAppleEnrollmentType) (*mdmlab.MDMAppleEnrollmentProfile, error) {
	var enrollment mdmlab.MDMAppleEnrollmentProfile
...
func (ds *Datastore) GetMDMAppleCommandResults(ctx context.Context, commandUUID string) ([]*mdmlab.MDMCommandResult, error) {
	query := `
SELECT
    ncr.id as host_uuid,
    ncr.command_uuid,
...
func (ds *Datastore) ListMDMAppleCommands(
	ctx context.Context,
	tmFilter mdmlab.TeamFilter,
	listOpts *mdmlab.MDMCommandListOptions,
) ([]*mdmlab.MDMAppleCommand, error) {
	stmt := fmt.Sprintf(`
SELECT
    nvq.id as device_id,
    nvq.command_uuid,
...
`, ds.whereFilterHostsByTeams(tmFilter, "h"))
	...
	var results []*mdmlab.MDMAppleCommand
...
func (ds *Datastore) NewMDMAppleInstaller(ctx context.Context, name string, size int64, manifest string, installer []byte, urlToken string) (*mdmlab.MDMAppleInstaller, error) {
	...
	id, _ := res.LastInsertId()
	return &mdmlab.MDMAppleInstaller{
		ID:        uint(id), //nolint:gosec // dismiss G115
		Size:      size,
		Name:      name,
		Manifest:  manifest,
...
func (ds *Datastore) MDMAppleInstaller(ctx context.Context, token string) (*mdmlab.MDMAppleInstaller, error) {
	var installer mdmlab.MDMAppleInstaller
...
func (ds *Datastore) MDMAppleInstallerDetailsByID(ctx context.Context, id uint) (*mdmlab.MDMAppleInstaller, error) {
	var installer mdmlab.MDMAppleInstaller
	if err := sqlx.GetContext(
		ctx,
		ds.writer(ctx),
...
func (ds *Datastore) MDMAppleInstallerDetailsByToken(ctx context.Context, token string) (*mdmlab.MDMAppleInstaller, error) {
	var installer mdmlab.MDMAppleInstaller
	if err := sqlx.GetContext(
		ctx,
		ds.writer(ctx),
...
func (ds *Datastore) ListMDMAppleInstallers(ctx context.Context) ([]mdmlab.MDMAppleInstaller, error) {
	var installers []mdmlab.MDMAppleInstaller
...
func (ds *Datastore) MDMAppleListDevices(ctx context.Context) ([]mdmlab.MDMAppleDevice, error) {
	var devices []mdmlab.MDMAppleDevice
	if err := sqlx.SelectContext(
		ctx,
		ds.writer(ctx),
		&devices,
...
func (ds *Datastore) MDMAppleUpsertHost(ctx context.Context, mdmHost *mdmlab.Host) error {
...
func mdmHostEnrollFields(mdmHost *mdmlab.Host) (refetchRequested bool, lastEnrolledAt time.Time) {
	supportsOsquery := mdmHost.SupportsOsquery()
	// 2000-01-01 00:00:00 is what MDMlab considers the zero/"Never" time.
...
func createHostFromMDMDB(
	...
	// third-party MDM, but gets assigned in ABM to MDMlab (during
...
func (ds *Datastore) IngestMDMAppleDevicesFromDEPSync(
	...
	abmTokenID uint,
	macOSTeam, iosTeam, ipadTeam *mdmlab.Team,
	...
	var teamIDs []*uint
	for _, team := range []*mdmlab.Team{macOSTeam, iosTeam, ipadTeam} {
		if team == nil {
...
func (ds *Datastore) UpsertMDMAppleHostDEPAssignments(ctx context.Context, hosts []mdmlab.Host, abmTokenID uint) error {
...
func upsertHostDEPAssignmentsDB(ctx context.Context, tx sqlx.ExtContext, hosts []mdmlab.Host, abmTokenID uint) error {
	if len(hosts) == 0 {
		return nil
	}
...
func upsertMDMAppleHostDisplayNamesDB(ctx context.Context, tx sqlx.ExtContext, hosts ...mdmlab.Host) error {
	args := []interface{}{}
	parts := []string{}
	for _, h := range hosts {
...
func upsertMDMAppleHostMDMInfoDB(ctx context.Context, tx sqlx.ExtContext, appCfg *mdmlab.AppConfig, fromSync bool, hostIDs ...uint) error {
	if len(hostIDs) == 0 {
		return nil
	}
	...
	if err != nil {
		return ctxerr.Wrap(ctx, err, "resolve MDMlab MDM URL")
	}
	...
	result, err := tx.ExecContext(ctx, `
		...
		mdmlab.WellKnownMDMMDMlab, serverURL)
	if err != nil {
	...
	if insertOnDuplicateDidInsertOrUpdate(result) {
		...
		if err := sqlx.GetContext(ctx, tx, &mdmID, stmt, mdmlab.WellKnownMDMMDMlab, serverURL); err != nil {
...
func upsertMDMAppleHostLabelMembershipDB(ctx context.Context, tx sqlx.ExtContext, logger log.Logger, hosts ...mdmlab.Host) error {
...
func (ds *Datastore) MDMTurnOff(ctx context.Context, uuid string) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if !mdmlab.MDMSupported(host.Platform) {
...
func unionSelectDevices(devices []hostToCreateFromMDM) (stmt string, args []interface{}) {
	for i, d := range devices {
		...
		// map the platform hint to MDMlab's hosts.platform field.
		...
		case strings.Contains(normalizedHint, "ipad"):
			platform = string(mdmlab.IPadOSPlatform)
		}
...
func (ds *Datastore) GetHostDEPAssignment(ctx context.Context, hostID uint) (*mdmlab.HostDEPAssignment, error) {
	var res mdmlab.HostDEPAssignment
...
func (ds *Datastore) DeleteHostDEPAssignments(ctx context.Context, abmTokenID uint, serials []string) error {
	...
	return ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		// If pending host is no longer in ABM, we should delete it because it will never enroll in MDMlab.
...
func (ds *Datastore) RestoreMDMApplePendingDEPHost(ctx context.Context, host *mdmlab.Host) error {
...
func (ds *Datastore) GetNanoMDMEnrollment(ctx context.Context, id string) (*mdmlab.NanoEnrollment, error) {
	var nanoEnroll mdmlab.NanoEnrollment
...
func (ds *Datastore) BatchSetMDMAppleProfiles(ctx context.Context, tmID *uint, profiles []*mdmlab.MDMAppleConfigProfile) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
`
	...
	// or processing
	incomingProfs := make(map[string]*mdmlab.MDMAppleConfigProfile, len(profiles))
	for i, p := range profiles {
		incomingIdents[i] = p.Identifier
		incomingProfs[p.Identifier] = p
	}

	var existingProfiles []*mdmlab.MDMAppleConfigProfile
	...
	// profiles that are managed and delivered by MDMlab
	mdmlabIdents := []string{}
	for ident := range mobileconfig.MDMlabPayloadIdentifiers() {
		mdmlabIdents = append(mdmlabIdents, ident)
	}
	...
	// delete the obsolete profiles (all those that are not in keepIdents or delivered by MDMlab)
	var result sql.Result
	stmt, args, err = sqlx.In(deleteProfilesNotInList, profTeamID, append(keepIdents, mdmlabIdents...))
	...
	incomingLabels := []mdmlab.ConfigurationProfileLabel{}
	if len(incomingIdents) > 0 {
		var newlyInsertedProfs []*mdmlab.MDMAppleConfigProfile
...
func (ds *Datastore) BulkDeleteMDMAppleHostsConfigProfiles(ctx context.Context, profs []*mdmlab.MDMAppleProfilePayload) error {
	return ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		return ds.bulkDeleteMDMAppleHostsConfigProfilesDB(ctx, tx, profs)
	})
}

func (ds *Datastore) bulkDeleteMDMAppleHostsConfigProfilesDB(ctx context.Context, tx sqlx.ExtContext, profs []*mdmlab.MDMAppleProfilePayload) error {
	if len(profs) == 0 {
		return nil
	}
...
`, fmt.Sprintf(appleMDMProfilesDesiredStateQuery, profileHostIn, profileHostIn, profileHostIn, profileHostIn))
	...
	var wantedProfiles []*mdmlab.MDMAppleProfilePayload
	for i := 0; i < selectProfilesTotalBatches; i++ {
		...
		if len(onlyProfileUUIDs) > 0 {
			...
			stmt, args, err = sqlx.In(toInstallStmt, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, mdmlab.MDMOperationTypeRemove)
		}
		...
		var partialResult []*mdmlab.MDMAppleProfilePayload
	...
	wantedProfiles = mdmlab.FilterMacOSOnlyProfilesFromIOSIPadOS(wantedProfiles)
...
`, fmt.Sprintf(appleMDMProfilesDesiredStateQuery, profileHostIn, profileHostIn, profileHostIn, profileHostIn), narrowByProfiles)
	...
	var currentProfiles []*mdmlab.MDMAppleProfilePayload
	for i := 0; i < selectProfilesTotalBatches; i++ {
		start := i * selectProfilesBatchSize
		end := start + selectProfilesBatchSize
		if end > len(hostUUIDs) {
		...
		if len(onlyProfileUUIDs) > 0 {
			...
			stmt, args, err = sqlx.In(toRemoveStmt, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, mdmlab.MDMOperationTypeRemove)
		}
	...
	// https://github.com/mdmlabdm/mdmlab/issues/21338
	...
	var hostProfilesToClean []*mdmlab.MDMAppleProfilePayload
	...
	profilesToInsert := make(map[string]*mdmlab.MDMAppleProfilePayload)
	...
	for _, p := range wantedProfiles {
		if pp, ok := profileIntersection.GetMatchingProfileInCurrentState(p); ok {
			if pp.Status != &mdmlab.MDMDeliveryFailed && bytes.Equal(pp.Checksum, p.Checksum) {
				profilesToInsert[fmt.Sprintf("%s\n%s", p.HostUUID, p.ProfileUUID)] = &mdmlab.MDMAppleProfilePayload{
		...
		pargs = append(pargs, p.ProfileUUID, p.HostUUID, p.ProfileIdentifier, p.ProfileName, p.Checksum, p.SecretsUpdatedAt,
			mdmlab.MDMOperationTypeInstall, nil, "", "")
		psb.WriteString("(?, ?, ?, ?, ?, ?, ?, ?, ?, ?),")
		batchCount++
	...
	for _, p := range currentProfiles {
		...
		// Doing so will result in MDMlab attempting to remove a profile that doesn't exist on the
		// host (since the installation failed). Skipping it here will lead to it being removed from
		// the host in MDMlab during profile reconciliation, which is what we want.
		...
		pargs = append(pargs, p.ProfileUUID, p.HostUUID, p.ProfileIdentifier, p.ProfileName, p.Checksum, p.SecretsUpdatedAt,
			mdmlab.MDMOperationTypeRemove, nil, "", "")
		psb.WriteString("(?, ?, ?, ?, ?, ?, ?, ?, ?, ?),")
		batchCount++
...
//     to independent verification by MDMlab (verifying), or has reached a terminal
...
//     but is still subject to independent verification by MDMlab (verifying)
//     or the operation has been completed and independenly verified by MDMlab (verified).
//
...
func (ds *Datastore) ListMDMAppleProfilesToInstall(ctx context.Context) ([]*mdmlab.MDMAppleProfilePayload, error) {
	query := fmt.Sprintf(`
	SELECT
		ds.profile_uuid,
		ds.host_uuid,
	...
	var profiles []*mdmlab.MDMAppleProfilePayload
	err := sqlx.SelectContext(ctx, ds.reader(ctx), &profiles, query, mdmlab.MDMOperationTypeRemove, mdmlab.MDMOperationTypeInstall)
	return profiles, err
}

func (ds *Datastore) ListMDMAppleProfilesToRemove(ctx context.Context) ([]*mdmlab.MDMAppleProfilePayload, error) {
	...
	var profiles []*mdmlab.MDMAppleProfilePayload
	err := sqlx.SelectContext(ctx, ds.reader(ctx), &profiles, query, mdmlab.MDMOperationTypeRemove)
...
func (ds *Datastore) BulkUpsertMDMAppleHostProfiles(ctx context.Context, payload []*mdmlab.MDMAppleBulkUpsertHostProfilePayload) error {
	if len(payload) == 0 {
		return nil
	}
	...
	generateValueArgs := func(p *mdmlab.MDMAppleBulkUpsertHostProfilePayload) (string, []any) {
		valuePart := "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),"
...
func (ds *Datastore) UpdateOrDeleteHostMDMAppleProfile(ctx context.Context, profile *mdmlab.HostMDMAppleProfile) error {
	if profile.OperationType == mdmlab.MDMOperationTypeRemove && profile.Status != nil {
		var ignoreError bool
		if *profile.Status == mdmlab.MDMDeliveryFailed {
		...
		if ignoreError ||
			(*profile.Status == mdmlab.MDMDeliveryVerifying || *profile.Status == mdmlab.MDMDeliveryVerified) {
			_, err := ds.writer(ctx).ExecContext(ctx, `
	...
	if profile.OperationType == mdmlab.MDMOperationTypeRemove && profile.Status != nil && *profile.Status == mdmlab.MDMDeliveryFailed {
	...
	if profile.OperationType == mdmlab.MDMOperationTypeInstall && profile.Status != nil && *profile.Status == mdmlab.MDMDeliveryVerifying {
...
func sqlCaseMDMAppleStatus() string {
	...
	var (
		failed    = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryFailed))
		pending   = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryPending))
		verifying = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryVerifying))
		verified  = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryVerified))
	)
	return `
...
func sqlJoinMDMAppleProfilesStatus() string {
	...
	var (
		...
		verifying = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryVerifying))
		verified  = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryVerified))
		install   = fmt.Sprintf("'%s'", string(mdmlab.MDMOperationTypeInstall))
		filevault = fmt.Sprintf("'%s'", mobileconfig.MDMlabFileVaultPayloadIdentifier)
	)
	return `
	LEFT JOIN (
...
func sqlJoinMDMAppleDeclarationsStatus() string {
	...
	var (
		...
		verified          = fmt.Sprintf("'%s'", string(mdmlab.MDMDeliveryVerified))
		install           = fmt.Sprintf("'%s'", string(mdmlab.MDMOperationTypeInstall))
		reservedDeclNames = fmt.Sprintf("'%s', '%s', '%s'", mdmlabmdm.MDMlabMacOSUpdatesProfileName, mdmlabmdm.MDMlabIOSUpdatesProfileName, mdmlabmdm.MDMlabIPadOSUpdatesProfileName)
	)
	return `
	LEFT JOIN (
...
func (ds *Datastore) GetMDMAppleProfilesSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMProfilesSummary, error) {
	stmt := `
SELECT
	COUNT(id) AS count,
	%s AS status
...
GROUP BY
	...
	var res mdmlab.MDMProfilesSummary
	for s, c := range byStatus {
		switch mdmlab.MDMDeliveryStatus(s) {
		case mdmlab.MDMDeliveryFailed:
			res.Failed = c
		case mdmlab.MDMDeliveryPending:
			res.Pending = c
		case mdmlab.MDMDeliveryVerifying:
			res.Verifying = c
		case mdmlab.MDMDeliveryVerified:
			res.Verified = c
		default:
...
func (ds *Datastore) InsertMDMIdPAccount(ctx context.Context, account *mdmlab.MDMIdPAccount) error {
	stmt := `
...
func (ds *Datastore) GetMDMIdPAccountByEmail(ctx context.Context, email string) (*mdmlab.MDMIdPAccount, error) {
	stmt := `SELECT uuid, username, fullname, email FROM mdm_idp_accounts WHERE email = ?`
	var acct mdmlab.MDMIdPAccount
...
func (ds *Datastore) GetMDMIdPAccountByUUID(ctx context.Context, uuid string) (*mdmlab.MDMIdPAccount, error) {
...
func subqueryFileVaultVerifying() (string, []interface{}) {
	...
	args := []interface{}{
		mobileconfig.MDMlabFileVaultPayloadIdentifier,
...
func subqueryFileVaultFailed() (string, []interface{}) {
	sql := `
            ...
            WHERE
                ...
                AND hmap.profile_identifier = ?
                AND hmap.status = ?`
	args := []interface{}{mobileconfig.MDMlabFileVaultPayloadIdentifier, mdmlab.MDMDeliveryFailed}
	return sql, args
}
...
func subqueryFileVaultRemovingEnforcement() (string, []interface{}) {
	sql := `
            ...
            WHERE
                ...
                AND (hmap.status IS NULL OR hmap.status = ?)
                AND hmap.operation_type = ?`
	args := []interface{}{mobileconfig.MDMlabFileVaultPayloadIdentifier, mdmlab.MDMDeliveryPending, mdmlab.MDMOperationTypeRemove}
	return sql, args
}

func (ds *Datastore) GetMDMAppleFileVaultSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleFileVaultSummary, error) {
	sqlFmt := `
SELECT
    COUNT(
        CASE WHEN EXISTS (%s)
...
WHERE
	...
	var res mdmlab.MDMAppleFileVaultSummary
...
func (ds *Datastore) BulkUpsertMDMAppleConfigProfiles(ctx context.Context, payload []*mdmlab.MDMAppleConfigProfile) error {
	if len(payload) == 0 {
		return nil
	}
...
func (ds *Datastore) InsertMDMAppleBootstrapPackage(ctx context.Context, bp *mdmlab.MDMAppleBootstrapPackage, pkgStore mdmlab.MDMBootstrapPackageStore) error {
...
func (ds *Datastore) CopyDefaultMDMAppleBootstrapPackage(ctx context.Context, ac *mdmlab.AppConfig, toTeamID uint) error {
	if ac == nil {
...
`
		...
		// mdmlabctl in a gitops workflow).
...
func (ds *Datastore) GetMDMAppleBootstrapPackageBytes(ctx context.Context, token string, pkgStore mdmlab.MDMBootstrapPackageStore) (*mdmlab.MDMAppleBootstrapPackage, error) {
	const stmt = `SELECT name, bytes, sha256 FROM mdm_apple_bootstrap_packages WHERE token = ?`

	var bp mdmlab.MDMAppleBootstrapPackage
...
func (ds *Datastore) GetMDMAppleBootstrapPackageSummary(ctx context.Context, teamID uint) (*mdmlab.MDMAppleBootstrapPackageSummary, error) {
	...
	// assigned to MDMlab in ABM but still manually enroll. We should probably keep using host_mdm,
	...
	var bp mdmlab.MDMAppleBootstrapPackageSummary
...
func (ds *Datastore) GetHostMDMMacOSSetup(ctx context.Context, hostID uint) (*mdmlab.HostMDMMacOSSetup, error) {
	// NOTE: Consider joining on host_dep_assignments instead of host_mdm so DEP hosts that
	// manually enroll or re-enroll are included in the results so long as they are not unassigned
	// in Apple Business Manager. The problem with using host_dep_assignments is that a host can be
	// assigned to MDMlab in ABM but still manually enroll. We should probably keep using host_mdm,
...
WHERE
	...
	args := []interface{}{mdmlab.MDMBootstrapPackageInstalled, mdmlab.MDMBootstrapPackageFailed, mdmlab.MDMBootstrapPackagePending, hostID}

	var dest mdmlab.HostMDMMacOSSetup
	...
	if dest.BootstrapPackageStatus == mdmlab.MDMBootstrapPackageFailed {
...
func (ds *Datastore) GetMDMAppleBootstrapPackageMeta(ctx context.Context, teamID uint) (*mdmlab.MDMAppleBootstrapPackage, error) {
...
func (ds *Datastore) CleanupUnusedBootstrapPackages(ctx context.Context, pkgStore mdmlab.MDMBootstrapPackageStore, removeCreatedBefore time.Time) error {
	if pkgStore == nil {
...
func getMDMAppleConfigProfileByTeamAndIdentifierDB(ctx context.Context, tx sqlx.QueryerContext, teamID *uint, profileIdentifier string) (*mdmlab.MDMAppleConfigProfile, error) {
	if teamID == nil {
		teamID = ptr.Uint(0)
	}
...
WHERE
	...
	var profile mdmlab.MDMAppleConfigProfile
	err := sqlx.GetContext(ctx, tx, &profile, stmt, teamID, profileIdentifier)
	if err != nil {
		if err == sql.ErrNoRows {
			return &mdmlab.MDMAppleConfigProfile{}, ctxerr.Wrap(ctx, notFound("MDMAppleConfigProfile").WithName(profileIdentifier))
		}
		return &mdmlab.MDMAppleConfigProfile{}, ctxerr.Wrap(ctx, err, "get mdm apple config profile by team and identifier")
	}
	return &profile, nil
}

func (ds *Datastore) SetOrUpdateMDMAppleSetupAssistant(ctx context.Context, asst *mdmlab.MDMAppleSetupAssistant) (*mdmlab.MDMAppleSetupAssistant, error) {
	const stmt = `
		INSERT INTO
...
func (ds *Datastore) GetMDMAppleSetupAssistant(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleSetupAssistant, error) {
	return ds.getMDMAppleSetupAssistant(ctx, ds.reader(ctx), teamID)
}

func (ds *Datastore) getMDMAppleSetupAssistant(ctx context.Context, q sqlx.QueryerContext, teamID *uint) (*mdmlab.MDMAppleSetupAssistant, error) {
	const stmt = `
	SELECT
		id,
		team_id,
	...
	var asst mdmlab.MDMAppleSetupAssistant
	var globalOrTmID uint
	if teamID != nil {
		globalOrTmID = *teamID
	}
...
func (ds *Datastore) updateHostDEPAssignProfileResponses(ctx context.Context, payload *godep.ProfileResponse, abmTokenID *uint) error {
	...
	for serial, status := range payload.Devices {
		switch status {
		case string(mdmlab.DEPAssignProfileResponseSuccess):
			success = append(success, serial)
		case string(mdmlab.DEPAssignProfileResponseNotAccessible):
			notAccessible = append(notAccessible, serial)
		case string(mdmlab.DEPAssignProfileResponseFailed):
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		if err := updateHostDEPAssignProfileResponses(ctx, tx, ds.logger, payload.ProfileUUID, success,
			string(mdmlab.DEPAssignProfileResponseSuccess), abmTokenID); err != nil {
			return err
		}
		if err := updateHostDEPAssignProfileResponses(ctx, tx, ds.logger, payload.ProfileUUID, notAccessible,
			string(mdmlab.DEPAssignProfileResponseNotAccessible), abmTokenID); err != nil {
			return err
		}
		if err := updateHostDEPAssignProfileResponses(ctx, tx, ds.logger, payload.ProfileUUID, failed,
			string(mdmlab.DEPAssignProfileResponseFailed), abmTokenID); err != nil {
			return err
		}
		return nil
	})
...
`
	...
	stmt, args, err := sqlx.In(stmt, string(mdmlab.DEPAssignProfileResponseFailed), depCooldownPeriod.Seconds(), serials)
	if err != nil {
...
WHERE
	...
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &rows, stmt, string(mdmlab.DEPAssignProfileResponseFailed), string(mdmlab.JobStateFailure), depCooldownPeriod.Seconds()); err != nil {
...
func (ds *Datastore) MDMResetEnrollment(ctx context.Context, hostUUID string) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// stage if the host is enrolling with MDMlab, SCEP renewals are
...
func (ds *Datastore) batchSetMDMAppleDeclarations(ctx context.Context, tx sqlx.ExtContext, tmID *uint,
	incomingDeclarations []*mdmlab.MDMAppleDeclaration) (updatedDB bool, err error) {
	...
	incomingDeclarationsMap := make(map[string]*mdmlab.MDMAppleDeclaration, len(incomingDeclarations))
	...
	keepNames := make([]string, 0, len(existingDecls)+len(mdmlabmdm.ListMDMlabReservedMacOSDeclarationNames()))
	for _, p := range existingDecls {
		if newP := incomingDeclarationsMap[p.Name]; newP != nil {
			keepNames = append(keepNames, p.Name)
		}
	}
	keepNames = append(keepNames, mdmlabmdm.ListMDMlabReservedMacOSDeclarationNames()...)
...
func (ds *Datastore) updateDeclarationsLabelAssociations(ctx context.Context, tx sqlx.ExtContext,
	incomingDeclarationsMap map[string]*mdmlab.MDMAppleDeclaration, teamID uint) (updatedDB bool, err error) {
	var incomingLabels []mdmlab.ConfigurationProfileLabel
...
func (ds *Datastore) insertOrUpdateDeclarations(ctx context.Context, tx sqlx.ExtContext, incomingDeclarations []*mdmlab.MDMAppleDeclaration,
...
`
	...
	for _, d := range incomingDeclarations {
		declUUID := mdmlab.MDMAppleDeclarationUUIDPrefix + uuid.NewString()
		var result sql.Result
...
func (ds *Datastore) getExistingDeclarations(ctx context.Context, tx sqlx.ExtContext, incomingNames []string,
	teamID uint) ([]*mdmlab.MDMAppleDeclaration, error) {
...
WHERE
  ...
  name IN (?)
`
	var existingDecls []*mdmlab.MDMAppleDeclaration
...
func (ds *Datastore) NewMDMAppleDeclaration(ctx context.Context, declaration *mdmlab.MDMAppleDeclaration) (*mdmlab.MDMAppleDeclaration, error) {
	const stmt = `
INSERT INTO mdm_apple_declarations (
	declaration_uuid,
	team_id,
...
func (ds *Datastore) SetOrUpdateMDMAppleDeclaration(ctx context.Context, declaration *mdmlab.MDMAppleDeclaration) (*mdmlab.MDMAppleDeclaration, error) {
	const stmt = `
INSERT INTO mdm_apple_declarations (
	declaration_uuid,
	team_id,
...
func (ds *Datastore) insertOrUpsertMDMAppleDeclaration(ctx context.Context, insOrUpsertStmt string, declaration *mdmlab.MDMAppleDeclaration) (*mdmlab.MDMAppleDeclaration, error) {
	declUUID := mdmlab.MDMAppleDeclarationUUIDPrefix + uuid.NewString()
	...
	err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		labels := make([]mdmlab.ConfigurationProfileLabel, 0,
...
func batchSetDeclarationLabelAssociationsDB(ctx context.Context, tx sqlx.ExtContext,
	declarationLabels []mdmlab.ConfigurationProfileLabel,
) (updatedDB bool, err error) {
	...
	var (
		...
		labelsToInsert  = make(map[string]*mdmlab.ConfigurationProfileLabel, len(declarationLabels))
	)
	...
	var existingProfileLabels []mdmlab.ConfigurationProfileLabel
	...
	if len(existingProfileLabels) == len(labelsToInsert) {
		for _, existing := range existingProfileLabels {
			...
			// The mdmlab.ConfigurationProfileLabel struct has no pointers, so we can use standard cmp.Equal
...
func (ds *Datastore) MDMAppleDDMDeclarationsToken(ctx context.Context, hostUUID string) (*mdmlab.MDMAppleDDMDeclarationsToken, error) {
	const stmt = `
SELECT
...
WHERE
	...
	var res mdmlab.MDMAppleDDMDeclarationsToken
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &res, stmt, hostUUID, mdmlab.MDMOperationTypeInstall); err != nil {
...
func (ds *Datastore) MDMAppleDDMDeclarationItems(ctx context.Context, hostUUID string) ([]mdmlab.MDMAppleDDMDeclarationItem, error) {
	const stmt = `
SELECT
...
WHERE
	...
	var res []mdmlab.MDMAppleDDMDeclarationItem
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &res, stmt, hostUUID, mdmlab.MDMOperationTypeInstall); err != nil {
...
func (ds *Datastore) MDMAppleDDMDeclarationsResponse(ctx context.Context, identifier string, hostUUID string) (*mdmlab.MDMAppleDeclaration, error) {
...
WHERE
	...
	var res mdmlab.MDMAppleDeclaration
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &res, stmt, hostUUID, identifier, mdmlab.MDMOperationTypeInstall); err != nil {
		if err == sql.ErrNoRows {
...
func (ds *Datastore) MDMAppleBatchSetHostDeclarationState(ctx context.Context) ([]string, error) {
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
		uuids, _, err = mdmAppleBatchSetHostDeclarationStateDB(ctx, tx, batchSize, &mdmlab.MDMDeliveryPending)
		return err
	})
...
func mdmAppleBatchSetPendingHostDeclarationsDB(
	...
	tx sqlx.ExtContext,
	batchSize int,
	changedDeclarations []*mdmlab.MDMAppleHostDeclaration,
	status *mdmlab.MDMDeliveryStatus,
) (updatedDB bool, err error) {
	baseStmt := `
	...
	profilesToInsert := make(map[string]*mdmlab.MDMAppleHostDeclaration)
	...
	executeUpsertBatch := func(valuePart string, args []any) error {
		...
		var existingProfiles []mdmlab.MDMAppleHostDeclaration
	...
	generateValueArgs := func(d *mdmlab.MDMAppleHostDeclaration) (string, []any) {
		profilesToInsert[fmt.Sprintf("%s\n%s", d.HostUUID, d.DeclarationUUID)] = &mdmlab.MDMAppleHostDeclaration{
			HostUUID:         d.HostUUID,
...
func mdmAppleGetHostsWithChangedDeclarationsDB(ctx context.Context, tx sqlx.ExtContext) ([]*mdmlab.MDMAppleHostDeclaration, error) {
	stmt := fmt.Sprintf(`
        (
            SELECT
                ds.host_uuid,
	...
	var decls []*mdmlab.MDMAppleHostDeclaration
	if err := sqlx.SelectContext(ctx, tx, &decls, stmt, mdmlab.MDMOperationTypeRemove, mdmlab.MDMOperationTypeInstall, mdmlab.MDMOperationTypeRemove); err != nil {
...
func (ds *Datastore) MDMAppleStoreDDMStatusReport(ctx context.Context, hostUUID string, updates []*mdmlab.MDMAppleHostDeclaration) error {
...
ON DUPLICATE KEY UPDATE
  ...
  `
	...
	var current []*mdmlab.MDMAppleHostDeclaration
	...
	updatesByToken := make(map[string]*mdmlab.MDMAppleHostDeclaration, len(updates))
...
func (ds *Datastore) MDMAppleSetPendingDeclarationsAs(ctx context.Context, hostUUID string, status *mdmlab.MDMDeliveryStatus, detail string) error {
	stmt := `
  ...
  `
	...
	_, err := ds.writer(ctx).ExecContext(
		...
		status, detail,
		// WHERE ...
		mdmlab.MDMOperationTypeInstall, mdmlab.MDMDeliveryPending, hostUUID,
	)
...
func (ds *Datastore) InsertMDMConfigAssets(ctx context.Context, assets []mdmlab.MDMConfigAsset, tx sqlx.ExtContext) error {
...
func (ds *Datastore) GetAllMDMConfigAssetsByName(ctx context.Context, assetNames []mdmlab.MDMAssetName,
...
WHERE
	...
	assetMap := make(map[mdmlab.MDMAssetName]mdmlab.MDMConfigAsset, len(res))
	for _, asset := range res {
		...
		assetMap[asset.Name] = mdmlab.MDMConfigAsset{Name: asset.Name, Value: decryptedVal}
	}
...
func (ds *Datastore) GetAllMDMConfigAssetsHashes(ctx context.Context, assetNames []mdmlab.MDMAssetName) (map[mdmlab.MDMAssetName]string, error) {
	if len(assetNames) == 0 {
		return nil, nil
	}
...
WHERE name IN (?) AND deletion_uuid = ''`
	...
	assetMap := make(map[mdmlab.MDMAssetName]string, len(res))
	for _, asset := range res {
...
func (ds *Datastore) DeleteMDMConfigAssetsByName(ctx context.Context, assetNames []mdmlab.MDMAssetName) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func (ds *Datastore) HardDeleteMDMConfigAsset(ctx context.Context, assetName mdmlab.MDMAssetName) error {
	stmt := `
...
func softDeleteMDMConfigAssetsByName(ctx context.Context, tx sqlx.ExtContext, assetNames []mdmlab.MDMAssetName) error {
	stmt := `
UPDATE
    mdm_config_assets
SET
...
func insertMDMConfigAssets(ctx context.Context, tx sqlx.ExtContext, assets []mdmlab.MDMConfigAsset, privateKey string) error {
	stmt := `
...
func (ds *Datastore) ReplaceMDMConfigAssets(ctx context.Context, assets []mdmlab.MDMConfigAsset, tx sqlx.ExtContext) error {
	replaceFunc := func(tx sqlx.ExtContext) error {
		var names []mdmlab.MDMAssetName
		for _, a := range assets {
...
func (ds *Datastore) ListIOSAndIPadOSToRefetch(ctx context.Context, interval time.Duration) (devices []mdmlab.AppleDevicesToRefetch,
	err error,
) {
	hostsStmt := `
...
func (ds *Datastore) GetABMTokenByOrgName(ctx context.Context, orgName string) (*mdmlab.ABMToken, error) {
...
func (ds *Datastore) SaveABMToken(ctx context.Context, tok *mdmlab.ABMToken) error {
	const stmt = `
UPDATE
	abm_tokens
SET
...
func (ds *Datastore) InsertABMToken(ctx context.Context, tok *mdmlab.ABMToken) (*mdmlab.ABMToken, error) {
	const stmt = `
INSERT INTO
	abm_tokens
...
func (ds *Datastore) ListABMTokens(ctx context.Context) ([]*mdmlab.ABMToken, error) {
	stmt := `
SELECT
	abt.id,
	abt.organization_name,
...
LEFT OUTER JOIN
	...
	stmt, args, err := sqlx.Named(stmt, map[string]any{"no_team": mdmlab.TeamNameNoTeam})
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "build list ABM tokens query from named args")
	}

	var tokens []*mdmlab.ABMToken
	...
	for _, tok := range tokens {
		...
		tok.MacOSTeam = mdmlab.ABMTokenTeam{Name: tok.MacOSTeamName, ID: macOSTeamID}
		tok.IOSTeam = mdmlab.ABMTokenTeam{Name: tok.IOSTeamName, ID: iOSTeamID}
		tok.IPadOSTeam = mdmlab.ABMTokenTeam{Name: tok.IPadOSTeamName, ID: iPadIOSTeamID}
...
func (ds *Datastore) GetABMTokenByID(ctx context.Context, tokenID uint) (*mdmlab.ABMToken, error) {
...
func (ds *Datastore) getABMToken(ctx context.Context, tokenID uint, orgName string) (*mdmlab.ABMToken, error) {
	stmt := `
SELECT
	abt.id,
	abt.organization_name,
...
func (ds *Datastore) AddHostMDMCommands(ctx context.Context, commands []mdmlab.HostMDMCommand) error {
	const baseStmt = `
...
func (ds *Datastore) GetHostMDMCommands(ctx context.Context, hostID uint) (commands []mdmlab.HostMDMCommand, err error) {
...
func (ds *Datastore) RemoveHostMDMCommand(ctx context.Context, command mdmlab.HostMDMCommand) error {
	const stmt = `
...
func (ds *Datastore) GetMDMAppleOSUpdatesSettingsByHostSerial(ctx context.Context, serial string) (*mdmlab.AppleOSUpdateSettings, error) {
	stmt := `
SELECT
	team_id, platform
FROM
...
func (ds *Datastore) BulkUpsertMDMManagedCertificates(ctx context.Context, payload []*mdmlab.MDMBulkUpsertManagedCertificatePayload) error {
	if len(payload) == 0 {
		return nil
	}
	...
	generateValueArgs := func(p *mdmlab.MDMBulkUpsertManagedCertificatePayload) (string, []any) {
		valuePart := "(?, ?, ?),"
