...
func testQueriesApply(t *testing.T, ds *Datastore) {
	...
	zwass := test.NewUser(t, ds, "Zach", "zwass@mdmlab.co", true)
	groob := test.NewUser(t, ds, "Victor", "victor@mdmlab.co", true)

	expectedQueries := []*mdmlab.Query{
		{
			Name:               "foo",
	...
	queries, count, _, err := ds.ListQueries(context.Background(), mdmlab.ListQueryOptions{})
	require.NoError(t, err)
	...
	err = ds.ApplyQueries(context.Background(), zwass.ID, []*mdmlab.Query{expectedQueries[2]}, nil)
	require.NoError(t, err)
	...
	require.ErrorIs(t, err, mdmlab.ErrQueryInvalidPlatform)
}

func testQueriesDelete(t *testing.T, ds *Datastore) {
	user := test.NewUser(t, ds, "Zach", "zwass@mdmlab.co", true)
	...
	err = ds.UpdateLiveQueryStats(
		context.Background(), query.ID, []*mdmlab.LiveQueryStats{
			{
				HostID:       hostID,
				Executions:   1,
	...
	err = ds.CalculateAggregatedPerfStatsPercentiles(context.Background(), mdmlab.AggregatedStatsTypeScheduledQuery, query.ID)
	require.NoError(t, err)
	...
	go func() {
		for {
			...
			require.NoError(t, err)
			if len(stats) == 0 {
				_, err = GetAggregatedStats(context.Background(), ds, mdmlab.AggregatedStatsTypeScheduledQuery, query.ID)
...
func testQueriesGetByName(t *testing.T, ds *Datastore) {
	...
	teamRocket, err := ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:        "Team Rocket",
...
func testQueriesDeleteMany(t *testing.T, ds *Datastore) {
	...
	hostIDs := []uint{10, 20}
	err = ds.UpdateLiveQueryStats(
		context.Background(), q1.ID, []*mdmlab.LiveQueryStats{
			{
				HostID:     hostIDs[0],
				Executions: 1,
			},
	...
	require.NoError(t, err)
	err = ds.UpdateLiveQueryStats(
		context.Background(), q3.ID, []*mdmlab.LiveQueryStats{
			{
				HostID:     hostIDs[0],
				Executions: 1,
			},
	...
	require.NoError(t, err)
	err = ds.CalculateAggregatedPerfStatsPercentiles(context.Background(), mdmlab.AggregatedStatsTypeScheduledQuery, q1.ID)
	require.NoError(t, err)
	err = ds.CalculateAggregatedPerfStatsPercentiles(context.Background(), mdmlab.AggregatedStatsTypeScheduledQuery, q3.ID)
	require.NoError(t, err)
	...
	go func() {
		for {
			...
			require.NoError(t, err)
			if len(stats) == 0 {
				_, err = GetAggregatedStats(context.Background(), ds, mdmlab.AggregatedStatsTypeScheduledQuery, q1.ID)
				if errors.Is(err, sql.ErrNoRows) {
					statsGone <- true
					break
				}
	...
	_, err = GetAggregatedStats(context.Background(), ds, mdmlab.AggregatedStatsTypeScheduledQuery, q3.ID)
...
func testQueriesSave(t *testing.T, ds *Datastore) {
	...
	query.Logging = mdmlab.LoggingDifferential
	...
	require.Equal(t, "zwass@mdmlab.co", actual.AuthorEmail)
...
func testQueriesList(t *testing.T, ds *Datastore) {
	...
	for i := 0; i < 10; i++ {
		...
		_, err := ds.NewQuery(context.Background(), &mdmlab.Query{
	...
	require.Equal(t, "zwass@mdmlab.co", results[0].AuthorEmail)
	...
	_, err = ds.writer(context.Background()).Exec(
		...
		idWithAgg, false, mdmlab.AggregatedStatsTypeScheduledQuery,
...
func testQueriesLoadPacksForQueries(t *testing.T, ds *Datastore) {
	...
	queries := []*mdmlab.Query{
		{Name: "q1", Query: "select * from time", Logging: mdmlab.LoggingSnapshot},
		{Name: "q2", Query: "select * from osquery_info", Logging: mdmlab.LoggingDifferential},
	}
...
func testQueriesDuplicateNew(t *testing.T, ds *Datastore) {
	user := test.NewUser(t, ds, "Mike Arpaia", "mike@mdmlab.co", true)

	// The uniqueness of 'global' queries should be based on their name alone.
	globalQ1, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:     "foo",
		Query:    "select * from time;",
		AuthorID: &user.ID,
		Logging:  mdmlab.LoggingSnapshot,
...
func testObserverCanRunQuery(t *testing.T, ds *Datastore) {
	...
	queries, _, _, err := ds.ListQueries(context.Background(), mdmlab.ListQueryOptions{})
	require.NoError(t, err)
...
func testListQueriesFiltersByTeamID(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	globalQ2, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:    "query2",
		Query:   "select 1;",
		Saved:   true,
		Logging: mdmlab.LoggingSnapshot,
	...
	require.NoError(t, err)
	globalQ3, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:    "query3",
		Query:   "select 1;",
		Saved:   true,
		Logging: mdmlab.LoggingSnapshot,
	...
	queries, count, _, err := ds.ListQueries(context.Background(), mdmlab.ListQueryOptions{})
	require.NoError(t, err)
	test.QueryElementsMatch(t, queries, []*mdmlab.Query{globalQ1, globalQ2, globalQ3})
	...
	teamQ1, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:    "query1",
		Query:   "select 1;",
		Saved:   true,
		TeamID:  &team.ID,
	...
	require.NoError(t, err)
	teamQ2, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:    "query2",
		Query:   "select 1;",
		Saved:   true,
		TeamID:  &team.ID,
	...
	require.NoError(t, err)
	teamQ3, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:    "query3",
		Query:   "select 1;",
		Saved:   true,
		TeamID:  &team.ID,
	...
	queries, count, _, err = ds.ListQueries(
		context.Background(),
		mdmlab.ListQueryOptions{
			TeamID: &team.ID,
		},
	)
	require.NoError(t, err)
	test.QueryElementsMatch(t, queries, []*mdmlab.Query{teamQ1, teamQ2, teamQ3})
	assert.Equal(t, count, 3)
	...
	require.NoError(t, err)
	test.QueryElementsMatch(t, queries, []*mdmlab.Query{globalQ1, globalQ2, globalQ3, teamQ1, teamQ2, teamQ3})
	assert.Equal(t, count, 6)
...
func testListQueriesFiltersByIsScheduled(t *testing.T, ds *Datastore) {
	q1, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:     "query1",
		Query:    "select 1;",
		Saved:    true,
		Interval: 0,
	...
	require.NoError(t, err)
	q3, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:               "query3",
		Query:              "select 1;",
		Saved:              true,
		Interval:           20,
	...
	testCases := []struct {
		opts     mdmlab.ListQueryOptions
		expected []*mdmlab.Query
	}{
		{
			opts:     mdmlab.ListQueryOptions{},
			expected: []*mdmlab.Query{q1, q2, q3},
		},
		{
			opts:     mdmlab.ListQueryOptions{IsScheduled: ptr.Bool(true)},
			expected: []*mdmlab.Query{q3},
		},
		{
			opts:     mdmlab.ListQueryOptions{IsScheduled: ptr.Bool(false)},
			expected: []*mdmlab.Query{q1, q2},
		},
	}
...
func testListScheduledQueriesForAgents(t *testing.T, ds *Datastore) {
	...
	for i, teamID := range []*uint{nil, &team.ID} {
		...
		_, err = ds.NewQuery(context.Background(), &mdmlab.Query{
			...
			TeamID:             teamID,
			DiscardData:        false,
			Logging:            mdmlab.LoggingDifferentialIgnoreRemovals,
		})
		require.NoError(t, err)

		// Interval=1, AutomationsEnabled=0, DiscardData=0, Snapshot=1
		q11, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		...
		q14, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		...
		q15, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		...
		q16, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		...
		q17, err := ds.NewQuery(context.Background(), &mdmlab.Query{
		...
		test.QueryElementsMatch(t, result, []*mdmlab.Query{q11, q14, q15, q16, q17}, i)
		...
		test.QueryElementsMatch(t, result, []*mdmlab.Query{q14, q15, q16, q17}, i)
	}
}
