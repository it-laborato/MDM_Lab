...
func TestMigrationStatus(t *testing.T) {
	...
	assert.EqualValues(t, mdmlab.NoMigrationsCompleted, status.StatusCode)
	...
	status, err = ds.MigrationStatus(context.Background())
	require.NoError(t, err)
	assert.EqualValues(t, mdmlab.SomeMigrationsCompleted, status.StatusCode)
	...
	status, err = ds.MigrationStatus(context.Background())
	require.NoError(t, err)
	assert.EqualValues(t, mdmlab.AllMigrationsCompleted, status.StatusCode)
	assert.Empty(t, status.MissingTable)
	assert.Empty(t, status.MissingData)
	...
	status, err = ds.MigrationStatus(context.Background())
	require.NoError(t, err)
	assert.EqualValues(t, mdmlab.UnknownMigrations, status.StatusCode)
