...
func (p *standalonePool) Mode() mdmlab.RedisMode {
	return mdmlab.RedisStandalone
}
...
func (p *clusterPool) Mode() mdmlab.RedisMode {
	return mdmlab.RedisCluster
}
...
func NewPool(config PoolConfig) (mdmlab.RedisPool, error) {
...
// commands.
func ReadOnlyConn(pool mdmlab.RedisPool, conn redis.Conn) redis.Conn {
...
// returned unaltered.
func ConfigureDoer(pool mdmlab.RedisPool, conn redis.Conn) redis.Conn {
...
// length of 1).
func SplitKeysBySlot(pool mdmlab.RedisPool, keys ...string) [][]string {
...
func EachNode(pool mdmlab.RedisPool, replicas bool, fn func(conn redis.Conn) error) error {
...
// keys.
func BindConn(pool mdmlab.RedisPool, conn redis.Conn, keys ...string) error {
	if _, isCluster := pool.(*clusterPool); isCluster {
...
func PublishHasListeners(pool mdmlab.RedisPool, conn redis.Conn, channel, message string) (bool, error) {
...
func ScanKeys(pool mdmlab.RedisPool, pattern string, count int) ([]string, error) {
	var keys []string
