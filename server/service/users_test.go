...
func TestUserAuth(t *testing.T) {
	...
	ds.TeamsSummaryFunc = func(ctx context.Context) ([]*mdmlab.TeamSummary, error) {
		team1 := &mdmlab.TeamSummary{ID: teamID}
		team2 := &mdmlab.TeamSummary{ID: otherTeamID}
		return []*mdmlab.TeamSummary{team1, team2}, nil
	}
	ds.InviteByTokenFunc = func(ctx context.Context, token string) (*mdmlab.Invite, error) {
	...
	ds.ListUsersFunc = func(ctx context.Context, opts mdmlab.UserListOptions) ([]*mdmlab.User, error) {
		return nil, nil
	}
	...
	testCases := []struct {
		...
		{
			name:                                 "global admin",
			user:                                 &mdmlab.User{ID: 1000, GlobalRole: ptr.String(mdmlab.RoleAdmin)},
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, GlobalRole: ptr.String(mdmlab.RoleMaintainer)},
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, GlobalRole: ptr.String(mdmlab.RoleObserver)},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  true,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: teamID}, Role: mdmlab.RoleAdmin}}},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  false,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: teamID}, Role: mdmlab.RoleMaintainer}}},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  true,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: teamID}, Role: mdmlab.RoleObserver}}},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  true,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: otherTeamID}, Role: mdmlab.RoleMaintainer}}},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  true,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: otherTeamID}, Role: mdmlab.RoleAdmin}}},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  true,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
		...
		},
		{
			...
			user:                                 &mdmlab.User{ID: 1000, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: otherTeamID}, Role: mdmlab.RoleObserver}}},
			shouldFailGlobalWrite:                true,
			shouldFailTeamWrite:                  true,
			shouldFailWriteRoleGlobalToGlobal:    true,
			shouldFailWriteRoleGlobalToTeam:      true,
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = svc.ModifyUser(ctx, tt.user.ID, mdmlab.UserPayload{Name: ptr.String("Foo")})
			require.NoError(t, err)

			// A user can always change its own password (change_password rego action).
			_, err = svc.ModifyUser(ctx, tt.user.ID, mdmlab.UserPayload{Password: ptr.String(test.GoodPassword), NewPassword: ptr.String(test.GoodPassword2)})
			require.NoError(t, err)

			changeRole := func(role string) string {
				switch role {
				case mdmlab.RoleMaintainer:
					return mdmlab.RoleAdmin // promote
				case mdmlab.RoleAdmin:
					return mdmlab.RoleMaintainer // demote
				case mdmlab.RoleObserver:
					return mdmlab.RoleAdmin // promote
				default:
			...
			if tt.user.GlobalRole != nil {
				_, err = svc.ModifyUser(ctx, tt.user.ID, mdmlab.UserPayload{GlobalRole: ptr.String(changeRole(*tt.user.GlobalRole))})
				checkAuthErr(t, tt.shouldFailWriteRoleOwnDomain, err)
			} else { // Team user
				ownTeamDifferentRole := []mdmlab.UserTeam{
					{
						Team: mdmlab.Team{ID: tt.user.Teams[0].ID},
						Role: changeRole(tt.user.Teams[0].Role),
					},
				}
				_, err = svc.ModifyUser(ctx, tt.user.ID, mdmlab.UserPayload{Teams: &ownTeamDifferentRole})
				checkAuthErr(t, tt.shouldFailWriteRoleOwnDomain, err)
			}

			teams := []mdmlab.UserTeam{{Team: mdmlab.Team{ID: teamID}, Role: mdmlab.RoleMaintainer}}
			_, _, err = svc.CreateUser(ctx, mdmlab.UserPayload{
			...
			_, err = svc.ModifyUser(ctx, userGlobalMaintainerID, mdmlab.UserPayload{Name: ptr.String("Foo")})
			checkAuthErr(t, tt.shouldFailGlobalWrite, err)

			_, err = svc.ModifyUser(ctx, userTeamMaintainerID, mdmlab.UserPayload{Name: ptr.String("Bar")})
			checkAuthErr(t, tt.shouldFailTeamWrite, err)

			_, err = svc.ModifyUser(ctx, userGlobalMaintainerID, mdmlab.UserPayload{GlobalRole: ptr.String(mdmlab.RoleMaintainer)})
			checkAuthErr(t, tt.shouldFailWriteRoleGlobalToGlobal, err)

			_, err = svc.ModifyUser(ctx, userGlobalMaintainerID, mdmlab.UserPayload{Teams: &teams})
			checkAuthErr(t, tt.shouldFailWriteRoleGlobalToTeam, err)

			anotherTeams := []mdmlab.UserTeam{{Team: mdmlab.Team{ID: otherTeamID}, Role: mdmlab.RoleMaintainer}}
			_, err = svc.ModifyUser(ctx, userTeamMaintainerID, mdmlab.UserPayload{Teams: &anotherTeams})
			checkAuthErr(t, tt.shouldFailWriteRoleTeamToAnotherTeam, err)

			_, err = svc.ModifyUser(ctx, userTeamMaintainerID, mdmlab.UserPayload{GlobalRole: ptr.String(mdmlab.RoleMaintainer)})
			...
			_, err = svc.ModifyUser(ctx, userGlobalMaintainerID, mdmlab.UserPayload{NewPassword: ptr.String(test.GoodPassword2)})
			checkAuthErr(t, tt.shouldFailGlobalChangePassword, err)

			_, err = svc.ModifyUser(ctx, userTeamMaintainerID, mdmlab.UserPayload{NewPassword: ptr.String(test.GoodPassword2)})
			checkAuthErr(t, tt.shouldFailTeamChangePassword, err)

			_, err = svc.ListUsers(ctx, mdmlab.UserListOptions{})
			checkAuthErr(t, tt.shouldFailListAll, err)

			_, err = svc.ListUsers(ctx, mdmlab.UserListOptions{TeamID: teamID})
...
func TestModifyUserEmail(t *testing.T) {
	...
	ms.UserByIDFunc = func(ctx context.Context, id uint) (*mdmlab.User, error) {
		return user, nil
	}
	ms.UserByEmailFunc = func(ctx context.Context, email string) (*mdmlab.User, error) {
		return nil, notFoundErr{}
	}
	ms.InviteByEmailFunc = func(ctx context.Context, email string) (*mdmlab.Invite, error) {
		return nil, notFoundErr{}
	}
	ms.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		config := &mdmlab.AppConfig{
			SMTPSettings: &mdmlab.SMTPSettings{
				...
				SMTPSenderAddress:      "xxx@mdmlab.co",
			},
		}
		return config, nil
	}
	ms.SaveUserFunc = func(ctx context.Context, u *mdmlab.User) error {
...
func TestModifyUserEmailNoPassword(t *testing.T) {
	...
	var iae *mdmlab.InvalidArgumentError
...
func TestMFAHandling(t *testing.T) {
	...
	appConfig := &mdmlab.AppConfig{SMTPSettings: &mdmlab.SMTPSettings{}}
	ms.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
	...
	ms.NewUserFunc = func(ctx context.Context, user *mdmlab.User) (*mdmlab.User, error) {
		user.ID = 4
		return user, nil
	}
	ms.NewActivityFunc = func(ctx context.Context, user *mdmlab.User, activity mdmlab.ActivityDetails, details []byte, createdAt time.Time) error {
		return nil
	}
	...
	_, err = svc.ModifyUser(ctx, user.ID, mdmlab.UserPayload{SSOEnabled: ptr.Bool(true)})
	require.ErrorContains(t, err, "SSO")

	user.SSOEnabled = true
	user.MFAEnabled = false
	_, err = svc.ModifyUser(ctx, user.ID, mdmlab.UserPayload{MFAEnabled: ptr.Bool(true)})
	require.ErrorContains(t, err, "license")

	_, err = svc.ModifyUser(premiumCtx, user.ID, mdmlab.UserPayload{MFAEnabled: ptr.Bool(true)})
	require.ErrorContains(t, err, "SSO")
	...
	_, err = svc.ModifyUser(ctx, user.ID, mdmlab.UserPayload{MFAEnabled: ptr.Bool(true), Name: ptr.String("Joe Bob")})
	require.NoError(t, err)
	_, err = svc.ModifyUser(ctx, user.ID, mdmlab.UserPayload{Name: ptr.String("Joe Bob")})
	require.NoError(t, err)
...
func testUsersCreateUserForcePasswdReset(t *testing.T, ds *mysql.Datastore) {
	...
	ctx = viewer.NewContext(ctx, viewer.Viewer{User: admin})
	user, sessionKey, err := svc.CreateUser(ctx, mdmlab.UserPayload{
...
func testUsersChangePassword(t *testing.T, ds *mysql.Datastore) {
	...
	passwordChangeTests := []struct {
		...
		{ // prevent password reuse
			...
			wantErr:     mdmlab.NewInvalidArgumentError("new_password", "Cannot reuse old password"),
		},
		{ // all good
		...
		{ // missing old password
			user:        users["user1@example.com"],
			newPassword: test.GoodPassword2,
			wantErr:     mdmlab.NewInvalidArgumentError("old_password", "Old password cannot be empty"),
		},
	}
...
func TestResetPassword(t *testing.T) {
	...
	passwordResetTests := []struct {
		...
		{ // bad token
			token:       "dcbaz",
			newPassword: test.GoodPassword,
			wantErr:     mdmlab.NewAuthFailedError("invalid password reset token"),
		},
		{ // missing token
			newPassword: test.GoodPassword,
			wantErr:     mdmlab.NewInvalidArgumentError("token", "Token cannot be empty field"),
		},
	}
	...
	for _, tt := range passwordResetTests {
		t.Run("", func(t *testing.T) {
			...
			serr := svc.ResetPassword(test.UserContext(ctx, &mdmlab.User{ID: 1}), tt.token, tt.newPassword)
			if tt.wantErr != nil {
...
func refreshCtx(t *testing.T, ctx context.Context, user *mdmlab.User, ds mdmlab.Datastore, session *mdmlab.Session) context.Context {
...
func TestIsAdminOfTheModifiedTeams(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			actionUserRoles: roles{
				global: ptr.String(mdmlab.RoleAdmin),
			},
			targetUserOriginalTeams: []teamWithRole{
				{
					teamID: 1,
		...
		},
		{
			...
			actionUserRoles: roles{
				global: ptr.String(mdmlab.RoleMaintainer),
			},
			targetUserOriginalTeams: []teamWithRole{
				{
					teamID: 1,
		...
		t.Run(tc.name, func(t *testing.T) {
			userTeamsFn := func(twr []teamWithRole) []mdmlab.UserTeam {
				var userTeams []mdmlab.UserTeam
				for _, ot := range twr {
					userTeams = append(userTeams, mdmlab.UserTeam{
						Team: mdmlab.Team{ID: ot.teamID},
						Role: ot.role,
					})
				}
				return userTeams
...
func TestTeamAdminAddRoleOtherTeam(t *testing.T) {
	...
	_, err := svc.ModifyUser(ctx, adminTeam2.ID, mdmlab.UserPayload{
		Teams: &[]mdmlab.UserTeam{
			{
				Team: mdmlab.Team{ID: 2},
				Role: mdmlab.RoleAdmin,
...
func testUsersCreateUserWithAPIOnly(t *testing.T, ds *mysql.Datastore) {
	...
	ctx = viewer.NewContext(ctx, viewer.Viewer{User: admin})
	apiOnlyUser, sessionKey, err := svc.CreateUser(ctx, mdmlab.UserPayload{
		Name:       ptr.String("Some Observer"),
		Email:      ptr.String("some-observer@email.com"),
		Password:   ptr.String(test.GoodPassword),
		GlobalRole: ptr.String(mdmlab.RoleObserver),
	...
	hosts, err := svc.ListHosts(ctx, mdmlab.HostListOptions{})
	require.NoError(t, err)
