...
func (m *Middleware) AuthzCheck() endpoint.Middleware {
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, req interface{}) (interface{}, error) {
			...
			// appropriately).
			var authFailedError *mdmlab.AuthFailedError
			var authRequiredError *mdmlab.AuthRequiredError
			var authHeaderRequiredError *mdmlab.AuthHeaderRequiredError
			if errors.As(err, &authFailedError) ||
				errors.As(err, &authRequiredError) ||
				errors.As(err, &authHeaderRequiredError) ||
				errors.Is(err, mdmlab.ErrPasswordResetRequired) {
				return nil, err
			}
