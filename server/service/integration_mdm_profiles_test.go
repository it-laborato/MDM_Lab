...
func (s *integrationMDMTestSuite) TestAppleProfileManagement() {
	...
	wantGlobalProfiles = append(
		wantGlobalProfiles,
		setupExpectedMDMlabdProfile(t, s.server.URL, t.Name(), nil),
	)
...
`)
	...
	res := s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{invalidSecretsProfile}}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "$FLEET_SECRET_INVALID")

	// create a new team
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "batch_set_mdm_profiles"})
	require.NoError(t, err)

	// add an enroll secret so the mdmlabd profiles differ
	var teamResp teamEnrollSecretsResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", tm.ID),
		modifyTeamEnrollSecretsRequest{
			Secrets: []mdmlab.EnrollSecret{{Secret: "team1_enroll_sec"}},
	...
	wantTeamProfiles = append(
		wantTeamProfiles,
		setupExpectedMDMlabdProfile(t, s.server.URL, "team1_enroll_sec", &tm.ID),
	)
	// add profiles to the team
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: teamProfiles}, http.StatusNoContent,
	...
	expectedNoTeamSummary := mdmlab.MDMProfilesSummary{
		Pending:   0,
		Failed:    0,
		Verifying: 1,
		Verified:  0,
	}
	expectedTeamSummary := mdmlab.MDMProfilesSummary{}
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: nil}, http.StatusNoContent, "team_id",
	...
	batchRequest := batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
		{Name: "N4", Contents: teamProfiles[0]},
		{Name: "N5", Contents: teamProfiles[1]},
		{Name: "NS1", Contents: teamProfiles[2]},
	}}
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchRequest, http.StatusNoContent, "team_id", fmt.Sprint(tm.ID), "dry_run", "true")
	s.assertConfigProfilesByIdentifier(&tm.ID, "I4", false)
	s.assertConfigProfilesByIdentifier(&tm.ID, "I5", false)
	s.assertConfigProfilesByIdentifier(&tm.ID, "IS1", false)
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchRequest, http.StatusNoContent, "team_id", fmt.Sprint(tm.ID))
	s.assertConfigProfilesByIdentifier(&tm.ID, "IS1", true)
	s.awaitTriggerProfileSchedule(t)
	installs, removes = checkNextPayloads(t, mdmDevice, false)
	assert.Empty(t, removes)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/%d", host.ID), getHostRequest{}, http.StatusOK, &hostResp)
	require.NotEmpty(t, hostResp.Host.MDM.Profiles)
	resProfiles := *hostResp.Host.MDM.Profiles
	// two extra profiles: mdmlabd config and root CA
	...
	require.Empty(t, removes)
	s.checkMDMProfilesSummaries(t, &tm.ID, mdmlab.MDMProfilesSummary{Verifying: 1}, nil)

	// can't resend profile while verifying
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, mcUUID), nil, http.StatusConflict)
	errMsg = extractServerErrorText(res.Body)
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		...
		_, err := q.ExecContext(context.Background(), stmt, mdmlab.MDMDeliveryPending, mcUUID, host.UUID)
		return err
	})
	s.checkMDMProfilesSummaries(t, &tm.ID, mdmlab.MDMProfilesSummary{Pending: 1}, nil)
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, mcUUID), nil, http.StatusConflict)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn’t resend. Configuration profiles with “pending” or “verifying” status can’t be resent.")
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		stmt := `UPDATE host_mdm_apple_profiles SET status = ? WHERE profile_uuid = ? AND host_uuid = ?`
		_, err := q.ExecContext(context.Background(), stmt, mdmlab.MDMDeliveryFailed, mcUUID, host.UUID)
		return err
	})
	s.checkMDMProfilesSummaries(t, &tm.ID, mdmlab.MDMProfilesSummary{Failed: 1}, nil)
	_ = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, mcUUID), nil, http.StatusAccepted)
	s.awaitTriggerProfileSchedule(t)
	installs, removes = checkNextPayloads(t, mdmDevice, false)
	require.Len(t, installs, 1)
	s.signedProfilesMatch([][]byte{prof}, installs)
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		stmt := `UPDATE host_mdm_apple_profiles SET status = ? WHERE profile_uuid = ? AND host_uuid = ?`
		_, err := q.ExecContext(context.Background(), stmt, mdmlab.MDMDeliveryVerified, mcUUID, host.UUID)
		return err
	})
	_ = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, mcUUID), nil, http.StatusAccepted)
	s.awaitTriggerProfileSchedule(t)
	...
	s.lastActivityMatches(
		mdmlab.ActivityTypeResentConfigurationProfile{}.ActivityName(),
	...
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/configuration_profiles", body.Bytes(), http.StatusOK, headers)
	...
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, declUUID), nil, http.StatusConflict)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn’t resend. Configuration profiles with “pending” or “verifying” status can’t be resent.")
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		...
		_, err := q.ExecContext(context.Background(), stmt, mdmlab.MDMDeliveryVerified, declUUID, host.UUID)
		return err
	})
	_ = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, declUUID), nil, http.StatusAccepted)
	checkDDMSync(mdmDevice)
	s.checkMDMProfilesSummaries(t, &tm.ID, mdmlab.MDMProfilesSummary{Verifying: 1}, nil)
	s.lastActivityMatches(
		mdmlab.ActivityTypeResentConfigurationProfile{}.ActivityName(),
	...
	expectedNoTeamSummary = mdmlab.MDMProfilesSummary{Verifying: 1}
	expectedTeamSummary = mdmlab.MDMProfilesSummary{}
	...
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, mcUUID), nil, http.StatusNotFound)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, wpUUID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Profile is not compatible with host platform")

	// invalid profile UUID prefix should return 404
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, "z"+uuid.NewString()), nil, http.StatusNotFound)
	errMsg = extractServerErrorText(res.Body)
	...
	s.Do("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm.ID), mdmlab.TeamPayload{
...
func (s *integrationMDMTestSuite) TestAppleProfileRetries() {
	...
	err := s.ds.ApplyEnrollSecrets(ctx, nil, []*mdmlab.EnrollSecret{{Secret: enrollSecret}})
	require.NoError(t, err)
	...
	initialExpectedProfiles = append(
		...
		setupExpectedMDMlabdProfile(t, s.server.URL, enrollSecret, nil),
	...
	expectedProfileStatuses := map[string]mdmlab.MDMDeliveryStatus{
		"I1": mdmlab.MDMDeliveryVerifying,
		"I2": mdmlab.MDMDeliveryVerifying,
		mobileconfig.MDMlabdConfigPayloadIdentifier:      mdmlab.MDMDeliveryVerifying,
		mobileconfig.MDMlabCARootConfigPayloadIdentifier: mdmlab.MDMDeliveryVerifying,
	}
	...
	expectedRetryCounts := map[string]uint{
		"I1": 0,
		"I2": 0,
		mobileconfig.MDMlabdConfigPayloadIdentifier:      0,
		mobileconfig.MDMlabCARootConfigPayloadIdentifier: 0,
	}
	...
	hostProfsByIdent := map[string]*mdmlab.HostMacOSProfile{
		"I1": {
			Identifier:  "I1",
			DisplayName: "N1",
		...
		mobileconfig.MDMlabdConfigPayloadIdentifier: {
			Identifier:  mobileconfig.MDMlabdConfigPayloadIdentifier,
			DisplayName: "MDMlabd configuration",
			InstallDate: time.Now().Add(15 * time.Minute),
		},
	}
	reportHostProfs := func(t *testing.T, identifiers ...string) {
		report := make(map[string]*mdmlab.HostMacOSProfile, len(hostProfsByIdent))
	...
	t.Run("retry after verifying", func(t *testing.T) {
		...
		s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		setProfileUploadedAt(t, time.Now().Add(-48*time.Hour), "I1", "I2", mobileconfig.MDMlabdConfigPayloadIdentifier)
		...
		reportHostProfs(t, "I1", mobileconfig.MDMlabdConfigPayloadIdentifier)
		expectedProfileStatuses["I2"] = mdmlab.MDMDeliveryPending
		expectedProfileStatuses["I1"] = mdmlab.MDMDeliveryVerified
		expectedProfileStatuses[mobileconfig.MDMlabdConfigPayloadIdentifier] = mdmlab.MDMDeliveryVerified
		...
		reportHostProfs(t, "I1", "I2", mobileconfig.MDMlabdConfigPayloadIdentifier)
		expectedProfileStatuses["I2"] = mdmlab.MDMDeliveryVerified
		checkProfilesStatus(t)
	...
	t.Run("retry after verification", func(t *testing.T) {
		...
		reportHostProfs(t, "I2", mobileconfig.MDMlabdConfigPayloadIdentifier)
		expectedProfileStatuses["I1"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		reportHostProfs(t, "I2", mobileconfig.MDMlabdConfigPayloadIdentifier)
		expectedProfileStatuses["I1"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("retry after device error", func(t *testing.T) {
		...
		s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		setProfileUploadedAt(t, time.Now().Add(-48*time.Hour), "I1", "I2", mobileconfig.MDMlabdConfigPayloadIdentifier, "I3")
		...
		require.Empty(t, removes)
		expectedProfileStatuses["I3"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		s.signedProfilesMatch([][]byte{newProfile}, installs)
		require.Empty(t, removes)
		expectedProfileStatuses["I3"] = mdmlab.MDMDeliveryVerifying
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
		...
		// retries exceeded)
		reportHostProfs(t, "I2", mobileconfig.MDMlabdConfigPayloadIdentifier)
		expectedProfileStatuses["I3"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("repeated device error", func(t *testing.T) {
		...
		testProfiles = append(testProfiles, newProfile)
		s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		setProfileUploadedAt(t, time.Now().Add(-48*time.Hour), "I1", "I2", mobileconfig.MDMlabdConfigPayloadIdentifier, "I3", "I4")
		...
		s.signedProfilesMatch([][]byte{newProfile}, installs)
		require.Empty(t, removes)
		expectedProfileStatuses["I4"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		s.signedProfilesMatch([][]byte{newProfile}, installs)
		require.Empty(t, removes)
		expectedProfileStatuses["I4"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("retry count does not reset", func(t *testing.T) {
		...
		testProfiles = append(testProfiles, newProfile)
		hostProfsByIdent["I5"] = &mdmlab.HostMacOSProfile{Identifier: "I5", DisplayName: "N5", InstallDate: time.Now()}
		s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		setProfileUploadedAt(t, time.Now().Add(-48*time.Hour), "I1", "I2", mobileconfig.MDMlabdConfigPayloadIdentifier, "I3", "I4", "I5")
		...
		s.signedProfilesMatch([][]byte{newProfile}, installs)
		require.Empty(t, removes)
		expectedProfileStatuses["I5"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		s.signedProfilesMatch([][]byte{newProfile}, installs)
		require.Empty(t, removes)
		expectedProfileStatuses["I5"] = mdmlab.MDMDeliveryVerifying
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged

		// report osquery results with I5 found and confirm that the I5 marked as verified
		reportHostProfs(t, "I2", mobileconfig.MDMlabdConfigPayloadIdentifier, "I5")
		expectedProfileStatuses["I5"] = mdmlab.MDMDeliveryVerified
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
		...
		reportHostProfs(t, "I2", mobileconfig.MDMlabdConfigPayloadIdentifier)
		expectedProfileStatuses["I5"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
...
func (s *integrationMDMTestSuite) TestWindowsProfileRetries() {
	...
	testProfiles := []mdmlab.MDMProfileBatchPayload{
	...
	expectedProfileStatuses := map[string]mdmlab.MDMDeliveryStatus{
		"N1": mdmlab.MDMDeliveryVerifying,
		"N2": mdmlab.MDMDeliveryVerifying,
	}
	checkProfilesStatus := func(t *testing.T) {
	...
	reportHostProfs := func(t *testing.T, profileNames ...string) {
		var responseOps []*mdmlab.SyncMLCmd
		for _, profileName := range profileNames {
			...
			for _, p := range report {
				...
				responseOps = append(responseOps, &mdmlab.SyncMLCmd{
	...
	verifyCommands := func(wantProfileInstalls int, status string) {
		...
		for _, c := range cmds {
			...
			mdmDevice.AppendResponse(mdmlab.SyncMLCmd{
				XMLName: xml.Name{Local: mdmlab.CmdStatus},
				MsgRef:  &msgID,
	...
	t.Run("retry after verifying", func(t *testing.T) {
		// upload test profiles then simulate expired grace period by setting updated_at timestamp of profiles back by 48 hours
		s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		...
		expectedProfileStatuses["N2"] = mdmlab.MDMDeliveryPending
		expectedProfileStatuses["N1"] = mdmlab.MDMDeliveryVerified
		checkProfilesStatus(t)
		...
		expectedProfileStatuses["N2"] = mdmlab.MDMDeliveryVerified
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("retry after verification", func(t *testing.T) {
		...
		expectedProfileStatuses["N1"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		reportHostProfs(t, "N2")
		expectedProfileStatuses["N1"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("retry after device error", func(t *testing.T) {
		...
		testProfiles = append(testProfiles, mdmlab.MDMProfileBatchPayload{
			Name:     "N3",
			Contents: newProfile,
		})
		s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		...
		expectedProfileStatuses["N3"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		// simulate a device ack
		verifyCommands(1, syncml.CmdStatusOK)
		expectedProfileStatuses["N3"] = mdmlab.MDMDeliveryVerifying
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
		...
		// retries exceeded)
		reportHostProfs(t, "N2")
		expectedProfileStatuses["N3"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("repeated device error", func(t *testing.T) {
		...
		// simulate a device error
		verifyCommands(1, syncml.CmdStatusAtomicFailed)
		expectedProfileStatuses["N4"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		verifyCommands(1, syncml.CmdStatusAtomicFailed)
		expectedProfileStatuses["N4"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
	...
	t.Run("retry count does not reset", func(t *testing.T) {
		...
		// hostProfsByIdent["N5"] = &mdmlab.HostMacOSProfile{Identifier: "N5", DisplayName: "N5", InstallDate: time.Now()}
		s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
		// trigger a profile sync and confirm that the install profile
		// command for N5 was sent and simulate a device error
		verifyCommands(1, syncml.CmdStatusAtomicFailed)
		expectedProfileStatuses["N5"] = mdmlab.MDMDeliveryPending
		checkProfilesStatus(t)
		...
		verifyCommands(1, syncml.CmdStatusOK)
		expectedProfileStatuses["N5"] = mdmlab.MDMDeliveryVerifying
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
		...
		expectedProfileStatuses["N5"] = mdmlab.MDMDeliveryVerified
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
		...
		// failed (max retries exceeded)
		reportHostProfs(t, "N2")
		expectedProfileStatuses["N5"] = mdmlab.MDMDeliveryFailed
		checkProfilesStatus(t)
		checkRetryCounts(t) // unchanged
...
func (s *integrationMDMTestSuite) TestPuppetMatchPreassignProfiles() {
	...
	// create a host enrolled in mdmlab
	mdmHost, _ := createHostThenEnrollMDM(s.ds, s.server.URL, t)

	// create a host that's not enrolled into MDM
	nonMDMHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		OsqueryHostID: ptr.String("not-mdm-enrolled"),
		NodeKey:       ptr.String("not-mdm-enrolled"),
		UUID:          uuid.New().String(),
		Hostname:      fmt.Sprintf("%sfoo.local.not.enrolled", t.Name()),
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "empty", HostUUID: nonMDMHost.UUID, Profile: nil}}, http.StatusUnprocessableEntity)

	// preassign a valid profile to the MDM host
	prof1 := mobileconfigForTest("n1", "i1")
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "mdm1", HostUUID: mdmHost.UUID, Profile: prof1}}, http.StatusNoContent)

	// preassign another valid profile to the MDM host
	prof2 := mobileconfigForTest("n2", "i2")
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "mdm1", HostUUID: mdmHost.UUID, Profile: prof2, Group: "g1"}}, http.StatusNoContent)
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "non-mdm", HostUUID: nonMDMHost.UUID, Profile: prof3, Group: "g2"}}, http.StatusNoContent)
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/match", matchMDMApplePreassignmentRequest{ExternalHostIdentifier: "no-such-id"}, http.StatusNoContent)

	// match with the non-mdm host fails
	res := s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/match", matchMDMApplePreassignmentRequest{ExternalHostIdentifier: "non-mdm"}, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "host is not enrolled in MDMlab MDM")

	// match with the mdm host succeeds and creates a team based on the group labels
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/match", matchMDMApplePreassignmentRequest{ExternalHostIdentifier: "mdm1"}, http.StatusNoContent)
	...
	runWithAdminToken(func() {
		...
		s.lastActivityOfTypeMatches(
			mdmlab.ActivityTypeCreatedTeam{}.ActivityName(),
			fmt.Sprintf(`{"team_id": %d, "team_name": %q}`, tm1.ID, tm1.Name),
			0)
		s.lastActivityOfTypeMatches(
			mdmlab.ActivityTypeEditedMacosProfile{}.ActivityName(),
			fmt.Sprintf(`{"team_id": %d, "team_name": %q}`, tm1.ID, tm1.Name),
			0)
		s.lastActivityOfTypeMatches(
			...
			0)
		s.lastActivityOfTypeMatches(
			mdmlab.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
	...
	// the mdm host has the same profiles (i1, i2, plus mdmlabd config and disk encryption)
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		mdmHost: {
			{Identifier: "i1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "i2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabFileVaultPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
		},
	})

	// create a team and set profiles to it (note that it doesn't have disk encryption enabled)
	tm2, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:    "g1 - g4",
		Secrets: []*mdmlab.EnrollSecret{{Secret: "tm2secret"}},
	})
	require.NoError(t, err)
	...
	tm3, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:    "team3_" + t.Name(),
		Secrets: []*mdmlab.EnrollSecret{{Secret: "tm3secret"}},
	})
	require.NoError(t, err)
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{
		prof1, prof2, prof4,
	...
	tm4, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:    "team4_" + t.Name(),
		Secrets: []*mdmlab.EnrollSecret{{Secret: "tm4secret"}},
	})
	require.NoError(t, err)
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{
		prof1, prof2, prof4,
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "6f36ab2c-1a40-429b-9c9d-07c9029f4aa8-puppetcompiler06.test.example.com", HostUUID: mdmHost.UUID, Profile: prof1, Group: "g1"}}, http.StatusNoContent)
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "6f36ab2c-1a40-429b-9c9d-07c9029f4aa8-puppetcompiler01.test.example.com", HostUUID: mdmHost.UUID, Profile: prof4, Group: "g4"}}, http.StatusNoContent)

	// match with the mdm host succeeds and assigns it to tm2
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/match", matchMDMApplePreassignmentRequest{ExternalHostIdentifier: "6f36ab2c-1a40-429b-9c9d-07c9029f4aa8-puppetcompiler03.test.example.com"}, http.StatusNoContent)
	...
	// - mdmlabd config being reinstalled (for new enroll secret)
	s.awaitTriggerProfileSchedule(t)
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		mdmHost: {
			{Identifier: "i1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "i4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			// Profiles from previous team being deleted
			{Identifier: "i2", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabFileVaultPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeRemove,
				Status: &mdmlab.MDMDeliveryPending},
		},
	})

	// create a new mdm host enrolled in mdmlab
	mdmHost2, _ := createHostThenEnrollMDM(s.ds, s.server.URL, t)

	// make it part of team 2
	s.Do("POST", "/api/v1/mdmlab/hosts/transfer",
		addHostsToTeamRequest{TeamID: &tm2.ID, HostIDs: []uint{mdmHost2.ID}}, http.StatusOK)

	// simulate having its profiles installed
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		res, err := q.ExecContext(ctx, `UPDATE host_mdm_apple_profiles SET status = ? WHERE host_uuid = ?`, mdmlab.OSSettingsVerifying, mdmHost2.UUID)
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "mdm2", HostUUID: mdmHost2.UUID, Profile: prof1, Group: "g1"}}, http.StatusNoContent)
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/preassign", preassignMDMAppleProfileRequest{MDMApplePreassignProfilePayload: mdmlab.MDMApplePreassignProfilePayload{ExternalHostIdentifier: "mdm2", HostUUID: mdmHost2.UUID, Profile: prof4, Group: "g4"}}, http.StatusNoContent)
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/match", matchMDMApplePreassignmentRequest{ExternalHostIdentifier: "mdm2"}, http.StatusNoContent)
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		mdmHost2: {
			{Identifier: "i1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "i4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	})
}
...
func (s *integrationMDMTestSuite) TestPuppetRun() {
	...
	preassignAndMatch := func(profs []mdmlab.MDMApplePreassignProfilePayload) {
		require.NotEmpty(t, profs)
		for _, prof := range profs {
			s.Do(
				"POST",
				"/api/latest/mdmlab/mdm/apple/profiles/preassign",
		...
		s.Do(
			"POST",
			"/api/latest/mdmlab/mdm/apple/profiles/match",
	...
	//       mdmlabdm::profile { 'n4':
	//         template => template('mdmlabdm/n4.mobileconfig.erb'),
	//         group    => 'kiosks',
	//       }
	//   }
	puppetRun := func(host *mdmlab.Host) {
		payload := []mdmlab.MDMApplePreassignProfilePayload{
			{
		...
		if host.UUID == host2.UUID {
			payload = append(payload, mdmlab.MDMApplePreassignProfilePayload{
				ExternalHostIdentifier: host.Hostname,
				HostUUID:               host.UUID,
				Profile:                prof4,
				Group:                  "kiosks",
	...
	//       mdmlabdm::profile { 'n3':
	//         template => template('mdmlabdm/n3.mobileconfig.erb'),
	//         group    => 'no-nudge',
	//       }
	//   } else {
	//       mdmlabdm::profile { 'n3':
	...
	// }
	puppetRun = func(host *mdmlab.Host) {
		manifest := []mdmlab.MDMApplePreassignProfilePayload{
			{
				ExternalHostIdentifier: host.Hostname,
				HostUUID:               host.UUID,
				Profile:                prof1,
		...
		if host.UUID == host3.UUID {
			manifest = append(manifest, mdmlab.MDMApplePreassignProfilePayload{
				ExternalHostIdentifier: host.Hostname,
				HostUUID:               host.UUID,
				Profile:                prof3,
				Group:                  "no-nudge",
...
func (s *integrationMDMTestSuite) TestMDMAppleListConfigProfiles() {
	...
	testTeam, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "TestTeam"})
	require.NoError(t, err)
	...
	t.Run("no profiles", func(t *testing.T) {
		...
		s.DoJSON("GET", "/api/v1/mdmlab/mdm/apple/profiles", nil, http.StatusOK, &listResp)
		...
		s.DoJSON("GET", fmt.Sprintf(`/api/v1/mdmlab/mdm/apple/profiles?team_id=%d`, testTeam.ID), nil, http.StatusOK, &listResp)
		require.NotNil(t, listResp.ConfigProfiles) // expect empty slice instead of nil
		require.Len(t, listResp.ConfigProfiles, 0)

		var hostProfilesResp getHostProfilesResponse
		s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/%d/configuration_profiles", mdmHost.ID), nil, http.StatusOK, &hostProfilesResp)
	...
	t.Run("with profiles", func(t *testing.T) {
		p1, err := mdmlab.NewMDMAppleConfigProfile(mcBytesForTest("p1", "p1.identifier", "p1.uuid"), nil)
		require.NoError(t, err)
		_, err = s.ds.NewMDMAppleConfigProfile(ctx, *p1)
		require.NoError(t, err)

		p2, err := mdmlab.NewMDMAppleConfigProfile(mcBytesForTest("p2", "p2.identifier", "p2.uuid"), &testTeam.ID)
		require.NoError(t, err)
		_, err = s.ds.NewMDMAppleConfigProfile(ctx, *p2)
		require.NoError(t, err)

		var resp listMDMAppleConfigProfilesResponse
		s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/profiles", listMDMAppleConfigProfilesRequest{TeamID: 0}, http.StatusOK, &resp)
		...
		s.DoJSON("GET", fmt.Sprintf(`/api/v1/mdmlab/mdm/apple/profiles?team_id=%d`, testTeam.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.ConfigProfiles)
		require.Len(t, resp.ConfigProfiles, 1)
		require.Equal(t, p2.Name, resp.ConfigProfiles[0].Name)
		require.Equal(t, p2.Identifier, resp.ConfigProfiles[0].Identifier)

		p3, err := mdmlab.NewMDMAppleConfigProfile(mcBytesForTest("p3", "p3.identifier", "p3.uuid"), &testTeam.ID)
		require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestAppConfigMDMCustomSettings() {
	...
	assert.Equal(t, []mdmlab.MDMProfileSpec{{Path: "foo", LabelsIncludeAll: []string{"baz"}}, {Path: "bar"}}, acResp.MDM.MacOSSettings.CustomSettings)
	...
	assert.Equal(t, []mdmlab.MDMProfileSpec{{Path: "foo", LabelsIncludeAll: []string{"baz"}}, {Path: "bar"}}, acResp.MDM.MacOSSettings.CustomSettings)
	assert.Equal(t, optjson.SetSlice([]mdmlab.MDMProfileSpec{{Path: "foo", LabelsExcludeAny: []string{"x", "y"}}, {Path: "bar", LabelsIncludeAll: []string{"a", "b"}}, {Path: "baz", LabelsIncludeAll: []string{"c"}}}), acResp.MDM.WindowsSettings.CustomSettings)
	...
	assert.Equal(t, optjson.Slice[mdmlab.MDMProfileSpec]{Set: true, Value: []mdmlab.MDMProfileSpec{}}, acResp.MDM.WindowsSettings.CustomSettings)
...
func (s *integrationMDMTestSuite) TestApplyTeamsMDMAppleProfiles() {
	...
	require.Equal(t, []mdmlab.MDMProfileSpec{
	...
	teamResp = getTeamResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
	require.Equal(t, []mdmlab.MDMProfileSpec{}, teamResp.Team.Config.MDM.MacOSSettings.CustomSettings)
	...
	res = s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
...
func (s *integrationMDMTestSuite) TestBatchSetMDMAppleProfiles() {
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: nil}, http.StatusNoContent)
	s.lastActivityMatches(
		mdmlab.ActivityTypeEditedMacosProfile{}.ActivityName(),
	...
	// Profile is too big
	resp := s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{[]byte(bigString)}},
		http.StatusUnprocessableEntity)
	...
	for p := range mobileconfig.MDMlabPayloadIdentifiers() {
		res := s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{
			mobileconfigForTest("N1", "I1"),
	...
	for p := range mobileconfig.MDMlabPayloadTypes() {
		res := s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{
...
func (s *integrationMDMTestSuite) TestHostMDMAppleProfilesStatus() {
	...
	createManualMDMEnrollWithOrbit := func(secret string) *mdmlab.Host {
	...
	triggerReconcileProfiles := func() {
		...
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
			_, err := q.ExecContext(ctx, `UPDATE host_mdm_apple_profiles SET status = ? WHERE status = ?`, mdmlab.OSSettingsVerifying, mdmlab.OSSettingsPending)
			return err
		})
	}

	assignHostToTeam := func(h *mdmlab.Host, teamID *uint) {
		var moveHostResp addHostsToTeamResponse
		s.DoJSON("POST", "/api/v1/mdmlab/hosts/transfer",
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret",
		applyEnrollSecretSpecRequest{
			Spec: &mdmlab.EnrollSecretSpec{
				Secrets: []*mdmlab.EnrollSecret{{Secret: globalEnrollSec}},
			},
		}, http.StatusOK, &applyResp)

	// create a team with a couple profiles
	tm1, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team_profiles_status_1"})
	require.NoError(t, err)
	tm1Profiles := [][]byte{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", tm1.ID),
		modifyTeamEnrollSecretsRequest{
			Secrets: []mdmlab.EnrollSecret{{Secret: tm1EnrollSec}},
		}, http.StatusOK, &teamResp)

	// create another team with different profiles
	tm2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team_profiles_status_2"})
	require.NoError(t, err)
	tm2Profiles := [][]byte{
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "G1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
		},
		h2: {
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h3: {
			{Identifier: "T1.1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T1.2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
		},
		h4: {
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "G1", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T2.1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h2: {
			{Identifier: "G1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	})
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h3: {
			{Identifier: "T1.1", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T1.2", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T2.1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h4: {
			{Identifier: "T1.1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "T1.2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	})
	...
	s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusOK, headers)
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h2: {
			{Identifier: "G1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
		},
		h4: {
			{Identifier: "G1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "T2.1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "T2.2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h3: {
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", g1ProfID),
		deleteMDMAppleConfigProfileRequest{}, http.StatusOK, &delProfResp)
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h2: {
			{Identifier: "G1", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h4: {
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", tm21ProfID),
		deleteMDMAppleConfigProfileRequest{}, http.StatusOK, &delProfResp)
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "T2.1", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T2.2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h3: {
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/profiles/batch",
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h2: {
			{Identifier: "G2", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G3", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h4: {
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "T2.2", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T2.2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T2.3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h3: {
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "T2.2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "T2.3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h2: {
			{Identifier: "G2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h3: {
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm2.ID), nil, http.StatusOK)
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "T2.2b", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "T2.3", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "G4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(context.Background(), s.ds, h1, map[string]*mdmlab.HostMacOSProfile{
		"G2b": {Identifier: "G2b", DisplayName: "G2b", InstallDate: time.Now()},
	}))
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: "G2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerified},
			{Identifier: "G4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	...
	label, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "test label 1", Query: "select 1;"})
	require.NoError(t, err)
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "T1.3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "label_prof", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
		h2: {
			{Identifier: "G2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		h1: {
			...
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "T1.3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "label_prof", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerified},
		},
		h2: {
			{Identifier: "G2b", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "G4", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
...
func (s *integrationMDMTestSuite) TestMDMConfigProfileCRUD() {
	...
	assertAppleProfile := func(filename, name, ident string, teamID uint, labelNames []string, wantStatus int, wantErrMsg string) string {
		...
		res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/configuration_profiles", body.Bytes(), wantStatus, headers)
  ...
  "Identifier": "%s"
		...
		require.NoError(t, err)
		require.NotEmpty(t, resp.ProfileUUID)
		require.Equal(t, mdmlab.MDMAppleDeclarationUUIDPrefix, string(resp.ProfileUUID[0]))
		return resp.ProfileUUID
	}
	...
	createAppleProfile := func(name, ident string, teamID uint, labelNames []string) string {
		...
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeCreatedMacosProfile{}.ActivityName(), wantJSON, 0)
	...
	createAppleDeclaration := func(name, ident string, teamID uint, labelNames []string) string {
		...
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeCreatedDeclarationProfile{}.ActivityName(), wantJSON, 0)
	...
	createWindowsProfile := func(name string, teamID uint, labels []string) string {
		...
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeCreatedWindowsProfile{}.ActivityName(), wantJSON, 0)
	...
	assertWindowsProfile("bitlocker.xml", syncml.MDMlabBitLockerTargetLocURI, 0, nil, http.StatusBadRequest, "Couldn't upload. Custom configuration profiles can't include BitLocker settings.")
	assertWindowsProfile("updates.xml", syncml.MDMlabOSUpdateTargetLocURI, testTeam.ID, nil, http.StatusBadRequest, "Couldn't upload. Custom configuration profiles can't include Windows updates settings.")

	// MDMlab-reserved profiles
	for name := range servermdm.MDMlabReservedProfileNames() {
	...
	labelFoo := &mdmlab.Label{Name: "foo", Query: "select * from foo;"}
	labelFoo, err = s.ds.NewLabel(context.Background(), labelFoo)
	require.NoError(t, err)
	labelBar := &mdmlab.Label{Name: "bar", Query: "select * from bar;"}
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/configuration_profiles", body.Bytes(), http.StatusBadRequest, headers)
	errMsg := extractServerErrorText(res.Body)
	...
	expectedProfiles := []mdmlab.MDMConfigProfilePayload{
		...
		{ProfileUUID: teamWinProfUUID, Platform: "windows", Name: "win-team-profile", TeamID: &testTeam.ID},
		{
			...
			LabelsIncludeAll: []mdmlab.ConfigurationProfileLabel{
		...
		},
		{
			...
			LabelsIncludeAny: []mdmlab.ConfigurationProfileLabel{
		...
		},
		{
			...
			LabelsExcludeAny: []mdmlab.ConfigurationProfileLabel{
				{LabelID: labelBar.ID, LabelName: labelBar.Name},
				{LabelID: labelFoo.ID, LabelName: labelFoo.Name},
			},
		},
	...
	for _, prof := range expectedProfiles {
		...
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", prof.ProfileUUID), nil, http.StatusOK, &getResp)
		...
		resp := s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", prof.ProfileUUID), nil, http.StatusOK, "alt", "media")
		...
		} else if strings.HasPrefix(prof.ProfileUUID, mdmlab.MDMAppleDeclarationUUIDPrefix) {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", "ano-such-profile"), nil, http.StatusNotFound, &getResp)
	s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", "ano-such-profile"), nil, http.StatusNotFound, "alt", "media")
	// get an unknown Apple declaration
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", fmt.Sprintf("%sno-such-profile", mdmlab.MDMAppleDeclarationUUIDPrefix)), nil, http.StatusNotFound, &getResp)
	s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", fmt.Sprintf("%sno-such-profile", mdmlab.MDMAppleDeclarationUUIDPrefix)), nil, http.StatusNotFound, "alt", "media")
	// get an unknown Windows profile
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", "wno-such-profile"), nil, http.StatusNotFound, &getResp)
	s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", "wno-such-profile"), nil, http.StatusNotFound, "alt", "media")

	var deleteResp deleteMDMConfigProfileResponse
	// delete existing Apple profiles
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", noTeamAppleProfUUID), nil, http.StatusOK, &deleteResp)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", teamAppleProfUUID), nil, http.StatusOK, &deleteResp)
	// delete non-existing Apple profile
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", "ano-such-profile"), nil, http.StatusNotFound, &deleteResp)

	// delete existing Apple declaration
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", uuidAppleDDMWithLabel), nil, http.StatusOK, &deleteResp)
	s.lastActivityOfTypeMatches(
		mdmlab.ActivityTypeDeletedDeclarationProfile{}.ActivityName(),
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", fmt.Sprintf("%sno-such-profile", mdmlab.MDMAppleDeclarationUUIDPrefix)), nil, http.StatusNotFound, &deleteResp)
	// delete existing Windows profiles
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", noTeamWinProfUUID), nil, http.StatusOK, &deleteResp)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", teamWinProfUUID), nil, http.StatusOK, &deleteResp)
	// delete non-existing Windows profile
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", "wno-such-profile"), nil, http.StatusNotFound, &deleteResp)

	// trying to create/delete profiles managed by MDMlab fails
	for p := range mobileconfig.MDMlabPayloadIdentifiers() {
		...
		var deleteResp deleteMDMConfigProfileResponse
		s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", uid), nil, http.StatusBadRequest, &deleteResp)
	...
	// make mdmlab add a FileVault profile
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{
		"mdm": { "enable_disk_encryption": true }
  }`), http.StatusOK, &acResp)
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	profile := s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabFileVaultPayloadIdentifier, true)

	// try to delete the profile
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", profile.ProfileUUID), nil, http.StatusBadRequest, &deleteResp)

	// make mdmlab add a Windows OS Updates profile
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{
		"mdm": { "windows_updates": {"deadline_days": 1, "grace_period_days": 1} }
  }`), http.StatusOK, &acResp)
	profUUID := checkWindowsOSUpdatesProfile(t, s.ds, nil, &mdmlab.WindowsUpdates{DeadlineDays: optjson.SetInt(1), GracePeriodDays: optjson.SetInt(1)})

	// try to delete the profile
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", profUUID), nil, http.StatusBadRequest, &deleteResp)
...
func (s *integrationMDMTestSuite) TestListMDMConfigProfiles() {
	...
	for i := 0; i < 5; i++ {
		...
		if i%2 == 0 {
			prof, err := mdmlab.NewMDMAppleConfigProfile(mcBytesForTest(name, name+".identifier", name+".uuid"), nil)
			require.NoError(t, err)
			_, err = s.ds.NewMDMAppleConfigProfile(ctx, *prof)
			require.NoError(t, err)

			tprof, err := mdmlab.NewMDMAppleConfigProfile(mcBytesForTest("t"+name, "t"+name+".identifier", "t"+name+".uuid"), nil)
			require.NoError(t, err)
			tprof.TeamID = &tm1.ID
			_, err = s.ds.NewMDMAppleConfigProfile(ctx, *tprof)
			require.NoError(t, err)
		} else {
			_, err = s.ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: name, SyncML: []byte(`<Replace></Replace>`)})
			require.NoError(t, err)
			_, err = s.ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{Name: "t" + name, TeamID: &tm1.ID, SyncML: []byte(`<Replace></Replace>`)})
			require.NoError(t, err)
		}
	}

	lblFoo, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "foo", Query: "select 1"})
	require.NoError(t, err)
	lblBar, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "bar", Query: "select 1"})
	require.NoError(t, err)
	lblBaz, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "baz", Query: "select 1"})
	require.NoError(t, err)

	// create a couple profiles (Win and mac) for team 2, and none for team 3
	tprof, err := mdmlab.NewMDMAppleConfigProfile(mcBytesForTest("tF", "tF.identifier", "tF.uuid"), nil)
	require.NoError(t, err)
	tprof.TeamID = &tm2.ID
	// make tm2ProfF a "exclude-any" label-based profile
	tprof.LabelsExcludeAny = []mdmlab.ConfigurationProfileLabel{
	...
	tm2ProfG, err := s.ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{
		Name:   "tG",
		TeamID: &tm2.ID,
		SyncML: []byte(`<Add></Add>`),
		LabelsIncludeAll: []mdmlab.ConfigurationProfileLabel{
	...
	tm2ProfH, err := s.ds.NewMDMWindowsConfigProfile(ctx, mdmlab.MDMWindowsConfigProfile{
		Name:   "tH",
		TeamID: &tm2.ID,
		SyncML: []byte(`<Add></Add>`),
		LabelsIncludeAny: []mdmlab.ConfigurationProfileLabel{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles", nil, http.StatusOK, &listResp, "team_id", fmt.Sprint(tm2.ID))
	...
	require.Equal(t, &mdmlab.MDMConfigProfilePayload{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles/"+tm2ProfG.ProfileUUID, nil, http.StatusOK, &getProfResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles/"+tm2ProfF.ProfileUUID, nil, http.StatusOK, &getProfResp)
	getProfResp.CreatedAt, getProfResp.UploadedAt = time.Time{}, time.Time{}
	require.Equal(t, &mdmlab.MDMConfigProfilePayload{
		ProfileUUID: tm2ProfF.ProfileUUID,
		TeamID:      tm2ProfF.TeamID,
	...
	getProfResp = getMDMConfigProfileResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles/"+tm2ProfH.ProfileUUID, nil, http.StatusOK, &getProfResp)
	getProfResp.CreatedAt, getProfResp.UploadedAt = time.Time{}, time.Time{}
	require.Equal(t, &mdmlab.MDMConfigProfilePayload{
		ProfileUUID: tm2ProfH.ProfileUUID,
		TeamID:      tm2ProfH.TeamID,
	...
	s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles", nil, http.StatusNotFound, &listResp, "team_id", "99999")
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("%v: %#v", c.teamID, c.queries), func(t *testing.T) {
			...
			s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles", nil, http.StatusOK, &listResp, queryArgs...)
...
func (s *integrationMDMTestSuite) TestWindowsProfileManagement() {
	...
	verifyHostProfileStatus := func(cmds []mdmlab.ProtoCmdOperation, wantStatus string) {
		for _, cmd := range cmds {
			...
			wantDeliveryStatus := mdmlab.WindowsResponseToDeliveryStatus(wantStatus)
			if gotProfile.Retries <= servermdm.MaxProfileRetries && wantDeliveryStatus == mdmlab.MDMDeliveryFailed {
	...
	verifyProfiles := func(device *mdmtest.TestWindowsMDMClient, n int, fail bool) {
		...
		var atomicCmds []mdmlab.ProtoCmdOperation
		...
		for _, c := range cmds {
			...
			device.AppendResponse(mdmlab.SyncMLCmd{
				XMLName: xml.Name{Local: mdmlab.CmdStatus},
				MsgRef:  &msgID,
				CmdRef:  &cmdID.Value,
				Cmd:     ptr.String(c.Verb),
	...
	checkHostsProfilesMatch := func(host *mdmlab.Host, wantUUIDs []string) {
		var gotUUIDs []string
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	checkHostDetails := func(t *testing.T, host *mdmlab.Host, wantProfs []string, wantStatus mdmlab.MDMDeliveryStatus) {
		var gotHostResp getHostResponse
		s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &gotHostResp)
		...
		for _, p := range *gotHostResp.Host.MDM.Profiles {
			...
			// MDMlab reserved profiles (e.g., OS updates) should be screened from the host details response
			require.NotContains(t, servermdm.ListMDMlabReservedWindowsProfileNames(), p.Name)
		}
		require.ElementsMatch(t, wantProfs, gotProfs)
	}

	checkHostsFilteredByOSSettingsStatus := func(t *testing.T, wantHosts []string, wantStatus mdmlab.MDMDeliveryStatus, teamID *uint, labels ...*mdmlab.Label) {
		var teamFilter string
		if teamID != nil {
			teamFilter = fmt.Sprintf("&team_id=%d", *teamID)
		}
		var gotHostsResp listHostsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts?os_settings=%s%s", wantStatus, teamFilter), nil, http.StatusOK, &gotHostsResp)
		...
		s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/count?os_settings=%s%s", wantStatus, teamFilter), nil, http.StatusOK, &countHostsResp)
		require.Equal(t, len(wantHosts), countHostsResp.Count)

		for _, l := range labels {
			gotHostsResp = listHostsResponse{}
			s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/labels/%d/hosts?os_settings=%s%s", l.ID, wantStatus, teamFilter), nil, http.StatusOK, &gotHostsResp)
			require.NotNil(t, gotHostsResp.Hosts)
			gotHosts = []string{}
			for _, h := range gotHostsResp.Hosts {
				gotHosts = append(gotHosts, h.Hostname)
			...
			s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/count?label_id=%d&os_settings=%s%s", l.ID, wantStatus, teamFilter), nil, http.StatusOK, &countHostsResp)
		}
	}
	...
	checkHostProfileStatus := func(t *testing.T, hostUUID string, profUUID string, wantStatus mdmlab.MDMDeliveryStatus) {
		var gotStatus mdmlab.MDMDeliveryStatus
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	checkHostDetails(t, host, globalProfiles, mdmlab.MDMDeliveryVerifying)

	// can't resend a profile while it is verifying
	res := s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, globalProfiles[0]), nil, http.StatusConflict)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn’t resend. Configuration profiles with “pending” or “verifying” status can’t be resent.")
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, host.ID, false, true, s.server.URL, false, mdmlab.WellKnownMDMMDMlab, ""))
	checkHostsFilteredByOSSettingsStatus(t, []string{host.Hostname}, mdmlab.MDMDeliveryVerifying, nil, label)
	s.checkMDMProfilesSummaries(t, nil, mdmlab.MDMProfilesSummary{
		Verifying: 1,
	}, nil)
	...
	// make mdmlab add a Windows OS Updates profile
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{"mdm": { "windows_updates": {"deadline_days": 1, "grace_period_days": 1} }}`), http.StatusOK, &acResp)
	osUpdatesProf := getProfileUUID(t, servermdm.MDMlabWindowsOSUpdatesProfileName, nil)
	...
	checkHostProfileStatus(t, host.UUID, osUpdatesProf, mdmlab.MDMDeliveryVerifying)
	checkHostsFilteredByOSSettingsStatus(t, []string{host.Hostname}, mdmlab.MDMDeliveryVerifying, nil, label)
	s.checkMDMProfilesSummaries(t, nil, mdmlab.MDMProfilesSummary{
		Verifying: 1,
	}, nil)
	...
	checkHostProfileStatus(t, host.UUID, osUpdatesProf, mdmlab.MDMDeliveryFailed)
	checkHostsFilteredByOSSettingsStatus(t, []string{host.Hostname}, mdmlab.MDMDeliveryVerifying, nil, label)
	s.checkMDMProfilesSummaries(t, nil, mdmlab.MDMProfilesSummary{
		Verifying: 1,
	}, nil)
	...
	checkHostProfileStatus(t, host.UUID, globalProfiles[0], mdmlab.MDMDeliveryFailed)
	checkHostsFilteredByOSSettingsStatus(t, []string{}, mdmlab.MDMDeliveryVerifying, nil, label)           // expect no hosts
	checkHostsFilteredByOSSettingsStatus(t, []string{host.Hostname}, mdmlab.MDMDeliveryFailed, nil, label) // expect host
	s.checkMDMProfilesSummaries(t, nil, mdmlab.MDMProfilesSummary{
		Failed:    1,
		Verifying: 0,
	}, nil)
	...
	_ = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/resend/%s", host.ID, globalProfiles[0]), nil,
		http.StatusAccepted)
	verifyProfiles(mdmDevice, 1, false)                                                 // trigger a profile sync, device gets the profile resent
	checkHostProfileStatus(t, host.UUID, globalProfiles[0], mdmlab.MDMDeliveryVerifying) // profile was resent, so it back to verifying
	...
	checkHostDetails(t, host, teamProfiles, mdmlab.MDMDeliveryVerifying)
	...
	// can't resend a profile while it is verifying
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, teamProfiles[0]), nil, http.StatusConflict)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn’t resend. Configuration profiles with “pending” or “verifying” status can’t be resent.")

	// can't resend a profile from the wrong team
	// purposefully using deprecated path for backwards compatibility
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/resend/%s", host.ID, globalProfiles[0]), nil, http.StatusNotFound)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Unable to match profile to host.")
	...
	// make mdmlab add a Windows OS Updates profile
	tmResp := teamResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm.ID), json.RawMessage(`{"mdm": { "windows_updates": {"deadline_days": 1, "grace_period_days": 1} }}`), http.StatusOK, &tmResp)
	osUpdatesProf = getProfileUUID(t, servermdm.MDMlabWindowsOSUpdatesProfileName, &tm.ID)
	...
	// os updates profile status doesn't matter for filtered hosts results or summaries
	checkHostProfileStatus(t, host.UUID, osUpdatesProf, mdmlab.MDMDeliveryVerifying)
	checkHostsFilteredByOSSettingsStatus(t, []string{host.Hostname}, mdmlab.MDMDeliveryVerifying, &tm.ID, label)
	s.checkMDMProfilesSummaries(t, &tm.ID, mdmlab.MDMProfilesSummary{
		Verifying: 1,
	}, nil)
	// force os updates profile to failed, doesn't impact filtered hosts results or summaries
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	checkHostProfileStatus(t, host.UUID, teamProfiles[0], mdmlab.MDMDeliveryFailed)
	checkHostsFilteredByOSSettingsStatus(t, []string{}, mdmlab.MDMDeliveryVerifying, &tm.ID, label)           // expect no hosts
	checkHostsFilteredByOSSettingsStatus(t, []string{host.Hostname}, mdmlab.MDMDeliveryFailed, &tm.ID, label) // expect host
	s.checkMDMProfilesSummaries(t, &tm.ID, mdmlab.MDMProfilesSummary{
		Failed:    1,
		Verifying: 0,
	}, nil)
	...
	// can resend a profile after it has failed
	_ = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, teamProfiles[0]), nil,
		http.StatusAccepted)
	verifyProfiles(mdmDevice, 1, false)                                               // trigger a profile sync, device gets the profile resent
	checkHostProfileStatus(t, host.UUID, teamProfiles[0], mdmlab.MDMDeliveryVerifying) // profile was resent, so back to verifying
	s.lastActivityMatches(
		mdmlab.ActivityTypeResentConfigurationProfile{}.ActivityName(),
	...
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/configuration_profiles/%s/resend", host.ID, mcUUID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Profile is not compatible with host platform")
}
...
func (s *integrationMDMTestSuite) TestApplyTeamsMDMWindowsProfiles() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", rawTeamSpec(`
		{
			"windows_settings": {
				"custom_settings": [
					{"path": "foo", "labels": ["baz"]},
	...
	var teamResp getTeamResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
	require.ElementsMatch(t, []mdmlab.MDMProfileSpec{
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", rawTeamSpec(`{ "enable_disk_encryption": true }`), http.StatusOK, &applyResp)
	require.Len(t, applyResp.TeamIDsByName, 1)
  ...
  `), http.StatusOK, &applyResp)
	...
	// apply with invalid mix of labels fails
	res := s.Do("POST", "/api/latest/mdmlab/spec/teams", rawTeamSpec(`
		{
			"windows_settings": {
				"custom_settings": [
...
func (s *integrationMDMTestSuite) TestBatchSetMDMProfiles() {
	...
	// Profile is too big
	resp := s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{{Contents: []byte(bigString)}}},
		http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(resp.Body), "Validation Failed: maximum configuration profile file size is 1 MB")

	// apply an empty set to no-team
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: nil}, http.StatusNoContent)
	...
	s.lastActivityOfTypeDoesNotMatch(
		mdmlab.ActivityTypeEditedWindowsProfile{}.ActivityName(),
		`{"team_id": null, "team_name": null}`,
		0,
	)
	s.lastActivityOfTypeDoesNotMatch(
		mdmlab.ActivityTypeEditedDeclarationProfile{}.ActivityName(),
		`{"team_id": null, "team_name": null}`,
		0,
	)

	// apply to both team id and name
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: nil},
		http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID), "team_name", tm.Name)
	...
	for p := range mobileconfig.MDMlabPayloadIdentifiers() {
		res := s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
			{Name: "N1", Contents: mobileconfigForTest("N1", "I1")},
	...
	for dt := range mdmlab.ForbiddenDeclTypes {
		res := s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
			{Name: "N1", Contents: mobileconfigForTest("N1", "I1")},
			{Name: "N3", Contents: syncMLForTest("./Foo/Bar")},
	...
	res = s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
		{Name: "N1", Contents: mobileconfigForTest("N1", "I1")},
		{Name: syncml.MDMlabBitLockerTargetLocURI, Contents: syncMLForTest(fmt.Sprintf("%s/Foo", syncml.MDMlabBitLockerTargetLocURI))},
		{Name: "N3", Contents: syncMLForTest("./Foo/Bar")},
	}}, http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID))
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
		{Name: "N1", Contents: mobileconfigForTest("N1", "I1")},
		{Name: syncml.MDMlabOSUpdateTargetLocURI, Contents: syncMLForTest(fmt.Sprintf("%s/Foo", syncml.MDMlabOSUpdateTargetLocURI))},
		{Name: "N3", Contents: syncMLForTest("./Foo/Bar")},
	}}, http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID))
	errMsg = extractServerErrorText(res.Body)
	...
	lbl1, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "L1", Query: "select 1;"})
	require.NoError(t, err)
	lbl2, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "L2", Query: "select 1;"})
	require.NoError(t, err)
	lbl3, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "L3", Query: "select 1;"})
	require.NoError(t, err)
	...
	var listResp listMDMConfigProfilesResponse
	s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles", nil, http.StatusOK, &listResp)
	require.Len(t, listResp.Profiles, 3)
	...
	require.Equal(t, listResp.Profiles[0].LabelsIncludeAll, []mdmlab.ConfigurationProfileLabel{
		{LabelID: lbl1.ID, LabelName: lbl1.Name},
		{LabelID: lbl2.ID, LabelName: lbl2.Name},
	})
	require.Nil(t, listResp.Profiles[0].LabelsExcludeAny)
	require.Equal(t, listResp.Profiles[1].LabelsIncludeAll, []mdmlab.ConfigurationProfileLabel{
		{LabelID: lbl1.ID, LabelName: lbl1.Name},
	})
	require.Nil(t, listResp.Profiles[1].LabelsExcludeAny)
	require.Equal(t, listResp.Profiles[2].LabelsExcludeAny, []mdmlab.ConfigurationProfileLabel{
		{LabelID: lbl2.ID, LabelName: lbl2.Name},
	})
	...
	require.Equal(t, "N1", listResp.Profiles[0].Name)
	require.Equal(t, "N2", listResp.Profiles[1].Name)
	require.Equal(t, listResp.Profiles[0].LabelsExcludeAny, []mdmlab.ConfigurationProfileLabel{
		{LabelID: lbl1.ID, LabelName: lbl1.Name},
	...
	"Identifier": "com.mdmlab.config.foo",
	"Payload": {
		"ServiceType": "com.apple.bash",
		"DataAssetReference": "com.mdmlab.asset.bash"
	}}`)
	mcBytes := mobileconfigForTest("N1", "I1")
	winBytes := syncMLForTest("./Foo/Bar")

	for _, p := range []struct {
		payload   []mdmlab.MDMProfileBatchPayload
		expectErr string
	}{
		{
			payload:   []mdmlab.MDMProfileBatchPayload{{Name: "N1", Contents: mcBytes}, {Name: "N1", Contents: winBytes}},
		...
		{
			payload:   []mdmlab.MDMProfileBatchPayload{{Name: "N1", Contents: declBytes}, {Name: "N1", Contents: winBytes}},
			expectErr: "More than one configuration profile have the same name 'N1' (macOS .json file name or Windows .xml file name).",
		},
		{
			payload:   []mdmlab.MDMProfileBatchPayload{{Name: "N1", Contents: mcBytes}, {Name: "N1", Contents: declBytes}},
		...
		// team profiles
		res = s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: p.payload},
			http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID))
		errMsg = extractServerErrorText(res.Body)
		require.Contains(t, errMsg, p.expectErr)
		// no team profiles
		res = s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: p.payload}, http.StatusUnprocessableEntity)
		errMsg = extractServerErrorText(res.Body)
		require.Contains(t, errMsg, p.expectErr)
	}
}
...
func (s *integrationMDMTestSuite) TestBatchSetMDMProfilesBackwardsCompat() {
	...
	// apply an empty set to no-team
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", map[string]any{"profiles": nil}, http.StatusNoContent)
	...
	// apply to both team id and name
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", map[string]any{"profiles": nil},
		http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID), "team_name", tm.Name)
	...
	// duplicate PayloadDisplayName
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", map[string]any{"profiles": map[string][]byte{
	...
	// profiles with reserved macOS identifiers
	for p := range mobileconfig.MDMlabPayloadIdentifiers() {
		res := s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", map[string]any{"profiles": map[string][]byte{
			"N1": mobileconfigForTest("N1", "I1"),
	...
	res := s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", map[string]any{"profiles": map[string][]byte{
		"N1":                              mobileconfigForTest("N1", "I1"),
		syncml.MDMlabBitLockerTargetLocURI: syncMLForTest(fmt.Sprintf("%s/Foo", syncml.MDMlabBitLockerTargetLocURI)),
		"N3":                              syncMLForTest("./Foo/Bar"),
	}}, http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID))
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Custom configuration profiles can't include BitLocker settings. To control these settings, use the mdm.enable_disk_encryption option.")
	...
	res = s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", map[string]any{"profiles": map[string][]byte{
		"N1":                             mobileconfigForTest("N1", "I1"),
		syncml.MDMlabOSUpdateTargetLocURI: syncMLForTest(fmt.Sprintf("%s/Foo", syncml.MDMlabOSUpdateTargetLocURI)),
		"N3":                             syncMLForTest("./Foo/Bar"),
	}}, http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID))
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Custom configuration profiles can't include Windows updates settings. To control these settings, use the mdm.windows_updates option.")
...
func (s *integrationMDMTestSuite) TestMDMBatchSetProfilesKeepsReservedNames() {
	...
	// ensures that the mdmlabd profile is created
	secrets, err := s.ds.GetEnrollSecrets(ctx, nil)
	require.NoError(t, err)
	if len(secrets) == 0 {
		require.NoError(t, s.ds.ApplyEnrollSecrets(ctx, nil, []*mdmlab.EnrollSecret{{Secret: t.Name()}}))
	}
	...
	checkMacProfs(nil, servermdm.ListMDMlabReservedMacOSProfileNames()...)
	checkWinProfs(nil, servermdm.ListMDMlabReservedWindowsProfileNames()...)
	...
	var testProfiles []mdmlab.MDMProfileBatchPayload
	testProfiles = append(testProfiles, mdmlab.MDMProfileBatchPayload{
		Name:     "n1",
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
	checkMacProfs(nil, servermdm.ListMDMlabReservedMacOSProfileNames()...)
	checkWinProfs(nil, append(servermdm.ListMDMlabReservedWindowsProfileNames(), "n1")...)
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testProfiles}, http.StatusNoContent)
	checkMacProfs(nil, append(servermdm.ListMDMlabReservedMacOSProfileNames(), "n2")...)
	checkWinProfs(nil, append(servermdm.ListMDMlabReservedWindowsProfileNames(), "n1")...)
	...
	s.DoJSON("POST", "/api/v1/mdmlab/teams", map[string]string{"Name": t.Name()}, http.StatusOK, &tmResp)

	// edit team mdm config to turn on disk encryption and os updates
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tmResp.Team.ID), modifyTeamRequest{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/teams/%d", tmResp.Team.ID), nil, http.StatusOK, &tmResp)
	...
	checkMacProfs(&tmResp.Team.ID, servermdm.ListMDMlabReservedMacOSProfileNames()...)
	checkWinProfs(&tmResp.Team.ID, servermdm.ListMDMlabReservedWindowsProfileNames()...)

	// batch set only windows profiles doesn't remove the reserved names
	var testTeamProfiles []mdmlab.MDMProfileBatchPayload
	testTeamProfiles = append(testTeamProfiles, mdmlab.MDMProfileBatchPayload{
		Name:     "n1",
		Contents: newWinProfile,
	})
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testTeamProfiles}, http.StatusNoContent,
		"team_id", fmt.Sprint(tmResp.Team.ID))
	checkMacProfs(&tmResp.Team.ID, servermdm.ListMDMlabReservedMacOSProfileNames()...)
	checkWinProfs(&tmResp.Team.ID, append(servermdm.ListMDMlabReservedWindowsProfileNames(), "n1")...)
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testTeamProfiles}, http.StatusNoContent,
		"team_id", fmt.Sprint(tmResp.Team.ID))
	checkMacProfs(&tmResp.Team.ID, append(servermdm.ListMDMlabReservedMacOSProfileNames(), "n2")...)
	checkWinProfs(&tmResp.Team.ID, append(servermdm.ListMDMlabReservedWindowsProfileNames(), "n1")...)

	// batch set only mac profiles doesn't remove the reserved names
	testTeamProfiles = []mdmlab.MDMProfileBatchPayload{{
		Name:     "n2",
		Contents: newMacProfile,
	}}
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: testTeamProfiles}, http.StatusNoContent,
...
func (s *integrationMDMTestSuite) TestMDMAppleConfigProfileCRUD() {
	...
	teamDelete, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "TeamDelete"})
	require.NoError(t, err)

	testProfiles := make(map[string]mdmlab.MDMAppleConfigProfile)
	generateTestProfile := func(name string, identifier string) {
		i := identifier
		if i == "" {
			i = fmt.Sprintf("%s.SomeIdentifier", name)
		}
		cp := mdmlab.MDMAppleConfigProfile{
			Name:       name,
			Identifier: i,
		}
	...
	checkGetResponse := func(resp *http.Response, expected mdmlab.MDMAppleConfigProfile) {
	...
	checkConfigProfile := func(expected mdmlab.MDMAppleConfigProfile, actual mdmlab.MDMAppleConfigProfile) {
		require.Equal(t, expected.Name, actual.Name)
		require.Equal(t, expected.Identifier, actual.Identifier)
	}

	host, _ := createHostThenEnrollMDM(s.ds, s.server.URL, t)
	s.Do("POST", "/api/latest/mdmlab/hosts/transfer", addHostsToTeamRequest{
	...
	newResp := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusOK, headers)
	var newCP mdmlab.MDMAppleConfigProfile
	...
	newResp = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusOK, headers)
	newCP = mdmlab.MDMAppleConfigProfile{}
	err = json.NewDecoder(newResp.Body).Decode(&newCP)
	require.NoError(t, err)
	require.NotEmpty(t, newCP.ProfileID)
	...
	deletePath := fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", deletedCP.ProfileID)
	var deleteResp deleteMDMAppleConfigProfileResponse
	s.DoJSON("DELETE", deletePath, nil, http.StatusOK, &deleteResp)
	// confirm deleted
	var listResp listMDMAppleConfigProfilesResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/profiles", listMDMAppleConfigProfilesRequest{TeamID: teamDelete.ID}, http.StatusOK, &listResp)
	require.Len(t, listResp.ConfigProfiles, 0)
	getPath := fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", deletedCP.ProfileID)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/profiles", nil, http.StatusOK, &listResp)
	...
	listResp = listMDMAppleConfigProfilesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/profiles", listMDMAppleConfigProfilesRequest{TeamID: testTeam.ID}, http.StatusOK, &listResp)
	require.Len(t, listResp.ConfigProfiles, 1)
	respCP = listResp.ConfigProfiles[0]
	require.Equal(t, expectedCP.Name, respCP.Name)
	checkConfigProfile(expectedCP, *respCP)
	...
	// get profile (no team)
	expectedCP = testProfiles["TestNoTeam"]
	getPath = fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", expectedCP.ProfileID)
	...
	// confirm deleted
	listResp = listMDMAppleConfigProfilesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/profiles", listMDMAppleConfigProfilesRequest{}, http.StatusOK, &listResp)
	require.Len(t, listResp.ConfigProfiles, 0)
	getPath = fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", deletedCP.ProfileID)
	_ = s.DoRawWithHeaders("GET", getPath, nil, http.StatusNotFound, map[string]string{"Authorization": fmt.Sprintf("Bearer %s", s.token)})
	...
	// fail to create new profile (no team), invalid mdmlab secret
	testProfiles["badSecrets"] = mdmlab.MDMAppleConfigProfile{
		Name:       "badSecrets",
...
`),
	...
	newResp = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusUnprocessableEntity, headers)
	errMsg := extractServerErrorText(newResp.Body)
	require.Contains(t, errMsg, "$FLEET_SECRET_INVALID")

	// trying to add/delete profiles with identifiers managed by MDMlab fails
	for p := range mobileconfig.MDMlabPayloadIdentifiers() {
		generateTestProfile("TestNoTeam", p)
		body, headers := generateNewReq("TestNoTeam", nil)
		s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusBadRequest, headers)

		generateTestProfile("TestWithTeamID", p)
		body, headers = generateNewReq("TestWithTeamID", nil)
		s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusBadRequest, headers)
		cp, err := mdmlab.NewMDMAppleConfigProfile(mobileconfigForTestWithContent("N1", "I1", p, "random", ""), nil)
		require.NoError(t, err)
	...
	// trying to add profiles with identifiers managed by MDMlab fails
	for p := range mobileconfig.MDMlabPayloadIdentifiers() {
		generateTestProfile("TestNoTeam", p)
		body, headers := generateNewReq("TestNoTeam", nil)
		s.DoRawWithHeaders("POST", "/api/latest/mdmlab/mdm/apple/profiles", body.Bytes(), http.StatusBadRequest, headers)
	...
	// trying to add profiles with names reserved by MDMlab fails
	for name := range servermdm.MDMlabReservedProfileNames() {
		cp := &mdmlab.MDMAppleConfigProfile{
			Name:         name,
		...
		cp, err := mdmlab.NewMDMAppleConfigProfile(mobileconfigForTestWithContent(
			"valid outer name",
	...
	// try to delete the profile
	deletePath = fmt.Sprintf("/api/latest/mdmlab/mdm/apple/profiles/%d", profile.ProfileID)
	deleteResp = deleteMDMAppleConfigProfileResponse{}
...
func (s *integrationMDMTestSuite) TestHostMDMProfilesExcludeLabels() {
	...
	triggerReconcileProfiles := func() {
		...
		// updating any "pending" (not NULL) profiles to "verifying"
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
			if _, err := q.ExecContext(ctx, `UPDATE host_mdm_apple_profiles SET status = ? WHERE status = ?`, mdmlab.OSSettingsVerifying, mdmlab.OSSettingsPending); err != nil {
				return err
			}
			if _, err := q.ExecContext(ctx, `UPDATE host_mdm_apple_declarations SET status = ? WHERE status = ?`, mdmlab.OSSettingsVerifying, mdmlab.OSSettingsPending); err != nil {
				return err
			}
			if _, err := q.ExecContext(ctx, `UPDATE host_mdm_windows_profiles SET status = ? WHERE status = ?`, mdmlab.OSSettingsVerifying, mdmlab.OSSettingsPending); err != nil {
				return err
			}
			return nil
		})
	...
	// run the crons immediately, will create the MDMlab-controlled profiles that
	// will then be expected to be applied (e.g. com.mdmlabdm.mdmlabd.config and
	// com.mdmlabdm.caroot)
	// first create the no-team enroll secret (required to create the mdmlab profiles)
	var applyResp applyEnrollSecretSpecResponse
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret",
		applyEnrollSecretSpecRequest{
			Spec: &mdmlab.EnrollSecretSpec{Secrets: []*mdmlab.EnrollSecret{{Secret: "super-global-secret"}}},
		}, http.StatusOK, &applyResp)
	s.awaitTriggerProfileSchedule(t)
	...
	// create a few labels
	labels := make([]*mdmlab.Label, 5)
	for i := 0; i < len(labels); i++ {
		label, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: fmt.Sprintf("label-%d", i), Query: "select 1;"})
		require.NoError(t, err)
		labels[i] = label
	}
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		appleHost: {
			{Identifier: "A1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "D3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
		},
	})
	s.assertHostWindowsConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMWindowsProfile{
		windowsHost: {
			{Name: "W2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryPending},
		},
	})
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		appleHost: {
			{Identifier: "A1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: "D3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	})
	s.assertHostWindowsConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMWindowsProfile{
		windowsHost: {
			{Name: "W2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	})

	// mark some profiles as verified (despite accepting a HostMacOSProfile struct, it supports Windows too)
	err = apple_mdm.VerifyHostMDMProfiles(ctx, s.ds, appleHost, map[string]*mdmlab.HostMacOSProfile{
		"A1": {Identifier: "A1", DisplayName: "A1", InstallDate: time.Now()},
	})
	require.NoError(t, err)
	err = apple_mdm.VerifyHostMDMProfiles(ctx, s.ds, windowsHost, map[string]*mdmlab.HostMacOSProfile{
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		appleHost: {
			{Identifier: "A1", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerified},
			{Identifier: "D3", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	...
	s.assertHostWindowsConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMWindowsProfile{
		windowsHost: {
			{Name: "W2", OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerified},
		},
	})
	...
	s.assertHostAppleConfigProfiles(map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile{
		appleHost: {
			{Identifier: "A1", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: "D3", OperationType: mdmlab.MDMOperationTypeRemove, Status: &mdmlab.MDMDeliveryPending},
			{Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
			{Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, OperationType: mdmlab.MDMOperationTypeInstall, Status: &mdmlab.MDMDeliveryVerifying},
		},
	})
...
func (s *integrationMDMTestSuite) TestOTAProfile() {
	...
	s.Do("GET", "/api/latest/mdmlab/enrollment_profiles/ota", getOTAProfileRequest{}, http.StatusOK, "enroll_secret", "not-real")
	...
	s.Do("POST", "/api/latest/mdmlab/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mdmlab.EnrollSecretSpec{
			Secrets: []*mdmlab.EnrollSecret{{Secret: globalEnrollSec}},
		},
	}, http.StatusOK)
	...
	resp := s.Do("GET", "/api/latest/mdmlab/enrollment_profiles/ota", getOTAProfileRequest{}, http.StatusOK, "enroll_secret", globalEnrollSec)
	require.NotZero(t, resp.ContentLength)
	require.Contains(t, resp.Header.Get("Content-Disposition"), `attachment;filename="mdmlab-mdm-enrollment-profile.mobileconfig"`)
	require.Contains(t, resp.Header.Get("Content-Type"), "application/x-apple-aspen-config")
	require.Contains(t, resp.Header.Get("X-Content-Type-Options"), "nosniff")
	b, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, resp.ContentLength, int64(len(b)))
	require.Contains(t, string(b), "com.mdmlabdm.mdmlab.mdm.apple.ota")
	require.Contains(t, string(b), fmt.Sprintf("%s/api/v1/mdmlab/ota_enrollment?enroll_secret=%s", cfg.ServerSettings.ServerURL, escSec))
	...
	tmpl := `
{
	...
	"Identifier": "com.mdmlab.config%d",
	"Payload": {
...
func (s *integrationMDMTestSuite) testSecretVariablesUpload(newProfileBytes func(i int) []byte,
	...
	profiles[0] = []byte(strings.ReplaceAll(string(profiles[0]), myBash, "$"+mdmlab.ServerSecretPrefix+"BASH"))
	secretProfile := profiles[1]
	profiles[1] = []byte("${" + mdmlab.ServerSecretPrefix + "PROFILE}")

	body, headers := generateNewProfileMultipartRequest(
		t, "secret-config0."+fileExtension, profiles[0], s.token, nil,
	)
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/configuration_profiles", body.Bytes(), http.StatusUnprocessableEntity, headers)
	...
	var listResp listMDMConfigProfilesResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &listResp)
	...
	for i := 0; i < numProfiles; i++ {
		s.Do("DELETE", "/api/latest/mdmlab/configuration_profiles/"+profileUUIDs[i], nil, http.StatusOK)
	}
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &listResp)
...
func (s *integrationMDMTestSuite) TestAppleProfileDeletion() {
	...
	mysql.ExecAdhocSQL(t, s.ds, func(e sqlx.ExtContext) error {
		_, err := e.ExecContext(ctx, `INSERT INTO host_emails (email, host_id, source) VALUES (?, ?, ?)`, "idp@example.com", host.ID,
			mdmlab.DeviceMappingMDMIdpAccounts)
		return err
	})
	...
	// Add a profile with a MDMlab variable. We are also testing that removal of a profile with a MDMlab variable works.
	// A unique command is created for each host when this MDMlab variable is used.
	globalProfilesPlusOne := [][]byte{
		globalProfiles[0],
		mobileconfigForTest("N2", "$FLEET_VAR_"+MDMlabVarHostEndUserEmailIDP),
	}
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: globalProfilesPlusOne},
		http.StatusNoContent)
	// trigger a profile sync
	s.awaitTriggerProfileSchedule(t)
	...
	sendErrorOnRemoveProfile := func(device *mdmtest.TestAppleMDMClient) {
		// The host grabs the removal command from MDMlab
		cmd, err := device.Idle()
		require.NoError(t, err)
	...
	// The host grabs the profile from MDMlab
	cmd, err := mdmDevice.Idle()
	require.NoError(t, err)
	assert.Equal(t, "InstallProfile", cmd.Command.RequestType)
	// Verify that the MDMlab variable was replaced with the IdP email
	type Command struct {
		Command struct {
			Payload []byte
		}
	...
	require.NoError(t, err)
	assert.NotContains(t, string(p.Command.Payload), "$FLEET_VAR_"+MDMlabVarHostEndUserEmailIDP)
