...
func TestStreamCampaignResultsClosesReditOnWSClose(t *testing.T) {
	...
	ds.CountHostsInTargetsFunc = func(ctx context.Context, filter mdmlab.TeamFilter, targets mdmlab.HostTargets, now time.Time) (mdmlab.TargetMetrics, error) {
		return mdmlab.TargetMetrics{TotalHosts: 1}, nil
	}
	...
	host := &mdmlab.Host{ID: 1, Platform: "windows"}
	...
	s := httptest.NewServer(pathHandler("/api/latest/mdmlab/results/"))
	defer s.Close()
	// Convert http://127.0.0.1 to ws://127.0.0.1
	u := "ws" + strings.TrimPrefix(s.URL, "http") + "/api/latest/mdmlab/results/websocket"
	...
	ds.MarkSessionAccessedFunc = func(context.Context, *mdmlab.Session) error {
		return nil
	}
	...
	ds.DistributedQueryCampaignTargetIDsFunc = func(ctx context.Context, id uint) (targets *mdmlab.HostTargets, err error) {
		return &mdmlab.HostTargets{HostIDs: []uint{1}}, nil
	}
...
func testUpdateStats(t *testing.T, ds *mysql.Datastore, usingReplica bool) {
	...
	var err error
	var aggStats mdmlab.AggregatedStats
	done := make(chan struct{}, 1)
	go func() {
		for {
			aggStats, err = mysql.GetAggregatedStats(ctx, svc.ds.(*mysql.Datastore), mdmlab.AggregatedStatsTypeScheduledQuery, queryID)
	...
	case <-time.After(2 * time.Second):
		...
		aggStats, err = mysql.GetAggregatedStats(
			ctxdb.RequirePrimary(ctx, true), svc.ds.(*mysql.Datastore), mdmlab.AggregatedStatsTypeScheduledQuery, queryID,
		)
		if err != nil {
	...
	var newAggStats mdmlab.AggregatedStats
	done = make(chan struct{}, 1)
	go func() {
		for {
			newAggStats, err = mysql.GetAggregatedStats(ctx, svc.ds.(*mysql.Datastore), mdmlab.AggregatedStatsTypeScheduledQuery, queryID)
...
func TestCalculateOutputSize(t *testing.T) {
	createResult := func() *mdmlab.DistributedQueryResult {
		result := mdmlab.DistributedQueryResult{}
	...
	t.Run(
		"output size empty", func(t *testing.T) {
			...
			size := calculateOutputSize(&tracker, &mdmlab.DistributedQueryResult{})
			require.Equal(t, uint64(0), size)
		},
	)
	t.Run(
