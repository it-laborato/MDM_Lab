...
// EnrollOrbitRequest is the request Orbit instances use to enroll to MDMlab.
...
func (r EnrollOrbitResponse) hijackRender(ctx context.Context, w http.ResponseWriter) {
	writeCapabilitiesHeader(w, mdmlab.GetServerOrbitCapabilities())
...
func enrollOrbitEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*EnrollOrbitRequest)
	nodeKey, err := svc.EnrollOrbit(ctx, mdmlab.OrbitHostInfo{
...
func (svc *Service) AuthenticateOrbitHost(ctx context.Context, orbitNodeKey string) (*mdmlab.Host, bool, error) {
	svc.authz.SkipAuthorization(ctx)

	if orbitNodeKey == "" {
		return nil, false, ctxerr.Wrap(ctx, mdmlab.NewAuthRequiredError("authentication error: missing orbit node key"))
	}
	...
	case mdmlab.IsNotFound(err):
		return nil, false, ctxerr.Wrap(ctx, mdmlab.NewAuthRequiredError("authentication error: invalid orbit node key"))
	default:
...
// EnrollOrbit enrolls an Orbit instance to MDMlab and returns the orbit node key.
func (svc *Service) EnrollOrbit(ctx context.Context, hostInfo mdmlab.OrbitHostInfo, enrollSecret string) (string, error) {
	...
	if err != nil {
		if mdmlab.IsNotFound(err) {
			...
			// 	2. User deletes the host in MDMlab.
			// 	3. Orbit tries to re-enroll using old secret.
			return "", mdmlab.NewAuthFailedError("invalid secret")
		}
		return "", mdmlab.OrbitError{Message: err.Error()}
	}

	orbitNodeKey, err := server.GenerateRandomText(svc.config.Osquery.NodeKeySize)
	if err != nil {
		return "", mdmlab.OrbitError{Message: "failed to generate orbit node key: " + err.Error()}
	}

	appConfig, err := svc.ds.AppConfig(ctx)
	if err != nil {
		return "", mdmlab.OrbitError{Message: "app config load failed: " + err.Error()}
	}

	host, err := svc.ds.EnrollOrbit(ctx, appConfig.MDM.EnabledAndConfigured, hostInfo, orbitNodeKey, secret.TeamID)
	if err != nil {
		return "", mdmlab.OrbitError{Message: "failed to enroll " + err.Error()}
	}

	if err := svc.NewActivity(
		ctx,
		nil,
		mdmlab.ActivityTypeMDMlabEnrolled{
			HostSerial:      hostInfo.HardwareSerial,
			HostDisplayName: host.DisplayName(),
		},
	); err != nil {
		level.Error(svc.logger).Log("msg", "record mdmlab enroll activity", "err", err)
	}

	return orbitNodeKey, nil
}

func getOrbitConfigEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetOrbitConfig(ctx context.Context) (mdmlab.OrbitConfig, error) {
	// this is not a user-authenticated endpoint
	svc.authz.SkipAuthorization(ctx)

	host, ok := hostctx.FromContext(ctx)
	if !ok {
		return mdmlab.OrbitConfig{}, mdmlab.OrbitError{Message: "internal error: missing host from request context"}
	}

	appConfig, err := svc.ds.AppConfig(ctx)
	if err != nil {
		return mdmlab.OrbitConfig{}, err
	}

	isConnectedToMDMlabMDM, err := svc.ds.IsHostConnectedToMDMlabMDM(ctx, host)
	if err != nil {
		return mdmlab.OrbitConfig{}, ctxerr.Wrap(ctx, err, "checking if host is connected to MDMlab")
	}

	mdmInfo, err := svc.ds.GetHostMDM(ctx, host.ID)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return mdmlab.OrbitConfig{}, ctxerr.Wrap(ctx, err, "retrieving host mdm info")
	}

	// set the host's orbit notifications for macOS MDM
	var notifs mdmlab.OrbitConfigNotifications
	if appConfig.MDM.EnabledAndConfigured && host.IsOsqueryEnrolled() && host.Platform == "darwin" {
		...
		manualMigrationEligible, err := mdmlab.IsEligibleForManualMigration(host, mdmInfo, isConnectedToMDMlabMDM)
		if err != nil {
			return mdmlab.OrbitConfig{}, ctxerr.Wrap(ctx, err, "checking manual migration eligibility")
		}

		if appConfig.MDM.MacOSMigration.Enable &&
			(mdmlab.IsEligibleForDEPMigration(host, mdmInfo, isConnectedToMDMlabMDM) || manualMigrationEligible) {
			notifs.NeedsMDMMigration = true
		}

		if isConnectedToMDMlabMDM {
			...
			if err != nil && !mdmlab.IsNotFound(err) {
				return mdmlab.OrbitConfig{}, ctxerr.Wrap(ctx, err, "checking if host is in setup experience")
			}
			...
			if inSetupAssistant {
				// If the client is running a mdmlabd that doesn't support setup
				// experience, then we should fall back to the "old way" of releasing
				// the device.
				mp, ok := capabilities.FromContext(ctx)
				if !ok || !mp.Has(mdmlab.CapabilitySetupExperience) {
					...
					if err := svc.processReleaseDeviceForOldMDMlabd(ctx, host); err != nil {
						return mdmlab.OrbitConfig{}, err
					}
				}
			}
	...
	if !appConfig.MDM.WindowsEnabledAndConfigured {
		if host.IsEligibleForWindowsMDMUnenrollment(isConnectedToMDMlabMDM) {
	...
	if host.TeamID != nil {
		...
		var nudgeConfig *mdmlab.NudgeConfig
		if appConfig.MDM.EnabledAndConfigured &&
			...
			if requiresNudge {
				nudgeConfig, err = mdmlab.NewNudgeConfig(mdmConfig.MacOSUpdates)
				if err != nil {
					return mdmlab.OrbitConfig{}, err
				}
			}
		...
		if err != nil {
			return mdmlab.OrbitConfig{}, ctxerr.Wrap(ctx, err, "setting team disk encryption notifications")
		}

		var updateChannels *mdmlab.OrbitUpdateChannels
		if len(opts.UpdateChannels) > 0 {
			var uc mdmlab.OrbitUpdateChannels
	...
	if appConfig.MDM.EnabledAndConfigured &&
		...
		if requiresNudge {
			nudgeConfig, err = mdmlab.NewNudgeConfig(appConfig.MDM.MacOSUpdates)
			if err != nil {
				return mdmlab.OrbitConfig{}, err
			}
		}
	...
	if err != nil {
		return mdmlab.OrbitConfig{}, ctxerr.Wrap(ctx, err, "setting no-team disk encryption notifications")
	}
...
func (svc *Service) processReleaseDeviceForOldMDMlabd(ctx context.Context, host *mdmlab.Host) error {
	...
	if !manualRelease {
		...
		// mdmlabd has already been installed, so we only need to check for the
...
func (svc *Service) setDiskEncryptionNotifications(
	...
	case "darwin":
		...
		if !mp.Has(mdmlab.CapabilityEscrowBuddy) {
...
func (svc *Service) filterExtensionsForHost(ctx context.Context, extensions json.RawMessage, host *mdmlab.Host) (json.RawMessage, error) {
	if len(extensions) == 0 {
		return nil, nil
	}
	var extensionsInfo mdmlab.Extensions
...
// there.
func orbitPingEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func setOrUpdateDeviceTokenEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) SetOrUpdateDeviceAuthToken(ctx context.Context, deviceAuthToken string) error {
	...
	if err := svc.ds.SetOrUpdateDeviceAuthToken(ctx, host.ID, deviceAuthToken); err != nil {
		if errors.As(err, &mdmlab.ConflictError{}) {
			return err
		}
...
func getOrbitScriptEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetHostScript(ctx context.Context, execID string) (*mdmlab.HostScriptResult, error) {
	// this is not a user-authenticated endpoint
	svc.authz.SkipAuthorization(ctx)
...
type orbitPostScriptResultRequest struct {
	OrbitNodeKey string `json:"orbit_node_key"`
	*mdmlab.HostScriptResultPayload
}
...
func postOrbitScriptResultEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) SaveHostScriptResult(ctx context.Context, result *mdmlab.HostScriptResultPayload) error {
	// this is not a user-authenticated endpoint
	svc.authz.SkipAuthorization(ctx)

	host, ok := hostctx.FromContext(ctx)
	if !ok {
		return mdmlab.OrbitError{Message: "internal error: missing host from request context"}
	}
	if result == nil {
		return ctxerr.Wrap(ctx, &mdmlab.BadRequestError{Message: "missing script result"}, "save host script result")
	}
	...
	if action == "" && mdmlab.IsSetupExperienceSupported(host.Platform) {
		// this might be a setup experience script result
		if updated, err := maybeUpdateSetupExperienceStatus(ctx, svc.ds, mdmlab.SetupExperienceScriptResult{
			HostUUID:    host.UUID,
	...
	if hsr != nil {
		...
		case "uninstall":
			...
			if err := svc.NewActivity(
				ctx,
				user,
				mdmlab.ActivityTypeUninstalledSoftware{
					HostID:          host.ID,
					HostDisplayName: host.DisplayName(),
...
func putOrbitDeviceMappingEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func postOrbitDiskEncryptionKeyEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) SetOrUpdateDiskEncryptionKey(ctx context.Context, encryptionKey, clientError string) error {
	...
	if !connected {
		return badRequest("host is not enrolled with mdmlab")
	}
	...
	if clientError == "" && encryptionKey != "" {
		...
		if err != nil {
			...
			// cached at the start of the mdmlab serve process.
...
func postOrbitLUKSEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
type orbitGetSoftwareInstallResponse struct {
	Err error `json:"error,omitempty"`
	*mdmlab.SoftwareInstallDetails
}

func (r orbitGetSoftwareInstallResponse) error() error { return r.Err }

func getOrbitSoftwareInstallDetails(ctx context.Context, request any, svc mdmlab.Service) (errorer, error) {
...
func orbitDownloadSoftwareInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*orbitDownloadSoftwareInstallerRequest)

	downloadRequested := req.Alt == "media"
	if !downloadRequested {
		// TODO: confirm error handling
		return orbitDownloadSoftwareInstallerResponse{Err: &mdmlab.BadRequestError{Message: "only alt=media is supported"}}, nil
	}
...
type orbitPostSoftwareInstallResultRequest struct {
	OrbitNodeKey string `json:"orbit_node_key"`
	*mdmlab.HostSoftwareInstallResultPayload
}
...
func postOrbitSoftwareInstallResultEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) SaveHostSoftwareInstallResult(ctx context.Context, result *mdmlab.HostSoftwareInstallResultPayload) error {
	// this is not a user-authenticated endpoint
	svc.authz.SkipAuthorization(ctx)
	...
	if mdmlab.IsSetupExperienceSupported(host.Platform) {
		// this might be a setup experience software install result
		if updated, err := maybeUpdateSetupExperienceStatus(ctx, svc.ds, mdmlab.SetupExperienceSoftwareInstallResult{
			HostUUID:        host.UUID,
	...
	if status := result.Status(); status != mdmlab.SoftwareInstallPending {
		...
		if err := svc.NewActivity(
			ctx,
			user,
			mdmlab.ActivityTypeInstalledSoftware{
				HostID:          host.ID,
				HostDisplayName: host.DisplayName(),
...
type getOrbitSetupExperienceStatusResponse struct {
	Results *mdmlab.SetupExperienceStatusPayload `json:"setup_experience_results,omitempty"`
	Err     error                               `json:"error,omitempty"`
}

func (r getOrbitSetupExperienceStatusResponse) error() error { return r.Err }

func getOrbitSetupExperienceStatusEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
