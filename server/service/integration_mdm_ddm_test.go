	...
	tmpl := `
{
	...
	"Payload": {
		...
		"DataAssetReference": "com.mdmlab.asset.bash" %s
	}
}`
	...
	res := s.Do("POST", "/api/latest/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
	...
	for ft := range mdmlab.ForbiddenDeclTypes {
		res = s.Do("POST", "/api/latest/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
		{Name: "N1", Contents: decls[0]},
		{Name: "N2", Contents: decls[1]},
	}}, http.StatusNoContent)

	var resp listMDMConfigProfilesResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &resp)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/labels", &mdmlab.LabelPayload{Name: "label_1", Query: "select 1"}, http.StatusOK, &createResp)
	require.NotZero(t, createResp.Label.ID)
	require.Equal(t, "label_1", createResp.Label.Name)
	lbl1 := createResp.Label.Label

	s.DoJSON("POST", "/api/latest/mdmlab/labels", &mdmlab.LabelPayload{Name: "label_2", Query: "select 1"}, http.StatusOK, &createResp)
	require.NotZero(t, createResp.Label.ID)
...
func (s *integrationMDMTestSuite) TestMDMAppleDeviceManagementRequests() {
	...
	insertDeclaration := func(t *testing.T, decl mdmlab.MDMAppleDeclaration) {
		stmt := `
INSERT INTO mdm_apple_declarations (
	...
	insertHostDeclaration := func(t *testing.T, hostUUID string, decl mdmlab.MDMAppleDeclaration) {
		stmt := `
INSERT INTO host_mdm_apple_declarations (
	...
	noTeamDeclsByUUID := map[string]mdmlab.MDMAppleDeclaration{
		"123": {
			DeclarationUUID: "123",
			TeamID:          ptr.Uint(0),
	...
	mapDeclsByChecksum := func(byUUID map[string]mdmlab.MDMAppleDeclaration) map[string]mdmlab.MDMAppleDeclaration {
		byChecksum := make(map[string]mdmlab.MDMAppleDeclaration)
	...
	assertDeclarationResponse := func(r *http.Response, expected mdmlab.MDMAppleDeclaration) {
		require.NotNil(t, r)

		// unmarsal the response and assert it's valid
		var wantParsed mdmlab.MDMAppleDDMDeclarationResponse
		require.NoError(t, json.Unmarshal(expected.RawJSON, &wantParsed))
		var gotParsed mdmlab.MDMAppleDDMDeclarationResponse
	...
	checkTokensResp := func(t *testing.T, r mdmlab.MDMAppleDDMTokensResponse, expectedTimestamp time.Time, prevToken string) {
	...
	checkDeclarationItemsResp := func(t *testing.T, r mdmlab.MDMAppleDDMDeclarationItemsResponse, expectedDeclTok string, expectedDeclsByChecksum map[string]mdmlab.MDMAppleDeclaration) {
	...
	t.Run("Tokens", func(t *testing.T) {
		...
		noTeamDeclsByUUID["456"] = mdmlab.MDMAppleDeclaration{
			DeclarationUUID: "456",
			TeamID:          ptr.Uint(0),
	...
	t.Run("DeclarationItems", func(t *testing.T) {
		...
		// insert a new declaration
		noTeamDeclsByUUID["789"] = mdmlab.MDMAppleDeclaration{
			DeclarationUUID: "789",
			TeamID:          ptr.Uint(0),
	...
	t.Run("Status", func(t *testing.T) {
		_, err := mdmDevice.DeclarativeManagement("status", mdmlab.MDMAppleDDMStatusReport{})
		require.NoError(t, err)
	})
	...
	t.Run("Declaration", func(t *testing.T) {
		...
		// insert a new declaration
		noTeamDeclsByUUID["abc"] = mdmlab.MDMAppleDeclaration{
			DeclarationUUID: "abc",
			TeamID:          ptr.Uint(0),
...
func parseTokensResp(t *testing.T, r *http.Response) mdmlab.MDMAppleDDMTokensResponse {
	require.NotNil(t, r)
	...
	var tok mdmlab.MDMAppleDDMTokensResponse
...
func parseDeclarationItemsResp(t *testing.T, r *http.Response) mdmlab.MDMAppleDDMDeclarationItemsResponse {
	require.NotNil(t, r)
	b, err := io.ReadAll(r.Body)
	require.NoError(t, err)
	defer r.Body.Close()
	...
	// unmarshal the response to make sure it's valid
	var di mdmlab.MDMAppleDDMDeclarationItemsResponse
...
func (s *integrationMDMTestSuite) TestAppleDDMSecretVariables() {
	...
	checkDeclarationItemsResp := func(t *testing.T, r mdmlab.MDMAppleDDMDeclarationItemsResponse, expectedDeclTok string,
		expectedDeclsByToken map[string]mdmlab.MDMAppleDeclaration) {
		require.Equal(t, expectedDeclTok, r.DeclarationsToken)
		require.NotEmpty(t, r.Declarations.Activations)
		require.Empty(t, r.Declarations.Assets)
		require.Empty(t, r.Declarations.Management)
	...
	tmpl := `
{
	...
	decls[1] = []byte(strings.ReplaceAll(string(decls[1]), myBash, "$"+mdmlab.ServerSecretPrefix+"BASH"))
	secretProfile := decls[2]
	decls[2] = []byte("${" + mdmlab.ServerSecretPrefix + "PROFILE}")

	// Create declarations
	profilesReq := batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
		{Name: "N0", Contents: decls[0]},
		{Name: "N1", Contents: decls[1]},
		{Name: "N2", Contents: decls[2]},
	}}
	// First dry run
	s.Do("POST", "/api/latest/mdmlab/mdm/profiles/batch", profilesReq, http.StatusNoContent, "dry_run", "true")
	...
	// Now real run
	s.Do("POST", "/api/latest/mdmlab/mdm/profiles/batch", profilesReq, http.StatusNoContent)
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &resp)
	...
	getDeclaration := func(t *testing.T, name string) mdmlab.MDMAppleDeclaration {
		stmt := `
SELECT
	declaration_uuid,
	team_id,
...
WHERE name = ?`
		...
		var decl mdmlab.MDMAppleDeclaration
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	var gotParsed mdmlab.MDMAppleDDMDeclarationResponse
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mdmlab.asset.bash","ServiceType":"com.apple.bash0"}`, gotParsed.Payload)

	declarationPath = fmt.Sprintf("declaration/configuration/%s", nameToIdentifier["N1"])
	r, err = mdmDevice.DeclarativeManagement(declarationPath)
	require.NoError(t, err)
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mdmlab.asset.bash","ServiceType":"com.apple.bash1"}`, gotParsed.Payload)

	declarationPath = fmt.Sprintf("declaration/configuration/%s", nameToIdentifier["N2"])
	r, err = mdmDevice.DeclarativeManagement(declarationPath)
	require.NoError(t, err)
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mdmlab.asset.bash","ServiceType":"com.apple.bash2"}`, gotParsed.Payload)
	...
	require.NoError(t, err)
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mdmlab.asset.bash","ServiceType":"my.new.bash"}`, gotParsed.Payload)
	...
	// Delete the profiles
	s.Do("DELETE", "/api/latest/mdmlab/configuration_profiles/"+nameToUUID["N0"], nil, http.StatusOK)
	s.Do("DELETE", "/api/latest/mdmlab/configuration_profiles/"+nameToUUID["N1"], nil, http.StatusOK)
	s.Do("DELETE", "/api/latest/mdmlab/configuration_profiles/"+nameToUUID["N2"], nil, http.StatusOK)
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &resp)
	require.Empty(t, resp.Profiles)
}
...
func (s *integrationMDMTestSuite) TestAppleDDMReconciliation() {
	...
	deleteDeclaration := func(declUUID string) {
		...
		s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/configuration_profiles/%s", declUUID), nil, http.StatusOK, &deleteResp)
	}
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", mdmHost.ID), nil, http.StatusOK, &hostResp)
	...
	label, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: t.Name(), Query: "select 1;"})
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestAppleDDMStatusReport() {
	...
	assertHostDeclarations := func(hostUUID string, wantDecls []*mdmlab.MDMAppleHostDeclaration) {
		var gotDecls []*mdmlab.MDMAppleHostDeclaration
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	declarations := []mdmlab.MDMProfileBatchPayload{
		{Name: "N1.json", Contents: declarationForTest("I1")},
		{Name: "N2.json", Contents: declarationForTest("I2")},
	}
	// add global declarations
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: declarations}, http.StatusNoContent)
	...
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mdmlab.MDMDeliveryPending, OperationType: mdmlab.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mdmlab.MDMDeliveryPending, OperationType: mdmlab.MDMOperationTypeInstall},
	})
	...
	require.NoError(t, err)
	var items mdmlab.MDMAppleDDMDeclarationItemsResponse
	...
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mdmlab.MDMDeliveryVerifying, OperationType: mdmlab.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mdmlab.MDMDeliveryVerifying, OperationType: mdmlab.MDMOperationTypeInstall},
	})

	// host sends a partial DDM report
	report := mdmlab.MDMAppleDDMStatusReport{}
	report.StatusItems.Management.Declarations.Configurations = []mdmlab.MDMAppleDDMStatusDeclaration{
		{Active: true, Valid: mdmlab.MDMAppleDeclarationValid, Identifier: "I1", ServerToken: i1ServerToken},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mdmlab.MDMDeliveryVerified, OperationType: mdmlab.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mdmlab.MDMDeliveryVerifying, OperationType: mdmlab.MDMOperationTypeInstall},
	})
	...
	report = mdmlab.MDMAppleDDMStatusReport{}
	report.StatusItems.Management.Declarations.Configurations = []mdmlab.MDMAppleDDMStatusDeclaration{
		{Active: true, Valid: mdmlab.MDMAppleDeclarationValid, Identifier: "I2", ServerToken: "foo"},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
	...
	report.StatusItems.Management.Declarations.Configurations = []mdmlab.MDMAppleDDMStatusDeclaration{
		{Active: true, Valid: mdmlab.MDMAppleDeclarationValid, Identifier: "I1", ServerToken: i1ServerToken},
		{Active: false, Valid: mdmlab.MDMAppleDeclarationInvalid, Identifier: "I2", ServerToken: i2ServerToken},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mdmlab.MDMDeliveryVerified, OperationType: mdmlab.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mdmlab.MDMDeliveryFailed, OperationType: mdmlab.MDMOperationTypeInstall},
	})
	...
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mdmlab.MDMDeliveryVerified, OperationType: mdmlab.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mdmlab.MDMDeliveryPending, OperationType: mdmlab.MDMOperationTypeRemove},
	})
	...
	report = mdmlab.MDMAppleDDMStatusReport{}
	report.StatusItems.Management.Declarations.Configurations = []mdmlab.MDMAppleDDMStatusDeclaration{
		{Active: false, Valid: mdmlab.MDMAppleDeclarationInvalid, Identifier: "I1", ServerToken: i1ServerToken},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mdmlab.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mdmlab.MDMDeliveryFailed, OperationType: mdmlab.MDMOperationTypeInstall},
	})
}
...
func (s *integrationMDMTestSuite) TestDDMUnsupportedDevice() {
	...
	ctx := context.Background()
	mdmlabHost, mdmDevice := createHostThenEnrollMDM(s.ds, s.server.URL, t)

	getProfiles := func(h *mdmlab.Host) map[string]*mdmlab.HostMDMAppleProfile {
		profs, err := s.ds.GetHostMDMAppleProfiles(ctx, h.UUID)
		require.NoError(t, err)
		out := make(map[string]*mdmlab.HostMDMAppleProfile, len(profs))
		for _, p := range profs {
			p := p
			out[p.Identifier] = &p
		}
	...
	profs := getProfiles(mdmlabHost)
	require.Equal(t, &mdmlab.MDMDeliveryPending, profs["I1"].Status)
	require.Equal(t, &mdmlab.MDMDeliveryPending, profs["I2"].Status)
	...
	// profiles are failed
	profs = getProfiles(mdmlabHost)
	require.Equal(t, &mdmlab.MDMDeliveryFailed, profs["I1"].Status)
	require.Contains(t, profs["I1"].Detail, "Feature Disabled")
	require.Equal(t, &mdmlab.MDMDeliveryFailed, profs["I2"].Status)
