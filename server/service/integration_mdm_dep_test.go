...
func (s *integrationMDMTestSuite) TestDEPEnrollReleaseDeviceGlobal() {
	...
	// set an enroll secret, the MDMlabd configuration profile will be installed
	// on the host
	...
	// false (the default)
	noTeamProf := `{"x": 1}`
	s.Do("POST", "/api/latest/mdmlab/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            nil,
		Name:              "no-team",
	...
	s.Do("PATCH", "/api/latest/mdmlab/setup_experience", json.RawMessage(jsonMustMarshal(t, payload)), http.StatusNoContent)
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{globalProfile}}, http.StatusNoContent)

	// enable FileVault
	s.Do("PATCH", "/api/latest/mdmlab/config", json.RawMessage([]byte(`{"mdm":{"macos_settings":{"enable_disk_encryption":true}}}`)), http.StatusOK)

	s.enableABM("mdmlab_ade_test")

	// add a setup experience script to run for no team
	extraArgs := make(map[string][]string)
	body, headers := generateNewScriptMultipartRequest(t,
		"script.sh", []byte(`echo "hello"`), s.token, extraArgs)
	s.DoRawWithHeaders("POST", "/api/latest/mdmlab/setup_experience/script", body.Bytes(), http.StatusOK, headers)
	...
	s.Do("DELETE", "/api/latest/mdmlab/setup_experience/script", nil, http.StatusOK)
	for _, enableReleaseManually := range []bool{false, true} {
...
func (s *integrationMDMTestSuite) TestDEPEnrollReleaseDeviceTeam() {
	...
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "test-team-device-release"})
	require.NoError(t, err)

	// set an enroll secret, the MDMlabd configuration profile will be installed
	// on the host
	enrollSecret := "test-release-dep-device-team"
	err = s.ds.ApplyEnrollSecrets(ctx, &tm.ID, []*mdmlab.EnrollSecret{{Secret: enrollSecret}})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	signedPkg := b
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: signedPkg, Name: "pkg.pkg", TeamID: tm.ID}, http.StatusOK, "")
	...
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(fmt.Sprintf(`{
		...
		}`, "mdmlab_ade_test", tm.Name, tm.Name, tm.Name)), http.StatusOK, &acResp)
	require.NotEmpty(t, acResp.MDM.EndUserAuthentication)
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{teamProfile}}, http.StatusNoContent, "team_id", fmt.Sprint(tm.ID))

	// enable FileVault
	s.Do("PATCH", "/api/latest/mdmlab/mdm/apple/settings", json.RawMessage([]byte(fmt.Sprintf(`{"enable_disk_encryption":true,"team_id":%d}`, tm.ID))), http.StatusNoContent)
	...
	// remove the setup experience script, run the new setup experience flow when
	// there is no setup experience item to process (so it is bypassed)
	s.Do("DELETE", "/api/latest/mdmlab/setup_experience/script", nil, http.StatusOK, "team_id", fmt.Sprint(tm.ID))
	for _, enableReleaseManually := range []bool{false, true} {
		t.Run(fmt.Sprintf("enableReleaseManually=%t;bypass_flow", enableReleaseManually), func(t *testing.T) {
			s.runDEPEnrollReleaseDeviceTest(t, teamDevice, enableReleaseManually, &tm.ID, "I2", false)
		})
...
func (s *integrationMDMTestSuite) runDEPEnrollReleaseDeviceTest(t *testing.T, device godep.Device, enableReleaseManually bool, teamID *uint, customProfileIdent string, useOldMDMlabdFlow bool) {
	ctx := context.Background()
	...
	s.mockDEPResponse("mdmlab_ade_test", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		encoder := json.NewEncoder(w)
		switch r.URL.Path {
		case "/session":
		...
		case "/profile/devices":
			...
			for _, device := range prof.Devices {
				resp.Devices[device] = string(mdmlab.DEPAssignProfileResponseSuccess)
			}
	...
	if isIphone {
		...
		// expected commands: install mdmlabd, install bootstrap, install CA, install profiles
		// (custom one, mdmlabd configuration, FileVault) (not expected: account
	...
	var profileCustomSeen, profileMDMlabdSeen, profileMDMlabCASeen, profileFileVaultSeen bool
	for _, cmd := range cmds {
		...
		case "InstallProfile":
			...
			} else if strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MDMlabdConfigPayloadIdentifier)) {
				profileMDMlabdSeen = true
			} else if strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MDMlabCARootConfigPayloadIdentifier)) {
				profileMDMlabCASeen = true
			} else if strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string", mobileconfig.MDMlabFileVaultPayloadIdentifier)) &&
	...
	if isIphone {
		...
		require.False(t, profileMDMlabdSeen)
		require.True(t, profileMDMlabCASeen)
		require.False(t, profileFileVaultSeen)

		// for iDevices, mdmlabd is not installed so the rest of this test does not apply.
	...
	require.Equal(t, 0, otherCount)
	require.True(t, profileCustomSeen)
	require.True(t, profileMDMlabdSeen)
	require.True(t, profileMDMlabCASeen)
	require.True(t, profileFileVaultSeen)

	// simulate mdmlabd being installed and the host being orbit-enrolled now
	...
	// call the /config endpoint as mdmlabd would
	var orbitConfigResp orbitGetConfigResponse
	var caps mdmlab.CapabilityMap
	if useOldMDMlabdFlow {
		// important thing is that it doesn't have the CapabilitySetupExperience
		caps.PopulateFromString(string(mdmlab.CapabilityEscrowBuddy))
	} else {
		caps = mdmlab.GetOrbitClientCapabilities()
	}

	res := s.DoRawWithHeaders("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *enrolledHost.OrbitNodeKey)),
		http.StatusOK, map[string]string{mdmlab.CapabilitiesHeader: caps.String()})
	...
	if useOldMDMlabdFlow || !hasSetupExpItems {
		...
		s.DoJSON("POST", "/api/mdmlab/orbit/setup_experience/status", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *enrolledHost.OrbitNodeKey)), http.StatusOK, &statusResp)
	}
...
func (s *integrationMDMTestSuite) TestDEPProfileAssignment() {
	...
	s.Do("PATCH", "/api/latest/mdmlab/setup_experience", json.RawMessage(jsonMustMarshal(t, map[string]any{
	...
	checkPostEnrollmentCommands := func(mdmDevice *mdmtest.TestAppleMDMClient, shouldReceive bool) {
		...
		var mdmlabdCmd, installProfileCmd *micromdm.CommandPayload
		cmd, err := mdmDevice.Idle()
		require.NoError(t, err)
		for cmd != nil {
			var fullCmd micromdm.CommandPayload
			...
			if fullCmd.Command.RequestType == "InstallEnterpriseApplication" &&
				...
				strings.Contains(*fullCmd.Command.InstallEnterpriseApplication.ManifestURL, mdmlabdbase.GetPKGManifestURL()) {
				mdmlabdCmd = &fullCmd
		...
		if shouldReceive {
			// received request to install mdmlabd
			require.NotNil(t, mdmlabdCmd, "host didn't get a command to install mdmlabd")
			require.NotNil(t, mdmlabdCmd.Command, "host didn't get a command to install mdmlabd")
			...
			require.Nil(t, mdmlabdCmd, "host got a command to install mdmlabd")
	...
	checkHostDEPAssignProfileResponses := func(deviceSerials []string, expectedProfileUUID string, expectedStatus mdmlab.DEPAssignProfileResponseStatus) map[string]hostDEPRow {
	...
	checkHostCooldown := func(serial, profUUID string, status mdmlab.DEPAssignProfileResponseStatus, expectUpdatedAt *time.Time, expectRetryJobID *uint) hostDEPRow {
	...
	checkListHostDEPError := func(serial string, expectStatus string, expectError bool) *mdmlab.HostResponse {
		listHostsRes := listHostsResponse{}
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts?query=%s", serial), nil, http.StatusOK, &listHostsRes)
		require.Len(t, listHostsRes.Hosts, 1)
		...
		case "/profile/devices":
			...
			for _, device := range prof.Devices {
				...
				case expectAssignProfileResponseNotAccessible:
					resp.Devices[device] = string(mdmlab.DEPAssignProfileResponseNotAccessible)
				case expectAssignProfileResponseFailed:
					resp.Devices[device] = string(mdmlab.DEPAssignProfileResponseFailed)
				default:
					resp.Devices[device] = string(mdmlab.DEPAssignProfileResponseSuccess)
				}
			}
	...
	require.Len(t, profileAssignmentReqs[0].Devices, 1)
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[0].Devices, profileAssignmentReqs[0].ProfileUUID, mdmlab.DEPAssignProfileResponseSuccess)
	require.Len(t, profileAssignmentReqs[1].Devices, len(devices))
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[1].Devices, profileAssignmentReqs[1].ProfileUUID, mdmlab.DEPAssignProfileResponseSuccess)
	...
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts?mdm_enrollment_status=pending", nil, http.StatusOK, &listHostsRes)
	require.Len(t, listHostsRes.Hosts, len(devices))

	// searching by display name works
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts?query=%s", url.QueryEscape("MacBook Mini")), nil, http.StatusOK, &listHostsRes)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/activities", nil, http.StatusOK, &activities, "order_key", "created_at")
	found := false
	...
	// Enroll the host to be deleted. It will stay in MDMlab after deletion from DEP.
	...
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[ix2Devices].Devices, profileAssignmentReqs[ix2Devices].ProfileUUID, mdmlab.DEPAssignProfileResponseSuccess)
	...
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[ix1Device].Devices, profileAssignmentReqs[ix1Device].ProfileUUID, mdmlab.DEPAssignProfileResponseSuccess)
	...
	// delete the device from MDMlab
	var delResp deleteHostResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", mdmDeviceID), nil, http.StatusOK, &delResp)

	// the device comes back as pending
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts?query=%s", mdmDevice.UUID), nil, http.StatusOK, &listHostsRes)
	require.Len(t, listHostsRes.Hosts, 1)
	...
	// The user unenrolls from MDMlab (e.g. was DEP enrolled but with `is_mdm_removable: true`
	...
	// Simulate mdmlabd re-enrolling automatically.
	err = mdmDevice.Enroll()
	require.NoError(t, err)
	...
	// enroll a host into MDMlab
	eHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		ID:             1,
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, eHost.ID, false, true, s.server.URL, true, mdmlab.WellKnownMDMMDMlab, ""))
	...
	d := checkHostCooldown(eHost.HardwareSerial, profUUID, mdmlab.DEPAssignProfileResponseFailed, nil, expectNoJobID)
	...
	checkHostCooldown(eHost.HardwareSerial, profUUID, mdmlab.DEPAssignProfileResponseFailed, &failedAt, expectNoJobID) // no change
	checkNoJobsPending()
	...
	// s.Do("DELETE", fmt.Sprintf("/api/v1/mdmlab/teams/%d", dummyTeam.ID), nil, http.StatusOK)
	// checkPendingMacOSSetupAssistantJob("team_deleted", nil, []string{eHost.HardwareSerial}, 0)
	// s.runIntegrationsSchedule()
	// require.Empty(t, profileAssignmentReqs) // screened for cooldown
	// bySerial = checkHostDEPAssignProfileResponses([]string{eHost.HardwareSerial}, profUUID, mdmlab.DEPAssignProfileResponseFailed)
	...
	d = checkHostCooldown(eHost.HardwareSerial, profUUID, mdmlab.DEPAssignProfileResponseFailed, &failedAt, nil)
	...
	s.runDEPSchedule()
	require.Empty(t, profileAssignmentReqs)                                                                    // assign profile request will be made when the retry job is processed on the next worker run
	checkHostCooldown(eHost.HardwareSerial, profUUID, mdmlab.DEPAssignProfileResponseFailed, &failedAt, &jobID) // no change
	checkPendingMacOSSetupAssistantJob("hosts_cooldown", nil, []string{eHost.HardwareSerial}, jobID)
	checkListHostDEPError(eHost.HardwareSerial, "On (automatic)", true)
	...
	d = checkHostCooldown(eHost.HardwareSerial, profUUID, mdmlab.DEPAssignProfileResponseSuccess, nil, expectNoJobID) // retry job cleared
	...
	s.runIntegrationsSchedule()
	require.Empty(t, profileAssignmentReqs)
	checkHostCooldown(eHost.HardwareSerial, profUUID, mdmlab.DEPAssignProfileResponseSuccess, &succeededAt, expectNoJobID) // no change
	checkNoJobsPending()
	...
	profUUID = profileAssignmentReqs[0].ProfileUUID
	d = checkHostCooldown(serial, profUUID, mdmlab.DEPAssignProfileResponseFailed, nil, expectNoJobID)
	require.NotZero(t, d.ResponseUpdatedAt)
	failedAt = d.ResponseUpdatedAt
	checkNoJobsPending()
	...
	checkHostCooldown(serial, profUUID, mdmlab.DEPAssignProfileResponseFailed, &failedAt, expectNoJobID) // no change
	checkNoJobsPending()
	...
	s.runIntegrationsSchedule()
	require.Empty(t, profileAssignmentReqs) // assign profile request will be made when the retry job is processed on the next worker run
	d = checkHostCooldown(serial, profUUID, mdmlab.DEPAssignProfileResponseFailed, &failedAt, nil)
	require.NotZero(t, d.RetryJobID) // retry job created
	jobID = d.RetryJobID
	...
	profUUID = profileAssignmentReqs[0].ProfileUUID
	d = checkHostCooldown(serial, profUUID, mdmlab.DEPAssignProfileResponseSuccess, nil, expectNoJobID) // retry job cleared
	require.True(t, d.ResponseUpdatedAt.After(failedAt))
	checkNoJobsPending()
	...
	for _, req := range profileAssignmentReqs {
		...
		d := checkHostCooldown(serial, req.ProfileUUID, mdmlab.DEPAssignProfileResponseNotAccessible, nil, expectNoJobID) // not accessible responses aren't retried
		require.NotZero(t, d.ResponseUpdatedAt)
		failedAt = d.ResponseUpdatedAt
	}
	...
	s.runIntegrationsSchedule()
	require.Empty(t, profileAssignmentReqs)
	checkHostCooldown(serial, expectProfileUUID, mdmlab.DEPAssignProfileResponseNotAccessible, &failedAt, expectNoJobID) // no change
	checkNoJobsPending()
...
func (s *integrationMDMTestSuite) TestDEPProfileAssignmentWithMultipleABMs() {
	...
	checkHostDEPAssignProfileResponses := func(deviceSerials []string, expectedProfileUUID string,
		expectedStatus mdmlab.DEPAssignProfileResponseStatus,
	...
	checkHostDEPAssignProfileResponses(defaultSerials, defaultProfileUUIDs[len(defaultProfileUUIDs)-1],
		mdmlab.DEPAssignProfileResponseSuccess)
	checkHostDEPAssignProfileResponses(teamSerials, teamProfileUUIDs[len(teamProfileUUIDs)-1], mdmlab.DEPAssignProfileResponseSuccess)
...
func (s *integrationMDMTestSuite) TestSetupExperienceScript() {
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/setup_experience/script", body.Bytes(), http.StatusOK, headers)
	...
	s.DoRawWithHeaders("POST", "/api/latest/mdmlab/setup_experience/script", body.Bytes(), http.StatusUnprocessableEntity, headers)

	// get team script metadata
	var getScriptResp getSetupExperienceScriptResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusOK, &getScriptResp)
	...
	res = s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d&alt=media", tm.ID), nil, http.StatusOK)
	b, err := io.ReadAll(res.Body)
	require.NoError(t, err)
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script42.sh", []byte(`echo "hello"`), s.token, map[string][]string{"team_id": {fmt.Sprintf("%d", tm.ID)}})
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/setup_experience/script", body.Bytes(), http.StatusConflict, headers)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/setup_experience/script", nil, http.StatusOK, &getScriptResp)
	require.Equal(t, "script42.sh", getScriptResp.Name)
	...
	require.NotZero(t, getScriptResp.UpdatedAt)
	// // TODO: confirm if we will allow team_id=0 requests
	// s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d", noTeamID), nil, http.StatusOK, &getScriptResp)

	// get no-team script contents
	res = s.Do("GET", "/api/latest/mdmlab/setup_experience/script?alt=media", nil, http.StatusOK)
	b, err = io.ReadAll(res.Body)
	require.NoError(t, err)
	require.Equal(t, `echo "hello"`, string(b))
	require.Equal(t, int64(len(`echo "hello"`)), res.ContentLength)
	require.Equal(t, fmt.Sprintf("attachment;filename=\"%s %s\"", time.Now().Format(time.DateOnly), "script42.sh"), res.Header.Get("Content-Disposition"))
	// // TODO: confirm if we will allow team_id=0 requests
	// res = s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d&alt=media", noTeamID), nil, http.StatusOK)

	// delete the no-team script
	s.Do("DELETE", "/api/latest/mdmlab/setup_experience/script", nil, http.StatusOK)

	// try get the no-team script
	s.Do("GET", "/api/latest/mdmlab/setup_experience/script", nil, http.StatusNotFound)

	// try deleting the no-team script again
	s.Do("DELETE", "/api/latest/mdmlab/setup_experience/script", nil, http.StatusOK) // TODO: confirm if we want to return not found

	// // TODO: confirm if we will allow team_id=0 requests
	// s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script/?team_id=%d", noTeamID), nil, http.StatusOK)

	// delete the team script
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusOK)

	// try get the team script
	s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusNotFound)

	// try deleting the team script again
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusOK) // TODO: confirm if we want to return not found
}

func (s *integrationMDMTestSuite) createTeamDeviceForSetupExperienceWithProfileSoftwareAndScript() (device godep.Device, host *mdmlab.Host, tm *mdmlab.Team) {
	t := s.T()
	ctx := context.Background()

	// enroll a device in a team with software to install and a script to execute
	s.enableABM("mdmlab-setup-experience")
	...
	s.DoJSON("PUT", "/api/v1/mdmlab/setup_experience/software", putSetupExperienceSoftwareRequest{TeamID: tm.ID, TitleIDs: []uint{titleID}}, http.StatusOK, &swInstallResp)
	...
	s.mockDEPResponse("mdmlab-setup-experience", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		encoder := json.NewEncoder(w)
		switch r.URL.Path {
		case "/session":
			err := encoder.Encode(map[string]string{"auth_session_token": "xyz"})
...
func (s *integrationMDMTestSuite) TestSetupExperienceFlowWithSoftwareAndScriptAutoRelease() {
	...
	// expected commands: install mdmlabd (install enterprise), install profiles
	// (custom one, mdmlabd configuration, mdmlab CA root)
	...
	for _, cmd := range cmds {
		...
		case "InstallProfile":
			...
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), "<string>I1</string>"):
				profileCustomSeen = true
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MDMlabdConfigPayloadIdentifier)):
				profileMDMlabdSeen = true
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MDMlabCARootConfigPayloadIdentifier)):
				profileMDMlabCASeen = true
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string", mobileconfig.MDMlabFileVaultPayloadIdentifier)) &&
				strings.Contains(string(cmd.Command.InstallProfile.Payload), "ForceEnableInSetupAssistant"):
				profileFileVaultSeen = true
			}
	...
	require.Len(t, statusResp.Results.ConfigurationProfiles, 3) // mdmlabd config, root CA, custom profile
	var profNames []string
	var profStatuses []mdmlab.MDMDeliveryStatus
	...
	require.ElementsMatch(t, []string{"N1", "MDMlabd configuration", "MDMlab root certificate authority (CA)"}, profNames)
	require.ElementsMatch(t, []mdmlab.MDMDeliveryStatus{mdmlab.MDMDeliveryVerifying, mdmlab.MDMDeliveryVerifying, mdmlab.MDMDeliveryVerifying}, profStatuses)
	...
	require.Equal(t, mdmlab.SetupExperienceStatusPending, statusResp.Results.Script.Status)
	require.Len(t, statusResp.Results.Software, 1)
	require.Equal(t, "DummyApp.app", statusResp.Results.Software[0].Name)
	require.Equal(t, mdmlab.SetupExperienceStatusPending, statusResp.Results.Software[0].Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", *statusResp.Results.Software[0].SoftwareTitleID), nil, http.StatusOK, &getSoftwareTitleResp, "team_id", fmt.Sprintf("%d", *enrolledHost.TeamID))
	require.NotNil(t, getSoftwareTitleResp.SoftwareTitle)
	require.NotNil(t, getSoftwareTitleResp.SoftwareTitle.SoftwarePackage)

	debugPrintActivities := func(activities []*mdmlab.Activity) []string {
		var res []string
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", enrolledHost.ID),
	...
	require.Equal(t, "DummyApp.app", statusResp.Results.Software[0].Name)
	require.Equal(t, mdmlab.SetupExperienceStatusRunning, statusResp.Results.Software[0].Status)
	require.NotNil(t, statusResp.Results.Software[0].SoftwareTitleID)
	require.NotZero(t, *statusResp.Results.Software[0].SoftwareTitleID)
	...
	require.Len(t, statusResp.Results.Software, 1)
	require.Equal(t, "DummyApp.app", statusResp.Results.Software[0].Name)
	require.Equal(t, mdmlab.SetupExperienceStatusSuccess, statusResp.Results.Software[0].Status)
	...
	require.NotNil(t, statusResp.Results.Script)
	require.Equal(t, "script.sh", statusResp.Results.Script.Name)
	require.Equal(t, mdmlab.SetupExperienceStatusRunning, statusResp.Results.Script.Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", enrolledHost.ID), nil, http.StatusOK, &getHostResp)
	...
	expectedActivityDetail = fmt.Sprintf(`
{
  ...
  "policy_name": null
	...
	s.lastActivityMatches(mdmlab.ActivityTypeInstalledSoftware{}.ActivityName(), expectedActivityDetail, 0)
	...
	expectedActivityDetail = fmt.Sprintf(`
{
	...
	require.NotNil(t, statusResp.Results.Script)
	require.Equal(t, "script.sh", statusResp.Results.Script.Name)
	require.Equal(t, mdmlab.SetupExperienceStatusSuccess, statusResp.Results.Script.Status)
	...
	expectedActivityDetail = fmt.Sprintf(`
{
	...
	s.lastActivityMatches(mdmlab.ActivityTypeRanScript{}.ActivityName(), expectedActivityDetail, 0)
}
...
func (s *integrationMDMTestSuite) TestSetupExperienceFlowWithSoftwareAndScriptForceRelease() {
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/setup_experience/status", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q, "force_release": true}`, *enrolledHost.OrbitNodeKey)), http.StatusOK, &statusResp)
...
func (s *integrationMDMTestSuite) TestReenrollingADEDeviceAfterRemovingItFromABM() {
	...
	// 1 profile with mdmlabd configuration + 1 root CA config
	...
	// Simulate the device getting unassigned from MDMlab in ABM
	devices = []godep.Device{
	...
	a := checkHostDEPAssignProfileResponses([]string{mdmDevice.SerialNumber}, profileAssignmentReqs[0].ProfileUUID, mdmlab.DEPAssignProfileResponseSuccess)
	...
	devices = []godep.Device{
		// In https://github.com/mdmlabdm/mdmlab/issues/23200, we saw a profileUUID being sent back on
...
func (s *integrationMDMTestSuite) TestEnforceMiniumOSVersion() {
	...
	// the enrollment
	encodeDeviceInfo := func(machineInfo mdmlab.MDMAppleMachineInfo) string {
	...
	checkMDMEnrollEndpoint := func(machineInfo *mdmlab.MDMAppleMachineInfo, expectEnrollInfo *mdmtest.AppleEnrollInfo, expectSoftwareUpdate *mdmlab.MDMAppleSoftwareUpdateRequiredDetails) error {
		...
		cc := mdmlabhttp.NewClient(mdmlabhttp.WithTLSClientConfig(&tls.Config{
		...
		case expectSoftwareUpdate != nil:
			...
			var sur mdmlab.MDMAppleSoftwareUpdateRequired
			require.NoError(t, json.Unmarshal(body, &sur))
			require.NotNil(t, sur)
			require.Equal(t, mdmlab.MDMAppleSoftwareUpdateRequiredCode, sur.Code)
	...
	// case) and checks for the expected response
	checkMDMSSOEndpoint := func(machineInfo *mdmlab.MDMAppleMachineInfo, expectSoftwareUpdate *mdmlab.MDMAppleSoftwareUpdateRequiredDetails) error {
		...
		case response.StatusCode == http.StatusOK:
			...
			// https://github.com/mdmlabdm/mdmlab/blob/e62956924bbe041a1e75be2b0b7c6d1dd235a09d/server/service/testing_utils.go#L420-L421
			return nil
	...
	setMinOSVersion := func(minVersion string, deadline string, teamID *uint) {
		...
		if teamID == nil {
			acResp := appConfigResponse{}
			s.DoJSON("PATCH", "/api/latest/mdmlab/config", raw, http.StatusOK, &acResp)
			assert.NotNil(t, acResp.MDM.MacOSUpdates)
		} else {
			tcResp := teamResponse{}
			s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", *teamID), raw, http.StatusOK, &tcResp)
		}
	}
	...
	testCases := []struct {
		...
		updateRequired *mdmlab.MDMAppleSoftwareUpdateRequiredDetails
		...
		t.Run("sso disabled", func(t *testing.T) {
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					var mi mdmlab.MDMAppleMachineInfo
