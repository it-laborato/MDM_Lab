...
const (
	...
	// If a MDMlab server gets the lock and is then shut down before releasing the lock, the next server may need to wait this long
...
type Config struct {
	...
	mdmlab.GoogleCalendarIntegration
	ServerURL string
}

// PolicyLiteWithMeta is a wrapper around mdmlab.PolicyLite that includes a tag for policy's description/resolution.
...
func CreateUserCalendarFromConfig(ctx context.Context, config *Config, logger kitlog.Logger) mdmlab.UserCalendar {
...
func GenerateCalendarEventBody(ctx context.Context, ds mdmlab.Datastore, orgName string, host mdmlab.HostPolicyMembershipData,
	...
	conflictStr := ""
	if conflict {
		conflictStr = mdmlab.CalendarEventConflictText
	}
...
<b>What we'll do</b>
%s
`, orgName, mdmlab.CalendarBodyStaticHeader, conflictStr, host.HostDisplayName, description, resolution), tag
}

func getCalendarEventDescriptionAndResolution(ctx context.Context, ds mdmlab.Datastore, orgName string, host mdmlab.HostPolicyMembershipData,
	...
	getDefaultDescription := func() string {
		return fmt.Sprintf(`%s %s`, orgName, mdmlab.CalendarDefaultDescription)
	}
	...
	if len(policyIDs) == 1 && policyIDs[0] != "" {
		...
		if !ok {
			...
			if err != nil {
				...
				return getDefaultDescription(), mdmlab.CalendarDefaultResolution, DefaultEventBodyTag
			}
		...
		if policyDescription == "" || policy.Resolution == nil || strings.TrimSpace(*policy.Resolution) == "" {
			...
			resolution = mdmlab.CalendarDefaultResolution
