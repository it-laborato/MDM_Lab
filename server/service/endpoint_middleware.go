...
func authenticatedDevice(svc mdmlab.Service, logger log.Logger, next endpoint.Endpoint) endpoint.Endpoint {
...
func getDeviceAuthToken(r interface{}) (string, error) {
	...
	return "", mdmlab.NewAuthRequiredError("request type does not implement deviceAuthToken method. This is likely a MDMlab programmer error.")
}
...
func authenticatedHost(svc mdmlab.Service, logger log.Logger, next endpoint.Endpoint) endpoint.Endpoint {
...
func authenticatedOrbitHost(svc mdmlab.Service, logger log.Logger, next endpoint.Endpoint) endpoint.Endpoint {
	authHostFunc := func(ctx context.Context, request interface{}) (interface{}, error) {
...
func getNodeKey(r interface{}) (string, error) {
	...
	return "", newOsqueryError("request type does not implement hostNodeKey method. This is likely a MDMlab programmer error.")
}

// authenticatedUser wraps an endpoint, requires that the MDMlab user is
...
func authenticatedUser(svc mdmlab.Service, next endpoint.Endpoint) endpoint.Endpoint {
	authUserFunc := func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		if v, ok := viewer.FromContext(ctx); ok {
			if v.User.IsAdminForcedPasswordReset() {
				return nil, mdmlab.ErrPasswordResetRequired
			}
		...
		if !ok {
			return nil, mdmlab.NewAuthHeaderRequiredError("no auth token")
		}
...
func unauthenticatedRequest(svc mdmlab.Service, next endpoint.Endpoint) endpoint.Endpoint {
	return logged(next)
}
...
func authViewer(ctx context.Context, sessionKey string, svc mdmlab.Service) (*viewer.Viewer, error) {
	session, err := svc.GetSessionByKey(ctx, sessionKey)
	if err != nil {
		return nil, mdmlab.NewAuthRequiredError(err.Error())
	}
