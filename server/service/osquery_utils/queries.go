...
type DetailQuery struct {
	...
	// Host Vitals documentation https://mdmlabdm.com/docs/using-mdmlab/understanding-host-vitals
	Description string
	// Query is the SQL query string.
	Query string
	// QueryFunc is optionally used to dynamically build a query.
	QueryFunc func(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore) string
	// Discovery is the SQL query that defines whether the query will run on the host or not.
	// If not set, MDMlab makes sure the query will always run.
	Discovery string
	...
	IngestFunc func(ctx context.Context, logger log.Logger, host *mdmlab.Host, rows []map[string]string) error
	...
	DirectIngestFunc func(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error
	...
	DirectTaskIngestFunc func(ctx context.Context, logger log.Logger, host *mdmlab.Host, task *async.Task, rows []map[string]string) error
}
...
// mdmlab.Host data model (via IngestFunc).
//
...
var hostDetailQueries = map[string]DetailQuery{
	...
	"os_version_windows": {
		// MDMlab requires the DisplayVersion as well as the UBR (4th part of the version number) to
	...
	"osquery_flags": {
		...
		Platforms: append(mdmlab.HostLinuxOSs, "darwin", "windows"), // not chrome
	},
	"osquery_info": {
	...
	"system_info": {
		...
		IngestFunc: func(ctx context.Context, logger log.Logger, host *mdmlab.Host, rows []map[string]string) error {
			...
			if rows[0]["hardware_serial"] != "-1" { // ignoring the default -1 serial. See: https://github.com/mdmlabdm/mdmlab/issues/19789
...
func ingestNetworkInterface(ctx context.Context, logger log.Logger, host *mdmlab.Host, rows []map[string]string) error {
...
func directIngestDiskSpace(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) != 1 {
...
func ingestKubequeryInfo(ctx context.Context, logger log.Logger, host *mdmlab.Host, rows []map[string]string) error {
	if len(rows) != 1 {
...
var extraDetailQueries = map[string]DetailQuery{
	...
	"battery": {
		...
		// (https://github.com/mdmlabdm/mdmlab/issues/6763) and its absence on Windows.
	...
	"orbit_info": {
		...
		Platforms:        append(mdmlab.HostLinuxOSs, "darwin", "windows"),
...
// mdmQueries are used by the MDMlab server to compliment certain MDM
// features.
// They are only sent to the device when MDMlab's MDM is on and properly
// configured
...
var SoftwareOverrideQueries = map[string]DetailQuery{
	...
	"macos_firefox": {
		Description: "A software override query[^1] to differentiate between Firefox and Firefox ESR on macOS. Requires `mdmlabd`",
		Query: `
			WITH app_paths AS (
				SELECT path
				FROM apps
	...
	//   - codesign is a mdmlabd table (not part of osquery core).
	...
	`,
		Description: "A software override query[^1] to append codesign information to macOS software entries. Requires `mdmlabd`",
		Platforms:   []string{"darwin"},
...
func directIngestOrbitInfo(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) != 1 {
...
func directIngestOSWindows(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) != 1 {
		return ctxerr.Errorf(ctx, "directIngestOSWindows invalid number of rows: %d", len(rows))
	}

	hostOS := mdmlab.OperatingSystem{
		Name:          rows[0]["name"],
		Arch:          rows[0]["arch"],
...
func directIngestOSUnixLike(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) != 1 {
	...
	hostOS := mdmlab.OperatingSystem{Name: name, Arch: arch, KernelVersion: kernelVersion, Platform: platform}
...
func directIngestChromeProfiles(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	mapping := make([]*mdmlab.HostDeviceMapping, 0, len(rows))
	for _, row := range rows {
		mapping = append(mapping, &mdmlab.HostDeviceMapping{
			HostID: host.ID,
			Email:  row["email"],
			Source: mdmlab.DeviceMappingGoogleChromeProfiles,
		})
	}
	return ds.ReplaceHostDeviceMapping(ctx, host.ID, mapping, mdmlab.DeviceMappingGoogleChromeProfiles)
}
...
// Due to a known osquery issue with M1 Macs (https://github.com/mdmlabdm/mdmlab/issues/6763)
...
// macOS and Windows.
func directIngestBattery(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	mapping := make([]*mdmlab.HostBattery, 0, len(rows))
	...
	for _, row := range rows {
		...
		mapping = append(mapping, &mdmlab.HostBattery{
			HostID:       host.ID,
...
func directIngestWindowsUpdateHistory(
	...
	uniq := make(map[uint]mdmlab.WindowsUpdate)
	for _, row := range rows {
		u, err := mdmlab.NewWindowsUpdate(row["title"], row["date"])
		if err != nil {
	...
	var updates []mdmlab.WindowsUpdate
	for _, v := range uniq {
...
func directIngestScheduledQueryStats(ctx context.Context, logger log.Logger, host *mdmlab.Host, task *async.Task, rows []map[string]string) error {
	packs := map[string][]mdmlab.ScheduledQueryStats{}
	for _, row := range rows {
...
func directIngestSoftware(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	var software []mdmlab.Software
	sPaths := map[string]struct{}{}

	for _, row := range rows {
		// Attempt to parse the last_opened_at and emit a debug log if it fails.
		if _, err := mdmlab.ParseSoftwareLastOpenedAtRowValue(row["last_opened_at"]); err != nil {
			level.Debug(logger).Log(
		...
		s, err := mdmlab.SoftwareFromOsqueryRow(
			row["name"],
			row["version"],
			row["source"],
			row["vendor"],
		...
		if installedPath != "" &&
			...
			teamIdentifier := truncateString(row["team_identifier"], mdmlab.SoftwareTeamIdentifierMaxLength)
			key := fmt.Sprintf(
				"%s%s%s%s%s",
				installedPath, mdmlab.SoftwareFieldSeparator, teamIdentifier, mdmlab.SoftwareFieldSeparator, s.ToUniqueStr(),
			)
			sPaths[key] = struct{}{}
		}
	}
...
var (
	...
	softwareSanitizers  = []struct {
		checkSoftware  func(*mdmlab.Host, *mdmlab.Software) bool
		mutateSoftware func(*mdmlab.Software, log.Logger)
	}{
		...
		{
			checkSoftware: func(h *mdmlab.Host, s *mdmlab.Software) bool {
				return h.Platform == "darwin" && (s.Name == "Microsoft Teams.app" || s.Name == "Microsoft Teams classic.app")
			},
			mutateSoftware: func(s *mdmlab.Software, logger log.Logger) {
...
func sanitizeSoftware(h *mdmlab.Host, s *mdmlab.Software, logger log.Logger) {
...
func shouldRemoveSoftware(h *mdmlab.Host, s *mdmlab.Software) bool {
...
func directIngestUsers(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
...
func directIngestMDMMac(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
	...
	if !enrolled && installedFromDep && mdmSolutionName != mdmlab.WellKnownMDMMDMlab && host.RefetchCriticalQueriesUntil != nil {
		// the host was unenrolled from a non-MDMlab DEP MDM solution, and the
	...
	// if the MDM solution is MDMlab, we need to extract the enrollment reference from the URL and
	// upsert host emails based on the MDM IdP account associated with the enrollment reference
	var mdmlabEnrollRef string
	if mdmSolutionName == mdmlab.WellKnownMDMMDMlab {
		mdmlabEnrollRef = serverURL.Query().Get(mobileconfig.MDMlabEnrollReferenceKey)
		if mdmlabEnrollRef == "" {
			...
			mdmlabEnrollRef = serverURL.Query().Get("enrollment_reference")
		}
		if mdmlabEnrollRef != "" {
			if err := ds.SetOrUpdateHostEmailsFromMdmIdpAccounts(ctx, host.ID, mdmlabEnrollRef); err != nil {
...
func deduceMDMNameMacOS(row map[string]string) string {
	// If the PayloadIdentifier is MDMlab's MDM then use MDMlab as name of the MDM solution.
	// (For MDMlab MDM we cannot use the URL because MDMlab can be deployed On-Prem.)
	if payloadIdentifier := row["payload_identifier"]; payloadIdentifier == apple_mdm.MDMlabPayloadIdentifier {
		return mdmlab.WellKnownMDMMDMlab
	}
	return mdmlab.MDMNameFromServerURL(row["server_url"])
}
...
func deduceMDMNameWindows(data map[string]string) string {
	...
	if name := data["provider_id"]; name == mdmlab.WellKnownMDMMDMlab {
		return name
	}

	return mdmlab.MDMNameFromServerURL(serverURL)
}

func directIngestMDMWindows(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
	...
	if !enrolled && mdmSolutionName != mdmlab.WellKnownMDMMDMlab && host.RefetchCriticalQueriesUntil != nil {
		// the host was unenrolled from a non-MDMlab MDM solution, and the refetch
...
func directIngestMunkiInfo(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
...
func directIngestDiskEncryptionLinux(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	encrypted := false
	for _, row := range rows {
...
func directIngestDiskEncryption(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
...
func directIngestMacOSProfiles(
	...
	installed := make(map[string]*mdmlab.HostMacOSProfile, len(rows))
	for _, row := range rows {
		...
		installed[row["identifier"]] = &mdmlab.HostMacOSProfile{
...
func directIngestMDMDeviceIDWindows(ctx context.Context, logger log.Logger, host *mdmlab.Host, ds mdmlab.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
...
func GetDetailQueries(
	...
	if !mdmlabConfig.Vulnerabilities.DisableWinOSVulnerabilities {
		generatedMap["windows_update_history"] = windowsUpdateHistory
	}

	if mdmlabConfig.App.EnableScheduledQueryStats {
...
func buildConfigProfilesWindowsQuery(
	...
	gotProfiles := false
	err := microsoft_mdm.LoopOverExpectedHostProfiles(ctx, ds, host, func(profile *mdmlab.ExpectedMDMProfile, hash, locURI, data string) {
