...
func TestDetailQueryNetworkInterfaces(t *testing.T) {
	var initialHost mdmlab.Host
	host := initialHost

	ingest := GetDetailQueries(context.Background(), config.MDMlabConfig{}, nil, nil)["network_interface_unix"].IngestFunc
...
func TestDetailQueryScheduledQueryStats(t *testing.T) {
	...
	var gotPackStats []mdmlab.PackStats
	ds.SaveHostPackStatsFunc = func(ctx context.Context, teamID *uint, hostID uint, stats []mdmlab.PackStats) error {
		if hostID != host.ID {
	...
	ingest := GetDetailQueries(context.Background(), config.MDMlabConfig{App: config.AppConfig{EnableScheduledQueryStats: true}}, nil, nil)["scheduled_query_stats"].DirectTaskIngestFunc
...
func TestGetDetailQueries(t *testing.T) {
	queriesNoConfig := GetDetailQueries(context.Background(), config.MDMlabConfig{}, nil, nil)
	...
	queriesWithoutWinOSVuln := GetDetailQueries(context.Background(), config.MDMlabConfig{Vulnerabilities: config.VulnerabilitiesConfig{DisableWinOSVulnerabilities: true}}, nil, nil)
	require.Len(t, queriesWithoutWinOSVuln, 25)

	queriesWithUsers := GetDetailQueries(context.Background(), config.MDMlabConfig{App: config.AppConfig{EnableScheduledQueryStats: true}}, nil, &mdmlab.Features{EnableHostUsers: true})
	qs := baseQueries
	...
	queriesWithUsersAndSoftware := GetDetailQueries(context.Background(), config.MDMlabConfig{App: config.AppConfig{EnableScheduledQueryStats: true}}, nil, &mdmlab.Features{EnableHostUsers: true, EnableSoftwareInventory: true})
	qs = baseQueries
	...
	gotQueries := GetDetailQueries(context.Background(), config.MDMlabConfig{}, &ac, nil)
...
func TestDetailQueriesOSVersionUnixLike(t *testing.T) {
	...
	ingest := GetDetailQueries(context.Background(), config.MDMlabConfig{}, nil, nil)["os_version"].IngestFunc
...
func TestDetailQueriesOSVersionWindows(t *testing.T) {
	...
	ingest := GetDetailQueries(context.Background(), config.MDMlabConfig{}, nil, nil)["os_version_windows"].IngestFunc
...
func TestDirectIngestMDMMac(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			[]any{false, false, "", false, mdmlab.UnknownMDMName},
			"",
			"",
		},
		{
			"with MDMlab payload identifier",
			map[string]string{
				"enrolled":           "true",
			...
			[]any{false, true, "https://test.example.com", true, mdmlab.WellKnownMDMMDMlab},
			"",
			"",
		},
		{
			...
			[]any{false, true, "https://jamf.com/1/some/path", true, mdmlab.WellKnownMDMJamf},
			"",
			"",
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			ds.SetOrUpdateMDMDataFunc = func(ctx context.Context, hostID uint, isServer, enrolled bool, serverURL string, installedFromDep bool, name string, mdmlabEnrollmentRef string) error {
				...
				require.Equal(t, mdmlabEnrollmentRef, c.enrollRef)
				return nil
			}
			ds.SetOrUpdateHostEmailsFromMdmIdpAccountsFunc = func(ctx context.Context, hostID uint, mdmlabEnrollmentRef string) error {
				return nil
			}
...
func TestDirectIngestMDMMDMlabEnrollRef(t *testing.T) {
	ds := new(mock.Store)
	var host mdmlab.Host
	...
	for _, tc := range []testCase{
		{
			name:                 "MDMlab enroll_reference",
			mdmData:              generateRows("https://test.example.com?enroll_reference=test-reference", apple_mdm.MDMlabPayloadIdentifier),
			wantServerURL:        "https://test.example.com",
			wantEnrollRef:        "test-reference",
			wantHostEmailsCalled: true,
		},
		{
			name:                 "MDMlab no enroll_reference",
			mdmData:              generateRows("https://test.example.com", apple_mdm.MDMlabPayloadIdentifier),
			wantServerURL:        "https://test.example.com",
			wantEnrollRef:        "",
			wantHostEmailsCalled: false,
		},
		{
			name:                 "MDMlab enrollment_reference",
			mdmData:              generateRows("https://test.example.com?enrollment_reference=test-reference", apple_mdm.MDMlabPayloadIdentifier),
			wantServerURL:        "https://test.example.com",
			wantEnrollRef:        "test-reference",
			wantHostEmailsCalled: true,
		},
		{
			name:                 "MDMlab enroll_reference with other query params",
			mdmData:              generateRows("https://test.example.com?token=abcdefg&enroll_reference=test-reference", apple_mdm.MDMlabPayloadIdentifier),
			wantServerURL:        "https://test.example.com",
			wantEnrollRef:        "test-reference",
			wantHostEmailsCalled: true,
		},
		{
			name:                 "non-MDMlab enroll_reference",
		...
		t.Run(tc.name, func(t *testing.T) {
			ds.SetOrUpdateHostEmailsFromMdmIdpAccountsFunc = func(ctx context.Context, hostID uint, mdmlabEnrollmentRef string) error {
				require.Equal(t, tc.wantEnrollRef, mdmlabEnrollmentRef)
				return nil
			}
			ds.SetOrUpdateMDMDataFunc = func(ctx context.Context, hostID uint, isServer, enrolled bool, serverURL string, installedFromDep bool, name string, mdmlabEnrollmentRef string) error {
				...
				if tc.mdmData[0]["payload_identifier"] == apple_mdm.MDMlabPayloadIdentifier {
					require.Equal(t, name, mdmlab.WellKnownMDMMDMlab)
				} else {
					require.Equal(t, name, mdmlab.UnknownMDMName)
				}
...
func TestDirectIngestMDMWindows(t *testing.T) {
	...
	cases := []struct {
...

		{
			...
			wantMDMSolName:       mdmlab.WellKnownMDMJumpCloud,
		},
		{
			...
			wantMDMSolName:       mdmlab.WellKnownMDMVMWare,
		},
		{
		...
		},
		{
			...
			wantMDMSolName:       mdmlab.WellKnownMDMIntune,
		},
		{
			name: "on manual mdmlabdm cloud hosted",
			data: []map[string]string{
				{
					"discovery_service_url": "https://mdmlabdm.com",
					"aad_resource_id":       "",
					"provider_id":           "Local_Management",
					"installation_type":     "Client",
				},
			...
			wantInstalledFromDep: false,
			wantIsServer:         false,
			wantServerURL:        "https://mdmlabdm.com",
			wantMDMSolName:       mdmlab.WellKnownMDMMDMlab,
		},

		{
			name: "on manual mdmlabdm self hosted",
			data: []map[string]string{
				{
	...
	for _, c := range cases {
		...
		err := directIngestMDMWindows(context.Background(), log.NewNopLogger(), &mdmlab.Host{}, ds, c.data)
		require.NoError(t, err)
		require.True(t, ds.SetOrUpdateMDMDataFuncInvoked)
		ds.SetOrUpdateMDMDataFuncInvoked = false
		require.False(t, ds.SetOrUpdateHostEmailsFromMdmIdpAccountsFuncInvoked)
...
func TestDirectIngestChromeProfiles(t *testing.T) {
	ds := new(mock.Store)
	ds.ReplaceHostDeviceMappingFunc = func(ctx context.Context, hostID uint, mapping []*mdmlab.HostDeviceMapping, source string) error {
		require.Equal(t, hostID, uint(1))
		require.Equal(t, mapping, []*mdmlab.HostDeviceMapping{
...
func TestDirectIngestBattery(t *testing.T) {
	tests := []struct {
		...
		expectedBattery *mdmlab.HostBattery
	}{
		{
			...
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "a", CycleCount: 2, Health: batteryStatusGood},
		},
		{
			name:            "max_capacity < 50%",
			input:           map[string]string{"serial_number": "b", "cycle_count": "3", "designed_capacity": "3000", "max_capacity": "2399"},
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "b", CycleCount: 3, Health: batteryStatusDegraded},
		},
		{
			name:            "missing max_capacity",
			input:           map[string]string{"serial_number": "c", "cycle_count": "4", "designed_capacity": "3000", "max_capacity": ""},
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "c", CycleCount: 4, Health: batteryStatusUnknown},
		},
		{
			...
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "d", CycleCount: 5, Health: batteryStatusUnknown},
		},
		{
			name:            "missing designed_capacity",
			input:           map[string]string{"serial_number": "e", "cycle_count": "6", "designed_capacity": "", "max_capacity": "2000"},
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "e", CycleCount: 6, Health: batteryStatusUnknown},
		},
		{
			...
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "f", CycleCount: 7, Health: batteryStatusUnknown},
		},
		{
			name:            "cycleCount >= 1000",
			input:           map[string]string{"serial_number": "g", "cycle_count": "1000", "designed_capacity": "3000", "max_capacity": "2400"},
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "g", CycleCount: 1000, Health: batteryStatusDegraded},
		},
		{
			...
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "h", CycleCount: 1001, Health: batteryStatusDegraded},
		},
		{
			name:            "missing cycle_count",
			input:           map[string]string{"serial_number": "i", "cycle_count": "", "designed_capacity": "3000", "max_capacity": "2400"},
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "i", CycleCount: 0, Health: batteryStatusGood},
		},
		{
			...
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "j", CycleCount: 0, Health: batteryStatusDegraded},
		},
		{
			name:            "invalid cycle_count",
			input:           map[string]string{"serial_number": "k", "cycle_count": "foo", "designed_capacity": "3000", "max_capacity": "2400"},
			expectedBattery: &mdmlab.HostBattery{HostID: uint(1), SerialNumber: "k", CycleCount: 0, Health: batteryStatusGood},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ds.ReplaceHostBatteriesFunc = func(ctx context.Context, id uint, mappings []*mdmlab.HostBattery) error {
...
func TestDirectIngestOSWindows(t *testing.T) {
	...
	testCases := []struct {
		expected mdmlab.OperatingSystem
		data     []map[string]string
	}{
		{
			expected: mdmlab.OperatingSystem{
...
func TestDirectIngestOSUnixLike(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		t.Run(tc.expected.Name, func(t *testing.T) {
			...
			err := directIngestOSUnixLike(context.Background(), log.NewNopLogger(), &mdmlab.Host{ID: uint(i)}, //nolint:gosec // dismiss G115
				ds, tc.data)
...
func TestAppConfigReplaceQuery(t *testing.T) {
	queries := GetDetailQueries(context.Background(), config.MDMlabConfig{}, nil, &mdmlab.Features{EnableHostUsers: true})
	originalQuery := queries["users"].Query

	replacementMap := make(map[string]*string)
	replacementMap["users"] = ptr.String("select 1 from blah")
	queries = GetDetailQueries(context.Background(), config.MDMlabConfig{}, nil, &mdmlab.Features{EnableHostUsers: true, DetailQueryOverrides: replacementMap})
...
func TestDirectIngestSoftware(t *testing.T) {
	...
	t.Run("ingesting installed software path", func(t *testing.T) {
		...
		t.Run("errors are reported back", func(t *testing.T) {
			ds.UpdateHostSoftwareInstalledPathsFunc = func(ctx context.Context, hostID uint, sPaths map[string]struct{}, result *mdmlab.UpdateHostSoftwareDBResult) error {
		...
		t.Run("only entries with installed_path set are persisted", func(t *testing.T) {
			...
			require.Contains(t, strings.Join(maps.Keys(calledWith), " "), fmt.Sprintf("%s%s%s%s%s", data[1]["installed_path"], mdmlab.SoftwareFieldSeparator, "", mdmlab.SoftwareFieldSeparator, data[1]["name"]))
...
func TestDirectIngestWindowsUpdateHistory(t *testing.T) {
	ds := new(mock.Store)
	ds.InsertWindowsUpdatesFunc = func(ctx context.Context, hostID uint, updates []mdmlab.WindowsUpdate) error {
		require.Len(t, updates, 6)
		require.ElementsMatch(t, []mdmlab.WindowsUpdate{
...
func TestIngestKubequeryInfo(t *testing.T) {
	err := ingestKubequeryInfo(context.Background(), log.NewNopLogger(), &mdmlab.Host{}, nil)
	require.Error(t, err)
	err = ingestKubequeryInfo(context.Background(), log.NewNopLogger(), &mdmlab.Host{}, []map[string]string{})
	require.Error(t, err)
	err = ingestKubequeryInfo(context.Background(), log.NewNopLogger(), &mdmlab.Host{}, []map[string]string{
		{
			"cluster_name": "foo",
...
func TestDirectIngestDiskEncryptionKeyDarwin(t *testing.T) {
	...
	ds.SetOrUpdateHostDiskEncryptionKeyFunc = func(ctx context.Context, incomingHost *mdmlab.Host, encryptedBase64Key, clientError string,
...
func TestDirectIngestHostMacOSProfiles(t *testing.T) {
	...
	toRows := func(profs []*mdmlab.HostMacOSProfile) []map[string]string {
	...
	var installedProfiles []*mdmlab.HostMacOSProfile
	ds.GetHostMDMProfilesExpectedForVerificationFunc = func(ctx context.Context, host *mdmlab.Host) (map[string]*mdmlab.ExpectedMDMProfile, error) {
		...
		expected := make(map[string]*mdmlab.ExpectedMDMProfile, len(installedProfiles))
		for _, p := range installedProfiles {
			expected[p.Identifier] = &mdmlab.ExpectedMDMProfile{
				Identifier:          p.Identifier,
	...
	ds.UpdateHostMDMProfilesVerificationFunc = func(ctx context.Context, host *mdmlab.Host, toVerify, toFailed, toRetry []string) error {
	...
	installedProfiles = []*mdmlab.HostMacOSProfile{
		{
	...
	installedProfiles = append(installedProfiles, &mdmlab.HostMacOSProfile{
		Identifier:  "",
		DisplayName: "",
		InstallDate: time.Now().Truncate(time.Second),
	})
...
func TestDirectIngestMDMDeviceIDWindows(t *testing.T) {
	...
	ctx := context.Background()
	logger := log.NewNopLogger()
	host := &mdmlab.Host{ID: 1, UUID: "mdm-windows-hw-uuid"}
...
func TestDirectIngestWindowsProfiles(t *testing.T) {
	...
	for _, tc := range []struct {
		hostProfiles []*mdmlab.ExpectedMDMProfile
		want         string
	}{
		{nil, ""},
		{
			[]*mdmlab.ExpectedMDMProfile{
		...
		ds.GetHostMDMProfilesExpectedForVerificationFunc = func(ctx context.Context, host *mdmlab.Host) (map[string]*mdmlab.ExpectedMDMProfile, error) {
			result := map[string]*mdmlab.ExpectedMDMProfile{}
		...
		gotQuery := buildConfigProfilesWindowsQuery(ctx, logger, &mdmlab.Host{}, ds)
		if tc.want != "" {
			...
			var extractedStruct, expectedStruct mdmlab.SyncBody
...
func TestShouldRemoveSoftware(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: true,
			h:    &mdmlab.Host{Platform: "darwin"},
			s:    &mdmlab.Software{BundleIdentifier: "com.parallels.winapp.notepad", Name: "Notepad.app"},
		},
		{
			name: "regular macos software",
			want: false,
			h:    &mdmlab.Host{Platform: "darwin"},
			s:    &mdmlab.Software{BundleIdentifier: "com.apple.dock", Name: "Dock.app"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestIngestNetworkInterface(t *testing.T) {
	...
	// see https://github.com/mdmlabdm/mdmlab/issues/11102.
	for _, tc := range []struct {
		name  string
		ip    string
		valid bool
		...
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			h := mdmlab.Host{PublicIP: "190.18.97.3"} // set to some old value that should always be overridden
	...
	t.Run("primaryIP and primaryMAC", func(t *testing.T) {
		...
		// No rows
		b.Reset()
		h = mdmlab.Host{PublicIP: "190.18.97.3"}
