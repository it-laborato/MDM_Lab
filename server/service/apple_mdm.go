...
import (
	...
	"github.com/it-laborato/MDM_Lab/server/mdm/apple/gdmf"
	...
	nano_service "github.com/it-laborato/MDM_Lab/server/mdm/nanomdm/service"
...
const (
	// MDMlabVarNDESSCEPChallenge and other variables are used as $FLEET_VAR_<VARIABLE_NAME>.
	...
	MDMlabVarNDESSCEPChallenge   = "NDES_SCEP_CHALLENGE"
	MDMlabVarNDESSCEPProxyURL    = "NDES_SCEP_PROXY_URL"
	MDMlabVarHostEndUserEmailIDP = "HOST_END_USER_EMAIL_IDP"
)

var (
	profileVariableRegex            = regexp.MustCompile(`(\$FLEET_VAR_(?P<name1>\w+))|(\${FLEET_VAR_(?P<name2>\w+)})`)
	mdmlabVarNDESSCEPChallengeRegexp = regexp.MustCompile(fmt.Sprintf(`(\$FLEET_VAR_%s)|(\${FLEET_VAR_%s})`, MDMlabVarNDESSCEPChallenge,
		MDMlabVarNDESSCEPChallenge))
	mdmlabVarNDESSCEPProxyURLRegexp = regexp.MustCompile(fmt.Sprintf(`(\$FLEET_VAR_%s)|(\${FLEET_VAR_%s})`, MDMlabVarNDESSCEPProxyURL,
		MDMlabVarNDESSCEPProxyURL))
	mdmlabVarHostEndUserEmailIDPRegexp = regexp.MustCompile(fmt.Sprintf(`(\$FLEET_VAR_%s)|(\${FLEET_VAR_%s})`, MDMlabVarHostEndUserEmailIDP,
		MDMlabVarHostEndUserEmailIDP))
	mdmlabVarsSupportedInConfigProfiles = []string{MDMlabVarNDESSCEPChallenge, MDMlabVarNDESSCEPProxyURL, MDMlabVarHostEndUserEmailIDP}
)
...
func getMDMAppleCommandResultsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleCommandResults(ctx context.Context, commandUUID string) ([]*mdmlab.MDMCommandResult, error) {
...
type listMDMAppleCommandsResponse struct {
	Results []*mdmlab.MDMAppleCommand `json:"results"`
	Err     error                    `json:"error,omitempty"`
}

func (r listMDMAppleCommandsResponse) error() error { return r.Err }

func listMDMAppleCommandsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*listMDMAppleCommandsRequest)
	results, err := svc.ListMDMAppleCommands(ctx, &mdmlab.MDMCommandListOptions{
...
func (svc *Service) ListMDMAppleCommands(ctx context.Context, opts *mdmlab.MDMCommandListOptions) ([]*mdmlab.MDMAppleCommand, error) {
	// first, authorize that the user has the right to list hosts
	if err := svc.authz.Authorize(ctx, &mdmlab.Host{}, mdmlab.ActionList); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
	...
	// MDM Apple commands.
	results, err := svc.ds.ListMDMAppleCommands(ctx, mdmlab.TeamFilter{
		User:            vc.User,
	...
	if authzErr != nil {
		...
		allowedResults := make([]*mdmlab.MDMAppleCommand, 0, len(results))
		for _, res := range results {
			var id uint
			if res.TeamID != nil {
				id = *res.TeamID
...
func newMDMAppleConfigProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	cp, err := svc.NewMDMAppleConfigProfile(ctx, req.TeamID, ff, nil, mdmlab.LabelsIncludeAll)
	if err != nil {
		return &newMDMAppleConfigProfileResponse{Err: err}, nil
	}
...
func (svc *Service) NewMDMAppleConfigProfile(ctx context.Context, teamID uint, r io.Reader, labels []string, labelsMembershipMode mdmlab.MDMLabelsMode) (*mdmlab.MDMAppleConfigProfile, error) {
	...
	if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
		err := mdmlab.NewInvalidArgumentError("profile", mdmlab.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
	...
	cp, err := mdmlab.NewMDMAppleConfigProfile([]byte(expanded), &teamID)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, &mdmlab.BadRequestError{
	...
	if err := cp.ValidateUserProvided(); err != nil {
		return nil, ctxerr.Wrap(ctx, &mdmlab.BadRequestError{Message: err.Error()})
	}
	err = validateConfigProfileMDMlabVariables(string(cp.Mobileconfig))
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "validating mdmlab variables")
	}
	...
	if err != nil {
		...
		if errors.As(err, &existsErr) {
			...
			err = mdmlab.NewInvalidArgumentError("profile", msg).
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeCreatedMacosProfile{
			TeamID:            actTeamID,
...
func validateConfigProfileMDMlabVariables(contents string) error {
	mdmlabVars := findMDMlabVariables(contents)
	for k := range mdmlabVars {
		if !slices.Contains(mdmlabVarsSupportedInConfigProfiles, k) {
			return &mdmlab.BadRequestError{Message: fmt.Sprintf("MDMlab variable $FLEET_VAR_%s is not supported in configuration profiles",
				k)}
		}
	}
	return nil
...
func (svc *Service) NewMDMAppleDeclaration(ctx context.Context, teamID uint, r io.Reader, labels []string, name string, labelsMembershipMode mdmlab.MDMLabelsMode) (*mdmlab.MDMAppleDeclaration, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMConfigProfileAuthz{TeamID: &teamID}, mdmlab.ActionWrite); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
	...
	// only that any MDM is enabled, maybe it's just Windows
	if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
		err := mdmlab.NewInvalidArgumentError("declaration", mdmlab.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
		return nil, ctxerr.Wrap(ctx, err, "check macOS MDM enabled")
	}

	mdmlabNames := mdm_types.MDMlabReservedProfileNames()
	if _, ok := mdmlabNames[name]; ok {
		err := mdmlab.NewInvalidArgumentError("declaration", fmt.Sprintf("Profile name %q is not allowed.", name)).WithStatus(http.StatusBadRequest)
		return nil, err
	}
	...
	if err != nil {
		return nil, mdmlab.NewInvalidArgumentError("profile", err.Error())
	}

	if err := validateDeclarationMDMlabVariables(dataWithSecrets); err != nil {
		return nil, err
	}

	// TODO(roberto): Maybe GetRawDeclarationValues belongs inside NewMDMAppleDeclaration? We can refactor this in a follow up.
	rawDecl, err := mdmlab.GetRawDeclarationValues([]byte(dataWithSecrets))
	if err != nil {
		return nil, err
	}
	...
	d := mdmlab.NewMDMAppleDeclaration(data, tmID, name, rawDecl.Type, rawDecl.Identifier)
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeCreatedDeclarationProfile{
			TeamID:      actTeamID,
			TeamName:    actTeamName,
...
func validateDeclarationMDMlabVariables(contents string) error {
	if len(findMDMlabVariables(contents)) > 0 {
		return &mdmlab.BadRequestError{Message: "MDMlab variables ($FLEET_VAR_*) are not currently supported in DDM profiles"}
	}
	return nil
}

func (svc *Service) batchValidateDeclarationLabels(ctx context.Context, labelNames []string) (map[string]mdmlab.ConfigurationProfileLabel, error) {
	if len(labelNames) == 0 {
		return nil, nil
	}
...
func (svc *Service) validateDeclarationLabels(ctx context.Context, labelNames []string) ([]mdmlab.ConfigurationProfileLabel, error) {
	...
	var declLabels []mdmlab.ConfigurationProfileLabel
...
type listMDMAppleConfigProfilesResponse struct {
	ConfigProfiles []*mdmlab.MDMAppleConfigProfile `json:"profiles"`
	Err            error                          `json:"error,omitempty"`
}

func (r listMDMAppleConfigProfilesResponse) error() error { return r.Err }

func listMDMAppleConfigProfilesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	if cps == nil {
		res.ConfigProfiles = []*mdmlab.MDMAppleConfigProfile{} // return empty json array instead of json null
	}
	return &res, nil
}

func (svc *Service) ListMDMAppleConfigProfiles(ctx context.Context, teamID uint) ([]*mdmlab.MDMAppleConfigProfile, error) {
...
func getMDMAppleConfigProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleConfigProfileByDeprecatedID(ctx context.Context, profileID uint) (*mdmlab.MDMAppleConfigProfile, error) {
...
func (svc *Service) GetMDMAppleConfigProfile(ctx context.Context, profileUUID string) (*mdmlab.MDMAppleConfigProfile, error) {
	// first we perform a perform basic authz check
	if err := svc.authz.Authorize(ctx, &mdmlab.Team{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) GetMDMAppleDeclaration(ctx context.Context, profileUUID string) (*mdmlab.MDMAppleDeclaration, error) {
	// first we perform a perform basic authz check
	if err := svc.authz.Authorize(ctx, &mdmlab.Team{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func deleteMDMAppleConfigProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) DeleteMDMAppleConfigProfile(ctx context.Context, profileUUID string) error {
	...
	// only that any MDM is enabled, maybe it's just Windows
	if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
		err := mdmlab.NewInvalidArgumentError("profile_uuid", mdmlab.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
	...
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMConfigProfileAuthz{TeamID: cp.TeamID}, mdmlab.ActionWrite); err != nil {
		return ctxerr.Wrap(ctx, err)
	}

	// prevent deleting profiles that are managed by MDMlab
	if _, ok := mobileconfig.MDMlabPayloadIdentifiers()[cp.Identifier]; ok {
		return &mdmlab.BadRequestError{
			Message:     "profiles managed by MDMlab can't be deleted using this endpoint.",
			InternalErr: fmt.Errorf("deleting profile %s for team %s not allowed because it's managed by MDMlab", cp.Identifier, teamName),
		}
	}
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeDeletedMacosProfile{
			TeamID:            actTeamID,
			TeamName:          actTeamName,
			ProfileName:       cp.Name,
...
func (svc *Service) DeleteMDMAppleDeclaration(ctx context.Context, declUUID string) error {
	...
	// has been provided by the user and can be deleted. We don't need to validate that it is a MDMlab declaration.
	hasSecretVariable := len(mdmlab.ContainsPrefixVars(string(decl.RawJSON), mdmlab.ServerSecretPrefix)) > 0
	if !hasSecretVariable {
		if _, ok := mdm_types.MDMlabReservedProfileNames()[decl.Name]; ok {
			return &mdmlab.BadRequestError{
				Message:     "profiles managed by MDMlab can't be deleted using this endpoint.",
				InternalErr: fmt.Errorf("deleting profile %s is not allowed because it's managed by MDMlab", decl.Name),
			}
		}

		// TODO: refine our approach to deleting restricted/forbidden types of declarations so that we
		// can check that MDMlab-managed aren't being deleted; this can be addressed once we add support
		// for more types of declarations
		var d mdmlab.MDMAppleRawDeclaration
		...
		if err := d.ValidateUserProvided(); err != nil {
			return ctxerr.Wrap(ctx, &mdmlab.BadRequestError{Message: err.Error()})
		}
	}
	...
	// now we can do a specific authz check based on team id of profile before we delete the profile
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMConfigProfileAuthz{TeamID: decl.TeamID}, mdmlab.ActionWrite); err != nil {
		return ctxerr.Wrap(ctx, err)
	}
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeDeletedDeclarationProfile{
			TeamID:      actTeamID,
			TeamName:    actTeamName,
			ProfileName: decl.Name,
			Identifier:  decl.Identifier,
...
type getMDMAppleFileVaultSummaryResponse struct {
	*mdmlab.MDMAppleFileVaultSummary
	Err error `json:"error,omitempty"`
}

func (r getMDMAppleFileVaultSummaryResponse) error() error { return r.Err }

func getMdmAppleFileVaultSummaryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleFileVaultSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleFileVaultSummary, error) {
	if err := svc.authz.Authorize(ctx, mdmlab.MDMConfigProfileAuthz{TeamID: teamID}, mdmlab.ActionRead); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
...
func getMDMAppleProfilesSummaryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleProfilesSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMProfilesSummary, error) {
	if err := svc.authz.Authorize(ctx, mdmlab.MDMConfigProfileAuthz{TeamID: teamID}, mdmlab.ActionRead); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}

	if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
		return &mdmlab.MDMProfilesSummary{}, nil
	}
...
func uploadAppleInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) UploadMDMAppleInstaller(ctx context.Context, name string, size int64, installer io.Reader) (*mdmlab.MDMAppleInstaller, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleInstaller{}, mdmlab.ActionWrite); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
...
func (svc *Service) installerURL(token string, appConfig *mdmlab.AppConfig) string {
...
type getAppleInstallerDetailsResponse struct {
	Installer *mdmlab.MDMAppleInstaller
	Err       error `json:"error,omitempty"`
}

func (r getAppleInstallerDetailsResponse) error() error { return r.Err }

func getAppleInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleInstallerByID(ctx context.Context, id uint) (*mdmlab.MDMAppleInstaller, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleInstaller{}, mdmlab.ActionWrite); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
...
func deleteAppleInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
type listMDMAppleDevicesResponse struct {
	Devices []mdmlab.MDMAppleDevice `json:"devices"`
	Err     error                  `json:"error,omitempty"`
}

func (r listMDMAppleDevicesResponse) error() error { return r.Err }

func listMDMAppleDevicesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ListMDMAppleDevices(ctx context.Context) ([]mdmlab.MDMAppleDevice, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleDevice{}, mdmlab.ActionWrite); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
...
func newMDMAppleDEPKeyPairEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) NewMDMAppleDEPKeyPair(ctx context.Context) (*mdmlab.MDMAppleDEPKeyPair, error) {
	// skipauth: Generating a new key pair does not actually make any changes to mdmlab, or expose any
	// information. The user must configure mdmlab with the new key pair and restart the server.
	...
	return &mdmlab.MDMAppleDEPKeyPair{
...
func enqueueMDMAppleCommandEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) EnqueueMDMAppleCommand(
	...
	rawBase64Cmd string,
	deviceIDs []string,
) (result *mdmlab.CommandEnqueueResult, err error) {
	hosts, err := svc.authorizeAllHostsTeams(ctx, deviceIDs, mdmlab.ActionWrite, &mdmlab.MDMCommandAuthz{})
	if err != nil {
		return nil, err
	}
	if len(hosts) == 0 {
...
type mdmAppleEnrollRequest struct {
	...
	MachineInfo         *mdmlab.MDMAppleMachineInfo
}
...
type mdmAppleEnrollResponse struct {
	...
	SoftwareUpdateRequired *mdmlab.MDMAppleSoftwareUpdateRequired
}
...
func (r mdmAppleEnrollResponse) hijackRender(ctx context.Context, w http.ResponseWriter) {
	...
	w.Header().Set("Content-Type", "application/x-apple-aspen-config")
	w.Header().Set("X-Content-Type-Options", "nosniff")
	w.Header().Set("Content-Disposition", "attachment;mdmlab-enrollment-profile.mobileconfig")
...
func mdmAppleEnrollEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleEnrollmentProfileByToken(ctx context.Context, token string, ref string) (profile []byte, err error) {
	...
	if err != nil {
		if mdmlab.IsNotFound(err) {
			return nil, mdmlab.NewAuthFailedError("enrollment profile not found")
		}
	...
	enrollURL, err := apple_mdm.AddEnrollmentRefToMDMlabURL(appConfig.MDMUrl(), ref)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "adding reference to mdmlab URL")
	}
	...
	assets, err := svc.ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{
		mdmlab.MDMAssetSCEPChallenge,
	}, nil)
	if err != nil {
	...
	enrollmentProf, err := apple_mdm.GenerateEnrollmentProfileMobileconfig(
		...
		enrollURL,
		string(assets[mdmlab.MDMAssetSCEPChallenge].Value),
		topic,
	)
	if err != nil {
...
func (svc *Service) CheckMDMAppleEnrollmentWithMinimumOSVersion(ctx context.Context, m *mdmlab.MDMAppleMachineInfo) (*mdmlab.MDMAppleSoftwareUpdateRequired, error) {
	// skipauth: The enroll profile endpoint is unauthenticated.
	svc.authz.SkipAuthorization(ctx)
...
func (svc *Service) needsOSUpdateForDEPEnrollment(ctx context.Context, m mdmlab.MDMAppleMachineInfo) (bool, error) {
...
func (svc *Service) getAppleSoftwareUpdateRequiredForDEPEnrollment(m mdmlab.MDMAppleMachineInfo) (*mdmlab.MDMAppleSoftwareUpdateRequired, error) {
	...
	return mdmlab.NewMDMAppleSoftwareUpdateRequired(mdmlab.MDMAppleSoftwareUpdateAsset{
...
func (svc *Service) mdmPushCertTopic(ctx context.Context) (string, error) {
	...
	block, _ := pem.Decode(assets[mdmlab.MDMAssetAPNSCert].Value)
...
func mdmAppleCommandRemoveEnrollmentProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) EnqueueMDMAppleCommandRemoveEnrollmentProfile(ctx context.Context, hostID uint) error {
	...
	case "ipados":
		return &mdmlab.BadRequestError{
			Message: mdmlab.CantTurnOffMDMForIOSOrIPadOSMessage,
		}
	case "windows":
		return &mdmlab.BadRequestError{
			Message: mdmlab.CantTurnOffMDMForWindowsHostsMessage,
		}
	default:
	...
	if err := svc.authz.Authorize(ctx, mdmlab.MDMCommandAuthz{
		TeamID: h.TeamID,
	}, mdmlab.ActionWrite); err != nil {
		return err
	}
	...
	if nanoEnroll == nil || !nanoEnroll.Enabled {
		return mdmlab.NewUserMessageError(ctxerr.New(ctx, fmt.Sprintf("mdm is not enabled for host %d", hostID)), http.StatusConflict)
	}

	cmdUUID := uuid.New().String()
	err = svc.mdmAppleCommander.RemoveProfile(ctx, []string{h.UUID}, apple_mdm.MDMlabPayloadIdentifier, cmdUUID)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "enqueuing mdm apple remove profile command")
	}

	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mdmlab.ActivityTypeMDMUnenrolled{
...
func mdmAppleGetInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleInstallerByToken(ctx context.Context, token string) (*mdmlab.MDMAppleInstaller, error) {
...
func mdmAppleHeadInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleInstallerDetailsByToken(ctx context.Context, token string) (*mdmlab.MDMAppleInstaller, error) {
	// skipauth: The installer endpoint uses token authentication.
	svc.authz.SkipAuthorization(ctx)
...
type listMDMAppleInstallersResponse struct {
	Installers []mdmlab.MDMAppleInstaller `json:"installers"`
	Err        error                     `json:"error,omitempty"`
}

func (r listMDMAppleInstallersResponse) error() error { return r.Err }

func listMDMAppleInstallersEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ListMDMAppleInstallers(ctx context.Context) ([]mdmlab.MDMAppleInstaller, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.MDMAppleInstaller{}, mdmlab.ActionWrite); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
...
func deviceLockEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func deviceWipeEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
type getHostProfilesResponse struct {
	...
	Profiles []*mdmlab.MDMAppleConfigProfile `json:"profiles"`
	Err      error                          `json:"error,omitempty"`
}

func (r getHostProfilesResponse) error() error { return r.Err }

func getHostProfilesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	if res.Profiles == nil {
		res.Profiles = []*mdmlab.MDMAppleConfigProfile{} // return empty json array instead of json null
	}
	return res, nil
}

func (svc *Service) MDMListHostConfigurationProfiles(ctx context.Context, hostID uint) ([]*mdmlab.MDMAppleConfigProfile, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func batchSetMDMAppleProfilesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) BatchSetMDMAppleProfiles(ctx context.Context, tmID *uint, tmName *string, profiles [][]byte, dryRun, skipBulkPending bool) error {
	...
	if !appCfg.MDM.EnabledAndConfigured {
		...
		return ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("mdm", "cannot set custom settings: MDMlab MDM is not configured"))
	}

	// any duplicate identifier or name in the provided set results in an error
	profs := make([]*mdmlab.MDMAppleConfigProfile, 0, len(profiles))
	byName, byIdent := make(map[string]bool, len(profiles)), make(map[string]bool, len(profiles))
	for i, prof := range profiles {
		if len(prof) > 1024*1024 {
			return ctxerr.Wrap(ctx,
				mdmlab.NewInvalidArgumentError(fmt.Sprintf("profiles[%d]", i), "maximum configuration profile file size is 1 MB"),
			)
		}
		...
		if err != nil {
			return ctxerr.Wrap(ctx,
				mdmlab.NewInvalidArgumentError(fmt.Sprintf("profiles[%d]", i), err.Error()),
				"missing mdmlab secrets")
		}
		mdmProf, err := mdmlab.NewMDMAppleConfigProfile([]byte(expanded), tmID)
		if err != nil {
			return ctxerr.Wrap(ctx,
				mdmlab.NewInvalidArgumentError(fmt.Sprintf("profiles[%d]", i), err.Error()),
		...
		if byName[mdmProf.Name] {
			return ctxerr.Wrap(ctx,
				mdmlab.NewInvalidArgumentError(fmt.Sprintf("profiles[%d]", i), fmt.Sprintf("Couldn’t edit custom_settings. More than one configuration profile have the same name (PayloadDisplayName): %q", mdmProf.Name)),
		...
		if byIdent[mdmProf.Identifier] {
			return ctxerr.Wrap(ctx,
				mdmlab.NewInvalidArgumentError(fmt.Sprintf("profiles[%d]", i), fmt.Sprintf("Couldn’t edit custom_settings. More than one configuration profile have the same identifier (PayloadIdentifier): %q", mdmProf.Identifier)),
	...
	if !skipBulkPending {
		...
		allProfs, _, err := svc.ds.ListMDMConfigProfiles(ctx, tmID, mdmlab.ListOptions{PerPage: 0})
		if err != nil {
		...
		for _, p := range allProfs {
			if byName[p.Name] {
				...
				case strings.HasPrefix(p.ProfileUUID, "w"):
					err := mdmlab.NewInvalidArgumentError("PayloadDisplayName", fmt.Sprintf(
...
type preassignMDMAppleProfileRequest struct {
	mdmlab.MDMApplePreassignProfilePayload
}
...
func preassignMDMAppleProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) MDMApplePreassignProfile(ctx context.Context, payload mdmlab.MDMApplePreassignProfilePayload) error {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func matchMDMApplePreassignmentEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
type updateMDMAppleSettingsRequest struct {
	mdmlab.MDMAppleSettingsPayload
}
...
func updateMDMAppleSettingsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	if didUpdate {
		...
		if ac.MDM.EnabledAndConfigured { // if macOS MDM is configured, set up FileVault escrow
			...
			if ac.MDM.EnableDiskEncryption.Value {
				act = mdmlab.ActivityTypeEnabledMacosDiskEncryption{}
				if err := svc.EnterpriseOverrides.MDMAppleEnableFileVaultAndEscrow(ctx, nil); err != nil {
					return ctxerr.Wrap(ctx, err, "enable no-team filevault and escrow")
				}
			} else {
				act = mdmlab.ActivityTypeDisabledMacosDiskEncryption{}
...
func uploadBootstrapPackageEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func downloadBootstrapPackageEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleBootstrapPackageBytes(ctx context.Context, token string) (*mdmlab.MDMAppleBootstrapPackage, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
type bootstrapPackageMetadataRequest struct {
	...
	// For context, see: https://github.com/mdmlabdm/mdmlab/issues/15337#issuecomment-1932878997
	ForUpdate bool `query:"for_update,optional"`
}

type bootstrapPackageMetadataResponse struct {
	Err                             error `json:"error,omitempty"`
	*mdmlab.MDMAppleBootstrapPackage `json:",omitempty"`
}

func (r bootstrapPackageMetadataResponse) error() error { return r.Err }

func bootstrapPackageMetadataEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	case mdmlab.IsNotFound(err):
		...
		return bootstrapPackageMetadataResponse{Err: mdmlab.NewInvalidArgumentError("team_id",
...
func (svc *Service) GetMDMAppleBootstrapPackageMetadata(ctx context.Context, teamID uint, forUpdate bool) (*mdmlab.MDMAppleBootstrapPackage, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func deleteBootstrapPackageEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
type getMDMAppleBootstrapPackageSummaryResponse struct {
	mdmlab.MDMAppleBootstrapPackageSummary
	Err error `json:"error,omitempty"`
}

func (r getMDMAppleBootstrapPackageSummaryResponse) error() error { return r.Err }

func getMDMAppleBootstrapPackageSummaryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleBootstrapPackageSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleBootstrapPackageSummary, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)

	return &mdmlab.MDMAppleBootstrapPackageSummary{}, mdmlab.ErrMissingLicense
}
...
type createMDMAppleSetupAssistantResponse struct {
	mdmlab.MDMAppleSetupAssistant
	Err error `json:"error,omitempty"`
}

func (r createMDMAppleSetupAssistantResponse) error() error { return r.Err }

func createMDMAppleSetupAssistantEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*createMDMAppleSetupAssistantRequest)
	asst, err := svc.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{
		TeamID:  req.TeamID,
		Name:    req.Name,
...
func (svc *Service) SetOrUpdateMDMAppleSetupAssistant(ctx context.Context, asst *mdmlab.MDMAppleSetupAssistant) (*mdmlab.MDMAppleSetupAssistant, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func getMDMAppleSetupAssistantEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetMDMAppleSetupAssistant(ctx context.Context, teamID *uint) (*mdmlab.MDMAppleSetupAssistant, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func deleteMDMAppleSetupAssistantEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
// to admins) to update some MDM Apple settings, while the update config/update
// team endpoints only allow write access to admins.
func updateMDMAppleSetupEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) UpdateMDMAppleSetup(ctx context.Context, payload mdmlab.MDMAppleSetupPayload) error {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func initiateMDMAppleSSOEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func callbackMDMAppleSSOEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	auth := request.(mdmlab.Auth)
	redirectURL := svc.InitiateMDMAppleSSOCallback(ctx, auth)
	return callbackMDMAppleSSOResponse{redirectURL: redirectURL}, nil
}

func (svc *Service) InitiateMDMAppleSSOCallback(ctx context.Context, auth mdmlab.Auth) string {
	// skipauth: No authorization check needed due to implementation
	// returning only license error.
	svc.authz.SkipAuthorization(ctx)

	return apple_mdm.MDMlabUISSOCallbackPath + "?error=true"
}
...
func getManualEnrollmentProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func NewMDMAppleCheckinAndCommandService(ds mdmlab.Datastore, commander *apple_mdm.MDMAppleCommander, logger kitlog.Logger) *MDMAppleCheckinAndCommandService {
...
func (svc *MDMAppleCheckinAndCommandService) Authenticate(r *mdm.Request, m *mdm.Authenticate) error {
	...
	return newActivity(
		r.Context, nil, &mdmlab.ActivityTypeMDMEnrolled{
			HostSerial:       updatedInfo.HardwareSerial,
			HostDisplayName:  updatedInfo.DisplayName,
			InstalledFromDEP: updatedInfo.DEPAssignedToMDMlab,
			MDMPlatform:      mdmlab.MDMPlatformApple,
		}, svc.ds, svc.logger,
	)
}
...
func (svc *MDMAppleCheckinAndCommandService) TokenUpdate(r *mdm.Request, m *mdm.TokenUpdate) error {
	...
	return svc.mdmLifecycle.Do(r.Context, mdmlifecycle.HostOptions{
		...
		Platform:                info.Platform,
		UUID:                    r.ID,
		EnrollReference:         r.Params[mobileconfig.MDMlabEnrollReferenceKey],
...
func (svc *MDMAppleCheckinAndCommandService) CheckOut(r *mdm.Request, m *mdm.CheckOut) error {
	...
	return newActivity(
		r.Context, nil, &mdmlab.ActivityTypeMDMUnenrolled{
...
func (svc *MDMAppleCheckinAndCommandService) CommandAndReportResults(r *mdm.Request, cmdResult *mdm.CommandResults) (*mdm.Command, error) {
	...
	if strings.HasPrefix(cmdResult.CommandUUID, mdmlab.RefetchBaseCommandUUIDPrefix) {
	...
	case "RemoveProfile":
		return nil, svc.ds.UpdateOrDeleteHostMDMAppleProfile(r.Context, &mdmlab.HostMDMAppleProfile{
	...
	case "DeviceLock", "EraseDevice":
		...
		if cmdResult.Status == mdmlab.MDMAppleStatusAcknowledged ||
			cmdResult.Status == mdmlab.MDMAppleStatusError ||
			cmdResult.Status == mdmlab.MDMAppleStatusCommandFormatError {
			return nil, svc.ds.UpdateHostLockWipeStatusFromAppleMDMResult(r.Context, cmdResult.UDID, cmdResult.CommandUUID, requestType,
				cmdResult.Status == mdmlab.MDMAppleStatusAcknowledged)
		}
	...
	case "InstallApplication":
		...
		if updated, err := maybeUpdateSetupExperienceStatus(r.Context, svc.ds, mdmlab.SetupExperienceVPPInstallResult{
			HostUUID:      cmdResult.UDID,
			CommandUUID:   cmdResult.CommandUUID,
...
func (svc *MDMAppleCheckinAndCommandService) handleRefetch(r *mdm.Request, cmdResult *mdm.CommandResults) (*mdm.Command, error) {
	...
	if strings.HasPrefix(cmdResult.CommandUUID, mdmlab.RefetchAppsCommandUUIDPrefix) {
		...
		err = svc.ds.RemoveHostMDMCommand(ctx, mdmlab.HostMDMCommand{
			HostID:      host.ID,
			CommandType: mdmlab.RefetchAppsCommandUUIDPrefix,
		})
		if err != nil {
	...
	// Otherwise, the command has prefix mdmlab.RefetchDeviceCommandUUIDPrefix, which is a refetch device command.
	// We remove pending command first in case there is an error processing the results, so that we don't prevent another refetch.
	err = svc.ds.RemoveHostMDMCommand(ctx, mdmlab.HostMDMCommand{
		HostID:      host.ID,
		CommandType: mdmlab.RefetchDeviceCommandUUIDPrefix,
	})
	if err != nil {
	...
	if err := svc.ds.UpdateHostOperatingSystem(r.Context, host.ID, mdmlab.OperatingSystem{
...
func unmarshalAppList(ctx context.Context, response []byte, source string) ([]mdmlab.Software,
	error,
) {
	...
	var software []mdmlab.Software
	for _, app := range appsResponse.InstalledApplicationList {
		software = append(software, mdmlab.Software{
			Name:             truncateString(app["Name"], mdmlab.SoftwareNameMaxLength),
			Version:          truncateString(app["ShortVersion"], mdmlab.SoftwareVersionMaxLength),
			BundleIdentifier: truncateString(app["Identifier"], mdmlab.SoftwareBundleIdentifierMaxLength),
			Source:           source,
		})
	}
...
func mdmAppleDeliveryStatusFromCommandStatus(cmdStatus string) *mdmlab.MDMDeliveryStatus {
	switch cmdStatus {
	case mdmlab.MDMAppleStatusAcknowledged:
		return &mdmlab.MDMDeliveryVerifying
	case mdmlab.MDMAppleStatusError, mdmlab.MDMAppleStatusCommandFormatError:
		return &mdmlab.MDMDeliveryFailed
	case mdmlab.MDMAppleStatusIdle, mdmlab.MDMAppleStatusNotNow:
		return &mdmlab.MDMDeliveryPending
	default:
		return nil
	}
}

// ensureMDMlabProfiles ensures there's a mdmlabd configuration profile in
...
// will only be used by hosts that have a mdmlabd installation without an enroll
// secret and mdmlab URL (mainly DEP enrolled hosts).
func ensureMDMlabProfiles(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger, signingCertDER []byte) error {
	...
	params := mobileconfig.MDMlabCARootTemplateOptions{
		...
		PayloadName:       mdm_types.MDMlabCAConfigProfileName,
		Certificate:       base64.StdEncoding.EncodeToString(signingCertDER),
	}

	if err := mobileconfig.MDMlabCARootTemplate.Execute(&rootCAProfContents, params); err != nil {
		return ctxerr.Wrap(ctx, err, "executing mdmlab root CA config template")
	}
	...
	var profiles []*mdmlab.MDMAppleConfigProfile
	for _, es := range enrollSecrets {
		if es.Secret == "" {
			var msg string
			if es.TeamID != nil {
			...
			if globalSecret == "" {
				logger.Log("err", msg+"no global enroll secret found, skipping the creation of a com.mdmlabdm.mdmlabd.config profile")
				continue
			}
			logger.Log("err", msg+"using a global enroll secret for com.mdmlabdm.mdmlabd.config profile")
		...
		params := mobileconfig.MDMlabdProfileOptions{
			...
			ServerURL:    appCfg.ServerSettings.ServerURL, // ServerURL must be set to the MDMlab URL.  Do not use appCfg.MDMUrl() here.
			PayloadType:  mobileconfig.MDMlabdConfigPayloadIdentifier,
			PayloadName:  mdm_types.MDMlabdConfigProfileName,
		}

		if err := mobileconfig.MDMlabdProfileTemplate.Execute(&contents, params); err != nil {
			return ctxerr.Wrap(ctx, err, "executing mdmlabd config template")
		}

		cp, err := mdmlab.NewMDMAppleConfigProfile(contents.Bytes(), es.TeamID)
		if err != nil {
			return ctxerr.Wrap(ctx, err, "building mdmlabd configuration profile")
		}
		profiles = append(profiles, cp)

		rootCAProf, err := mdmlab.NewMDMAppleConfigProfile(b, es.TeamID)
		if err != nil {
...
func ReconcileAppleProfiles(
	...
	if err := ensureMDMlabProfiles(ctx, ds, logger, block.Bytes); err != nil {
		logger.Log("err", "unable to ensure a mdmlabd configuration profiles are in place", "details", err)
	}
	...
	toInstall = mdmlab.FilterMacOSOnlyProfilesFromIOSIPadOS(toInstall)
	...
	hostProfiles := make([]*mdmlab.MDMAppleBulkUpsertHostProfilePayload, 0, len(toInstall)+len(toRemove))
	...
	// command.
	hostProfilesToCleanup := []*mdmlab.MDMAppleProfilePayload{}

	// Index host profiles to install by host and profile UUID, for easier bulk error processing
	hostProfilesToInstallMap := make(map[hostProfileUUID]*mdmlab.MDMAppleBulkUpsertHostProfilePayload, len(toInstall))
	...
	for _, p := range toInstall {
		if pp, ok := profileIntersection.GetMatchingProfileInCurrentState(p); ok {
			...
			// command.
			if pp.Status != &mdmlab.MDMDeliveryFailed && bytes.Equal(pp.Checksum, p.Checksum) {
				hostProfile := &mdmlab.MDMAppleBulkUpsertHostProfilePayload{
	...
	for _, p := range toRemove {
		...
		hostProfiles = append(hostProfiles, &mdmlab.MDMAppleBulkUpsertHostProfilePayload{
			ProfileUUID:       p.ProfileUUID,
			HostUUID:          p.HostUUID,
			OperationType:     mdmlab.MDMOperationTypeRemove,
			Status:            &mdmlab.MDMDeliveryPending,
	...
	execCmd := func(profUUID string, target *cmdTarget, op mdmlab.MDMOperationType) {
		defer wgProd.Done()

		var err error
		switch op {
		case mdmlab.MDMOperationTypeInstall:
		...
		case mdmlab.MDMOperationTypeRemove:
	...
	for profUUID, target := range installTargets {
		wgProd.Add(1)
		go execCmd(profUUID, target, mdmlab.MDMOperationTypeInstall)
	}
	for profUUID, target := range removeTargets {
		wgProd.Add(1)
		go execCmd(profUUID, target, mdmlab.MDMOperationTypeRemove)
	}

	// index the host profiles by cmdUUID, for ease of error processing in the
	// consumer goroutine below.
	hostProfsByCmdUUID := make(map[string][]*mdmlab.MDMAppleBulkUpsertHostProfilePayload, len(installTargets)+len(removeTargets))
	...
	// failures.
	failed := []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{}
	wgCons.Add(1)
	go func() {
		defer wgCons.Done()
...
func findProfilesWithSecrets(
	...
	for profUUID := range installTargets {
		...
		profileStr := string(p)
		vars := mdmlab.ContainsPrefixVars(profileStr, mdmlab.ServerSecretPrefix)
		if len(vars) > 0 {
...
func preprocessProfileContents(
	...
	profileContents map[string]mobileconfig.Mobileconfig,
	hostProfilesToInstallMap map[hostProfileUUID]*mdmlab.MDMAppleBulkUpsertHostProfilePayload,
) error {
	// This method replaces MDMlab variables ($FLEET_VAR_<NAME>) in the profile contents, generating a unique profile for each host.
	...
	isNDESSCEPConfigured := func(profUUID string, target *cmdTarget) (bool, error) {
		if !license.IsPremium(ctx) {
			profilesToUpdate := make([]*mdmlab.MDMAppleBulkUpsertHostProfilePayload, 0, len(target.hostUUIDs))
			for _, hostUUID := range target.hostUUIDs {
				...
				profile.Detail = "NDES SCEP Proxy requires a MDMlab Premium license."
	...
	var ndesConfig *mdmlab.NDESSCEPProxyIntegration
	...
	for profUUID, target := range targets {
		...
		// Check if MDMlab variables are present.
		contentsStr := string(contents)
		mdmlabVars := findMDMlabVariables(contentsStr)
		if len(mdmlabVars) == 0 {
			continue
		}
		...
		valid := true
		for mdmlabVar := range mdmlabVars {
			switch mdmlabVar {
			case MDMlabVarNDESSCEPChallenge, MDMlabVarNDESSCEPProxyURL:
			...
			case MDMlabVarHostEndUserEmailIDP:
			...
			default:
				...
				for _, hostUUID := range target.hostUUIDs {
					...
					profile.Status = &mdmlab.MDMDeliveryFailed
					profile.Detail = fmt.Sprintf("Unknown MDMlab variable $FLEET_VAR_%s found in profile. Please update or remove.",
						mdmlabVar)
					profilesToUpdate = append(profilesToUpdate, profile)
				}
				if err := ds.BulkUpsertMDMAppleHostProfiles(ctx, profilesToUpdate); err != nil {
		...
		// Currently, all supported MDMlab variables are unique per host, so we split the profile into multiple profiles.
		...
		var managedCertificatePayloads []*mdmlab.MDMBulkUpsertManagedCertificatePayload
		...
		for _, hostUUID := range target.hostUUIDs {
			...
			for mdmlabVar := range mdmlabVars {
				switch mdmlabVar {
				case MDMlabVarNDESSCEPChallenge:
					if ndesConfig == nil {
						// Retrieve the NDES admin password. This is done once per run.
						configAssets, err := ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{mdmlab.MDMAssetNDESPassword}, nil)
						if err != nil {
						...
						configWithPassword.Password = string(configAssets[mdmlab.MDMAssetNDESPassword].Value)
					...
					if err != nil {
						...
						case errors.As(err, &eeservice.NDESInvalidError{}):
							detail = fmt.Sprintf("Invalid NDES admin credentials. "+
								"MDMlab couldn't populate $FLEET_VAR_%s. "+
						...
						case errors.As(err, &eeservice.NDESInsufficientPermissionsError{}):
							detail = fmt.Sprintf("This account does not have sufficient permissions to enroll with SCEP. "+
								...
								MDMlabVarNDESSCEPChallenge)
						default:
							detail = fmt.Sprintf("MDMlab couldn't populate $FLEET_VAR_%s. %s", MDMlabVarNDESSCEPChallenge, err.Error())
						}
					...
					payload := &mdmlab.MDMBulkUpsertManagedCertificatePayload{
						HostUUID:             hostUUID,
					...
					hostContents = replaceMDMlabVariable(mdmlabVarNDESSCEPChallengeRegexp, hostContents, challenge)
				case MDMlabVarNDESSCEPProxyURL:
					...
					hostContents = replaceMDMlabVariable(mdmlabVarNDESSCEPProxyURLRegexp, hostContents, proxyURL)
				case MDMlabVarHostEndUserEmailIDP:
					// Insert the end user email IDP into the profile contents
					emails, err := ds.GetHostEmails(ctx, hostUUID, mdmlab.DeviceMappingMDMIdpAccounts)
					if err != nil {
					...
					if len(emails) == 0 {
						...
						err := ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mdmlab.HostMDMAppleProfile{
							...
							Detail: fmt.Sprintf("There is no IdP email for this host. "+
								"MDMlab couldn't populate $FLEET_VAR_%s. "+
								"[Learn more](https://mdmlabdm.com/learn-more-about/idp-email)",
								MDMlabVarHostEndUserEmailIDP),
							OperationType: mdmlab.MDMOperationTypeInstall,
						})
						if err != nil {
					...
					hostContents = replaceMDMlabVariable(mdmlabVarHostEndUserEmailIDPRegexp, hostContents, emails[0])
				default:
...
func replaceMDMlabVariable(regExp *regexp.Regexp, contents string, replacement string) string {
...
func findMDMlabVariables(contents string) map[string]interface{} {
...
// renewals.
//
// For a default of 180 days as a threshold this gives us room for a mdmlab of
...
func RenewSCEPCertificates(
	...
	// SSO to authenticate.
	assocsWithRefs := []mdmlab.SCEPIdentityAssociation{}
	// assocsWithoutRefs stores hosts that don't have an enrollment
	// reference in their enrollment profile.
	assocsWithoutRefs := []mdmlab.SCEPIdentityAssociation{}
	// assocsFromMigration stores hosts that were migrated from another MDM
	// using the process described in
	// https://github.com/mdmlabdm/mdmlab/issues/19387
	assocsFromMigration := []mdmlab.SCEPIdentityAssociation{}
	...
	for _, assoc := range assocsWithRefs {
		enrollURL, err := apple_mdm.AddEnrollmentRefToMDMlabURL(appConfig.MDMUrl(), assoc.EnrollReference)
		if err != nil {
			return ctxerr.Wrap(ctx, err, "adding reference to mdmlab URL")
		}
		...
		if err := renewSCEPWithProfile(ctx, ds, commander, logger, []mdmlab.SCEPIdentityAssociation{assoc}, profile); err != nil {
			return ctxerr.Wrap(ctx, err, "sending profile to hosts without associations")
		}
	}
...
func renewSCEPWithProfile(
	...
	commander *apple_mdm.MDMAppleCommander,
	logger kitlog.Logger,
	assocs []mdmlab.SCEPIdentityAssociation,
	profile []byte,
) error {
	...
	for _, assoc := range assocs {
		...
		// https://github.com/mdmlabdm/mdmlab/issues/19311 where a
...
func NewMDMAppleDDMService(ds mdmlab.Datastore, logger kitlog.Logger) *MDMAppleDDMService {
...
func (svc *MDMAppleDDMService) handleTokens(ctx context.Context, hostUUID string) ([]byte, error) {
	...
	b, err := json.Marshal(mdmlab.MDMAppleDDMTokensResponse{
		SyncTokens: *tok,
	})
	if err != nil {
...
func (svc *MDMAppleDDMService) handleDeclarationItems(ctx context.Context, hostUUID string) ([]byte, error) {
	...
	activations := []mdmlab.MDMAppleDDMManifest{}
	configurations := []mdmlab.MDMAppleDDMManifest{}
	for _, d := range di {
		configurations = append(configurations, mdmlab.MDMAppleDDMManifest(d))
		activations = append(activations, mdmlab.MDMAppleDDMManifest{
	...
	b, err := json.Marshal(mdmlab.MDMAppleDDMDeclarationItemsResponse{
		Declarations: mdmlab.MDMAppleDDMManifestItems{
			Activations:    activations,
			Configurations: configurations,
			Assets:         []mdmlab.MDMAppleDDMManifest{},
			Management:     []mdmlab.MDMAppleDDMManifest{},
		},
...
func (svc *MDMAppleDDMService) handleDeclarationStatus(ctx context.Context, dm *mdm.DeclarativeManagement) error {
	var status mdmlab.MDMAppleDDMStatusReport
	...
	updates := make([]*mdmlab.MDMAppleHostDeclaration, len(configurationReports))
	for i, r := range configurationReports {
		var status mdmlab.MDMDeliveryStatus
		var detail string
		switch {
		case r.Active && r.Valid == mdmlab.MDMAppleDeclarationValid:
			status = mdmlab.MDMDeliveryVerified
		case r.Valid == mdmlab.MDMAppleDeclarationInvalid:
			status = mdmlab.MDMDeliveryFailed
		...
		updates[i] = &mdmlab.MDMAppleHostDeclaration{
			Status:        &status,
			OperationType: mdmlab.MDMOperationTypeInstall,
			Detail:        detail,
			Token:         r.ServerToken,
...
func generateABMKeyPairEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GenerateABMKeyPair(ctx context.Context) (*mdmlab.MDMAppleDEPKeyPair, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.AppleBM{}, mdmlab.ActionWrite); err != nil {
		return nil, err
	}
	...
	if len(privateKey) == 0 {
		return nil, ctxerr.New(ctx, "Couldn't download public key. Missing required private key. Learn how to configure the private key here: https://mdmlabdm.com/learn-more-about/mdmlab-server-private-key")
	}
	...
	if len(assets) == 0 {
		...
		err = svc.ds.InsertMDMConfigAssets(ctx, []mdmlab.MDMConfigAsset{
			{Name: mdmlab.MDMAssetABMCert, Value: publicKeyPEM},
			{Name: mdmlab.MDMAssetABMKey, Value: privateKeyPEM},
		}, nil)
		if err != nil {
		...
		publicKeyPEM = assets[mdmlab.MDMAssetABMCert].Value
		privateKeyPEM = assets[mdmlab.MDMAssetABMKey].Value
	}
...
func (uploadABMTokenRequest) DecodeRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	if !ok || len(token) < 1 {
		return nil, &mdmlab.BadRequestError{Message: "no file headers for token"}
	}
...
type uploadABMTokenResponse struct {
	Token *mdmlab.ABMToken `json:"abm_token,omitempty"`
	Err   error           `json:"error,omitempty"`
}

func (r uploadABMTokenResponse) error() error { return r.Err }

func uploadABMTokenEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) UploadABMToken(ctx context.Context, token io.Reader) (*mdmlab.ABMToken, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func deleteABMTokenEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
type listABMTokensResponse struct {
	Err    error             `json:"error,omitempty"`
	Tokens []*mdmlab.ABMToken `json:"abm_tokens"`
}

func (r listABMTokensResponse) error() error { return r.Err }

func listABMTokensEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ListABMTokens(ctx context.Context) ([]*mdmlab.ABMToken, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func countABMTokensEndpoint(ctx context.Context, _ interface{}, svc mdmlab.Service) (errorer, error) {
...
type updateABMTokenTeamsResponse struct {
	ABMToken *mdmlab.ABMToken `json:"abm_token,omitempty"`
	Err      error           `json:"error,omitempty"`
}

func (r updateABMTokenTeamsResponse) error() error { return r.Err }

func updateABMTokenTeamsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) UpdateABMTokenTeams(ctx context.Context, tokenID uint, macOSTeamID, iOSTeamID, iPadOSTeamID *uint) (*mdmlab.ABMToken, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func (renewABMTokenRequest) DecodeRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	// /api/latest/mdmlab/abm_tokens/19/renew
...
func renewABMTokenEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) RenewABMToken(ctx context.Context, token io.Reader, tokenID uint) (*mdmlab.ABMToken, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func getOTAProfileEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	return &getMDMAppleConfigProfileResponse{fileReader: io.NopCloser(reader), fileLength: reader.Size(), fileName: "mdmlab-mdm-enrollment-profile"}, nil
}
...
func mdmAppleOTAEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) MDMAppleProcessOTAEnrollment(
	...
	// was issued by MDMlab, only let the enrollment through if so.
