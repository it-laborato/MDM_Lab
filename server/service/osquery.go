...
type osqueryError struct {
	...
	nodeInvalid bool
	statusCode  int
	mdmlab.ErrorWithUUID
}

var _ mdmlab.ErrorUUIDer = (*osqueryError)(nil)
...
func (svc *Service) AuthenticateHost(ctx context.Context, nodeKey string) (*mdmlab.Host, bool, error) {
...
func enrollAgentEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) serialUpdateHost(host *mdmlab.Host) {
...
func getClientConfigEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) getScheduledQueries(ctx context.Context, teamID *uint) (mdmlab.Queries, error) {
	appConfig, err := svc.ds.AppConfig(ctx)
	if err != nil {
	...
	config := make(mdmlab.Queries, len(queries))
...
func (svc *Service) GetClientConfig(ctx context.Context) (map[string]interface{}, error) {
	...
	for _, pack := range packs {
		...
		configQueries := mdmlab.Queries{}
		for _, query := range queries {
			queryContent := mdmlab.QueryContent{
				Query:    query.Query,
		...
		// the pack's queries
		packConfig[pack.Name] = mdmlab.PackContent{
	...
	if len(globalQueries) > 0 {
		packConfig["Global"] = mdmlab.PackContent{
	...
	if host.TeamID != nil {
		...
		if len(teamQueries) > 0 {
			...
			packConfig[packName] = mdmlab.PackContent{
				Queries: teamQueries,
			}
		}
	}
	...
	intervals := mdmlab.HostOsqueryIntervals{
...
func (svc *Service) AgentOptionsForHost(ctx context.Context, hostTeamID *uint, hostPlatform string) (json.RawMessage, error) {
	...
	if hostTeamID != nil {
		...
		if teamAgentOptions != nil && len(*teamAgentOptions) > 0 {
			var options mdmlab.AgentOptions
...
func getDistributedQueriesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetDistributedQueries(ctx context.Context) (queries map[string]string, discovery map[string]string, accelerate uint, err error) {
	...
	for name, query := range queries {
		// there's a bug somewhere (MDMlab, osquery or both?)
...
func (svc *Service) detailQueriesForHost(ctx context.Context, host *mdmlab.Host) (queries map[string]string, discovery map[string]string, err error) {
...
func (svc *Service) labelQueriesForHost(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
...
func (svc *Service) policyQueriesForHost(ctx context.Context, host *mdmlab.Host) (policyQueries map[string]string, noPoliciesForHost bool, err error) {
...
type submitDistributedQueryResultsRequestShim struct {
	...
	Stats    map[string]*mdmlab.Stats    `json:"stats"`
}
...
func (shim *submitDistributedQueryResultsRequestShim) toRequest(ctx context.Context) (*SubmitDistributedQueryResultsRequest, error) {
	...
	for query, status := range shim.Statuses {
		...
		case string:
			...
			statuses[query] = mdmlab.OsqueryStatus(sint)
		case float64:
			statuses[query] = mdmlab.OsqueryStatus(s)
		default:
...
type SubmitDistributedQueryResultsRequest struct {
	NodeKey  string                               `json:"node_key"`
	Results  mdmlab.OsqueryDistributedQueryResults `json:"queries"`
	Statuses map[string]mdmlab.OsqueryStatus       `json:"statuses"`
	Messages map[string]string                    `json:"messages"`
	Stats    map[string]*mdmlab.Stats              `json:"stats"`
}
...
func submitDistributedQueryResultsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
const (
	...
	hostAdditionalQueryPrefix = "mdmlab_additional_query_"
	...
	// osqueryd writes the distributed query results.
	hostPolicyQueryPrefix = "mdmlab_policy_query_"
	...
	// When MDMlab receives results from such query then it will update the host's
...
func (svc *Service) SubmitDistributedQueryResults(
	...
	detailUpdated := false
	additionalResults := make(mdmlab.OsqueryDistributedQueryResults)
	...
	for query, rows := range results {
		...
		failed := ok && status != mdmlab.StatusOK
...
func processCalendarPolicies(
	...
	switch {
	case err == nil:
		if hostCalendarEvent.WebhookStatus != mdmlab.CalendarWebhookStatusPending {
			return nil
		}
	case mdmlab.IsNotFound(err):
		return nil
	...
	if now.Before(calendarEvent.StartTime) {
		...
		if err = ds.UpdateHostCalendarWebhookStatus(context.Background(), host.ID, mdmlab.CalendarWebhookStatusError); err != nil {
	...
	go func() {
		...
		err := backoff.Retry(
			func() error {
				if err := mdmlab.FireCalendarWebhook(
					...
					if errors.As(err, &statusCoder) && statusCoder.StatusCode() == http.StatusTooManyRequests {
						...
						if err := ds.UpdateHostCalendarWebhookStatus(
							context.Background(), host.ID, mdmlab.CalendarWebhookStatusRetry,
						); err != nil {
		...
		nextStatus := mdmlab.CalendarWebhookStatusSent
		if err != nil {
			level.Error(logger).Log("msg", "fire webhook", "err", err)
			nextStatus = mdmlab.CalendarWebhookStatusError
		}
...
func getFailingCalendarPolicies(policyResults map[uint]*bool, calendarPolicies []mdmlab.PolicyCalendarData) []mdmlab.PolicyCalendarData {
	var failingPolicies []mdmlab.PolicyCalendarData
...
func pythonPackageFilter(platform string, results *mdmlab.OsqueryDistributedQueryResults, statuses *map[string]mdmlab.OsqueryStatus) {
	...
	if status, ok := (*statuses)[linuxSoftware]; !ok || status != mdmlab.StatusOK {
		return
	}
...
func preProcessSoftwareExtraResults(
	...
	for _, query := range []string{
		...
		if status, ok := (*statuses)[query]; ok && status != mdmlab.StatusOK {
...
func globalPolicyAutomationsEnabled(webhookSettings mdmlab.WebhookSettings, integrations mdmlab.Integrations) bool {
...
func teamPolicyAutomationsEnabled(webhookSettings mdmlab.TeamWebhookSettings, integrations mdmlab.TeamIntegrations) bool {
	if webhookSettings.FailingPoliciesWebhook.Enable {
		return true
	}
	for _, j := range integrations.Jira {
...
func (svc *Service) ingestQueryResults(
	...
	policyResults map[uint]*bool,
	labelResults map[uint]*bool,
	additionalResults mdmlab.OsqueryDistributedQueryResults,
	stats *mdmlab.Stats,
) (bool, bool, error) {
...
func (svc *Service) directIngestDetailQuery(ctx context.Context, host *mdmlab.Host, name string, rows []map[string]string) (ingested bool, err error) {
	features, err := svc.HostFeatures(ctx, host)
	if err != nil {
...
// provided mdmlab.Host appropriately.
func (svc *Service) ingestDistributedQuery(
	ctx context.Context, host mdmlab.Host, name string, rows []map[string]string, errMsg string, stats *mdmlab.Stats,
) error {
	...
	res := mdmlab.DistributedQueryResult{
	...
	if err != nil {
		...
		if campaign.CreatedAt.After(svc.clock.Now().Add(-1 * time.Minute)) {
			...
			//	2. The client (UI/mdmlabctl) starts listenting for query results.
			//
		...
		if campaign.Status != mdmlab.QueryComplete {
			campaign.Status = mdmlab.QueryComplete
...
// provided mdmlab.Host appropriately.
func (svc *Service) ingestDetailQuery(ctx context.Context, host *mdmlab.Host, name string, rows []map[string]string) error {
	features, err := svc.HostFeatures(ctx, host)
	if err != nil {
...
func (svc *Service) registerFlippedPolicies(ctx context.Context, hostID uint, hostname, displayName string, newFailing, newPassing []uint) error {
	...
	for _, policyID := range newPassing {
		if err := svc.failingPolicySet.RemoveHosts(policyID, []mdmlab.PolicySetHost{host}); err != nil {
			return err
		}
	}
	return nil
...
func (svc *Service) processSoftwareForNewlyFailingPolicies(
	...
	if hostTeamID == nil {
		policyTeamID = mdmlab.PolicyNoTeamID
	} else {
	...
	policiesWithInstallersMap := make(map[uint]mdmlab.PolicySoftwareInstallerData)
	...
	var failingPoliciesWithInstaller []mdmlab.PolicySoftwareInstallerData
	for _, policyWithInstaller := range policiesWithInstaller {
	...
	for _, failingPolicyWithInstaller := range failingPoliciesWithInstaller {
		...
		if mdmlab.PlatformFromHost(hostPlatform) != installerMetadata.Platform {
		...
		// hostLastInstall.Status == nil can happen when a software is installed by MDMlab and later removed.
		if hostLastInstall != nil && hostLastInstall.Status != nil &&
			*hostLastInstall.Status == mdmlab.SoftwareInstallPending {
		...
		// (in SaveHostSoftwareInstallResult) the author will be set to MDMlab.
		installUUID, err := svc.ds.InsertSoftwareInstallRequest(
			ctx, hostID,
			installerMetadata.InstallerID,
			false, // Set Self-service as false because this is triggered by MDMlab.
			&policyID,
		)
		if err != nil {
...
func (svc *Service) processVPPForNewlyFailingPolicies(
	...
	policiesWithVPPMap := make(map[uint]mdmlab.PolicyVPPData)
	...
	var failingPoliciesWithVPP []mdmlab.PolicyVPPData
	for _, policyWithVPP := range policiesWithVPP {
...
func (svc *Service) processScriptsForNewlyFailingPolicies(
	...
	policiesWithScriptsMap := make(map[uint]mdmlab.PolicyScriptData)
	...
	var failingPoliciesWithScript []mdmlab.PolicyScriptData
	for _, policyWithScript := range policiesWithScript {
	...
	for _, failingPolicyWithScript := range failingPoliciesWithScript {
		...
		hostPlatform := mdmlab.PlatformFromHost(hostPlatform)
		...
		runScriptRequest := mdmlab.HostScriptRequestPayload{
			HostID:          hostID,
			...
			TeamID:          policyTeamID,
			PolicyID:        &policyID,
			// no user ID as scripts are executed by MDMlab
		}
...
func submitLogsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
//   - `unmarshaledResults` with each result unmarshaled to `mdmlab.ScheduledQueryResult`s, where if an item is `nil` it means the corresponding
...
func (svc *Service) preProcessOsqueryResults(
...
) (unmarshaledResults []*mdmlab.ScheduledQueryResult, queriesDBData map[string]*mdmlab.Query) {
	// skipauth: Authorization is currently for user endpoints only.
	svc.authz.SkipAuthorization(ctx)
	...
	for _, raw := range osqueryResults {
		var result *mdmlab.ScheduledQueryResult
	...
	queriesDBData = make(map[string]*mdmlab.Query)
	for _, queryResult := range unmarshaledResults {
		...
		if errors.Is(err, mdmlab.ErrLegacyQueryPack) {
...
func (svc *Service) SubmitResultLogs(ctx context.Context, logs []json.RawMessage) error {
	...
	for i, unmarshaledResult := range unmarshaledResults {
		...
		if queryReportsDisabled {
			...
			// and thus MDMlab won't receive any further results anymore.
		...
		if !ok {
			// If MDMlab doesn't know of the query we write the logs to the logging destination
			...
			//
			// If a query was configured from MDMlab but was recently removed, we may still write
			// the results for it here. Eventually the query will be removed from the host schedule
			// and thus MDMlab won't receive any further results anymore.
			filteredLogs = append(filteredLogs, logs[i])
			continue
...
func (svc *Service) saveResultLogsToQueryReports(
	ctx context.Context,
	unmarshaledResults []*mdmlab.ScheduledQueryResult,
	queriesDBData map[string]*mdmlab.Query,
	...
	for _, result := range filtered {
		...
		if dbQuery.DiscardData || dbQuery.Logging != mdmlab.LoggingSnapshot {
...
func (svc *Service) overwriteResultRows(ctx context.Context, result *mdmlab.ScheduledQueryResult, queryID, hostID uint, maxQueryReportRows int) error {
	fetchTime := time.Now()

	rows := make([]*mdmlab.ScheduledQueryResultRow, 0, len(result.Snapshot))

	// If the snapshot is empty, we still want to save a row with a null value
	// to capture LastFetched.
	if len(result.Snapshot) == 0 {
		rows = append(rows, &mdmlab.ScheduledQueryResultRow{
			QueryID:     queryID,
			HostID:      hostID,
			Data:        nil,
	...
	for _, snapshotItem := range result.Snapshot {
		row := &mdmlab.ScheduledQueryResultRow{
			QueryID:     queryID,
			HostID:      hostID,
			Data:        snapshotItem,
			LastFetched: fetchTime,
...
func getMostRecentResults(results []*mdmlab.ScheduledQueryResult) []*mdmlab.ScheduledQueryResult {
	// Use a map to track the most recent entry for each unique QueryName
	latestResults := make(map[string]*mdmlab.ScheduledQueryResult)
	...
	var filteredResults []*mdmlab.ScheduledQueryResult
...
//
// The expected format of query names managed by MDMlab is:
...
func getQueryNameAndTeamIDFromResult(path string) (*uint, string, error) {
	...
	if sep == "" {
		...
		if strings.HasPrefix(path, "pack/") && strings.Count(path, "/") == 2 {
			return nil, "", mdmlab.ErrLegacyQueryPack
		}
...
func (svc *Service) YaraRuleByName(ctx context.Context, name string) (*mdmlab.YaraRule, error) {
...
func getYaraEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
