...
func TestRedisLock(t *testing.T) {
	for _, f := range []func(*testing.T, mdmlab.Lock){
...
func setupRedis(t testing.TB, cluster, redir bool) mdmlab.Lock {
...
// only in tests.
func NewLockTest(t TestName, pool mdmlab.RedisPool) mdmlab.Lock {
	lock := &redisLock{
		pool:       pool,
		testPrefix: t.Name() + ":",
	}
	return mdmlab.Lock(lock)
}

func testRedisAcquireLock(t *testing.T, lock mdmlab.Lock) {
...
func testRedisSet(t *testing.T, lock mdmlab.Lock) {
	ctx := context.Background()
