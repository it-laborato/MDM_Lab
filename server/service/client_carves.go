...
func (c *Client) ListCarves(opt mdmlab.CarveListOptions) ([]*mdmlab.CarveMetadata, error) {
	endpoint := "/api/latest/mdmlab/carves"
	rawQuery := ""
	if opt.Expired {
		rawQuery = "expired=1"
	}
	...
	if err != nil {
		return nil, fmt.Errorf("GET /api/latest/mdmlab/carves: %w", err)
	}
...
func (c *Client) GetCarve(carveId int64) (*mdmlab.CarveMetadata, error) {
	endpoint := fmt.Sprintf("/api/latest/mdmlab/carves/%d", carveId)
...
func (c *Client) getCarveBlock(carveId, blockId int64) ([]byte, error) {
	path := fmt.Sprintf(
		"/api/latest/mdmlab/carves/%d/block/%d",
		carveId,
		blockId,
	)
...
func newCarveReader(carve mdmlab.CarveMetadata, client *Client) *carveReader {
	return &carveReader{
		carve:     carve,
		client:    client,
		bytesRead: 0,
...
func (c *Client) DownloadCarve(id int64) (io.Reader, error) {
	path := fmt.Sprintf("/api/latest/mdmlab/carves/%d", id)
	response, err := c.AuthenticatedDo("GET", path, "", nil)
	if err != nil {
		return nil, fmt.Errorf("GET %s: %w", path, err)
	}
