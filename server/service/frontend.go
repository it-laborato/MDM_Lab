...
func ServeFrontend(urlPrefix string, sandbox bool, logger log.Logger) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// See https://github.com/mdmlabdm/mdmlab/issues/16182.
		if r.Method == "POST" {
...
func ServeEndUserEnrollOTA(svc mdmlab.Service, urlPrefix string, logger log.Logger) http.Handler {
	herr := func(w http.ResponseWriter, err string) {
		logger.Log("err", err)
		http.Error(w, err, http.StatusInternalServerError)
	}
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if setupRequired {
			herr(w, "mdmlab instance not setup")
			return
		}
...
func generateEnrollOTAURL(mdmlabURL string, enrollSecret string) (string, error) {
	path, err := url.JoinPath(mdmlabURL, "/api/v1/mdmlab/enrollment_profiles/ota")
	if err != nil {
