...
//   - Noah: When MDM is turned on, install mdmlabd, bootstrap package (if DEP),
...
type mdmLifecycleAssertion[T any] func(t *testing.T, host *mdmlab.Host, device T)
...
func (s *integrationMDMTestSuite) TestTurnOnLifecycleEventsApple() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestAppleMDMClient) {
				s.Do(
					"POST",
					fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", host.ID),
					nil,
		...
		},
		{
			...
			func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestAppleMDMClient) {
				s.Do(
					"POST",
					fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", host.ID),
					nil,
					http.StatusNoContent,
				)
		...
		},
		{
			...
			func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestAppleMDMClient) {
				...
				s.Do(
					"DELETE",
					fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", host.ID),
					nil,
					http.StatusOK,
				)
		...
		},
		{
			...
			func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestAppleMDMClient) {
				...
				s.DoJSON(
					"DELETE",
					fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID),
					nil,
					http.StatusOK,
					&delResp,
				)
	...
	assertAction := func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestAppleMDMClient, action mdmLifecycleAssertion[*mdmtest.TestAppleMDMClient]) {
...
func (s *integrationMDMTestSuite) TestTurnOnLifecycleEventsWindows() {
	...
	testCases := []struct {
		...
		{
			"wiped host turns on MDM",
			func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestWindowsMDMClient) {
				s.Do(
					"POST",
					fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", host.ID),
					nil,
		...
		},
		{
			...
			func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestWindowsMDMClient) {
				...
				s.DoJSON(
					"POST",
					"/api/mdmlab/orbit/scripts/result",
					json.RawMessage(
						fmt.Sprintf(
	...
	assertAction := func(t *testing.T, host *mdmlab.Host, device *mdmtest.TestWindowsMDMClient, action mdmLifecycleAssertion[*mdmtest.TestWindowsMDMClient]) {
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			t.Run("programmatic enrollment", func(t *testing.T) {
				...
				err := s.ds.SetOrUpdateMDMData(context.Background(), host.ID, false, true, s.server.URL, false, mdmlab.WellKnownMDMMDMlab, "")
				require.NoError(t, err)
				assertAction(t, host, device, tt.Action)
			})
			...
			t.Run("automatic enrollment", func(t *testing.T) {
				...
				err := s.ds.ApplyEnrollSecrets(context.Background(), nil, []*mdmlab.EnrollSecret{{Secret: t.Name()}})
				require.NoError(t, err)
...
) ([]mdmlab.ProtoCmdOperation, getHostMDMSummaryResponse, getHostMDMResponseTest) {
	t := s.T()

	var recordedCmds []mdmlab.ProtoCmdOperation
	cmds, err := device.StartManagementSession()
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) setupLifecycleSettings() {
	...
	s.uploadBootstrapPackage(
		&mdmlab.MDMAppleBootstrapPackage{Bytes: bp, Name: "pkg.pkg", TeamID: 0},
		http.StatusOK,
		"",
	)
...
func (s *integrationMDMTestSuite) TestLifecycleSCEPCertExpiration() {
	...
	s.Do(
		"POST",
		"/api/v1/mdmlab/mdm/apple/profiles/batch",
	...
	// handle the case of a host being deleted, see https://github.com/mdmlabdm/mdmlab/issues/19149
	expireCerts()
	req := deleteHostsRequest{
