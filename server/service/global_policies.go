...
type globalPolicyResponse struct {
	Policy *mdmlab.Policy `json:"policy,omitempty"`
	Err    error         `json:"error,omitempty"`
}

func (r globalPolicyResponse) error() error { return r.Err }

func globalPolicyEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*globalPolicyRequest)
	resp, err := svc.NewGlobalPolicy(ctx, mdmlab.PolicyPayload{
		QueryID:     req.QueryID,
		Query:       req.Query,
		Name:        req.Name,
...
func (svc Service) NewGlobalPolicy(ctx context.Context, p mdmlab.PolicyPayload) (*mdmlab.Policy, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.Policy{}, mdmlab.ActionWrite); err != nil {
		return nil, err
	}
	...
	if err := svc.NewActivity(
		...
		mdmlab.ActivityTypeCreatedPolicy{
			ID:   policy.ID,
			Name: policy.Name,
		},
	); err != nil {
...
type listGlobalPoliciesRequest struct {
	Opts mdmlab.ListOptions `url:"list_options"`
}

type listGlobalPoliciesResponse struct {
	Policies []*mdmlab.Policy `json:"policies,omitempty"`
	Err      error           `json:"error,omitempty"`
}

func (r listGlobalPoliciesResponse) error() error { return r.Err }

func listGlobalPoliciesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc Service) ListGlobalPolicies(ctx context.Context, opts mdmlab.ListOptions) ([]*mdmlab.Policy, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.Policy{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
type getPolicyByIDResponse struct {
	Policy *mdmlab.Policy `json:"policy"`
	Err    error         `json:"error,omitempty"`
}

func (r getPolicyByIDResponse) error() error { return r.Err }

func getPolicyByIDEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc Service) GetPolicyByIDQueries(ctx context.Context, policyID uint) (*mdmlab.Policy, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.Policy{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func countGlobalPoliciesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func deleteGlobalPoliciesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc Service) DeleteGlobalPolicies(ctx context.Context, ids []uint) ([]uint, error) {
	...
	for _, id := range deletedIDs {
		if err := svc.NewActivity(
			ctx,
			authz.UserFromContext(ctx),
			mdmlab.ActivityTypeDeletedPolicy{
				ID:   id,
...
func modifyGlobalPolicyEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ModifyGlobalPolicy(ctx context.Context, id uint, p mdmlab.ModifyPolicyPayload) (*mdmlab.Policy, error) {
...
func resetAutomationEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ResetAutomation(ctx context.Context, teamIDs, policyIDs []uint) error {
	...
	for _, teamID := range teamIDs {
		p1, p2, err := svc.ds.ListTeamPolicies(ctx, teamID, mdmlab.ListOptions{}, mdmlab.ListOptions{})
		if err != nil {
			return err
		}
		for _, p := range p1 {
	...
	for id := range tIDs {
		if err := svc.authz.Authorize(ctx, &mdmlab.Team{ID: id}, mdmlab.ActionWrite); err != nil {
			return err
		}
...
func automationPolicies(wh mdmlab.FailingPoliciesWebhookSettings, ji []*mdmlab.JiraIntegration, zi []*mdmlab.ZendeskIntegration) map[uint]struct{} {
	enabled := wh.Enable
	for _, j := range ji {
...
func teamAutomationPolicies(wh mdmlab.FailingPoliciesWebhookSettings, ji []*mdmlab.TeamJiraIntegration, zi []*mdmlab.TeamZendeskIntegration) map[uint]struct{} {
	enabled := wh.Enable
	for _, j := range ji {
		if j.EnableFailingPolicies {
			enabled = true
...
type applyPolicySpecsRequest struct {
	Specs []*mdmlab.PolicySpec `json:"specs"`
}
...
func applyPolicySpecsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) checkPolicySpecAuthorization(ctx context.Context, policies []*mdmlab.PolicySpec) error {
	...
	for _, policy := range policies {
		if policy.Team != "" && policy.Team != "No team" {
			...
			if err := svc.authz.Authorize(ctx, &mdmlab.Policy{
...
func (svc *Service) ApplyPolicySpecs(ctx context.Context, policies []*mdmlab.PolicySpec) error {
	...
	if name := mdmlab.FirstDuplicatePolicySpecName(policies); name != "" {
		return ctxerr.Wrap(ctx, &mdmlab.BadRequestError{
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mdmlab.ActivityTypeAppliedSpecPolicy{
			Policies: policies,
		},
	); err != nil {
...
func autofillPoliciesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
var (
	getHumanInterpretationFromOsquerySqlUrl     = "https://mdmlabdm.com/api/v1/get-human-interpretation-from-osquery-sql"
...
func (svc *Service) AutofillPolicySql(ctx context.Context, sql string) (description string, resolution string, err error) {
	...
	if vc.User.GlobalRole != nil || len(vc.User.Teams) == 0 {
		...
		for _, teamUser := range vc.User.Teams {
			if teamUser.Role == mdmlab.RoleAdmin || teamUser.Role == mdmlab.RoleMaintainer || teamUser.Role == mdmlab.RoleGitOps {
				teamID = teamUser.Team.ID
				break
			}
		}
		err = svc.authz.Authorize(
			ctx, &mdmlab.Policy{PolicyData: mdmlab.PolicyData{TeamID: &teamID}}, mdmlab.ActionWrite,
		)
		if err != nil {
			return "", "", err
		}
	...
	if sql == "" {
		return "", "", ctxerr.Wrap(ctx, &mdmlab.BadRequestError{Message: "'sql' cannot be empty"})
	}

	// Using a timeout smaller than the MDMlab server's WriteTimeout
	client := mdmlabhttp.NewClient(mdmlabhttp.WithTimeout(getHumanInterpretationFromOsquerySqlTimeout))
