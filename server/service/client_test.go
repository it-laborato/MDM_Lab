  ...
  mdm:
    macos_settings:
      custom_settings:
        ...
        - path: "b"
`,
			[]mdmlab.MDMProfileSpec{{Path: "a", Labels: []string{"foo", "bar"}}, {Path: "b"}},
		},
		{
      ...
      custom_settings:
        ...
        - path: "c"
          labels:
            - baz
`,
			[]mdmlab.MDMProfileSpec{{Path: "a"}, {Path: "c", Labels: []string{"baz"}}},
		},
		{
  ...
  mdm:
    macos_settings:
      custom_settings:
        ...
        - "b"
`,
			[]mdmlab.MDMProfileSpec{{Path: "a"}, {Path: "b"}},
		},
		{
  ...
  mdm:
    macos_settings:
      custom_settings:
        ...
        - "c"
`,
			[]mdmlab.MDMProfileSpec{{Path: "a"}, {Path: "c"}},
		},
	}
	for _, c := range cases {
  ...
  team:
    ...
    mdm:
      ...
      windows_settings:
        custom_settings:
`,
			map[string]profileSpecsByPlatform{"MDMlab": {windows: []mdmlab.MDMProfileSpec{}, macos: []mdmlab.MDMProfileSpec{}}, "MDMlab2": {windows: []mdmlab.MDMProfileSpec{}, macos: []mdmlab.MDMProfileSpec{}}},
		},
		{
			"custom settings specified",
			`
        ...
        custom_settings:
           ...
           - path: "d"
             labels:
               ...
               - baz
`,
			map[string]profileSpecsByPlatform{"MDMlab": {
...
`,
			map[string]profileSpecsByPlatform{"MDMlab": {
				macos: []mdmlab.MDMProfileSpec{{Path: "a"}, {Path: "b"}},
				windows: []mdmlab.MDMProfileSpec{
					{Path: "c"},
					{Path: "d"},
				},
...
<Replace>
  <Item>
	...
	tests := []struct {
		...
		expandEnv     bool
		expectError   bool
		want          []mdmlab.MDMProfileBatchPayload
		wantErr       string
	}{
		{
			...
			expectError: true,
			want:        []mdmlab.MDMProfileBatchPayload{{Name: "foo"}},
		},
		{
  ...
  <Item>
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			macPaths := []mdmlab.MDMProfileSpec{}
			for _, fileSpec := range tt.macSetupFiles {
				...
				macPaths = append(macPaths, mdmlab.MDMProfileSpec{Path: filePath, Labels: tt.labels})
			}

			winPaths := []mdmlab.MDMProfileSpec{}
			for _, fileSpec := range tt.winSetupFiles {
				filePath := filepath.Join(tempDir, fileSpec[0])
				require.NoError(t, os.WriteFile(filePath, []byte(fileSpec[1]), 0o644))
				winPaths = append(winPaths, mdmlab.MDMProfileSpec{Path: filePath, Labels: tt.labels})
			}
...
func TestGitOpsErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			config.OrgSettings["secrets"] = []*mdmlab.EnrollSecret{}
