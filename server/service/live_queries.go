...
type runLiveQueryResponse struct {
	...
	Results []mdmlab.QueryCampaignResult `json:"live_query_results"`
}
...
type runOneLiveQueryResponse struct {
	...
	TargetedHostCount  int                 `json:"targeted_host_count"`
	RespondedHostCount int                 `json:"responded_host_count"`
	Results            []mdmlab.QueryResult `json:"results"`
	Err                error               `json:"error,omitempty"`
}
...
func runOneLiveQueryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	queryResults := []mdmlab.QueryResult{}
...
func runLiveQueryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func runLiveQueryOnHostEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func runLiveQueryOnHostByIDEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func runLiveQueryOnHost(svc mdmlab.Service, ctx context.Context, host *mdmlab.HostLite, query string) (errorer, error) {
...
func runLiveQuery(ctx context.Context, svc mdmlab.Service, queryIDs []uint, query string, hostIDs []uint) (
	[]mdmlab.QueryCampaignResult, int, error,
) {
	// The period used here should always be less than the request timeout for any load
	// balancer/proxy between MDMlab and the API client.
...
func (svc *Service) RunLiveQueryDeadline(
	...
	resultsCh := make(chan mdmlab.QueryCampaignResult)
	...
	for _, queryID := range queryIDs {
		...
		go func() {
			...
			campaign, err := svc.NewDistributedQueryCampaign(ctx, queryString, queryIDPtr, mdmlab.HostTargets{HostIDs: hostIDs})
			if err != nil {
				...
				resultsCh <- mdmlab.QueryCampaignResult{QueryID: queryID, Error: ptr.String(err.Error()), Err: err}
				return
			}
			...
			defer func() {
				...
				if err != nil {
					...
					resultsCh <- mdmlab.QueryCampaignResult{
						QueryID: queryID,
			...
			var results []mdmlab.QueryResult
		...
		loop:
			for {
				...
				case res := <-readChan:
					switch res := res.(type) {
					case mdmlab.DistributedQueryResult:
						results = append(results, mdmlab.QueryResult{HostID: res.Host.ID, Rows: res.Rows, Error: res.Error})
						counterMutex.Lock()
					...
					case error:
						resultsCh <- mdmlab.QueryCampaignResult{QueryID: queryID, Error: ptr.String(res.Error()), Err: res}
						return
					}
				case <-timeout:
					break loop
			...
			resultsCh <- mdmlab.QueryCampaignResult{QueryID: queryID, Results: results}
		}()
	}
	...
	var results []mdmlab.QueryCampaignResult
...
func (svc *Service) GetCampaignReader(ctx context.Context, campaign *mdmlab.DistributedQueryCampaign) (<-chan interface{}, context.CancelFunc, error) {
	...
	campaign.Status = mdmlab.QueryRunning
...
func (svc *Service) CompleteCampaign(ctx context.Context, campaign *mdmlab.DistributedQueryCampaign) error {
