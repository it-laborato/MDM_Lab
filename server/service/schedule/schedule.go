...
type Schedule struct {
	...
	jobs   []Job
	errors mdmlab.CronScheduleErrors
...
type CronStatsStore interface {
	...
	GetLatestCronStats(ctx context.Context, name string) ([]mdmlab.CronStats, error)
	// InsertCronStats inserts cron stats for the named cron schedule
	InsertCronStats(ctx context.Context, statsType mdmlab.CronStatsType, name string, instance string, status mdmlab.CronStatsStatus) (int, error)
	// UpdateCronStats updates the status of the identified cron stats record
	UpdateCronStats(ctx context.Context, id int, status mdmlab.CronStatsStatus, cronErrors *mdmlab.CronScheduleErrors) error
}
...
// is used. This affects when the schedule starts running after MDMlab is
// started, e.g. if the schedule has an interval of 1h and has no previous run
// recorded, by default its first run after MDMlab starts will be in 1h.
...
func New(
	...
	sch.errors = make(mdmlab.CronScheduleErrors)
	return sch
}
...
func (s *Schedule) Start() {
	...
	} else if s.runOnce && prevScheduledRun.Status == mdmlab.CronStatsStatusCompleted {
	...
	go func() {
		...
		for {
			...
			case <-s.trigger:
				...
				s.runWithStats(mdmlab.CronStatsTypeTriggered)
			...
			case <-schedTicker.C:
				...
				if prevScheduledRun.Status == mdmlab.CronStatsStatusPending || prevTriggeredRun.Status == mdmlab.CronStatsStatusPending {
					// skip ahead to the next interval
				...
				s.runWithStats(mdmlab.CronStatsTypeScheduled)
...
func (s *Schedule) Trigger() (*mdmlab.CronStats, error) {
	sched, trig, err := s.GetLatestStats()
	switch {
	case err != nil:
		return nil, err
	case sched.Status == mdmlab.CronStatsStatusPending:
		return &sched, nil
	case trig.Status == mdmlab.CronStatsStatusPending:
		return &trig, nil
	default:
		// ok
	}
...
func (s *Schedule) runWithStats(statsType mdmlab.CronStatsType) {
	statsID, err := s.insertStats(statsType, mdmlab.CronStatsStatusPending)
	if err != nil {
	...
	if err := s.updateStats(statsID, mdmlab.CronStatsStatusCompleted); err != nil {
...
func (s *Schedule) GetLatestStats() (mdmlab.CronStats, mdmlab.CronStats, error) {
	var scheduled, triggered mdmlab.CronStats

	cs, err := s.statsStore.GetLatestCronStats(s.ctx, s.name)
	if err != nil {
		return mdmlab.CronStats{}, mdmlab.CronStats{}, err
	}
	if len(cs) > 2 {
		return mdmlab.CronStats{}, mdmlab.CronStats{}, fmt.Errorf("get latest stats expected length to be no more than two but got length: %d", len(cs))
	}

	for _, stats := range cs {
		switch stats.StatsType {
		case mdmlab.CronStatsTypeScheduled:
			scheduled = stats
		case mdmlab.CronStatsTypeTriggered:
			triggered = stats
		default:
...
func (s *Schedule) insertStats(statsType mdmlab.CronStatsType, status mdmlab.CronStatsStatus) (int, error) {
	return s.statsStore.InsertCronStats(s.ctx, statsType, s.name, s.instanceID, status)
}

func (s *Schedule) updateStats(id int, status mdmlab.CronStatsStatus) error {
