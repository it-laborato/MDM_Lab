...
func (NopStatsStore) GetLatestCronStats(ctx context.Context, name string) ([]mdmlab.CronStats, error) {
	return []mdmlab.CronStats{}, nil
}

func (NopStatsStore) InsertCronStats(ctx context.Context, statsType mdmlab.CronStatsType, name string, instance string, status mdmlab.CronStatsStatus) (int, error) {
	return 0, nil
}

func (NopStatsStore) UpdateCronStats(ctx context.Context, id int, status mdmlab.CronStatsStatus, cronErrors *mdmlab.CronScheduleErrors) error {
	return nil
}
...
type MockStatsStore struct {
	sync.Mutex
	stats map[int]mdmlab.CronStats
...
func (m *MockStatsStore) GetLatestCronStats(ctx context.Context, name string) ([]mdmlab.CronStats, error) {
	m.Lock()
	defer m.Unlock()
	...
	latest := make(map[mdmlab.CronStatsType]mdmlab.CronStats)
	...
	res := []mdmlab.CronStats{}
	if s, ok := latest[mdmlab.CronStatsTypeScheduled]; ok {
		res = append(res, s)
	}
	if s, ok := latest[mdmlab.CronStatsTypeTriggered]; ok {
		res = append(res, s)
	}
...
func (m *MockStatsStore) InsertCronStats(ctx context.Context, statsType mdmlab.CronStatsType, name string, instance string, status mdmlab.CronStatsStatus) (int, error) {
	m.Lock()
	defer m.Unlock()
	...
	id := len(m.stats) + 1
	m.stats[id] = mdmlab.CronStats{ID: id, StatsType: statsType, Name: name, Instance: instance, Status: status, CreatedAt: time.Now().Truncate(1 * time.Second), UpdatedAt: time.Now().Truncate(time.Second)}

	return id, nil
}

func (m *MockStatsStore) UpdateCronStats(ctx context.Context, id int, status mdmlab.CronStatsStatus, cronErrors *mdmlab.CronScheduleErrors) error {
	m.Lock()
	defer m.Unlock()
...
func SetUpMockStatsStore(name string, initialStats ...mdmlab.CronStats) *MockStatsStore {
	stats := make(map[int]mdmlab.CronStats)
