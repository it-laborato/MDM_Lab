...
var reVersion = regexp.MustCompile(`\{mdmlabversion:\(\?:([^\}\)]+)\)\}`)

func makeStreamDistributedQueryCampaignResultsHandler(config config.ServerConfig, svc mdmlab.Service, logger kitlog.Logger) func(string) http.Handler {
	...
	return func(path string) http.Handler {
		...
		if len(matches) == 0 {
			panic("unexpected path, could not expand mdmlabversion: " + path)
		}
