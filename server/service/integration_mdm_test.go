...
import (
	...
	"github.com:it-laborato/MDM_Lab/pkg/mdmlabdbase"
	...
	servermdm "github.com:it-laborato/MDM_Lab/server/mdm"
	...
	nanomdm_pushsvc "github.com:it-laborato/MDM_Lab/server/mdm/nanomdm/push/service"
...
type integrationMDMTestSuite struct {
	...
	mdmlabDMNextCSRStatus       atomic.Value
	...
	mockedDownloadMDMlabdmMeta mdmlabdbase.Metadata
}
...
func (s *integrationMDMTestSuite) SetupSuite() {
	...
	var softwareInstallerStore mdmlab.SoftwareInstallerStore
	var bootstrapPackageStore mdmlab.MDMBootstrapPackageStore
		...
		StartCronSchedules: []TestNewScheduleFunc{
			func(ctx context.Context, ds mdmlab.Datastore) mdmlab.NewCronScheduleFunc {
				return func() (mdmlab.CronSchedule, error) {
					const name = string(mdmlab.CronMDMAppleProfileManager)
					logger := cronLog
			...
			func(ctx context.Context, ds mdmlab.Datastore) mdmlab.NewCronScheduleFunc {
				return func() (mdmlab.CronSchedule, error) {
					const name = string(mdmlab.CronWorkerIntegrations)
					logger := cronLog
			...
			func(ctx context.Context, ds mdmlab.Datastore) mdmlab.NewCronScheduleFunc {
				return func() (mdmlab.CronSchedule, error) {
					const name = string(mdmlab.CronAppleMDMIPhoneIPadRefetcher)
					logger := cronLog
	...
	err = s.ds.InsertMDMConfigAssets(context.Background(), []mdmlab.MDMConfigAsset{
		{Name: mdmlab.MDMAssetSCEPChallenge, Value: []byte(s.scepChallenge)},
	}, nil)
	require.NoError(s.T(), err)
	...
	mdmlabdmSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		status := s.mdmlabDMNextCSRStatus.Swap(http.StatusOK)
	...
	s.T().Setenv("TEST_FLEETDM_API_URL", mdmlabdmSrv.URL)
	s.T().Setenv("FLEET_DEV_ITUNES_URL", s.appleITunesSrv.URL)

	s.mockedDownloadMDMlabdmMeta = mdmlabdbase.Metadata{
		MSIURL:           fmt.Sprintf("https://download-testing.mdmlabdm.com/archive/stable/%s/mdmlabd-base.msi", uuid.NewString()),
		MSISha256:        uuid.NewString(),
		PKGURL:           fmt.Sprintf("https://download-testing.mdmlabdm.com/archive/stable/%s/mdmlabd-base.pkg", uuid.NewString()),
		PKGSha256:        uuid.NewString(),
		ManifestPlistURL: fmt.Sprintf("https://download-testing.mdmlabdm.com/archive/stable/%s/mdmlabd-base-manifest.plist", uuid.NewString()),
		Version:          "2024-06-25_03-01-17",
	}
	downloadMDMlabdmSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/stable/meta.json" {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			require.NoError(s.T(), json.NewEncoder(w).Encode(s.mockedDownloadMDMlabdmMeta))
		}
	}))
	s.T().Setenv("FLEET_DEV_DOWNLOAD_FLEETDM_URL", downloadMDMlabdmSrv.URL)
	s.T().Cleanup(downloadMDMlabdmSrv.Close)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mdmlab.EnrollSecretSpec{
			Secrets: []*mdmlab.EnrollSecret{{Secret: "global-secret"}},
		},
	}, http.StatusOK, &applyResp)

	s.T().Cleanup(mdmlabdmSrv.Close)
...
func (s *integrationMDMTestSuite) FailNextCSRRequestWith(status int) {
	s.mdmlabDMNextCSRStatus.Store(status)
}

func (s *integrationMDMTestSuite) SucceedNextCSRRequest() {
	s.mdmlabDMNextCSRStatus.Store(http.StatusOK)
}
...
func (s *integrationMDMTestSuite) TearDownTest() {
	...
	appCfg.MDM.WindowsUpdates = mdmlab.WindowsUpdates{}
	...
	// deleting the mdmlab-specific ones.
...
func (s *integrationMDMTestSuite) TestGetBootstrapToken() {
	...
	t.Run("no device record", func(t *testing.T) {
		...
		// if not found, server returns empty body and no error (see https://github.com/mdmlabdm/nanomdm/pull/8)
		res, err := mdmDevice.GetBootstrapToken()
		require.NoError(t, err)
		require.Nil(t, res)
	})
...
func (s *integrationMDMTestSuite) TestAppleGetAppleMDM() {
	...
	require.Equal(t, mdmlab.TeamNameNoTeam, tok.MacOSTeam.Name)
	require.Equal(t, mdmlab.TeamNameNoTeam, tok.IOSTeam.Name)
	require.Equal(t, mdmlab.TeamNameNoTeam, tok.IPadOSTeam.Name)
	...
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(fmt.Sprintf(`{
		"mdm": {
...
func (s *integrationMDMTestSuite) getABMTokenByName(orgName string, tokens []*mdmlab.ABMToken) *mdmlab.ABMToken {
...
func (s *integrationMDMTestSuite) TestABMExpiredToken() {
	...
	ctx := context.Background()
	mdmlabSyncer := apple_mdm.NewDEPService(s.ds, s.depStorage, s.logger)

	// not signed error flips the AppleBMTermsExpired flag
	returnType = "not_signed"
	err := mdmlabSyncer.RunAssigner(ctx)
...
func setupExpectedMDMlabdProfile(t *testing.T, serverURL string, enrollSecret string, teamID *uint) []byte {
	var b bytes.Buffer
	params := mobileconfig.MDMlabdProfileOptions{
		EnrollSecret: enrollSecret,
		ServerURL:    serverURL,
		PayloadType:  mobileconfig.MDMlabdConfigPayloadIdentifier,
		PayloadName:  servermdm.MDMlabdConfigProfileName,
	}
	err := mobileconfig.MDMlabdProfileTemplate.Execute(&b, params)
	require.NoError(t, err)
	return b.Bytes()
}

func setupExpectedCAProfile(t *testing.T, ds *mysql.Datastore) []byte {
	assets, err := ds.GetAllMDMConfigAssetsByName(context.Background(), []mdmlab.MDMAssetName{
		mdmlab.MDMAssetCACert,
	}, nil)
	require.NoError(t, err)

	block, _ := pem.Decode(assets[mdmlab.MDMAssetCACert].Value)
	require.NotNil(t, block)
	require.Equal(t, "CERTIFICATE", block.Type)

	var b bytes.Buffer
	params := mobileconfig.MDMlabCARootTemplateOptions{
		PayloadName:       servermdm.MDMlabCAConfigProfileName,
		PayloadIdentifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier,
		Certificate:       base64.StdEncoding.EncodeToString(block.Bytes),
	}
	err = mobileconfig.MDMlabCARootTemplate.Execute(&b, params)
	require.NoError(t, err)
	return b.Bytes()
}
...
func createHostThenEnrollMDM(ds mdmlab.Datastore, mdmlabServerURL string, t *testing.T) (*mdmlab.Host, *mdmtest.TestAppleMDMClient) {
	desktopToken := uuid.New().String()
	mdmDevice := mdmtest.NewTestMDMClientAppleDesktopManual(mdmlabServerURL, desktopToken)
	mdmlabHost, err := ds.NewHost(context.Background(), &mdmlab.Host{
	...
	err = ds.SetOrUpdateDeviceAuthToken(context.Background(), mdmlabHost.ID, desktopToken)
	require.NoError(t, err)

	err = mdmDevice.Enroll()
	require.NoError(t, err)

	return mdmlabHost, mdmDevice
}

func (s *integrationMDMTestSuite) createAppleMobileHostThenEnrollMDM(platform string) (*mdmlab.Host, *mdmtest.TestAppleMDMClient) {
	ctx := context.Background()
	t := s.T()
	...
	mdmlabHost, err := s.ds.NewHost(ctx, &mdmlab.Host{
	...
	require.NoError(t, err)
	require.Equal(t, dbZeroTime, mdmlabHost.LastEnrolledAt)
...
func createWindowsHostThenEnrollMDM(ds mdmlab.Datastore, mdmlabServerURL string, t *testing.T) (*mdmlab.Host, *mdmtest.TestWindowsMDMClient) {
	host := createOrbitEnrolledHost(t, "windows", "h1", ds)
	mdmDevice := mdmtest.NewTestMDMClientWindowsProgramatic(mdmlabServerURL, *host.OrbitNodeKey)
	err := mdmDevice.Enroll()
	require.NoError(t, err)
	err = ds.UpdateMDMWindowsEnrollmentsHostUUID(context.Background(), host.UUID, mdmDevice.DeviceID)
	require.NoError(t, err)
	err = ds.SetOrUpdateMDMData(context.Background(), host.ID, false, true, mdmlabServerURL, false, mdmlab.WellKnownMDMMDMlab, "")
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestDeviceMDMManualEnroll() {
	...
	// invalid token fails
	s.DoRaw("GET", "/api/latest/mdmlab/device/invalid_token/mdm/apple/manual_enrollment_profile", nil, http.StatusUnauthorized)

	// valid token downloads the profile
	s.downloadAndVerifyOTAEnrollmentProfile("/api/latest/mdmlab/device/" + token + "/mdm/apple/manual_enrollment_profile")
}
...
func (s *integrationMDMTestSuite) TestAppleMDMDeviceEnrollment() {
	...
	require.NoError(t, err)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeMDMEnrolled{}.ActivityName(),
	...
	// Find the ID of MDMlab's MDM solution
	var mdmID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &mdmID,
			`SELECT id FROM mobile_device_management_solutions WHERE name = ?`, mdmlab.WellKnownMDMMDMlab)
	})

	// Check that both devices are returned by the /hosts endpoint
	listHostsRes := listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &listHostsRes, "mdm_id", fmt.Sprint(mdmID))
	...
	// query all hosts
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &listHostsRes)
	// we still have only two hosts
	require.Len(t, listHostsRes.Hosts, 2)

	// LastEnrolledAt should have been updated
	var getHostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", targetHostID), nil, http.StatusOK, &getHostResp)
...
func (s *integrationMDMTestSuite) TestAppleMDMCSRRequest() {
	...
	// missing arguments
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/request_csr", requestMDMAppleCSRRequest{}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Equal(t, errResp.Errors[0].Name, "email_address")

	// invalid email address
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "abc", Organization: "def"}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Equal(t, errResp.Errors[0].Name, "email_address")

	// missing organization
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: ""}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Equal(t, errResp.Errors[0].Name, "organization")

	// mdmlabdm CSR request failed
	s.FailNextCSRRequestWith(http.StatusBadRequest)
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	...
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusBadGateway, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Contains(t, errResp.Errors[0].Reason, "MDMlabDM CSR request failed")

	var reqCSRResp requestMDMAppleCSRResponse
	// mdmlabdm CSR request succeeds
...
func (s *integrationMDMTestSuite) TestGetMDMCSR() {
	...
	s.uploadDataViaForm("/api/latest/mdmlab/mdm/apple/apns_certificate", "certificate", "certificate.pem", []byte("-----BEGIN CERTIFICATE-----\nZm9vCg==\n-----END CERTIFICATE-----"), http.StatusInternalServerError, "Couldn't upload APNs certificate. Missing required private key. Learn how to configure the private key here: https://mdmlabdm.com/learn-more-about/mdmlab-server-private-key", nil)

	r := s.Do("GET", "/api/latest/mdmlab/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusInternalServerError)
	require.Contains(t, extractServerErrorText(r.Body), "Couldn't download signed CSR. Missing required private key. Learn how to configure the private key here: https://mdmlabdm.com/learn-more-about/mdmlab-server-private-key")
	...
	s.Do("DELETE", "/api/latest/mdmlab/mdm/apple/apns_certificate", nil, http.StatusOK)

	assets, err := s.ds.GetAllMDMConfigAssetsByName(ctx,
		[]mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey, mdmlab.MDMAssetAPNSKey, mdmlab.MDMAssetAPNSCert}, nil)
	...
	s.FailNextCSRRequestWith(http.StatusInternalServerError)
	errResp := validationErrResp{}
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusBadGateway, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Contains(t, errResp.Errors[0].Reason, "MDMlabDM CSR request failed")
	...
	errResp = validationErrResp{}
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Contains(t, errResp.Errors[0].Reason, "this email address is not valid")

	// Invalid APNS cert upload attempt
	s.uploadDataViaForm("/api/latest/mdmlab/mdm/apple/apns_certificate", "certificate", "certificate.pem", []byte("invalid-cert"), http.StatusUnprocessableEntity, "Invalid certificate. Please provide a valid certificate from Apple Push Certificate Portal.", nil)
...
func (s *integrationMDMTestSuite) TestMDMAppleUnenroll() {
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/%d", h.ID), getHostRequest{}, http.StatusOK, &hostResp)
	// 3 profiles added + 1 profile with mdmlabd configuration + 1 root CA config
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", h.ID), nil, http.StatusOK)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", h.ID), nil, http.StatusBadGateway)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", h.ID), nil, http.StatusInternalServerError)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", h.ID), nil, http.StatusConflict)
...
func (s *integrationMDMTestSuite) TestMDMDiskEncryptionSettingBackwardsCompat() {
	...
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabFileVaultPayloadIdentifier, false)
	...
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabFileVaultPayloadIdentifier, true)
	...
	// new config takes precedence over old config
	teamSpecs := applyTeamSpecsRequest{Specs: []*mdmlab.TeamSpec{{
		Name: team.Name,
	...
	checkTeamDiskEncryption(false)
	s.assertConfigProfilesByIdentifier(ptr.Uint(team.ID), mobileconfig.MDMlabFileVaultPayloadIdentifier, false)
	...
	s.assertConfigProfilesByIdentifier(ptr.Uint(team.ID), mobileconfig.MDMlabFileVaultPayloadIdentifier, true)
...
func (s *integrationMDMTestSuite) TestDiskEncryptionSharedSetting() {
	...
	checkConfigSetSucceeds := func() {
		...
		res = s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK)
		errMsg = extractServerErrorText(res.Body)
		require.Empty(t, errMsg)
		...
		s.Do("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{
		"mdm": { "enable_disk_encryption": false }
  }`), http.StatusOK)
		teamSpecs = map[string]any{
			"specs": []any{
...
func (s *integrationMDMTestSuite) TestEscrowBuddyBackwardsCompat() {
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, &orbitConfigResp)
	...
	orbitConfigResp = orbitGetConfigResponse{}
	res := s.DoRawWithHeaders("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, map[string]string{
		"Authorization":          fmt.Sprintf("Bearer %s", s.token),
		mdmlab.CapabilitiesHeader: string(mdmlab.CapabilityEscrowBuddy),
	})
...
func (s *integrationMDMTestSuite) TestMDMAppleHostDiskEncryption() {
	...
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	fileVaultProf := s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabFileVaultPayloadIdentifier, true)
	hostCmdUUID := uuid.New().String()
	err = s.ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{
		{
	...
	t.Cleanup(func() {
		err := s.ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mdmlab.HostMDMAppleProfile{
			HostUUID:      host.UUID,
			CommandUUID:   hostCmdUUID,
			ProfileUUID:   fileVaultProf.ProfileUUID,
	...
	require.Equal(t, mdmlab.DiskEncryptionEnforcing, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	...
	require.Equal(t, mdmlab.DiskEncryptionEnforcing, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mdmlab.DiskEncryptionFailed, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Nil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionFailed, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mdmlab.DiskEncryptionActionRequired, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.Equal(t, mdmlab.ActionRequiredRotateKey, *getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionActionRequired, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", getHostResp.Host.MDM.OSSettings.DiskEncryption.Detail)

	// add an encryption key for the host
	assets, err := s.ds.GetAllMDMConfigAssetsByName(ctx, []mdmlab.MDMAssetName{
		mdmlab.MDMAssetCACert,
	}, nil)
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mdmlab.DiskEncryptionVerifying, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Nil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionVerifying, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", getHostResp.Host.MDM.OSSettings.DiskEncryption.Detail)

	// request with no token
	res := s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/encryption_key", host.ID), nil, http.StatusUnauthorized)
	res.Body.Close()

	// encryption key not processed yet
	resp := getHostEncryptionKeyResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/encryption_key", host.ID), nil, http.StatusNotFound, &resp)
	...
	// decryptable key
	checkDecryptableKey := func(u mdmlab.User) {
		err = s.ds.SetHostsDiskEncryptionKeyStatus(ctx, []uint{host.ID}, true, time.Now())
		require.NoError(t, err)
		resp = getHostEncryptionKeyResponse{}
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/encryption_key", host.ID), nil, http.StatusOK, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mdmlab.DiskEncryptionVerified, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Nil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionVerified, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", getHostResp.Host.MDM.OSSettings.DiskEncryption.Detail)
	...
	s.token = s.getTestToken(u.Email, test.GoodPassword)
	resp = getHostEncryptionKeyResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/encryption_key", host.ID), nil, http.StatusForbidden, &resp)
}
...
func (s *integrationMDMTestSuite) TestWindowsMDMGetEncryptionKey() {
	...
	// create a host and enroll it in MDMlab
	host := createOrbitEnrolledHost(t, "windows", "h1", s.ds)
	err := s.ds.SetOrUpdateMDMData(ctx, host.ID, false, true, s.server.URL, false, mdmlab.WellKnownMDMMDMlab, "")
	require.NoError(t, err)
	...
	// invalid host id
	resp = getHostEncryptionKeyResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/encryption_key", host.ID+999), nil, http.StatusNotFound, &resp)

	// add an encryption key for the host
	cert, _, _, err := s.mdmlabCfg.MDM.MicrosoftWSTEP()
	require.NoError(t, err)
	recoveryKey := "AAA-BBB-CCC"
	...
	require.Equal(t, recoveryKey, resp.EncryptionKey.DecryptedValue)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeReadHostDiskEncryptionKey{}.ActivityName(),
...
func (s *integrationMDMTestSuite) TestAppConfigMDMAppleDiskEncryption() {
	...
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	enabledDiskActID := s.lastActivityMatches(mdmlab.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
	...
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	assert.Equal(t, []mdmlab.MDMProfileSpec{{Path: "a"}}, acResp.MDM.MacOSSettings.CustomSettings)
	s.lastActivityMatches(mdmlab.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, enabledDiskActID)
	...
	assert.False(t, acResp.MDM.EnableDiskEncryption.Value)
	assert.Equal(t, []mdmlab.MDMProfileSpec{{Path: "b"}}, acResp.MDM.MacOSSettings.CustomSettings)
	s.lastActivityMatches(mdmlab.ActivityTypeDisabledMacosDiskEncryption{}.ActivityName(),
		`{"team_id": null, "team_name": null}`, 0)
	...
	s.Do("POST", "/api/latest/mdmlab/disk_encryption",
	...
	s.Do("PATCH", "/api/latest/mdmlab/mdm/apple/settings",
		mdmlab.MDMAppleSettingsPayload{}, http.StatusNoContent)
	s.lastActivityMatches(mdmlab.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, enabledDiskActID)
...
func (s *integrationMDMTestSuite) TestMDMAppleDiskEncryptionAggregate() {
	...
	expectedNoTeamDiskEncryptionSummary := mdmlab.MDMDiskEncryptionSummary{}
	s.checkMDMDiskEncryptionSummaries(t, nil, expectedNoTeamDiskEncryptionSummary, true)
	expectedNoTeamProfilesSummary := mdmlab.MDMProfilesSummary{}
	...
	prof, err := mdmlab.NewMDMAppleConfigProfile(mobileconfigForTest("filevault-1", mobileconfig.MDMlabFileVaultPayloadIdentifier), ptr.Uint(0))
	require.NoError(t, err)

	// generates a disk encryption aggregate value based on the arguments passed in
	generateAggregateValue := func(
		hosts []*mdmlab.Host,
		operationType mdmlab.MDMOperationType,
		status *mdmlab.MDMDeliveryStatus,
		decryptable bool,
	) {
	...
	generateAggregateValue(hosts[0:2], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, true)
	...
	generateAggregateValue(hosts[2:4], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryVerifying, false)
	...
	generateAggregateValue(hosts[4:6], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryPending, true)
	...
	generateAggregateValue(hosts[4:6], mdmlab.MDMOperationTypeInstall, nil, true)
	...
	generateAggregateValue(hosts[6:8], mdmlab.MDMOperationTypeInstall, &mdmlab.MDMDeliveryFailed, true)
	...
	generateAggregateValue(hosts[8:10], mdmlab.MDMOperationTypeRemove, &mdmlab.MDMDeliveryPending, true)
	...
	tm, _ := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team-1"})
	err = s.ds.AddHostsToTeam(ctx, &tm.ID, []uint{hosts[0].ID, hosts[1].ID})
	require.NoError(t, err)

	// new filevault profile for team 1
	prof, err = mdmlab.NewMDMAppleConfigProfile(mobileconfigForTest("filevault-1", mobileconfig.MDMlabFileVaultPayloadIdentifier), ptr.Uint(1))
	require.NoError(t, err)
	prof.TeamID = &tm.ID
	require.NoError(t, err)
	...
	var expectedTeamDiskEncryptionSummary mdmlab.MDMDiskEncryptionSummary
	...
	expectedTeamProfilesSummary := mdmlab.MDMProfilesSummary{Verifying: 2}
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, s.ds, hosts[0], map[string]*mdmlab.HostMacOSProfile{prof.Identifier: {Identifier: prof.Identifier, DisplayName: prof.Name, InstallDate: time.Now()}}))
...
func (s *integrationMDMTestSuite) TestTeamsMDMAppleDiskEncryption() {
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK)
	lastDiskActID := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
	...
	res := s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: nil},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
	require.True(t, teamResp.Team.Config.MDM.EnableDiskEncryption)
	require.Equal(t, []mdmlab.MDMProfileSpec{{Path: "a"}}, teamResp.Team.Config.MDM.MacOSSettings.CustomSettings)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, lastDiskActID)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeDisabledMacosDiskEncryption{}.ActivityName(),
		fmt.Sprintf(`{"team_id": %d, "team_name": %q}`, team.ID, teamName), 0)
	...
	s.Do("PATCH", "/api/latest/mdmlab/mdm/apple/settings",
		mdmlab.MDMAppleSettingsPayload{TeamID: ptr.Uint(team.ID)}, http.StatusNoContent)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, lastDiskActID)
...
func (s *integrationMDMTestSuite) TestEnrollOrbitAfterDEPSync() {
	...
	var hostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", h.ID), nil, http.StatusOK, &hostResp)
...
func (s *integrationMDMTestSuite) TestMDMlabdConfiguration() {
	t := s.T()
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabdConfigPayloadIdentifier, false)
	...
	// a new mdmlabd configuration profile for "no team" is created
	s.awaitTriggerProfileSchedule(t)
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabdConfigPayloadIdentifier, true)
	...
	require.NoError(t, err)
	s.assertConfigProfilesByIdentifier(&tm.ID, mobileconfig.MDMlabdConfigPayloadIdentifier, false)
	...
	s.awaitTriggerProfileSchedule(t)
	p := s.assertConfigProfilesByIdentifier(&tm.ID, mobileconfig.MDMlabdConfigPayloadIdentifier, true)
	require.Contains(t, string(p.Mobileconfig), t.Name())

	// create an enroll secret for the team
	teamSpecs := applyTeamSpecsRequest{Specs: []*mdmlab.TeamSpec{{
		Name:    tm.Name,
		Secrets: &[]mdmlab.EnrollSecret{{Secret: t.Name() + "team-secret"}},
	}}}
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK)

	// a new mdmlabd configuration profile for that team is created
	s.awaitTriggerProfileSchedule(t)
	p = s.assertConfigProfilesByIdentifier(&tm.ID, mobileconfig.MDMlabdConfigPayloadIdentifier, true)
...
func (s *integrationMDMTestSuite) TestEnqueueMDMCommand() {
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/commands", nil, http.StatusOK, &listCmdResp0)
...
<dict>
	...
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/enqueue",
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/commandresults", nil, http.StatusNotFound, &cmdResResp, "command_uuid", uuid1)
	var getMDMCmdResp getMDMCommandResultsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/commands/results", nil, http.StatusNotFound, &cmdResResp, "command_uuid", uuid1)

	// list commands returns empty set
	var listCmdResp listMDMAppleCommandsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/commands", nil, http.StatusOK, &listCmdResp)
	require.Empty(t, listCmdResp.Results)

	// call with unenrolled host UUID
	res := s.Do("POST", "/api/latest/mdmlab/mdm/apple/enqueue",
		enqueueMDMAppleCommandRequest{
	...
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/enqueue",
		enqueueMDMAppleCommandRequest{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/commandresults", nil, http.StatusOK, &cmdResResp, "command_uuid", uuid2)
	require.Len(t, cmdResResp.Results, 0)
	s.DoJSON("GET", "/api/latest/mdmlab/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", uuid2)
	...
	require.Equal(t, &mdmlab.MDMCommandResult{
	...
	require.NoError(t, err)
	t.Logf("GET /api/latest/mdmlab/mdm/apple/commands response:\n%s", results)
	...
	var profileListCommands []*mdmlab.MDMAppleCommand
	...
	require.Equal(t, &mdmlab.MDMAppleCommand{
...
func (s *integrationMDMTestSuite) TestEnqueueMDMCommandWithSecret() {
	...
	// list commands should return all the commands we sent
	var listCmdResp listMDMAppleCommandsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/commands", nil, http.StatusOK, &listCmdResp, "host_identifier", mdmClient.UUID)
	require.Empty(t, listCmdResp.Results)
...
func (s *integrationMDMTestSuite) TestMDMWindowsCommandResults() {
	...
	dev := &mdmlab.MDMWindowsEnrolledDevice{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/commands/results?command_uuid=%s", cmdUUID), nil, http.StatusOK, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/commands/results?command_uuid=%s", uuid.New().String()), nil, http.StatusNotFound, &resp)
...
func (s *integrationMDMTestSuite) TestAppConfigMDMMacOSMigration() {
	...
	checkDefaultAppConfig := func() {
		...
		s.DoJSON("GET", "/api/v1/mdmlab/config", nil, http.StatusOK, &ac)
	...
	s.DoJSON("GET", "/api/v1/mdmlab/config", nil, http.StatusOK, &acResp)
	require.True(t, acResp.MDM.MacOSMigration.Enable)
	require.Equal(t, mdmlab.MacOSMigrationModeVoluntary, acResp.MDM.MacOSMigration.Mode)
...
func (s *integrationMDMTestSuite) TestBootstrapPackage() {
	...
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{}, http.StatusBadRequest, "package multipart field is required")
	// no name
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: signedPkg}, http.StatusBadRequest, "package multipart field is required")
	// invalid
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: invalidPkg, Name: "invalid.tar.gz"}, http.StatusBadRequest, "invalid file type")
	// invalid names
	for _, char := range file.InvalidMacOSChars {
		s.uploadBootstrapPackage(
			&mdmlab.MDMAppleBootstrapPackage{
				Bytes: signedPkg,
				Name:  fmt.Sprintf("invalid_%c_name.pkg", char),
			}, http.StatusBadRequest, "")
	}
	// unsigned
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: unsignedPkg, Name: "pkg.pkg"}, http.StatusBadRequest, "file is not signed")
	// wrong TOC
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: wrongTOCPkg, Name: "pkg.pkg"}, http.StatusBadRequest, "invalid package")
	// successfully upload a package
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: signedPkg, Name: "pkg.pkg", TeamID: 0}, http.StatusOK, "")
	// check the activity log
	s.lastActivityMatches(
		mdmlab.ActivityTypeAddedBootstrapPackage{}.ActivityName(),
	...
	s.DoJSON("GET", "/api/latest/mdmlab/bootstrap/0/metadata", nil, http.StatusOK, &metadataResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/bootstrap?token=bad", nil, http.StatusNotFound, &downloadResp)

	resp := s.DoRaw("GET", fmt.Sprintf("/api/latest/mdmlab/bootstrap?token=%s", metadataResp.MDMAppleBootstrapPackage.Token), nil, http.StatusOK)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/bootstrap/1/metadata", nil, http.StatusNotFound, &metadataResp)

	// delete package
	var deleteResp deleteBootstrapPackageResponse
	s.DoJSON("DELETE", "/api/latest/mdmlab/bootstrap/0", nil, http.StatusOK, &deleteResp)
	// check the activity log
	s.lastActivityMatches(
		mdmlab.ActivityTypeDeletedBootstrapPackage{}.ActivityName(),
		`{"bootstrap_package_name": "pkg.pkg", "team_id": null, "team_name": null}`,
		0,
	)

	metadataResp = bootstrapPackageMetadataResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/bootstrap/0/metadata", nil, http.StatusNotFound, &metadataResp)
	// trying to delete again is a bad request
	s.DoJSON("DELETE", "/api/latest/mdmlab/bootstrap/0", nil, http.StatusNotFound, &deleteResp)
}
...
func (s *integrationMDMTestSuite) TestBootstrapPackageStatus() {
	...
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: pkg, Name: "pkg.pkg", TeamID: 0}, http.StatusOK, "")
	...
	s.uploadBootstrapPackage(&mdmlab.MDMAppleBootstrapPackage{Bytes: pkg, Name: "pkg.pkg", TeamID: team.ID}, http.StatusOK, "")

	// get package metadata
	metadataResp = bootstrapPackageMetadataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/bootstrap/%d/metadata", team.ID), nil, http.StatusOK, &metadataResp)
	...
	// will be sent by the device, which should in turn be interpreted by MDMlab as "Pending"). See
	...
	expectedSerialsByTeamAndStatus := make(map[uint]map[mdmlab.MDMBootstrapPackageStatus][]string)
	expectedSerialsByTeamAndStatus[0] = map[mdmlab.MDMBootstrapPackageStatus][]string{
		mdmlab.MDMBootstrapPackageInstalled: {noTeamDevices[0].device.SerialNumber, noTeamDevices[1].device.SerialNumber, noTeamDevices[2].device.SerialNumber},
		mdmlab.MDMBootstrapPackageFailed:    {noTeamDevices[3].device.SerialNumber},
		mdmlab.MDMBootstrapPackagePending:   {noTeamDevices[4].device.SerialNumber, noTeamDevices[5].device.SerialNumber, noTeamDevices[6].device.SerialNumber, noTeamDevices[7].device.SerialNumber},
	}
	expectedSerialsByTeamAndStatus[team.ID] = map[mdmlab.MDMBootstrapPackageStatus][]string{
		mdmlab.MDMBootstrapPackageInstalled: {teamDevices[0].device.SerialNumber, teamDevices[1].device.SerialNumber},
		mdmlab.MDMBootstrapPackageFailed:    {teamDevices[2].device.SerialNumber, teamDevices[3].device.SerialNumber, teamDevices[4].device.SerialNumber},
		mdmlab.MDMBootstrapPackagePending:   {teamDevices[5].device.SerialNumber, teamDevices[6].device.SerialNumber},
	}
	...
	winHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		OsqueryHostID: ptr.String("non-macos-host"),
		NodeKey:       ptr.String("non-macos-host"),
		UUID:          uuid.New().String(),
		Hostname:      fmt.Sprintf("%sfoo.local.non.macos", t.Name()),
		Platform:      "windows",
		MDM:           mdmlab.MDMHostData{},
	})
	require.NoError(t, err)

	err = s.ds.SetOrUpdateMDMData(context.Background(), winHost.ID, false, true, s.server.URL+apple_mdm.MDMPath, true, mdmlab.WellKnownMDMMDMlab, "")
	require.NoError(t, err)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/bootstrap/summary", nil, http.StatusOK, &summaryResp)
	require.Equal(t, mdmlab.MDMAppleBootstrapPackageSummary{Pending: uint(len(noTeamDevices))}, summaryResp.MDMAppleBootstrapPackageSummary)

	var lhr listHostsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &lhr, "team_id", "0", "bootstrap_package", "pending")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/bootstrap/summary?team_id=%d", team.ID), nil, http.StatusOK, &summaryResp)
	require.Equal(t, mdmlab.MDMAppleBootstrapPackageSummary{Pending: uint(len(teamDevices))}, summaryResp.MDMAppleBootstrapPackageSummary)
	...
	enrollAndCheckBootstrapPackage := func(d *deviceWithResponse, bp *mdmlab.MDMAppleBootstrapPackage) {
	...
	checkHostDetails := func(t *testing.T, hostID uint, hostUUID string, expectedStatus mdmlab.MDMBootstrapPackageStatus) {
		var hostResp getHostResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", hostID), nil, http.StatusOK, &hostResp)
		...
		if expectedStatus == mdmlab.MDMBootstrapPackageFailed {
		...
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/identifier/%s", hostUUID), nil, http.StatusOK, &hostByIdentifierResp)
	...
	checkHostAPIs := func(t *testing.T, status mdmlab.MDMBootstrapPackageStatus, teamID *uint) {
		...
		listHostsPath := fmt.Sprintf("/api/latest/mdmlab/hosts?bootstrap_package=%s", status)
		if teamID != nil {
		...
		gotHostsBySerial := make(map[string]mdmlab.HostResponse)
		...
		countPath := fmt.Sprintf("/api/latest/mdmlab/hosts/count?bootstrap_package=%s", status)
		if teamID != nil {
	...
	summaryResp = getMDMAppleBootstrapPackageSummaryResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/bootstrap/summary", nil, http.StatusOK, &summaryResp)
	require.Equal(t, mdmlab.MDMAppleBootstrapPackageSummary{
		Installed: uint(3),
		Pending:   uint(4),
		Failed:    uint(1),
	}, summaryResp.MDMAppleBootstrapPackageSummary)

	checkHostAPIs(t, mdmlab.MDMBootstrapPackageInstalled, nil)
	checkHostAPIs(t, mdmlab.MDMBootstrapPackagePending, nil)
	checkHostAPIs(t, mdmlab.MDMBootstrapPackageFailed, nil)
	...
	checkHostAPIs(t, mdmlab.MDMBootstrapPackageInstalled, &team.ID)
	checkHostAPIs(t, mdmlab.MDMBootstrapPackagePending, &team.ID)
	checkHostAPIs(t, mdmlab.MDMBootstrapPackageFailed, &team.ID)
}
...
func (s *integrationMDMTestSuite) TestEULA() {
	...
	s.DoJSON("GET", "/api/latest/mdmlab/setup_experience/eula/metadata", nil, http.StatusNotFound, &metadataResp)

	// trying to upload a file that is not a PDF fails
	s.uploadEULA(&mdmlab.MDMEULA{Bytes: []byte("should-fail"), Name: "should-fail.pdf"}, http.StatusBadRequest, "invalid file type")
	// trying to upload an empty file fails
	s.uploadEULA(&mdmlab.MDMEULA{Bytes: []byte{}, Name: "should-fail.pdf"}, http.StatusBadRequest, "invalid file type")

	// admin is able to upload a new EULA
	s.uploadEULA(&mdmlab.MDMEULA{Bytes: pdfBytes, Name: pdfName}, http.StatusOK, "")

	// get EULA metadata
	metadataResp = getMDMEULAMetadataResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/setup_experience/eula/metadata", nil, http.StatusOK, &metadataResp)
	...
	// download EULA
	resp := s.DoRaw("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/eula/%s", eulaToken), nil, http.StatusOK)
	...
	var downloadResp downloadBootstrapPackageResponse
	s.DoJSON("GET", "/api/latest/mdmlab/setup_experience/eula/bad-token", nil, http.StatusNotFound, &downloadResp)

	// trying to upload any EULA without deleting the previous one first results in an error
	s.uploadEULA(&mdmlab.MDMEULA{Bytes: pdfBytes, Name: "should-fail.pdf"}, http.StatusConflict, "")

	// delete EULA
	var deleteResp deleteMDMEULAResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/setup_experience/eula/%s", eulaToken), nil, http.StatusOK, &deleteResp)
	metadataResp = getMDMEULAMetadataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/setup_experience/eula/%s", eulaToken), nil, http.StatusNotFound, &metadataResp)
	// trying to delete again is a bad request
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/setup_experience/eula/%s", eulaToken), nil, http.StatusNotFound, &deleteResp)
}
...
func (s *integrationMDMTestSuite) TestMigrateMDMDeviceWebhook() {
	...
	webhookSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/test_mdm_migration":
			var payload mdmlab.MigrateMDMDeviceWebhookPayload
	...
	s.Do("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/migrate_mdm", "good-token"), nil, http.StatusBadRequest)
	...
	// host is already enrolled to MDMlab MDM so migration is not allowed
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), h.ID, !isServer, enrolled, mdmURL, installedFromDEP, mdmlab.WellKnownMDMMDMlab, ""))
	s.Do("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/migrate_mdm", "good-token"), nil, http.StatusBadRequest)
	require.False(t, webhookCalled)
	...
	// simulate that the device is assigned to MDMlab in ABM
	s.mockDEPResponse(t.Name(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		switch r.URL.Path {
		case "/session":
		...
		case "/profile/devices":
			...
			resp.Devices = map[string]string{
				prof.Devices[0]: string(mdmlab.DEPAssignProfileResponseSuccess),
			}
			encoder := json.NewEncoder(w)
	...
	s.Do("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/migrate_mdm", "good-token"), nil, http.StatusNoContent)
	...
	// bad token
	s.Do("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/migrate_mdm", "bad-token"), nil, http.StatusUnauthorized)
	require.False(t, webhookCalled)
...
func (s *integrationMDMTestSuite) TestMDMMacOSSetup() {
	...
	tm, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	...
	t.Run("UpdateAppConfig", func(t *testing.T) {
		...
		for i, c := range enableReleaseDeviceCases {
			t.Run(strconv.Itoa(i), func(t *testing.T) {
				...
				if c.setupAssistant != "" {
					s.Do("POST", "/api/v1/mdmlab/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
	...
	t.Run("UpdateTeamConfig", func(t *testing.T) {
		path := fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm.ID)
		fmtJSON := `{
			"name": %q,
			%s
		}`
			...
			t.Run(strconv.Itoa(i), func(t *testing.T) {
				if c.setupAssistant != "" {
					...
					if uploadSucceeded {
						...
						teamSpecs := applyTeamSpecsRequest{Specs: []*mdmlab.TeamSpec{{
							Name: tm.Name,
							MDM:  mdmlab.TeamSpecMDM{MacOSSetup: macSetup},
						}}}
						s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK)
					}
				} else {
					...
					s.Do("PATCH", "/api/latest/mdmlab/setup_experience", json.RawMessage(jsonMustMarshal(t, payload)), expectedPatchStatus)
				}
	...
	t.Run("TestMDMAppleSetupEndpoint", func(t *testing.T) {
		t.Run("TestNoTeam", func(t *testing.T) {
			var acResp appConfigResponse
			s.Do("PATCH", "/api/latest/mdmlab/setup_experience",
				mdmlab.MDMAppleSetupPayload{TeamID: ptr.Uint(0), EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusNoContent)
			acResp = appConfigResponse{}
			s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
			require.True(t, acResp.MDM.MacOSSetup.EnableEndUserAuthentication)
			lastActivityID := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledMacosSetupEndUserAuth{}.ActivityName(),
				`{"team_id": null, "team_name": null}`, 0)
			...
			s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
			require.True(t, acResp.MDM.MacOSSetup.EnableEndUserAuthentication)
			s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledMacosSetupEndUserAuth{}.ActivityName(),
				``, lastActivityID) // no new activity

			s.Do("PATCH", "/api/latest/mdmlab/setup_experience",
				mdmlab.MDMAppleSetupPayload{TeamID: ptr.Uint(0), EnableEndUserAuthentication: ptr.Bool(false)}, http.StatusNoContent)
			acResp = appConfigResponse{}
			s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
			require.False(t, acResp.MDM.MacOSSetup.EnableEndUserAuthentication)
			require.Greater(t, s.lastActivityOfTypeMatches(mdmlab.ActivityTypeDisabledMacosSetupEndUserAuth{}.ActivityName(),
				`{"team_id": null, "team_name": null}`, 0), lastActivityID)
		})

		t.Run("TestTeam", func(t *testing.T) {
			tmConfigPath := fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm.ID)
			expectedActivityDetail := fmt.Sprintf(`{"team_id": %d, "team_name": %q}`, tm.ID, tm.Name)
			var tmResp teamResponse
			s.Do("PATCH", "/api/latest/mdmlab/setup_experience",
				mdmlab.MDMAppleSetupPayload{TeamID: &tm.ID, EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusNoContent)
			tmResp = teamResponse{}
			...
			s.Do("PATCH", "/api/latest/mdmlab/setup_experience",
				mdmlab.MDMAppleSetupPayload{TeamID: &tm.ID, EnableEndUserAuthentication: ptr.Bool(false)}, http.StatusNoContent)
			tmResp = teamResponse{}
			s.DoJSON("GET", tmConfigPath, nil, http.StatusOK, &tmResp)
	...
	t.Run("ValidateEnableEndUserAuthentication", func(t *testing.T) {
		...
		s.Do("PATCH", "/api/latest/mdmlab/setup_experience",
			mdmlab.MDMAppleSetupPayload{TeamID: ptr.Uint(0), EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusUnprocessableEntity)

		// can't enable end user authentication on team config without a configured IdP already on app config
		var teamResp teamResponse
		s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm.ID), json.RawMessage(fmt.Sprintf(`{
			"name": %q,
			"mdm": {
				"macos_setup": {
					"enable_end_user_authentication": true
		...
		s.Do("PATCH", "/api/latest/mdmlab/setup_experience",
			mdmlab.MDMAppleSetupPayload{TeamID: &tm.ID, EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusUnprocessableEntity)
...
`
	...
	s.DoJSON("GET", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusNotFound, &getResp)
	// get for non-existing team returns 404
	s.DoJSON("GET", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusNotFound, &getResp, "team_id", "123")

	// Profile name too long
	var createResp createMDMAppleSetupAssistantResponse
	r := s.Do("POST", "/api/latest/mdmlab/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            nil,
	...
	s.DoJSON("POST", "/api/latest/mdmlab/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            nil,
		Name:              "no-team",
	...
	s.lastActivityMatches(mdmlab.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
		`{"name": "no-team", "team_id": null, "team_name": null}`, 0)

	// create a team and a setup assistant for that team
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		Name:        t.Name(),
		Description: "desc",
	})
	require.NoError(t, err)
	...
	lastChangedActID := s.lastActivityMatches(mdmlab.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
	...
	latestChangedActID := s.lastActivityMatches(mdmlab.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
		fmt.Sprintf(`{"name": "team2", "team_id": %d, "team_name": %q}`, tm.ID, tm.Name), 0)
	require.Greater(t, latestChangedActID, lastChangedActID)

	// get no team
	s.DoJSON("GET", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusOK, &getResp)
	...
	// get team
	s.DoJSON("GET", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusOK, &getResp, "team_id", fmt.Sprint(tm.ID))
	...
	res := s.Do("POST", "/api/latest/mdmlab/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            &tm.ID,
		Name:              "team5",
		EnrollmentProfile: json.RawMessage(tmProf),
	}, http.StatusUnprocessableEntity)
	...
	s.Do("DELETE", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusNoContent)
	latestChangedActID = s.lastActivityMatches(mdmlab.ActivityTypeDeletedMacosSetupAssistant{}.ActivityName(),
		`{"name": "no-team2", "team_id": null, "team_name": null}`, 0)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusNotFound, &getResp, "team_id", fmt.Sprint(tm.ID))
	...
	s.lastActivityMatches(mdmlab.ActivityTypeDeletedMacosSetupAssistant{}.ActivityName(),
	...
	s.Do("DELETE", "/api/latest/mdmlab/enrollment_profiles/automatic", nil, http.StatusNoContent, "team_id", fmt.Sprint(tm2.ID))
	s.lastActivityMatches(mdmlab.ActivityTypeDeletedMacosSetupAssistant{}.ActivityName(),
		fmt.Sprintf(`{"name": "teamB", "team_id": %d, "team_name": %q}`, tm2.ID, tm2.Name), 0)

	// Try with a team that has no relevant ABM tokens
	teamNoABM, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		Name:        t.Name() + "no_abm",
		Description: "no abm",
	})
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) assertHostAppleConfigProfiles(want map[*mdmlab.Host][]mdmlab.HostMDMAppleProfile) {
	t := s.T()
	ds := s.ds
	ctx := context.Background()
...
func (s *integrationMDMTestSuite) assertHostWindowsConfigProfiles(want map[*mdmlab.Host][]mdmlab.HostMDMWindowsProfile) {
	t := s.T()
	ds := s.ds
	ctx := context.Background()
...
func (s *integrationMDMTestSuite) assertConfigProfilesByIdentifier(teamID *uint, profileIdent string, exists bool) (profile *mdmlab.MDMAppleConfigProfile) {
	t := s.T()
	if teamID == nil {
		teamID = ptr.Uint(0)
	}
	var cfgProfs []*mdmlab.MDMAppleConfigProfile
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
...
func (s *integrationMDMTestSuite) assertWindowsConfigProfilesByName(teamID *uint, profileName string, exists bool) {
	...
	var cfgProfs []*mdmlab.MDMWindowsConfigProfile
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
...
// used via s.DoRawWithHeaders to POST /api/_version_/mdmlab/mdm/apple/profiles.
...
func (s *integrationMDMTestSuite) uploadBootstrapPackage(
	pkg *mdmlab.MDMAppleBootstrapPackage,
	expectedStatus int,
	wantErr string,
) {
	t := s.T()
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/bootstrap", b.Bytes(), expectedStatus, headers)
...
func (s *integrationMDMTestSuite) uploadEULA(
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/setup_experience/eula", b.Bytes(), expectedStatus, headers)
...
func (s *integrationMDMTestSuite) TestGitOpsUserActions() {
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: globalProfiles}, http.StatusNoContent)
	...
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{
	...
	require.NoError(t, err)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/mdm/hosts/%d/profiles", h1.ID), getHostRequest{}, http.StatusForbidden, &getHostResponse{})

	err = s.ds.AddHostsToTeam(ctx, &t1.ID, []uint{h1.ID})
	require.NoError(t, err)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/mdm/hosts/%d/profiles", h1.ID), getHostRequest{}, http.StatusOK, &getHostResponse{})
}
...
func (s *integrationMDMTestSuite) TestSSO() {
	...
	wantSettings := mdmlab.SSOProviderSettings{
	...
	configFlows := []string{
		...
		"https://localhost:8080", // app that supports both MDM and MDMlab UI SSO
	}
	...
	resp := s.DoRaw("GET", "/api/latest/mdmlab/setup_experience/eula/"+q.Get("eula_token"), nil, http.StatusOK)
	require.EqualValues(t, len(pdfBytes), resp.ContentLength)
	require.Equal(t, "application/pdf", resp.Header.Get("content-type"))
	respBytes, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	...
	// Enroll generated the TokenUpdate request to MDMlab and enqueued the
	...
	var hostResp getHostResponse
	s.DoJSON("GET", "/api/v1/mdmlab/hosts/identifier/"+mdmDevice.UUID, nil, http.StatusOK, &hostResp)

	ac, err := s.ds.AppConfig(context.Background())
	require.NoError(t, err)

	detailQueries := osquery_utils.GetDetailQueries(context.Background(), config.MDMlabConfig{}, ac, &ac.Features)
	...
	rows := []map[string]string{
		{
			...
			"payload_identifier": apple_mdm.MDMlabPayloadIdentifier,
		},
	}
	err = detailQueries["mdm"].DirectIngestFunc(
		context.Background(),
		kitlog.NewNopLogger(),
		&mdmlab.Host{ID: hostResp.Host.ID},
		s.ds,
		rows,
	)
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts/%d/device_mapping", hostResp.Host.ID), nil, http.StatusOK, &dmResp)
	...
	require.True(t, ok)
	require.Equal(t, mdmlab.DeviceMappingMDMIdpAccounts, source)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/hosts?query=%s&device_mapping=true", url.QueryEscape("sso_user@example.com")), nil, http.StatusOK, &hostsResp)
	...
	require.Equal(t, mdmlab.DeviceMappingMDMIdpAccounts, dmResp.DeviceMapping[0].Source)
	...
	require.Equal(t, hostResp.Host.ID, gotHost.ID)
	require.NotNil(t, gotHost.DeviceMapping)
	dm = []mdmlab.HostDeviceMapping{}
	require.NoError(t, json.Unmarshal(*gotHost.DeviceMapping, &dm))
	require.Len(t, dm, 1)
	require.Equal(t, "sso_user@example.com", dm[0].Email)
	require.Equal(t, mdmlab.DeviceMappingMDMIdpAccounts, dm[0].Source)
	...
	res = s.DoRawNoAuth("POST", "/api/v1/mdmlab/mdm/sso/callback?SAMLResponse="+url.QueryEscape(rawSSOResp), nil, http.StatusSeeOther)
	require.NotEmpty(t, res.Header.Get("Location"))
	u, err = url.Parse(res.Header.Get("Location"))
	require.NoError(t, err)
	q = u.Query()
...
func (s *integrationMDMTestSuite) downloadAndVerifyOTAEnrollmentProfile(path string) {
	...
	assets, err := s.ds.GetAllMDMConfigAssetsByName(context.Background(), []mdmlab.MDMAssetName{
		mdmlab.MDMAssetCACert,
	}, nil)
	require.NoError(t, err)

	require.True(t, rootCA.AppendCertsFromPEM(assets[mdmlab.MDMAssetCACert].Value))
	...
	require.NoError(t, err)
	require.Contains(t, otaEnrollmentProfile.PayloadContent.URL, s.getConfig().ServerSettings.ServerURL+"/api/v1/mdmlab/ota_enrollment")
}
...
func (s *integrationMDMTestSuite) TestMDMMigration() {
	...
	checkMigrationResponses := func(host *mdmlab.Host, token string) {
		getDesktopResp := mdmlabDesktopResponse{}
		res := s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/desktop", nil, http.StatusOK)
		require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
		require.NoError(t, res.Body.Close())
		...
		// simulate that the device is assigned to MDMlab in ABM
		profileAssignmentStatusResponse := mdmlab.DEPAssignProfileResponseSuccess
		s.mockDEPResponse(t.Name(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
			switch r.URL.Path {
			case "/session":
		...
		profileAssignmentStatusResponse = mdmlab.DEPAssignProfileResponseFailed
		s.runDEPSchedule()
		getDesktopResp = mdmlabDesktopResponse{}
		res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/desktop", nil, http.StatusOK)
		require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
		...
		profileAssignmentStatusResponse = mdmlab.DEPAssignProfileResponseNotAccessible
		s.runDEPSchedule()
		getDesktopResp = mdmlabDesktopResponse{}
		res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/desktop", nil, http.StatusOK)
		require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
		...
		// simulate that the device needs to be enrolled in mdmlab, DEP capable
		err = s.ds.SetOrUpdateMDMData(
			ctx,
			host.ID,
			false,
		...
		// simulate that the device is manually enrolled into mdmlab, but DEP capable
		err = s.ds.SetOrUpdateMDMData(
			ctx,
			host.ID,
			false,
		...
		// simulate a host that was reset to factory, but mdmlab still has an mdm record active
		err = s.ds.SetOrUpdateMDMData(
			ctx,
			host.ID,
			false,
...
func (s *integrationMDMTestSuite) TestAppConfigWindowsMDM() {
	...
	tm1, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "1"})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "2"})
	require.NoError(t, err)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledWindowsMDM{}.ActivityName(), `{}`, 0)
	...
	// team 2, Windows workstation already enrolled in MDMlab in no team, and a
	...
	metadataHosts := []struct {
		...
		{"windows", "win-third-party", false, &tm2.ID, mdmlab.WellKnownMDMSimpleMDM, false, true},    // is enrolled in 3rd-party
		{"windows", "win-mdmlab", false, nil, mdmlab.WellKnownMDMMDMlab, false, false},                 // is already MDMlab-enrolled
		{"darwin", "macos-no-team", false, nil, "", false, false},                                   // is not Windows
		{"windows", "win-server-third-party", true, nil, mdmlab.WellKnownMDMSimpleMDM, false, false}, // is enrolled in 3rd-party, but is a server
	}
	hostsBySuffix := make(map[string]*mdmlab.Host, len(metadataHosts))
	for _, meta := range metadataHosts {
		var host *mdmlab.Host
		if meta.os == "windows" && meta.enrolledName == mdmlab.WellKnownMDMMDMlab {
			// special-case to create a properly MDM-enrolled into MDMlab host
			...
			require.NoError(t, err)
			err = s.ds.SetOrUpdateMDMData(ctx, host.ID, meta.isServer, true, s.server.URL, false, mdmlab.WellKnownMDMMDMlab, "")
			require.NoError(t, err)
		} else {
			host = createOrbitEnrolledHost(t, meta.os, meta.suffix, s.ds)
	...
	s.lastActivityMatches(mdmlab.ActivityTypeEnabledWindowsMDMMigration{}.ActivityName(), `{}`, 0)
	...
	assert.False(t, acResp.MDM.WindowsEnabledAndConfigured)
	assert.False(t, acResp.MDM.WindowsMigrationEnabled)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeDisabledWindowsMDM{}.ActivityName(), `{}`, 0)
	...
	// get the orbit config for each host, only the mdmlab-enrolled ones get the unenrollment,
	...
	for _, meta := range metadataHosts {
		...
		require.False(t, resp.Notifications.NeedsProgrammaticWindowsMDMEnrollment)
		require.False(t, resp.Notifications.NeedsMDMMigration)
		if meta.enrolledName == mdmlab.WellKnownMDMMDMlab {
			require.True(t, resp.Notifications.NeedsProgrammaticWindowsMDMUnenrollment)
		} else {
			require.False(t, resp.Notifications.NeedsProgrammaticWindowsMDMUnenrollment)
		}
...
func (s *integrationMDMTestSuite) TestOrbitConfigNudgeSettings() {
 ...
 `))
	...
	err := s.ds.UpdateHostOperatingSystem(context.Background(), h.ID, mdmlab.OperatingSystem{Platform: "darwin", Version: "12.0"})
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h.OrbitNodeKey)), http.StatusOK, &resp)
 ...
 `))
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h.OrbitNodeKey)), http.StatusOK, &resp)
	wantCfg, err := mdmlab.NewNudgeConfig(mdmlab.AppleOSUpdateSettings{Deadline: optjson.SetString("2022-01-04"), MinimumVersion: optjson.SetString("12.1.3")})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	s.assertMacOSDeclarationsByName(&team.ID, servermdm.MDMlabMacOSUpdatesProfileName, false)
	...
	s.assertMacOSDeclarationsByName(&team.ID, servermdm.MDMlabMacOSUpdatesProfileName, true)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h.OrbitNodeKey)), http.StatusOK, &resp)
	wantCfg, err = mdmlab.NewNudgeConfig(mdmlab.AppleOSUpdateSettings{Deadline: optjson.SetString("1992-01-01"), MinimumVersion: optjson.SetString("13.1.1")})
	require.NoError(t, err)
	require.Equal(t, wantCfg, resp.NudgeConfig)
	...
	err = s.ds.UpdateHostOperatingSystem(context.Background(), h2.ID, mdmlab.OperatingSystem{Platform: "darwin", Version: "12.0"})
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h2.OrbitNodeKey)), http.StatusOK, &resp)
	wantCfg, err = mdmlab.NewNudgeConfig(mdmlab.AppleOSUpdateSettings{Deadline: optjson.SetString("2022-01-04"), MinimumVersion: optjson.SetString("12.1.3")})
	require.NoError(t, err)
	require.Equal(t, wantCfg, resp.NudgeConfig)
	require.Equal(t, wantCfg.OSVersionRequirements[0].RequiredInstallationDate.String(), "2022-01-04 20:00:00 +0000 UTC")
	...
	err = s.ds.UpdateHostOperatingSystem(context.Background(), h3.ID, mdmlab.OperatingSystem{Platform: "darwin", Version: "14.1"})
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h3.OrbitNodeKey)), http.StatusOK, &resp)
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h4.OrbitNodeKey)), http.StatusOK, &resp)
	require.Nil(t, resp.NudgeConfig)
}
...
func (s *integrationMDMTestSuite) TestValidGetPoliciesRequestWithDeviceToken() {
	...
	encodedBinToken, err := mdmlab.GetEncodedBinarySecurityToken(mdmlab.WindowsMDMProgrammaticEnrollmentType, *windowsHost.OrbitNodeKey)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestGetPoliciesRequestWithInvalidUUID() {
	...
	// Preparing the GetPolicies Request message
	encodedBinToken, err := mdmlab.GetEncodedBinarySecurityToken(mdmlab.WindowsMDMProgrammaticEnrollmentType, "not_exists")
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestGetPoliciesRequestWithNotElegibleHost() {
	...
	// Preparing the GetPolicies Request message
	encodedBinToken, err := mdmlab.GetEncodedBinarySecurityToken(mdmlab.WindowsMDMProgrammaticEnrollmentType, *linuxHost.OrbitNodeKey)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestValidRequestSecurityTokenRequestWithDeviceToken() {
	...
	s.lastActivityOfTypeMatches(
		mdmlab.ActivityTypeMDMEnrolled{}.ActivityName(),
		fmt.Sprintf(`{
...
func (s *integrationMDMTestSuite) TestValidGetTOC() {
	...
	defer func() {
		if panicVal := recover(); panicVal != nil {
			...
			if ok && strings.Contains(s, "Assets may not be used when running MDMlab as a library") {
...
func (s *integrationMDMTestSuite) TestWindowsMDM() {
	...
	commandOne := &mdmlab.MDMWindowsCommand{
	...
	require.Equal(t, receivedCmd.Verb, mdmlab.CmdExec)
	...
	d.AppendResponse(mdmlab.SyncMLCmd{
	...
	commandTwo := &mdmlab.MDMWindowsCommand{
	...
	commandThree := &mdmlab.MDMWindowsCommand{
	...
	commandFour := &mdmlab.MDMWindowsCommand{
	...
	require.Equal(t, receivedCmdTwo.Verb, mdmlab.CmdGet)
	...
	require.Equal(t, receivedCmdThree.Verb, mdmlab.CmdReplace)
	...
	require.Equal(t, receivedCmdFour.Verb, mdmlab.CmdAdd)
		...
		Items: []mdmlab.CmdItem{
			{
				...
				Data:   &mdmlab.RawXmlData{Content: "0"},
			},
		},
		CmdID: mdmlab.CmdID{Value: cmdTwoRespUUID},
	})
	...
	var getMDMCmdResp getMDMCommandResultsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdOneUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mdmlab.MDMCommandResult{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdTwoUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mdmlab.MDMCommandResult{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdThreeUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mdmlab.MDMCommandResult{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdFourUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mdmlab.MDMCommandResult{
...
func (s *integrationMDMTestSuite) TestWindowsAutomaticEnrollmentCommands() {
	...
	err := s.ds.ApplyEnrollSecrets(ctx, nil, []*mdmlab.EnrollSecret{{Secret: t.Name()}})
	require.NoError(t, err)
	...
	checkinAndAck := func(expectMDMlabdCmds bool) {
		cmds, err := d.StartManagementSession()
		require.NoError(t, err)

		if !expectMDMlabdCmds {
		...
		// 2 status + 2 commands to install mdmlabd
		require.Len(t, cmds, 4)
		var mdmlabdAddCmd, mdmlabdExecCmd mdmlab.ProtoCmdOperation
		for _, c := range cmds {
			switch c.Verb {
			case "Add":
				mdmlabdAddCmd = c
		...
		require.Equal(t, syncml.MDMlabdWindowsInstallerGUID, mdmlabdAddCmd.Cmd.GetTargetURI())
		require.Equal(t, syncml.MDMlabdWindowsInstallerGUID, mdmlabdExecCmd.Cmd.GetTargetURI())
		require.Len(t, mdmlabdExecCmd.Cmd.Items, 1)
		...
		err = xml.Unmarshal([]byte(mdmlabdExecCmd.Cmd.Items[0].Data.Content), &installJob)
		require.NoError(t, err)
		require.Equal(t, s.mockedDownloadMDMlabdmMeta.MSIURL, installJob.Product.ContentURL)
		require.Equal(t, s.mockedDownloadMDMlabdmMeta.MSISha256, installJob.Product.FileHash)
		...
		d.AppendResponse(mdmlab.SyncMLCmd{
			XMLName: xml.Name{Local: mdmlab.CmdStatus},
			MsgRef:  &msgID,
			CmdRef:  &mdmlabdAddCmd.Cmd.CmdID.Value,
			Cmd:     &mdmlabdAddCmd.Verb,
			Data:    ptr.String("200"),
			Items:   nil,
			CmdID:   mdmlab.CmdID{Value: uuid.NewString()},
		})
		d.AppendResponse(mdmlab.SyncMLCmd{
			XMLName: xml.Name{Local: mdmlab.CmdStatus},
			MsgRef:  &msgID,
			CmdRef:  &mdmlabdExecCmd.Cmd.CmdID.Value,
			Cmd:     &mdmlabdExecCmd.Verb,
			Data:    ptr.String("200"),
			Items:   nil,
			CmdID:   mdmlab.CmdID{Value: uuid.NewString()},
		})
	...
	// start a management session, will receive the install mdmlabd commands
	checkinAndAck(true)

	// start a new management session again, MDMlabd is not reported as installed
	// so it receives the commands again
	checkinAndAck(true)

	// simulate mdmlabd installed and enrolled
	host := createOrbitEnrolledHost(t, "windows", "h1", s.ds)
	...
	// start a new management session again, MDMlabd is reported as installed so
...
func (s *integrationMDMTestSuite) TestValidManagementUnenrollRequest() {
	...
	enrolledDevice := &mdmlab.MDMWindowsEnrolledDevice{
...
`
	...
	// no host provided
	s.DoJSON("POST", "/api/latest/mdmlab/commands/run", &runMDMCommandRequest{
	...
	res := s.Do("POST", "/api/latest/mdmlab/commands/run", &runMDMCommandRequest{
		Command:   base64.StdEncoding.EncodeToString([]byte(macRawCmd)),
...
func (s *integrationMDMTestSuite) TestUpdateMDMWindowsEnrollmentsHostUUID() {
	...
	// simulate device that is MDM enrolled before mdmlabd is installed
	d := mdmlab.MDMWindowsEnrolledDevice{
		MDMDeviceID:            "test-device-id",
		MDMHardwareID:          "test-hardware-id",
		MDMDeviceState:         "ds",
		MDMDeviceType:          "dt",
...
func (s *integrationMDMTestSuite) TestBitLockerEnforcementNotifications() {
	...
	checkNotification := func(want bool) {
		resp := orbitGetConfigResponse{}
		s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *windowsHost.OrbitNodeKey)), http.StatusOK, &resp)
	...
	// enroll the host into MDMlab MDM
	encodedBinToken, err := mdmlab.GetEncodedBinarySecurityToken(mdmlab.WindowsMDMProgrammaticEnrollmentType, *windowsHost.OrbitNodeKey)
	require.NoError(t, err)
	requestBytes, err := s.newSecurityTokenMsg(encodedBinToken, true, false)
	require.NoError(t, err)
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), windowsHost.ID, false, true, "https://example.com", true, mdmlab.WellKnownMDMMDMlab, ""))
	...
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "mdm": { "macos_settings": { "enable_disk_encryption": true } } }`), http.StatusOK, &acResp)
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	...
	// host has disk encryption on, the key is not decryptable by mdmlab. Gets the notification
	err = s.ds.SetHostsDiskEncryptionKeyStatus(ctx, []uint{windowsHost.ID}, false, time.Now())
	require.NoError(t, err)
	checkNotification(true)

	// host has disk encryption on, the disk was encrypted by mdmlab. Doesn't get the notification
...
func (s *integrationMDMTestSuite) TestHostDiskEncryptionKey() {
	...
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "mdm": { "enable_disk_encryption": true } }`), http.StatusOK, &acResp)
	...
	require.Contains(t, msg, "host is not enrolled with mdmlab")
	...
	s.Do("POST", "/api/mdmlab/orbit/disk_encryption_key", orbitPostDiskEncryptionKeyRequest{
		OrbitNodeKey:  *host.OrbitNodeKey,
	...
	require.Equal(t, mdmlab.DiskEncryptionEnforcing, *hostResp.Host.MDM.OSSettings.DiskEncryption.Status) // still pending because disk encryption status is not set
	...
	wstepCert, _, _, err := s.mdmlabCfg.MDM.MicrosoftWSTEP()
	require.NoError(t, err)
	...
	require.NotNil(t, hostResp.Host.MDM.OSSettings)
	require.NotNil(t, hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionFailed, *hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	...
	require.NotNil(t, hostResp.Host.MDM.OSSettings)
	require.NotNil(t, hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionVerified, *hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", hostResp.Host.MDM.OSSettings.DiskEncryption.Detail)
}
...
func (s *integrationMDMTestSuite) TestMDMEnabledAndConfigured() {
	...
	compareMacOSSetupValues := (func(t *testing.T, got mdmlab.MacOSSetup, want mdmlab.MacOSSetup) {
	...
	insertBootstrapPackageAndSetupAssistant := func(t *testing.T, teamID *uint) {
		...
		require.NoError(t, s.ds.InsertMDMAppleBootstrapPackage(ctx, &mdmlab.MDMAppleBootstrapPackage{
			TeamID: tmID,
			Name:   "foo",
		...
		_, err := s.ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mdmlab.MDMAppleSetupAssistant{
			TeamID:  teamID,
			Name:    "bar",
			Profile: []byte("{}"),
		})
	...
	t.Run("apply app config spec", func(t *testing.T) {
		t.Run("disk encryption", func(t *testing.T) {
			...
			s.DoJSON("PATCH", "/api/latest/mdmlab/config", ac, http.StatusOK, &acResp)
		...
		t.Run("macos setup", func(t *testing.T) {
			...
			acResp := checkAppConfig(t, true, true)
			compareMacOSSetupValues(t, mdmlab.MacOSSetup{}, acResp.AppConfig.MDM.MacOSSetup) // disabled by default

			// initialize our test app config
			ac := appConfig.Copy()
			ac.AgentOptions = nil
			ac.MDM.EndUserAuthentication = mdmlab.MDMEndUserAuthentication{
			...
			ac.MDM.MacOSSetup = mdmlab.MacOSSetup{
			...
			s.DoJSON("PATCH", "/api/latest/mdmlab/config", ac, http.StatusUnprocessableEntity, &acResp)
		...
		t.Run("custom settings", func(t *testing.T) {
			...
			ac := appConfig.Copy()
			ac.AgentOptions = nil
			ac.MDM.MacOSSettings.CustomSettings = []mdmlab.MDMProfileSpec{}
			ac.MDM.WindowsSettings.CustomSettings = optjson.SetSlice([]mdmlab.MDMProfileSpec{})
			require.NoError(t, s.ds.SaveAppConfig(ctx, ac))
			acResp := checkAppConfig(t, true, true)
			...
			// add custom settings
			ac.MDM.MacOSSettings.CustomSettings = []mdmlab.MDMProfileSpec{{Path: "foo"}, {Path: "bar"}}
			ac.MDM.WindowsSettings.CustomSettings = optjson.SetSlice([]mdmlab.MDMProfileSpec{{Path: "baz"}, {Path: "zab"}})
			s.DoJSON("PATCH", "/api/latest/mdmlab/config", ac, http.StatusOK, &acResp)
			acResp = checkAppConfig(t, true, true)                                                                                 // both mac and windows mdm enabled
			...
			ac.MDM.MacOSSettings.CustomSettings = []mdmlab.MDMProfileSpec{{Path: "oof"}, {Path: "bar"}}
			ac.MDM.WindowsSettings.CustomSettings = optjson.SetSlice([]mdmlab.MDMProfileSpec{{Path: "foo"}, {Path: "zab"}})
			s.DoJSON("PATCH", "/api/latest/mdmlab/config", ac, http.StatusUnprocessableEntity, &acResp)
			acResp = checkAppConfig(t, false, false) // both disabled
	...
	t.Run("modify team", func(t *testing.T) {
		...
		checkTeam := func(t *testing.T, team *mdmlab.Team, checkMDM *mdmlab.TeamPayloadMDM) teamResponse {
			var wantDiskEncryption bool
			var wantMacOSSetup mdmlab.MacOSSetup
			if checkMDM != nil {
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), nil, http.StatusOK, &resp)
		...
		var createTeamResp teamResponse
		s.DoJSON("POST", "/api/latest/mdmlab/teams", createTeamRequest{mdmlab.TeamPayload{
			Name: ptr.String("Ninjas"),
			MDM:  &mdmlab.TeamPayloadMDM{EnableDiskEncryption: optjson.SetBool(true)}, // mdm is ignored by the create team endpoint
	...
	t.Run("edit team spec", func(t *testing.T) {
		...
		checkTeam := func(t *testing.T, team *mdmlab.Team, checkMDM *mdmlab.TeamSpecMDM) teamResponse {
		...
		tmSpecReq := applyTeamSpecsRequest{Specs: []*mdmlab.TeamSpec{{Name: "Pirates"}}}
		var tmSpecResp applyTeamSpecsResponse
		s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", tmSpecReq, http.StatusOK, &tmSpecResp)
		teamID, ok := tmSpecResp.TeamIDsByName["Pirates"]
		require.True(t, ok)
		team := mdmlab.Team{ID: teamID, Name: "Pirates"}
		...
		for _, c := range cases {
			...
			t.Run(c.name, func(t *testing.T) {
				...
				tmSpecReq = applyTeamSpecsRequest{Specs: []*mdmlab.TeamSpec{{
					Name: team.Name,
					MDM:  *c.mdm,
				}}}
				s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", tmSpecReq, c.expectedStatus, &tmSpecResp)
...
func (s *integrationMDMTestSuite) checkMDMProfilesSummaries(t *testing.T, teamID *uint, expectedSummary mdmlab.MDMProfilesSummary, expectedAppleSummary *mdmlab.MDMProfilesSummary) {
	...
	if expectedAppleSummary != nil {
		...
		s.DoJSON("GET", "/api/v1/mdmlab/mdm/apple/profiles/summary", getMDMAppleProfilesSummaryRequest{}, http.StatusOK, &apple, queryParams...)
	...
	s.DoJSON("GET", "/api/v1/mdmlab/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &combined, queryParams...)
...
func (s *integrationMDMTestSuite) checkMDMDiskEncryptionSummaries(t *testing.T, teamID *uint, expectedSummary mdmlab.MDMDiskEncryptionSummary, checkFileVaultSummary bool) {
	var queryParams []string
	if teamID != nil {
		queryParams = append(queryParams, "team_id", fmt.Sprintf("%d", *teamID))
	}
	...
	if checkFileVaultSummary {
		...
		s.DoJSON("GET", "/api/v1/mdmlab/mdm/apple/filevault/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &fileVault, queryParams...)
	...
	s.DoJSON("GET", "/api/v1/mdmlab/disk_encryption", getMDMProfilesSummaryRequest{}, http.StatusOK, &combined, queryParams...)
...
func (s *integrationMDMTestSuite) TestWindowsFreshEnrollEmptyQuery() {
	...
	s.Do(
		"POST",
		"/api/v1/mdmlab/mdm/profiles/batch",
		batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{}},
		http.StatusNoContent,
	)
	...
	require.NotContains(t, dqResp.Queries, "mdmlab_detail_query_mdm_config_profiles_windows")

	// add two profiles
	s.Do("POST", "/api/v1/mdmlab/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mdmlab.MDMProfileBatchPayload{
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_mdm_config_profiles_windows")
	require.NotEmpty(t, dqResp.Queries, "mdmlab_detail_query_mdm_config_profiles_windows")
}

func (s *integrationMDMTestSuite) TestManualEnrollmentCommands() {
	t := s.T()

	checkInstallMDMlabdCommandSent := func(mdmDevice *mdmtest.TestAppleMDMClient, wantCommand bool) {
		foundInstallMDMlabdCommand := false
		cmd, err := mdmDevice.Idle()
		require.NoError(t, err)
		for cmd != nil {
			var fullCmd micromdm.CommandPayload
			...
			if manifest := fullCmd.Command.InstallEnterpriseApplication.ManifestURL; manifest != nil {
				foundInstallMDMlabdCommand = true
				require.Equal(t, "InstallEnterpriseApplication", cmd.Command.RequestType)
				require.Contains(t, *fullCmd.Command.InstallEnterpriseApplication.ManifestURL, mdmlabdbase.GetPKGManifestURL())
			}
			cmd, err = mdmDevice.Acknowledge(cmd.CommandUUID)
			require.NoError(t, err)
		}
		require.Equal(t, wantCommand, foundInstallMDMlabdCommand)
	}

	// create a device that's not enrolled into MDMlab, it should get a command to
	// install mdmlabd
	mdmDevice := mdmtest.NewTestMDMClientAppleDirect(mdmtest.AppleEnrollInfo{
		SCEPChallenge: s.scepChallenge,
		SCEPURL:       s.server.URL + apple_mdm.SCEPPath,
	...
	require.NoError(t, err)
	s.runWorker()
	checkInstallMDMlabdCommandSent(mdmDevice, true)

	// create a device that's enrolled into MDMlab before turning on MDM features,
	// it should still get the command to install mdmlabd if turns on MDM.
	desktopToken := uuid.New().String()
...
func (s *integrationMDMTestSuite) TestLockUnlockWipeWindowsLinux() {
	...
	err := s.ds.SetOrUpdateMDMData(ctx, winHost.ID, false, true, s.server.URL, false, mdmlab.WellKnownMDMMDMlab, "")
	require.NoError(t, err)
	linuxHost := createOrbitEnrolledHost(t, "linux", "lock_unlock_linux", s.ds)

	for _, host := range []*mdmlab.Host{winHost, linuxHost} {
		t.Run(host.MDMlabPlatform(), func(t *testing.T) {
			...
			s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", host.ID), nil, http.StatusConflict, &unlockResp)

			// lock the host
			s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", host.ID), nil, http.StatusNoContent)
			...
			res := s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg := extractServerErrorText(res.Body)
			...
			s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", host.ID), nil, http.StatusConflict)
			// try to wipe a locked host
			res = s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			require.Contains(t, errMsg, "Host cannot be wiped until it is unlocked.")

			// unlock the host
			s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", host.ID), nil, http.StatusNoContent)
			...
			res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			...
			// wipe the host
			s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", host.ID), nil, http.StatusNoContent)
			wipeActID := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeWipedHost{}.ActivityName(), fmt.Sprintf(`{"host_id": %d, "host_display_name": %q}`, host.ID, host.DisplayName()), 0)

			// try to wipe the host again, already have it pending
			res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			require.Contains(t, errMsg, "Host has pending wipe request.")
			// no activity created
			s.lastActivityOfTypeMatches(mdmlab.ActivityTypeWipedHost{}.ActivityName(), fmt.Sprintf(`{"host_id": %d, "host_display_name": %q}`, host.ID, host.DisplayName()), wipeActID)
			...
			if host.MDMlabPlatform() == "linux" {
				...
				require.Equal(t, wipeCmd.Verb, mdmlab.CmdExec)
				...
				winMDMClient.AppendResponse(mdmlab.SyncMLCmd{
					XMLName: xml.Name{Local: mdmlab.CmdStatus},
					MsgRef:  &msgID,
			...
			res = s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			require.Contains(t, errMsg, "Cannot process lock requests once host is wiped.")
			res = s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			...
			s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", host.ID), nil, http.StatusConflict)
			// no activity created
			s.lastActivityOfTypeMatches(mdmlab.ActivityTypeWipedHost{}.ActivityName(), fmt.Sprintf(`{"host_id": %d, "host_display_name": %q}`, host.ID, host.DisplayName()), wipeActID)

			// re-enroll the host, simulating that another user received the wiped host
			newOrbitKey := uuid.New().String()
			newHost, err := s.ds.EnrollOrbit(ctx, true, mdmlab.OrbitHostInfo{
...
func (s *integrationMDMTestSuite) TestLockUnlockWipeMacOS() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", host.ID), nil, http.StatusOK, &lockResp, "view_pin", "true")
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", host.ID), nil, http.StatusOK, &unlockResp)
	...
	unlockActID := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeUnlockedHost{}.ActivityName(),
		fmt.Sprintf(`{"host_id": %d, "host_display_name": %q, "host_platform": %q}`, host.ID, host.DisplayName(), host.MDMlabPlatform()), 0)
	...
	newUnlockActID := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeUnlockedHost{}.ActivityName(),
		fmt.Sprintf(`{"host_id": %d, "host_display_name": %q, "host_platform": %q}`, host.ID, host.DisplayName(), host.MDMlabPlatform()), 0)
...
func (s *integrationMDMTestSuite) TestCustomConfigurationWebURL() {
	...
	require.NoError(t, err)
	s.DoJSON("POST", "/api/latest/mdmlab/hosts/transfer", addHostsToTeamRequest{TeamID: &teamID, HostIDs: []uint{h.ID}}, http.StatusOK, &addHostsToTeamResponse{})
	...
	applyResp = applyTeamSpecsResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusUnprocessableEntity, &applyResp)
}
...
func (s *integrationMDMTestSuite) TestDontIgnoreAnyProfileErrors() {
	...
	// The profiles should be associated with the host we made + the standard mdmlab configs
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(context.Background(), s.ds, host, map[string]*mdmlab.HostMacOSProfile{
		"I1": {Identifier: "I1", DisplayName: "I1", InstallDate: time.Now()},
		"I2": {Identifier: "I2", DisplayName: "I2", InstallDate: time.Now()},
		mobileconfig.MDMlabdConfigPayloadIdentifier:      {Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, DisplayName: "I2", InstallDate: time.Now()},
		mobileconfig.MDMlabCARootConfigPayloadIdentifier: {Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, DisplayName: "I2", InstallDate: time.Now()},
	}))
	...
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		require.Equal(t, mdmlab.MDMDeliveryVerified, *hm.Status)
	}

	// remove the profiles
	s.Do("POST", "/api/v1/mdmlab/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{}, http.StatusNoContent)
	s.awaitTriggerProfileSchedule(t)
	...
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		if wantErr, ok := expectedErrs[hm.Name]; ok {
			require.Equal(t, mdmlab.MDMDeliveryFailed, *hm.Status)
...
func (s *integrationMDMTestSuite) TestMDMDiskEncryptionIssue16636() {
	// see https://github.com/mdmlabdm/mdmlab/issues/16636
...
func (s *integrationMDMTestSuite) TestIsServerBitlockerStatus() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &hr)
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, host2.ID, false, true, "http://example.com", false, mdmlab.WellKnownMDMMDMlab, ""))

	hr = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host2.ID), nil, http.StatusOK, &hr)
	require.NotNil(t, hr.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mdmlab.DiskEncryptionEnforcing, *hr.Host.MDM.OSSettings.DiskEncryption.Status)
}
...
func (s *integrationMDMTestSuite) TestRemoveFailedProfiles() {
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(context.Background(), s.ds, host, map[string]*mdmlab.HostMacOSProfile{
		"I1": {Identifier: "I1", DisplayName: "I1", InstallDate: time.Now()},
		mobileconfig.MDMlabdConfigPayloadIdentifier:      {Identifier: mobileconfig.MDMlabdConfigPayloadIdentifier, DisplayName: "dn1", InstallDate: time.Now()},
		mobileconfig.MDMlabCARootConfigPayloadIdentifier: {Identifier: mobileconfig.MDMlabCARootConfigPayloadIdentifier, DisplayName: "dn2", InstallDate: time.Now()},
	}))
	...
	s.DoJSON("POST", "/api/latest/mdmlab/hosts/transfer", addHostsToTeamRequest{TeamID: &team.ID, HostIDs: []uint{host.ID}}, http.StatusOK, &addHostsToTeamResponse{})
	s.awaitTriggerProfileSchedule(t)
	...
	var profUUID string
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		require.Equal(t, mdmlab.MDMDeliveryPending, *hm.Status)
		if hm.Name == "N3" {
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/mdm/profiles/%s", profUUID), &deleteMDMAppleConfigProfileRequest{}, http.StatusOK)
	s.awaitTriggerProfileSchedule(t)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.Profiles)
	// Since MDMlab doesn't know for sure whether profile was installed or not, it sends a remove command just in case.
	require.Len(t, *getHostResp.Host.MDM.Profiles, 3)
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		require.Equal(t, mdmlab.MDMDeliveryPending, *hm.Status)
		if hm.Name == "N3" {
			assert.Equal(t, mdmlab.MDMOperationTypeRemove, hm.OperationType)
		}
	}
}
...
func (s *integrationMDMTestSuite) TestABMAssetManagement() {
	...
	r := s.Do("GET", "/api/latest/mdmlab/mdm/apple/abm_public_key", generateABMKeyPairResponse{}, http.StatusInternalServerError)
	require.Contains(t, extractServerErrorText(r.Body), "Couldn't download public key. Missing required private key. Learn how to configure the private key here: https://mdmlabdm.com/learn-more-about/mdmlab-server-private-key")
	testSetEmptyPrivateKey = false

	// grab the current public key
	var abmResp generateABMKeyPairResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/abm_public_key", nil, http.StatusOK, &abmResp)
	...
	// disable ABM
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/abm_tokens/%d", tok.ID), nil, http.StatusNoContent)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/abm_public_key", nil, http.StatusOK, &newABMResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/abm_public_key", nil, http.StatusOK, &renewABMResp)
...
func (s *integrationMDMTestSuite) enableABM(orgName string) *mdmlab.ABMToken {
	t := s.T()
	var abmResp generateABMKeyPairResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/abm_public_key", nil, http.StatusOK, &abmResp)
	require.Nil(t, abmResp.Err)
	...
	require.Equal(t, abmResp.PublicKey, assets[mdmlab.MDMAssetABMCert].Value)
...
func (s *integrationMDMTestSuite) appleCoreCertsSetup() {
	...
	s.SucceedNextCSRRequest()
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusOK, &resp)
	...
	originalAssets, err := s.ds.GetAllMDMConfigAssetsByName(ctx,
		[]mdmlab.MDMAssetName{mdmlab.MDMAssetCACert, mdmlab.MDMAssetCAKey, mdmlab.MDMAssetAPNSKey}, nil)
	require.NoError(t, err)
	...
	s.uploadDataViaForm("/api/latest/mdmlab/mdm/apple/apns_certificate", "certificate", "certificate.pem", certPEM, http.StatusAccepted, "", nil)
...
func (s *integrationMDMTestSuite) uploadABMToken(encryptedToken []byte, expectedStatus int, wantErr string) {
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/abm_tokens", b.Bytes(), expectedStatus, headers)
	if wantErr != "" {
		errMsg := extractServerErrorText(res.Body)
		assert.Contains(t, errMsg, wantErr)
	}
...
func (s *integrationMDMTestSuite) TestSilentMigrationGotchas() {
	...
	err := s.ds.SetOrUpdateMDMData(ctx, host.ID, true, true, "https://foo.com", false, mdmlab.WellKnownMDMSimpleMDM, "")
	require.NoError(t, err)

	var hostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
	require.NotNil(t, hostResp.Host)
	require.NotNil(t, hostResp.Host.MDM.ConnectedToMDMlab)
	require.False(t, *hostResp.Host.MDM.ConnectedToMDMlab)
	...
	resp := orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, &resp)
	require.False(t, resp.Notifications.RenewEnrollmentProfile)
	require.True(t, resp.Notifications.NeedsMDMMigration)

	// simulate that's actually enrolled to MDMlab under the hood
	mdmDevice := mdmtest.NewTestMDMClientAppleDirect(mdmtest.AppleEnrollInfo{
		SCEPChallenge: s.scepChallenge,
		SCEPURL:       s.server.URL + apple_mdm.SCEPPath,
		MDMURL:        s.server.URL + apple_mdm.MDMPath,
	...
	// host response says that it's connected to MDMlab (this just checks that the
	...
	require.NotNil(t, hostResp.Host)
	require.NotNil(t, hostResp.Host.MDM.ConnectedToMDMlab)
	require.True(t, *hostResp.Host.MDM.ConnectedToMDMlab)
	...
	// set an enroll secret so the mdmlabd profile is delivered
	var applyResp applyEnrollSecretSpecResponse
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mdmlab.EnrollSecretSpec{
			Secrets: []*mdmlab.EnrollSecret{{Secret: t.Name()}},
	...
	// explicitly run the worker, which will send the install mdmlabd command
	...
	require.Len(t, installs, 2) // mdmlabd profile and root cert profile

	// trigger the scep renewals cron
	cert, key, err := generateCertWithAPNsTopic()
	require.NoError(t, err)
	mdmlabCfg := config.TestConfig()
	config.SetTestMDMConfig(s.T(), &mdmlabCfg, cert, key, "")
	logger := kitlog.NewJSONLogger(os.Stdout)
	err = RenewSCEPCertificates(ctx, logger, s.ds, &mdmlabCfg, s.mdmCommander)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestMDMEnrollDoesntClearLastEnrolledAtForMacOS() {
	...
	// Enroll to MDMlab with mdmlabd first.
	desktopToken := uuid.New().String()
	lastEnrolledAt := time.Now().UTC()
	mdmDevice := mdmtest.NewTestMDMClientAppleDesktopManual(s.server.URL, desktopToken)
	mdmlabHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
	...
	require.NoError(t, err)
	err = s.ds.SetOrUpdateDeviceAuthToken(context.Background(), mdmlabHost.ID, desktopToken)
	require.NoError(t, err)
	...
	hostResp := getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", mdmlabHost.ID), nil, http.StatusOK, &hostResp)
	require.NotNil(t, hostResp.Host)
	require.Equal(t, lastEnrolledAt.Truncate(1*time.Second).UTC(), hostResp.Host.LastEnrolledAt.Truncate(1*time.Second).UTC())
}

func (s *integrationMDMTestSuite) TestConnectedToMDMlabWithoutCheckout() {
	t := s.T()
	ctx := context.Background()
	...
	// host is not connected to MDMlab anymore
	hostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
	require.NotNil(t, hostResp.Host)
	require.NotNil(t, hostResp.Host.MDM.ConnectedToMDMlab)
...
func (s *integrationMDMTestSuite) TestBatchAssociateAppStoreApps() {
	t := s.T()
	batchURL := "/api/latest/mdmlab/software/app_store_apps/batch"

	// non-existent team
	s.Do("POST", batchURL, batchAssociateAppStoreAppsRequest{}, http.StatusNotFound, "team_name", "foo")

	// create a team
	tmGood, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:        t.Name() + " good",
		Description: "desc",
	})
	require.NoError(t, err)
	...
	// create a team
	tmEmpty, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:        t.Name() + " empty",
		Description: "desc",
	})
	require.NoError(t, err)
	...
	// FIXME
	// s.Do("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mdmlab.VPPBatchPayload{{AppStoreID: s.appleVPPConfigSrvConfig.Assets[0].AdamID}}}, http.StatusUnprocessableEntity, "team_name", tmGood.Name)

	// Valid token
	orgName := "MDMlab Device Management Inc."
	token := "mycooltoken"
	...
	s.uploadDataViaForm("/api/latest/mdmlab/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &vppRes)

	var resPatchVPP patchVPPTokensTeamsResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{}}, http.StatusOK, &resPatchVPP)
	...
	hValid, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		HardwareSerial:  "123",
	...
	s.Do("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mdmlab.VPPBatchPayload{{AppStoreID: "fake-app"}}}, http.StatusUnprocessableEntity, "team_name", tmGood.Name)
	...
	s.DoJSON("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mdmlab.VPPBatchPayload{
	...
	assert.Contains(t, assoc, mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform: mdmlab.MacOSPlatform})

	// Associating one good and one bad app
	s.Do("POST",
		batchURL,
		batchAssociateAppStoreAppsRequest{Apps: []mdmlab.VPPBatchPayload{
			{AppStoreID: s.appleVPPConfigSrvConfig.Assets[0].AdamID},
			{AppStoreID: "fake-app"},
	...
	assert.Contains(t, assoc, mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform: mdmlab.MacOSPlatform})
	assert.Contains(t, assoc, mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.IOSPlatform})
	assert.Contains(t, assoc, mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.IPadOSPlatform})
	// Only macOS version should be self-service
	assert.Equal(t, mdmlab.VPPAppTeam{
		VPPAppID:           mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.MacOSPlatform},
		SelfService:        true,
		InstallDuringSetup: ptr.Bool(false),
	},
		assoc[mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.MacOSPlatform}])
	...
	require.Len(t, assoc, 4)
	assert.Equal(t, mdmlab.VPPAppTeam{
		VPPAppID:           mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform: mdmlab.MacOSPlatform},
		SelfService:        true,
		InstallDuringSetup: ptr.Bool(false),
	}, assoc[mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform: mdmlab.MacOSPlatform}])
	assert.Equal(t, mdmlab.VPPAppTeam{
		VPPAppID:           mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.IOSPlatform},
		InstallDuringSetup: ptr.Bool(false),
	}, assoc[mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.IOSPlatform}])
	assert.Equal(t, mdmlab.VPPAppTeam{
		VPPAppID:           mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.IPadOSPlatform},
		InstallDuringSetup: ptr.Bool(false),
	}, assoc[mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.IPadOSPlatform}])
	assert.Equal(t, mdmlab.VPPAppTeam{
		VPPAppID:           mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.MacOSPlatform},
		InstallDuringSetup: ptr.Bool(false),
	}, assoc[mdmlab.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform: mdmlab.MacOSPlatform}])
	...
	s.DoJSON("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mdmlab.VPPBatchPayload{{AppStoreID: s.appleVPPConfigSrvConfig.Assets[0].AdamID}}}, http.StatusOK, &batchAssociateResponse, "team_name", tmEmpty.Name)
...
func (s *integrationMDMTestSuite) TestRefetchIOSIPadOS() {
	...
	require.NoError(t, err)
	r := s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/refetch", mdmlabHost.ID), nil, http.StatusUnprocessableEntity, "error",
	...
	// Refetch host
	_ = s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/refetch", host.ID), nil, http.StatusOK)
	...
	assert.ElementsMatch(t, []mdmlab.HostMDMCommand{
		{HostID: host.ID, CommandType: mdmlab.RefetchAppsCommandUUIDPrefix},
		{HostID: host.ID, CommandType: mdmlab.RefetchDeviceCommandUUIDPrefix},
	}, commands)
	...
	expectedSoftware := []mdmlab.HostSoftwareEntry{
		{
	...
	cmd, err = mdmClient.AcknowledgeInstalledApplicationList(mdmClient.UUID, cmd.CommandUUID,
		[]mdmlab.Software{expectedSoftware[0].Software})
	require.NoError(t, err)
	...
	// Refetch host
	_ = s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/refetch", hostIPad.ID), nil, http.StatusOK)
	commandsSent += commandsSentPerRefetch

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", hostIPad.ID), nil, http.StatusOK, &hostResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", nil, http.StatusOK, &resp, "query", "Evernote")
	expectedTitles := []mdmlab.SoftwareTitleListResult{
		{
	...
	require.NotNil(t, cmd)
	require.Equal(t, "InstalledApplicationList", cmd.Command.RequestType)
	cmd, err = mdmClient.AcknowledgeInstalledApplicationList(mdmClient.UUID, cmd.CommandUUID, []mdmlab.Software{})
	require.NoError(t, err)
	require.Equal(t, "DeviceInformation", cmd.Command.RequestType)
	...
	trigger := triggerRequest{
		Name: string(mdmlab.CronAppleMDMIPhoneIPadRefetcher),
	}
	_ = s.Do("POST", "/api/latest/mdmlab/trigger", trigger, http.StatusOK)
	commandsSent += commandsSentPerRefetch
...
func (s *integrationMDMTestSuite) TestVPPApps() {
	...
	s.uploadDataViaForm("/api/latest/mdmlab/vpp_tokens", "token", "token.vpptoken", []byte("foobar"), http.StatusUnprocessableEntity, "Invalid token. Please provide a valid content token from Apple Business Manager.", nil)
	// Attempt to renew an invalid (nonexistent) token, should fail
	s.uploadDataViaFormWithVerb("/api/latest/mdmlab/vpp_tokens/999/renew", "PATCH", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte("foobar"))), http.StatusUnprocessableEntity, "Invalid token. Please provide a valid content token from Apple Business Manager.", nil)

	// Simulate a server error from the Apple API
	t.Setenv("FLEET_DEV_VPP_URL", s.appleVPPConfigSrv.URL+"?serverError")
	s.uploadDataViaForm("/api/latest/mdmlab/vpp_tokens", "token", "token.vpptoken", []byte("foobar"), http.StatusInternalServerError, "Apple VPP endpoint returned error: Internal server error (error number: 9603)", nil)
	...
	expDate := expTime.Format(mdmlab.VPPTimeFormat)
	tokenJSON := fmt.Sprintf(`{"expDate":"%s","token":"%s","orgName":"%s"}`, expDate, token, orgName)
	t.Setenv("FLEET_DEV_VPP_URL", s.appleVPPConfigSrv.URL)
	var validToken uploadVPPTokenResponse
	s.uploadDataViaForm("/api/latest/mdmlab/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &validToken)

	s.lastActivityMatches(mdmlab.ActivityEnabledVPP{}.ActivityName(), "", 0)

	// Renew valid token
	expTime = expTime.Add(24 * time.Hour).UTC().Round(time.Second)
	expDate = expTime.Format(mdmlab.VPPTimeFormat)
	tokenJSON = fmt.Sprintf(`{"expDate":"%s","token":"%s","orgName":"%s"}`, expDate, token, orgName)
	s.uploadDataViaFormWithVerb(fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/renew", validToken.Token.ID), "PATCH", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &validToken)
	// Valid token bytes, but non-existent token ID. Should get not found
	s.uploadDataViaFormWithVerb("/api/latest/mdmlab/vpp_tokens/999/renew", "PATCH", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusNotFound, "VPPToken was not found in the datastore", nil)

	// Get the token
	var resp getVPPTokensResponse
	s.DoJSON("GET", "/api/latest/mdmlab/vpp_tokens", &getVPPTokensRequest{}, http.StatusOK, &resp)
	...
	var resPatchVPP patchVPPTokensTeamsResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", resp.Tokens[0].ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID}}, http.StatusOK, &resPatchVPP)
	...
	// Add the team back using the PATCH /api/latest/mdmlab/config endpoint now (what GitOps uses).
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(fmt.Sprintf(`{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/app_store_apps", &getAppStoreAppsRequest{}, http.StatusOK, &appResp, "team_id",
		fmt.Sprint(team.ID))
	require.NoError(t, appResp.Err)

	macOSApp := mdmlab.VPPApp{
	...
	getSoftwareTitleIDFromApp := func(app *mdmlab.VPPApp) uint {
		var titleID uint
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
			ctx := context.Background()
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/app_store_apps", &addAppStoreAppRequest{TeamID: ptr.Uint(9999), AppStoreID: addedApp.AdamID}, http.StatusNotFound, &addAppResp)

	s.DoJSON("POST", "/api/latest/mdmlab/software/app_store_apps", &addAppStoreAppRequest{TeamID: &team.ID, AppStoreID: addedApp.AdamID, SelfService: true}, http.StatusOK, &addAppResp)
	s.lastActivityMatches(mdmlab.ActivityAddedAppStoreApp{}.ActivityName(),
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", nil, http.StatusOK, &listSw, "team_id", fmt.Sprint(team.ID), "available_for_install", "true")
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", nil, http.StatusOK, &listSw, "team_id", fmt.Sprint(team.ID), "self_service", "true")
	require.Len(t, listSw.SoftwareTitles, 1)
	require.Equal(t, macOSTitleID, listSw.SoftwareTitles[0].ID)

	// delete the app store app for team 1
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", macOSTitleID), nil, http.StatusNoContent,
		"team_id", fmt.Sprint(team.ID))
	s.lastActivityMatches(mdmlab.ActivityDeletedAppStoreApp{}.ActivityName(),
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", macOSTitleID), nil, http.StatusNotFound,
		"team_id", fmt.Sprint(team.ID))

	// get the list of available apps, returns all apps now
	appResp = getAppStoreAppsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/software/app_store_apps", nil, http.StatusOK, &appResp, "team_id", fmt.Sprint(team.ID))
	require.NoError(t, appResp.Err)
	assert.ElementsMatch(t, expectedApps, appResp.AppStoreApps)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/app_store_apps",
	...
	// list the software titles for that team, to get the title id of the VPP app
	listSw = listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", nil, http.StatusOK, &listSw, "team_id", fmt.Sprint(team.ID),
	...
	s.Do("POST", "/api/latest/mdmlab/hosts/transfer",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/app_store_apps",
		...
		http.StatusOK, &addAppResp)
	s.lastActivityMatches(
		mdmlab.ActivityAddedAppStoreApp{}.ActivityName(),
		fmt.Sprintf(`{"team_name": "%s", "software_title": "%s", "software_title_id": %d, "app_store_id": "%s", "team_id": %d, "platform": "%s", "self_service": true}`, team.Name,
	...
	r := s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", orbitHost.ID, macOSTitleID), &installSoftwareRequest{}, http.StatusBadRequest)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", validToken.Token.ID), patchVPPTokensTeamsRequest{}, http.StatusOK, &resPatchVPP)

	// Spoof an expired VPP token and attempt to install VPP app
	tokenJSONBad := fmt.Sprintf(`{"expDate":"%s","token":"%s","orgName":"%s"}`, "2099-06-24T15:50:50+0000", "badtoken", "Evil MDMlab")
	s.appleVPPConfigSrvConfig.Location = "Spooky Haunted House"
	var vppRes uploadVPPTokenResponse
	s.uploadDataViaForm("/api/latest/mdmlab/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSONBad))), http.StatusAccepted, "", &vppRes)

	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID, 99}}, http.StatusUnprocessableEntity, &resPatchVPP)

	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID}}, http.StatusOK, &resPatchVPP)

	// Disable the token
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{}, http.StatusOK, &resPatchVPP)

	// Enable all teams token
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d/teams", validToken.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{}}, http.StatusOK, &resPatchVPP)

	// Attempt to install non-existent app
	r = s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", mdmHost.ID, 99999), &installSoftwareRequest{},
		http.StatusBadRequest)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", mdmHost.ID, errTitleID), &installSoftwareRequest{},
		http.StatusAccepted, &installResp)

	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d", vppRes.Token.ID), &deleteVPPTokenRequest{}, http.StatusNoContent)

	// Check if the host is listed as pending
	var listResp listHostsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &listResp, "software_status", "pending", "team_id", fmt.Sprint(team.ID),
		"software_title_id", fmt.Sprint(errTitleID))
	require.Len(t, listResp.Hosts, 1)
	require.Equal(t, listResp.Hosts[0].ID, mdmHost.ID)
	var countResp countHostsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "software_status", "pending", "team_id",
	...
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &listResp, "software_status", "failed", "team_id", fmt.Sprint(team.ID),
		"software_title_id", fmt.Sprint(errTitleID))
	require.Len(t, listResp.Hosts, 1)
	require.Equal(t, listResp.Hosts[0].ID, mdmHost.ID)
	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "software_status", "failed", "team_id",
		fmt.Sprint(team.ID), "software_title_id", fmt.Sprint(errTitleID))
	require.Equal(t, 1, countResp.Count)

	s.lastActivityMatches(
		mdmlab.ActivityInstalledAppStoreApp{}.ActivityName(),
		fmt.Sprintf(
	...
	// Trigger install to the host
	installResp = installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", mdmHost.ID, macOSTitleID), &installSoftwareRequest{},
		http.StatusAccepted, &installResp)
	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "software_status", "pending", "team_id",
	...
	listResp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &listResp, "software_status", "installed", "team_id",
		fmt.Sprint(team.ID), "software_title_id", fmt.Sprint(macOSTitleID))
	require.Len(t, listResp.Hosts, 1)
	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "software_status", "installed", "team_id",
		fmt.Sprint(team.ID), "software_title_id", fmt.Sprint(macOSTitleID))
	require.Equal(t, 1, countResp.Count)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSw)
	...
	require.Equal(t, *got1.Status, mdmlab.SoftwareInstalled)
	...
	require.Equal(t, *got2.Status, mdmlab.SoftwareInstallFailed)
	...
	// Check with a query
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSw, "query", "App 1")
	...
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSw, "self_service", "true")
	...
	for name, install := range installs {
		t.Run(name, func(t *testing.T) {
			...
			if install.deviceToken != "" {
				...
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/device/%s/software/install/%d", install.deviceToken, install.titleID),
					&mdmlabSelfServiceSoftwareInstallRequest{}, http.StatusAccepted, &ssInstallResp)
			} else {
				installResp = installSoftwareResponse{}
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", installHost.ID, titleID),
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", installHost.ID),
				nil, http.StatusOK, &hostActivitiesResp)
			activitiesToString := func(activities []*mdmlab.Activity) []string {
				var res []string
			...
			assert.Equal(t, hostActivitiesResp.Activities[0].Type, mdmlab.ActivityInstalledAppStoreApp{}.ActivityName())
			...
			// Check list host software
			getHostSw = getHostSoftwareResponse{}
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", installHost.ID), nil, http.StatusOK, &getHostSw)
	...
	var ssInstallResp submitSelfServiceSoftwareInstallResponse
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/device/%s/software/install/%d", selfServiceToken, iPadOSTitleID), &mdmlabSelfServiceSoftwareInstallRequest{},
		http.StatusBadRequest, &ssInstallResp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/device/%s/software/install/%d", selfServiceToken, iOSTitleID), &mdmlabSelfServiceSoftwareInstallRequest{},
		http.StatusBadRequest, &ssInstallResp)

	// Delete VPP token and check that it's not appearing anymore

	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/vpp_tokens/%d", validToken.Token.ID), &deleteVPPTokenResponse{}, http.StatusNoContent)
	s.DoJSON("GET", "/api/latest/mdmlab/vpp_tokens", &getVPPTokensRequest{}, http.StatusOK, &resp)
}
...
func (s *integrationMDMTestSuite) TestVPPAppPolicyAutomation() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/app_store_apps", &addAppStoreAppRequest{TeamID: &team.ID, AppStoreID: addedApp.AdamID, SelfService: true}, http.StatusOK, &addedMacOSApp)
	...
	policy0Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{
		Name:     "policy0Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	require.NoError(t, err)
	policy1Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{
		Name:     "policy1Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	require.NoError(t, err)
	policy2Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mdmlab.PolicyPayload{ // will be set up with same VPP app
		Name:     "policy2Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team.ID, policy1Team1.ID), modifyTeamPolicyRequest{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", macOSTitleID), getSoftwareTitleRequest{
		TeamID: &team.ID,
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team.ID, policy2Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: macOSTitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	host3 := newMDMlabdHost("host3", &team.ID, "ubuntu")
	...
	var hostActivitiesResp listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", mdmHost.ID),
		nil, http.StatusOK, &hostActivitiesResp)
	activitiesToString := func(activities []*mdmlab.Activity) []string {
		var res []string
		for _, activity := range activities {
...
func (s *integrationMDMTestSuite) TestEnrollmentProfilesWithSpecialChars() {
	...
	nameWithInvalidChars := "MDMlab & Device <3 Management"
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mdmlab.EnrollSecretSpec{
			Secrets: []*mdmlab.EnrollSecret{{Secret: enrollSecretWithInvalidChars}},
		},
	...
	_, err = s.ds.NewMDMAppleEnrollmentProfile(ctx, mdmlab.MDMAppleEnrollmentProfilePayload{
		Type:       "automatic",
		DEPProfile: &rawMsg,
		Token:      "abcd",
	})
	...
	s.downloadAndVerifyEnrollmentProfile("/api/latest/mdmlab/enrollment_profiles/manual")

	// ensure the mdmlabd profile sends a good enroll secret too
	s.awaitTriggerProfileSchedule(t)
	prof := s.assertConfigProfilesByIdentifier(nil, mobileconfig.MDMlabdConfigPayloadIdentifier, true)

	type mdmlabdPlist struct {
		PayloadContent []struct {
...
func (s *integrationMDMTestSuite) TestOTAEnrollment() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mdmlab.EnrollSecretSpec{
			Secrets: []*mdmlab.EnrollSecret{{Secret: globalSecret}},
		},
	}, http.StatusOK, &applyResp)
...
<dict>
	...
	httpResp := s.DoRawNoAuth("POST", "/api/latest/mdmlab/ota_enrollment", reqBody, http.StatusForbidden)
	...
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mdmlab/ota_enrollment?enroll_secret=foo", nil, http.StatusBadRequest)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "invalid request body")
	require.NoError(t, httpResp.Body.Close())

	// request with unsigned body
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mdmlab/ota_enrollment?enroll_secret=foo", reqBody, http.StatusBadRequest)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "invalid request body")
	require.NoError(t, httpResp.Body.Close())
	...
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mdmlab/ota_enrollment?enroll_secret=foo", signedReqBody, http.StatusForbidden)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "Couldn't install the profile. Invalid enroll secret. Please contact your IT admin.")
	require.NoError(t, httpResp.Body.Close())
...
<dict>
	...
	signedReqBody, err = signedData.Finish()
	require.NoError(t, err)
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mdmlab/ota_enrollment?enroll_secret=foo", signedReqBody, http.StatusBadRequest)
	errMsg = extractServerErrorText(httpResp.Body)
	...
	var hostByIdentifierResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/identifier/%s", mdmDevice.UUID), nil, http.StatusOK, &hostByIdentifierResp)
	...
	teamSpecs := applyTeamSpecsRequest{Specs: []*mdmlab.TeamSpec{{Name: "newteam", Secrets: &[]mdmlab.EnrollSecret{{Secret: teamSecret}}}}}
	s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK, &specResp)
	...
	require.NoError(t, mdmDevice.Enroll())
	s.runWorker()
	checkInstallMDMlabdCommandSent(mdmDevice, false)
...
func (s *integrationMDMTestSuite) TestSCEPProxy() {
	...
	var badProfile mdmlab.HostMDMAppleProfile
	for _, profile := range profiles {
		switch profile.Identifier {
		case "Good1":
			profileUUID = profile.ProfileUUID
		case "Bad1":
			profile.Status = &mdmlab.MDMDeliveryFailed
			badProfile = profile
		}
	}
	err = s.ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mdmlab.MDMAppleBulkUpsertHostProfilePayload{{
	...
	// Spin up an "external" SCEP server, which MDMlab server will proxy
	...
	// Expired challenge
	err = s.ds.BulkUpsertMDMManagedCertificates(ctx, []*mdmlab.MDMBulkUpsertManagedCertificatePayload{
		{
			HostUUID:             host.UUID,
...
func (s *integrationMDMTestSuite) TestVPPAppsMDMFiltering() {
	...
	s.Do("POST", "/api/latest/mdmlab/hosts/transfer", &addHostsToTeamRequest{HostIDs: []uint{orbitHost.ID, mdmHost.ID}, TeamID: &team.ID}, http.StatusOK)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", mdmHost.ID), getHostSoftwareRequest{}, http.StatusOK, &resp)
...
func (s *integrationMDMTestSuite) TestSetupExperience() {
	...
	team1, err := ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)

	tfr1, err := mdmlab.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerID1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
	...
	app1 := &mdmlab.VPPApp{Name: "vpp_app_1", VPPAppTeam: mdmlab.VPPAppTeam{VPPAppID: mdmlab.VPPAppID{AdamID: "1", Platform: mdmlab.MacOSPlatform}}, BundleIdentifier: "b1"}
	...
	s.DoJSON("GET", "/api/latest/mdmlab/setup_experience/software", getSetupExperienceSoftwareRequest{}, http.StatusOK, &respGetSetupExperience, "team_id", fmt.Sprint(team1.ID))
	require.Len(t, respGetSetupExperience.SoftwareTitles, 2)

	var respPutSetupExperience putSetupExperienceSoftwareResponse
	s.DoJSON("PUT", "/api/latest/mdmlab/setup_experience/software", putSetupExperienceSoftwareRequest{TeamID: team1.ID, TitleIDs: []uint{respListTitles.SoftwareTitles[0].ID, respListTitles.SoftwareTitles[1].ID}}, http.StatusOK, &respPutSetupExperience)
	...
	orbitNodeKey := setOrbitEnrollment(t, mdmlabHost, ds)

	err = mdmDevice.Enroll()
	require.NoError(t, err)

	var orbitRes getOrbitSetupExperienceStatusResponse
	s.DoJSON("POST", "/api/mdmlab/orbit/setup_experience/status", getOrbitSetupExperienceStatusRequest{OrbitNodeKey: orbitNodeKey}, http.StatusOK, &orbitRes)
	...
	awaitingConfig, err := s.ds.GetHostAwaitingConfiguration(ctx, mdmlabHost.UUID)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestWindowsMigrationEnabled() {
	...
	require.True(t, acResp.MDM.WindowsEnabledAndConfigured)
	require.False(t, acResp.MDM.WindowsMigrationEnabled)
	s.lastActivityMatches(mdmlab.ActivityTypeDisabledWindowsMDMMigration{}.ActivityName(), "", 0)
	...
	require.True(t, acResp.MDM.WindowsEnabledAndConfigured)
	require.True(t, acResp.MDM.WindowsMigrationEnabled)
	lastEnabledID := s.lastActivityMatches(mdmlab.ActivityTypeEnabledWindowsMDMMigration{}.ActivityName(), "", 0)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEnabledWindowsMDMMigration{}.ActivityName(), "", lastEnabledID)
...
func (s *integrationMDMTestSuite) TestHostsCantTurnMDMOff() {
	...
	r := s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", iOSHost.ID), nil, http.StatusBadRequest)
	require.Contains(t, extractServerErrorText(r.Body), mdmlab.CantTurnOffMDMForIOSOrIPadOSMessage)
	...
	r = s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", iPadOSHost.ID), nil, http.StatusBadRequest)
	require.Contains(t, extractServerErrorText(r.Body), mdmlab.CantTurnOffMDMForIOSOrIPadOSMessage)

	winHost, _ := createWindowsHostThenEnrollMDM(s.ds, s.server.URL, t)
	r = s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/mdm", winHost.ID), nil, http.StatusBadRequest)
	require.Contains(t, extractServerErrorText(r.Body), mdmlab.CantTurnOffMDMForWindowsHostsMessage)
}
