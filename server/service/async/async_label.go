...
func (t *Task) RecordLabelQueryExecutions(ctx context.Context, host *mdmlab.Host, results map[uint]*bool, ts time.Time, deferred bool) error {
...
func (t *Task) collectLabelQueryExecutions(ctx context.Context, ds mdmlab.Datastore, pool mdmlab.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskLabelMembership]
...
func (t *Task) GetHostLabelReportedAt(ctx context.Context, host *mdmlab.Host) time.Time {
	cfg := t.taskConfigs[config.AsyncTaskLabelMembership]
