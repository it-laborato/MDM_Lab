...
func (t *Task) RecordScheduledQueryStats(ctx context.Context, teamID *uint, hostID uint, stats []mdmlab.PackStats, ts time.Time) error {
	...
	// mdmlab.ScheduledQueryStats struct. Using this structure ensures the fields
...
func (t *Task) collectScheduledQueryStats(ctx context.Context, ds mdmlab.Datastore, pool mdmlab.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskScheduledQueryStats]
	...
	getHostStats := func(hostID uint) (sqStats []mdmlab.ScheduledQueryStats, schedQueryNames [][2]string, err error) {
	...
	hostsStats := make(map[uint][]mdmlab.ScheduledQueryStats, len(hosts)) // key is host ID
	...
	schedIDs, err := ds.ScheduledQueryIDsByName(ctx, mdmlab.DefaultScheduledQueryIDsByNameBatchSize, schedNames...)
	if err != nil {
	...
	// scheduled queries
	batchStatsByHost := make(map[uint][]mdmlab.ScheduledQueryStats, len(hostsStats))
