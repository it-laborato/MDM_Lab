...
func (t *Task) RecordPolicyQueryExecutions(ctx context.Context, host *mdmlab.Host, results map[uint]*bool, ts time.Time, deferred bool) error {
...
func (t *Task) collectPolicyQueryExecutions(ctx context.Context, ds mdmlab.Datastore, pool mdmlab.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskPolicyMembership]
  ...
  `)
	...
	getKeyTuples := func(hostID uint) (inserts []mdmlab.PolicyMembershipResult, err error) {
		...
		inserts = make([]mdmlab.PolicyMembershipResult, 0, len(res))
		stats.Items += len(res)
		for _, item := range res {
			...
			var tup mdmlab.PolicyMembershipResult
	...
	runInsertBatch := func(batch []mdmlab.PolicyMembershipResult) error {
		stats.Inserts++
	...
	insertBatch := make([]mdmlab.PolicyMembershipResult, 0, cfg.InsertBatch)
...
func (t *Task) GetHostPolicyReportedAt(ctx context.Context, host *mdmlab.Host) time.Time {
	cfg := t.taskConfigs[config.AsyncTaskPolicyMembership]
