...
func NewTask(ds mdmlab.Datastore, pool mdmlab.RedisPool, clck clock.Clock, conf config.OsqueryConfig) *Task {
...
func storePurgeActiveHostID(pool mdmlab.RedisPool, zsetKey string, hid uint, reportedAt, purgeOlder time.Time) (int, error) {
	// KEYS[1]: the zsetKey
...
func loadActiveHostIDs(pool mdmlab.RedisPool, zsetKey string, scanCount int) ([]hostIDLastReported, error) {
...
func removeProcessedHostIDs(pool mdmlab.RedisPool, zsetKey string, batch []hostIDLastReported) (int, error) {
	...
	//   e.g. if the clocks are not exactly in sync between mdmlab instances, or
