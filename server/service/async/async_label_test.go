...
func testCollectLabelQueryExecutions(t *testing.T, ds *mysql.Datastore, pool mdmlab.RedisPool) {
...
func testRecordLabelQueryExecutionsSync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
...
func testRecordLabelQueryExecutionsAsync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mdmlab.Host{
