...
func testCollectScheduledQueryStats(t *testing.T, ds *mysql.Datastore, pool mdmlab.RedisPool) {
	...
	cases := []struct {
		name      string
		hostStats map[uint][]mdmlab.PackStats // key is host ID
		...
		},
		{
			...
			map[uint][]mdmlab.PackStats{
				hid(1): {
					{PackName: p1.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq1.Name, Executions: 1}}},
				},
			},
			[]hostSQStats{
				{hid(1), sq1.ID, 1},
		...
		},
		{
			...
			map[uint][]mdmlab.PackStats{
				hid(2): {
					{PackName: p1.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq1.Name, Executions: 2}}},
					{PackName: p2.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq2.Name, Executions: 3}}},
				},
				hid(3): {
					{PackName: p2.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq2.Name, Executions: 4}}},
					{PackName: p3.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq3.Name, Executions: 5}}},
					{PackName: p3.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq4.Name, Executions: 6}}},
				},
			},
			[]hostSQStats{
				{hid(1), sq1.ID, 1},
		...
		},
		{
			...
			map[uint][]mdmlab.PackStats{
				hid(1): {
					{PackName: p1.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq1.Name, Executions: 7}}},
					{PackName: p2.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq2.Name, Executions: 8}}},
				},
				hid(3): {
					{PackName: p3.Name, QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: sq4.Name, Executions: 9}}},
				},
			},
			[]hostSQStats{
				{hid(1), sq1.ID, 7},
	...
	setupTest := func(t *testing.T, task *Task, data map[uint][]mdmlab.PackStats) collectorExecStats {
...
func testRecordScheduledQueryStatsSync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	host := &mdmlab.Host{ID: 1}

	stats := []mdmlab.PackStats{{PackName: "p1", QueryStats: []mdmlab.ScheduledQueryStats{{ScheduledQueryName: "sq1"}}}}
...
func testRecordScheduledQueryStatsAsync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	host := &mdmlab.Host{ID: 1}

	stats := []mdmlab.PackStats{
		{
			PackName: "p1", QueryStats: []mdmlab.ScheduledQueryStats{
				{ScheduledQueryName: "sq1", Executions: 1},
				{ScheduledQueryName: "sq2", Executions: 2},
			},
		},
		{
			PackName: "p2", QueryStats: []mdmlab.ScheduledQueryStats{
	...
	var sqStat mdmlab.ScheduledQueryStats
