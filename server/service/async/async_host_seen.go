...
func (t *Task) collectHostsLastSeen(ctx context.Context, ds mdmlab.Datastore, pool mdmlab.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskHostLastSeen]
...
func (t *Task) loadSeenHostsIDs(ctx context.Context, pool mdmlab.RedisPool) ([]uint, error) {
	cfg := t.taskConfigs[config.AsyncTaskHostLastSeen]
