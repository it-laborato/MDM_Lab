...
func testCollectPolicyQueryExecutions(t *testing.T, ds *mysql.Datastore, pool mdmlab.RedisPool) {
...
func testRecordPolicyQueryExecutionsSync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
...
func testRecordPolicyQueryExecutionsAsync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mdmlab.Host{
...
func testRecordPolicyQueryExecutionsNoPoliciesSync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mdmlab.Host{
...
func testRecordPolicyQueryExecutionsNoPoliciesAsync(t *testing.T, ds *mock.Store, pool mdmlab.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mdmlab.Host{
