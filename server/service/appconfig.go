...
type appConfigResponseFields struct {
	UpdateInterval  *mdmlab.UpdateIntervalConfig  `json:"update_interval"`
	Vulnerabilities *mdmlab.VulnerabilitiesConfig `json:"vulnerabilities"`

	// License is loaded from the service
	License *mdmlab.LicenseInfo `json:"license,omitempty"`
	// Logging is loaded on the fly rather than from the database.
	Logging *mdmlab.Logging `json:"logging,omitempty"`
	// Email is returned when the email backend is something other than SMTP, for example SES
	Email *mdmlab.EmailConfig `json:"email,omitempty"`
	// SandboxEnabled is true if mdmlab serve was ran with server.sandbox_enabled=true
...
func getAppConfigEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	var ssoSettings *mdmlab.SSOSettings
	...
	if license.IsPremium() && appConfig.MDMlabDesktop.TransparencyURL != "" {
		transparencyURL = appConfig.MDMlabDesktop.TransparencyURL
	}
	mdmlabDesktop := mdmlab.MDMlabDesktopSettings{TransparencyURL: transparencyURL}

	if appConfig.OrgInfo.ContactURL == "" {
		appConfig.OrgInfo.ContactURL = mdmlab.DefaultOrgInfoContactURL
	}
...
func (svc *Service) AppConfigObfuscated(ctx context.Context) (*mdmlab.AppConfig, error) {
...
func modifyAppConfigEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*modifyAppConfigRequest)
	appConfig, err := svc.ModifyAppConfig(ctx, req.RawMessage, mdmlab.ApplySpecOptions{
		Force:  req.Force,
		DryRun: req.DryRun,
	})
	if err != nil {
	...
	if (!license.IsPremium()) || response.MDMlabDesktop.TransparencyURL == "" {
		response.MDMlabDesktop.TransparencyURL = mdmlab.DefaultTransparencyURL
	}

	return response, nil
}

func (svc *Service) ModifyAppConfig(ctx context.Context, p []byte, applyOpts mdmlab.ApplySpecOptions) (*mdmlab.AppConfig, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.AppConfig{}, mdmlab.ActionWrite); err != nil {
		return nil, err
	}
	...
	var oldSMTPSettings mdmlab.SMTPSettings
	...
	storedJiraByProjectKey, err := mdmlab.IndexJiraIntegrations(appConfig.Integrations.Jira)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "modify AppConfig")
	}

	storedZendeskByGroupID, err := mdmlab.IndexZendeskIntegrations(appConfig.Integrations.Zendesk)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "modify AppConfig")
	}

	invalid := &mdmlab.InvalidArgumentError{}
	var newAppConfig mdmlab.AppConfig
	...
	// default transparency URL is https://mdmlabdm.com/transparency so you are allowed to apply as long as it's not changing
	if newAppConfig.MDMlabDesktop.TransparencyURL != "" && newAppConfig.MDMlabDesktop.TransparencyURL != mdmlab.DefaultTransparencyURL {
		if !license.IsPremium() {
			invalid.Append("transparency_url", ErrMissingLicense.Error())
			return nil, ctxerr.Wrap(ctx, invalid)
		}
		if _, err := url.Parse(newAppConfig.MDMlabDesktop.TransparencyURL); err != nil {
	...
	if newAppConfig.Integrations.NDESSCEPProxy.Set && newAppConfig.Integrations.NDESSCEPProxy.Valid && !license.IsPremium() {
		...
		default:
			...
			appConfig.Integrations.NDESSCEPProxy.Value.URL = mdmlab.Preprocess(newAppConfig.Integrations.NDESSCEPProxy.Value.URL)
			appConfig.Integrations.NDESSCEPProxy.Value.AdminURL = mdmlab.Preprocess(newAppConfig.Integrations.NDESSCEPProxy.Value.AdminURL)
			appConfig.Integrations.NDESSCEPProxy.Value.Username = mdmlab.Preprocess(newAppConfig.Integrations.NDESSCEPProxy.Value.Username)
			// do not preprocess password
			if len(svc.config.Server.PrivateKey) == 0 {
				invalid.Append("integrations.ndes_scep_proxy",
					"Cannot encrypt NDES password. Missing required private key. Learn how to configure the private key here: https://mdmlabdm.com/learn-more-about/mdmlab-server-private-key")
			}
			...
			if !oldAppConfig.Integrations.NDESSCEPProxy.Valid {
				...
				if newSCEPProxy.AdminURL != oldSCEPProxy.AdminURL ||
					...
					(newSCEPProxy.Password != "" && newSCEPProxy.Password != mdmlab.MaskedPassword) {
					ndesStatus = ndesStatusEdited
	...
	if legacyKeys := appConfig.DidUnmarshalLegacySettings(); len(legacyKeys) > 0 {
		...
		// were encountered.
		legacyUsedWarning = &mdmlab.BadRequestError{
			Message: fmt.Sprintf("warning: deprecated settings were used in the configuration: %v; consider updating to the new settings: https://mdmlabdm.com/docs/using-mdmlab/configuration-files#settings", legacyKeys),
		}
	}
	...
	if appConfig.ServerSettings.ServerURL == "" {
		invalid.Append("server_url", "MDMlab server URL must be present")
	}
	...
	if newAppConfig.AgentOptions != nil {
		...
		if err := mdmlab.ValidateJSONAgentOptions(ctx, svc.ds, *appConfig.AgentOptions, license.IsPremium()); err != nil {
			err = mdmlab.NewUserMessageError(err, http.StatusBadRequest)
	...
	mdmlab.ValidateGoogleCalendarIntegrations(appConfig.Integrations.GoogleCalendar, invalid)
	mdmlab.ValidateEnabledVulnerabilitiesIntegrations(appConfig.WebhookSettings.VulnerabilitiesWebhook, appConfig.Integrations, invalid)
	mdmlab.ValidateEnabledFailingPoliciesIntegrations(appConfig.WebhookSettings.FailingPoliciesWebhook, appConfig.Integrations, invalid)
	mdmlab.ValidateEnabledHostStatusIntegrations(appConfig.WebhookSettings.HostStatusWebhook, invalid)
	mdmlab.ValidateEnabledActivitiesWebhook(appConfig.WebhookSettings.ActivitiesWebhook, invalid)
	...
	// prevent using the output of mdmlabctl get config as input to mdmlabctl apply or this endpoint.
		...
		if appConfig.SMTPSettings.SMTPEnabled {
			if oldSMTPSettings != *appConfig.SMTPSettings || !appConfig.SMTPSettings.SMTPConfigured {
				if err = svc.sendTestEmail(ctx, appConfig); err != nil {
					return nil, mdmlab.NewInvalidArgumentError("SMTP Options", err.Error())
				}
			}
	...
	if newAppConfig.Integrations.Jira != nil || newAppConfig.Integrations.Zendesk != nil {
		delJira, err := mdmlab.ValidateJiraIntegrations(ctx, storedJiraByProjectKey, newAppConfig.Integrations.Jira)
		if err != nil {
			if errors.As(err, &mdmlab.IntegrationTestError{}) {
				return nil, ctxerr.Wrap(ctx, &mdmlab.BadRequestError{
					Message: err.Error(),
				})
			}
			return nil, ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("Jira integration", err.Error()))
		}
		appConfig.Integrations.Jira = newAppConfig.Integrations.Jira

		delZendesk, err := mdmlab.ValidateZendeskIntegrations(ctx, storedZendeskByGroupID, newAppConfig.Integrations.Zendesk)
		if err != nil {
			if errors.As(err, &mdmlab.IntegrationTestError{}) {
				return nil, ctxerr.Wrap(ctx, &mdmlab.BadRequestError{
					Message: err.Error(),
			...
			return nil, ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("Zendesk integration", err.Error()))
		}
		...
		if len(delJira)+len(delZendesk) > 0 {
			if err := svc.ds.DeleteIntegrationsFromTeams(ctx, mdmlab.Integrations{Jira: delJira, Zendesk: delZendesk}); err != nil {
	...
	if !license.IsPremium() {
		...
		appConfig.MDMlabDesktop.TransparencyURL = ""
	}
	...
	case appConfig.WebhookSettings.ActivitiesWebhook.Enable && !oldAppConfig.WebhookSettings.ActivitiesWebhook.Enable:
		act := mdmlab.ActivityTypeEnabledActivityAutomations{WebhookUrl: appConfig.WebhookSettings.ActivitiesWebhook.DestinationURL}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "create activity for enabled activity automations")
		}
	case !appConfig.WebhookSettings.ActivitiesWebhook.Enable && oldAppConfig.WebhookSettings.ActivitiesWebhook.Enable:
		act := mdmlab.ActivityTypeDisabledActivityAutomations{}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
	...
	case appConfig.WebhookSettings.ActivitiesWebhook.Enable &&
		...
		act := mdmlab.ActivityTypeEditedActivityAutomations{
	...
	case ndesStatusAdded:
		if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mdmlab.ActivityAddedNDESSCEPProxy{}); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "create activity for added NDES SCEP proxy")
		}
	case ndesStatusEdited:
		if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mdmlab.ActivityEditedNDESSCEPProxy{}); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "create activity for edited NDES SCEP proxy")
		}
	case ndesStatusDeleted:
		// Delete stored password
		if err := svc.ds.HardDeleteMDMConfigAsset(ctx, mdmlab.MDMAssetNDESPassword); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "delete NDES SCEP password")
		}
		if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mdmlab.ActivityDeletedNDESSCEPProxy{}); err != nil {
	...
	// Reset teams for ABM tokens that exist in MDMlab but aren't present in the config being passed
	...
	if vppAssignmentsDefined {
		// 1. Reset teams for VPP tokens that exist in MDMlab but aren't present in the config being passed
		clear(tokensInCfg)
		...
		for tokenID, tokenTeams := range vppAssignments {
			if _, err := svc.ds.UpdateVPPTokenTeams(ctx, tokenID, tokenTeams); err != nil {
				var errTokConstraint mdmlab.ErrVPPTokenTeamConstraint
				if errors.As(err, &errTokConstraint) {
					return nil, ctxerr.Wrap(ctx, mdmlab.NewUserMessageError(errTokConstraint, http.StatusConflict))
				}
				return nil, ctxerr.Wrap(ctx, err, "saving ABM token assignments")
			}
		}
	...
	//
	if err := svc.processAppleOSUpdateSettings(ctx, license, mdmlab.MacOS,
	...
	if err := svc.processAppleOSUpdateSettings(ctx, license, mdmlab.IOS,
		oldAppConfig.MDM.IOSUpdates,
		appConfig.MDM.IOSUpdates,
	); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "process iOS OS updates config change")
	}
	if err := svc.processAppleOSUpdateSettings(ctx, license, mdmlab.IPadOS,
	...
	if mdmEnableEndUserAuthChanged {
		...
		if appConfig.MDM.MacOSSetup.EnableEndUserAuthentication {
			act = mdmlab.ActivityTypeEnabledMacosSetupEndUserAuth{}
		} else {
			act = mdmlab.ActivityTypeDisabledMacosSetupEndUserAuth{}
		}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
	...
	if oldAppConfig.MDM.WindowsEnabledAndConfigured != appConfig.MDM.WindowsEnabledAndConfigured {
		...
		if appConfig.MDM.WindowsEnabledAndConfigured {
			act = mdmlab.ActivityTypeEnabledWindowsMDM{}
		} else {
			act = mdmlab.ActivityTypeDisabledWindowsMDM{}
		}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
	...
	if appConfig.MDM.WindowsEnabledAndConfigured && oldAppConfig.MDM.WindowsMigrationEnabled != appConfig.MDM.WindowsMigrationEnabled {
		...
		if appConfig.MDM.WindowsMigrationEnabled {
			act = mdmlab.ActivityTypeEnabledWindowsMDMMigration{}
		} else {
			act = mdmlab.ActivityTypeDisabledWindowsMDMMigration{}
		}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
			return nil, ctxerr.Wrapf(ctx, err, "create activity %s", act.ActivityName())
		}
...
func (svc *Service) processAppleOSUpdateSettings(
	...
	appleDevice mdmlab.AppleDevice,
	oldOSUpdateSettings mdmlab.AppleOSUpdateSettings,
	newOSUpdateSettings mdmlab.AppleOSUpdateSettings,
) error {
	if oldOSUpdateSettings.MinimumVersion.Value != newOSUpdateSettings.MinimumVersion.Value ||
		...
		var activity mdmlab.ActivityDetails
		switch appleDevice {
		case mdmlab.MacOS:
			activity = mdmlab.ActivityTypeEditedMacOSMinVersion{
				MinimumVersion: newOSUpdateSettings.MinimumVersion.Value,
				Deadline:       newOSUpdateSettings.Deadline.Value,
			}
		case mdmlab.IOS:
			activity = mdmlab.ActivityTypeEditedIOSMinVersion{
				MinimumVersion: newOSUpdateSettings.MinimumVersion.Value,
				Deadline:       newOSUpdateSettings.Deadline.Value,
			}
		case mdmlab.IPadOS:
			activity = mdmlab.ActivityTypeEditedIPadOSMinVersion{
				MinimumVersion: newOSUpdateSettings.MinimumVersion.Value,
				Deadline:       newOSUpdateSettings.Deadline.Value,
			}
		}
...
func (svc *Service) HasCustomSetupAssistantConfigurationWebURL(ctx context.Context, teamID *uint) (bool, error) {
	...
	if !ok || !az.Checked() {
		return false, mdmlab.NewAuthRequiredError("method requires previous authorization")
	}
...
func (svc *Service) validateMDM(
	...
	// we want to use `oldMdm` here as this boolean is set by the mdmlab
	// server at startup and can't be modified by the user
	if !oldMdm.EnabledAndConfigured {
		if len(mdm.MacOSSettings.CustomSettings) > 0 && !mdmlab.MDMProfileSpecsMatch(mdm.MacOSSettings.CustomSettings, oldMdm.MacOSSettings.CustomSettings) {
	...
	checkCustomSettings := func(prefix string, customSettings []mdmlab.MDMProfileSpec) {
	...
	if !mdm.WindowsEnabledAndConfigured {
		if mdm.WindowsSettings.CustomSettings.Set &&
			...
			!mdmlab.MDMProfileSpecsMatch(mdm.WindowsSettings.CustomSettings.Value, oldMdm.WindowsSettings.CustomSettings.Value) {
			invalid.Append("windows_settings.custom_settings",
				`Couldn’t edit windows_settings.custom_settings. Windows MDM isn’t turned on. This can be enabled by setting "controls.windows_enabled_and_configured: true" in the default configuration. Visit https://mdmlabdm.com/guides/windows-mdm-setup and https://mdmlabdm.com/docs/configuration/yaml-files#controls to learn more about enabling MDM.`)
		}
	}
	...
	if mdm.MacOSSetup.EnableEndUserAuthentication != oldMdm.MacOSSetup.EnableEndUserAuthentication {
		...
		if hasCustomConfigurationWebURL {
			invalid.Append("end_user_authentication", mdmlab.EndUserAuthDEPWebURLConfiguredErrMsg)
		}
	}
	...
	if !svc.config.MDM.IsMicrosoftWSTEPSet() {
		if mdm.WindowsEnabledAndConfigured {
			invalid.Append("mdm.windows_enabled_and_configured", "Couldn't turn on Windows MDM. Please configure MDMlab with a certificate and key pair first.")
			return nil
		}
	}
...
func (svc *Service) validateABMAssignments(
	ctx context.Context,
	mdm, oldMdm *mdmlab.MDM,
	invalid *mdmlab.InvalidArgumentError,
	license *mdmlab.LicenseInfo,
) ([]*mdmlab.ABMToken, error) {
	if mdm.DeprecatedAppleBMDefaultTeam != "" && mdm.AppleBusinessManager.Set && mdm.AppleBusinessManager.Valid {
		invalid.Append("mdm.apple_bm_default_team", mdmlab.AppleABMDefaultTeamDeprecatedMessage)
		return nil, nil
	}
	...
	if name := mdm.DeprecatedAppleBMDefaultTeam; name != "" && name != oldMdm.DeprecatedAppleBMDefaultTeam {
		...
		return []*mdmlab.ABMToken{tok}, nil
	}
	...
	if mdm.AppleBusinessManager.Set && mdm.AppleBusinessManager.Valid {
		...
		tokensByName := map[string]*mdmlab.ABMToken{}
		...
		var tokensToSave []*mdmlab.ABMToken
...
func (svc *Service) validateVPPAssignments(
	ctx context.Context,
	volumePurchasingProgramInfo []mdmlab.MDMAppleVolumePurchasingProgramInfo,
	invalid *mdmlab.InvalidArgumentError,
	license *mdmlab.LicenseInfo,
) (map[uint][]uint, error) {
	...
	teamsByName := map[string]uint{mdmlab.TeamNameNoTeam: 0}
	for _, tm := range teams {
		teamsByName[tm.Name] = tm.ID
	}
	...
	tokensByLocation := map[string]*mdmlab.VPPTokenDB{}
	for _, token := range tokens {
		// The default assignments for all tokens is "no team"
		// (ie: team_id IS NULL), here we reset the assignments
		// for all tokens, those will be re-added below.
	...
	for _, vpp := range volumePurchasingProgramInfo {
		for _, tmName := range vpp.Teams {
			if _, ok := teamsByName[norm.NFC.String(tmName)]; !ok && tmName != mdmlab.TeamNameAllTeams {
		...
		for _, teamName := range vpp.Teams {
			if teamName == mdmlab.TeamNameAllTeams {
				if len(vpp.Teams) > 1 {
					invalid.Appendf("mdm.volume_purchasing_program", "token cannot belong to %s and other teams", mdmlab.TeamNameAllTeams)
					return nil, nil
				}
				tokenTeams = []uint{}
				break
...
func validateSSOProviderSettings(incoming, existing mdmlab.SSOProviderSettings, invalid *mdmlab.InvalidArgumentError) {
...
func validateSSOSettings(p mdmlab.AppConfig, existing *mdmlab.AppConfig, invalid *mdmlab.InvalidArgumentError, license *mdmlab.LicenseInfo) {
	if p.SSOSettings != nil && p.SSOSettings.EnableSSO {

		var existingSSOProviderSettings mdmlab.SSOProviderSettings
...
type applyEnrollSecretSpecRequest struct {
	Spec   *mdmlab.EnrollSecretSpec `json:"spec"`
	DryRun bool                    `json:"-" query:"dry_run,optional"` // if true, apply validation but do not save changes
}
...
func applyEnrollSecretSpecEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*applyEnrollSecretSpecRequest)
	err := svc.ApplyEnrollSecretSpec(
		ctx, req.Spec, mdmlab.ApplySpecOptions{
			DryRun: req.DryRun,
		},
	)
	if err != nil {
...
func (svc *Service) ApplyEnrollSecretSpec(ctx context.Context, spec *mdmlab.EnrollSecretSpec, applyOpts mdmlab.ApplySpecOptions) error {
	if err := svc.authz.Authorize(ctx, &mdmlab.EnrollSecret{}, mdmlab.ActionWrite); err != nil {
		return err
	}
	if len(spec.Secrets) > mdmlab.MaxEnrollSecretsCount {
		return ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("secrets", "too many secrets"))
	}
	...
	if svc.config.Packaging.GlobalEnrollSecret != "" {
		return ctxerr.New(ctx, "enroll secret cannot be changed when mdmlab_packaging.global_enroll_secret is set")
	}
	...
	if applyOpts.DryRun {
		for _, s := range spec.Secrets {
			...
			if !available {
				return ctxerr.Wrap(ctx, mdmlab.NewInvalidArgumentError("secrets", "a provided global enroll secret is already being used"))
			}
		}
		return nil
	}
...
func getEnrollSecretSpecEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetEnrollSecretSpec(ctx context.Context) (*mdmlab.EnrollSecretSpec, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.EnrollSecret{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
	...
	return &mdmlab.EnrollSecretSpec{Secrets: secrets}, nil
}
...
func versionEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) Version(ctx context.Context) (*version.Info, error) {
	if err := svc.authz.Authorize(ctx, &mdmlab.Version{}, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func getCertificateEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) HostFeatures(ctx context.Context, host *mdmlab.Host) (*mdmlab.Features, error) {
