...
var _ mdmlab.FailingPolicySet = (*redisFailingPolicySet)(nil)

// NewFailing creates a redis policy set for failing policies.
func NewFailing(pool mdmlab.RedisPool) *redisFailingPolicySet {
...
// only in tests.
func NewFailingTest(t TestNamer, pool mdmlab.RedisPool) *redisFailingPolicySet {
	return &redisFailingPolicySet{
		pool:       pool,
		testPrefix: t.Name() + ":",
	}
...
func (r *redisFailingPolicySet) AddHost(policyID uint, host mdmlab.PolicySetHost) error {
...
func (r *redisFailingPolicySet) ListHosts(policyID uint) ([]mdmlab.PolicySetHost, error) {
	hostEntries, err := r.scanPolicySet(policyID)
	if err != nil {
		return nil, err
	}
	hosts := make([]mdmlab.PolicySetHost, len(hostEntries))
...
func (r *redisFailingPolicySet) RemoveHosts(policyID uint, hosts []mdmlab.PolicySetHost) error {
	if len(hosts) == 0 {
		return nil
	}
...
func (r *redisFailingPolicySet) addHostToPolicySet(policyID uint, host mdmlab.PolicySetHost) error {
	conn := redis.ConfigureDoer(r.pool, r.pool.Get())
	defer conn.Close()
...
func hostEntry(host mdmlab.PolicySetHost) string {
	return fmt.Sprint(host.ID) + "," + host.Hostname
}

func parseHostEntry(v string) (*mdmlab.PolicySetHost, error) {
