...
func (s *liveQueriesTestSuite) TestLiveQueriesRestOneHostOneQuery() {
	test := func(endpoint liveQueryEndpoint, savedQuery bool, hasStats bool) {
		...
		if endpoint == oneQueryEndpoint { //nolint:gocritic // ignore ifElseChain
			...
			go func() {
				defer wg.Done()
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/queries/%d/run", q1.ID), liveQueryRequest, http.StatusOK, &oneLiveQueryResp)
			}()
			...
			go func() {
				defer wg.Done()
				s.DoJSON("GET", "/api/latest/mdmlab/queries/run", liveQueryRequest, http.StatusOK, &liveQueryResp)
			}()
		} else { // customQueryOneHostId(.*)Endpoint
			liveQueryRequest := runLiveQueryOnHostRequest{
				Query: query,
			}
			url := fmt.Sprintf("/api/latest/mdmlab/hosts/%d/query", host.ID)
			if endpoint == customQueryOneHostIdentifierEndpoint {
				url = fmt.Sprintf("/api/latest/mdmlab/hosts/identifier/%s/query", host.UUID)
			}
			go func() {
				defer wg.Done()
				s.DoJSON(
		...
		go func() {
			...
			for range ticker.C {
				...
				if len(campaigns) == 1 && campaigns[0].Status == mdmlab.QueryRunning {
		...
		if endpoint == oneQueryEndpoint { //nolint:gocritic // ignore ifElseChain
			...
			assert.Equal(t, mdmlab.StatusOnline, liveQueryOnHostResp.Status)
			assert.Equal(t, query, liveQueryOnHostResp.Query)
			result = mdmlab.QueryResult{
		...
		var activity *mdmlab.ActivityTypeLiveQuery
		activityUpdated := make(chan *mdmlab.ActivityTypeLiveQuery)
		go func() {
			ticker := time.NewTicker(100 * time.Millisecond)
			defer ticker.Stop()
			for range ticker.C {
				...
				if err == nil {
					act := mdmlab.ActivityTypeLiveQuery{}
		...
		aggStats, err := mysql.GetAggregatedStats(context.Background(), s.ds, mdmlab.AggregatedStatsTypeScheduledQuery, q1.ID)
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestOneHostMultipleQuery() {
	...
	q1, err := s.ds.NewQuery(context.Background(), &mdmlab.Query{
	...
	q2, err := s.ds.NewQuery(context.Background(), &mdmlab.Query{
...
func getCIDForQ(s *liveQueriesTestSuite, q1 *mdmlab.Query) string {
	t := s.T()
	campaigns, err := s.ds.DistributedQueryCampaignsForQuery(context.Background(), q1.ID)
	require.NoError(t, err)
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestMultipleHostMultipleQuery() {
	...
	cid1 := getCIDForQ(s, q1)
	cid2 := getCIDForQ(s, q2)
	for i, h := range []*mdmlab.Host{h1, h2} {
		distributedReq := SubmitDistributedQueryResultsRequest{
			NodeKey: *h.NodeKey,
			Results: map[string][]map[string]string{
...
func (s *liveQueriesTestSuite) TestLiveQueriesInvalidInputs() {
	...
	liveQueryResp := runLiveQueryResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/queries/run", liveQueryRequest, http.StatusBadRequest, &liveQueryResp)
	...
	oneLiveQueryResp := runOneLiveQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/queries/%d/run", q1.ID), oneLiveQueryRequest, http.StatusBadRequest, &oneLiveQueryResp)
	...
	liveQueryOnHostResp := runLiveQueryOnHostResponse{}
	s.DoJSON(
		"POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/query", host.ID), liveQueryOnHostRequest, http.StatusBadRequest,
		&liveQueryOnHostResp,
	)
	s.DoJSON(
		"POST", fmt.Sprintf("/api/latest/mdmlab/hosts/identifier/%s/query", host.UUID), liveQueryOnHostRequest, http.StatusBadRequest,
		&liveQueryOnHostResp,
	)
}
...
func (s *liveQueriesTestSuite) TestLiveQueriesFailsToAuthorize() {
	...
	s.DoJSON("GET", "/api/latest/mdmlab/queries/run", liveQueryRequest, http.StatusForbidden, &liveQueryResp)
	oneLiveQueryRequest := runOneLiveQueryRequest{
		HostIDs: []uint{host.ID},
	}
	oneLiveQueryResp := runOneLiveQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/queries/%d/run", q1.ID), oneLiveQueryRequest, http.StatusForbidden, &oneLiveQueryResp)
}
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestFailsToCreateCampaign() {
	...
	oneLiveQueryResp := runOneLiveQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/queries/%d/run", 999), oneLiveQueryRequest, http.StatusNotFound, &oneLiveQueryResp)
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestFailsOnSomeHost() {
	test := func(newEndpoint bool) {
		...
		var qResults []mdmlab.QueryResult
		if newEndpoint {
			assert.Equal(t, q1.ID, oneLiveQueryResp.QueryID)
...
func (s *liveQueriesTestSuite) TestCreateDistributedQueryCampaign() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", nil, http.StatusUnprocessableEntity, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", req, http.StatusNotFound, &createResp)

	// create with no hosts
	req = createDistributedQueryCampaignRequest{
		QuerySQL: "SELECT 1",
	}
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", req, http.StatusBadRequest, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", req, http.StatusOK, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run_by_identifiers", req2, http.StatusOK, &createResp)
	assert.NotEqual(t, camp1.ID, createResp.Campaign.ID)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run_by_identifiers", req3, http.StatusBadRequest, &createResp)
}
...
func (s *liveQueriesTestSuite) TestCreateDistributedQueryCampaignBadRequest() {
	...
	req = createDistributedQueryCampaignRequest{
		Selected: mdmlab.HostTargets{HostIDs: []uint{host.ID}},
	}
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", req, http.StatusUnprocessableEntity, &createResp)
	...
	appCfg := mdmlab.AppConfig{ServerSettings: mdmlab.ServerSettings{LiveQueryDisabled: true, ServerURL: acResp.ServerSettings.ServerURL}, OrgInfo: mdmlab.OrgInfo{OrgName: acResp.OrgInfo.OrgName}}
	s.DoRaw("PATCH", "/api/latest/mdmlab/config", jsonMustMarshal(t, appCfg), http.StatusOK)

	req = createDistributedQueryCampaignRequest{
		QuerySQL: "select \"With automounting enabled anyone with physical access could attach a USB drive or disc and have its contents available in system even if they lacked permissions to mount it themselves.\" as Rationale;",
		Selected: mdmlab.HostTargets{HostIDs: []uint{host.ID}},
	}
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", req, http.StatusForbidden, &createResp)

	s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
	appCfg = mdmlab.AppConfig{ServerSettings: mdmlab.ServerSettings{LiveQueryDisabled: false, ServerURL: acResp.ServerSettings.ServerURL}, OrgInfo: mdmlab.OrgInfo{OrgName: acResp.OrgInfo.OrgName}}
	s.DoRaw("PATCH", "/api/latest/mdmlab/config", jsonMustMarshal(t, appCfg), http.StatusOK)
}
