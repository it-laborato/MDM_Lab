...
func (ts *withServer) commonTearDownTest(t *testing.T) {
	...
	filter := mdmlab.TeamFilter{User: &u}
	hosts, err := ts.ds.ListHosts(ctx, filter, mdmlab.HostListOptions{})
	require.NoError(t, err)
	...
	teams, err := ts.ds.ListTeams(ctx, mdmlab.TeamFilter{User: &u}, mdmlab.ListOptions{})
	require.NoError(t, err)
	...
	lbls, err := ts.ds.ListLabels(ctx, mdmlab.TeamFilter{}, mdmlab.ListOptions{})
	require.NoError(t, err)
	for _, lbl := range lbls {
		if lbl.LabelType != mdmlab.LabelTypeBuiltIn {
			err := ts.ds.DeleteLabel(ctx, lbl.Name)
			require.NoError(t, err)
		}
	}

	queries, _, _, err := ts.ds.ListQueries(ctx, mdmlab.ListQueryOptions{})
	require.NoError(t, err)
	...
	users, err := ts.ds.ListUsers(ctx, mdmlab.UserListOptions{})
	require.NoError(t, err)
	for _, u := range users {
	...
	globalPolicies, err := ts.ds.ListGlobalPolicies(ctx, mdmlab.ListOptions{})
	require.NoError(t, err)
	...
	packs, err := ts.ds.ListPacks(ctx, mdmlab.PackListOptions{})
	require.NoError(t, err)
...
func (ts *withServer) DoRawWithHeaders(
	...
	opts := []mdmlabhttp.ClientOpt{}
	if expectedStatusCode >= 300 && expectedStatusCode <= 399 {
		opts = append(opts, mdmlabhttp.WithFollowRedir(false))
	}
	client := mdmlabhttp.NewClient(opts...)
	...
	if resp.StatusCode != expectedStatusCode {
		...
		if err != nil {
			t.Logf("Error trying to decode response body as MDMlab jsonError: %s", err)
			require.Equal(t, expectedStatusCode, resp.StatusCode, fmt.Sprintf("response: %+v", resp))
		}
		require.Equal(t, expectedStatusCode, resp.StatusCode, fmt.Sprintf("MDMlab jsonError: %+v", je))
	}
...
func (ts *withServer) applyConfig(spec []byte) {
	...
	ts.Do("PATCH", "/api/latest/mdmlab/config", appConfigSpec, http.StatusOK)
}

func (ts *withServer) getConfig() *appConfigResponse {
	var responseBody *appConfigResponse
	ts.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &responseBody)
	return responseBody
}
...
func (ts *withServer) applyTeamSpec(yamlSpec []byte) {
	...
	ts.Do("POST", "/api/latest/mdmlab/spec/teams", specsReq, http.StatusOK)
}

func (ts *withServer) LoginSSOUser(username, password string) (mdmlab.Auth, string) {
	t := ts.s.T()
	auth, res := ts.loginSSOUser(username, password, "/api/v1/mdmlab/sso", http.StatusOK)
...
func (ts *withServer) LoginMDMSSOUser(username, password string) *http.Response {
	_, res := ts.loginSSOUser(username, password, "/api/v1/mdmlab/mdm/sso", http.StatusSeeOther)
	return res
}

func (ts *withServer) loginSSOUser(username, password string, basePath string, callbackStatus int) (mdmlab.Auth, *http.Response) {
	t := ts.s.T()
	...
	client := mdmlabhttp.NewClient(
		mdmlabhttp.WithFollowRedir(false),
		mdmlabhttp.WithCookieJar(jar),
	)
	...
	// to submit to the MDMlab server from here
	defer resp.Body.Close()
...
func (ts *withServer) lastActivityMatches(name, details string, id uint) uint {
	...
	ts.DoJSON("GET", "/api/latest/mdmlab/activities", nil, http.StatusOK, &listActivities, "order_key", "a.id", "order_direction", "desc", "per_page", "1")
...
func (ts *withServer) lastActivityOfTypeMatches(name, details string, id uint) uint {
	...
	var listActivities listActivitiesResponse
	ts.DoJSON("GET", "/api/latest/mdmlab/activities", nil, http.StatusOK,
...
func (ts *withServer) uploadSoftwareInstallerWithErrorNameReason(
	...
	tfr, err := mdmlab.NewKeepFileReader(filepath.Join("testdata", "software-installers", payload.Filename))
	// Try the test installers in the pkg/file testdata (to reduce clutter/copies).
	if errors.Is(err, os.ErrNotExist) {
		var err2 error
		tfr, err2 = mdmlab.NewKeepFileReader(filepath.Join("..", "..", "pkg", "file", "testdata", "software-installers", payload.Filename))
		if err2 == nil {
			err = nil
		}
	}
	...
	r := ts.DoRawWithHeaders("POST", "/api/latest/mdmlab/software/package", b.Bytes(), expectedStatus, headers)
	defer r.Body.Close()
...
func (ts *withServer) updateSoftwareInstaller(
	...
	r := ts.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package", payload.TitleID), b.Bytes(), expectedStatus, headers)
	defer r.Body.Close()
