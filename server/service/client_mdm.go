...
func (c *Client) GetAppleMDM() (*mdmlab.AppleMDM, error) {
	verb, path := "GET", "/api/latest/mdmlab/mdm/apple"
...
func (c *Client) GetAppleBM() (*mdmlab.AppleBM, error) {
	verb, path := "GET", "/api/latest/mdmlab/mdm/apple_bm"
...
func (c *Client) CountABMTokens() (int, error) {
	verb, path := "GET", "/api/latest/mdmlab/abm_tokens/count"
	var responseBody countABMTokensResponse
	err := c.authenticatedRequestWithQuery(nil, verb, path, &responseBody, "")
	return responseBody.Count, err
}

// RequestAppleCSR requests a signed CSR from the MDMlab server and returns the
// CSR bytes
func (c *Client) RequestAppleCSR() ([]byte, error) {
	verb, path := "GET", "/api/latest/mdmlab/mdm/apple/request_csr"
	var resp getMDMAppleCSRResponse
	err := c.authenticatedRequest(nil, verb, path, &resp)
	return resp.CSR, err
}

// RequestAppleABM requests a signed CSR from the MDMlab server and returns the
// public key bytes
func (c *Client) RequestAppleABM() ([]byte, error) {
	verb, path := "GET", "/api/latest/mdmlab/mdm/apple/abm_public_key"
	var resp generateABMKeyPairResponse
	err := c.authenticatedRequest(nil, verb, path, &resp)
	return resp.PublicKey, err
}

func (c *Client) GetBootstrapPackageMetadata(teamID uint, forUpdate bool) (*mdmlab.MDMAppleBootstrapPackage, error) {
	verb, path := "GET", fmt.Sprintf("/api/latest/mdmlab/mdm/bootstrap/%d/metadata", teamID)
...
func (c *Client) DeleteBootstrapPackage(teamID uint) error {
	verb, path := "DELETE", fmt.Sprintf("/api/latest/mdmlab/mdm/bootstrap/%d", teamID)
...
func (c *Client) UploadBootstrapPackage(pkg *mdmlab.MDMAppleBootstrapPackage) error {
	verb, path := "POST", "/api/latest/mdmlab/mdm/bootstrap"
...
func (c *Client) EnsureBootstrapPackage(bp *mdmlab.MDMAppleBootstrapPackage, teamID uint) error {
	isFirstTime := false
...
func (c *Client) ValidateBootstrapPackageFromURL(url string) (*mdmlab.MDMAppleBootstrapPackage, error) {
...
func downloadRemoteMacosBootstrapPackage(pkgURL string) (*mdmlab.MDMAppleBootstrapPackage, error) {
	...
	if err := file.CheckPKGSignature(pkgReader); err != nil {
		...
		case errors.Is(err, file.ErrNotSigned):
			return nil, errors.New("Couldn’t edit bootstrap_package. The bootstrap_package must be signed. Learn how to sign the package in the MDMlab documentation: https://mdmlabdm.com/docs/using-mdmlab/mdm-macos-setup-experience#step-2-sign-the-package")
		default:
...
func (c *Client) uploadMacOSSetupAssistant(data []byte, teamID *uint, name string) error {
	verb, path := "POST", "/api/latest/mdmlab/mdm/apple/enrollment_profile"
...
func (c *Client) MDMListCommands(opts mdmlab.MDMCommandListOptions) ([]*mdmlab.MDMCommand, error) {
	const defaultCommandsPerPage = 20

	verb, path := http.MethodGet, "/api/latest/mdmlab/mdm/commands"
...
func (c *Client) MDMGetCommandResults(commandUUID string) ([]*mdmlab.MDMCommandResult, error) {
	verb, path := http.MethodGet, "/api/latest/mdmlab/mdm/commandresults"
...
func (c *Client) RunMDMCommand(hostUUIDs []string, rawCmd []byte, forPlatform string) (*mdmlab.CommandEnqueueResult, error) {
	...
	if err := c.authenticatedRequest(request, "POST", "/api/latest/mdmlab/mdm/commands/run", &response); err != nil {
...
func (c *Client) prepareWindowsMDMCommand(rawCmd []byte) ([]byte, error) {
	if _, err := mdmlab.ParseWindowsMDMCommand(rawCmd); err != nil {
		return nil, err
	}
...
func (c *Client) MDMLockHost(hostID uint) error {
	...
	if err := c.authenticatedRequest(nil, "POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", hostID), &response); err != nil {
...
func (c *Client) MDMUnlockHost(hostID uint) (string, error) {
	...
	if err := c.authenticatedRequest(nil, "POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", hostID), &response); err != nil {
		return "", fmt.Errorf("lock host request: %w", err)
	}
...
func (c *Client) MDMWipeHost(hostID uint) error {
	...
	if err := c.authenticatedRequest(nil, "POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", hostID), &response); err != nil {
