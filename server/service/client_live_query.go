...
type LiveQueryResultsHandler struct {
	errors  chan error
	results chan mdmlab.DistributedQueryResult
...
func NewLiveQueryResultsHandler() *LiveQueryResultsHandler {
	return &LiveQueryResultsHandler{
		...
		results: make(chan mdmlab.DistributedQueryResult),
	}
}
...
func (h *LiveQueryResultsHandler) Results() <-chan mdmlab.DistributedQueryResult {
	return h.results
}
...
func (c *Client) LiveQueryWithContext(
	...
	verb, path := "POST", "/api/latest/mdmlab/queries/run_by_identifiers"
	...
	wssURL.Path = c.urlPrefix + "/api/latest/mdmlab/results/websocket"
