...
import (
	...
	"github.com/it-laborato/MDM_Lab/server/cron"
	...
	commonCalendar "github.com/it-laborato/MDM_Lab/server/service/calendar"
...
func (s *integrationEnterpriseTestSuite) TestTeamSpecs() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK, &applyResp)
	...
	s.lastActivityMatches(mdmlab.ActivityTypeAppliedSpecTeam{}.ActivityName(), fmt.Sprintf(`{"teams": [{"id": %d, "name": %q}]}`, team.ID, team.Name), 0)

	// Create team policy
	teamPolicy, err := s.ds.NewTeamPolicy(
		context.Background(), team.ID, nil, mdmlab.PolicyPayload{Name: "TestSpecTeamPolicy", Query: "SELECT 1"},
	)
	require.NoError(t, err)
	defer func() {
	...
	res := s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusUnprocessableEntity, "dry_run", "true")
	...
	s.DoJSON("GET", "/api/latest/mdmlab/teams/"+fmt.Sprint(team.ID), nil, http.StatusOK, &getTmResp)
	require.Equal(t, mdmlab.TeamMDM{
		MacOSUpdates: mdmlab.AppleOSUpdateSettings{
			MinimumVersion: optjson.SetString("10.15.0"),
			Deadline:       optjson.SetString("2021-01-01"),
	...
	s.DoJSON("GET", "/api/latest/mdmlab/teams", nil, http.StatusOK, &listTmResp, "query", teamName)
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusBadRequest, "dry_run", "true")

	// dry-run with empty body
	res = s.DoRaw("POST", "/api/latest/mdmlab/spec/teams", nil, http.StatusBadRequest, "force", "true")
	errBody, err := io.ReadAll(res.Body)
	require.NoError(t, err)
	require.Contains(t, string(errBody), `"Expected JSON Body"`)

	// dry-run with invalid top-level key
	s.Do("POST", "/api/latest/mdmlab/spec/teams", json.RawMessage(`{
		"specs": [
	...
	res = s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusUnprocessableEntity, "dry_run", "true")
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't update macos_settings because MDM features aren't turned on in MDMlab.")
	...
	s.DoJSON("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK, &applyResp, "dry_run", "true")
	...
	res = s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusUnprocessableEntity, "dry_run", "true")
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't update macos_setup because MDM features aren't turned on in MDMlab.")
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK, "dry_run", "true")
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK)
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusOK, "force", "true")
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", teamSpecs, http.StatusBadRequest)
	...
	teams, err := s.ds.ListTeams(context.Background(), mdmlab.TeamFilter{User: user}, mdmlab.ListOptions{})
	require.NoError(t, err)
	...
	teamSpecs = map[string]any{
		"specs": []any{
			map[string]any{
				"name":     "team2",
				"secrets":  []mdmlab.EnrollSecret{{Secret: "ABC"}},
				"features": nil,
			},
		},
	}
...
func (s *integrationEnterpriseTestSuite) TestTeamSpecsPermissions() {
	...
	team1, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		ID:          42,
		Name:        "team1",
		Description: "desc team1",
	})
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		ID:          43,
		Name:        "team2",
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", editTeam1Spec, http.StatusOK)
	...
	s.Do("POST", "/api/latest/mdmlab/spec/teams", editTeam2Spec, http.StatusForbidden)
}
...
func (s *integrationEnterpriseTestSuite) TestTeamSchedule() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule", team1.ID), nil, http.StatusOK, &ts)
	...
	gsParams := teamScheduleQueryRequest{ScheduledQueryPayload: mdmlab.ScheduledQueryPayload{
		QueryID:  &qr.ID,
		Interval: ptr.Uint(42),
	}}
	r := teamScheduleQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule", team1.ID), gsParams, http.StatusOK, &r)
	...
	modifyParams := modifyTeamScheduleRequest{ScheduledQueryPayload: mdmlab.ScheduledQueryPayload{Interval: ptr.Uint(55)}}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule/%d", team1.ID, id), modifyParams, http.StatusOK, &modifyResp)

	// just to satisfy my paranoia, wanted to make sure the contents of the json would work
	s.DoRaw("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule/%d", team1.ID, id), []byte(`{"interval": 77}`), http.StatusOK)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule/%d", team1.ID, id), nil, http.StatusOK, &deleteResp)
...
func (s *integrationEnterpriseTestSuite) TestTeamPolicies() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 0)
	require.Len(t, ts.InheritedPolicies, 0)

	// create a global policy
	gpol, err := s.ds.NewGlobalPolicy(context.Background(), nil, mdmlab.PolicyPayload{Name: "TestGlobalPolicy", Query: "SELECT 1"})
	require.NoError(t, err)
	defer func() {
	...
	qr, err := s.ds.NewQuery(context.Background(), &mdmlab.Query{
		Name:           "TestQuery2",
		Description:    "Some description",
		Query:          "select * from osquery;",
		ObserverCanRun: true,
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), tpParams, http.StatusOK, &r)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &tc)
	require.Nil(t, tc.Err)
	require.Equal(t, 1, tc.Count)

	gc := countGlobalPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/policies/count", nil, http.StatusOK, &gc)
	require.Nil(t, gc.Err)
	require.Equal(t, 1, gc.Count)

	// Test merge inherited
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), nil, http.StatusOK, &ts, "merge_inherited", "true", "order_key", "team_id", "order_direction", "desc")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &countResp, "merge_inherited", "true")
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/delete", team1.ID), deletePolicyParams, http.StatusOK, &deletePolicyResp)
...
func (s *integrationEnterpriseTestSuite) TestNoTeamPolicies() {
	...
	ts := listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies", nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 0)
	require.Len(t, ts.InheritedPolicies, 0)
	// Create a placeholder global policy.
	_, err := s.ds.NewGlobalPolicy(ctx, nil, mdmlab.PolicyPayload{
		Name:  "globalPolicy1",
		Query: "SELECT 0;",
	})
	require.NoError(t, err)
	...
	r := teamPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/teams/0/policies", tpParams, http.StatusOK, &r)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/teams/0/policies", tpParams, http.StatusConflict, &r)
	...
	tc := countTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies/count", nil, http.StatusOK, &tc)
	require.Equal(t, 2, tc.Count)
	// Test merge inherited for "No team" policies.
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies", nil, http.StatusOK, &ts, "merge_inherited", "true", "order_key", "team_id", "order_direction", "desc")
	...
	countResp := countTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies/count", nil, http.StatusOK, &countResp, "merge_inherited", "true")
	require.Nil(t, countResp.Err)
	...
	deletePolicyResp := deleteTeamPoliciesResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/teams/0/policies/delete", deletePolicyParams, http.StatusOK, &deletePolicyResp)
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies", nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 1)
	...
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies", nil, http.StatusForbidden, &ts)
	tpParams = teamPolicyRequest{
		Name:  "noTeamPolicy1",
		Query: "SELECT 1;",
	}
	r = teamPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/teams/0/policies", tpParams, http.StatusForbidden, &r)
	tc = countTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/teams/0/policies/count", nil, http.StatusForbidden, &tc)
	deletePolicyParams = deleteTeamPoliciesRequest{
		IDs: []uint{noTeamPolicy2.ID},
	}
	s.DoJSON("POST", "/api/latest/mdmlab/teams/0/policies/delete", deletePolicyParams, http.StatusForbidden, &deleteTeamPoliciesResponse{})
}
...
func (s *integrationEnterpriseTestSuite) TestTeamQueries() {
	...
	// create global query
	params := mdmlab.QueryPayload{
		Name:  ptr.String("global1"),
		Query: ptr.String("select * from time;"),
	}
	var createQueryResp createQueryResponse
	s.DoJSON("POST", "/api/latest/mdmlab/queries", &params, http.StatusOK, &createQueryResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/queries", nil, http.StatusOK, &listQueriesResp, "team_id", fmt.Sprint(team1.ID))
	require.Len(t, listQueriesResp.Queries, 1)
	assert.Equal(t, "team1", listQueriesResp.Queries[0].Name)

	// list merged team queries
	s.DoJSON("GET", "/api/latest/mdmlab/queries", nil, http.StatusOK, &listQueriesResp, "team_id", fmt.Sprint(team1.ID), "merge_inherited", "true", "order_key", "team_id", "order_direction", "desc")
...
func (s *integrationEnterpriseTestSuite) TestModifyTeamEnrollSecrets() {
	...
	team := &mdmlab.Team{
		...
		Secrets:     []*mdmlab.EnrollSecret{{Secret: "initialSecret"}},
	}
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", team.ID), req, http.StatusOK, &resp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", team.ID), json.RawMessage(`{"secrets": []}`), http.StatusOK, &resp)
	require.Len(t, resp.Secrets, 0)

	// Test bad requests
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", team.ID), json.RawMessage(`{"foo": [{"secret": "testSecret3"}]}`), http.StatusUnprocessableEntity, &resp)
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", team.ID), json.RawMessage(`{}`), http.StatusUnprocessableEntity, &resp)

	// too many secrets
	secrets := createEnrollSecrets(t, mdmlab.MaxEnrollSecretsCount+1)
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", team.ID), json.RawMessage(`{"secrets": `+string(jsonMustMarshal(t, secrets))+`}`), http.StatusUnprocessableEntity, &resp)
}
...
func (s *integrationEnterpriseTestSuite) TestAvailableTeams() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/users/%d", user.ID), nil, http.StatusOK, &getResp)
	...
	user.GlobalRole = nil
	user.Teams = []mdmlab.UserTeam{{Team: *team, Role: "maintainer"}}
	...
	require.NoError(t, err)
	resp := s.DoRawWithHeaders("GET", "/api/latest/mdmlab/me", []byte(""), http.StatusOK, map[string]string{
...
func (s *integrationEnterpriseTestSuite) TestTeamEndpoints() {
	...
	team := &mdmlab.Team{
		...
		Secrets:     []*mdmlab.EnrollSecret{{Secret: "DEF"}},
	}

	var tmResp teamResponse
	s.DoJSON("POST", "/api/latest/mdmlab/teams", team, http.StatusOK, &tmResp)
	...
	team2 := &mdmlab.Team{
		...
		Secrets:     []*mdmlab.EnrollSecret{{Secret: "GHI"}},
	}
	tmResp.Team = nil
	s.DoJSON("POST", "/api/latest/mdmlab/teams", team2, http.StatusConflict, &tmResp)

	// create a team with reserved team names; should be case-insensitive
	teamReserved := &mdmlab.Team{
		Name:        "no TeAm",
		Description: "description",
		Secrets:     []*mdmlab.EnrollSecret{{Secret: "foobar"}},
	}

	r := s.Do("POST", "/api/latest/mdmlab/teams", teamReserved, http.StatusUnprocessableEntity)
	...
	tmResp.Team = nil
	s.DoJSON("POST", "/api/latest/mdmlab/teams", team3, http.StatusUnprocessableEntity, &tmResp)

	// create a team with invalid host expiry window
	team4 := &mdmlab.TeamPayload{
		Name:        ptr.String(name + "invalid host_expiry_window"),
		Description: ptr.String("Team4 description"),
		Secrets:     []*mdmlab.EnrollSecret{{Secret: "TEAM4"}},
		HostExpirySettings: &mdmlab.HostExpirySettings{
			HostExpiryEnabled: true,
			HostExpiryWindow:  -1,
		},
	}
	s.DoJSON("POST", "/api/latest/mdmlab/teams", team4, http.StatusUnprocessableEntity, &tmResp)

	// list teams matching name of one team
	var listResp listTeamsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/teams", nil, http.StatusOK, &listResp, "query", name, "per_page", "2")
	...
	s.DoJSON("GET", "/api/latest/mdmlab/teams", nil, http.StatusOK, &listResp, "query", " nope ", "per_page", "2")
	require.Len(t, listResp.Teams, 0)

	// get team
	var getResp getTeamResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), nil, http.StatusOK, &getResp)
	...
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), team, http.StatusOK, &tmResp)
	assert.Contains(t, tmResp.Team.Description, "Alt ")

	// modify team's disk encryption, impossible without mdm enabled
	res := s.Do("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), mdmlab.TeamPayload{
		MDM: &mdmlab.TeamPayloadMDM{
			EnableDiskEncryption: optjson.SetBool(true),
		},
	}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	assert.Contains(t, errMsg, `Couldn't update macos_settings because MDM features aren't turned on in MDMlab.`)
	...
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID+1), team, http.StatusNotFound, &tmResp)

	// modify team host expiry
	modifyExpiry := mdmlab.TeamPayload{
		HostExpirySettings: &mdmlab.HostExpirySettings{
			HostExpiryEnabled: true,
			HostExpiryWindow:  10,
		},
	}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), modifyExpiry, http.StatusOK, &tmResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), modifyExpiry, http.StatusUnprocessableEntity, &tmResp)

	// try to rename to reserved names
	r = s.Do("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), mdmlab.TeamPayload{Name: ptr.String("no TEAM")}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(r.Body), `"No team" is a reserved team name`)

	r = s.Do("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), mdmlab.TeamPayload{Name: ptr.String("ALL teAMs")}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(r.Body), `"All teams" is a reserved team name`)

	// Modify team's calendar config
	modifyCalendar := mdmlab.TeamPayload{
		Integrations: &mdmlab.TeamIntegrations{
			GoogleCalendar: &mdmlab.TeamGoogleCalendarIntegration{
				WebhookURL: "https://example.com/modified",
			},
		},
	}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), modifyCalendar, http.StatusOK, &tmResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), modifyCalendar, http.StatusUnprocessableEntity, &tmResp)

	// list team users
	var usersResp listUsersResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), nil, http.StatusOK, &usersResp)
	assert.Len(t, usersResp.Users, 0)

	// list team users - non-existing team
	usersResp.Users = nil
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID+1), nil, http.StatusNotFound, &usersResp)
	...
	// add a team user
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mdmlab.TeamUser{{User: *user, Role: mdmlab.RoleObserver}}}, http.StatusOK, &tmResp)
	require.Len(t, tmResp.Team.Users, 1)
	assert.Equal(t, user.ID, tmResp.Team.Users[0].ID)

	// add a team user - non-existing team
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID+1), modifyTeamUsersRequest{Users: []mdmlab.TeamUser{{User: *user, Role: mdmlab.RoleObserver}}}, http.StatusNotFound, &tmResp)

	// add a team user - invalid user role
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mdmlab.TeamUser{{User: *user, Role: "foobar"}}}, http.StatusUnprocessableEntity, &tmResp)

	// search for that user
	usersResp.Users = nil
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), nil, http.StatusOK, &usersResp, "query", "user")
	require.Len(t, usersResp.Users, 1)
	assert.Equal(t, user.ID, usersResp.Users[0].ID)

	// search for unknown user
	usersResp.Users = nil
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), nil, http.StatusOK, &usersResp, "query", "notauser")
	require.Len(t, usersResp.Users, 0)

	// delete team user
	tmResp.Team = nil
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mdmlab.TeamUser{{User: mdmlab.User{ID: user.ID}}}}, http.StatusOK, &tmResp)
	require.Len(t, tmResp.Team.Users, 0)

	// delete team user - unknown user
	tmResp.Team = nil
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mdmlab.TeamUser{{User: mdmlab.User{ID: user.ID + 1}}}}, http.StatusOK, &tmResp)
	require.Len(t, tmResp.Team.Users, 0)

	// delete team user - unknown team
	tmResp.Team = nil
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", tm1ID+1), modifyTeamUsersRequest{Users: []mdmlab.TeamUser{{User: mdmlab.User{ID: user.ID}}}}, http.StatusNotFound, &tmResp)

	// modify team agent options with invalid options
	tmResp.Team = nil
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", tm1ID), json.RawMessage(`{
		"x": "y"
	}`), http.StatusBadRequest, &tmResp)

	// modify team agent options with invalid key
	badRes := s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", tm1ID), json.RawMessage(`{
		"bad_key": 1
	...
	tmResp.Team = nil
	resp := s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", tm1ID), json.RawMessage(`{
		"config": {
			"options": {
	...
	s.lastActivityMatches(mdmlab.ActivityTypeEditedAgentOptions{}.ActivityName(), fmt.Sprintf(`{"global": false, "team_id": %d, "team_name": %q}`, tm1ID, team.Name), 0)

	// modify team agent options - unknown team
	tmResp.Team = nil
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", tm1ID+1), json.RawMessage(`{}`), http.StatusNotFound, &tmResp)

	// get team enroll secrets
	var secResp teamEnrollSecretsResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", tm1ID), nil, http.StatusOK, &secResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", tm1ID+1), nil, http.StatusOK, &secResp)
	assert.Len(t, secResp.Secrets, 0)

	// delete team
	var delResp deleteTeamResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), nil, http.StatusOK, &delResp)

	// delete team again, now an unknown team
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tm1ID), nil, http.StatusNotFound, &delResp)
}
...
func (s *integrationEnterpriseTestSuite) TestTeamSecretsAreObfuscated() {
	...
	users := []*mdmlab.User{global_obs, global_obs_plus, team_obs, team_obs_plus}
	for _, u := range users {
	...
	// --------------------------------------------------------------------
	for _, u := range []*mdmlab.User{global_obs, global_obs_plus} {

		s.setTokenForTest(t, u.Email, test.GoodPassword)

		// list all teams
		var listResp listTeamsResponse
		s.DoJSON("GET", "/api/latest/mdmlab/teams", nil, http.StatusOK, &listResp)
		...
		for _, team := range listResp.Teams {
			for _, secret := range team.Secrets {
				require.Equal(t, mdmlab.MaskedPassword, secret.Secret)
			}
		}
		...
		for _, team := range teams {
			var getResp getTeamResponse
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), nil, http.StatusOK, &getResp)
			...
			var secResp teamEnrollSecretsResponse
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", team.ID), nil, http.StatusOK, &secResp)
	...
	// --------------------------------------------------------------------
	for _, u := range []*mdmlab.User{team_obs, team_obs_plus} {
		...
		for _, team := range listResp.Teams {
			for _, secret := range team.Secrets {
				...
				if u.ID == team_obs.ID {
					require.Equal(t, mdmlab.MaskedPassword == secret.Secret, team.ID == teams[0].ID)
					require.Equal(t, mdmlab.MaskedPassword != secret.Secret, team.ID == teams[1].ID)
				}
...
func (s *integrationEnterpriseTestSuite) TestExternalIntegrationsTeamConfig() {
	...
	team := &mdmlab.Team{
		Name:        t.Name(),
		Description: "Team description",
		Secrets:     []*mdmlab.EnrollSecret{{Secret: "XYZ"}},
	}
	var tmResp teamResponse
	s.DoJSON("POST", "/api/latest/mdmlab/teams", team, http.StatusOK, &tmResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), mdmlab.TeamPayload{WebhookSettings: &mdmlab.TeamWebhookSettings{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), mdmlab.TeamPayload{Integrations: &mdmlab.TeamIntegrations{
	...
	s.DoRaw("PATCH", "/api/v1/mdmlab/config", []byte(fmt.Sprintf(`{
		"integrations": {
			"jira": [
				{
					"url": %q,
	...
	tmResp = teamResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), mdmlab.TeamPayload{
		Integrations: &mdmlab.TeamIntegrations{
			Jira: []*mdmlab.TeamJiraIntegration{
				{
					URL:                   srvURL,
	...
	s.DoJSON("PATCH", "/api/v1/mdmlab/config", json.RawMessage(`{
		"org_info": {
...
func (s *integrationEnterpriseTestSuite) TestWindowsUpdatesTeamConfig() {
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), map[string]any{
		"mdm": map[string]any{
			"windows_updates": &mdmlab.WindowsUpdates{
	...
	s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "deadline_days": 5, "grace_period_days": 2}`, team.ID, team.Name), 0)

	checkWindowsOSUpdatesProfile(t, s.ds, &team.ID, &mdmlab.WindowsUpdates{
		DeadlineDays:    optjson.SetInt(5),
		GracePeriodDays: optjson.SetInt(2),
	})
	...
	require.Equal(t, 2, tmResp.Team.Config.MDM.WindowsUpdates.GracePeriodDays.Value)
	lastActivity := s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "deadline_days": 6, "grace_period_days": 2}`, team.ID, team.Name), 0)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), map[string]any{
		"mdm": map[string]any{
			"macos_updates": &mdmlab.AppleOSUpdateSettings{
				MinimumVersion: optjson.SetString("10.15.0"),
				Deadline:       optjson.SetString("2021-01-01"),
			},
		},
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), "", lastActivity)
	lastActivity = s.lastActivityMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), ``, 0)
	...
	s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "deadline_days": null, "grace_period_days": null}`, team.ID, team.Name), 0)
...
func (s *integrationEnterpriseTestSuite) assertAppleOSUpdatesDeclaration(teamID *uint, profileName string, expected *mdmlab.AppleOSUpdateSettings) {
	t := s.T()
	if teamID == nil {
		teamID = ptr.Uint(0)
	}
...
func (s *integrationEnterpriseTestSuite) TestAppleOSUpdatesTeamConfig() {
	...
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabMacOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabIOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabIPadOSUpdatesProfileName, nil)
	...
	require.Equal(t, "10.15.0", tmResp.Team.Config.MDM.MacOSUpdates.MinimumVersion.Value)
	require.Equal(t, "2021-01-01", tmResp.Team.Config.MDM.MacOSUpdates.Deadline.Value)
	s.lastActivityMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "10.15.0", "deadline": "2021-01-01"}`, team.ID, team.Name), 0)

	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabMacOSUpdatesProfileName, macOSUpdates)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabIOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabIPadOSUpdatesProfileName, nil)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "10.15.0", "deadline": "2021-01-01"}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "11.11.11", "deadline": "2022-02-02"}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "12.12.12", "deadline": "2023-03-03"}`, team.ID, team.Name), 0)

	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabMacOSUpdatesProfileName, macOSUpdates)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabIOSUpdatesProfileName, iOSUpdates)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MDMlabIPadOSUpdatesProfileName, iPadOSUpdates)
	...
	macOSLastActivity := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "10.15.0", "deadline": "2025-10-01"}`, team.ID, team.Name), 0)
	iOSLastActivity := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "11.11.11", "deadline": "2024-02-02"}`, team.ID, team.Name), 0)
	iPadOSLastActivity := s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "12.12.12", "deadline": "2024-03-03"}`, team.ID, team.Name), 0)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), "", macOSLastActivity)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), "", iOSLastActivity)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), "", iPadOSLastActivity)
	lastActivity := s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), ``, 0)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "", "deadline": ""}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "", "deadline": ""}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "", "deadline": ""}`, team.ID, team.Name), 0)
...
func (s *integrationEnterpriseTestSuite) TestLinuxDiskEncryption() {
	...
	// create a Linux host
	noTeamHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
	...
	team, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{Name: "A team"})
	require.NoError(t, err)
	teamID := ptr.Uint(team.ID)
	teamHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &profileSummary)
	require.Equal(t, mdmlab.MDMProfilesSummary{}, profileSummary.MDMProfilesSummary)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", noTeamHost.ID), nil, http.StatusOK, &getHostResp)
	...
	res := s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+deviceToken, nil, http.StatusOK)
	err = json.NewDecoder(res.Body).Decode(&getDeviceHostResp)
	require.NoError(t, err)
	require.Nil(t, getHostResp.Host.MDM.OSSettings)

	// turn on disk encryption enforcement
	s.Do("POST", "/api/latest/mdmlab/disk_encryption", updateDiskEncryptionRequest{EnableDiskEncryption: true}, http.StatusNoContent)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &profileSummary)
	require.Equal(t, mdmlab.MDMProfilesSummary{Pending: 1}, profileSummary.MDMProfilesSummary)

	// encryption summary should succeed (Linux encryption doesn't require MDM)
	var summary getMDMDiskEncryptionSummaryResponse
	s.DoJSON("GET", "/api/latest/mdmlab/mdm/disk_encryption/summary", getMDMDiskEncryptionSummaryRequest{}, http.StatusOK, &summary)
	s.DoJSON("GET", "/api/latest/mdmlab/disk_encryption", getMDMDiskEncryptionSummaryRequest{}, http.StatusOK, &summary)
	// disk is encrypted but key hasn't been escrowed yet
	require.Equal(t, mdmlab.MDMDiskEncryptionSummary{ActionRequired: mdmlab.MDMPlatformsCounts{Linux: 1}}, *summary.MDMDiskEncryptionSummary)

	// trigger escrow process from device
	// should fail because default Orbit version is too old
	res = s.DoRawNoAuth("POST", fmt.Sprintf("/api/latest/mdmlab/device/%s/mdm/linux/trigger_escrow", deviceToken), nil, http.StatusBadRequest)
	res.Body.Close()

	// should succeed now that Orbit version isn't too old
	require.NoError(t, s.ds.SetOrUpdateHostOrbitInfo(context.Background(), noTeamHost.ID, mdmlab.MinOrbitLUKSVersion, sql.NullString{}, sql.NullBool{}))
	res = s.DoRawNoAuth("POST", fmt.Sprintf("/api/latest/mdmlab/device/%s/mdm/linux/trigger_escrow", deviceToken), nil, http.StatusNoContent)
	res.Body.Close()

	// confirm that Orbit endpoint shows notification flag
	var orbitResponse orbitGetConfigResponse
	s.DoJSON("POST", "/api/mdmlab/orbit/config", orbitGetConfigRequest{OrbitNodeKey: orbitKey}, http.StatusOK, &orbitResponse)
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/config", orbitGetConfigRequest{OrbitNodeKey: orbitKey}, http.StatusOK, &secondOrbitResponse)
	...
	s.Do("POST", "/api/mdmlab/orbit/luks_data", orbitPostLUKSRequest{
	...
	// confirm verified
	s.DoJSON("GET", "/api/latest/mdmlab/disk_encryption", getMDMDiskEncryptionSummaryRequest{}, http.StatusOK, &summary)
	require.Equal(t, mdmlab.MDMDiskEncryptionSummary{Verified: mdmlab.MDMPlatformsCounts{Linux: 1}}, *summary.MDMDiskEncryptionSummary)

	// get passphrase back
	var keyResponse getHostEncryptionKeyResponse
	s.DoJSON("GET", fmt.Sprintf(`/api/latest/mdmlab/mdm/hosts/%d/encryption_key`, noTeamHost.ID), getHostEncryptionKeyRequest{}, http.StatusOK, &keyResponse)
	s.DoJSON("GET", fmt.Sprintf(`/api/latest/mdmlab/hosts/%d/encryption_key`, noTeamHost.ID), getHostEncryptionKeyRequest{}, http.StatusOK, &keyResponse)
	require.Equal(t, "whale makes pail rise", keyResponse.EncryptionKey.DecryptedValue)

	// TEAM //
	s.DoJSON("GET", "/api/latest/mdmlab/configuration_profiles/summary", getMDMProfilesSummaryRequest{TeamID: teamID}, http.StatusOK, &profileSummary)
	require.Equal(t, mdmlab.MDMProfilesSummary{}, profileSummary.MDMProfilesSummary)
	...
	s.Do("POST", "/api/latest/mdmlab/disk_encryption", updateDiskEncryptionRequest{TeamID: teamID, EnableDiskEncryption: true}, http.StatusNoContent)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/disk_encryption", getMDMDiskEncryptionSummaryRequest{TeamID: teamID}, http.StatusOK, &summary)
	require.Equal(t, mdmlab.MDMDiskEncryptionSummary{ActionRequired: mdmlab.MDMPlatformsCounts{Linux: 1}}, *summary.MDMDiskEncryptionSummary)
	...
	// get passphrase back
	s.DoJSON("GET", fmt.Sprintf(`/api/latest/mdmlab/hosts/%d/encryption_key`, teamHost.ID), getHostEncryptionKeyRequest{}, http.StatusOK, &keyResponse)
...
func (s *integrationEnterpriseTestSuite) TestListDevicePolicies() {
	...
	s.DoJSON("PATCH", "/api/latest/mdmlab/config",
		json.RawMessage(`{
		"org_info":{
	...
	team, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		ID:          51,
		Name:        "team1-policies",
		Description: "desc team1",
	})
	...
	qr, err := s.ds.NewQuery(ctx, &mdmlab.Query{
	...
	s.DoJSON("POST", "/api/latest/mdmlab/policies", gpParams, http.StatusOK, &gpResp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team.ID), tpParams, http.StatusOK, &tpResp)

	// try with invalid token
	res := s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/invalid_token/policies", nil, http.StatusUnauthorized)
	err = res.Body.Close()
	require.NoError(t, err)

	// GET `/api/_version_/mdmlab/device/{token}/policies`
	listDevicePoliciesResp := listDevicePoliciesResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/policies", nil, http.StatusOK)
	...
	// GET `/api/_version_/mdmlab/device/{token}`
	getDeviceHostResp := getDeviceHostResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token, nil, http.StatusOK)
	err = json.NewDecoder(res.Body).Decode(&getDeviceHostResp)
	require.NoError(t, err)
	err = res.Body.Close()
	require.NoError(t, err)
	...
	// GET `/api/_version_/mdmlab/device/{token}/desktop`
	getDesktopResp := mdmlabDesktopResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/desktop", nil, http.StatusOK)
...
// TestCustomTransparencyURL tests that MDMlab Premium licensees can use custom transparency urls.
func (s *integrationEnterpriseTestSuite) TestCustomTransparencyURL() {
	...
	s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
	require.NotNil(t, acResp)
	require.Equal(t, mdmlab.DefaultTransparencyURL, acResp.MDMlabDesktop.TransparencyURL)

	// confirm device endpoint returns initial default url
	deviceResp := &transparencyURLResponse{}
	rawResp := s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/transparency", nil, http.StatusTemporaryRedirect)
	...
	require.Equal(t, mdmlab.DefaultTransparencyURL, rawResp.Header.Get("Location"))

	// set custom url
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{"mdmlab_desktop":{"transparency_url": "customURL"}}`), http.StatusOK, &acResp)
	require.NotNil(t, acResp)
	require.Equal(t, "customURL", acResp.MDMlabDesktop.TransparencyURL)
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{"mdmlab_desktop":{"transparency_url": ""}}`), http.StatusOK, &acResp)
	require.NotNil(t, acResp)
	require.Equal(t, mdmlab.DefaultTransparencyURL, acResp.MDMlabDesktop.TransparencyURL)
...
func (s *integrationEnterpriseTestSuite) TestMDMWindowsUpdates() {
	...
	s.DoJSON("GET", "/api/latest/mdmlab/activities", nil, http.StatusOK, &activitiesResp, "order_key", "a.id", "order_direction", "desc")
	...
	checkInvalidConfig := func(config string) {
		...
		acResp := appConfigResponse{}
		s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(config), http.StatusUnprocessableEntity, &acResp)

		// get the appconfig, nothing changed
		acResp = appConfigResponse{}
		s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
		require.Equal(t, mdmlab.WindowsUpdates{DeadlineDays: optjson.Int{Set: true}, GracePeriodDays: optjson.Int{Set: true}}, acResp.MDM.WindowsUpdates)
	...
	checkWindowsOSUpdatesProfile(t, s.ds, nil, &mdmlab.WindowsUpdates{
		DeadlineDays:    optjson.SetInt(5),
		GracePeriodDays: optjson.SetInt(1),
	})

	// edited windows updates activity got created
	s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), `{"deadline_days":5, "grace_period_days":1, "team_id": null, "team_name": null}`, 0)
	...
	lastActivity = s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), `{"deadline_days":6, "grace_period_days":1, "team_id": null, "team_name": null}`, 0)
	...
	lastActivity = s.lastActivityMatches(mdmlab.ActivityTypeEditedWindowsUpdates{}.ActivityName(), `{"deadline_days":null, "grace_period_days":null, "team_id": null, "team_name": null}`, 0)
...
func (s *integrationEnterpriseTestSuite) TestMDMAppleOSUpdates() {
	...
	checkInvalidConfig := func(config string) {
		...
		acResp = appConfigResponse{}
		s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acResp)
		require.Equal(t, mdmlab.AppleOSUpdateSettings{MinimumVersion: optjson.String{Set: true}, Deadline: optjson.String{Set: true}}, acResp.MDM.MacOSUpdates)
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), `{"deadline":"2022-01-01", "minimum_version":"12.3.1", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), `{"deadline":"2023-03-03", "minimum_version":"13.13.13", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), `{"deadline":"2024-04-04", "minimum_version":"14.14.14", "team_id": null, "team_name": null}`, 0)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabMacOSUpdatesProfileName, &mdmlab.AppleOSUpdateSettings{
		MinimumVersion: optjson.SetString("12.3.1"), Deadline: optjson.SetString("2022-01-01"),
	})
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabIOSUpdatesProfileName, &mdmlab.AppleOSUpdateSettings{
		MinimumVersion: optjson.SetString("13.13.13"), Deadline: optjson.SetString("2023-03-03"),
	})
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabIPadOSUpdatesProfileName, &mdmlab.AppleOSUpdateSettings{
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), `{"deadline":"2024-01-01", "minimum_version":"12.3.1", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), `{"deadline":"2025-05-05", "minimum_version":"13.13.13", "team_id": null, "team_name": null}`, 0)
	lastActivity = s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), `{"deadline":"2026-06-06", "minimum_version":"14.14.14", "team_id": null, "team_name": null}`, 0)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabMacOSUpdatesProfileName, &mdmlab.AppleOSUpdateSettings{
	...
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), `{"deadline":"", "minimum_version":"", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIOSMinVersion{}.ActivityName(), `{"deadline":"", "minimum_version":"", "team_id": null, "team_name": null}`, 0)
	lastActivity = s.lastActivityOfTypeMatches(mdmlab.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), `{"deadline":"", "minimum_version":"", "team_id": null, "team_name": null}`, 0)

	// check DDM profiles were removed
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabMacOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabIOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MDMlabIPadOSUpdatesProfileName, nil)
...
func (s *integrationEnterpriseTestSuite) TestInvitedUserMFA() {
	...
	// create valid invite
	createInviteReq := createInviteRequest{InvitePayload: mdmlab.InvitePayload{
		Email:      ptr.String("some email"),
		Name:       ptr.String("some name"),
		GlobalRole: null.StringFrom(mdmlab.RoleAdmin),
		MFAEnabled: ptr.Bool(true),
		SSOEnabled: ptr.Bool(true),
	}}
	createInviteResp := createInviteResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/invites", createInviteReq, http.StatusConflict, &createInviteResp)
	createInviteReq.SSOEnabled = nil
	s.DoJSON("POST", "/api/latest/mdmlab/invites", createInviteReq, http.StatusOK, &createInviteResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/invites/"+validInviteToken, nil, http.StatusOK, &verifyInvResp)
	require.Equal(t, validInvite.ID, verifyInvResp.Invite.ID)

	var createFromInviteResp createUserResponse
	s.DoJSON("POST", "/api/latest/mdmlab/users", mdmlab.UserPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/invites/%d", validInvite.ID), updateInviteRequest{
		InvitePayload: mdmlab.InvitePayload{MFAEnabled: ptr.Bool(true), SSOEnabled: ptr.Bool(false)},
...
func (s *integrationEnterpriseTestSuite) TestSSOJITProvisioning() {
	...
	require.Condition(t, func() bool {
		for _, a := range activitiesResp.Activities {
			if (a.Type == mdmlab.ActivityTypeUserAddedBySSO{}.ActivityName()) && *a.ActorEmail == auth.UserID() {
				return true
			}
		}
		return false
	...
	assert.Contains(t, auth.AssertionAttributes(), mdmlab.SAMLAttribute{
	...
	_, err = s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:        "team_" + t.Name(),
	...
	// FLEET_JIT_USER_ROLE_* attributes with value `null` are ignored by MDMlab.
	assert.Contains(t, auth.AssertionAttributes(), mdmlab.SAMLAttribute{
		Name: "FLEET_JIT_USER_ROLE_GLOBAL",
		Values: []mdmlab.SAMLAttributeValue{{
			Value: "null",
...
`)
	...
	team, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		ID:          8324,
		Name:        "team1_" + t.Name(),
	...
	host, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
	...
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_users")
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_software_macos")
	require.NotContains(t, dqResp.Queries, "mdmlab_additional_query_time")
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.NotContains(t, dqResp.Queries, "mdmlab_detail_query_users")
	require.NotContains(t, dqResp.Queries, "mdmlab_detail_query_software_macos")
	require.Contains(t, dqResp.Queries, "mdmlab_additional_query_time")
}
...
func (s *integrationEnterpriseTestSuite) TestListHosts() {
	...
	host1, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	host2, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	host3, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/labels/%d/hosts", allHostsLabel.ID), nil, http.StatusOK, &resp, "low_disk_space", "32")
	require.Len(t, resp.Hosts, 1)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "low_disk_space", "32")
	require.Len(t, resp.Hosts, 1)
	assert.Equal(t, host1.ID, resp.Hosts[0].ID)
	assert.Equal(t, 10.0, resp.Hosts[0].GigsDiskSpaceAvailable)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "low_disk_space", "100")
	require.Len(t, resp.Hosts, 2)

	// returns an error when the low_disk_space value is invalid (outside 1-100)
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusBadRequest, &resp, "low_disk_space", "101")
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusBadRequest, &resp, "low_disk_space", "0")

	// counting hosts works with and without the filter too
	var countResp countHostsResponse
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp)
	require.Equal(t, 3, countResp.Count)
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "low_disk_space", "32")
	require.Equal(t, 1, countResp.Count)
	s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "low_disk_space", "100")
	require.Equal(t, 2, countResp.Count)

	// host summary returns counts for low disk space
	var summaryResp getHostSummaryResponse
	s.DoJSON("GET", "/api/latest/mdmlab/host_summary", nil, http.StatusOK, &summaryResp, "low_disk_space", "32")
	...
	s.DoJSON("GET", "/api/latest/mdmlab/host_summary", nil, http.StatusOK, &summaryResp, "platform", "windows", "low_disk_space", "32")
	...
	summaryResp = getHostSummaryResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/host_summary", nil, http.StatusOK, &summaryResp, "team_id", "1", "platform", "linux", "low_disk_space", "32")
	...
	summaryResp = getHostSummaryResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/host_summary", nil, http.StatusOK, &summaryResp, "team_id", "1", "platform", "linux")
	require.Equal(t, uint(0), summaryResp.TotalsHostsCount)
	...
	inserted, err := s.ds.InsertSoftwareVulnerability(context.Background(), mdmlab.SoftwareVulnerability{
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "populate_software", "true")
	require.Len(t, resp.Hosts, 3)
	for _, h := range resp.Hosts {
		if h.ID == host1.ID {
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "populate_software", "without_vulnerability_details")
	require.Len(t, resp.Hosts, 3)
	for _, h := range resp.Hosts {
		if h.ID == host1.ID {
			require.NotEmpty(t, h.Software)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "populate_software", "false")
	require.Len(t, resp.Hosts, 3)
	for _, h := range resp.Hosts {
		require.Empty(t, h.Software)
	}

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/labels/%d/hosts", allHostsLabel.ID), nil, http.StatusOK, &resp)
	...
	s.DoJSON(
		"GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "order_key", "issues",
	) // defaults to ascending order (lowest issues to most issues)
	require.Len(t, resp.Hosts, 3)
	assert.Equal(t, host1.ID, resp.Hosts[2].ID)
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/labels/%d/hosts", allHostsLabel.ID), nil, http.StatusOK, &resp, "order_key", "issues",
...
func (s *integrationEnterpriseTestSuite) TestHostHealth() {
	...
	passingTeamPolicy, err := s.ds.NewTeamPolicy(context.Background(), team.ID, nil, mdmlab.PolicyPayload{
	...
	failingTeamPolicy, err := s.ds.NewTeamPolicy(context.Background(), team.ID, nil, mdmlab.PolicyPayload{
	...
	passingGlobalPolicy, err := s.ds.NewGlobalPolicy(context.Background(), nil, mdmlab.PolicyPayload{
		Name:       "Passing Global Policy",
		Query:      "select 1",
		Resolution: "Run this command to fix it",
	})
	...
	failingGlobalPolicy, err := s.ds.NewGlobalPolicy(context.Background(), nil, mdmlab.PolicyPayload{
		Name:       "Failing Global Policy",
		Query:      "select 1",
		Resolution: "Run this command to fix it",
		Critical:   false,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/health", host.ID), nil, http.StatusOK, &hh)
	...
	assert.Contains(t, hh.HostHealth.FailingPolicies, &mdmlab.HostHealthFailingPolicy{
...
func (s *integrationEnterpriseTestSuite) TestListVulnerabilities() {
	...
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities", nil, http.StatusOK, &resp)

	// Invalid Order Key
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities", nil, http.StatusBadRequest, &resp, "order_key", "foo", "order_direction", "asc")

	// EE Only Order Key
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities", nil, http.StatusOK, &resp, "order_key", "cvss_score", "order_direction", "asc")
	...
	err = s.ds.UpdateHostOperatingSystem(context.Background(), host.ID, mdmlab.OperatingSystem{
	...
	_, err = s.ds.InsertOSVulnerability(context.Background(), mdmlab.OSVulnerability{
		OSID:              os.ID,
		CVE:               "CVE-2021-1234",
	...
	res, err := s.ds.UpdateHostSoftware(context.Background(), host.ID, []mdmlab.Software{
		{Name: "foo", Version: "0.0.1", Source: "chrome_extensions"},
	})
	require.NoError(t, err)
	sw := res.Inserted[0]

	_, err = s.ds.InsertSoftwareVulnerability(context.Background(), mdmlab.SoftwareVulnerability{
		SoftwareID: sw.ID,
		CVE:        "CVE-2021-1235",
	}, mdmlab.NVDSource)
	require.NoError(t, err)
	...
	err = s.ds.InsertCVEMeta(context.Background(), []mdmlab.CVEMeta{
		{
			CVE:              "CVE-2021-1234",
	...
	expected := map[string]struct {
		...
		HostCount   uint
		DetailsLink string
		Source      mdmlab.VulnerabilitySource
	}{
		"CVE-2021-1234": {
			HostCount:   1,
	...
	// EE Exploit Filter
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities", nil, http.StatusOK, &resp, "exploit", "true")
	require.Len(t, resp.Vulnerabilities, 1)
	require.Equal(t, "CVE-2021-1234", resp.Vulnerabilities[0].CVE.CVE)

	// Test Team Filter
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities", nil, http.StatusOK, &resp, "team_id", "1")
	require.Len(s.T(), resp.Vulnerabilities, 0)

	team, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	err = s.ds.AddHostsToTeam(context.Background(), &team.ID, []uint{host.ID})
	require.NoError(t, err)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities", nil, http.StatusOK, &resp, "team_id", fmt.Sprintf("%d", team.ID))
	...
	s.DoJSON("GET", "/api/latest/mdmlab/vulnerabilities/CVE-2021-1234", nil, http.StatusOK, &gResp)
...
func (s *integrationEnterpriseTestSuite) TestOSVersions() {
	...
	testOS := mdmlab.OperatingSystem{Name: "Windows 11 Pro", Version: "10.0.22621.2861", Arch: "x86_64", KernelVersion: "10.0.22621.2861", Platform: "windows"}
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "os_name", testOS.Name, "os_version", testOS.Version)
	require.Len(t, resp.Hosts, 1)

	expected := resp.Hosts[0]
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &resp, "os_id", fmt.Sprintf("%d", osinfo.ID))
	require.Len(t, resp.Hosts, 1)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/os_versions", nil, http.StatusOK, &osVersionsResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/os_versions/%d", osinfo.OSVersionID), nil, http.StatusOK, &osVersionResp)
	require.Equal(t, &expectedOSVersion, osVersionResp.OSVersion)

	// OS versions with invalid team
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/os_versions/%d", osinfo.OSVersionID), nil, http.StatusNotFound, &osVersionResp, "team_id",
		"99999",
	)
	...
	tr := teamResponse{}
	s.DoJSON(
		"POST", "/api/latest/mdmlab/teams", createTeamRequest{
			TeamPayload: mdmlab.TeamPayload{
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/os_versions/%d", osinfo.OSVersionID), nil, http.StatusOK, &osVersionResp, "team_id",
	...
	team0, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{Name: "new team"})
	require.NoError(t, err)
	require.NoError(t, s.ds.AddHostsToTeam(context.Background(), &team0.ID, []uint{hosts[0].ID}))
	s.DoJSON("GET", "/api/latest/mdmlab/os_versions", nil, http.StatusOK, &osVersionsResp, "team_id", fmt.Sprintf("%d", team0.ID))
	require.Len(t, osVersionsResp.OSVersions, 0)

	// return err if team_id is invalid
	s.DoJSON("GET", "/api/latest/mdmlab/os_versions", nil, http.StatusBadRequest, &osVersionsResp, "team_id", "invalid")

	// Create another team and a team user
	team1, err := s.ds.NewTeam(
		context.Background(), &mdmlab.Team{
			ID:          42,
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/os_versions/%d", osinfo.OSVersionID), nil, http.StatusForbidden, &osVersionResp, "team_id",
		"99999",
	)

	// team user doesn't have acess to "no team"
	osVersionsResp = osVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/os_versions", nil, http.StatusForbidden, &osVersionsResp, "team_id", "0")
	require.Len(t, osVersionsResp.OSVersions, 0)
	...
	osVersionsResp = osVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/os_versions", nil, http.StatusOK, &osVersionsResp, "team_id", "0")
	require.Len(t, osVersionsResp.OSVersions, 0)
	osVersionsResp = osVersionsResponse{}
	...
	require.Len(t, osVersionsResp.OSVersions, 1)
	osVersionResp = getOSVersionResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/os_versions/%d", osinfo.OSVersionID), nil, http.StatusOK, &osVersionResp, "team_id", "0")
	require.Equal(t, &expectedOSVersion, osVersionResp.OSVersion)
...
func (s *integrationEnterpriseTestSuite) TestMDMNotConfiguredEndpoints() {
	...
	for _, route := range mdmConfigurationRequiredEndpoints() {
		var expectedErr mdmlab.ErrWithStatusCode = mdmlab.ErrMDMNotConfigured
		path := route.path
	...
	mdmlabdmSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	t.Setenv("TEST_FLEETDM_API_URL", mdmlabdmSrv.URL)
	t.Cleanup(mdmlabdmSrv.Close)

	// Always accessible
	var reqCSRResp requestMDMAppleCSRResponse
	s.DoJSON("POST", "/api/latest/mdmlab/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusOK, &reqCSRResp)
	s.Do("POST", "/api/latest/mdmlab/mdm/apple/dep/key_pair", nil, http.StatusOK)

	// setting enable release device manually requires MDM
	res := s.Do("PATCH", "/api/v1/mdmlab/setup_experience", mdmlab.MDMAppleSetupPayload{EnableReleaseDeviceManually: ptr.Bool(true)}, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mdmlab.ErrMDMNotConfigured.Error())

	res = s.Do("PATCH", "/api/v1/mdmlab/config", json.RawMessage(`{
...
func (s *integrationEnterpriseTestSuite) TestGlobalPolicyCreateReadPatch() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/policies", createPol1Req, http.StatusOK, &createPol1)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/policies", createPol2Req, http.StatusOK, &createPol2)
	allEqual(t, createPol2Req, createPol2.Policy, fields...)

	listPol := &listGlobalPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/policies", nil, http.StatusOK, listPol)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/policies", nil, http.StatusOK, listPolByName, "query", " name1 ")
	...
	patchPol1Req := &modifyGlobalPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/policies/%d", createPol1.Policy.ID), patchPol1Req, http.StatusOK, patchPol1)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/policies/%d", createPol2.Policy.ID), patchPol2Req, http.StatusOK, patchPol2)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/policies/%d", createPol2.Policy.ID), nil, http.StatusOK, getPol2)
...
func (s *integrationEnterpriseTestSuite) TestTeamPolicyCreateReadPatch() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), createPol1Req, http.StatusOK, &createPol1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), createPol2Req, http.StatusOK, &createPol2)
	allEqual(s.T(), createPol2Req, createPol2.Policy, fields...)

	listPol := &listTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), nil, http.StatusOK, listPol)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, createPol1.Policy.ID), patchPol1Req, http.StatusOK, patchPol1)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, createPol2.Policy.ID), patchPol2Req, http.StatusOK, patchPol2)
	...
	getPol2 := &getPolicyByIDResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, createPol2.Policy.ID), nil, http.StatusOK, getPol2)
...
func (s *integrationEnterpriseTestSuite) TestResetAutomation() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team1.ID), createPol3Req, http.StatusOK, &createPol3)

	var tmResp teamResponse
	// modify the team's config - enable the webhook
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team1.ID), mdmlab.TeamPayload{WebhookSettings: &mdmlab.TeamWebhookSettings{
		FailingPoliciesWebhook: mdmlab.FailingPoliciesWebhookSettings{
			Enable:         true,
	...
	h1, err := s.ds.NewHost(ctx, &mdmlab.Host{})
	require.NoError(s.T(), err)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/automations/reset", resetAutomationRequest{
		TeamIDs:   nil,
		PolicyIDs: []uint{},
	}, http.StatusOK, &tmResp)
...
func createHostAndDeviceToken(t *testing.T, ds *mysql.Datastore, token string) *mdmlab.Host {
...
func (s *integrationEnterpriseTestSuite) TestListSoftware() {
	...
	host, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	inserted, err := s.ds.InsertSoftwareVulnerability(
		ctx, mdmlab.SoftwareVulnerability{
			SoftwareID:        bar.ID,
			CVE:               "cve-123",
	...
	require.NoError(t, s.ds.InsertCVEMeta(ctx, []mdmlab.CVEMeta{{
		CVE:              "cve-123",
		CVSSScore:        ptr.Float64(5.4),
		EPSSProbability:  ptr.Float64(0.5),
		CISAKnownExploit: ptr.Bool(true),
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software", nil, http.StatusOK, &resp)
	require.NotNil(t, resp)

	var fooPayload, barPayload mdmlab.Software
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/versions", nil, http.StatusOK, &respVersions)
	require.NotNil(t, resp)
	...
	s.DoJSON(
		"GET", "/api/latest/mdmlab/software/versions",
		listSoftwareRequest{},
...
// TestGitOpsUserActions tests the MDM permissions listed in ../../docs/Using\ MDMlab/manage-access.md
func (s *integrationEnterpriseTestSuite) TestGitOpsUserActions() {
	...
	require.NoError(t, err)
	t1, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		Name: "Foo",
	})
	require.NoError(t, err)
	t2, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		Name: "Bar",
	})
	require.NoError(t, err)
	t3, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		Name: "Zoo",
	})
	require.NoError(t, err)
	team1Host, err := s.ds.NewHost(ctx, &mdmlab.Host{
		NodeKey:  ptr.String(t.Name() + "2"),
		UUID:     t.Name() + "2",
	...
	require.NoError(t, err)
	globalHost, err := s.ds.NewHost(ctx, &mdmlab.Host{
		NodeKey:  ptr.String(t.Name() + "3"),
		UUID:     t.Name() + "3",
	...
	s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &acr)
	require.False(t, acr.WebhookSettings.VulnerabilitiesWebhook.Enable)
	q1, err := s.ds.NewQuery(ctx, &mdmlab.Query{
		Name:    "Foo",
	...
	s.DoJSON("GET", "/api/latest/mdmlab/schedule", nil, http.StatusOK, &ggsr)
	require.NoError(t, ggsr.Err)
	cpar := createPackResponse{}
	var userPackID uint
	s.DoJSON("POST", "/api/latest/mdmlab/packs", createPackRequest{
	...
	s.DoJSON("POST", "/api/latest/mdmlab/users/admin", createUserRequest{
		UserPayload: mdmlab.UserPayload{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/carves", listCarvesRequest{}, http.StatusOK, &lcr)
	...
	require.NoError(t, err)
	gp2, err := s.ds.NewGlobalPolicy(ctx, &admin.ID, mdmlab.PolicyPayload{
		Name:  "Zoo",
		Query: "SELECT 0;",
	})
	require.NoError(t, err)
	t2p, err := s.ds.NewTeamPolicy(ctx, t2.ID, &admin.ID, mdmlab.PolicyPayload{
		Name:  "Zoo2",
		Query: "SELECT 2;",
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	manualLabel1, err := s.ds.NewLabel(ctx, &mdmlab.Label{
		Name:                "manualLabel1",
		Query:               "SELECT 2;",
	...
	s.DoJSON("GET", "/api/latest/mdmlab/activities", nil, http.StatusForbidden, &listActivitiesResponse{})

	// Attempt to retrieve hosts, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusForbidden, &listHostsResponse{})

	// Attempt to retrieve a host by identifier should succeed
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/identifier/%s", h1.Hostname), hostByIdentifierRequest{}, http.StatusOK, &getHostResponse{})

	// Attempt to filter hosts using labels, should fail (label ID 6 is the builtin label "All Hosts")
	s.DoJSON("GET", "/api/latest/mdmlab/labels/6/hosts", nil, http.StatusForbidden, &listHostsResponse{})

	// Attempt to delete hosts, should fail.
	s.DoJSON("DELETE", "/api/latest/mdmlab/hosts/1", nil, http.StatusForbidden, &deleteHostResponse{})

	// Attempt to transfer host from global to a team, should allow.
	s.DoJSON("POST", "/api/latest/mdmlab/hosts/transfer", addHostsToTeamRequest{
		TeamID:  &t1.ID,
		HostIDs: []uint{h1.ID},
	...
	s.DoJSON("POST", "/api/latest/mdmlab/labels", createLabelRequest{
		LabelPayload: mdmlab.LabelPayload{
			Name:  "foo",
			Query: "SELECT 1;",
		},
	}, http.StatusOK, &clr)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/labels/%d", clr.Label.ID), modifyLabelRequest{
		ModifyLabelPayload: mdmlab.ModifyLabelPayload{
			Name: ptr.String("foo2"),
		},
	}, http.StatusOK, &modifyLabelResponse{})

	// Attempt to get a label, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/labels/%d", clr.Label.ID), getLabelRequest{}, http.StatusForbidden, &getLabelResponse{})

	// Attempt to list all labels, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/labels", listLabelsRequest{}, http.StatusForbidden, &listLabelsResponse{})

	// Attempt to delete a label, should allow.
	s.DoJSON("DELETE", "/api/latest/mdmlab/labels/foo2", deleteLabelRequest{}, http.StatusOK, &deleteLabelResponse{})

	// Attempt to list all software, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/software/versions", listSoftwareRequest{}, http.StatusForbidden, &listSoftwareVersionsResponse{})
	s.DoJSON("GET", "/api/latest/mdmlab/software", listSoftwareRequest{}, http.StatusForbidden, &listSoftwareResponse{})
	s.DoJSON("GET", "/api/latest/mdmlab/software/count", countSoftwareRequest{}, http.StatusForbidden, &countSoftwareResponse{})
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{}, http.StatusForbidden, &listSoftwareTitlesResponse{})
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles/1", getSoftwareTitleRequest{}, http.StatusForbidden, &getSoftwareTitleResponse{})

	// Attempt to list a software, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/software/1", getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResponse{})
	s.DoJSON("GET", "/api/latest/mdmlab/software/versions/1", getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResponse{})

	// Attempt to read app config, should pass.
	s.DoJSON("GET", "/api/latest/mdmlab/config", nil, http.StatusOK, &appConfigResponse{})
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/labels", h1.ID), addLabelsToHostRequest{
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/labels", h1.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusOK, &removeLabelsFromHostResp)

	// Attempt to run live queries synchronously, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/queries/run", runLiveQueryRequest{
		HostIDs:  []uint{h1.ID},
		QueryIDs: []uint{q1.ID},
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries/run", createDistributedQueryCampaignRequest{
	...
	s.DoJSON("POST", "/api/latest/mdmlab/queries", createQueryRequest{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/queries/%d", cqr.Query.ID), modifyQueryRequest{
		QueryPayload: mdmlab.QueryPayload{
			Name:  ptr.String("foo4"),
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/queries/%d", cqr.Query.ID), getQueryRequest{}, http.StatusOK, &getQueryResponse{})

	// Attempt to list all queries, should work.
	s.DoJSON("GET", "/api/latest/mdmlab/queries", listQueriesRequest{}, http.StatusOK, &listQueriesResponse{})

	// Attempt to delete queries, should allow.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/queries/id/%d", cqr.Query.ID), deleteQueryByIDRequest{}, http.StatusOK, &deleteQueryByIDResponse{})
	s.DoJSON("POST", "/api/latest/mdmlab/queries/delete", deleteQueriesRequest{IDs: []uint{cqr2.Query.ID}}, http.StatusOK, &deleteQueriesResponse{})
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/queries/%s", cqr3.Query.Name), deleteQueryRequest{}, http.StatusOK, &deleteQueryResponse{})

	// Attempt to add a query to a user pack, should allow.
	sqr := scheduleQueryResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/packs/schedule", scheduleQueryRequest{
		PackID:   userPackID,
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/packs/schedule/%d", sqr.Scheduled.ID), modifyScheduledQueryRequest{
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/packs/schedule/%d", sqr.Scheduled.ID), deleteScheduledQueryRequest{}, http.StatusOK, &scheduleQueryResponse{})

	// Attempt to read the global schedule, should allow.
	s.DoJSON("GET", "/api/latest/mdmlab/schedule", nil, http.StatusOK, &getGlobalScheduleResponse{})
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/packs/%d", cpr.Pack.ID), modifyPackRequest{
		PackPayload: mdmlab.PackPayload{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/packs/%d", cpr.Pack.ID), nil, http.StatusOK, &getPackResponse{})

	// Attempt to delete a pack, should allow.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/packs/id/%d", cpr.Pack.ID), deletePackRequest{}, http.StatusOK, &deletePackResponse{})

	// Attempt to create a global policy, should allow.
	gplr := globalPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/policies", globalPolicyRequest{
		Name:  "foo9",
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/policies/%d", gplr.Policy.ID), modifyGlobalPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/policies/%d", gplr.Policy.ID), getPolicyByIDRequest{}, http.StatusOK,
		&getPolicyByIDResponse{},
	)

	// Attempt to delete a global policy, should allow.
	s.DoJSON("POST", "/api/latest/mdmlab/policies/delete", deleteGlobalPoliciesRequest{
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/team/%d/policies", t1.ID), teamPolicyRequest{
		Name:  "foo10",
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", t1.ID, tplr.Policy.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/team/%d/policies/%d", t1.ID, tplr.Policy.ID), getTeamPolicyByIDRequest{}, http.StatusOK,
		&getTeamPolicyByIDResponse{},
	)

	// Attempt to delete a team policy, should allow.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/delete", t1.ID), deleteTeamPoliciesRequest{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/users/%d", admin.ID), modifyUserRequest{
		UserPayload: mdmlab.UserPayload{
			GlobalRole: ptr.String("observer"),
		},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/users/%d", admin.ID), getUserRequest{}, http.StatusForbidden, &getUserResponse{})

	// Attempt to delete a user, should fail.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/users/%d", admin.ID), deleteUserRequest{}, http.StatusForbidden, &deleteUserResponse{})

	// Attempt to add users to team, should allow.
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", t1.ID), modifyTeamUsersRequest{
		Users: []mdmlab.TeamUser{
			{
				User: *maintainer,
				Role: "admin",
			},
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", t1.ID), modifyTeamUsersRequest{
		Users: []mdmlab.TeamUser{
			{
				User: *maintainer,
				Role: "admin",
	...
	tr := teamResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/teams", createTeamRequest{
		TeamPayload: mdmlab.TeamPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tr.Team.ID), modifyTeamRequest{
		TeamPayload: mdmlab.TeamPayload{
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", tr.Team.ID), json.RawMessage(`{
		"config": {
			"options": {
				"aws_debug": true
			}
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tr.Team.ID), getTeamRequest{}, http.StatusForbidden, &teamResponse{})

	// Attempt to delete a team, should allow.
	dtr := deleteTeamResponse{}
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d", tr.Team.ID), deleteTeamRequest{}, http.StatusOK, &dtr)

	// Attempt to create/edit enroll secrets, should allow.
	s.DoJSON("POST", "/api/latest/mdmlab/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mdmlab.EnrollSecretSpec{
	...
	s.DoJSON("GET", "/api/latest/mdmlab/spec/enroll_secret", nil, http.StatusForbidden, &getEnrollSecretSpecResponse{})

	// Attempt to get team enroll secret, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/secrets", t1.ID), teamEnrollSecretsRequest{}, http.StatusForbidden, &teamEnrollSecretsResponse{})

	// Attempt to list carved files, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/carves", listCarvesRequest{}, http.StatusForbidden, &listCarvesResponse{})

	// Attempt to get a carved file, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/carves/%d", carveID), listCarvesRequest{}, http.StatusForbidden, &listCarvesResponse{})

	// Attempt to search hosts, should fail.
	s.DoJSON("POST", "/api/latest/mdmlab/targets", searchTargetsRequest{
		MatchQuery: "foo",
		QueryID:    &q1.ID,
	}, http.StatusForbidden, &searchTargetsResponse{})

	// Attempt to count target hosts, should fail.
	s.DoJSON("POST", "/api/latest/mdmlab/targets/count", countTargetsRequest{
		Selected: mdmlab.HostTargets{
			HostIDs:  []uint{h1.ID},
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/queries/%d", tcqr.Query.ID), modifyQueryRequest{
		QueryPayload: mdmlab.QueryPayload{
			Name:  ptr.String("foo4"),
			Query: ptr.String("SELECT * FROM system_info;"),
		},
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/queries/id/%d", tcqr.Query.ID), deleteQueryByIDRequest{}, http.StatusOK, &deleteQueryByIDResponse{})

	// Attempt to edit query created by somebody else, should fail.
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/queries/%d", cqr4.Query.ID), modifyQueryRequest{
		QueryPayload: mdmlab.QueryPayload{
			Name:  ptr.String("foo4"),
			Query: ptr.String("SELECT * FROM system_info;"),
		},
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/queries/id/%d", cqr4.Query.ID), deleteQueryByIDRequest{}, http.StatusForbidden, &deleteQueryByIDResponse{})

	// Attempt to read the global schedule, should fail.
	s.DoJSON("GET", "/api/latest/mdmlab/schedule", nil, http.StatusForbidden, &getGlobalScheduleResponse{})

	// Attempt to read the team's schedule, should allow.
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule", t1.ID), getTeamScheduleRequest{}, http.StatusOK,
		&getTeamScheduleResponse{},
	)

	// Attempt to read other team's schedule, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule", t2.ID), getTeamScheduleRequest{}, http.StatusForbidden, &getTeamScheduleResponse{})
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule", t1.ID), teamScheduleQueryRequest{
		ScheduledQueryPayload: mdmlab.ScheduledQueryPayload{
			QueryID:  ptr.Uint(q1.ID),
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/schedule/%d", t1.ID, ttsqr.Scheduled.ID), deleteTeamScheduleRequest{}, http.StatusOK, &deleteTeamScheduleResponse{})

	// Attempt to add/remove a manual label from a team host, should allow.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/labels", team1Host.ID), addLabelsToHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusOK, &addLabelsToHostResp)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/labels", team1Host.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusOK, &removeLabelsFromHostResp)

	// Attempt to add/remove a manual label from a global host, should not allow.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/labels", globalHost.ID), addLabelsToHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusForbidden, &addLabelsToHostResp)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/labels", globalHost.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/policies/%d", gp2.ID), getPolicyByIDRequest{}, http.StatusForbidden, &getPolicyByIDResponse{})
	...
	// Attempt to edit a team policy, should allow.
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", t1.ID, ttplr.Policy.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", t2.ID, t2p.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
			Query: ptr.String("SELECT * from file WHERE path = 'foobar';"),
		},
	...
	// Attempt to view a team policy, should allow.
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/team/%d/policies/%d", t1.ID, ttplr.Policy.ID), getTeamPolicyByIDRequest{}, http.StatusOK,
		&getTeamPolicyByIDResponse{},
	)

	// Attempt to view another team's policy, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/team/%d/policies/%d", t2.ID, t2p.ID), getTeamPolicyByIDRequest{}, http.StatusForbidden, &getTeamPolicyByIDResponse{})
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", t1.ID), modifyTeamRequest{
		TeamPayload: mdmlab.TeamPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d", t2.ID), modifyTeamRequest{
		TeamPayload: mdmlab.TeamPayload{
			Name: ptr.String("foo123456"),
		},
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", t1.ID), json.RawMessage(`{
		"config": {
			"options": {
				"aws_debug": true
			}
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/teams/%d/agent_options", t2.ID), json.RawMessage(`{
		"config": {
			"options": {
				"aws_debug": true
			}
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", t3.ID), modifyTeamUsersRequest{
		Users: []mdmlab.TeamUser{
			{
				User: *u3,
				Role: "maintainer",
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", t3.ID), modifyTeamUsersRequest{
		Users: []mdmlab.TeamUser{
			{
				User: *u3,
			},
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", t2.ID), modifyTeamUsersRequest{
		Users: []mdmlab.TeamUser{
			{
				User: *u3,
				Role: "maintainer",
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/teams/%d/users", t2.ID), modifyTeamUsersRequest{
		Users: []mdmlab.TeamUser{
			{
				User: *u2,
			},
...
func (s *integrationEnterpriseTestSuite) TestDesktopEndpointWithInvalidPolicy() {
	...
	policy, err := s.ds.NewGlobalPolicy(context.Background(), &admin.ID, mdmlab.PolicyPayload{
	...
	desktopRes := mdmlabDesktopResponse{}
	res := s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/desktop", nil, http.StatusOK)
...
func (s *integrationEnterpriseTestSuite) TestRunHostScript() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID + 100, ScriptContents: "echo"}, http.StatusNotFound, &runResp)

	// attempt to run an empty script
	res := s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: ""}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: strings.Repeat("a", mdmlab.UnsavedScriptMaxRuneLen+1)}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo $FLEET_SECRET_INVALID"}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	req := secretVariablesRequest{
		SecretVariables: []mdmlab.SecretVariable{
			{
	...
	s.DoJSON("PUT", "/api/latest/mdmlab/spec/secret_variables", req, http.StatusOK, &secretResp)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run",
		mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: expectedScriptContents}, http.StatusAccepted, &runResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+runResp.ExecutionID, nil, http.StatusOK, &scriptResultResp)
	...
	require.Contains(t, scriptResultResp.Message, mdmlab.RunScriptAsyncScriptEnqueuedMsg)
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/config",
	...
	srResp := s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusForbidden)
	assertBodyContains(t, srResp, mdmlab.RunScriptScriptsDisabledGloballyErrMsg)

	srResp = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusForbidden)
	assertBodyContains(t, srResp, mdmlab.RunScriptScriptsDisabledGloballyErrMsg)
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/scripts/request",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID + 100, ScriptContents: "echo"}, http.StatusNotFound, &runSyncResp)

	// attempt to sync run an empty script
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: ""}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: strings.Repeat("a", mdmlab.UnsavedScriptMaxRuneLen+1)}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Script is too large. It's limited to 10,000 characters")
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusConflict)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mdmlab.RunScriptAlreadyRunningErrMsg)

	// save a result via the orbit endpoint
	var orbitPostScriptResp orbitPostScriptResultResponse
	s.DoJSON("POST", "/api/mdmlab/orbit/scripts/result",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusRequestTimeout, &runSyncResp)
	...
	require.Contains(t, runSyncResp.Message, mdmlab.RunScriptHostTimeoutErrMsg)
	...
	resultsCh := make(chan *mdmlab.HostScriptResultPayload, 1)
	go func() {
	...
	resultsCh <- &mdmlab.HostScriptResultPayload{
		HostID:   host.ID,
		Output:   "ok",
		Runtime:  1,
		ExitCode: 0,
	...
	runSyncResp = runScriptSyncResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusOK, &runSyncResp)
	require.Equal(t, host.ID, runSyncResp.HostID)
	require.NotEmpty(t, runSyncResp.ExecutionID)
	...
	scriptResultResp = getScriptResultResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+runSyncResp.ExecutionID, nil, http.StatusOK, &scriptResultResp)
	require.Equal(t, host.ID, scriptResultResp.HostID)
	require.Equal(t, "echo", scriptResultResp.ScriptContents)
	require.Nil(t, scriptResultResp.ExitCode)
	require.True(t, scriptResultResp.HostTimeout)
	require.Contains(t, scriptResultResp.Message, mdmlab.RunScriptHostTimeoutErrMsg)
	...
	require.NoError(t, err)
	s.DoJSON(
		"POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"},
		http.StatusUnprocessableEntity, &runResp,
	)
	s.DoJSON(
		"POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"},
		http.StatusUnprocessableEntity, &runResp,
	)
	...
	// is offline.
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mdmlab.RunScriptHostOfflineErrMsg)

	// attempt to create an async script execution request, succeeds because script is added to queue.
	s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusAccepted)

	// attempt to run a script on a plain osquery host
	plainOsqueryHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
	...
	require.NoError(t, err)
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptContents: "echo"}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mdmlab.RunScriptDisabledErrMsg)
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptContents: "echo"}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mdmlab.RunScriptDisabledErrMsg)

	// create a execution request that will return a timeout
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusAccepted, &runResp)
	...
	require.Equal(t, "Timeout. MDMlab stopped the script after 900 seconds to protect host performance.", scriptResultResp.Message)
...
func (s *integrationEnterpriseTestSuite) TestRunHostSavedScript() {
	...
	host := createOrbitEnrolledHost(t, "linux", "", s.ds)
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team 1"})
	require.NoError(t, err)
	savedNoTmScript, err := s.ds.NewScript(ctx, &mdmlab.Script{
		TeamID:         nil,
		Name:           "no_team_script.sh",
		ScriptContents: "echo 'no team'",
	})
	require.NoError(t, err)
	savedTmScript, err := s.ds.NewScript(ctx, &mdmlab.Script{
		TeamID:         &tm.ID,
		Name:           "team_script.sh",
	...
	// attempt to run a script on a non-existing host
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID + 100, ScriptID: &savedNoTmScript.ID}, http.StatusNotFound, &runResp)

	// attempt to run with both script contents and id
	res := s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptID: ptr.Uint(savedTmScript.ID + 999)}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_contents' is allowed.`)

	// attempt to run with unknown script id
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999)}, http.StatusNotFound)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedTmScript.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	// create a valid script execution request
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedNoTmScript.ID}, http.StatusAccepted, &runResp)
	require.Equal(t, host.ID, runResp.HostID)
	require.NotEmpty(t, runResp.ExecutionID)
	...
	s.lastActivityMatches(
		mdmlab.ActivityTypeRanScript{}.ActivityName(),
		fmt.Sprintf(
	...
	// request will time-out waiting for a result.
	var runSyncResp runScriptSyncResponse
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedNoTmScript.ID}, http.StatusRequestTimeout, &runSyncResp)
	require.Equal(t, host.ID, runSyncResp.HostID)
	require.NotEmpty(t, runSyncResp.ExecutionID)
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptID: ptr.Uint(savedTmScript.ID + 999)}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_contents' is allowed.`)

	// attempt to run sync with both script contents and script name
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'script_name' is allowed.`)

	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'script_name' is allowed.`)

	// attempt to run sync with both script id and script name
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_name' is allowed.`)

	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_name' is allowed.`)

	// attempt to run sync with both script contents and team id
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'team_id' is allowed.`)

	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'team_id' is allowed.`)

	// attempt to run sync with both script id and team id
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'team_id' is allowed.`)

	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'team_id' is allowed.`)

	// attempt to run sync without script contents, script id, or script name
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `One of 'script_id', 'script_contents', or 'script_name' is required.`)

	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `One of 'script_id', 'script_contents', or 'script_name' is required.`)

	// deleting the saved script should delete the pending script
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", savedNoTmScript.ID), nil, http.StatusNoContent)
	scriptResultResp = getScriptResultResponse{}
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+runSyncResp.ExecutionID, nil, http.StatusNotFound, &scriptResultResp)
	...
	require.NoError(t, err)
	for i := 1; i <= 1000; i++ {
		script, err := s.ds.NewScript(ctx, &mdmlab.Script{
			TeamID:         nil,
		...
		_, err = s.ds.NewHostScriptExecutionRequest(ctx, &mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID})
		require.NoError(t, err)
	}
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID}, http.StatusConflict, &runResp)

	// set up a new host, new team, and some new scripts
	host2 := createOrbitEnrolledHost(t, "linux", "f1337", s.ds)
	tm2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team 2"})
	require.NoError(t, err)
	savedNoTmScript2, err := s.ds.NewScript(ctx, &mdmlab.Script{
		TeamID:         nil,
		Name:           "f1337.sh",
		ScriptContents: "echo 'ALL YOUR BASE ARE BELONG TO US'",
	})
	require.NoError(t, err)
	savedTmScript2, err := s.ds.NewScript(ctx, &mdmlab.Script{
		TeamID:         &tm2.ID,
		Name:           "f1337.sh",
		ScriptContents: "echo 'ALL YOUR BASE ARE BELONG TO US'",
	})
	...
	_, err = s.ds.NewScript(ctx, &mdmlab.Script{
		TeamID:         nil,
		Name:           "f13372.sh",
		ScriptContents: "echo 'ALL YOUR BASE ARE BELONG TO US'",
	})
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm2.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `The script does not belong to the same team`)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh"}, http.StatusRequestTimeout, &runSyncResp2)
	...
	require.Contains(t, runSyncResp2.Message, mdmlab.RunScriptHostTimeoutErrMsg)
	...
	require.NoError(t, err)
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptID: &script.ID}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mdmlab.RunScriptDisabledErrMsg)
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run/sync", mdmlab.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptID: &script.ID}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mdmlab.RunScriptDisabledErrMsg)

	// Async Run Script by Name

	// attempt to run async with a script that does not exist on the specified team
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Script 'f1337.sh' doesn’t exist.`)

	// attempt to run async with an existing team script that belongs to a team different from the host's team
	res = s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm2.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `The script does not belong to the same team`)

	var runSyncResp3 runScriptSyncResponse
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f13372.sh"}, http.StatusAccepted, &runSyncResp3)
	require.Equal(t, host2.ID, runSyncResp3.HostID)
	require.NotEmpty(t, runSyncResp3.ExecutionID)

	// verify pending result
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+runSyncResp3.ExecutionID, nil, http.StatusOK, &scriptResultResp)
...
func (s *integrationEnterpriseTestSuite) TestEnqueueSameScriptTwice() {
	...
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID}, http.StatusAccepted, &runResp)
	require.Equal(t, host.ID, runResp.HostID)
	require.NotEmpty(t, runResp.ExecutionID)

	// Should fail because the same script is already enqueued for this host.
	resp := s.Do("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID}, http.StatusConflict)
...
func (s *integrationEnterpriseTestSuite) TestOrbitConfigExtensions() {
	...
	foobarLabel, err := s.ds.NewLabel(ctx, &mdmlab.Label{
		Name:  "Foobar",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
	zoobarLabel, err := s.ds.NewLabel(ctx, &mdmlab.Label{
		Name:  "Zoobar",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
	...
	s.DoRaw("PATCH", "/api/latest/mdmlab/config", []byte(`{
  "agent_options": {
	"config": {
		"options": {
		"pack_delimiter": "/",
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *orbitDarwinClient.OrbitNodeKey)), http.StatusOK, &resp)
	require.JSONEq(t, `{
	"hello_mars_macos": {
		"channel": "stable",
		"platform": "macos"
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *orbitLinuxClient.OrbitNodeKey)), http.StatusOK, &resp)
	require.JSONEq(t, `{
	"hello_world_linux": {
		"channel": "stable",
		"platform": "linux"
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mdmlab/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *orbitWindowsClient.OrbitNodeKey)), http.StatusOK, &resp)
...
func (s *integrationEnterpriseTestSuite) TestSavedScripts() {
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mdmlab/scripts", body.Bytes(), http.StatusOK, headers)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScriptID), nil, http.StatusOK, &getScriptResp)
	...
	res = s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScriptID), nil, http.StatusOK, "alt", "media")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScriptID+999), nil, http.StatusNotFound, &getScriptResp)
	// download a non-existing script
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScriptID+999), nil, http.StatusNotFound, &getScriptResp, "alt", "media")

	// file name is empty
	body, headers = generateNewScriptMultipartRequest(t,
		"", []byte(`echo "hello"`), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/scripts", body.Bytes(), http.StatusBadRequest, headers)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "no file headers for script")

	// contains invalid mdmlab secret
	body, headers = generateNewScriptMultipartRequest(t,
		"secrets.sh", []byte(`echo "$FLEET_SECRET_INVALID"`), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/scripts", body.Bytes(), http.StatusUnprocessableEntity, headers)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "$FLEET_SECRET_INVALID")
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script2.sh", []byte(strings.Repeat("a", mdmlab.SavedScriptMaxRuneLen+1)), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/scripts", body.Bytes(), http.StatusUnprocessableEntity, headers)
	errMsg = extractServerErrorText(res.Body)
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script1.sh", []byte(`echo "hello"`), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/scripts", body.Bytes(), http.StatusConflict, headers)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.DoRawWithHeaders("POST", "/api/latest/mdmlab/scripts", body.Bytes(), http.StatusNotFound, headers)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "The team does not exist.")

	// create a team
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", tmScriptID), nil, http.StatusOK, &getScriptResp)
	...
	res = s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", tmScriptID), nil, http.StatusOK, "alt", "media")
	b, err = io.ReadAll(res.Body)
	require.NoError(t, err)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScriptID), nil, http.StatusNoContent)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", tmScriptID), nil, http.StatusNoContent)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScriptID), nil, http.StatusNotFound)
}
...
func (s *integrationEnterpriseTestSuite) TestListSavedScripts() {
	...
	// create some teams
	tm1, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team2"})
	require.NoError(t, err)
	tm3, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team3"})
	require.NoError(t, err)
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		_, err = s.ds.NewScript(ctx, &mdmlab.Script{Name: string('a' + byte(i)), TeamID: &tm1.ID, ScriptContents: "echo"})
		require.NoError(t, err)
	}

	// create a single script for team 2
	_, err = s.ds.NewScript(ctx, &mdmlab.Script{Name: "a", TeamID: &tm2.ID, ScriptContents: "echo"})
	require.NoError(t, err)
	...
	cases := []struct {
		...
		teamID    *uint
		wantNames []string
		wantMeta  *mdmlab.PaginationMetadata
	}{
		{
			wantNames: []string{"a", "b", "c", "d", "e"},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: false},
		},
		{
			queries:   []string{"per_page", "2"},
			wantNames: []string{"a", "b"},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: false},
		},
		{
			queries:   []string{"per_page", "2", "page", "1"},
			wantNames: []string{"c", "d"},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: true, HasPreviousResults: true},
		},
		{
			queries:   []string{"per_page", "2", "page", "2"},
			wantNames: []string{"e"},
			wantMeta:  &mdmlab.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
		},
		{
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("%v: %#v", c.teamID, c.queries), func(t *testing.T) {
			...
			s.DoJSON("GET", "/api/latest/mdmlab/scripts", nil, http.StatusOK, &listResp, queryArgs...)
...
func (s *integrationEnterpriseTestSuite) TestHostScriptDetails() {
	...
	// create some teams
	tm1, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "test-script-details-team1"})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "test-script-details-team2"})
	require.NoError(t, err)
	tm3, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "test-script-details-team3"})
	require.NoError(t, err)
	tm4, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "test-script-details-team4-windows"})
	require.NoError(t, err)

	// create 5 scripts for no team and team 1
	for i := 0; i < 5; i++ {
		_, err = s.ds.NewScript(ctx, &mdmlab.Script{Name: fmt.Sprintf("test-script-details-%d.sh", i), ScriptContents: "echo"})
		require.NoError(t, err)
		_, err = s.ds.NewScript(ctx, &mdmlab.Script{Name: fmt.Sprintf("test-script-details-%d.sh", i), TeamID: &tm1.ID, ScriptContents: "echo"})
		require.NoError(t, err)
	}

	// add a windows script to team 4
	_, err = s.ds.NewScript(ctx, &mdmlab.Script{Name: "test-script-details-windows.ps1", TeamID: &tm4.ID, ScriptContents: `Write-Host "Hello, World!"`})
	require.NoError(t, err)

	// create a single script for team 2
	_, err = s.ds.NewScript(ctx, &mdmlab.Script{Name: "test-script-details-team-2.sh", TeamID: &tm2.ID, ScriptContents: "echo"})
	require.NoError(t, err)

	// create a host without a team
	host0, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host1, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host2, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host3, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	// create a Linux host
	host4, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host5, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	insertResults := func(t *testing.T, hostID uint, script *mdmlab.Script, createdAt time.Time, execID string, exitCode *int64) {
		stmt := `
INSERT INTO
...
VALUES
	...
	insertResults(t, host0.ID, &mdmlab.Script{Name: "ad hoc script", ScriptContents: "echo foo"}, now, "ad-hoc-0", ptr.Int64(0))
	insertResults(t, host1.ID, &mdmlab.Script{Name: "ad hoc script", ScriptContents: "echo foo"}, now.Add(-1*time.Hour), "ad-hoc-1", ptr.Int64(1))

	t.Run("no team", func(t *testing.T) {
		noTeamScripts, _, err := s.ds.ListScripts(ctx, nil, mdmlab.ListOptions{})
		require.NoError(t, err)
		...
		// insert some ad hoc script results, these are never included in the host script details
		insertResults(t, host0.ID, &mdmlab.Script{Name: "ad hoc script", ScriptContents: "echo foo"}, now.Add(-3*time.Hour), "exec0-3", ptr.Int64(0))

		// check host script details, should include all no team scripts
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/scripts", host0.ID), nil, http.StatusOK, &resp)
		require.Len(t, resp.Scripts, len(noTeamScripts))
		byScriptID := make(map[uint]*mdmlab.HostScriptDetail, len(resp.Scripts))
	...
	t.Run("team 1", func(t *testing.T) {
		tm1Scripts, _, err := s.ds.ListScripts(ctx, &tm1.ID, mdmlab.ListOptions{})
		require.NoError(t, err)
		...
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/scripts", host1.ID), nil, http.StatusOK, &resp)
	...
	t.Run("deleted script", func(t *testing.T) {
		...
		// delete a script
		s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", noTeamScripts[0].ID), nil, http.StatusNoContent)
	...
	t.Run("transfer team", func(t *testing.T) {
		...
		tm2Scripts, _, err := s.ds.ListScripts(ctx, &tm2.ID, mdmlab.ListOptions{})
		require.NoError(t, err)
	...
	t.Run("no scripts", func(t *testing.T) {
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/scripts", host2.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
		require.Len(t, resp.Scripts, 0)
	})

	t.Run("windows", func(t *testing.T) {
		team4Scripts, _, err := s.ds.ListScripts(ctx, &tm4.ID, mdmlab.ListOptions{})
		require.NoError(t, err)
		require.Len(t, team4Scripts, 1)

		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/scripts", host3.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
	...
	t.Run("linux", func(t *testing.T) {
		...
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/scripts", host4.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
	...
	t.Run("unspecified platform", func(t *testing.T) {
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/scripts", host5.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
		require.Len(t, resp.Scripts, 4)
	})
	...
	(?,?,?,?)`,
		...
		for _, c := range []struct {
		...
		}{
			{
				...
				expected:   mdmlab.RunScriptHostTimeoutErrMsg,
			},
			{
				name:       "script-timeout",
				exitCode:   ptr.Int64(-1),
				executedAt: now.Add(-1 * time.Hour),
				expected:   mdmlab.HostScriptTimeoutMessage(ptr.Int(int(scripts.MaxHostExecutionTime.Seconds()))),
			},
			{
				name:       "pending",
				exitCode:   nil,
				executedAt: now.Add(-1 * time.Minute),
				expected:   mdmlab.RunScriptAlreadyRunningErrMsg,
			},
			{
				name:       "success",
			...
			},
			{
				...
				executedAt: now.Add(-1 * time.Hour),
				expected:   mdmlab.RunScriptDisabledErrMsg,
			},
		} {
			t.Run(c.name, func(t *testing.T) {
				insertResults(t, host0.ID, &mdmlab.Script{ID: oldScriptID, ScriptContentID: oldScriptContentsID, Name: "test-script-details-timeout.sh"}, c.executedAt, "test-user-message_"+c.name, c.exitCode)

				var resp getScriptResultResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/results/%s", "test-user-message_"+c.name), nil, http.StatusOK, &resp)
...
// used via s.DoRawWithHeaders to POST /api/_version_/mdmlab/scripts.
...
func (s *integrationEnterpriseTestSuite) TestAppConfigScripts() {
	...
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "scripts": ["foo", "bar"] }`), http.StatusOK, &acResp)
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{}`), http.StatusOK, &acResp)
	assert.ElementsMatch(t, []string{"foo", "bar"}, acResp.Scripts.Value)

	// patch with explicitly empty scripts fields, would remove
	// them but this is a dry-run
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "scripts": null }`), http.StatusOK, &acResp, "dry_run", "true")
	assert.ElementsMatch(t, []string{"foo", "bar"}, acResp.Scripts.Value)

	// patch with explicitly empty scripts fields, removes them
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "scripts": null }`), http.StatusOK, &acResp)
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "scripts": [] }`), http.StatusOK, &acResp)
	assert.Empty(t, acResp.Scripts.Value)

	// patch with an invalid array returns an error
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mdmlab/config", json.RawMessage(`{ "scripts": ["foo", 1] }`), http.StatusBadRequest, &acResp)
	assert.Empty(t, acResp.Scripts.Value)
}
...
func (s *integrationEnterpriseTestSuite) TestApplyTeamsScriptsConfig() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/teams", team, http.StatusOK, &createTeamResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
...
func (s *integrationEnterpriseTestSuite) TestBatchApplyScriptsEndpoints() {
	...
	saveAndCheckScripts := func(team *mdmlab.Team, scripts []mdmlab.ScriptPayload) {
		var teamID *uint
		teamIDStr := ""
		...
		s.DoJSON("POST", "/api/v1/mdmlab/scripts/batch", batchSetScriptsRequest{Scripts: scripts}, http.StatusOK, &scriptsBatchResponse, "team_id", teamIDStr)
		s.lastActivityMatches(
			mdmlab.ActivityTypeEditedScript{}.ActivityName(),
			teamActivity,
			0,
		)
		...
		var listResp listScriptsResponse
		s.DoJSON("GET", "/api/latest/mdmlab/scripts", nil, http.StatusOK, &listResp, "team_id", teamIDStr)
		require.Len(t, listResp.Scripts, len(scripts))

		got := make([]mdmlab.ScriptPayload, len(scripts))
		for i, gotScript := range listResp.Scripts {
			// add the script contents
			res := s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", gotScript.ID), nil, http.StatusOK, "alt", "media")
			b, err := io.ReadAll(res.Body)
			require.NoError(t, err)
			got[i] = mdmlab.ScriptPayload{
				Name:           gotScript.Name,
				ScriptContents: b,
			}
	...
	// create a new team
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "batch_set_scripts"})
	require.NoError(t, err)
	...
	s.Do("POST", "/api/v1/mdmlab/scripts/batch", batchSetScriptsRequest{Scripts: nil},
	...
	s.Do("POST", "/api/v1/mdmlab/scripts/batch", batchSetScriptsRequest{Scripts: []mdmlab.ScriptPayload{
	...
	saveAndCheckScripts(tm, []mdmlab.ScriptPayload{
		{Name: "N1.sh", ScriptContents: []byte("foo")},
		{Name: "N2.sh", ScriptContents: []byte("bar")},
	})

	// successfully apply scripts for "no team"
	saveAndCheckScripts(nil, []mdmlab.ScriptPayload{
		{Name: "N1.sh", ScriptContents: []byte("foo")},
		{Name: "N2.sh", ScriptContents: []byte("bar")},
	})
...
`, teamName))
	...
	// create a linux host
	linuxHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.NotContains(t, dqResp.Queries, "mdmlab_detail_query_users")
	require.NotContains(t, dqResp.Queries, "mdmlab_detail_query_disk_encryption_linux")
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_software_linux")
	require.Contains(t, dqResp.Queries, fmt.Sprintf("mdmlab_distributed_query_%s", t.Name()))
...
`, teamName))
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_users")
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_disk_encryption_linux")
	require.Contains(t, dqResp.Queries, "mdmlab_detail_query_software_linux")
	require.Contains(t, dqResp.Queries, fmt.Sprintf("mdmlab_distributed_query_%s", t.Name()))
}
...
func (s *integrationEnterpriseTestSuite) TestAllSoftwareTitles() {
	...
	softwareTitleListResultsMatch := func(want, got []mdmlab.SoftwareTitleListResult) {
	...
	softwareTitlesMatch := func(want, got []mdmlab.SoftwareTitle) {
		// compare only the fields we care about
		for i := range got {
			require.NotZero(t, got[i].ID)
	...
	tmHost, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	team1, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "team2"})
	require.NoError(t, err)
	...
	cpes := []mdmlab.SoftwareCPE{{SoftwareID: soft1.ID, CPE: "somecpe"}}
	...
	inserted, err := s.ds.InsertSoftwareVulnerability(
		context.Background(), mdmlab.SoftwareVulnerability{
			SoftwareID: soft1.ID,
			CVE:        "cve-123-123-132",
		}, mdmlab.NVDSource,
	)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &resp, "self_service", "1")
	require.Empty(t, resp.SoftwareTitles)
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &resp)
	require.Equal(t, 2, resp.Count)
	require.NotEmpty(t, resp.CountsUpdatedAt)
	softwareTitleListResultsMatch([]mdmlab.SoftwareTitleListResult{
		{
			Name:          "foo",
			Source:        "homebrew",
			VersionsCount: 2,
			HostsCount:    1,
			Versions: []mdmlab.SoftwareVersion{
		...
		},
		{
			...
			HostsCount:    1,
			Versions: []mdmlab.SoftwareVersion{
				{Version: "0.0.4", Vulnerabilities: &mdmlab.SliceString{"cve-123-123-132"}},
			},
		},
	}, resp.SoftwareTitles)
	...
	s.DoJSON(
		"GET", "/api/latest/mdmlab/software/titles",
	...
	softwareTitleListResultsMatch([]mdmlab.SoftwareTitleListResult{}, resp.SoftwareTitles)

	// asking for vulnerable only software returns the expected values
	expectedVulnSoftware := []mdmlab.SoftwareTitleListResult{
		{
			Name:          "bar",
			Source:        "apps",
			VersionsCount: 1,
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles/999", getSoftwareTitleRequest{}, http.StatusNotFound, &stResp)

	// valid title
	stResp = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", fooTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp)
	s.NotZero(*stResp.SoftwareTitle.CountsUpdatedAt)
	softwareTitlesMatch([]mdmlab.SoftwareTitle{
		{
			Name:          "foo",
			Source:        "homebrew",
			VersionsCount: 2,
	...
	}, []mdmlab.SoftwareTitle{*stResp.SoftwareTitle})

	// valid title for team
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", fooTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp,
		"team_id", fmt.Sprintf("%d", team1.ID),
	)
	softwareTitlesMatch(
		[]mdmlab.SoftwareTitle{
			{
				Name:          "foo",
				Source:        "homebrew",
				VersionsCount: 1,
	...
	expected := []mdmlab.SoftwareTitle{
		{
			...
			Versions: []mdmlab.SoftwareVersion{
				{
					Version:         "0.0.4",
					Vulnerabilities: &mdmlab.SliceString{"cve-123-123-132"},
	...
	// Global
	stResp = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", barTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp)
	softwareTitlesMatch(expected, []mdmlab.SoftwareTitle{*stResp.SoftwareTitle})
	...
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", barTitle.ID), getSoftwareTitleRequest{}, http.StatusNotFound, &stResp,
		"team_id", fmt.Sprintf("%d", team1.ID),
	)
	...
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", bazTitle.ID), getSoftwareTitleRequest{}, http.StatusNotFound, &stResp,
		"team_id", "0")
	...
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", barTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp,
		"team_id", fmt.Sprintf("%d", team1.ID),
	)
	softwareTitlesMatch(
		[]mdmlab.SoftwareTitle{
	...
	payloadRubyTm1 := &mdmlab.UploadSoftwareInstallerPayload{
	...
	payloadEmacsMissingSecret := &mdmlab.UploadSoftwareInstallerPayload{
	...
	payloadEmacsMissingPostSecret := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:     "install",
		Filename:          "emacs.deb",
	...
	payloadEmacsMissingUnSecret := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:     "install",
		Filename:          "emacs.deb",
		PostInstallScript: "d",
	...
	payloadEmacs := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript: "install",
		Filename:      "emacs.deb",
		SelfService:   true,
	}
	...
	payloadVim := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript: "install",
		Filename:      "vim.deb",
		SelfService:   true,
		TeamID:        ptr.Uint(0),
	...
	payloadRubyTm2 := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript: "install",
	...
	emacsPath := fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", resp.SoftwareTitles[0].ID)
...
func (s *integrationEnterpriseTestSuite) TestLockUnlockWipeWindowsLinux() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", winHost.ID), nil, http.StatusOK, &getHostResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", linuxHost.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
	require.Equal(t, "unlocked", *getHostResp.Host.MDM.DeviceStatus)
	require.NotNil(t, getHostResp.Host.MDM.PendingAction)
	require.Equal(t, "", *getHostResp.Host.MDM.PendingAction)
	...
	res := s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", winHost.ID), nil, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Windows MDM isn't turned on.")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", winHost.ID), nil, http.StatusBadRequest)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Windows MDM isn't turned on.")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", winHost.ID), nil, http.StatusBadRequest)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Windows MDM isn't turned on.")
	...
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", linuxHost.ID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't lock host. To lock, deploy the mdmlabd agent with --enable-scripts")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", linuxHost.ID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't unlock host. To unlock, deploy the mdmlabd agent with --enable-scripts")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/wipe", linuxHost.ID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't wipe host. To wipe, deploy the mdmlabd agent with --enable-scripts")
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", linuxHost.ID), nil, http.StatusNoContent)
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", linuxHost.ID), nil, http.StatusNoContent)
...
func checkWindowsOSUpdatesProfile(t *testing.T, ds *mysql.Datastore, teamID *uint, wantSettings *mdmlab.WindowsUpdates) string {
	ctx := context.Background()

	var prof mdmlab.MDMWindowsConfigProfile
	mysql.ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
		var globalOrTeamID uint
		if teamID != nil {
			globalOrTeamID = *teamID
		}
		err := sqlx.GetContext(ctx, tx, &prof, `SELECT profile_uuid, syncml FROM mdm_windows_configuration_profiles WHERE team_id = ? AND name = ?`, globalOrTeamID, mdm.MDMlabWindowsOSUpdatesProfileName)
...
func (s *integrationEnterpriseTestSuite) createHosts(t *testing.T, platforms ...string) []*mdmlab.Host {
...
func (s *integrationEnterpriseTestSuite) TestSoftwareAuth() {
	...
	tmHost, err := s.ds.NewHost(ctx, &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	// Create two teams, team1 and team2.
	team1, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		ID:          42,
		Name:        "team1",
		Description: "desc team1",
	})
	...
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(ctx, &mdmlab.Team{
		ID:          43,
		Name:        "team2",
		Description: "desc team2",
	})
	...
	_, err = s.ds.UpdateHostSoftware(ctx, tmHost.ID, []mdmlab.Software{allSoftware[0]})
	require.NoError(t, err)
	...
	extraTestUsers := make(map[string]mdmlab.User)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &listSoftwareTitlesResp)

	var softwareFoo, softwareBar *mdmlab.SoftwareTitleListResult
	...
	var teamFooVersion *mdmlab.SoftwareVersion
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldFailGlobalRead {
				...
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{}, http.StatusForbidden, &listSoftwareTitlesResp)

				// List all software versions
				var resp listSoftwareVersionsResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/versions", listSoftwareTitlesRequest{}, http.StatusForbidden, &resp)

				// Get a global software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", softwareBar.ID), getSoftwareTitleRequest{}, http.StatusForbidden, &getSoftwareTitleResp)

				// Get a global software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/versions/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)

				// Get a global software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)

				// Get a count of software vesions using the deprecated endpoint
				var countSoftwareResp countSoftwareResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/count", getSoftwareRequest{}, http.StatusForbidden, &countSoftwareResp)

				// List all software versions using the deprecated endpoint
				var softwareListResp listSoftwareResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software", listSoftwareRequest{}, http.StatusForbidden, &softwareListResp)
			} else {
				// List all software titles
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &listSoftwareTitlesResp)
				...
				// List all software versions
				var resp listSoftwareVersionsResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/versions", listSoftwareRequest{}, http.StatusOK, &resp)
				require.Equal(t, 3, resp.Count)
				require.NotEmpty(t, resp.CountsUpdatedAt)

				// Get a global software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", softwareBar.ID), getSoftwareTitleRequest{}, http.StatusOK, &getSoftwareTitleResp)

				// Get a global software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/versions/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a global software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a global count of software vesions using the deprecated endpoint
				var countSoftwareResp countSoftwareResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/count", countSoftwareRequest{}, http.StatusOK, &countSoftwareResp)
				require.Equal(t, 3, countSoftwareResp.Count)

				// List all software versions using the deprecated endpoint
				var softwareListResp listSoftwareResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software", listSoftwareRequest{}, http.StatusOK, &softwareListResp)
			...
			if tc.shouldFailTeamRead {
				...
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{SoftwareTitleListOptions: mdmlab.SoftwareTitleListOptions{TeamID: &team1.ID}}, http.StatusForbidden, &listSoftwareTitlesResp)

				// List software versions for a team.
				var resp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/versions", listSoftwareRequest{SoftwareListOptions: mdmlab.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusForbidden, &resp)

				// Get a team software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", softwareFoo.ID), getSoftwareTitleRequest{}, http.StatusForbidden, &getSoftwareTitleResp)

				// Get a team software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/versions/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)

				// Get a team software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)
				...
				// List all software titles for a team
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/titles", listSoftwareTitlesRequest{SoftwareTitleListOptions: mdmlab.SoftwareTitleListOptions{TeamID: &team1.ID}}, http.StatusOK, &listSoftwareTitlesResp)
				require.Equal(t, 1, listSoftwareTitlesResp.Count)
				require.NotEmpty(t, listSoftwareTitlesResp.CountsUpdatedAt)

				// List software versions for a team.
				var resp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/versions", listSoftwareRequest{SoftwareListOptions: mdmlab.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusOK, &resp)
				require.Equal(t, 1, resp.Count)
				require.NotEmpty(t, resp.CountsUpdatedAt)

				// Get a team software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", softwareFoo.ID), getSoftwareTitleRequest{}, http.StatusOK, &getSoftwareTitleResp)

				// Get a team software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/versions/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a team software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a team count of software vesions using the deprecated endpoint
				var countSoftwareResp countSoftwareResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software/count", countSoftwareRequest{SoftwareListOptions: mdmlab.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusOK, &countSoftwareResp)
				require.Equal(t, 1, countSoftwareResp.Count)

				// List all software versions using the deprecated endpoint for a team
				var softwareListResp listSoftwareResponse
				s.DoJSON("GET", "/api/latest/mdmlab/software", listSoftwareRequest{SoftwareListOptions: mdmlab.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusOK, &softwareListResp)
...
func (s *integrationEnterpriseTestSuite) TestCalendarEvents() {
	...
	newHost := func(name string, teamID *uint) *mdmlab.Host {
		h, err := s.ds.NewHost(ctx, &mdmlab.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
			SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	team1Policy1Calendar, err := s.ds.NewTeamPolicy(
		ctx, team1.ID, nil, mdmlab.PolicyPayload{
	...
	genDistributedReqWithPolicyResults := func(host *mdmlab.Host, policyResults map[uint]*bool) submitDistributedQueryResultsRequestShim {
		var (
	...
	appCfg, err := s.ds.AppConfig(ctx)
	require.NoError(t, err)
	appCfg.Integrations.GoogleCalendar = []*mdmlab.GoogleCalendarIntegration{
		{
			Domain: "example.com",
			ApiKey: map[string]string{
				mdmlab.GoogleCalendarEmail: "calendar-mock@example.com",
			},
		},
	}
	err = s.ds.SaveAppConfig(ctx, appCfg)
	...
	team1.Config.Integrations.GoogleCalendar = &mdmlab.TeamGoogleCalendarIntegration{
		Enable:     true,
	...
	team2.Config.Integrations.GoogleCalendar = &mdmlab.TeamGoogleCalendarIntegration{
		Enable:     true,
	...
	err = s.ds.ReplaceHostDeviceMapping(ctx, host1Team1.ID, []*mdmlab.HostDeviceMapping{
		{
...
func (s *integrationEnterpriseTestSuite) TestCalendarEventsTransferringHosts() {
	...
	err = s.ds.ReplaceHostDeviceMapping(ctx, host1.ID, []*mdmlab.HostDeviceMapping{
		{
			HostID: host1.ID,
			Email:  "user1@example.com",
			Source: "google_chrome_profiles",
	...
	team2Policy1, err := s.ds.NewTeamPolicy(
		ctx, team2.ID, nil, mdmlab.PolicyPayload{
			Name:                  "team2Policy1",
			Query:                 "SELECT 2;",
			CalendarEventsEnabled: true,
		},
...
func (s *integrationEnterpriseTestSuite) TestLabelsHostsCounts() {
	...
	users := []mdmlab.UserPayload{
		{
			...
			AdminForcedPasswordReset: ptr.Bool(false),
			Teams: &[]mdmlab.UserTeam{
				{Team: mdmlab.Team{ID: tm1.ID}, Role: mdmlab.RoleMaintainer},
			},
		},
		{
			...
			AdminForcedPasswordReset: ptr.Bool(false),
			Teams: &[]mdmlab.UserTeam{
				{Team: mdmlab.Team{ID: tm2.ID}, Role: mdmlab.RoleAdmin},
			},
		},
		{
			...
			AdminForcedPasswordReset: ptr.Bool(false),
			Teams: &[]mdmlab.UserTeam{
				{Team: mdmlab.Team{ID: tm1.ID}, Role: mdmlab.RoleObserver},
				{Team: mdmlab.Team{ID: tm2.ID}, Role: mdmlab.RoleObserverPlus},
			},
		},
	}
	for _, u := range users {
		var createResp createUserResponse
		s.DoJSON("POST", "/api/latest/mdmlab/users/admin", u, http.StatusOK, &createResp)
	}
	...
	adminUserPayload := mdmlab.UserPayload{
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/labels/%d", c.lblID), nil, http.StatusOK, &getLbl)
			require.Equal(t, c.want, getLbl.Label.Count)

			var listLbls listLabelsResponse
			s.DoJSON("GET", "/api/latest/mdmlab/labels", nil, http.StatusOK, &listLbls)
			var found bool
			...
			if c.u != adminUserPayload {
				...
				s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/labels/%d", c.lblID), modifyLabelRequest{
					ModifyLabelPayload: mdmlab.ModifyLabelPayload{
...
func (s *integrationEnterpriseTestSuite) TestListHostSoftware() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw)
	require.Len(t, getHostSw.Software, 0)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "self_service", "true")
	require.Len(t, getHostSw.Software, 0)

	var getDeviceSw getDeviceSoftwareResponse
	res := s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/software", nil, http.StatusOK)
	err := json.NewDecoder(res.Body).Decode(&getDeviceSw)
	require.NoError(t, err)
	require.Len(t, getDeviceSw.Software, 0)
	res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/software?self_service=1", nil, http.StatusOK)
	err = json.NewDecoder(res.Body).Decode(&getDeviceSw)
	require.NoError(t, err)
	require.Len(t, getDeviceSw.Software, 0)
	...
	_, err = s.ds.InsertSoftwareVulnerability(ctx, mdmlab.SoftwareVulnerability{SoftwareID: barSoftwareID, CVE: "CVE-bar-1234"}, mdmlab.NVDSource)
	require.NoError(t, err)
	_, err = s.ds.InsertSoftwareVulnerability(ctx, mdmlab.SoftwareVulnerability{SoftwareID: barSoftwareID, CVE: "CVE-bar-5678"}, mdmlab.NVDSource)
	require.NoError(t, err)

	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "vulnerable", "true")
	...
	s.DoJSON("POST", "/api/latest/mdmlab/labels", &createLabelRequest{mdmlab.LabelPayload{
		Name:  "label1",
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install",
	...
	require.Equal(t, mdmlab.SoftwareInstallPending, *getHostSw.Software[2].Status)
	require.NotNil(t, getHostSw.Software[2].SoftwarePackage.SelfService)
	require.True(t, *getHostSw.Software[2].SoftwarePackage.SelfService)
	...
	require.Equal(t, mdmlab.SoftwareInstallPending, *getDeviceSw.Software[2].Status)
	...
	// test with a query
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "query", "foo")
	...
	res = s.DoRawNoAuth("GET", "/api/latest/mdmlab/device/"+token+"/software?query=bar", nil, http.StatusOK)
	getDeviceSw = getDeviceSoftwareResponse{}
	err = json.NewDecoder(res.Body).Decode(&getDeviceSw)
	require.NoError(t, err)
	...
	// Add new software to host -- installed on host, but not by MDMlab
	installedVersion := "1.0.1"
	softwareAlreadyInstalled := mdmlab.Software{
	...
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "available_for_install",
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerUploadDownloadAndDelete() {
	...
	checkSoftwareInstaller := func(t *testing.T, payload *mdmlab.UploadSoftwareInstallerPayload) (installerID uint, titleID uint) {
		var tid uint
	...
	t.Run("upload no team software installer", func(t *testing.T) {
		...
		s.lastActivityMatches(mdmlab.ActivityTypeAddedSoftware{}.ActivityName(), activityData, 0)

		// upload again fails
		s.uploadSoftwareInstaller(t, payload, http.StatusConflict, "already exists")

		// update should succeed
		s.updateSoftwareInstaller(t, &mdmlab.UpdateSoftwareInstallerPayload{
		...
		"team_id": null, "self_service": true, "software_title_id": %d, "labels_include_any": [{"id": %d, "name": %q}]}`,
			titleID, labelResp.Label.ID, t.Name())
		s.lastActivityMatches(mdmlab.ActivityTypeEditedSoftware{}.ActivityName(), activityData, 0)
		...
		s.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package", titleID), body.Bytes(), http.StatusOK, headers)
		...
		resp := s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusBadRequest)
		...
		s.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package", titleID), b3.Bytes(), http.StatusOK, headers)
		expectedPayload.PreInstallQuery = "some other pre install query"
		expectedPayload.LabelsIncludeAny = []string{labelResp.Label.Name}
		expectedPayload.LabelsExcludeAny = nil
		checkSoftwareInstaller(t, &expectedPayload)

		s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusAccepted)
		...
		s.Do("POST", "/api/mdmlab/orbit/software_install/package?alt=media", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  123,
			OrbitNodeKey: uuid.NewString(),
		}, http.StatusUnauthorized)

		// download the installer
		s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package?alt=media", titleID), nil, http.StatusBadRequest)

		// delete the installer from nil team fails
		s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", titleID), nil, http.StatusBadRequest)

		// delete from team 0 succeeds
		s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent, "team_id", "0")
		activityData = fmt.Sprintf(`{"software_title": "ruby", "software_package": "ruby.deb", "team_name": null,
		"team_id": null, "self_service": true, "labels_include_any": [{"id": %d, "name": %q}]}`,
			labelResp.Label.ID, labelResp.Label.Name)
		s.lastActivityMatches(mdmlab.ActivityTypeDeletedSoftware{}.ActivityName(), activityData, 0)
	})

	t.Run("create team software installer", func(t *testing.T) {
		var createTeamResp teamResponse
		s.DoJSON("POST", "/api/latest/mdmlab/teams", &mdmlab.Team{
			Name: t.Name(),
		...
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeAddedSoftware{}.ActivityName(), activityData, 0)

		// upload again fails
		s.uploadSoftwareInstaller(t, payload, http.StatusConflict, "already exists")

		// download the installer
		r := s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package?alt=media", titleID), nil, http.StatusOK, "team_id", fmt.Sprintf("%d", *payload.TeamID))
		...
		s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package/token?alt=media", titleID), nil, http.StatusOK,
			&tokenResp, "team_id", fmt.Sprintf("%d", *payload.TeamID))
		require.NotEmpty(t, tokenResp.Token)
		r = s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package/token/%s", titleID, tokenResp.Token), nil,
			http.StatusOK)
		checkDownloadResponse(t, r, payload.Filename)

		// downloading a second time using the same token should fail
		_ = s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package/token/%s", titleID, tokenResp.Token), nil,
			http.StatusForbidden)

		// alt != media should fail
		s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package/token?alt=bozo", titleID), nil,
			http.StatusUnprocessableEntity,
			&tokenResp, "team_id", fmt.Sprintf("%d", *payload.TeamID))

		// missing team_id should fail
		s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package/token?alt=media", titleID), nil,
			http.StatusBadRequest,
			&tokenResp)
		...
		s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", hostInTeam.ID, titleID), installSoftwareRequest{}, http.StatusAccepted)

		// requesting download with alt != media fails
		r = s.Do("POST", "/api/mdmlab/orbit/software_install/package?alt=FOOBAR", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  installerID,
		...
		// valid download
		r = s.Do("POST", "/api/mdmlab/orbit/software_install/package?alt=media", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  installerID,
			OrbitNodeKey: *hostInTeam.OrbitNodeKey,
		}, http.StatusOK)
		checkDownloadResponse(t, r, payload.Filename)
		...
		s.Do("POST", "/api/mdmlab/orbit/software_install/result", orbitPostSoftwareInstallResultRequest{
			OrbitNodeKey: *hostInTeam.OrbitNodeKey,
			HostSoftwareInstallResultPayload: &mdmlab.HostSoftwareInstallResultPayload{
		...
		_ = s.Do("POST", "/api/mdmlab/orbit/software_install/package?alt=media", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  installerID,
			OrbitNodeKey: *hostInTeam.OrbitNodeKey,
		}, http.StatusForbidden)

		// delete the installer
		s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent, "team_id", fmt.Sprintf("%d", *payload.TeamID))

		// check activity
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeDeletedSoftware{}.ActivityName(), fmt.Sprintf(`{"software_title": "ruby", "software_package": "ruby.deb", "team_name": "%s", "team_id": %d, "self_service": true}`, createTeamResp.Team.Name, createTeamResp.Team.ID), 0)

		// download the installer, not found anymore
		s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package?alt=media", titleID), nil, http.StatusNotFound, "team_id", fmt.Sprintf("%d", *payload.TeamID))
	})
	...
	t.Run("create team 0 software installer", func(t *testing.T) {
		...
		// check activity
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeAddedSoftware{}.ActivityName(),
		...
		// download the installer
		r := s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package?alt=media", titleID), nil, http.StatusOK, "team_id", fmt.Sprintf("%d", 0))
		checkDownloadResponse(t, r, payload.Filename)
		...
		// check activity
		s.lastActivityOfTypeMatches(mdmlab.ActivityTypeDeletedSoftware{}.ActivityName(),
			`{"software_title": "ruby", "software_package": "ruby.deb", "team_name": null, "team_id": null, "self_service": true}`, 0)

		// download the installer, not found anymore
		s.Do("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/package?alt=media", titleID), nil, http.StatusNotFound, "team_id", fmt.Sprintf("%d", 0))
	})
	...
	t.Run("uninstall migration for software installer", func(t *testing.T) {
		...
		respTitle := getSoftwareTitleResponse{}
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", titleID), nil, http.StatusOK, &respTitle, "team_id",
		...
		// delete the installer
		s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent,
...
func (s *integrationEnterpriseTestSuite) TestApplyTeamsSoftwareConfig() {
	...
	wantSoftwarePackages := []mdmlab.SoftwarePackageSpec{
		{
			URL:               "http://foo.com",
			SelfService:       true,
			InstallScript:     mdmlab.TeamSpecSoftwareAsset{Path: "./foo/install-script.sh"},
			PostInstallScript: mdmlab.TeamSpecSoftwareAsset{Path: "./foo/post-install-script.sh"},
			PreInstallQuery:   mdmlab.TeamSpecSoftwareAsset{Path: "./foo/query.yaml"},
		},
		{
			URL:               "http://bar.com",
			SelfService:       false,
			InstallScript:     mdmlab.TeamSpecSoftwareAsset{Path: "./bar/install-script.sh"},
			PostInstallScript: mdmlab.TeamSpecSoftwareAsset{Path: "./bar/post-install-script.sh"},
			PreInstallQuery:   mdmlab.TeamSpecSoftwareAsset{Path: "./bar/query.yaml"},
		},
	}
	wantAppStoreApps := []mdmlab.TeamSpecAppStoreApp{
		{
			AppStoreID: "1234",
		},
		{
...
func (s *integrationEnterpriseTestSuite) TestBatchSetSoftwareInstallers() {
	...
	// non-existent team
	s.Do("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{}, http.StatusNotFound, "team_name", "foo")

	// create a team
	tm, err := s.ds.NewTeam(context.Background(), &mdmlab.Team{
		Name:        t.Name(),
		Description: "desc",
	})
	require.NoError(t, err)
	...
	softwareToInstall := []*mdmlab.SoftwareInstallerPayload{
		{URL: "."},
	}
	s.Do("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusUnprocessableEntity, "team_name", tm.Name)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	message := waitBatchSetSoftwareInstallersFailed(t, s, tm.Name, batchResponse.RequestUUID)
	require.NotEmpty(t, message)
	require.Contains(t, message, fmt.Sprintf("validation failed: software.url Couldn't edit software. URL (\"%s/not_found.pkg\") returned \"Not Found\". Please make sure that URLs are reachable from your MDMlab server.", srv.URL))
	...
	softwareToInstallBadSecret := []*mdmlab.SoftwareInstallerPayload{
		{
			URL:           rubyURL,
			InstallScript: "echo $FLEET_SECRET_INVALID",
		},
	}
	resp := s.Do("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstallBadSecret}, http.StatusUnprocessableEntity, "team_name", tm.Name)
	...
	s.DoJSON("GET", "/api/v1/mdmlab/software/titles", nil, http.StatusOK, &titlesResp, "available_for_install", "true", "team_id",
		fmt.Sprint(tm.ID))
	...
	s.DoJSON("GET", "/api/v1/mdmlab/software/titles", nil, http.StatusOK, &newTitlesResp, "available_for_install", "true", "team_id",
		fmt.Sprint(tm.ID))
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse)
	...
	titlesResp = listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/v1/mdmlab/software/titles", nil, http.StatusOK, &titlesResp, "available_for_install", "true", "team_id", strconv.Itoa(int(0)))
	require.Equal(t, 1, titlesResp.Count)
	require.Len(t, titlesResp.SoftwareTitles, 1)
	...
	require.Nil(t, packages[0].TeamID)
	newTitlesResp = listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/v1/mdmlab/software/titles", nil, http.StatusOK, &newTitlesResp, "available_for_install", "true", "team_id", strconv.Itoa(int(0)))
	require.Equal(t, titlesResp, newTitlesResp)
	...
	lblA, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "A"})
	require.NoError(t, err)
	lblB, err := s.ds.NewLabel(ctx, &mdmlab.Label{Name: "B"})
	require.NoError(t, err)
	...
	res := s.Do("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusBadRequest)
...
func waitBatchSetSoftwareInstallersCompleted(t *testing.T, s *integrationEnterpriseTestSuite, teamName string, requestUUID string) []mdmlab.SoftwarePackageResponse {
	timeout := time.After(1 * time.Minute)
	for {
		var batchResultResponse batchSetSoftwareInstallersResultResponse
		s.DoJSON("GET", "/api/latest/mdmlab/software/batch/"+requestUUID, nil, http.StatusOK, &batchResultResponse, "team_name", teamName)
		if batchResultResponse.Status == mdmlab.BatchSetSoftwareInstallersStatusCompleted {
...
func waitBatchSetSoftwareInstallersFailed(t *testing.T, s *integrationEnterpriseTestSuite, teamName string, requestUUID string) string {
	...
	for {
		var batchResultResponse batchSetSoftwareInstallersResultResponse
		s.DoJSON("GET", "/api/latest/mdmlab/software/batch/"+requestUUID, nil, http.StatusOK, &batchResultResponse, "team_name", teamName)
		if batchResultResponse.Status == mdmlab.BatchSetSoftwareInstallersStatusFailed {
...
func (s *integrationEnterpriseTestSuite) TestBatchSetSoftwareInstallersSideEffects() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/software/titles/%d", titlesResp.SoftwareTitles[0].ID), nil, http.StatusOK, &titleResponse,
		"team_id", fmt.Sprint(tm.ID))
	uploadedAt := titleResponse.SoftwareTitle.SoftwarePackage.UploadedAt

	// create a host that doesn't have mdmlabd installed
	h, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	// host installs mdmlabd
	orbitKey := setOrbitEnrollment(t, h, s.ds)
	h.OrbitNodeKey = &orbitKey

	// create another host that doesn't have mdmlabd installed
	h2, err := s.ds.NewHost(context.Background(), &mdmlab.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h.ID, titlesResp.SoftwareTitles[0].ID), nil, http.StatusAccepted, &installResp)

	// Get the install response, should be pending
	getHostSoftwareResp := getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Equal(t, mdmlab.SoftwareInstallPending, *getHostSoftwareResp.Software[0].Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h.ID), nil, http.StatusOK, &afterSelfServiceHostResp)
	require.Equal(t, mdmlab.SoftwareInstallPending, *getHostSoftwareResp.Software[0].Status)

	// update pre-install query
	withUpdatedPreinstallQuery := []*mdmlab.SoftwareInstallerPayload{
		{URL: srv.URL, PreInstallQuery: "SELECT * FROM os_version"},
	}
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: withUpdatedPreinstallQuery}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, tm.Name, batchResponse.RequestUUID)
	require.Len(t, packages, 1)
	require.NotNil(t, packages[0].TitleID)
	require.NotNil(t, packages[0].TeamID)
	...
	require.Equal(t, srv.URL, packages[0].URL)
	titleResponse = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/software/titles/%d", newTitlesResp.SoftwareTitles[0].ID), nil, http.StatusOK, &titleResponse,
		"team_id", fmt.Sprint(tm.ID))
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h.ID), nil, http.StatusOK, &afterPreinstallHostResp)
	...
	s.Do("POST", "/api/mdmlab/orbit/software_install/result", json.RawMessage(fmt.Sprintf(`{
			"orbit_node_key": %q,
			"install_uuid": %q,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h.ID), nil, http.StatusOK, &hostResp)
	require.Equal(t, mdmlab.SoftwareInstalled, *hostResp.Software[0].Status)

	// update install script
	withUpdatedInstallScript := []*mdmlab.SoftwareInstallerPayload{
		{URL: srv.URL, InstallScript: "apt install ruby"},
	}
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: withUpdatedInstallScript}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, tm.Name, batchResponse.RequestUUID)
	require.Len(t, packages, 1)
	require.NotNil(t, packages[0].TitleID)
	require.NotNil(t, packages[0].TeamID)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/install/%s/results", installUUID), nil, http.StatusOK, &installDetailsResp)
	require.Equal(t, mdmlab.SoftwareInstalled, installDetailsResp.Results.Status)

	// queue another install before we delete the installer via batch
	pendingResp := installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h.ID, titlesResp.SoftwareTitles[0].ID), nil, http.StatusAccepted, &pendingResp)

	// install should show as pending
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h.ID), nil, http.StatusOK, &afterPreinstallHostResp)
	require.Equal(t, mdmlab.SoftwareInstallPending, *afterPreinstallHostResp.Software[0].Status)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/uninstall", h2.ID, titlesResp.SoftwareTitles[0].ID), nil, http.StatusAccepted, &uninstallResp)

	// uninstall should show as pending
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h2.ID), nil, http.StatusOK, &afterPreinstallHostResp)
	require.Equal(t, mdmlab.SoftwareUninstallPending, *afterPreinstallHostResp.Software[0].Status)

	// delete all installers
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: []*mdmlab.SoftwareInstallerPayload{}}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, tm.Name, batchResponse.RequestUUID)
	...
	installDetailsResp = getSoftwareInstallResultsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/install/%s/results", installUUID), nil, http.StatusNotFound, &installDetailsResp)
}
...
func (s *integrationEnterpriseTestSuite) TestBatchSetSoftwareInstallersWithPoliciesAssociated() {
	...
	team1, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team1"})
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "team2"})
	require.NoError(t, err)
	...
	var batchResponse batchSetSoftwareInstallersResponse
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", team1.Name)
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", team2.Name)
	...
	mtplr := modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, policy1Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team2.ID, policy2Team2.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: rubyDebTitleID},
		},
	}, http.StatusOK, &mtplr)
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerNewInstallRequestPlatformValidation() {
	...
	hostsByPlatform := map[string]*mdmlab.Host{
	...
	for _, tc := range testCases {
		for platform, host := range hostsByPlatform {
			for _, kind := range tc.supportedInstallers {
				...
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", host.ID, softwareTitles[kind]), nil,
					wantStatus, &resp)
			}
		}
	}
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerHostRequests() {
	...
	var resp installSoftwareResponse
	// non-existent host
	s.DoJSON("POST", "/api/latest/mdmlab/hosts/1/software/1/install", nil, http.StatusNotFound, &resp)
	s.DoJSON("POST", "/api/latest/mdmlab/hosts/1/software/1/uninstall", nil, http.StatusNotFound, &resp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/1/install", h.ID), nil, http.StatusUnprocessableEntity, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/1/uninstall", h.ID), nil, http.StatusUnprocessableEntity, &resp)
	...
	resp = installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/1/install", h.ID), nil, http.StatusBadRequest, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/1/uninstall", h.ID), nil, http.StatusBadRequest, &resp)
	...
	payloadDummy := &mdmlab.UploadSoftwareInstallerPayload{
		Filename: "dummy_installer.pkg",
		Title:    "DummyApp.app",
		TeamID:   teamID,
	}
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", pkgTitleID), nil, http.StatusOK, &respTitle, "team_id",
		fmt.Sprintf("%d", *teamID))
	require.NotNil(t, respTitle.SoftwareTitle.SoftwarePackage)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h.ID, pkgTitleID), nil, http.StatusBadRequest, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/uninstall", h.ID, pkgTitleID), nil, http.StatusBadRequest, &resp)

	// delete software installer which we will not use
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d/available_for_install", pkgTitleID), nil, http.StatusNoContent,
		"team_id", fmt.Sprintf("%d", *teamID))

	// install script request succeeds
	resp = installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h.ID, titleID), nil, http.StatusAccepted, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/install/%s/results", installUUID), nil, http.StatusOK, &gsirr)
	...
	require.Equal(t, mdmlab.SoftwareInstallPending, results.Status)

	// Can't install/uninstall if software install is pending
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h.ID, titleID), nil, http.StatusBadRequest, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/uninstall", h.ID, titleID), nil, http.StatusBadRequest, &resp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h2.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h2.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h3.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h3.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", h4.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h4.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", titleID), nil, http.StatusOK, &titleResp, "team_id",
		fmt.Sprint(*teamID))
	...
	require.Equal(t, mdmlab.SoftwareInstallerStatusSummary{
		Installed:      1,
		PendingInstall: 2,
		FailedInstall:  1,
	...
	for _, c := range cases {
		t.Run(c.status, func(t *testing.T) {
			...
			s.DoJSON("GET", "/api/latest/mdmlab/hosts", nil, http.StatusOK, &listResp, "software_status", c.status, "team_id",
			...
			var countResp countHostsResponse
			s.DoJSON("GET", "/api/latest/mdmlab/hosts/count", nil, http.StatusOK, &countResp, "software_status", c.status, "team_id",
				fmt.Sprint(*teamID), "software_title_id", fmt.Sprint(titleID))
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/labels/%d/hosts", labelResp.Label.ID), nil, http.StatusOK, &listResp,
	...
	// filter validations
	r := s.Do("GET", "/api/latest/mdmlab/hosts", nil, http.StatusBadRequest, "software_status", "uninstalled")
	require.Contains(t, extractServerErrorText(r.Body), "Invalid software_status")
	r = s.Do("GET", "/api/latest/mdmlab/hosts", nil, http.StatusBadRequest, "software_status", "installed")
	require.Contains(t, extractServerErrorText(r.Body), "Missing software_title_id")
	r = s.Do("GET", "/api/latest/mdmlab/hosts", nil, http.StatusBadRequest, "software_status", "installed", "software_title_id", "1")
	require.Contains(t, extractServerErrorText(r.Body), "Missing team_id")
	r = s.Do("GET", "/api/latest/mdmlab/hosts", nil, http.StatusBadRequest, "software_status", "installed", "team_id", "1")
	require.Contains(t, extractServerErrorText(r.Body), "Missing software_title_id")
	r = s.Do("GET", "/api/latest/mdmlab/hosts", nil, http.StatusBadRequest, "software_status", "installed", "team_id", "1", "software_title_id", "1", "software_version_id", "1")
	...
	r = s.Do("GET", "/api/latest/mdmlab/hosts", nil, http.StatusBadRequest, "software_status", "installed", "team_id", "1", "software_title_id", "1", "software_id", "1")
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/lock", h.ID), nil, http.StatusNoContent)
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/scripts/result",
		...
		http.StatusOK, &orbitScriptResp)
	s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/unlock", h.ID), nil, http.StatusNoContent)
	status, err = s.ds.GetHostLockWipeStatus(context.Background(), h)
	require.NoError(t, err)
	s.DoJSON("POST", "/api/mdmlab/orbit/scripts/result",
	...
	// Do uninstall
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/uninstall", h.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", h.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	assert.NotNil(t, getHostSoftwareResp.Software[0].SoftwarePackage.LastInstall)
	assert.Equal(t, mdmlab.SoftwareUninstallPending, *getHostSoftwareResp.Software[0].Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", h.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	assert.Equal(t, mdmlab.ActivityTypeUninstalledSoftware{}.ActivityName(), listUpcomingAct.Activities[0].Type)
	details := make(map[string]interface{}, 5)
	require.NoError(t, json.Unmarshal(*listUpcomingAct.Activities[0].Details, &details))
	assert.EqualValues(t, mdmlab.SoftwareUninstallPending, details["status"])
	...
	require.NotNil(t, titleResp.SoftwareTitle.SoftwarePackage.Status)
	assert.Equal(t, mdmlab.SoftwareInstallerStatusSummary{
		PendingInstall:   1,
		FailedInstall:    1,
		PendingUninstall: 1,
	}, *titleResp.SoftwareTitle.SoftwarePackage.Status)
	...
	// Check activity feed
	var activitiesResp listActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities", h.ID), nil, http.StatusOK, &activitiesResp, "order_key", "a.id",
		"order_direction", "desc")
	require.NotEmpty(t, activitiesResp.Activities)
	assert.Equal(t, mdmlab.ActivityTypeUninstalledSoftware{}.ActivityName(), activitiesResp.Activities[0].Type)
	details = make(map[string]interface{}, 5)
	...
	results = gsirr.Results
	require.Equal(t, installUUID, results.InstallUUID)
	require.Equal(t, mdmlab.SoftwareInstalled, results.Status)

	var scriptResultResp getScriptResultResponse
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+uninstallExecutionID, nil, http.StatusOK, &scriptResultResp)
...
func (s *integrationEnterpriseTestSuite) TestSelfServiceSoftwareInstall() {
	...
	payloadNoSS := &mdmlab.UploadSoftwareInstallerPayload{
	...
	payloadSS := &mdmlab.UploadSoftwareInstallerPayload{
	...
	res := s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/software/install/%d", token, titleIDNoSS), nil, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	...
	payloadLabelSS := &mdmlab.UploadSoftwareInstallerPayload{
	...
	resp := s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/software/install/%d", token, titleIDLabelSS), nil, http.StatusBadRequest)
	require.Contains(t, extractServerErrorText(resp.Body), "Couldn't install. Host isn't member of the labels defined for this software title.")

	// request self-install of software that allows it (is self-service + label scoped)
	s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mdmlab/device/%s/software/install/%d", token, titleIDSS), nil, http.StatusAccepted)

	// it shows up as "self-installed" in the upcoming activities of the host
	var listUpcomingAct listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", host1.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	require.Nil(t, listUpcomingAct.Activities[0].ActorID)

	var details mdmlab.ActivityTypeInstalledSoftware
	...
	require.EqualValues(t, mdmlab.SoftwareInstallPending, details.Status)
	installID := details.InstallUUID

	// record the installation results
	s.Do("POST", "/api/mdmlab/orbit/software_install/result",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities", host1.ID), nil, http.StatusOK, &listPastAct)
	...
	require.Equal(t, details.SoftwareTitle, payloadSS.Title)
	require.True(t, details.SelfService)
	require.EqualValues(t, mdmlab.SoftwareInstalled, details.Status)
}
...
func (s *integrationEnterpriseTestSuite) TestHostSoftwareInstallResult() {
	...
	s.uploadSoftwareInstaller(t, payload, http.StatusOK, "")
	titleID := getSoftwareTitleID(t, s.ds, payload.Title, "deb_packages")
	payload2 := &mdmlab.UploadSoftwareInstallerPayload{
	...
	payload3 := &mdmlab.UploadSoftwareInstallerPayload{
	...
	for i := 0; i < len(installUUIDs); i++ {
		resp := installSoftwareResponse{}
		s.DoJSON("POST", fmt.Sprintf("/api/v1/mdmlab/hosts/%d/software/%d/install", host.ID, titleIDs[i]), nil, http.StatusAccepted, &resp)
	...
	checkResults := func(want result) {
		...
		s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/software/install/%s/results", want.InstallUUID), nil, http.StatusOK, &resp)
	...
	checkResults(result{
		...
		PreInstallQueryOutput: ptr.String(mdmlab.SoftwareInstallerQuerySuccessCopy),
		Output:                ptr.String(fmt.Sprintf(mdmlab.SoftwareInstallerInstallFailCopy, "failed")),
	})
	wantAct := mdmlab.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
	...
	checkResults(result{
		...
		Status:                mdmlab.SoftwareInstallFailed,
		PreInstallQueryOutput: ptr.String(mdmlab.SoftwareInstallerQueryFailCopy),
	})
	wantAct = mdmlab.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
		HostDisplayName: host.DisplayName(),
	...
	checkResults(result{
		...
		PreInstallQueryOutput:   ptr.String(mdmlab.SoftwareInstallerQuerySuccessCopy),
		Output:                  ptr.String(fmt.Sprintf(mdmlab.SoftwareInstallerInstallSuccessCopy, "success")),
		PostInstallScriptOutput: ptr.String(fmt.Sprintf(mdmlab.SoftwareInstallerPostInstallSuccessCopy, "ok")),
	})
	wantAct = mdmlab.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
		HostDisplayName: host.DisplayName(),
...
func (s *integrationEnterpriseTestSuite) TestHostScriptSoftDelete() {
	...
	tm, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: "host_soft_delete_team"})
	require.NoError(t, err)
	host := createOrbitEnrolledHost(t, "linux", "", s.ds)
	...
	s.Do("POST", "/api/mdmlab/orbit/scripts/result",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedScriptID}, http.StatusAccepted, &runResp)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+scriptExecID, nil, http.StatusOK, &scriptRes)
	...
	s.DoJSON("GET", "/api/latest/mdmlab/scripts/results/"+savedScriptExecID, nil, http.StatusOK, &scriptRes)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", host.ID), nil, http.StatusOK, &deleteResp)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mdmlab/scripts/%d", savedScriptID), nil, http.StatusNoContent)
...
func genDistributedReqWithPolicyResults(host *mdmlab.Host, policyResults map[uint]*bool) submitDistributedQueryResultsRequestShim {
	var (
		results  = make(map[string]json.RawMessage)
		statuses = make(map[string]interface{})
		messages = make(map[string]string)
...
func triggerAndWait(ctx context.Context, t *testing.T, ds mdmlab.Datastore, s *schedule.Schedule, timeout time.Duration) {
	...
	for {
		stats, err := ds.GetLatestCronStats(ctx, s.Name())
		require.NoError(t, err)
		if len(stats) > 0 && stats[0].ID > previousRunID && stats[0].Status == mdmlab.CronStatsStatusCompleted {
...
func (s *integrationEnterpriseTestSuite) cleanupQuery(queryID uint) {
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mdmlab/queries/id/%d", queryID), nil, http.StatusOK, &delResp)
}
...
func (s *integrationEnterpriseTestSuite) TestAutofillPoliciesAuthTeamUser() {
	...
	tests := []struct {
		...
		{role: mdmlab.RoleAdmin, pass: true},
		{role: mdmlab.RoleMaintainer, pass: true},
		{role: mdmlab.RoleGitOps, pass: true},
		{role: mdmlab.RoleObserver, pass: false},
		{role: mdmlab.RoleObserverPlus, pass: false},
	}
		...
		t.Run(
			tt.role, func(t *testing.T) {
				...
				if tt.pass {
					...
					s.DoJSON("POST", "/api/latest/mdmlab/autofill/policy", req, http.StatusOK, &res)
					assert.Equal(t, "description", res.Description)
					assert.Equal(t, "resolution", res.Resolution)
				} else {
					_ = s.Do("POST", "/api/latest/mdmlab/autofill/policy", req, http.StatusForbidden)
				}
			},
		)
	}
...
func (s *integrationEnterpriseTestSuite) TestPKGNewSoftwareTitleFlow() {
	...
	team, err := s.ds.NewTeam(ctx, &mdmlab.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)
...
func (s *integrationEnterpriseTestSuite) TestAutomaticPoliciesWithExeFails() {
	...
	s.uploadSoftwareInstaller(t, payload, http.StatusBadRequest, "Couldn't add. MDMlab can't create a policy to detect existing installations for .exe packages. Please add the software, add a custom policy, and enable the install software policy automation.")
}
...
func (s *integrationEnterpriseTestSuite) TestCalendarCallback() {
	...
	appCfg.Integrations.GoogleCalendar = []*mdmlab.GoogleCalendarIntegration{
		{
			Domain: "example.com",
			ApiKey: map[string]string{
				mdmlab.GoogleCalendarEmail: calendar.MockEmail,
			},
		},
	}
	err = s.ds.SaveAppConfig(ctx, appCfg)
	...
	// Send a sync command
	_ = s.DoRawWithHeaders("POST", "/api/v1/mdmlab/calendar/webhook/"+event.UUID, []byte(""), http.StatusOK, map[string]string{
		"X-Goog-Channel-Id":     details.ChannelID,
		"X-Goog-Resource-State": "sync",
	})

	// Send a regular callback with bad channel ID
	_ = s.DoRawWithHeaders("POST", "/api/v1/mdmlab/calendar/webhook/"+event.UUID, []byte(""), http.StatusForbidden, map[string]string{
	...
	_ = s.DoRawWithHeaders("POST", "/api/v1/mdmlab/calendar/webhook/"+eventRecreated.UUID, []byte(""), http.StatusOK,
		map[string]string{
			"X-Goog-Channel-Id":     details.ChannelID,
			"X-Goog-Resource-State": "exists",
		})
...
func (s *integrationEnterpriseTestSuite) TestCalendarEventBodyUpdate() {
	...
	calEvents = getEvents()
	require.Len(t, calEvents, 1)
	assert.Contains(t, calEvents[0].Description, mdmlab.CalendarDefaultDescription)
	assert.Contains(t, calEvents[0].Description, mdmlab.CalendarDefaultResolution)
...
func (s *integrationEnterpriseTestSuite) TestVPPAppsWithoutMDM() {
	...
	s.DoJSON("POST", "/api/latest/mdmlab/teams", &createTeamRequest{TeamPayload: mdmlab.TeamPayload{Name: ptr.String("Team 1")}}, http.StatusOK, &newTeamResp)
	team := newTeamResp.Team

	s.Do("POST", "/api/latest/mdmlab/hosts/transfer", &addHostsToTeamRequest{HostIDs: []uint{orbitHost.ID}, TeamID: &team.ID}, http.StatusOK)

	// Add an app so that we don't get a not found error
	app, err := s.ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name:             "App " + t.Name(),
	...
	pkgPayload := &mdmlab.UploadSoftwareInstallerPayload{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", orbitHost.ID), getHostSoftwareRequest{}, http.StatusOK, &resp)
	assert.Len(t, resp.Software, 1)
	assert.NotNil(t, resp.Software[0].SoftwarePackage)
	assert.Nil(t, resp.Software[0].AppStoreApp)

	r := s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", orbitHost.ID, app.TitleID), &installSoftwareRequest{},
		http.StatusUnprocessableEntity)
...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationsSoftwareInstallers() {
	...
	newHost := func(name string, teamID *uint, platform string) *mdmlab.Host {
		h, err := s.ds.NewHost(ctx, &mdmlab.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
	...
	newMDMlabdHost := func(name string, teamID *uint, platform string) *mdmlab.Host {
	...
	host0NoTeam := newMDMlabdHost("host1NoTeam", nil, "darwin")
	host1Team1 := newMDMlabdHost("host1Team1", &team1.ID, "darwin")
	host2Team1 := newMDMlabdHost("host2Team1", &team1.ID, "ubuntu")
	host3Team2 := newMDMlabdHost("host3Team2", &team2.ID, "windows")
	...
	require.NoError(t, err)
	rubyPayload := &mdmlab.UploadSoftwareInstallerPayload{
	...
	// Upload mdmlab-osquery.msi to team2.
	mdmlabOsqueryPayload := &mdmlab.UploadSoftwareInstallerPayload{
	...
	s.uploadSoftwareInstaller(t, mdmlabOsqueryPayload, http.StatusOK, "")
	// Get software title ID of the uploaded installer.
	resp = listSoftwareTitlesResponse{}
	s.DoJSON(
		"GET", "/api/latest/mdmlab/software/titles",
		listSoftwareTitlesRequest{},
		http.StatusOK, &resp,
		"query", "MDMlab osquery",
		"team_id", fmt.Sprintf("%d", team2.ID),
	)
	require.Len(t, resp.SoftwareTitles, 1)
	require.NotNil(t, resp.SoftwareTitles[0].SoftwarePackage)
	mdmlabOsqueryMSITitleID := resp.SoftwareTitles[0].ID
	var mdmlabOsqueryMSIInstallerID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(ctx, q,
			&mdmlabOsqueryMSIInstallerID,
			`SELECT id FROM software_installers WHERE global_or_team_id = ? AND filename = ?`,
			team2.ID, "mdmlab-osquery.msi",
		)
	})
	require.NotZero(t, mdmlabOsqueryMSIInstallerID)

	// Create a VPP app to test that policies *can* be assigned to them (VPP automation is tested in MDM integration test)
	_, err = s.ds.InsertVPPAppWithTeam(ctx, &mdmlab.VPPApp{
		Name:             "App123 " + t.Name(),
		BundleIdentifier: "bid_" + t.Name(),
		VPPAppTeam: mdmlab.VPPAppTeam{
			VPPAppID: mdmlab.VPPAppID{
	...
	policy0AllTeams, err := s.ds.NewGlobalPolicy(ctx, nil, mdmlab.PolicyPayload{
	...
	policy1Team1, err := s.ds.NewTeamPolicy(ctx, team1.ID, nil, mdmlab.PolicyPayload{
		Name:     "policy1Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	policy2Team1, err := s.ds.NewTeamPolicy(ctx, team1.ID, nil, mdmlab.PolicyPayload{
		Name:     "policy2Team1",
		Query:    "SELECT 2;",
	...
	policy3Team1, err := s.ds.NewTeamPolicy(ctx, team1.ID, nil, mdmlab.PolicyPayload{
		Name:  "policy3Team1",
		Query: "SELECT 3;",
	})
	require.NoError(t, err)
	// policy4Team2 runs on Windows devices.
	policy4Team2, err := s.ds.NewTeamPolicy(ctx, team2.ID, nil, mdmlab.PolicyPayload{
		Name:     "policy4Team2",
		Query:    "SELECT 4;",
		Platform: "windows",
	})
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, policy1Team1.ID),
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoRaw("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, policy1Team1.ID), []byte(`{
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team1.ID, policy2Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: rubyDebTitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	require.Equal(t, mdmlab.SoftwareInstallPending, *host1LastInstall.Status)
	...
	require.Equal(t, mdmlab.SoftwareInstallPending, *host2LastInstall.Status)

	// Associate mdmlab-osquery.msi to policy4Team2.
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies/%d", team2.ID, policy4Team2.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: mdmlabOsqueryMSITitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	host3LastInstall, err := s.ds.GetHostLastInstallData(ctx, host3Team2.ID, mdmlabOsqueryMSIInstallerID)
	require.NoError(t, err)
	require.NotNil(t, host3LastInstall)
	require.NotEmpty(t, host3LastInstall.ExecutionID)
	require.NotNil(t, host3LastInstall.Status)
	require.Equal(t, mdmlab.SoftwareInstallPending, *host3LastInstall.Status)
	host3LastInstallDetails, err := s.ds.GetSoftwareInstallDetails(ctx, host3LastInstall.ExecutionID)
	require.NoError(t, err)
	// Even if mdmlab-osquery.msi was uploaded as Self-service, it was installed by MDMlab, so
	...
	var listUpcomingAct listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", host1Team1.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	require.Nil(t, listUpcomingAct.Activities[0].ActorID)
	require.Equal(t, "MDMlab", *listUpcomingAct.Activities[0].ActorFullName)
	...
	s.lastActivityMatches(mdmlab.ActivityTypeInstalledSoftware{}.ActivityName(), fmt.Sprintf(`{
		"host_id": %d,
		"host_display_name": "%s",
		"software_title": "%s",
		"software_package": "%s",
	...
	activityID := s.lastActivityMatches(mdmlab.ActivityTypeInstalledSoftware{}.ActivityName(), fmt.Sprintf(`{
		"host_id": %d,
		"host_display_name": "%s",
		"software_title": "%s",
		"software_package": "%s",
		...
		"policy_id": %d,
		"policy_name": "%s"
	}`, host2Team1.ID, host2Team1.DisplayName(), "ruby", "ruby.deb", host2LastInstall.ExecutionID, mdmlab.SoftwareInstallFailed, policy2Team1.ID, policy2Team1.Name), 0)

	// Check that the activity item generated for ruby.deb installation is shown as coming from MDMlab
	var actor struct {
		UserID     *uint   `db:"user_id"`
		UserName   *string `db:"user_name"`
		UserEmail  string  `db:"user_email"`
	...
	require.Equal(t, "MDMlab", *actor.UserName)
	...
	// host3Team2 posts the installation result for mdmlab-osquery.msi.
	s.Do("POST", "/api/mdmlab/orbit/software_install/result", json.RawMessage(fmt.Sprintf(`{
			"orbit_node_key": %q,
			"install_uuid": %q,
			"pre_install_condition_output": "ok",
	...
	activityID = s.lastActivityMatches(mdmlab.ActivityTypeInstalledSoftware{}.ActivityName(), fmt.Sprintf(`{
		...
		"policy_id": %f,
		"policy_name": "%s"
	}`, host3Team2.ID, host3Team2.DisplayName(), "MDMlab osquery", "mdmlab-osquery.msi", host3LastInstall.ExecutionID,
		mdmlab.SoftwareInstallFailed, float64(policy4Team2.ID), policy4Team2.Name), 0)

	// Check that the activity item generated for mdmlab-osquery.msi installation has MDMlab set as author.
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(ctx, q,
			&actor,
			`SELECT user_id, user_name, user_email, details->>'$.policy_id' policy_id, details->>'$.policy_name' policy_name FROM activities WHERE id = ?`,
	...
	// MDMlab should not queue an install for vanilla osquery hosts.
	distributedResp = submitDistributedQueryResultsResponse{}
	s.DoJSONWithoutAuth("POST", "/api/osquery/distributed/write", genDistributedReqWithPolicyResults(
...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationsSoftwareInstallersLabelScoping() {
	...
	s.DoJSON("POST", "/api/v1/mdmlab/labels", mdmlab.LabelPayload{
		Name:  uuid.NewString(),
		Query: "SELECT 1",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", rubyDebTitleID), nil, http.StatusOK, &rubyDetail)
	...
	policy1, err := s.ds.NewTeamPolicy(ctx, 0, nil, mdmlab.PolicyPayload{
		Name:     "policy1",
		Query:    "SELECT 1;",
		Platform: "linux",
	})
	...
	mtplr := modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/0/policies/%d", policy1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: rubyDebTitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	vimPayload := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:    "some deb install script",
		Filename:         "vim.deb",
		TeamID:           nil,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", vimTitleID), nil, http.StatusOK, &vimDetail)
	...
	policy2, err := s.ds.NewTeamPolicy(ctx, 0, nil, mdmlab.PolicyPayload{
		Name:     "policy2",
		Query:    "SELECT 2;",
		Platform: "linux",
	})
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mdmlab/teams/0/policies/%d", policy2.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mdmlab.ModifyPolicyPayload{
...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationsScripts() {
	...
	winScript, err := s.ds.NewScript(ctx, &mdmlab.Script{
	...
	psScript, err := s.ds.NewScript(ctx, &mdmlab.Script{
		Name:           "windows-script.ps1",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/scripts/run", mdmlab.HostScriptRequestPayload{HostID: host1Team1.ID, ScriptID: &script.ID}, http.StatusConflict, &scriptRunResp)
	...
	// MDMlab should not queue scripts for vanilla osquery hosts.
	distributedResp = submitDistributedQueryResultsResponse{}
	s.DoJSONWithoutAuth("POST", "/api/osquery/distributed/write", genDistributedReqWithPolicyResults(
		hostVanillaOsquery5Team1,
		map[uint]*bool{
	...
	// activity feed should show script run as pending, with "MDMlab" as author, policy ID and name set in body
	var listResp listActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities/upcoming", host3Team2.ID), nil, http.StatusOK, &listResp)
	require.Len(t, listResp.Activities, 1)
	require.Nil(t, listResp.Activities[0].ActorEmail)
	require.Equal(t, "MDMlab", *listResp.Activities[0].ActorFullName)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/activities", host3Team2.ID), nil, http.StatusOK, &listResp)
	require.Len(t, listResp.Activities, 1)
...
func (s *integrationEnterpriseTestSuite) TestSoftwareUploadRPM() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/v1/mdmlab/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusAccepted, &installSoftwareResp)
	...
	var resp getSoftwareInstallResultsResponse
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mdmlab/software/install/%s/results", installUUID), nil, http.StatusOK, &resp)
	assert.Equal(t, host.ID, resp.Results.HostID)
	assert.Equal(t, installUUID, resp.Results.InstallUUID)
	assert.Equal(t, mdmlab.SoftwareInstallFailed, resp.Results.Status)
	assert.NotNil(t, resp.Results.PreInstallQueryOutput)
	assert.Equal(t, mdmlab.SoftwareInstallerQuerySuccessCopy, *resp.Results.PreInstallQueryOutput)
	assert.NotNil(t, resp.Results.Output)
	assert.Equal(t, fmt.Sprintf(mdmlab.SoftwareInstallerInstallFailCopy, "failed"), *resp.Results.Output)
...
func (s *integrationEnterpriseTestSuite) TestMaintainedApps() {
	...
	getSoftwareInstallerIDByMAppID := func(mappID uint) uint {
		var id uint
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
			return sqlx.GetContext(ctx, q, &id, "SELECT id FROM software_installers WHERE mdmlab_library_app_id = ?", mappID)
		})
	...
	s.Do("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", &addMDMlabMaintainedAppRequest{AppID: 1}, http.StatusNotFound)
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		...
		_, err = q.ExecContext(ctx, "UPDATE mdmlab_library_apps SET sha256 = ?, installer_url = ?", spoofedSHA, installerServer.URL+"/installer.zip")
		require.NoError(t, err)
		_, err = q.ExecContext(ctx, "UPDATE mdmlab_library_apps SET installer_url = ? WHERE id = 2", installerServer.URL+"/badinstaller")
		require.NoError(t, err)
		_, err = q.ExecContext(ctx, "UPDATE mdmlab_library_apps SET installer_url = ? WHERE id = 3", installerServer.URL+"/timeout")
		return err
	})
	...
	// Check apps returned
	var listMAResp listMDMlabMaintainedAppsResponse
	s.DoJSON(http.MethodGet, "/api/latest/mdmlab/software/mdmlab_maintained_apps", listMDMlabMaintainedAppsRequest{}, http.StatusOK,
	...
	require.Len(t, listMAResp.MDMlabMaintainedApps, len(expectedApps))
	var listAppsNoID []mdmlab.MaintainedApp
	for _, app := range listMAResp.MDMlabMaintainedApps {
		app.ID = 0
		listAppsNoID = append(listAppsNoID, app)
	}
	slices.SortFunc(listAppsNoID, func(a, b mdmlab.MaintainedApp) int {
		return cmp.Compare(a.Name, b.Name)
	})
	slices.SortFunc(expectedApps, func(a, b mdmlab.MaintainedApp) int {
		return cmp.Compare(a.Name, b.Name)
	})
	require.Equal(t, expectedApps, listAppsNoID)

	var listMAResp2 listMDMlabMaintainedAppsResponse
	s.DoJSON(
		http.MethodGet,
		"/api/latest/mdmlab/software/mdmlab_maintained_apps",
		listMDMlabMaintainedAppsRequest{},
		http.StatusOK,
		&listMAResp2,
	...
	require.Len(t, listMAResp2.MDMlabMaintainedApps, 2)
	require.Equal(t, listMAResp.MDMlabMaintainedApps[4:6], listMAResp2.MDMlabMaintainedApps)

	// Check individual app fetch
	var getMAResp getMDMlabMaintainedAppResponse
	s.DoJSON(http.MethodGet, fmt.Sprintf("/api/latest/mdmlab/software/mdmlab_maintained_apps/%d", listMAResp.MDMlabMaintainedApps[0].ID), getMDMlabMaintainedAppRequest{}, http.StatusOK, &getMAResp)
	// TODO this will change when actual install scripts are created.
	actualApp := listMAResp.MDMlabMaintainedApps[0]
	require.NotEmpty(t, getMAResp.MDMlabMaintainedApp.InstallScript)
	require.NotEmpty(t, getMAResp.MDMlabMaintainedApp.UninstallScript)
	getMAResp.MDMlabMaintainedApp.InstallScript = ""
	getMAResp.MDMlabMaintainedApp.UninstallScript = ""
	require.Equal(t, actualApp, *getMAResp.MDMlabMaintainedApp)

	// Try adding ingested app with invalid secret
	reqInvalidSecret := &addMDMlabMaintainedAppRequest{
		AppID:             1,
		TeamID:            &team.ID,
		SelfService:       true,
		PreInstallQuery:   "SELECT 1",
	...
	respBadSecret := s.Do("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", reqInvalidSecret, http.StatusUnprocessableEntity)
	...
	var addMAResp addMDMlabMaintainedAppResponse
	req := &addMDMlabMaintainedAppRequest{
		AppID:             1,
		TeamID:            &team.ID,
		SelfService:       true,
		PreInstallQuery:   "SELECT 1",
	...
	s.DoJSON("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", req, http.StatusOK, &addMAResp)
	...
	require.Nil(t, listMAResp.Err)
	require.False(t, listMAResp.Meta.HasPreviousResults)
	require.Len(t, listMAResp.MDMlabMaintainedApps, len(expectedApps)-1)
	...
	require.NoError(t, err)
	require.Equal(t, ptr.Uint(1), i.MDMlabLibraryAppID)
	...
	// Check activity
	s.lastActivityOfTypeMatches(
		mdmlab.ActivityTypeAddedSoftware{}.ActivityName(),
	...
	r := s.Do("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", &addMDMlabMaintainedAppRequest{AppID: 2}, http.StatusInternalServerError)
	...
	r = s.Do("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", &addMDMlabMaintainedAppRequest{AppID: 3}, http.StatusGatewayTimeout)
	...
	addMAResp = addMDMlabMaintainedAppResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", req, http.StatusOK, &addMAResp)
	require.Nil(t, addMAResp.Err)
	...
	require.NoError(t, err)
	require.Equal(t, ptr.Uint(4), i.MDMlabLibraryAppID)
	require.Equal(t, mapp.SHA256, i.StorageID)
	require.Equal(t, "darwin", i.Platform)
	require.NotEmpty(t, i.InstallScriptContentID)
	require.Equal(t, req.PreInstallQuery, i.PreInstallQuery)
	...
	tpResp := teamPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mdmlab/teams/0/policies", tpParams, http.StatusOK, &tpResp)
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d", addMAResp.SoftwareTitleID),
	...
	s.DoJSON(
		"GET", "/api/latest/mdmlab/teams/0/policies",
	...
	addMAResp = addMDMlabMaintainedAppResponse{}
	r = s.Do("POST", "/api/latest/mdmlab/software/mdmlab_maintained_apps", req, http.StatusBadRequest)
...
func (s *integrationEnterpriseTestSuite) TestDeleteLabels() {
	...
	installer := &mdmlab.UploadSoftwareInstallerPayload{
		InstallScript:    "install",
		Filename:         "ruby.deb",
		SelfService:      false,
		TeamID:           nil,
	...
	res := s.Do("DELETE", "/api/v1/mdmlab/labels/id/"+fmt.Sprint(lbl1), nil, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	...
	s.DoJSON("DELETE", "/api/v1/mdmlab/labels/id/"+fmt.Sprint(lbl1+1000), nil, http.StatusNotFound, &delLabelResp)
	s.DoJSON("DELETE", "/api/v1/mdmlab/labels/no-such-label", nil, http.StatusNotFound, &delLabelResp)

	// delete the unused label2
	s.DoJSON("DELETE", "/api/v1/mdmlab/labels/id/"+fmt.Sprint(lbl2), nil, http.StatusOK, &delLabelResp)
}
...
func (s *integrationEnterpriseTestSuite) TestListHostSoftwareWithLabelScoping() {
	...
	resp := installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/v1/mdmlab/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusAccepted, &resp)
	installUUID := latestInstallUUID()
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/uninstall", host.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw)
	require.Len(t, getHostSw.Software, 1)
	assert.NotNil(t, getHostSw.Software[0].SoftwarePackage.LastInstall)
	assert.Equal(t, mdmlab.SoftwareUninstallPending, *getHostSw.Software[0].Status)
...
func (s *integrationEnterpriseTestSuite) TestAutomaticPolicies() {
	...
	pkgTitleID = getSoftwareTitleID(t, s.ds, "DummyApp.app", "apps")
	respTitle := getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/software/titles/%d?team_id=%d", pkgTitleID, team1.ID), listSoftwareTitlesRequest{}, http.StatusOK, &respTitle)
	require.NotNil(t, respTitle.SoftwareTitle)
	require.NotNil(t, respTitle.SoftwareTitle.SoftwarePackage)
	...
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mdmlab/teams/%d/policies", team2.ID), nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 1)
	require.Len(t, ts.InheritedPolicies, 0)
	require.Equal(t, "[Install software] DummyApp.app (pkg)", ts.Policies[0].Name)

	// Upload ruby.deb to team1 with automatic policy.
	payloadRubyDEB := &mdmlab.UploadSoftwareInstallerPayload{
		Filename:         "ruby.deb",
		TeamID:           &team1.ID,
		AutomaticInstall: true,
	}
	...
	payloadRubyRPM := &mdmlab.UploadSoftwareInstallerPayload{
		Filename:         "ruby.rpm",
		TeamID:           &team1.ID,
		AutomaticInstall: true,
	}
	...
	// Upload mdmlab-osquery.msi to team1 with automatic policy.
	mdmlabOsqueryPayload := &mdmlab.UploadSoftwareInstallerPayload{
		Filename:         "mdmlab-osquery.msi",
		TeamID:           &team1.ID,
		AutomaticInstall: true,
	...
	require.Equal(t, "[Install software] MDMlab osquery (msi)", ts.Policies[3].Name)
}
