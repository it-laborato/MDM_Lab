...
func (svc *service) PKIOperation(ctx context.Context, data []byte) ([]byte, error) {
	if len(data) == 0 {
		return nil, &mdmlab.BadRequestError{Message: "missing data for PKIOperation"}
	}
...
func NewSCEPService(ds mdmlab.MDMAssetRetriever, signer scepserver.CSRSignerContext, logger log.Logger) scepserver.Service {
	return &service{
		signer:      signer,
