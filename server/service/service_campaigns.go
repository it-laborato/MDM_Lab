...
type statsTracker struct {
	...
	stats             []statsToSave
	lastStatsEntry    *mdmlab.LiveQueryStats
}
...
func (svc Service) StreamCampaignResults(ctx context.Context, conn *websocket.Conn, campaignID uint) {
	...
	if err := svc.authz.Authorize(ctx, &mdmlab.TargetedQuery{Query: &mdmlab.Query{ObserverCanRun: true}}, mdmlab.ActionRun); err != nil {
	...
	var campaign *mdmlab.DistributedQueryCampaign
	go func() {
		var err error
		for {
			select {
	...
	mapHostnameRows := func(res *mdmlab.DistributedQueryResult) {
...
func (svc Service) addLiveQueryActivity(
	...
	activityData := mdmlab.ActivityTypeLiveQuery{
...
func calculateOutputSize(perfStatsTracker *statsTracker, res *mdmlab.DistributedQueryResult) uint64 {
	outputSize := uint64(0)
...
func (svc Service) updateStats(
	...
	if len(tracker.stats) > 0 {
		...
		statsMap := make(map[uint]*mdmlab.LiveQueryStats)
		...
		for _, gatheredStats := range tracker.stats {
			...
			if !ok {
				newStats := mdmlab.LiveQueryStats{
	...
	if aggregateStats && tracker.aggregationNeeded {
		...
		if tracker.lastStatsEntry != nil { // This check is just to be safe. It should never be nil.
			...
			stop := make(chan struct{}, 1)
			go func() {
				var stats []*mdmlab.LiveQueryStats
				var err error
				for {
					select {
					case <-stop:
		...
		err := svc.ds.CalculateAggregatedPerfStatsPercentiles(ctx, mdmlab.AggregatedStatsTypeScheduledQuery, queryID)
		if err != nil {
