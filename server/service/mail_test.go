...
func TestMailService(t *testing.T) {
	...
	ds.InviteFunc = func(ctx context.Context, id uint) (*mdmlab.Invite, error) {
		return invite, nil
	}
  ...
  "smtp_settings": {
	...
	require.Equal(t, "Hello from MDMlab", lastMessage["Subject"])
	...
	lastMessage = getLastMailPitMessage()
	require.Equal(t, "You have been invited to MDMlab!", lastMessage["Subject"])
	...
	_, err = svc.ModifyUser(ctx, 1, mdmlab.UserPayload{
		Email: ptr.String("useradmin_2@example.com"),
	})
	require.NoError(t, err)

	lastMessage = getLastMailPitMessage()
	require.Equal(t, "Confirm MDMlab Email Change", lastMessage["Subject"])
}
