...
func (req *SoapRequestContainer) DecodeBody(ctx context.Context, r io.Reader, u url.Values, c []*x509.Certificate) error {
	...
	req.Data = &mdmlab.SoapRequest{Raw: reqBytes}
...
func (req *SyncMLReqMsgContainer) DecodeBody(ctx context.Context, r io.Reader, u url.Values, c []*x509.Certificate) error {
	...
	// Handle empty body scenario
	req.Data = &mdmlab.SyncML{Raw: reqBytes}
...
func NewSoapResponse(payload interface{}, relatesTo string) (mdmlab.SoapResponse, error) {
	// Sanity check
	if len(relatesTo) == 0 {
		return mdmlab.SoapResponse{}, errors.New("relatesTo is invalid")
	}
	...
	default:
		return mdmlab.SoapResponse{}, errors.New("mdm response message not supported")
	}

	// Return the SoapRequest type with the appropriate fields set
	return mdmlab.SoapResponse{
		XMLNSS: urlNSS,
		XMLNSA: urlNSA,
		XMLNSU: headerXsu,
...
func NewBinarySecurityTokenPayload(encodedToken string) (mdmlab.WindowsMDMAccessTokenPayload, error) {
	if len(encodedToken) == 0 {
		return mdmlab.WindowsMDMAccessTokenPayload{}, errors.New("binary security token: token is empty")
	}

	rawBytes, err := base64.StdEncoding.DecodeString(encodedToken)
	if err != nil {
		return mdmlab.WindowsMDMAccessTokenPayload{}, fmt.Errorf("binary security token: %v", err)
	}

	var tokenPayload mdmlab.WindowsMDMAccessTokenPayload
...
// in MDMlab's Windows MDM (if it was enabled).
func isEligibleForWindowsMDMEnrollment(host *mdmlab.Host, mdmInfo *mdmlab.HostMDM) bool {
	return host.MDMlabPlatform() == "windows" &&
...
// MDMlab's Windows MDM (if it was enabled).
func isEligibleForWindowsMDMMigration(host *mdmlab.Host, mdmInfo *mdmlab.HostMDM) bool {
	return host.MDMlabPlatform() == "windows" &&
		host.IsOsqueryEnrolled() &&
		(mdmInfo != nil && !mdmInfo.IsServer && mdmInfo.Enrolled && mdmInfo.Name != mdmlab.WellKnownMDMMDMlab)
}
...
func mdmMicrosoftDiscoveryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func mdmMicrosoftAuthEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func mdmMicrosoftPolicyEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*SoapRequestContainer).Data
...
func mdmMicrosoftEnrollEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*SoapRequestContainer).Data
...
func mdmMicrosoftManagementEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func mdmMicrosoftTOSEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) authBinarySecurityToken(ctx context.Context, authToken *mdmlab.HeaderBinarySecurityToken) (claim string, hostUUID string, err error) {
	if authToken == nil {
...
func (svc *Service) GetMDMMicrosoftDiscoveryResponse(ctx context.Context, upnEmail string) (*mdmlab.DiscoverResponse, error) {
...
func (svc *Service) GetMDMMicrosoftSTSAuthResponse(ctx context.Context, appru string, loginHint string) (string, error) {
	...
	// encodedBST, err := GetEncodedBinarySecurityToken(mdmlab.WindowsMDMAutomaticEnrollmentType, authToken)
...
func (svc *Service) GetMDMWindowsPolicyResponse(ctx context.Context, authToken *mdmlab.HeaderBinarySecurityToken) (*mdmlab.GetPoliciesResponse, error) {
	if authToken == nil {
		return nil, mdmlab.NewInvalidArgumentError("policy response", "authToken is invalid")
	}
...
func (svc *Service) GetMDMWindowsEnrollResponse(ctx context.Context, secTokenMsg *mdmlab.RequestSecurityToken, authToken *mdmlab.HeaderBinarySecurityToken) (*mdmlab.RequestSecurityTokenResponseCollection, error) {
	if authToken == nil {
		return nil, mdmlab.NewInvalidArgumentError("enroll response", "authToken is not present")
	}
...
func (svc *Service) GetMDMWindowsManagementResponse(ctx context.Context, reqSyncML *mdmlab.SyncML, reqCerts []*x509.Certificate) (*mdmlab.SyncML, error) {
	if reqSyncML == nil {
		return nil, mdmlab.NewInvalidArgumentError("syncml req message", "message is not present")
	}
...
func (svc *Service) isTrustedRequest(ctx context.Context, reqSyncML *mdmlab.SyncML, reqCerts []*x509.Certificate) error {
	if reqSyncML == nil {
		return mdmlab.NewInvalidArgumentError("syncml req message", "message is not present")
	}
...
// isMDMlabdPresentOnDevice checks if the device requires MDMlabd to be deployed
func (svc *Service) isMDMlabdPresentOnDevice(ctx context.Context, deviceID string) (bool, error) {
	...
	// This means that mdmlabd might not be installed
...
func (svc *Service) enqueueInstallMDMlabdCommand(ctx context.Context, deviceID string) error {
	...
	if len(secrets) == 0 {
		level.Warn(svc.logger).Log("msg", "unable to find a global enroll secret to install mdmlabd")
		return nil
	}
	...
	mdmlabdMetadata, err := mdmlabdbase.GetMetadata()
	if err != nil {
		level.Warn(svc.logger).Log("msg", "unable to get mdmlabd-base metadata")
		return nil
	}
	...
	mdmlabURL := appCfg.ServerSettings.ServerURL
...
<Add>
	...
	<Item>
		<Target>
			<LocURI>` + syncml.MDMlabdWindowsInstallerGUID + `</LocURI>
		</Target>
	</Item>
</Add>`)
	...
	// MDMlabURL and MDMlabSecret properties are passed to the MDMlab MSI
	rawExecCmd := []byte(`
<Exec>
	...
	<Item>
		...
		<Data>
			...
			<Product Version="1.0.0.0">
				<Download>
					<ContentURLList>
						<ContentURL>` + mdmlabdMetadata.MSIURL + `</ContentURL>
					</ContentURLList>
				</Download>
				<Validation>
					<FileHash>` + mdmlabdMetadata.MSISha256 + `</FileHash>
				</Validation>
				<Enforcement>
					<CommandLine>/quiet FLEET_URL="` + mdmlabURL + `" FLEET_SECRET="` + globalEnrollSecret + `" ENABLE_SCRIPTS="True"</CommandLine>
					<TimeOut>10</TimeOut>
...
`)
	...
	addMDMlabdCmd := &mdmlab.MDMWindowsCommand{
		CommandUUID:  addCommandUUID,
		RawCommand:   rawAddCmd,
		TargetLocURI: syncml.MDMlabdWindowsInstallerGUID,
	}
	if err := svc.ds.MDMWindowsInsertCommandForHosts(ctx, []string{deviceID}, addMDMlabdCmd); err != nil {
		return ctxerr.Wrap(ctx, err, "insert add command to install mdmlabd")
	}

	execMDMlabCmd := &mdmlab.MDMWindowsCommand{
		CommandUUID:  execCommandUUID,
		RawCommand:   rawExecCmd,
		TargetLocURI: syncml.MDMlabdWindowsInstallerGUID,
	}
	if err := svc.ds.MDMWindowsInsertCommandForHosts(ctx, []string{deviceID}, execMDMlabCmd); err != nil {
		return ctxerr.Wrap(ctx, err, "insert exec command to install mdmlabd")
	}
...
func (svc *Service) processNewSessionAlert(ctx context.Context, messageID string, deviceID string, cmd mdm_types.ProtoCmdOperation) error {
	// Checking if mdmlabd is present on the device
	mdmlabdPresent, err := svc.isMDMlabdPresentOnDevice(ctx, deviceID)
	if err != nil {
		return err
	}

	if !mdmlabdPresent {
		return svc.enqueueInstallMDMlabdCommand(ctx, deviceID)
	}
...
func (svc *Service) processIncomingMDMCmds(ctx context.Context, deviceID string, reqMsg *mdmlab.SyncML) ([]*mdmlab.SyncMLCmd, error) {
	var responseCmds []*mdmlab.SyncMLCmd
	...
	enrichedSyncML := mdmlab.NewEnrichedSyncML(reqMsg)
...
func (svc *Service) createResponseSyncML(ctx context.Context, req *mdmlab.SyncML, responseOps []*mdm_types.SyncMLCmd) (*mdmlab.SyncML, error) {
	// Get the DeviceID
...
func (svc *Service) getManagementResponse(ctx context.Context, reqMsg *mdmlab.SyncML) (*mdm_types.SyncML, error) {
	if reqMsg == nil {
		return nil, mdmlab.NewInvalidArgumentError("syncml req message", "message is not present")
	}
...
func (svc *Service) removeWindowsDeviceIfAlreadyMDMEnrolled(ctx context.Context, secTokenMsg *mdmlab.RequestSecurityToken) error {
...
func (svc *Service) getDeviceProvisioningInformation(ctx context.Context, secTokenMsg *mdmlab.RequestSecurityToken) (string, error) {
	// Getting the HW DeviceID from the RequestSecurityToken msg
	reqHWDeviceID, err := GetContextItem(secTokenMsg, syncml.ReqSecTokenContextItemHWDevID)
	if err != nil {
		return "", err
...
func (svc *Service) storeWindowsMDMEnrolledDevice(ctx context.Context, userID string, hostUUID string, secTokenMsg *mdmlab.RequestSecurityToken) error {
	const (
	...
	err = svc.NewActivity(
		ctx, nil, &mdmlab.ActivityTypeMDMEnrolled{
			HostDisplayName: displayName,
			MDMPlatform:     mdmlab.MDMPlatformMicrosoft,
			HostSerial:      serial,
		})
	if err != nil {
...
func GetContextItem(secTokenMsg *mdmlab.RequestSecurityToken, contextItem string) (string, error) {
...
func (svc *Service) GetAuthorizedSoapFault(ctx context.Context, eType string, origMsg int, errorMsg error) *mdmlab.SoapFault {
	svc.authz.SkipAuthorization(ctx)
...
func createSyncMLMessage(sessionID string, msgID string, deviceID string, source string, protoCommands []*mdm_types.SyncMLCmd) (*mdm_types.SyncML, error) {
	...
	for _, protoCmd := range protoCommands {
		msg.AppendCommand(mdmlab.MDMRaw, *protoCmd)
	}
...
func (svc *Service) GetMDMWindowsProfilesSummary(ctx context.Context, teamID *uint) (*mdmlab.MDMProfilesSummary, error) {
...
func ReconcileWindowsProfiles(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger) error {
	...
	hostProfiles := make([]*mdmlab.MDMWindowsBulkUpsertHostProfilePayload, 0, len(toInstall))
	...
	for _, p := range toInstall {
		...
		hostProfiles = append(hostProfiles, &mdmlab.MDMWindowsBulkUpsertHostProfilePayload{
...
func buildCommandFromProfileBytes(profileBytes []byte, commandUUID string) (*mdmlab.MDMWindowsCommand, error) {
	...
	command := &mdmlab.MDMWindowsCommand{
