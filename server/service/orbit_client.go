...
// OrbitClient exposes the Orbit API to communicate with the MDMlab server.
type OrbitClient struct {
	*baseClient
	nodeKeyFilePath string
	enrollSecret    string
	hostInfo        mdmlab.OrbitHostInfo
	...
	ConfigReceivers []mdmlab.OrbitConfigReceiver
...
//   - addr is the address of the MDMlab server.
//   - orbitHostInfo is the host system information used for enrolling to MDMlab.
...
func NewOrbitClient(
	...
	enrollSecret string,
	mdmlabClientCert *tls.Certificate,
	orbitHostInfo mdmlab.OrbitHostInfo,
	onGetConfigErrFns *OnGetConfigErrFuncs,
) (*OrbitClient, error) {
	orbitCapabilities := mdmlab.GetOrbitClientCapabilities()
	bc, err := newBaseClient(addr, insecureSkipVerify, rootCA, "", mdmlabClientCert, orbitCapabilities)
	if err != nil {
		return nil, err
	}
...
// connection causing errors in the client (e.g. https://github.com/mdmlabdm/mdmlab/issues/18783).
...
func (oc *OrbitClient) RegisterConfigReceiver(cr mdmlab.OrbitConfigReceiver) {
...
// GetConfig returns the Orbit config fetched from MDMlab server for this instance of OrbitClient.
// Since this method is called in multiple places, we use a cache with configCacheTTL time-to-live
// to reduce traffic to the MDMlab server.
// Upon network errors, this method will retry the get config request (every 30 seconds).
func (oc *OrbitClient) GetConfig() (*mdmlab.OrbitConfig, error) {
	...
	if now.After(oc.configCache.lastUpdated.Add(configCacheTTL)) {
		verb, path := "POST", "/api/mdmlab/orbit/config"
		var (
			resp mdmlab.OrbitConfig
			err  error
		)
...
func (oc *OrbitClient) SetOrUpdateDeviceToken(deviceAuthToken string) error {
	verb, path := "POST", "/api/mdmlab/orbit/device_token"
...
func (oc *OrbitClient) SetOrUpdateDeviceMappingEmail(email string) error {
	verb, path := "PUT", "/api/mdmlab/orbit/device_mapping"
...
// GetHostScript returns the script fetched from MDMlab server to run on this
// host.
func (oc *OrbitClient) GetHostScript(execID string) (*mdmlab.HostScriptResult, error) {
	verb, path := "POST", "/api/mdmlab/orbit/scripts/request"
...
func (oc *OrbitClient) SaveHostScriptResult(result *mdmlab.HostScriptResultPayload) error {
	verb, path := "POST", "/api/mdmlab/orbit/scripts/result"
...
func (oc *OrbitClient) GetInstallerDetails(installId string) (*mdmlab.SoftwareInstallDetails, error) {
	verb, path := "POST", "/api/mdmlab/orbit/software_install/details"
...
func (oc *OrbitClient) SaveInstallerResult(payload *mdmlab.HostSoftwareInstallResultPayload) error {
	verb, path := "POST", "/api/mdmlab/orbit/software_install/result"
...
func (oc *OrbitClient) DownloadSoftwareInstaller(installerID uint, downloadDirectory string) (string, error) {
	verb, path := "POST", "/api/mdmlab/orbit/software_install/package?alt=media"
...
func (oc *OrbitClient) Ping() error {
	verb, path := "HEAD", "/api/mdmlab/orbit/ping"
...
func (oc *OrbitClient) enroll() (string, error) {
	verb, path := "POST", "/api/mdmlab/orbit/enroll"
...
// otherwise it enrolls the host with MDMlab and saves the node key to disk
...
func (oc *OrbitClient) SetOrUpdateDiskEncryptionKey(diskEncryptionStatus mdmlab.OrbitHostDiskEncryptionKeyPayload) error {
	verb, path := "POST", "/api/mdmlab/orbit/disk_encryption_key"
...
func (oc *OrbitClient) GetSetupExperienceStatus() (*mdmlab.SetupExperienceStatusPayload, error) {
	verb, path := "POST", "/api/mdmlab/orbit/setup_experience/status"
...
func (oc *OrbitClient) SendLinuxKeyEscrowResponse(lr luks.LuksResponse) error {
	verb, path := "POST", "/api/mdmlab/orbit/luks_data"
