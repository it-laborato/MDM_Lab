...
func listOptionsFromRequest(r *http.Request) (mdmlab.ListOptions, error) {
	var err error
	...
	if pageString != "" {
		...
		if err != nil {
			return mdmlab.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("non-int page value"))
		}
		if page < 0 {
			return mdmlab.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("negative page value"))
		}
	}
	...
	if perPageString != "" {
		...
		if err != nil {
			return mdmlab.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("non-int per_page value"))
		}
		if perPage <= 0 {
			return mdmlab.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("invalid per_page value"))
		}
	}
	...
	if orderKey == "" && orderDirectionString != "" {
		return mdmlab.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("order_key must be specified with order_direction"))
	}

	if orderKey == "" && afterString != "" {
		return mdmlab.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("order_key must be specified with after"))
	}

	var orderDirection mdmlab.OrderDirection
...
func hostListOptionsFromRequest(r *http.Request) (mdmlab.HostListOptions, error) {
	opt, err := listOptionsFromRequest(r)
	if err != nil {
		return mdmlab.HostListOptions{}, err
	}

	hopt := mdmlab.HostListOptions{ListOptions: opt}

	status := r.URL.Query().Get("status")
	switch mdmlab.HostStatus(status) {
	case mdmlab.StatusNew, mdmlab.StatusOnline, mdmlab.StatusOffline, mdmlab.StatusMIA, mdmlab.StatusMissing:
		hopt.StatusFilter = mdmlab.HostStatus(status)
	case "":
		// No error when unset
	default:
	...
	softwareStatus := mdmlab.SoftwareInstallerStatus(strings.ToLower(r.URL.Query().Get("software_status")))
	if softwareStatus != "" {
	...
	switch mdmlab.MDMEnrollStatus(enrollmentStatus) {
	case mdmlab.MDMEnrollStatusManual, mdmlab.MDMEnrollStatusAutomatic,
		mdmlab.MDMEnrollStatusPending, mdmlab.MDMEnrollStatusUnenrolled, mdmlab.MDMEnrollStatusEnrolled:
		hopt.MDMEnrollmentStatusFilter = mdmlab.MDMEnrollStatus(enrollmentStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	connectedToMDMlab := r.URL.Query().Has("connected_to_mdmlab")
	if connectedToMDMlab {
		hopt.ConnectedToMDMlabFilter = ptr.Bool(true)
	}

	macOSSettingsStatus := r.URL.Query().Get("macos_settings")
	switch mdmlab.OSSettingsStatus(macOSSettingsStatus) {
	case mdmlab.OSSettingsFailed, mdmlab.OSSettingsPending, mdmlab.OSSettingsVerifying, mdmlab.OSSettingsVerified:
		hopt.MacOSSettingsFilter = mdmlab.OSSettingsStatus(macOSSettingsStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mdmlab.DiskEncryptionStatus(macOSSettingsDiskEncryptionStatus) {
	case
		...
		hopt.MacOSSettingsDiskEncryptionFilter = mdmlab.DiskEncryptionStatus(macOSSettingsDiskEncryptionStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mdmlab.OSSettingsStatus(osSettingsStatus) {
	case mdmlab.OSSettingsFailed, mdmlab.OSSettingsPending, mdmlab.OSSettingsVerifying, mdmlab.OSSettingsVerified:
		hopt.OSSettingsFilter = mdmlab.OSSettingsStatus(osSettingsStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mdmlab.DiskEncryptionStatus(osSettingsDiskEncryptionStatus) {
	case
		mdmlab.DiskEncryptionVerifying,
		mdmlab.DiskEncryptionVerified,
		mdmlab.DiskEncryptionActionRequired,
		...
		mdmlab.DiskEncryptionFailed,
		mdmlab.DiskEncryptionRemovingEnforcement:
		hopt.OSSettingsDiskEncryptionFilter = mdmlab.DiskEncryptionStatus(osSettingsDiskEncryptionStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mdmlab.MDMBootstrapPackageStatus(mdmBootstrapPackageStatus) {
	case mdmlab.MDMBootstrapPackageFailed, mdmlab.MDMBootstrapPackagePending, mdmlab.MDMBootstrapPackageInstalled:
		bpf := mdmlab.MDMBootstrapPackageStatus(mdmBootstrapPackageStatus)
...
func carveListOptionsFromRequest(r *http.Request) (mdmlab.CarveListOptions, error) {
	opt, err := listOptionsFromRequest(r)
	if err != nil {
		return mdmlab.CarveListOptions{}, err
	}

	carveOpts := mdmlab.CarveListOptions{ListOptions: opt}
...
func userListOptionsFromRequest(r *http.Request) (mdmlab.UserListOptions, error) {
	opt, err := listOptionsFromRequest(r)
	if err != nil {
		return mdmlab.UserListOptions{}, err
	}

	userOpts := mdmlab.UserListOptions{ListOptions: opt}
	...
	if tid := r.URL.Query().Get("team_id"); tid != "" {
		...
		// GitHub CodeQL flags this as: Incorrect conversion between integer types. Previously waived: https://github.com/mdmlabdm/mdmlab/security/code-scanning/516
