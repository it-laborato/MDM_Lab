...
func getInfoAboutSessionEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetInfoAboutSession(ctx context.Context, id uint) (*mdmlab.Session, error) {
	...
	if err := svc.authz.Authorize(ctx, session, mdmlab.ActionRead); err != nil {
		return nil, err
	}
...
func deleteSessionEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) DeleteSession(ctx context.Context, id uint) error {
	...
	if err := svc.authz.Authorize(ctx, session, mdmlab.ActionWrite); err != nil {
		return err
	}
...
type loginRequest struct {
	...
	// If false/omitted, users that require email verification (MDMlab MFA) to log in will fail to log in, rather than
...
func loginEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) Login(ctx context.Context, email, password string, supportsEmailVerification bool) (*mdmlab.User, *mdmlab.Session, error) {
	...
	defer func(start time.Time) {
		if err != nil && !errors.Is(err, sendingMFAEmail) && !errors.Is(err, mfaNotSupportedForClient) {
			if err := svc.NewActivity(
				ctx, nil, mdmlab.ActivityTypeUserFailedLogin{
					Email:    email,
	...
	if errors.As(err, &nfe) {
		return nil, nil, mdmlab.NewAuthFailedError("user not found")
	}
	if err != nil {
		return nil, nil, mdmlab.NewAuthFailedError(err.Error())
	}

	if err = user.ValidatePassword(password); err != nil {
		return nil, nil, mdmlab.NewAuthFailedError("invalid password")
	}

	if user.SSOEnabled {
		return nil, nil, mdmlab.NewAuthFailedError("password login disabled for sso users")
	...
	if err := svc.NewActivity(
		ctx, user, mdmlab.ActivityTypeUserLoggedIn{
			PublicIP: publicip.FromContext(ctx),
		}); err != nil {
		return nil, nil, err
	}
...
func (svc *Service) makeSession(ctx context.Context, userID uint) (*mdmlab.Session, error) {
...
func sessionCreateEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) CompleteMFA(ctx context.Context, token string) (*mdmlab.Session, *mdmlab.User, error) {
	// skipauth: No user context available yet to authorize against.
	svc.authz.SkipAuthorization(ctx)
...
func logoutEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	err := svc.Logout(ctx)
	if err != nil {
...
func (svc *Service) DestroySession(ctx context.Context) error {
	...
	if !ok {
		return mdmlab.NewAuthRequiredError(mdmlab.ErrNoContext.Error())
	}
...
func initiateSSOEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) InitiateSSO(ctx context.Context, redirectURL string) (string, error) {
	...
	settings := sso.Settings{
		...
		AssertionConsumerServiceURL: serverURL + svc.config.Server.URLPrefix + "/api/v1/mdmlab/sso/callback",
...
func makeCallbackSSOEndpoint(urlPrefix string) handlerFunc {
	return func(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
		authResponse := request.(mdmlab.Auth)
		session, err := getSSOSession(ctx, svc, authResponse)
		var resp callbackSSOResponse
		if err != nil {
			if err := svc.NewActivity(ctx, nil, mdmlab.ActivityTypeUserFailedLogin{
     ...
     <body>
     Redirecting to MDMlab at {{ .RedirectURL }} ...
     </body>
     </html>
    `
...
func getSSOSession(ctx context.Context, svc mdmlab.Service, auth mdmlab.Auth) (*mdmlab.SSOSession, error) {
...
func (svc *Service) InitSSOCallback(ctx context.Context, auth mdmlab.Auth) (string, error) {
	// skipauth: User context does not yet exist. Unauthenticated users may
	...
	err = sso.ValidateAudiences(
		*metadata,
		...
		appConfig.ServerSettings.ServerURL+svc.config.Server.URLPrefix+"/api/v1/mdmlab/sso/callback", // ACS
	)
	if err != nil {
...
func (svc *Service) GetSSOUser(ctx context.Context, auth mdmlab.Auth) (*mdmlab.User, error) {
...
func (svc *Service) LoginSSOUser(ctx context.Context, user *mdmlab.User, redirectURL string) (*mdmlab.SSOSession, error) {
	...
	err = svc.NewActivity(
		ctx,
		user,
		mdmlab.ActivityTypeUserLoggedIn{
			PublicIP: publicip.FromContext(ctx),
		},
	)
	if err != nil {
...
type ssoSettingsResponse struct {
	Settings *mdmlab.SessionSSOSettings `json:"settings,omitempty"`
	Err      error                     `json:"error,omitempty"`
}

func (r ssoSettingsResponse) error() error { return r.Err }

func settingsSSOEndpoint(ctx context.Context, _ interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) SSOSettings(ctx context.Context) (*mdmlab.SessionSSOSettings, error) {
	...
	settings := &mdmlab.SessionSSOSettings{
...
func (svc *Service) makeMFAEmail(ctx context.Context, user mdmlab.User) error {
...
func (svc *Service) GetSessionByKey(ctx context.Context, key string) (*mdmlab.Session, error) {
...
func (svc *Service) validateSession(ctx context.Context, session *mdmlab.Session) error {
	if session == nil {
		return mdmlab.NewAuthRequiredError("active session not present")
	}
	...
	if sessionDuration != 0 && time.Since(session.AccessedAt) >= sessionDuration {
		...
		return mdmlab.NewAuthRequiredError("expired session")
	}
