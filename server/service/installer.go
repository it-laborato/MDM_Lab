...
func (r getInstallerResponse) hijackRender(ctx context.Context, w http.ResponseWriter) {
	...
	w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment;filename="mdmlab-osquery.%s"`, r.fileExt))
...
func getInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(getInstallerRequest)

	fileReader, fileLength, err := svc.GetInstaller(ctx, mdmlab.Installer{
...
func (svc *Service) GetInstaller(ctx context.Context, installer mdmlab.Installer) (io.ReadCloser, int64, error) {
...
func checkInstallerEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	req := request.(*checkInstallerRequest)

	err := svc.CheckInstallerExistence(ctx, mdmlab.Installer{
		EnrollSecret: req.EnrollSecret,
		Kind:         req.Kind,
		Desktop:      req.Desktop,
	})
...
func (svc *Service) CheckInstallerExistence(ctx context.Context, installer mdmlab.Installer) error {
	if err := svc.authz.Authorize(ctx, &mdmlab.EnrollSecret{}, mdmlab.ActionRead); err != nil {
		return err
	}
