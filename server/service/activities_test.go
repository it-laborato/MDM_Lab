...
func TestListActivities(t *testing.T) {
	...
	globalUsers := []*mdmlab.User{test.UserAdmin, test.UserMaintainer, test.UserObserver, test.UserObserverPlus}
	teamUsers := []*mdmlab.User{test.UserTeamAdminTeam1, test.UserTeamMaintainerTeam1, test.UserTeamObserverTeam1}

	ds.ListActivitiesFunc = func(ctx context.Context, opts mdmlab.ListActivitiesOptions) ([]*mdmlab.Activity, *mdmlab.PaginationMetadata, error) {
	...
	for _, u := range globalUsers {
		activities, _, err := svc.ListActivities(test.UserContext(ctx, u), mdmlab.ListActivitiesOptions{})
	...
	for _, u := range teamUsers {
		_, _, err := svc.ListActivities(test.UserContext(ctx, u), mdmlab.ListActivitiesOptions{})
		require.Error(t, err)
		require.Contains(t, err.Error(), authz.ForbiddenErrorMessage)
	}

	// user with no roles cannot read activities
	_, _, err := svc.ListActivities(test.UserContext(ctx, test.UserNoRoles), mdmlab.ListActivitiesOptions{})
	require.Error(t, err)
	require.Contains(t, err.Error(), authz.ForbiddenErrorMessage)

	// no user in context
	_, _, err = svc.ListActivities(ctx, mdmlab.ListActivitiesOptions{})
	require.Error(t, err)
	require.Contains(t, err.Error(), authz.ForbiddenErrorMessage)
}
...
func Test_logRoleChangeActivities(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			oldTeams := make([]mdmlab.UserTeam, 0, len(tt.oldTeamRoles))
			for id, r := range tt.oldTeamRoles {
				oldTeams = append(oldTeams, mdmlab.UserTeam{
					Team: mdmlab.Team{ID: id},
					Role: r,
				})
			}
			newTeams := make([]mdmlab.UserTeam, 0, len(tt.newTeamRoles))
			for id, r := range tt.newTeamRoles {
				newTeams = append(newTeams, mdmlab.UserTeam{
					Team: mdmlab.Team{ID: id},
					Role: r,
				})
			}
			...
			require.NoError(t, mdmlab.LogRoleChangeActivities(ctx, svc, &mdmlab.User{}, tt.oldRole, oldTeams, newUser))
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		return &mdmlab.AppConfig{
			WebhookSettings: mdmlab.WebhookSettings{
				ActivitiesWebhook: mdmlab.ActivitiesWebhookSettings{
					Enable:         true,
