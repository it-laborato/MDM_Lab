...
func (c *Client) GetHosts(query string) ([]mdmlab.HostResponse, error) {
	verb, path := "GET", "/api/latest/mdmlab/hosts"
...
func (c *Client) GetHost(id uint) (*HostDetailResponse, error) {
	verb, path := "GET", fmt.Sprintf("/api/latest/mdmlab/hosts/%d", id)
...
func (c *Client) HostByIdentifier(identifier string) (*HostDetailResponse, error) {
	verb, path := "GET", "/api/latest/mdmlab/hosts/identifier/"+identifier
	var responseBody getHostResponse
	err := c.authenticatedRequest(nil, verb, path, &responseBody)
	return responseBody.Host, err
}
...
func (c *Client) translateTransferHostsToIDs(hosts []string, label string, team string) ([]uint, uint, uint, error) {
	...
	var translatePayloads []mdmlab.TranslatePayload
	for _, host := range hosts {
		translatedPayload, err := encodeTranslatedPayload(mdmlab.TranslatorTypeHost, host)
		if err != nil {
			return nil, 0, 0, err
		}
	...
	if label != "" {
		translatedPayload, err := encodeTranslatedPayload(mdmlab.TranslatorTypeLabel, label)
		if err != nil {
			return nil, 0, 0, err
		}
		translatePayloads = append(translatePayloads, translatedPayload)
	...
	if team != "" {
		translatedPayload, err := encodeTranslatedPayload(mdmlab.TranslatorTypeTeam, team)
		if err != nil {
			return nil, 0, 0, err
		}
		translatePayloads = append(translatePayloads, translatedPayload)
...
func encodeTranslatedPayload(translatorType string, identifier string) (mdmlab.TranslatePayload, error) {
	translatedPayload := mdmlab.TranslatePayload{
		Type:    translatorType,
		Payload: mdmlab.StringIdentifierToIDPayload{Identifier: identifier},
	}
...
func (c *Client) TransferHosts(hosts []string, label string, status, searchQuery string, team string) error {
	...
	if len(hosts) != 0 {
		verb, path := "POST", "/api/latest/mdmlab/hosts/transfer"
	...
	if status != "" {
		filter["status"] = mdmlab.HostStatus(status)
	}
	...
	verb, path := "POST", "/api/latest/mdmlab/hosts/transfer/filter"
...
func (c *Client) GetHostsReport(columns ...string) ([][]string, error) {
	verb, path := "GET", "/api/latest/mdmlab/hosts/report"
