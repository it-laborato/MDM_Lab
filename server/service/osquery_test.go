...
import (
	...
	mdmlabLogging "github.com/it-laborato/MDM_Lab/server/contexts/logging"
	"github.com/it-laborato/MDM_Lab/server/contexts/viewer"
	"github.com/it-laborato/MDM_Lab/server/datastore/mysqlredis"
	...
	mockresult "github.com/it-laborato/MDM_Lab/server/mock/mockresult"
...
func TestGetClientConfig(t *testing.T) {
	...
	ds.ListPacksForHostFunc = func(ctx context.Context, hid uint) ([]*mdmlab.Pack, error) {
		return []*mdmlab.Pack{}, nil
	}
	ds.ListScheduledQueriesInPackFunc = func(ctx context.Context, pid uint) (mdmlab.ScheduledQueryList, error) {
		tru := true
		fals := false
		fortytwo := uint(42)
		switch pid {
	...
	ds.ListScheduledQueriesForAgentsFunc = func(ctx context.Context, teamID *uint, queryReportsDisabled bool) ([]*mdmlab.Query, error) {
		if teamID == nil {
			return nil, nil
		}
		return []*mdmlab.Query{
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		return &mdmlab.AppConfig{AgentOptions: ptr.RawMessage(json.RawMessage(`{"config":{"options":{"baz":"bar"}}}`))}, nil
	}
	...
	ds.HostLiteFunc = func(ctx context.Context, id uint) (*mdmlab.Host, error) {
		...
		return &mdmlab.Host{ID: id}, nil
	}

	svc, ctx := newTestService(t, ds, nil, nil)

	ctx1 := hostctx.NewContext(ctx, &mdmlab.Host{ID: 1})
	ctx2 := hostctx.NewContext(ctx, &mdmlab.Host{ID: 2})
	ctx3 := hostctx.NewContext(ctx, &mdmlab.Host{ID: 1, TeamID: ptr.Uint(1)})
...
var allDetailQueries = osquery_utils.GetDetailQueries(
	context.Background(),
	config.MDMlabConfig{Vulnerabilities: config.VulnerabilitiesConfig{DisableWinOSVulnerabilities: true}},
	nil,
	&mdmlab.Features{
...
func TestEnrollAgent(t *testing.T) {
	ds := new(mock.Store)
	ds.VerifyEnrollSecretFunc = func(ctx context.Context, secret string) (*mdmlab.EnrollSecret, error) {
		switch secret {
		case "valid_secret":
			return &mdmlab.EnrollSecret{Secret: "valid_secret", TeamID: ptr.Uint(3)}, nil
		default:
			return nil, errors.New("not found")
		}
	}
	ds.EnrollHostFunc = func(ctx context.Context, isMDMEnabled bool, osqueryHostId, hUUID, hSerial, nodeKey string, teamID *uint, cooldown time.Duration) (*mdmlab.Host, error) {
...
func TestEnrollAgentEnforceLimit(t *testing.T) {
	runTest := func(t *testing.T, pool mdmlab.RedisPool) {
		const maxHosts = 2
		...
		ds.VerifyEnrollSecretFunc = func(ctx context.Context, secret string) (*mdmlab.EnrollSecret, error) {
			switch secret {
			case "valid_secret":
				return &mdmlab.EnrollSecret{Secret: "valid_secret"}, nil
			default:
				return nil, errors.New("not found")
			}
		}
		...
		svc, ctx := newTestService(t, redisWrapDS, nil, nil, &TestServerOpts{
			...
			License:           &mdmlab.LicenseInfo{DeviceCount: maxHosts},
		})
...
func TestAuthenticateHost(t *testing.T) {
	...
	var gotKey string
	host := mdmlab.Host{ID: 1, Hostname: "foobar"}
	ds.LoadHostByNodeKeyFunc = func(ctx context.Context, nodeKey string) (*mdmlab.Host, error) {
		gotKey = nodeKey
		return &host, nil
	}
	var gotHostIDs []uint
	...
	host = mdmlab.Host{ID: 7, Hostname: "foobar"}
...
func TestSubmitResultLogsToLogDestination(t *testing.T) {
	...
	ds.OverwriteQueryResultRowsFunc = func(ctx context.Context, rows []*mdmlab.ScheduledQueryResultRow, maxQueryReportRows int) error {
		if len(rows) == 0 {
			return nil
		}
		switch {
	...
	validLogResults := []string{
		...
		// MDMlab doesn't know of this query, so this result should be streamed as is (This is to support streaming results for osquery nodes that are configured outside of MDMlab, e.g. `--config_plugin=filesystem`).
		...
		// The "name" field has invalid format, so this result will be streamed as is (This is to support streaming results for osquery nodes that are configured outside of MDMlab, e.g. `--config_plugin=filesystem`).
...
func TestSaveResultLogsToQueryReports(t *testing.T) {
	...
	results := []*mdmlab.ScheduledQueryResult{
		{
	...
	discardDataFalse := map[string]*mdmlab.Query{
		"pack/Global/Uptime": {
			ID:          1,
			DiscardData: true,
			Logging:     mdmlab.LoggingSnapshot,
	...
	serv.saveResultLogsToQueryReports(ctx, results, discardDataFalse, mdmlab.DefaultMaxQueryReportRows)
	assert.False(t, ds.OverwriteQueryResultRowsFuncInvoked)

	// Happy Path: Results saved
	discardDataTrue := map[string]*mdmlab.Query{
		"pack/Global/Uptime": {
			ID:          1,
			DiscardData: false,
			Logging:     mdmlab.LoggingSnapshot,
	...
	serv.saveResultLogsToQueryReports(ctx, results, discardDataTrue, mdmlab.DefaultMaxQueryReportRows)
...
func TestGetMostRecentResults(t *testing.T) {
	tests := []struct {
		name     string
		input    []*mdmlab.ScheduledQueryResult
		expected []*mdmlab.ScheduledQueryResult
	}{
		{
			name: "basic test",
			input: []*mdmlab.ScheduledQueryResult{
...
func TestHostDetailQueries(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		return &mdmlab.AppConfig{Features: mdmlab.Features{
	...
	host := mdmlab.Host{
		...
		UpdateCreateTimestamps: mdmlab.UpdateCreateTimestamps{
			UpdateTimestamp: mdmlab.UpdateTimestamp{
...
func TestQueriesAndHostFeatures(t *testing.T) {
	...
	ds.TeamFeaturesFunc = func(ctx context.Context, id uint) (*mdmlab.Features, error) {
		switch id {
		case uint(1):
	...
	ds.LabelQueriesForHostFunc = func(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
		return map[string]string{}, nil
	}

	ds.PolicyQueriesForHostFunc = func(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
		return map[string]string{}, nil
	}
	...
	t.Run("free license", func(t *testing.T) {
		license := &mdmlab.LicenseInfo{Tier: mdmlab.TierFree}
		svc, ctx := newTestService(t, ds, nil, lq, &TestServerOpts{License: license})

		ctx = hostctx.NewContext(ctx, &host)
		queries, _, _, err := svc.GetDistributedQueries(ctx)
		require.NoError(t, err)
		require.NotContains(t, queries, "mdmlab_detail_query_users")
		require.NotContains(t, queries, "mdmlab_detail_query_software_macos")
		require.NotContains(t, queries, "mdmlab_detail_query_software_linux")
		require.NotContains(t, queries, "mdmlab_detail_query_software_windows")
	...
	t.Run("premium license", func(t *testing.T) {
		...
		queries, _, _, err = svc.GetDistributedQueries(ctx)
		require.NoError(t, err)
		require.Contains(t, queries, "mdmlab_detail_query_users")
		require.NotContains(t, queries, "mdmlab_detail_query_software_macos")
		require.NotContains(t, queries, "mdmlab_detail_query_software_linux")
		require.NotContains(t, queries, "mdmlab_detail_query_software_windows")
		...
		require.NoError(t, err)
		require.NotContains(t, queries, "mdmlab_detail_query_users")
		require.Contains(t, queries, "mdmlab_detail_query_software_macos")
	})
}
...
func TestGetDistributedQueriesEmptyQuery(t *testing.T) {
	...
	ds.UpdateHostFunc = func(ctx context.Context, gotHost *mdmlab.Host) error {
		return nil
	}
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		return &mdmlab.AppConfig{Features: mdmlab.Features{EnableHostUsers: true}}, nil
	}
	ds.PolicyQueriesForHostFunc = func(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
...
func TestLabelQueries(t *testing.T) {
	...
	require.NoError(t, err)
	// +1 for the mdmlab_no_policies_wildcard query.
	...
	queries, discovery, acc, err = svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	require.Len(t, queries, 1) // mdmlab_no_policies_wildcard query
	verifyDiscovery(t, queries, discovery)
	assert.Zero(t, acc)
	...
	ds.RecordLabelQueryExecutionsFunc = func(ctx context.Context, host *mdmlab.Host, results map[uint]*bool, t time.Time, deferred bool) error {
		gotHost = host
		gotResults = results
		gotTime = t
		return nil
	...
	err = svc.SubmitDistributedQueryResults(
		...
		map[string]mdmlab.OsqueryStatus{},
		map[string]string{},
		map[string]*mdmlab.Stats{},
	)
	require.NoError(t, err)
	...
	queries, discovery, acc, err = svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +3 for label queries, +1 for the mdmlab_no_policies_wildcard query.
...
func TestDetailQueriesWithEmptyStrings(t *testing.T) {
	...
	ds.LabelQueriesForHostFunc = func(context.Context, *mdmlab.Host) (map[string]string, error) {
		return map[string]string{}, nil
	}
	ds.PolicyQueriesForHostFunc = func(ctx context.Context, host *mdmlab.Host) (map[string]string, error) {
		return map[string]string{}, nil
	...
	queries, discovery, acc, err := svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 due to 'windows_update_history', +1 due to mdmlab_no_policies_wildcard query.
	...
	resultJSON := `
{
  "mdmlab_detail_query_network_interface_windows": [
    {
      "address": "192.168.0.1",
      "mac": "5f:3d:4b:10:25:82"
    }
  ],
  "mdmlab_detail_query_os_version": [
    {
      "platform": "darwin",
      "build": "15G1004",
      "major": "10",
  ...
  "mdmlab_detail_query_osquery_info": [
    {
      "build_distro": "10.10",
  ...
  "mdmlab_detail_query_system_info": [
    {
  ...
  "mdmlab_detail_query_osquery_flags": [
    {
  ...
  "mdmlab_detail_query_orbit_info": [
    {
      "name": "version",
      "value": "42"
    },
...
`
	...
	var results mdmlab.OsqueryDistributedQueryResults
	...
	require.NoError(
		t, svc.SubmitDistributedQueryResults(ctx, results, map[string]mdmlab.OsqueryStatus{}, map[string]string{}, map[string]*mdmlab.Stats{}),
	)
	...
	// +1 due to mdmlab_no_policies_wildcard query.
...
func TestDetailQueries(t *testing.T) {
	...
	ds.SetOrUpdateMDMDataFunc = func(ctx context.Context, hostID uint, isServer, enrolled bool, serverURL string, installedFromDep bool, name string, mdmlabEnrollmentRef string) error {
		require.True(t, enrolled)
		require.False(t, installedFromDep)
		require.Equal(t, "hi.com", serverURL)
		require.Empty(t, mdmlabEnrollmentRef)
		return nil
	}
	...
	queries, discovery, acc, err := svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 for mdmlab_no_policies_wildcard
	...
	resultJSON := `
{
"mdmlab_detail_query_network_interface": [
    {
        "address": "192.168.0.1",
...
"mdmlab_detail_query_software_macos": [
    {
      "name": "app1",
      "version": "1.0.0",
      "source": "source1"
...
"mdmlab_detail_query_disk_space_unix": [
	{
...
"mdmlab_detail_query_munki_info": [
	{
		"version": "3.4.5"
	}
],
...
`
	...
	var gotUsers []mdmlab.HostUser
	ds.SaveHostUsersFunc = func(ctx context.Context, hostID uint, users []mdmlab.HostUser) error {
		if hostID != 1 {
	...
	var gotSoftware []mdmlab.Software
	ds.UpdateHostSoftwareFunc = func(ctx context.Context, hostID uint, software []mdmlab.Software) (*mdmlab.UpdateHostSoftwareDBResult, error) {
		if hostID != 1 {
			return nil, errors.New("not found")
		}
	...
	ds.UpdateHostSoftwareInstalledPathsFunc = func(ctx context.Context, hostID uint, paths map[string]struct{}, result *mdmlab.UpdateHostSoftwareDBResult) error {
		return nil
	}
	...
	assert.Equal(t, "computer.local", gotHost.Hostname)
	assert.Equal(t, "uuid", gotHost.UUID)
	// The hardware serial should not have updated because return value was -1. See: https://github.com/mdmlabdm/mdmlab/issues/19789
	...
	assert.Equal(t, mdmlab.HostUser{
		Uid:       1234,
		Username:  "user1",
		Type:      "sometype",
		GroupName: "somegroup",
	...
	assert.Equal(t, []mdmlab.Software{
		{
			Name:    "app1",
			Version: "1.0.0",
			Source:  "source1",
	...
	queries, discovery, acc, err = svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 mdmlab_no_policies_wildcard query
	require.Equal(t, len(expectedDetailQueriesForPlatform(host.Platform))+1, len(queries), distQueriesMapKeys(queries))
	verifyDiscovery(t, queries, discovery)
	assert.Zero(t, acc)
}
...
func TestMDMQueries(t *testing.T) {
	...
	expectedMDMQueries := []struct {
		...
		{"mdmlab_detail_query_mdm_config_profiles_darwin", "macos_profiles"},
		{"mdmlab_detail_query_mdm_disk_encryption_key_file_darwin", "filevault_prk"},
		{"mdmlab_detail_query_mdm_disk_encryption_key_file_lines_darwin", "file_lines"},
	}

	mdmEnabled := true
	ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
		return &mdmlab.AppConfig{MDM: mdmlab.MDM{EnabledAndConfigured: mdmEnabled}}, nil
	}
...
func TestNewDistributedQueryCampaign(t *testing.T) {
	...
	var gotTargets []*mdmlab.DistributedQueryCampaignTarget
	...
	campaign, err := svc.NewDistributedQueryCampaign(viewerCtx, q, nil, mdmlab.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}})
	require.NoError(t, err)
	assert.Equal(t, gotQuery.ID, gotCampaign.QueryID)
	assert.Equal(t, []*mdmlab.DistributedQueryCampaignTarget{
		{
...
func TestDistributedQueryResults(t *testing.T) {
	...
	campaign := &mdmlab.DistributedQueryCampaign{ID: 42}
	...
	require.NoError(t, err)
	// +1 for the distributed query for campaign ID 42, +1 for windows update history, +1 for the mdmlab_no_policies_wildcard query.
	...
	go func() {
		...
		select {
		case val := <-readChan:
			if res, ok := val.(mdmlab.DistributedQueryResult); ok {
	...
	err = svc.SubmitDistributedQueryResults(
		hostCtx, results, map[string]mdmlab.OsqueryStatus{}, map[string]string{}, stats,
	)
	require.NoError(t, err)
...
func TestIngestDistributedQueryOrphanedCampaignLoadError(t *testing.T) {
	...
	err := svc.ingestDistributedQuery(context.Background(), host, "mdmlab_distributed_query_42", []map[string]string{}, "", nil)
	require.Error(t, err)
...
func TestIngestDistributedQueryOrphanedCloseError(t *testing.T) {
	...
	ds.SaveDistributedQueryCampaignFunc = func(ctx context.Context, campaign *mdmlab.DistributedQueryCampaign) error {
...
func TestUpdateHostIntervals(t *testing.T) {
	...
	testCases := []struct {
		name                  string
		initIntervals         mdmlab.HostOsqueryIntervals
		finalIntervals        mdmlab.HostOsqueryIntervals
		configOptions         json.RawMessage
		updateIntervalsCalled bool
	}{
		{
			"Both updated",
			mdmlab.HostOsqueryIntervals{
				ConfigTLSRefresh: 60,
			},
			mdmlab.HostOsqueryIntervals{
		...
		{
			"MDMlab not managing distributed_interval",
			mdmlab.HostOsqueryIntervals{
				ConfigTLSRefresh:    60,
				DistributedInterval: 11,
			},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			ctx := hostctx.NewContext(ctx, &mdmlab.Host{
				ID:                  1,
			...
			ds.AppConfigFunc = func(ctx context.Context) (*mdmlab.AppConfig, error) {
				return &mdmlab.AppConfig{AgentOptions: ptr.RawMessage(json.RawMessage(`{"config":` + string(tt.configOptions) + `}`))}, nil
			}

			updateIntervalsCalled := false
			ds.UpdateHostOsqueryIntervalsFunc = func(ctx context.Context, hostID uint, intervals mdmlab.HostOsqueryIntervals) error {
				if hostID != 1 {
					return errors.New("not found")
				}
...
func TestAuthenticationErrors(t *testing.T) {
	ms := new(mock.Store)
	ms.LoadHostByNodeKeyFunc = func(ctx context.Context, nodeKey string) (*mdmlab.Host, error) {
		return nil, nil
	}
...
func TestDistributedQueriesLogsManyErrors(t *testing.T) {
	...
	lCtx := &mdmlabLogging.LoggingContext{}
	ctx = mdmlabLogging.NewContext(ctx, lCtx)
	ctx = hostctx.NewContext(ctx, host)
...
func TestObserversCanOnlyRunDistributedCampaigns(t *testing.T) {
	...
	viewerCtx := viewer.NewContext(ctx, viewer.Viewer{
		User: &mdmlab.User{ID: 0, GlobalRole: ptr.String(mdmlab.RoleObserver)},
	})
	...
	_, err := svc.NewDistributedQueryCampaign(viewerCtx, q, nil, mdmlab.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}})
	require.Error(t, err)

	_, err = svc.NewDistributedQueryCampaign(viewerCtx, "", ptr.Uint(42), mdmlab.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}})
	require.Error(t, err)
...
func TestTeamMaintainerCanRunNewDistributedCampaigns(t *testing.T) {
	...
	viewerCtx := viewer.NewContext(ctx, viewer.Viewer{
		User: &mdmlab.User{ID: 99, Teams: []mdmlab.UserTeam{{Team: mdmlab.Team{ID: 123}, Role: mdmlab.RoleMaintainer}}},
	})
	...
	_, err := svc.NewDistributedQueryCampaign(viewerCtx, q, nil, mdmlab.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}, TeamIDs: []uint{123}})
	require.NoError(t, err)
}
...
func TestPolicyQueries(t *testing.T) {
	...
	ds.RecordPolicyQueryExecutionsFunc = func(ctx context.Context, gotHost *mdmlab.Host, results map[uint]*bool, updated time.Time, deferred bool) error {
...
func TestPolicyWebhooks(t *testing.T) {
	...
	cmpSets := func(expSets map[uint][]mdmlab.PolicySetHost) error {
	...
	assert.Eventually(t, func() bool {
		err = cmpSets(map[uint][]mdmlab.PolicySetHost{
			3: {{
				ID:       host.ID,
	...
	err = failingPolicySet.RemoveHosts(1, []mdmlab.PolicySetHost{{
		ID:       host.ID,
		Hostname: host.Hostname,
	}})
	require.NoError(t, err)
...
func TestLiveQueriesFailing(t *testing.T) {
	...
	queries, discovery, _, err := svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 to account for the mdmlab_no_policies_wildcard query.
	require.Equal(t, len(expectedDetailQueriesForPlatform(host.Platform))+1, len(queries), distQueriesMapKeys(queries))
	verifyDiscovery(t, queries, discovery)
...
func distQueriesMapKeys(m map[string]string) []string {
	...
	for k := range m {
		keys = append(keys, strings.TrimPrefix(k, "mdmlab_detail_query_"))
	}
	sort.Strings(keys)
	return keys
}
...
func TestPreProcessSoftwareResults(t *testing.T) {
	...
	for _, tc := range []struct {
		name       string
		host       *mdmlab.Host
		resultsIn  mdmlab.OsqueryDistributedQueryResults
		statusesIn map[string]mdmlab.OsqueryStatus
		messagesIn map[string]string
		overrides  map[string]osquery_utils.DetailQuery

		resultsOut mdmlab.OsqueryDistributedQueryResults
	}{
		{
			name: "software query works and there are vs code extensions in extra",

			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "other_detail_query":         mdmlab.StatusOK,
				hostDetailQueryPrefix + "software_macos":             mdmlab.StatusOK,
				hostDetailQueryPrefix + "software_vscode_extensions": mdmlab.StatusOK,
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":             mdmlab.StatusOK,
				hostDetailQueryPrefix + "software_vscode_extensions": mdmlab.OsqueryStatus(1),
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{
					foobarApp,
		...
		},
		{
			...
			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":             mdmlab.OsqueryStatus(1),
				hostDetailQueryPrefix + "software_vscode_extensions": mdmlab.StatusOK,
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{},
		...
		},
		{
			...
			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":      mdmlab.StatusOK,
				hostDetailQueryPrefix + "software_overrideMe": mdmlab.StatusOK,
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{
					appToOverride,
		...
		},
		{
			...
			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "software_linux": mdmlab.StatusOK,
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			host: &mdmlab.Host{ID: 1, Platform: "debian"},
			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "software_linux": mdmlab.StatusOK,
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			host: &mdmlab.Host{ID: 1, Platform: "rhel"},
			statusesIn: map[string]mdmlab.OsqueryStatus{
				hostDetailQueryPrefix + "software_linux": mdmlab.StatusOK,
			},
			resultsIn: mdmlab.OsqueryDistributedQueryResults{
...
func TestDetailQueriesLinuxDistros(t *testing.T) {
	for _, linuxPlatform := range mdmlab.HostLinuxOSs {
...
func mockUbuntuResults() *mdmlab.OsqueryDistributedQueryResults {
	results := &mdmlab.OsqueryDistributedQueryResults{
...
func BenchmarkPreprocessUbuntuPythonPackageFilter(b *testing.B) {
	...
	statuses := &map[string]mdmlab.OsqueryStatus{
		hostDetailQueryPrefix + "software_linux": mdmlab.StatusOK,
	}

	for i := 0; i < b.N; i++ {
		preProcessSoftwareResults(&mdmlab.Host{ID: 1, Platform: platform}, results, statuses, nil, nil, log.NewNopLogger())
	}
}
