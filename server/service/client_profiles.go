...
func (c *Client) DeleteProfile(profileID uint) error {
	verb, path := "DELETE", "/api/latest/mdmlab/mdm/apple/profiles/"+strconv.FormatUint(uint64(profileID), 10)
	var responseBody deleteMDMAppleConfigProfileResponse
	return c.authenticatedRequest(nil, verb, path, &responseBody)
}

func (c *Client) ListProfiles(teamID *uint) ([]*mdmlab.MDMAppleConfigProfile, error) {
	verb, path := "GET", "/api/latest/mdmlab/mdm/apple/profiles"
...
func (c *Client) AddProfile(teamID uint, configurationProfile []byte) (uint, error) {
	...
	request, err := http.NewRequest(
		"POST",
		c.baseURL.String()+"/api/latest/mdmlab/mdm/apple/profiles",
		body,
	)
	if err != nil {
		return 0, err
	...
	if response.Header.Get(mdmlab.HeaderLicenseKey) == mdmlab.HeaderLicenseValueExpired {
...
func (c *Client) GetConfigProfilesSummary(teamID *uint) (*mdmlab.MDMProfilesSummary, error) {
	verb, path := "GET", "/api/latest/mdmlab/mdm/profiles/summary"
	query := make(url.Values)
	if teamID != nil {
		query.Add("team_id", strconv.FormatUint(uint64(*teamID), 10))
	}
