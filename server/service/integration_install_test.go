...
import (
	...
	software_mock "github.com/it-laborato/MDM_Lab/server/mock/software"
...
func (s *integrationInstallTestSuite) SetupSuite() {
	...
	mdmlabConfig.S3.SoftwareInstallersCloudFrontSigner = signer
...
// We test using both mock and real mdmlab.SoftwareInstallerStore.Sign functions.
func (s *integrationInstallTestSuite) TestSoftwareInstallerSignedURL() {
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mdmlab/hosts/%d/software/%d/install", hostInTeam.ID, titleID), installSoftwareRequest{},
		http.StatusAccepted)
	...
	s.DoJSON("POST", "/api/mdmlab/orbit/software_install/details", orbitGetSoftwareInstallRequest{
