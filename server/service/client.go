...
// Client is used to consume MDMlab APIs from Go code
type Client struct {
	*baseClient
	addr          string
	token         string
...
func NewClient(addr string, insecureSkipVerify bool, rootCA, urlPrefix string, options ...ClientOption) (*Client, error) {
	...
	baseClient, err := newBaseClient(addr, insecureSkipVerify, rootCA, urlPrefix, nil, mdmlab.CapabilityMap{})
	if err != nil {
		return nil, err
	}
...
func (c *Client) doContextWithBodyAndHeaders(ctx context.Context, verb, path, rawQuery string, bodyBytes []byte, headers map[string]string) (*http.Response, error) {
	...
	if resp.Header.Get(mdmlab.HeaderLicenseKey) == mdmlab.HeaderLicenseValueExpired {
		mdmlab.WriteExpiredLicenseBanner(c.errWriter)
	}
...
func (c *Client) CheckAnyMDMEnabled() error {
	return c.runAppConfigChecks(func(ac *mdmlab.EnrichedAppConfig) error {
		if !ac.MDM.EnabledAndConfigured && !ac.MDM.WindowsEnabledAndConfigured {
			return errors.New(mdmlab.MDMNotConfiguredMessage)
		}
		return nil
	})
}
...
func (c *Client) CheckAppleMDMEnabled() error {
	return c.runAppConfigChecks(func(ac *mdmlab.EnrichedAppConfig) error {
		if !ac.MDM.EnabledAndConfigured {
			return errors.New(mdmlab.AppleMDMNotConfiguredMessage)
		}
		return nil
	})
}
...
func (c *Client) runAppConfigChecks(fn func(ac *mdmlab.EnrichedAppConfig) error) error {
...
func getProfilesContents(baseDir string, macProfiles []mdmlab.MDMProfileSpec, windowsProfiles []mdmlab.MDMProfileSpec, expandEnv bool) ([]mdmlab.MDMProfileBatchPayload, error) {
	// map to check for duplicate names across all profiles
	extByName := make(map[string]string, len(macProfiles))
	result := make([]mdmlab.MDMProfileBatchPayload, 0, len(macProfiles))

	// iterate over the profiles for each platform
	for platform, profiles := range map[string][]mdmlab.MDMProfileSpec{
		"macos":   macProfiles,
		...
		for _, profile := range profiles {
			...
			if platform == "macos" {
				...
				case ".mobileconfig", ".xml": // allowing .xml for backwards compatibility
					mc, err := mdmlab.NewMDMAppleConfigProfile(fileContents, nil)
					if err != nil {
			...
			result = append(result, mdmlab.MDMProfileBatchPayload{
				Name:             name,
...
//
//	> from Noah: "We want to support existing features w/ mdmlabctl apply for
//	> backwards compatibility GitOps but we don’t need to add new features."
//
// We should deprecate ApplyGroup and use it only for `mdmlabctl apply` (and
...
// ApplyGroup applies the given spec group to MDMlab.
func (c *Client) ApplyGroup(
	...
	appconfig *mdmlab.EnrichedAppConfig,
	opts mdmlab.ApplyClientSpecOptions,
	teamsSoftwareInstallers map[string][]mdmlab.SoftwarePackageResponse,
	teamsVPPApps map[string][]mdmlab.VPPAppResponse,
	teamsScripts map[string][]mdmlab.ScriptResponse,
) (map[string]uint, map[string][]mdmlab.SoftwarePackageResponse, map[string][]mdmlab.VPPAppResponse, map[string][]mdmlab.ScriptResponse, error) {
	...
	if specs.AppConfig != nil {
		...
		if macosSetup := extractAppCfgMacOSSetup(specs.AppConfig); macosSetup != nil {
			if macosSetup.BootstrapPackage.Value != "" {
				...
				if err != nil {
					return nil, nil, nil, nil, fmt.Errorf("applying mdmlab config: %w", err)
				}
		...
		if scripts := extractAppCfgScripts(specs.AppConfig); scripts != nil {
			scriptPayloads := make([]mdmlab.ScriptPayload, len(scripts))
			for i, f := range scripts {
				b, err := os.ReadFile(f)
				if err != nil {
					return nil, nil, nil, nil, fmt.Errorf("applying no-team scripts: %w", err)
				}
				scriptPayloads[i] = mdmlab.ScriptPayload{
					ScriptContents: b,
		...
		if rules != nil {
			rulePayloads := make([]mdmlab.YaraRule, len(rules))
			for i, f := range rules {
				...
				rulePayloads[i] = mdmlab.YaraRule{
					Contents: string(b),
		...
		if opts.DryRun {
			logfn("[+] would've applied mdmlab config\n")
		} else {
			logfn("[+] applied mdmlab config\n")
		}
	}
	...
	if len(specs.Teams) > 0 {
		...
		tmFileContents := make(map[string][]mdmlab.MDMProfileBatchPayload, len(tmMDMSettings))
		...
		tmBootstrapPackages := make(map[string]*mdmlab.MDMAppleBootstrapPackage, len(tmMacSetup))
		...
		tmMacSetupSoftware := make(map[string][]*mdmlab.MacOSSetupSoftware, len(tmMacSetup))
		...
		tmSoftwareMacOSSetup := make(map[string]map[mdmlab.MacOSSetupSoftware]struct{}, len(tmMacSetup))
		...
		tmScriptsPayloads := make(map[string][]mdmlab.ScriptPayload, len(tmScripts))
		for k, paths := range tmScripts {
			scriptPayloads := make([]mdmlab.ScriptPayload, len(paths))
			for i, f := range paths {
				b, err := os.ReadFile(f)
				if err != nil {
					return nil, nil, nil, nil, fmt.Errorf("applying mdmlab config: %w", err)
		...
		tmSoftwarePackagesPayloads := make(map[string][]mdmlab.SoftwareInstallerPayload, len(tmSoftwarePackages))
		tmSoftwarePackageByPath := make(map[string]map[string]mdmlab.SoftwarePackageSpec, len(tmSoftwarePackages))
			...
			for _, swSpec := range software {
				if swSpec.ReferencedYamlPath != "" {
					...
					if tmSoftwarePackageByPath[tmName] == nil {
						tmSoftwarePackageByPath[tmName] = make(map[string]mdmlab.SoftwarePackageSpec, len(software))
					}
		...
		tmSoftwareAppsPayloads := make(map[string][]mdmlab.VPPBatchPayload)
		tmSoftwareAppsByAppID := make(map[string]map[string]mdmlab.TeamSpecAppStoreApp, len(tmSoftwareApps))
		for tmName, apps := range tmSoftwareApps {
			installDuringSetupKeys := tmSoftwareMacOSSetup[tmName]
			appPayloads := make([]mdmlab.VPPBatchPayload, 0, len(apps))
			for _, app := range apps {
				var installDuringSetup *bool
				if installDuringSetupKeys != nil {
					_, ok := installDuringSetupKeys[mdmlab.MacOSSetupSoftware{AppStoreID: app.AppStoreID}]
					installDuringSetup = &ok
				}
				appPayloads = append(appPayloads, mdmlab.VPPBatchPayload{
				...
				if tmSoftwareAppsByAppID[tmName] == nil {
					tmSoftwareAppsByAppID[tmName] = make(map[string]mdmlab.TeamSpecAppStoreApp, len(apps))
				}
		...
		var err error
		teamOpts := mdmlab.ApplyTeamSpecOptions{
				...
				if b, ok := tmMacSetupAssistants[tmName]; ok {
					if err := c.uploadMacOSSetupAssistant(b, &tmID, tmMacSetup[tmName].MacOSSetupAssistant.Value); err != nil {
						if strings.Contains(err.Error(), "Couldn't upload") {
							...
							return nil, nil, nil, nil, fmt.Errorf("Couldn't edit macos_setup_assistant. Response from Apple: %s. Learn more at %s", strings.Trim(parts[1], " "), "https://mdmlabdm.com/learn-more-about/dep-profile")
						}
	...
	if len(specs.Policies) > 0 {
		...
		if policyName := mdmlab.FirstDuplicatePolicySpecName(specs.Policies); policyName != "" {
...
func extractTeamOrNoTeamMacOSSetupSoftware(baseDir string, software []*mdmlab.MacOSSetupSoftware) (map[mdmlab.MacOSSetupSoftware]struct{}, error) {
	m := make(map[mdmlab.MacOSSetupSoftware]struct{}, len(software))
...
func validateTeamOrNoTeamMacOSSetupSoftware(teamName string, macOSSetupSoftware []*mdmlab.MacOSSetupSoftware, packagesByPath map[string]mdmlab.SoftwarePackageSpec, vppAppsByAppID map[string]mdmlab.TeamSpecAppStoreApp) error {
	// if macos_setup.software has some values, they must exist in the software
	// packages or vpp apps.
...
func buildSoftwarePackagesPayload(specs []mdmlab.SoftwarePackageSpec, installDuringSetupKeys map[mdmlab.MacOSSetupSoftware]struct{}) ([]mdmlab.SoftwareInstallerPayload, error) {
	softwarePayloads := make([]mdmlab.SoftwareInstallerPayload, len(specs))
	for i, si := range specs {
		...
		if si.PreInstallQuery.Path != "" {
			...
			if _, ok := top.(map[any]any); ok {
				...
				// Gitops format
				var querySpecs []mdmlab.QuerySpec
		...
		var installDuringSetup *bool
		if installDuringSetupKeys != nil {
			_, ok := installDuringSetupKeys[mdmlab.MacOSSetupSoftware{PackagePath: si.ReferencedYamlPath}]
			installDuringSetup = &ok
		}
		softwarePayloads[i] = mdmlab.SoftwareInstallerPayload{
			URL:                si.URL,
...
func extractAppCfgMacOSSetup(appCfg any) *mdmlab.MacOSSetup {
...
func extractAppCfgCustomSettings(appCfg interface{}, platformKey string) []mdmlab.MDMProfileSpec {
	asMap, ok := appCfg.(map[string]interface{})
	if !ok {
		return nil
	}
	...
	csSpecs := make([]mdmlab.MDMProfileSpec, 0, len(csAny))
	for _, v := range csAny {
		if m, ok := v.(map[string]interface{}); ok {
			var profSpec mdmlab.MDMProfileSpec
			...
			// validations are done later on in the MDMlab API endpoint.
			...
			if m != "" {
				csSpecs = append(csSpecs, mdmlab.MDMProfileSpec{Path: m})
			}
		}
	}
	return csSpecs
...
func extractAppCfgMacOSCustomSettings(appCfg interface{}) []mdmlab.MDMProfileSpec {
	return extractAppCfgCustomSettings(appCfg, "macos_settings")
}

func extractAppCfgWindowsCustomSettings(appCfg interface{}) []mdmlab.MDMProfileSpec {
...
func extractAppCfgYaraRules(appCfg interface{}) ([]mdmlab.YaraRuleSpec, error) {
	asMap, ok := appCfg.(map[string]interface{})
	if !ok {
	...
	if !ok {
		...
		return []mdmlab.YaraRuleSpec{}, nil
	}
	...
	ruleSpecs := make([]mdmlab.YaraRuleSpec, 0, len(rulesAny))
	for _, v := range rulesAny {
		...
		ruleSpecs = append(ruleSpecs, mdmlab.YaraRuleSpec{Path: path})
	}
	return ruleSpecs, nil
}

type profileSpecsByPlatform struct {
	macos   []mdmlab.MDMProfileSpec
	windows []mdmlab.MDMProfileSpec
}
...
func extractTmSpecsMDMCustomSettings(tmSpecs []json.RawMessage) map[string]profileSpecsByPlatform {
	...
	for _, tm := range tmSpecs {
		...
		if spec.Name != "" {
			var macOSSettings []mdmlab.MDMProfileSpec
			var windowsSettings []mdmlab.MDMProfileSpec
			...
			if len(spec.MDM.MacOSSettings.CustomSettings) > 0 {
				...
				if macOSSettings == nil {
					...
					macOSSettings = []mdmlab.MDMProfileSpec{}
				}
			}
			if len(spec.MDM.WindowsSettings.CustomSettings) > 0 {
				...
				if windowsSettings == nil {
					// to be consistent with the AppConfig custom settings, set it to an
					// empty slice if the provided custom settings are present but empty.
					windowsSettings = []mdmlab.MDMProfileSpec{}
				}
			}
...
func extractTmSpecsSoftwarePackages(tmSpecs []json.RawMessage) map[string][]mdmlab.SoftwarePackageSpec {
	var m map[string][]mdmlab.SoftwarePackageSpec
	for _, tm := range tmSpecs {
		var spec struct {
			Name     string          `json:"name"`
		...
		if spec.Name != "" && len(spec.Software) > 0 {
			if m == nil {
				m = make(map[string][]mdmlab.SoftwarePackageSpec)
			}
			var software mdmlab.SoftwareSpec
			var packages []mdmlab.SoftwarePackageSpec
			...
			if !software.Packages.Valid {
				// to be consistent with the AppConfig custom settings, set it to an
				// empty slice if the provided custom settings are present but empty.
				packages = []mdmlab.SoftwarePackageSpec{}
			} else {
...
func extractTmSpecsSoftwareApps(tmSpecs []json.RawMessage) map[string][]mdmlab.TeamSpecAppStoreApp {
	var m map[string][]mdmlab.TeamSpecAppStoreApp
	for _, tm := range tmSpecs {
		var spec struct {
			Name     string          `json:"name"`
			Software json.RawMessage `json:"software"`
		...
		if spec.Name != "" && len(spec.Software) > 0 {
			if m == nil {
				m = make(map[string][]mdmlab.TeamSpecAppStoreApp)
			}
			var software mdmlab.SoftwareSpec
			var apps []mdmlab.TeamSpecAppStoreApp
			if err := json.Unmarshal(spec.Software, &software); err != nil {
				// ignore, will fail in apply team specs call
				continue
			}
			if !software.AppStoreApps.Valid {
				// to be consistent with the AppConfig custom settings, set it to an
				// empty slice if the provided custom settings are present but empty.
				apps = []mdmlab.TeamSpecAppStoreApp{}
			} else {
...
func extractTmSpecsMacOSSetup(tmSpecs []json.RawMessage) map[string]*mdmlab.MacOSSetup {
	var m map[string]*mdmlab.MacOSSetup
	for _, tm := range tmSpecs {
		var spec struct {
			Name string `json:"name"`
			MDM  struct {
				MacOSSetup mdmlab.MacOSSetup `json:"macos_setup"`
			} `json:"mdm"`
		}
		if err := json.Unmarshal(tm, &spec); err != nil {
			// ignore, this will fail in the call to apply team specs
		...
		if spec.Name != "" {
			if m == nil {
				m = make(map[string]*mdmlab.MacOSSetup)
			}
...
func (c *Client) SaveEnvSecrets(alreadySaved map[string]string, toSave map[string]string, dryRun bool) error {
	...
	var secretsToSave []mdmlab.SecretVariable
	for key, value := range toSave {
		if _, ok := alreadySaved[key]; !ok {
			secretsToSave = append(secretsToSave, mdmlab.SecretVariable{Name: key, Value: value})
...
// DoGitOps applies the GitOps config to MDMlab.
func (c *Client) DoGitOps(
	...
	logf func(format string, args ...interface{}),
	dryRun bool,
	teamDryRunAssumptions *mdmlab.TeamSpecsDryRunAssumptions,
	appConfig *mdmlab.EnrichedAppConfig,
	// pass-by-ref to build lists
	teamsSoftwareInstallers map[string][]mdmlab.SoftwarePackageResponse,
	teamsVPPApps map[string][]mdmlab.VPPAppResponse,
	teamsScripts map[string][]mdmlab.ScriptResponse,
) (*mdmlab.TeamSpecsDryRunAssumptions, error) {
	baseDir := filepath.Dir(fullFilename)
	filename := filepath.Base(fullFilename)
	var teamAssumptions *mdmlab.TeamSpecsDryRunAssumptions
	var err error
	logFn := func(format string, args ...interface{}) {
		if logf != nil {
			logf(format, args...)
	...
	if config.TeamName == nil {
		...
		group.EnrollSecret = &mdmlab.EnrollSecretSpec{Secrets: config.OrgSettings["secrets"].([]*mdmlab.EnrollSecret)}
		...
		if windowsEnabledAndConfiguredAssumption, ok := mdmAppConfig["windows_enabled_and_configured"].(bool); ok {
			teamAssumptions = &mdmlab.TeamSpecsDryRunAssumptions{
	...
	teamIDsByName, teamsSoftwareInstallers, teamsVPPApps, teamsScripts, err := c.ApplyGroup(ctx, true, &group, baseDir, logf, appConfig, mdmlab.ApplyClientSpecOptions{
	...
	var teamSoftwareInstallers []mdmlab.SoftwarePackageResponse
	var teamVPPApps []mdmlab.VPPAppResponse
	var teamScripts []mdmlab.ScriptResponse
...
func (c *Client) doGitOpsNoTeamSoftware(
	...
	logFn func(format string, args ...interface{}),
	dryRun bool,
) ([]mdmlab.SoftwarePackageResponse, []mdmlab.VPPAppResponse, error) {
	...
	var macOSSetup mdmlab.MacOSSetup
	...
	var softwareInstallers []mdmlab.SoftwarePackageResponse

	packages := make([]mdmlab.SoftwarePackageSpec, 0, len(config.Software.Packages))
	packagesByPath := make(map[string]mdmlab.SoftwarePackageSpec, len(config.Software.Packages))
	...
	var appsPayload []mdmlab.VPPBatchPayload
	appsByAppID := make(map[string]mdmlab.TeamSpecAppStoreApp, len(config.Software.AppStoreApps))
	for _, vppApp := range config.Software.AppStoreApps {
		if vppApp != nil {
			// can be referenced by macos_setup.software
			appsByAppID[vppApp.AppStoreID] = *vppApp

			_, installDuringSetup := noTeamSoftwareMacOSSetup[mdmlab.MacOSSetupSoftware{AppStoreID: vppApp.AppStoreID}]
			appsPayload = append(appsPayload, mdmlab.VPPBatchPayload{
	...
	softwareInstallers, err = c.ApplyNoTeamSoftwareInstallers(swPkgPayload, mdmlab.ApplySpecOptions{DryRun: dryRun})
	if err != nil {
		return nil, nil, fmt.Errorf("applying software installers: %w", err)
	}
	logFn("[+] applying %d app store apps for 'No team'\n", len(appsPayload))
	vppApps, err := c.ApplyNoTeamAppStoreAppsAssociation(appsPayload, mdmlab.ApplySpecOptions{DryRun: dryRun})
	if err != nil {
...
func (c *Client) doGitOpsPolicies(config *spec.GitOps, teamSoftwareInstallers []mdmlab.SoftwarePackageResponse, teamVPPApps []mdmlab.VPPAppResponse, teamScripts []mdmlab.ScriptResponse, logFn func(format string, args ...interface{}), dryRun bool) error {
	...
	if teamID != nil {
		...
		for _, vppApp := range teamVPPApps {
			if vppApp.Platform != mdmlab.MacOSPlatform {
	...
	if len(config.Policies) > 0 {
		...
		if !dryRun {
			...
			for i := 0; i < len(config.Policies); i += batchSize {
				...
				policiesSpec := make([]*mdmlab.PolicySpec, len(policiesToApply))
...
func (c *Client) GetGitOpsSecrets(
	...
	if config.TeamName == nil {
		...
		if ok {
			secrets, ok := orgSecrets.([]*mdmlab.EnrollSecret)
			if ok {
		...
		if ok {
			secrets, ok := teamSecrets.([]*mdmlab.EnrollSecret)
			if ok {
				secretValues := make([]string, 0, len(secrets))
				for _, secret := range secrets {
					secretValues = append(secretValues, secret.Secret)
