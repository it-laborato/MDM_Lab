...
type getQueryResponse struct {
	Query *mdmlab.Query `json:"query,omitempty"`
	Err   error        `json:"error,omitempty"`
}

func (r getQueryResponse) error() error { return r.Err }

func getQueryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetQuery(ctx context.Context, id uint) (*mdmlab.Query, error) {
...
type listQueriesResponse struct {
	Queries []mdmlab.Query             `json:"queries"`
...
func listQueriesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	respQueries := make([]mdmlab.Query, 0, len(queries))
...
func (svc *Service) ListQueries(ctx context.Context, opt mdmlab.ListOptions, teamID *uint, scheduled *bool, mergeInherited bool, urlPlatform *string) ([]*mdmlab.Query, int, *mdmlab.PaginationMetadata, error) {
	// Check the user is allowed to list queries on the given team.
	if err := svc.authz.Authorize(ctx, &mdmlab.Query{
		TeamID: teamID,
	...
	if urlPlatform != nil {
		...
		if strings.Contains(*urlPlatform, ",") {
			return nil, 0, nil, &mdmlab.BadRequestError{Message: "queries can only be filtered by one platform at a time"}
		}
		targetableDBPlatforms := []string{"darwin", "windows", "linux"}
		if !slices.Contains(targetableDBPlatforms, *dbPlatform) {
			return nil, 0, nil, &mdmlab.BadRequestError{Message: fmt.Sprintf("platform %q cannot be a scheduled query target, supported platforms are: %s", *dbPlatform, strings.Join(targetableDBPlatforms, ","))}
		}
	}

	queries, count, meta, err := svc.ds.ListQueries(ctx, mdmlab.ListQueryOptions{
		ListOptions:    opt,
		TeamID:         teamID,
...
type getQueryReportResponse struct {
	...
	Results       []mdmlab.HostQueryResultRow `json:"results"`
...
func getQueryReportEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
	...
	results := []mdmlab.HostQueryResultRow{}
...
func (svc *Service) GetQueryReportResults(ctx context.Context, id uint, teamID *uint) ([]mdmlab.HostQueryResultRow, bool, error) {
	// Load query first to get its teamID.
	query, err := svc.ds.Query(ctx, id)
	if err != nil {
		setAuthCheckedOnPreAuthErr(ctx)
	...
	if !ok {
		return nil, false, mdmlab.ErrNoContext
	}
	...
	queryReportResults, err := mdmlab.MapQueryReportResultsToRows(queryReportResultRows)
	if err != nil {
...
func createQueryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) NewQuery(ctx context.Context, p mdmlab.QueryPayload) (*mdmlab.Query, error) {
	...
	if p.Logging == nil || (p.Logging != nil && *p.Logging == "") {
		p.Logging = ptr.String(mdmlab.LoggingSnapshot)
	}
	...
	if err := svc.NewActivity(
		...
		mdmlab.ActivityTypeCreatedSavedQuery{
			ID:   query.ID,
			Name: query.Name,
		},
	); err != nil {
...
func modifyQueryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ModifyQuery(ctx context.Context, id uint, p mdmlab.QueryPayload) (*mdmlab.Query, error) {
	...
	if err := svc.authz.Authorize(ctx, query, mdmlab.ActionWrite); err != nil {
		return nil, err
	}
	...
	if p.Logging != nil {
		if query.Logging != *p.Logging && *p.Logging != mdmlab.LoggingSnapshot {
			shouldDiscardQueryResults = true
		}
		query.Logging = *p.Logging
	}
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mdmlab.ActivityTypeEditedSavedQuery{
			ID:   query.ID,
			Name: query.Name,
		},
	); err != nil {
...
func deleteQueryEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) DeleteQuery(ctx context.Context, teamID *uint, name string) error {
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mdmlab.ActivityTypeDeletedSavedQuery{
			Name: name,
		},
	); err != nil {
...
func deleteQueryByIDEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func deleteQueriesEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) DeleteQueries(ctx context.Context, ids []uint) (uint, error) {
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mdmlab.ActivityTypeDeletedMultipleSavedQuery{
			IDs: ids,
		},
	); err != nil {
...
type applyQuerySpecsRequest struct {
	Specs []*mdmlab.QuerySpec `json:"specs"`
}
...
func applyQuerySpecsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) ApplyQuerySpecs(ctx context.Context, specs []*mdmlab.QuerySpec) error {
	...
	for _, query := range queries {
		...
		if (query.DiscardData && query.DiscardData != dbQuery.DiscardData) ||
			(query.Logging != dbQuery.Logging && query.Logging != mdmlab.LoggingSnapshot) ||
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mdmlab.ActivityTypeAppliedSpecSavedQuery{
			Specs: specs,
		},
	); err != nil {
...
func (svc *Service) queryFromSpec(ctx context.Context, spec *mdmlab.QuerySpec) (*mdmlab.Query, error) {
	var teamID *uint
	if spec.TeamName != "" {
...
func getQuerySpecsEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetQuerySpecs(ctx context.Context, teamID *uint) ([]*mdmlab.QuerySpec, error) {
	queries, _, _, err := svc.ListQueries(ctx, mdmlab.ListOptions{}, teamID, nil, false, nil)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "getting queries")
	}

	// Turn queries into specs.
	var specs []*mdmlab.QuerySpec
	for _, query := range queries {
...
func (svc *Service) specFromQuery(ctx context.Context, query *mdmlab.Query) (*mdmlab.QuerySpec, error) {
	var teamName string
	if query.TeamID != nil {
...
type getQuerySpecResponse struct {
	Spec *mdmlab.QuerySpec `json:"specs,omitempty"`
	Err  error            `json:"error,omitempty"`
}
...
func getQuerySpecEndpoint(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error) {
...
func (svc *Service) GetQuerySpec(ctx context.Context, teamID *uint, name string) (*mdmlab.QuerySpec, error) {
