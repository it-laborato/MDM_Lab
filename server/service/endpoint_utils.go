...
type handlerFunc func(ctx context.Context, request interface{}, svc mdmlab.Service) (errorer, error)
...
// a `url` tag with value list_options it'll gather mdmlab.ListOptions from the
			...
			if ok {
				...
				case reflect.Int:
					...
					case "order_direction", "inherited_order_direction":
						switch queryVal {
						case "desc":
							queryValInt = int(mdmlab.OrderDescending)
						case "asc":
							queryValInt = int(mdmlab.OrderAscending)
						case "":
							queryValInt = int(mdmlab.OrderAscending)
						default:
							return mdmlab.ListOptions{}, badRequest("unknown order_direction: " + queryVal)
						}
					default:
		...
		if !license.IsPremium(ctx) {
			for _, fp := range fields {
				if prem, ok := fp.sf.Tag.Lookup("premium"); ok {
					...
					if val && !fp.v.IsZero() {
						return nil, &mdmlab.BadRequestError{Message: fmt.Sprintf(
...
func badRequest(msg string) error {
	return &mdmlab.BadRequestError{Message: msg}
}
...
type authEndpointer struct {
	...
	r                 *mux.Router
	authFunc          func(svc mdmlab.Service, next endpoint.Endpoint) endpoint.Endpoint
	versions          []string
...
func newDeviceAuthenticatedEndpointer(svc mdmlab.Service, logger log.Logger, opts []kithttp.ServerOption, r *mux.Router, versions ...string) *authEndpointer {
	authFunc := func(svc mdmlab.Service, next endpoint.Endpoint) endpoint.Endpoint {
		return authenticatedDevice(svc, logger, next)
	}

	// Inject the mdmlab.CapabilitiesHeader header to the response for device endpoints
	opts = append(opts, capabilitiesResponseFunc(mdmlab.GetServerDeviceCapabilities()))
...
func newUserAuthenticatedEndpointer(svc mdmlab.Service, opts []kithttp.ServerOption, r *mux.Router, versions ...string) *authEndpointer {
	return &authEndpointer{
		svc:      svc,
		opts:     opts,
		r:        r,
...
func newHostAuthenticatedEndpointer(svc mdmlab.Service, logger log.Logger, opts []kithttp.ServerOption, r *mux.Router, versions ...string) *authEndpointer {
	authFunc := func(svc mdmlab.Service, next endpoint.Endpoint) endpoint.Endpoint {
...
func newOrbitAuthenticatedEndpointer(svc mdmlab.Service, logger log.Logger, opts []kithttp.ServerOption, r *mux.Router, versions ...string) *authEndpointer {
	authFunc := func(svc mdmlab.Service, next endpoint.Endpoint) endpoint.Endpoint {
		return authenticatedOrbitHost(svc, logger, next)
	}

	// Inject the mdmlab.Capabilities header to the response for Orbit hosts
	opts = append(opts, capabilitiesResponseFunc(mdmlab.GetServerOrbitCapabilities()))
...
func newNoAuthEndpointer(svc mdmlab.Service, opts []kithttp.ServerOption, r *mux.Router, versions ...string) *authEndpointer {
	return &authEndpointer{
		svc:      svc,
		opts:     opts,
		r:        r,
...
func getNameFromPathAndVerb(verb, path, startAt string) string {
	...
	return prefix + pathReplacer.Replace(strings.TrimPrefix(strings.TrimRight(path, "/"), "/api/_version_/mdmlab/"))
}

func capabilitiesResponseFunc(capabilities mdmlab.CapabilityMap) kithttp.ServerOption {
...
func writeCapabilitiesHeader(w http.ResponseWriter, capabilities mdmlab.CapabilityMap) {
	if len(capabilities) == 0 {
		return
	}

	w.Header().Set(mdmlab.CapabilitiesHeader, capabilities.String())
}
...
func (e *authEndpointer) handlePathHandler(path string, pathHandler func(path string) http.Handler, verb string) {
	...
	versionedPath := strings.Replace(path, "/_version_/", fmt.Sprintf("/{mdmlabversion:(?:%s)}/", strings.Join(versions, "|")), 1)
	...
	for _, alias := range e.alternativePaths {
		...
		versionedPath := strings.Replace(alias, "/_version_/", fmt.Sprintf("/{mdmlabversion:(?:%s)}/", strings.Join(versions, "|")), 1)
		if e.usePathPrefix {
			e.r.PathPrefix(versionedPath).Handler(pathHandler(versionedPath)).Name(nameAndVerb).Methods(verb)
		} else {
			e.r.Handle(versionedPath, pathHandler(versionedPath)).Name(nameAndVerb).Methods(verb)
