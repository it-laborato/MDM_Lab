...
func triggerGlobalHostStatusWebhook(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger) error {
...
func processWebhook(ctx context.Context, ds mdmlab.Datastore, teamID *uint, settings mdmlab.HostStatusWebhookSettings) error {
	...
	if percentUnseen >= settings.HostPercentage {
		...
		message := fmt.Sprintf(
			"More than %.2f%% of your hosts have not checked into MDMlab for more than %d days. "+
				"You've been sent this message because the Host status webhook is enabled in your MDMlab instance.",
...
func triggerTeamHostStatusWebhook(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger) error {
