...
func TestTriggerFailingPoliciesWebhookBasic(t *testing.T) {
	...
	err := failingPolicySet.AddHost(policyID1, mdmlab.PolicySetHost{
		ID:          1,
	...
	mockClock := time.Now()
	err = policies.TriggerFailingPoliciesAutomation(context.Background(), ds, kitlog.NewNopLogger(), failingPolicySet, func(pol *mdmlab.Policy, cfg policies.FailingPolicyAutomationConfig) error {
...
func TestTriggerFailingPoliciesWebhookTeam(t *testing.T) {
	...
	policiesByID := map[uint]*mdmlab.Policy{
		1: {
			PolicyData: mdmlab.PolicyData{
				ID:                    1,
				Name:                  "policy1",
	...
	failingPolicySet := service.NewMemFailingPolicySet()
	err := failingPolicySet.AddHost(1, mdmlab.PolicySetHost{
		ID:          1,
		Hostname:    "host1",
		DisplayName: "display1",
	})
	require.NoError(t, err)
	err = failingPolicySet.AddHost(2, mdmlab.PolicySetHost{
		ID:          2,
		Hostname:    "host2",
		DisplayName: "display2",
	})
...
func TestSendBatchedPOSTs(t *testing.T) {
	...
	makeHosts := func(c int) []mdmlab.PolicySetHost {
		hosts := make([]mdmlab.PolicySetHost, c)
		for i := 0; i < len(hosts); i++ {
			hosts[i] = mdmlab.PolicySetHost{
	...
	now := time.Now()
	serverURL, err := url.Parse("https://mdmlab.example.com")
	require.NoError(t, err)
