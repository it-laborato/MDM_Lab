...
func NewHandler(ctx context.Context, pool mdmlab.RedisPool, logger kitlog.Logger, ttl time.Duration) *Handler {
	eh := &Handler{
		pool:   pool,
		logger: logger,
		ttl:    ttl,
...
func newTestHandler(ctx context.Context, pool mdmlab.RedisPool, logger kitlog.Logger, ttl time.Duration, onStart func(), onStore func(error)) *Handler {
	eh := &Handler{
		pool:   pool,
		logger: logger,
		ttl:    ttl,
...
func hashError(err error) string {
	...
	ferr := ctxerr.MDMlabCause(err)

	var sb strings.Builder
	// hash the cause type and message (it might not be a MDMlabError)
	fmt.Fprintf(&sb, "%T\n%s\n", cause, cause.Error())

	// hash the stack trace of the root MDMlabError in the chain
	if ferr != nil {
		fmt.Fprint(&sb, strings.Join(ferr.Stack(), "\n"))
	}

	// hash the vital MDMlabError fields
	if mdmlabdErr, ok := cause.(mdmlab.MDMlabdError); ok {
		_, _ = fmt.Fprint(&sb,
			mdmlabdErr.ErrorSource,
			mdmlabdErr.ErrorSourceVersion,
			mdmlabdErr.ErrorAdditionalInfo,
		)
	}
