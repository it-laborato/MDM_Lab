...
// UserID partially implements the mdmlab.Auth interface.
...
// UserDisplayName partially implements the mdmlab.Auth interface.
...
// RequestID partially implements the mdmlab.Auth interface.
...
// AssertionAttributes partially implements the mdmlab.Auth interface.
func (r resp) AssertionAttributes() []mdmlab.SAMLAttribute {
	if r.response == nil {
		return nil
	}
	var attrs []mdmlab.SAMLAttribute
	for _, attr := range r.response.Assertion.AttributeStatement.Attributes {
		var values []mdmlab.SAMLAttributeValue
		for _, value := range attr.AttributeValues {
			values = append(values, mdmlab.SAMLAttributeValue{
				Type:  value.Type,
				Value: value.Value,
			})
		}
		attrs = append(attrs, mdmlab.SAMLAttribute{
			Name:   attr.Name,
			Values: values,
		})
	}
...
func DecodeAuthResponse(samlResponse string) (mdmlab.Auth, error) {
...
func decodeSAMLResponse(rawXML []byte) (mdmlab.Auth, error) {
	var saml Response
