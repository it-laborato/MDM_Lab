...
func NewCalendarSchedule(
	...
	const (
		name = string(mdmlab.CronCalendar)
	)
...
func cronCalendarEvents(ctx context.Context, ds mdmlab.Datastore, distributedLock mdmlab.Lock, serverConfig config.CalendarConfig,
...
func cronCalendarEventsForTeam(
	...
	var (
		passingHosts                       []mdmlab.HostPolicyMembershipData
		failingHosts                       []mdmlab.HostPolicyMembershipData
		failingHostsWithoutAssociatedEmail []mdmlab.HostPolicyMembershipData
	)
...
func processCalendarFailingHosts(
	...
	calendarConfig *calendar.Config,
	orgName string,
	hosts []mdmlab.HostPolicyMembershipData,
	logger kitlog.Logger,
) {
	hosts = filterHostsWithSameEmail(hosts)

	hostsCh := make(chan mdmlab.HostPolicyMembershipData)
		...
		go func() {
			...
			for host := range hostsCh {
				...
				if err == nil {
					...
					if hostCalendarEvent.WebhookStatus == mdmlab.CalendarWebhookStatusPending || hostCalendarEvent.WebhookStatus == mdmlab.CalendarWebhookStatusRetry {
						// This can happen if the host went offline (and never returned results)
						// after setting the webhook as pending.
						continue // continue with next host
					}
					now := time.Now()
					webhookAlreadyFired := hostCalendarEvent.WebhookStatus == mdmlab.CalendarWebhookStatusSent
				...
				case mdmlab.IsNotFound(err) || expiredEvent:
...
func filterHostsWithSameEmail(hosts []mdmlab.HostPolicyMembershipData) []mdmlab.HostPolicyMembershipData {
	minHostPerEmail := make(map[string]mdmlab.HostPolicyMembershipData)
	for _, host := range hosts {
	...
	filtered := make([]mdmlab.HostPolicyMembershipData, 0, len(minHostPerEmail))
...
func processFailingHostExistingCalendarEvent(
	...
	ds mdmlab.Datastore,
	distributedLock mdmlab.Lock,
	userCalendar mdmlab.UserCalendar,
	orgName string,
	hostCalendarEvent *mdmlab.HostCalendarEvent,
	calendarEvent *mdmlab.CalendarEvent,
	host mdmlab.HostPolicyMembershipData,
	policyIDtoPolicy *sync.Map,
	calendarConfig *calendar.Config,
	logger kitlog.Logger,
) error {
	...
	var newETag string
	var genBodyFn mdmlab.CalendarGenBodyFn = func(conflict bool) (string, bool, error) {
		var body string
	...
	if calendarConfig.AlwaysReloadEvent() || shouldReloadCalendarEvent(now, calendarEvent, hostCalendarEvent) {
		...
		updatedEvent, _, err = userCalendar.GetAndUpdateEvent(calendarEvent, genBodyFn,
			mdmlab.CalendarGetAndUpdateEventOpts{UpdateTimezone: true})
		if err != nil {
	...
	if hostCalendarEvent.WebhookStatus == mdmlab.CalendarWebhookStatusSent {
		return nil
	}
	...
	if err := ds.UpdateHostCalendarWebhookStatus(ctx, host.HostID, mdmlab.CalendarWebhookStatusPending); err != nil {
...
func getBodyTag(ctx context.Context, ds mdmlab.Datastore, host mdmlab.HostPolicyMembershipData, policyIDtoPolicy *sync.Map,
...
func shouldReloadCalendarEvent(now time.Time, calendarEvent *mdmlab.CalendarEvent, hostCalendarEvent *mdmlab.HostCalendarEvent) bool {
	// Check the user calendar regularly (but not every cron run)
	// to reduce load on both MDMlab and the calendar service.
	...
	if eventHappeningNow(now, calendarEvent) && hostCalendarEvent.WebhookStatus == mdmlab.CalendarWebhookStatusNone {
		return true
	}
	return false
}

func eventHappeningNow(now time.Time, calendarEvent *mdmlab.CalendarEvent) bool {
...
func processFailingHostCreateCalendarEvent(
	...
	if _, err := ds.CreateOrUpdateCalendarEvent(
		...
		host.HostID, mdmlab.CalendarWebhookStatusNone,
	); err != nil {
	...
	for {
		...
		calendarEvent, err := userCalendar.CreateEvent(
			preferredDate, func(conflict bool) (string, bool, error) {
				...
				body, generatedTag = calendar.GenerateCalendarEventBody(ctx, ds, orgName, host, policyIDtoPolicy, conflict, logger)
				return body, true, nil
			}, mdmlab.CalendarCreateEventOpts{},
		)
		var dee mdmlab.DayEndedError
		switch {
		case err == nil:
...
func isHostOnline(ctx context.Context, ds mdmlab.Datastore, hostID uint) (bool, error) {
	...
	switch status {
	case mdmlab.StatusOnline, mdmlab.StatusNew:
		return true, nil
	case mdmlab.StatusOffline, mdmlab.StatusMIA, mdmlab.StatusMissing:
		return false, nil
	default:
...
func cronCalendarEventsCleanup(ctx context.Context, ds mdmlab.Datastore, logger kitlog.Logger) error {
	appConfig, err := ds.AppConfig(ctx)
	if err != nil {
		return fmt.Errorf("load app config: %w", err)
	}

	var userCalendar mdmlab.UserCalendar
...
func deleteCalendarEventsInParallel(
	ctx context.Context, ds mdmlab.Datastore, calendarConfig *calendar.Config, calendarEvents []*mdmlab.CalendarEvent,
	logger kitlog.Logger,
) {
	if len(calendarEvents) > 0 {
		calendarEventCh := make(chan *mdmlab.CalendarEvent)
		var wg sync.WaitGroup
		for i := 0; i < calendarConsumers; i++ {
			wg.Add(+1)
			go func() {
...
func deleteCalendarEvent(
	ctx context.Context, ds mdmlab.Datastore, userCalendar mdmlab.UserCalendar, calendarEvent *mdmlab.CalendarEvent,
) error {
	if userCalendar != nil {
