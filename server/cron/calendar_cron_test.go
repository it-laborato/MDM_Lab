...
func TestEventForDifferentHost(t *testing.T) {
	...
	policyID1 := uint(10)
	ds.GetCalendarPoliciesFunc = func(ctx context.Context, teamID uint) ([]mdmlab.PolicyCalendarData, error) {
		require.Equal(t, teamID1, teamID)
		return []mdmlab.PolicyCalendarData{
			{
				ID:   policyID1,
				Name: "Policy 1",
			},
	...
	ds.GetTeamHostsPolicyMembershipsFunc = func(
		...
		return []mdmlab.HostPolicyMembershipData{
			{
				HostID:           hostID1,
				Email:            userEmail1,
				Passing:          false,
	...
	ds.GetHostCalendarEventByEmailFunc = func(ctx context.Context, email string) (*mdmlab.HostCalendarEvent, *mdmlab.CalendarEvent, error) {
		require.Equal(t, userEmail1, email)
		calEvent := &mdmlab.CalendarEvent{
			ID:        1,
			Email:     email,
			StartTime: eventTime,
			EndTime:   eventTime,
		}
		hcEvent := &mdmlab.HostCalendarEvent{
			ID:              1,
			HostID:          hostID2,
			CalendarEventID: 1,
			WebhookStatus:   mdmlab.CalendarWebhookStatusNone,
		}
...
func TestCalendarEventsMultipleHosts(t *testing.T) {
	...
	ds.PolicyLiteFunc = func(ctx context.Context, policyID uint) (*mdmlab.PolicyLite, error) {
		switch policyID {
		case policyID1:
			return &mdmlab.PolicyLite{
				ID:          policyID1,
	...
	calendarEvents := make(map[string]*mdmlab.CalendarEvent)
	hostCalendarEvents := make(map[uint]*mdmlab.HostCalendarEvent)
	...
	ds.CreateOrUpdateCalendarEventFunc = func(ctx context.Context,
		...
		timeZone *string,
		hostID uint,
		webhookStatus mdmlab.CalendarWebhookStatus,
	) (*mdmlab.CalendarEvent, error) {
		assert.NotEmpty(t, uuid)
		require.Equal(t, hostID1, hostID)
		require.Equal(t, userEmail1, email)
		require.Equal(t, mdmlab.CalendarWebhookStatusNone, webhookStatus)
		...
		calendarEvents[email] = &mdmlab.CalendarEvent{
			ID:        calendarEventID,
			Email:     email,
			StartTime: startTime,
			EndTime:   endTime,
		...
		hostCalendarEvents[hostID] = &mdmlab.HostCalendarEvent{
	...
	strings.Contains(createdCalendarEvents["1"].Description, mdmlab.CalendarDefaultDescription)
	strings.Contains(createdCalendarEvents["1"].Description, mdmlab.CalendarDefaultResolution)
}
...
func TestCalendarEvents1KHosts(t *testing.T) {
	...
	hosts := make([]mdmlab.HostPolicyMembershipData, 0, 1000)
	for i := 0; i < 1000; i++ {
		newHost := mdmlab.HostPolicyMembershipData{
	...
	eventPerHost := make(map[uint]*mdmlab.CalendarEvent)
	...
	ds.CreateOrUpdateCalendarEventFunc = func(ctx context.Context,
		...
		eventsCreated += 1
		eventPerHost[hostID] = &mdmlab.CalendarEvent{
			ID:        hostID,
			Email:     email,
			StartTime: startTime,
			EndTime:   endTime,
	...
	hosts = make([]mdmlab.HostPolicyMembershipData, 0, 1000)
	for i := 0; i < 1000; i++ {
		hosts = append(hosts, mdmlab.HostPolicyMembershipData{
			Email:              fmt.Sprintf("user%d@example.com", i),
			Passing:            true,
			HostID:             uint(i), //nolint:gosec // dismiss G115
			HostDisplayName:    fmt.Sprintf("display_name%d", i),
	...
	ds.GetHostCalendarEventByEmailFunc = func(ctx context.Context, email string) (*mdmlab.HostCalendarEvent, *mdmlab.CalendarEvent, error) {
		...
		return &mdmlab.HostCalendarEvent{
...
func TestEventBody(t *testing.T) {
	...
	var eventsMu sync.Mutex
	calendarEvents := make(map[uint]*mdmlab.CalendarEvent)
	hostCalendarEvents := make(map[uint]*mdmlab.HostCalendarEvent)
	...
	ds.CreateOrUpdateCalendarEventFunc = func(
		...
		eventsMu.Lock()
		calendarEventID := uint(len(calendarEvents) + 1) //nolint:gosec // dismiss G115
		calendarEvents[hostID] = &mdmlab.CalendarEvent{
			ID:        calendarEventID,
			Email:     email,
			StartTime: startTime,
			EndTime:   endTime,
	...
	for _, hostCalEvent := range hostCalendarEvents {
		...
		require.NoError(t, err)
		// What Google Calendar calls the "Description" is what MDMlab calls the "Body," since the Body
		// contains a description and a resolution.
		eventBody := createdCalendarEvents[details["id"]].Description
		switch hostCalEvent.HostID {
		case hostID1:
			assert.Contains(t, eventBody, fmt.Sprintf(`%s %s (Host 1).`, orgName, mdmlab.CalendarBodyStaticHeader))
			assert.Contains(t, eventBody, "Description for policy 1")
			assert.Contains(t, eventBody, "Resolution for policy 1")
		case hostID6:
			assert.Contains(t, eventBody, fmt.Sprintf(`%s %s (Host 6).`, orgName, mdmlab.CalendarBodyStaticHeader))
			assert.Contains(t, eventBody, "Description for policy 1")
			assert.Contains(t, eventBody, "Resolution for policy 1")
		default:
			assert.Contains(t, eventBody, fmt.Sprintf(`%s %s (Host`, orgName, mdmlab.CalendarBodyStaticHeader))
			assert.Contains(t, eventBody, mdmlab.CalendarDefaultResolution)
		}
	}
}
