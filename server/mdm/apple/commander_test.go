...
import (
	...
	svcmock "github.com/it-laborato/MDM_Lab/server/service/mock"
...
func TestMDMAppleCommander(t *testing.T) {
	...
	host := &mdmlab.Host{ID: 1, UUID: "A", Platform: "darwin"}
	cmdUUID = uuid.New().String()
	mdmStorage.EnqueueDeviceLockCommandFunc = func(ctx context.Context, gotHost *mdmlab.Host, cmd *mdm.Command, pin string) error {
	...
	cmdUUID = uuid.New().String()
	mdmStorage.EnqueueDeviceWipeCommandFunc = func(ctx context.Context, gotHost *mdmlab.Host, cmd *mdm.Command) error {
		require.NotNil(t, gotHost)
		require.Equal(t, host.ID, gotHost.ID)
		require.Equal(t, host.UUID, gotHost.UUID)
