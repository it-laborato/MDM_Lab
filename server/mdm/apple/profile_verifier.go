...
// Profile verification is a set of related processes that run on the MDMlab server to ensure that
// the MDM profiles installed on a host are the ones expected by the MDMlab server. Expected profiles
// comprise the profiles that belong to the host's assigned team (or no
// team, as applicable).
//
// The MDMlab server enqueues commands to install profiles on hosts via the MDM
// protocol. The MDMlab server periodically runs a cron that enqueues install profile
...
// enrolls in MDMlab, when a host's team membership changes, when a new profile is uploaded, when an
...
//
// - When the results of an install profile command are reported via the MDM protocol, the MDMlab server
...
//
// - When host details are reported via osquery, the MDMlab server ingests a list of installed
...
// updates the verification status in the datastore. It is intended to be called by MDMlab osquery
// service when the MDMlab server ingests host details.
func VerifyHostMDMProfiles(ctx context.Context, ds mdmlab.ProfileVerificationStore, host *mdmlab.Host, installed map[string]*mdmlab.HostMacOSProfile) error {
...
// called by the MDMlab MDM checkin and command service install profile request handler.
func HandleHostMDMProfileInstallResult(ctx context.Context, ds mdmlab.ProfileVerificationStore, hostUUID string, cmdUUID string, status *mdmlab.MDMDeliveryStatus, detail string) error {
	if status != nil && *status == mdmlab.MDMDeliveryFailed {
		...
		host := &mdmlab.Host{UUID: hostUUID, Platform: "darwin"}
