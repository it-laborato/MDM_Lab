...
import (
	...
	ctxabm "github.com:it-laborato/MDM_Lab/server/contexts/apple_bm"
...
const (
	// SCEPPath is MDMlab's HTTP path for the SCEP service.
	SCEPPath = "/mdm/apple/scep"
	// MDMPath is MDMlab's HTTP path for the core MDM service.
	...
	// MDMlabUISSOCallbackPath is the front-end route used to
	// redirect after the SSO flow is completed.
	MDMlabUISSOCallbackPath = "/mdm/sso/callback"

	// MDMlabPayloadIdentifier is the value for the "<key>PayloadIdentifier</key>"
	// used by MDMlab MDM on the enrollment profile.
	MDMlabPayloadIdentifier = "com.mdmlabdm.mdmlab.mdm.apple"
...
// internal usage within MDMlab. If you need to expose any of the functionality
...
func (d *DEPService) getDefaultProfile() *godep.Profile {
	return &godep.Profile{
		ProfileName:      "MDMlab default enrollment profile",
		AllowPairing:     true,
...
func (d *DEPService) createDefaultAutomaticProfile(ctx context.Context) error {
	...
	payload := mdmlab.MDMAppleEnrollmentProfilePayload{
		Token:      token,
		Type:       mdmlab.MDMAppleEnrollmentTypeAutomatic,
...
func (d *DEPService) buildJSONProfile(ctx context.Context, setupAsstJSON json.RawMessage, appCfg *mdmlab.AppConfig, team *mdmlab.Team, enrollURL string) (*godep.Profile, error) {
	...
	if jsonProf.ConfigurationWebURL == "" {
		...
		// flow, otherwise use MDMlab's enroll URL.
		//
	...
	// automatically by MDMlab or manually by the user if
...
func (d *DEPService) RegisterProfileWithAppleDEPServer(ctx context.Context, team *mdmlab.Team, setupAsst *mdmlab.MDMAppleSetupAssistant, abmTokenOrgName string) (string, time.Time, error) {
	...
	defaultProf, err := d.ds.GetMDMAppleEnrollmentProfileByType(ctx, mdmlab.MDMAppleEnrollmentTypeAutomatic)
	if err != nil {
...
// Apple API.
func (d *DEPService) ValidateSetupAssistant(ctx context.Context, team *mdmlab.Team, setupAsst *mdmlab.MDMAppleSetupAssistant, abmTokenOrgName string) error {
	appCfg, err := d.ds.AppConfig(ctx)
	if err != nil {
...
// token.
func (d *DEPService) EnsureDefaultSetupAssistant(ctx context.Context, team *mdmlab.Team, abmTokenOrgName string) (string, time.Time, error) {
	...
	defProf, err := d.ds.GetMDMAppleEnrollmentProfileByType(ctx, mdmlab.MDMAppleEnrollmentTypeAutomatic)
...
// no error.
func (d *DEPService) EnsureCustomSetupAssistantIfExists(ctx context.Context, team *mdmlab.Team, abmTokenOrgName string) (string, time.Time, error) {
	var tmID *uint
	if team != nil {
		tmID = &team.ID
	}
...
func (d *DEPService) RunAssigner(ctx context.Context) error {
	...
	teamsByID := make(map[uint]*mdmlab.Team, len(teams))
	...
	for _, token := range tokens {
		var macOSTeam, iosTeam, ipadTeam *mdmlab.Team
		...
		teams := []*mdmlab.Team{macOSTeam, iosTeam, ipadTeam}
...
func (d *DEPService) processDeviceResponse(
	...
	macOSTeam *mdmlab.Team,
	iosTeam *mdmlab.Team,
	ipadTeam *mdmlab.Team,
) error {
	...
	// MDMlab in ABM. A host in this state will have a row in `host_dep_assignments` where the
	...
	// See https://github.com/mdmlabdm/mdmlab/issues/23200 for more context.
...
// changes.
func NewDEPClient(storage godep.ClientStorage, updater mdmlab.ABMTermsUpdater, logger kitlog.Logger) *godep.Client {
	return godep.NewClient(storage, mdmlabhttp.NewClient(), godep.WithAfterHook(func(ctx context.Context, reqErr error) error {
        ...
        <dict>
          ...
          <array>
            <array>
              <array>
                <string>O</string>
                <string>MDMlab</string>
              </array>
            </array>
            <array>
              <array>
                <string>CN</string>
                <string>MDMlab Identity</string>
              </array>
            </array>
          </array>
        </dict>
...
// enrollmentProfileMobileconfigTemplate is the template MDMlab uses to assemble a .mobileconfig enrollment profile to serve to devices.
//
...
<dict>
	...
	<array>
		<dict>
			...
			<dict>
				...
				<key>Subject</key>
				<array>
					<array><array><string>O</string><string>MDMlab</string></array></array>
					<array><array><string>CN</string><string>MDMlab Identity</string></array></array>
				</array>
			</dict>
			<key>PayloadIdentifier</key>
			<string>com.mdmlabdm.mdmlab.mdm.apple.scep</string>
	...
	<key>PayloadIdentifier</key>
	<string>` + MDMlabPayloadIdentifier + `</string>
...
func GenerateEnrollmentProfileMobileconfig(orgName, mdmlabURL, scepChallenge, topic string) ([]byte, error) {
	scepURL, err := ResolveAppleSCEPURL(mdmlabURL)
	if err != nil {
		return nil, fmt.Errorf("resolve Apple SCEP url: %w", err)
	}
	serverURL, err := ResolveAppleMDMURL(mdmlabURL)
	if err != nil {
...
func AddEnrollmentRefToMDMlabURL(mdmlabURL, reference string) (string, error) {
	if reference == "" {
		return mdmlabURL, nil
	}

	u, err := url.Parse(mdmlabURL)
	if err != nil {
		return "", fmt.Errorf("parsing configured server URL: %w", err)
	}
	q := u.Query()
	q.Add(mobileconfig.MDMlabEnrollReferenceKey, reference)
	u.RawQuery = q.Encode()
	return u.String(), nil
}
...
type ProfileBimap struct {
	wantedState  map[*mdmlab.MDMAppleProfilePayload]*mdmlab.MDMAppleProfilePayload
	currentState map[*mdmlab.MDMAppleProfilePayload]*mdmlab.MDMAppleProfilePayload
}

// NewProfileBimap retuns a new ProfileBimap
func NewProfileBimap() *ProfileBimap {
	return &ProfileBimap{
		map[*mdmlab.MDMAppleProfilePayload]*mdmlab.MDMAppleProfilePayload{},
		map[*mdmlab.MDMAppleProfilePayload]*mdmlab.MDMAppleProfilePayload{},
	}
}

// GetMatchingProfileInDesiredState returns the addition key that matches the given removal
func (pb *ProfileBimap) GetMatchingProfileInDesiredState(removal *mdmlab.MDMAppleProfilePayload) (*mdmlab.MDMAppleProfilePayload, bool) {
	value, ok := pb.currentState[removal]
	return value, ok
}

// GetMatchingProfileInCurrentState returns the removal key that matches the given addition
func (pb *ProfileBimap) GetMatchingProfileInCurrentState(addition *mdmlab.MDMAppleProfilePayload) (*mdmlab.MDMAppleProfilePayload, bool) {
...
func (pb *ProfileBimap) IntersectByIdentifierAndHostUUID(wantedProfiles, currentProfiles []*mdmlab.MDMAppleProfilePayload) {
	key := func(p *mdmlab.MDMAppleProfilePayload) string {
		return fmt.Sprintf("%s-%s", p.ProfileIdentifier, p.HostUUID)
	}

	removeProfs := map[string]*mdmlab.MDMAppleProfilePayload{}
...
func (pb *ProfileBimap) add(wantedProfile, currentProfile *mdmlab.MDMAppleProfilePayload) {
	pb.wantedState[wantedProfile] = currentProfile
	pb.currentState[currentProfile] = wantedProfile
}

func IOSiPadOSRefetch(ctx context.Context, ds mdmlab.Datastore, commander *MDMAppleCommander, logger kitlog.Logger) error {
	...
	hostMDMCommands := make([]mdmlab.HostMDMCommand, 0, 2*len(devices))
	installedAppsUUIDs := make([]string, 0, len(devices))
	for _, device := range devices {
		if !slices.Contains(device.CommandsAlreadySent, mdmlab.RefetchAppsCommandUUIDPrefix) {
	...
	if len(installedAppsUUIDs) > 0 {
		err = commander.InstalledApplicationList(ctx, installedAppsUUIDs, mdmlab.RefetchAppsCommandUUIDPrefix+commandUUID)
		if err != nil {
	...
	for _, device := range devices {
		if !slices.Contains(device.CommandsAlreadySent, mdmlab.RefetchDeviceCommandUUIDPrefix) {
	...
	if len(deviceInfoUUIDs) > 0 {
		if err := commander.DeviceInformation(ctx, deviceInfoUUIDs, mdmlab.RefetchDeviceCommandUUIDPrefix+commandUUID); err != nil {
...
func GenerateOTAEnrollmentProfileMobileconfig(orgName, mdmlabURL, enrollSecret string) ([]byte, error) {
	path, err := url.JoinPath(mdmlabURL, "/api/v1/mdmlab/ota_enrollment")
	if err != nil {
