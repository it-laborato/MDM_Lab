...
// MDMAppleCommander contains methods to enqueue commands managed by MDMlab and
// send push notifications to hosts.
//
// It's intentionally decoupled from mdmlab.Service so it can be used internally
// in crons and other services, leaving authentication/permission handling to
// the caller.
type MDMAppleCommander struct {
	storage mdmlab.MDMAppleStore
	pusher  nanomdm_push.Pusher
}

// NewMDMAppleCommander creates a new commander instance.
func NewMDMAppleCommander(mdmStorage mdmlab.MDMAppleStore, mdmPushService nanomdm_push.Pusher) *MDMAppleCommander {
...
func (svc *MDMAppleCommander) DeviceLock(ctx context.Context, host *mdmlab.Host, uuid string) (unlockPIN string, err error) {
...
func (svc *MDMAppleCommander) EraseDevice(ctx context.Context, host *mdmlab.Host, uuid string) error {
