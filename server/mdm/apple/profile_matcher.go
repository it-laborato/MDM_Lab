...
const (
	...
	// See https://github.com/mdmlabdm/mdmlab/issues/12483 for more info.
...
func NewProfileMatcher(pool mdmlab.RedisPool) mdmlab.ProfileMatcher {
...
// the matching stage.
func (p *profileMatcher) PreassignProfile(ctx context.Context, payload mdmlab.MDMApplePreassignProfilePayload) error {
	var invArg mdmlab.InvalidArgumentError
	...
	if len(payload.Profile) == 0 {
		...
		if cp, err := mdmlab.NewMDMAppleConfigProfile(payload.Profile, nil); err != nil {
...
func (p *profileMatcher) RetrieveProfiles(ctx context.Context, externalHostIdentifier string) (mdmlab.MDMApplePreassignHostProfiles, error) {
	var hostProfs mdmlab.MDMApplePreassignHostProfiles
	...
	for k, v := range profs {
		...
		hostProfs.Profiles = append(hostProfs.Profiles, mdmlab.MDMApplePreassignProfile{
			Profile:    []byte(v),
