...
type AssetSets struct {
	...
	// VisionOS []Asset `json:"visionOS"` // MDMlab doesn't support visionOS yet
	// XROS     []Asset `json:"xrOS"`    // MDMlab doesn't support xrOS yet
}
...
type APIResponse struct {
	...
	// PublicRapidSecurityResponses interface{} `json:"PublicRapidSecurityResponses"` // MDMlab doesn't support PublicRapidSecurityResponses yet
}
...
// [1]: http://gdmf.apple.com/v2/pmv
// [2]: https://support.apple.com/guide/deployment/use-mdm-to-deploy-software-updates-depafd2fad80/web
func GetLatestOSVersion(device mdmlab.MDMAppleMachineInfo) (*Asset, error) {
	...
	for i, s := range assetSet {
		for _, d := range s.SupportedDevices {
			if d == device.Product || d == device.SoftwareUpdateDeviceID {
				...
				if mdmlab.CompareVersions(assetSet[latestIdx].ProductVersion, s.ProductVersion) < 0 {
...
// use.
var client = mdmlabhttp.NewClient(mdmlabhttp.WithTimeout(10 * time.Second))
...
func GetAssetMetadata() (*APIResponse, error) {
	...
	req.Header.Set("User-Agent", "mdmlab-device-management")
