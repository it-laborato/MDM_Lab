...
const (
	// MDMlabFileVaultPayloadIdentifier is the value for the PayloadIdentifier
	// used by MDMlab to configure FileVault and FileVault Escrow.
	MDMlabFileVaultPayloadIdentifier = "com.mdmlabdm.mdmlab.mdm.filevault"

	// MDMlabdConfigPayloadIdentifier is the value for the PayloadIdentifier used
	// by mdmlabd to read configuration values from the system.
	MDMlabdConfigPayloadIdentifier = "com.mdmlabdm.mdmlabd.config"

	// MDMlabCARootConfigPayloadIdentifier TODO
	MDMlabCARootConfigPayloadIdentifier = "com.mdmlabdm.caroot"

	// MDMlabEnrollmentPayloadIdentifier is the value for the PayloadIdentifier used
	// by MDMlab to enroll a device with the MDM server.
	MDMlabEnrollmentPayloadIdentifier = "com.mdmlabdm.mdmlab.mdm.apple.mdm"

	// MDMlabEnrollReferenceKey is the key used by MDMlab of the URL query parameter representing a unique
	// identifier for an MDM enrollment. The unique value of the query parameter is appended to the
	// MDMlab server URL when an MDM enrollment profile is generated for download by a device.
	//
	...
	MDMlabEnrollReferenceKey = "enroll_reference"
)

// MDMlabPayloadIdentifiers returns a map of PayloadIdentifier strings
// that are handled and delivered by MDMlab.
//
...
func MDMlabPayloadIdentifiers() map[string]struct{} {
	return map[string]struct{}{
		MDMlabFileVaultPayloadIdentifier:    {},
		MDMlabdConfigPayloadIdentifier:      {},
		MDMlabCARootConfigPayloadIdentifier: {},
	}
}

// MDMlabPayloadTypes returns a map of PayloadType strings
// that are handled and delivered by MDMlab.
//
...
func MDMlabPayloadTypes() map[string]struct{} {
	return map[string]struct{}{
...
// of an Apple MDM configuration profile in MDMlab.
//
...
// ParseConfigProfile attempts to parse the Mobileconfig byte slice as a MDMlab MDMAppleConfigProfile.
//
// The byte slice must be XML or PKCS7 parseable. MDMlab also requires that it contains both
...
func (mc *Mobileconfig) ScreenPayloads() error {
	...
	mdmlabIdentifiers := MDMlabPayloadIdentifiers()
	mdmlabTypes := MDMlabPayloadTypes()
	screenedTypes := []string{}
	screenedIdentifiers := []string{}
	screenedNames := []string{}
	for _, t := range pct {
		if _, ok := mdmlabTypes[t.Type]; ok {
			screenedTypes = append(screenedTypes, t.Type)
		}
		if _, ok := mdmlabIdentifiers[t.Identifier]; ok {
			screenedIdentifiers = append(screenedIdentifiers, t.Identifier)
		}
		if _, ok := mdmlabNames[t.Name]; ok {
