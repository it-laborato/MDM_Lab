...
func New(ds mdmlab.Datastore, logger kitlog.Logger) *HostLifecycle {
...
func (t *HostLifecycle) turnOnDarwin(ctx context.Context, opts HostOptions) error {
	...
	if info.DEPAssignedToMDMlab || info.InstalledFromDEP {
...
func (t *HostLifecycle) restorePendingDEPHost(ctx context.Context, host *mdmlab.Host, abmTokenID *uint) error {
	if abmTokenID == nil {
...
func (t *HostLifecycle) getDefaultTeamForABMToken(ctx context.Context, host *mdmlab.Host, abmTokenID uint) (*uint, error) {
	...
	default:
		return nil, ctxerr.NewWithData(ctx, "attempting to get default ABM team for host with invalid platform", map[string]any{"host_platform": host.MDMlabPlatform(), "host_id": host.ID})
	}
