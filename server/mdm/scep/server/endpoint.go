...
func MakeClientEndpoints(instance string, timeout *time.Duration) (*Endpoints, error) {
	...
	var mdmlabOpts []mdmlabhttp.ClientOpt
	if timeout != nil {
		mdmlabOpts = append(mdmlabOpts, mdmlabhttp.WithTimeout(*timeout))
	}
	options := []httptransport.ClientOption{httptransport.SetClient(mdmlabhttp.NewClient(mdmlabOpts...))}
