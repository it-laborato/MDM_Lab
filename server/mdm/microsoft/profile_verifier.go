...
func LoopOverExpectedHostProfiles(
	ctx context.Context,
	ds mdmlab.ProfileVerificationStore,
	host *mdmlab.Host,
	fn func(profile *mdmlab.ExpectedMDMProfile, hash, locURI, data string),
) error {
...
func VerifyHostMDMProfiles(ctx context.Context, logger log.Logger, ds mdmlab.ProfileVerificationStore, host *mdmlab.Host,
...
func splitMissingProfilesIntoFailAndRetryBuckets(ctx context.Context, ds mdmlab.ProfileVerificationStore, host *mdmlab.Host,
...
func compareResultsToExpectedProfiles(ctx context.Context, logger log.Logger, ds mdmlab.ProfileVerificationStore, host *mdmlab.Host,
	profileResults profileResultsTransform) (verified map[string]struct{}, missing map[string]struct{}, err error) {
	missing = map[string]struct{}{}
	verified = map[string]struct{}{}
	err = LoopOverExpectedHostProfiles(ctx, ds, host, func(profile *mdmlab.ExpectedMDMProfile, ref, locURI, wantData string) {
...
func transformProfileResults(rawProfileResultsSyncML []byte) (profileResultsTransform, error) {
	...
	for _, r := range syncML.GetOrderedCmds() {
		...
		ref := *r.Cmd.CmdRef
		if r.Verb == mdmlab.CmdStatus && r.Cmd.Data != nil {
			transform.cmdRefToStatus[ref] = *r.Cmd.Data
		}

		if r.Verb == mdmlab.CmdResults {
