...
org_settings:
  server_settings:
    server_url: https://mdmlab.example.com
  org_info:
...
func TestValidGitOpsYaml(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"global_config_no_paths": {
			environment: map[string]string{
				"FLEET_SECRET_FLEET_SECRET_": "mdmlab_secret",
		...
		"global_config_with_paths": {
			environment: map[string]string{
				...
				"ORG_NAME":                      "MDMlab Device Management",
				"FLEET_SECRET_FLEET_SECRET_":    "mdmlab_secret",
				"FLEET_SECRET_NAME":             "secret_name",
				"FLEET_SECRET_length":           "10",
				"FLEET_SECRET_BANANA":           "bread",
	...
	for name, test := range tests {
		...
		t.Run(
			name, func(t *testing.T) {
				...
				var appConfig *mdmlab.EnrichedAppConfig
				if test.isTeam {
					appConfig = &mdmlab.EnrichedAppConfig{}
					appConfig.License = &mdmlab.LicenseInfo{
				...
				if test.isTeam {
					...
					require.Len(t, secrets.([]*mdmlab.EnrollSecret), 2)
					assert.Equal(t, "SampleSecret123", secrets.([]*mdmlab.EnrollSecret)[0].Secret)
					assert.Equal(t, "ABC", secrets.([]*mdmlab.EnrollSecret)[1].Secret)
					...
					assert.Equal(t, "https://mdmlab.example.com", serverSettings.(map[string]interface{})["server_url"])
					...
					assert.True(t, ok)
					assert.Equal(t, "MDMlab Device Management", orgInfo["org_name"])
					...
					assert.Contains(t, gitops.OrgSettings, "mdmlab_desktop")
				...
				require.Len(t, gitops.MDMlabSecrets, 4)
				assert.Equal(t, "mdmlab_secret", gitops.MDMlabSecrets["FLEET_SECRET_FLEET_SECRET_"])
				assert.Equal(t, "secret_name", gitops.MDMlabSecrets["FLEET_SECRET_NAME"])
				assert.Equal(t, "10", gitops.MDMlabSecrets["FLEET_SECRET_length"])
				assert.Equal(t, "bread", gitops.MDMlabSecrets["FLEET_SECRET_BANANA"])
				...
				if test.isTeam {
					...
					assert.Equal(t, "./lib/collect-mdmlabd-logs.sh", gitops.Policies[7].RunScript.Path)
					...
					assert.Contains(t, gitops.Policies[8].RunScript.Path, "./lib/collect-mdmlabd-logs.sh")
				}
			},
		)
	}
...
- name: orbit_info \$NOT_EXPANDED \\\$ALSO_NOT_EXPANDED
  query: "SELECT * from orbit_info; -- double quotes are escaped by YAML after MDMlab's escaping of backslashes \\\\\$NOT_EXPANDED"
  interval: 0
...
`
	...
	require.Equal(t, `SELECT * from orbit_info; -- double quotes are escaped by YAML after MDMlab's escaping of backslashes \$NOT_EXPANDED`, gitOps.Queries[0].Query)
...
`
	...
	require.NoError(t, err)
	err = file.Copy(
		filepath.Join("testdata", "lib", "collect-mdmlabd-logs.sh"),
		filepath.Join(basePath, "lib", "collect-mdmlabd-logs.sh"),
		0o755,
	)
	require.NoError(t, err)
	appConfig := mdmlab.EnrichedAppConfig{}
...
func TestIllegalMDMlabSecret(t *testing.T) {
	t.Parallel()
	config := getGlobalConfig([]string{"policies"})
	config += `
policies:
