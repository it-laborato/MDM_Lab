...
// BenchmarkExtractInstallerMetadata/MDMlab_osquery.msi/file_size:_43775_kb-8                               4    277.60 ms/op   252408264 B/op     3737 allocs/op
...
// BenchmarkExtractInstallerMetadata/MDMlab_osquery.msi/file_size:_43775_kb-8                               4    296.01 ms/op   252408208 B/op     3737 allocs/op
	...
	for _, dent := range dents {
		...
		b.Run(wantName+"."+wantExtension, func(b *testing.B) {
			...
			b.Run(fmt.Sprintf("file size: %d kb", info.Size()/1024), func(b *testing.B) {
				...
				// iteration.
				for i := 0; i < b.N; i++ {
					tfr, err := mdmlab.NewKeepFileReader(filepath.Join("testdata", "installers", dent.Name()))
					require.NoError(b, err)
