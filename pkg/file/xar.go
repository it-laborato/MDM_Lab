...
// in the XAR format.
func ExtractXARMetadata(tfr *mdmlab.TempFileReader) (*InstallerMetadata, error) {
	var hdr xarHeader
...
func getPackageInfo(p *packageInfoXML) (name string, identifier string, version string, packageIDs []string) {
	...
	for _, bundle := range p.Bundles {
		...
		if base, isValid := isValidAppFilePath(installPath); isValid {
			identifier = mdmlab.Preprocess(bundle.ID)
			name = base
			version = mdmlab.Preprocess(bundle.CFBundleShortVersionString)
		}
		bundleID := mdmlab.Preprocess(bundle.ID)
		if bundleID != "" {
