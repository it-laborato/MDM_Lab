...
type TestAppleMDMClient struct {
	...
	// mdmlabServerURL is the URL of the MDMlab server, used to fetch the enrollment profile.
	...
	// will fetch the enrollment profile from MDMlab as if it were a device running
	// MDMlab Desktop.
	fetchEnrollmentProfileFromDesktop bool
	// desktopURLToken is the MDMlab Desktop token used to fetch the enrollment profile
	// from MDMlab as if it were a device running MDMlab Desktop.
	desktopURLToken string

	// fetchEnrollmentProfileFromDEP indicates whether this simulated device will fetch
	// the enrollment profile from MDMlab as if it were a device running the DEP flow.
	fetchEnrollmentProfileFromDEP bool

	// fetchEnrollmentProfileFromOTA indicates whether this simulated device will fetch
	// the enrollment profile from MDMlab as if it were a device running the OTA flow.
	...
	// from MDMlab as if it were a device running the DEP flow.
	depURLToken string
...
// enrollment profile from MDMlab as if it were a device running MDMlab Desktop.
...
// enrollment profile from MDMlab as if it were a device running the DEP flow.
...
// profile from MDMlab. The enrollment information is to be provided in the enrollInfo.
...
// enrollment profile from MDMlab as if it were a device running the Over The
// Air (OTA) flow.
...
func (c *TestAppleMDMClient) fetchEnrollmentProfileFromDesktopURL() error {
	return c.fetchOTAProfile(
		"/api/latest/mdmlab/device/" + c.desktopURLToken + "/mdm/apple/manual_enrollment_profile",
	)
}
...
func (c *TestAppleMDMClient) fetchEnrollmentProfileFromOTAURL() error {
	return c.fetchOTAProfile(
		"/api/latest/mdmlab/enrollment_profiles/ota?enroll_secret=" + url.QueryEscape(c.otaEnrollSecret),
	)
}

func (c *TestAppleMDMClient) fetchOTAProfile(url string) error {
	request, err := http.NewRequest("GET", c.mdmlabServerURL+url, nil)
	if err != nil {
...
func (c *TestAppleMDMClient) fetchEnrollmentProfile(path string) error {
	request, err := http.NewRequest("GET", c.mdmlabServerURL+path, nil)
	if err != nil {
		return fmt.Errorf("create request: %w", err)
	}
	// #nosec (this client is used for testing only)
...
func (c *TestAppleMDMClient) doSCEP(url, challenge string) (*x509.Certificate, *rsa.PrivateKey, error) {
	...
	// (3). Generate CSR.
	cn := fmt.Sprintf("mdmlab-testdevice-%s", c.UUID)
	csrTemplate := x509util.CertificateRequest{
		CertificateRequest: x509.CertificateRequest{
			Subject: pkix.Name{
				CommonName:   cn,
				Organization: []string{"mdmlab-organization"},
			},
...
func (c *TestAppleMDMClient) DeclarativeManagement(endpoint string, data ...mdmlab.MDMAppleDDMStatusReport) (*http.Response, error) {
	payload := map[string]any{
...
func (c *TestAppleMDMClient) AcknowledgeInstalledApplicationList(udid, cmdUUID string, software []mdmlab.Software) (*mdm.Command, error) {
...
func makeClientSCEPEndpoints(instance string) (*scepserver.Endpoints, error) {
	...
	// #nosec (this client is used for testing only)
	c := mdmlabhttp.NewClient(mdmlabhttp.WithTLSClientConfig(&tls.Config{
		InsecureSkipVerify: true,
	}))
