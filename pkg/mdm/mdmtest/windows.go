...
type TestWindowsMDMClient struct {
	...
	HardwareID string
	// mdmlabServerURL is the URL of the MDMlab server, used to ping the MDM endpoints.
	mdmlabServerURL string
	...
	enrollmentType mdmlab.WindowsMDMEnrollmentType
...
func NewTestMDMClientWindowsProgramatic(serverURL string, orbitNodeKey string, opts ...TestWindowsMDMClientOption) *TestWindowsMDMClient {
	c := TestWindowsMDMClient{
		mdmlabServerURL:  serverURL,
		DeviceID:        uuid.NewString(),
		enrollmentType:  mdmlab.WindowsMDMProgrammaticEnrollmentType,
...
func NewTestMDMClientWindowsAutomatic(serverURL string, email string, opts ...TestWindowsMDMClientOption) *TestWindowsMDMClient {
	c := TestWindowsMDMClient{
		mdmlabServerURL:  serverURL,
		DeviceID:        uuid.NewString(),
		enrollmentType:  mdmlab.WindowsMDMAutomaticEnrollmentType,
...
func (c *TestWindowsMDMClient) StartManagementSession() (map[string]mdmlab.ProtoCmdOperation, error) {
	// Get SessionID
	sessionIDInt := 0
...
<SyncHdr>
	...
	<MsgID>1</MsgID>
	<Target>
	<LocURI>` + c.mdmlabServerURL + microsoft_mdm.MDE2ManagementPath + `</LocURI>
	</Target>
	<Source>
...
func (c *TestWindowsMDMClient) doManagementReq(rawXMLReq []byte) (map[string]mdmlab.ProtoCmdOperation, error) {
	if c.debug {
	...
	c.queuedCommandResponses = make(map[string]mdmlab.SyncMLCmd)
	return cmds, nil
}

func (c *TestWindowsMDMClient) SendResponse() (map[string]mdmlab.ProtoCmdOperation, error) {
	// Get SessionID
	sessionID, err := c.lastManagementResp.GetSessionID()
	if err != nil {
		return nil, fmt.Errorf("session ID processing error %w", err)
	...
	msg.SyncHdr = mdmlab.SyncHdr{
		...
		Source:    &mdmlab.LocURI{LocURI: &c.DeviceID},
		Target: &mdmlab.LocURI{
			LocURI: ptr.String(c.mdmlabServerURL + microsoft_mdm.MDE2ManagementPath),
		},
	}
...
func (c *TestWindowsMDMClient) AppendResponse(op mdmlab.SyncMLCmd) {
...
<s:Envelope
    ...
    <s:Header>
        ...
        <a:To s:mustUnderstand="1">` + c.mdmlabServerURL + microsoft_mdm.MDE2EnrollPath + `</a:To>
...
<s:Envelope
    ...
    <s:Header>
        ...
        <a:To s:mustUnderstand="1">` + c.mdmlabServerURL + microsoft_mdm.MDE2DiscoveryPath + `</a:To>
    </s:Header>
    <s:Body>
        <Discover
...
<s:Envelope
    ...
    <s:Header>
        ...
        <a:To s:mustUnderstand="1">` + c.mdmlabServerURL + microsoft_mdm.MDE2PolicyPath + `</a:To>
        <wsse:Security s:mustUnderstand="1">
            <wsse:BinarySecurityToken ValueType="` + tokenValueType + `" EncodingType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd#base64binary">` + binarySecToken + `</wsse:BinarySecurityToken>
        </wsse:Security>
    </s:Header>
...
func (c *TestWindowsMDMClient) request(path string, reqBody []byte) (*http.Response, error) {
	request, err := http.NewRequest("POST", c.mdmlabServerURL+path, bytes.NewReader(reqBody))
	if err != nil {
...
func (c *TestWindowsMDMClient) getToken() (binarySecToken string, tokenValueType string, err error) {
	...
	case mdmlab.WindowsMDMAutomaticEnrollmentType:
	...
	case mdmlab.WindowsMDMProgrammaticEnrollmentType:
		var err error
		tokenValueType = syncml.BinarySecurityDeviceEnroll
		binarySecToken, err = mdmlab.GetEncodedBinarySecurityToken(c.enrollmentType, c.TokenIdentifier)
		if err != nil {
